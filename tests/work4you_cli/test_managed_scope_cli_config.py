...
work4you_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `work4you config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_work4you_home = get_work4you_home()`` at import time (module
    singleton), so monkeypatching WORK4YOU_HOME after import doesn't move it.
    ...
    this only matters for tests that swap WORK4YOU_HOME.)
    """
    import cli

    cli._work4you_home = home
