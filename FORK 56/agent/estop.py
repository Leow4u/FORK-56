...
``work4you pause`` writes a sentinel file at ``$WORK4YOU_HOME/ESTOP``;
``work4you resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Work4You is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.work4you/ESTOP``.
...
def _work4you_home() -> Path:
    """Resolve the active WORK4YOU_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active WORK4YOU_HOME."""
    return _work4you_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on WORK4YOU_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Work4You is paused ({reason}). New work is on hold; "
            "run `work4you resume` to pick things back up."
        )
    return (
        "⏸️ Work4You is paused. New work is on hold; "
        "run `work4you resume` to pick things back up."
    )
