...
description: "Set up Work4You as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Work4You can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Work4You gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Work4You limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Work4You degrades gracefully if absent.
...
2. **Add** → name (e.g. `work4you-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.work4you/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Work4You to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Work4You gateway has to be reachable from Meta's servers.  Three common ways:
...
## Allowlist (Work4You-side)

In addition to Meta's recipient whitelist, Work4You has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.work4you/.env`:
...
The `work4you whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.work4you/.env`.  Required values are in **bold**.
...
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, OpenAI/Work4You, Groq, etc.), then handed to the agent as text.
...
When the agent invokes any of these flows, Work4You uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Work4You acknowledges inbound messages immediately:
...
Work4You TTS produces MP3. Two paths:
...
Work4You warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Work4You. If you need it, please [open an issue](https://github.com/Leow4u/FORK-56/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Work4You's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Work4You doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.work4you/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `work4you gateway` is up.
- **App Secret not set** — without it, Work4You refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Work4You.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `work4you tools list` and verify the platform is using `work4you-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `work4you_cli/platforms.py` for the platform → default toolset mapping.

### STT (voice note transcription) returns empty / "could not transcribe"

The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Work4You subscriber, you can route STT through Meta's managed audio gateway instead:

```bash
work4you config set stt.provider openai
work4you config set stt.use_gateway true
work4you gateway restart
```

This uses your Work4You Portal access token instead of needing a separate OpenAI key.

---

## Security notes

- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Work4You will accept as authentic.
...
| | Baileys (`work4you whatsapp`) | Cloud API (`work4you whatsapp-cloud`) |
|---|---|---|
...
Most users running Work4You for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
