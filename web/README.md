...
Browser-based dashboard for managing Work4You configuration, API keys, and monitoring active sessions.
...
cd ../
python -m work4you_cli.main web --no-open
...
`work4you dashboard` on port 9119 serves the **built** bundle from `work4you_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../work4you_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from `@work4you-research/ui`'s `globals.css`:
...
- The dashboard preserves the Work4You brand uppercase aesthetic, but it is **opt-in per element, not global**.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `work4you/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Work4You” — mixed case, no Mondwest, no `text-display` |
