...
interface Work4YouConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useWork4YouConfig({ activeSessionIdRef }: Work4YouConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshWork4YouConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        ...
        // rendering/applying Work4You' built-in medium over the user's config.
  ...
  return { refreshWork4YouConfig, sttEnabled, voiceMaxRecordingSeconds }
}
