"""
Work4You Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``work4you desktop`` / ``work4you gui``)
     Built inside the agent checkout under ``$WORK4YOU_HOME/work4you/``:
       ...
       - ``$WORK4YOU_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Work4You.app`` or ``~/Applications/Work4You.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Work4You`` (NSIS per-user)
...
the app name ("Work4You"), separate from ``$WORK4YOU_HOME``:
  - macOS:   ``~/Library/Application Support/Work4You``
  - Windows: ``%APPDATA%\\Work4You``
  - Linux:   ``$XDG_CONFIG_HOME/Work4You`` (default ``~/.config/Work4You``)
...
``work4you uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(work4you_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return work4you_home / "work4you"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Work4You"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Work4You"
...
def source_built_gui_artifacts(work4you_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``work4you desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``work4you/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(work4you_home)
    ...
    return [
        ...
        work4you_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Work4You" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Work4You).
        ...
        # ``uninstall_gui``.
        from work4you_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `work4you desktop` installs. Its icon lives
...
def agent_is_installed(work4you_home: Path) -> bool:
    """Return True when a usable Python agent install exists under WORK4YOU_HOME.
    ...
    if (agent_root / "work4you_cli").is_dir():
        return True
...
def gui_is_installed(work4you_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(work4you_home):
        if p.exists():
            return True
...
def gui_install_summary(work4you_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = work4you_home if work4you_home is not None else get_work4you_home()
...
def uninstall_gui(work4you_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``work4you/work4you_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$WORK4YOU_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Work4You disappears from the launcher.
        try:
            from work4you_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove work4you' or "
            "'sudo dnf remove work4you'). AppImage builds are a single file you "
