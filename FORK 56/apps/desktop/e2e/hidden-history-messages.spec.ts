...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.work4youHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.work4youHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.work4youHome)
  const builder = await RealSessionBuilder.start(sandbox.work4youHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.work4youHome, mock.url)
  fs.appendFileSync(path.join(sandbox.work4youHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.work4youHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
