...
#
# When a user invokes a /skill (or /bundle), Work4You expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_work4you_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``WORK4YOU_PLATFORM`` env var and
    ``WORK4YOU_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("WORK4YOU_PLATFORM")
            or get_session_env("WORK4YOU_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("WORK4YOU_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Work4You home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_work4you_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.work4you.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_work4you_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.work4you/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Work4You slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Work4You command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Work4You home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.work4you/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``work4you -s <skill>`` or
    a deployment's ``WORK4YOU_TUI_SKILLS`` env var could force-load a skill an
