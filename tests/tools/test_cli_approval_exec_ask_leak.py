...
When ``WORK4YOU_EXEC_ASK`` leaks into a classic CLI process (historically via
...
class TestExecuteCodeGuardCliApprovalSurvivesExecAskLeak:
    ...
    def test_cli_callback_used_for_platform_marker_leak_without_exec_ask(
        ...
        ``WORK4YOU_SESSION_PLATFORM`` is set, so the whole-script gate is
        reached with ``WORK4YOU_EXEC_ASK`` entirely absent. That path must
...
print("EXEC_ASK=" + repr(os.environ.get("WORK4YOU_EXEC_ASK")))
""" % (str(REPO_ROOT),)
        work4you_home = tmp_path / "import-test-home"
        proc = subprocess.run(
