...
from work4you_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from work4you_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from work4you_state_portability import SessionPortabilityMixin
from work4you_state_schema import SessionSchemaMixin
from work4you_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``work4you -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_work4you_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.work4you`` even when a test fixture later
    redirects ``WORK4YOU_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_work4you_home()`` fresh so a runtime
       ``WORK4YOU_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_work4you_home() / "state.db"
...
# ~/.work4you/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects WORK4YOU_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without WORK4YOU_HOME, a stale worktree
# without the re-pin, or a developer shell that exports WORK4YOU_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "WORK4YOU_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.work4you``).  The test conftest injects the pre-sandbox production
#: root here so custom-``WORK4YOU_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Work4You root for the guard.

    Deliberately avoids ``Path.home()`` / ``work4you_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``work4you_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "work4you"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "work4you"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".work4you"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``WORK4YOU_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Work4You test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``WORK4YOU_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "WORK4YOU_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``WORK4YOU_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``work4you`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Work4You home *root*.
    ...
    under ``~/.work4you/work4you/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``WORK4YOU_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``WORK4YOU_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Work4You root {root}). "
                "Tests must run against a temporary WORK4YOU_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "WORK4YOU_HOME. If this test genuinely needs the live "
...
# work4you_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``work4you sessions optimize`` (FTS merge + VACUUM)
    ...
    ``work4you_cli/kanban_db.py`` already bounds its WAL growth with
...
def _enforce_macos_synchrowork4you_full(conn: sqlite3.Connection) -> None:
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``work4you_cli.kanban_db.connect`` so
    ...
    if current_mode == "wal":
        ...
        _enforce_macos_synchrowork4you_full(conn)
        return "wal"
...
def _wal_reset_repair_hint() -> str:
    ...
    ``work4you update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Work4You-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Work4You"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Work4You-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `work4you doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see work4you_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with work4you_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``work4you doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Work4You host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``work4you serve`` backend, interactive CLI sessions,
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``work4you_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Work4You home tree (``get_work4you_home()``) — the safe repair scope:
      Work4You owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``work4you_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_work4you_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Work4You needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (work4you_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_work4you_state.py:600-632)
        ...
        probe_session_id = f"_work4you_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.work4you/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``work4you sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("WORK4YOU_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_work4you_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("WORK4YOU_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship work4you_state without work4you_cli). A
    ...
    try:
        ...
        logger.debug(
            "work4you_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch work4you_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``work4you_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `work4you snapshot list` / "
                "`work4you snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (work4you doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple work4you processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Work4You
    ...
    # `work4you update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `work4you snapshot list` / "
                    f"`work4you snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`work4you db optimize`),
            ...
            # ``work4you_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `work4you update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `work4you sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Work4You process held the "
    ...
    #
    # `optimize_fts_storage()` (the `work4you sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`work4you sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``work4you sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``work4you --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Work4You) receive
        ...
        if repair_alternation and messages:
            # Lazy import: work4you_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Work4You Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``work4you -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``work4you sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``WORK4YOU_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Work4You, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Work4You session.

        A Work4You session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Work4You session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `work4you sessions vacuum` runs in a transient
