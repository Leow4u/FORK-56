...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const cents = usdToPresentmentCents(amount, presentment)
    ...
    if (
      message.includes('STRIPE_USD_TO_BRL') ||
      message.startsWith('unsupported_presentment_currency') ||
      message === 'invalid_usd_amount'
    ) {
      return NextResponse.json(
        { error: 'presentment_fx_missing', message },
        { status: 503 },
      )
    }
