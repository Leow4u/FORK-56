...
Module-level run.py helpers a handler needs (``_work4you_home``,
...
from work4you_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Work4You commands.
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _work4you_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _work4you_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _work4you_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _work4you_home, _load_gateway_config
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _work4you_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from work4you_cli.route_identity import should_clear_context_pin_async
                        ...
                        from work4you_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Work4You default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        All resolution/persistence goes through work4you_cli.personality —
        ...
        # Persist the selection only — work4you_cli.personality never writes
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from work4you_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `work4you cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from work4you_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Work4You itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _work4you_home
        from work4you_cli.config import read_user_config_raw
        config_path = _work4you_home / "config.yaml"
        try:
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.work4you/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``work4you skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _work4you_home
        from work4you_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `work4you skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.work4you/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from work4you_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _work4you_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _work4you_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        temp_dir = tempfile.mkdtemp(prefix="work4you_save_")
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (work4you_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Work4You balance and hand off to the portal.
        ...
        lines: list[str] = ["💳 **Work4You balance**"]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Work4You credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via work4you_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a Work4You account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Work4You-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            from agent.account_usage import work4you_credits_lines

            credits_lines = await asyncio.to_thread(work4you_credits_lines, markdown=True)
        except Exception:
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``work4you debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``work4you update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``work4you update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _work4you_home, _resolve_work4you_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        work4you_cmd = _resolve_work4you_bin()
        if not work4you_cmd:
            return t("gateway.update.work4you_cmd_not_found")

        pending_path = _work4you_home / ".update_pending.json"
        output_path = _work4you_home / ".update_output.txt"
        exit_code_path = _work4you_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `work4you update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `work4you update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through work4you_cmd (venv\Scripts\work4you.exe): the shim
                ...
                work4you_cmd_str = " ".join(shlex.quote(part) for part in work4you_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {work4you_cmd_str} update --gateway"
