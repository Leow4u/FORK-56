...
If you only run one Work4You agent, you don't need this page — see
...
[Connecting Desktop to Many Work4You Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Work4You agents that should all
...
(`ai.work4you.gateway-<name>.plist`) or systemd user service
(`work4you-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
work4you profile create coder
work4you profile create personal-bot
work4you profile create research
...
```bash
work4you config set gateway.multiplex_profiles true
work4you gateway restart
```

Equivalently, in the default profile's `~/.work4you/config.yaml`:
...
You do **not** run `work4you gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `work4you gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.work4you/profiles/coder/.env`; the default listener key is rejected.
...
home). `work4you status` reports the multiplexer and the profiles it serves;
`work4you status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/work4you-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: work4you-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    work4you gateway "$action"
  else
    work4you -p "$profile" gateway "$action"
  fi
}
...
```bash
work4you-gateways start      # start every configured profile
work4you-gateways stop       # stop every configured profile
work4you-gateways restart    # restart all
work4you-gateways status     # status across all
work4you-gateways list       # delegates to `work4you gateway list`
```

:::tip
The `default` profile is targeted with `work4you gateway <action>` (no `-p`),
not `work4you -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `work4you -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.work4you.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/work4you-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.work4you.gateway.plist` /
`work4you-gateway.service`.
...
```bash
# Default profile
tail -f ~/.work4you/logs/gateway.log
tail -f ~/.work4you/logs/gateway.error.log

# Named profile
tail -f ~/.work4you/profiles/<name>/logs/gateway.log
tail -f ~/.work4you/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.work4you/logs/gateway.log ~/.work4you/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
work4you logs -f                  # follow default profile
work4you -p coder logs -f         # follow one profile
work4you logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
work4you profile list             # profiles + model + gateway state
work4you-gateways status          # full status across every profile
launchctl list | grep work4you    # macOS — PIDs and labels
systemctl --user list-units 'work4you-gateway-*'   # Linux — units
```
...
```
~/.work4you/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.work4you/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
work4you config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.work4you/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=work4you --why="gateways running" \
  sleep infinity &
...
`work4you-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.work4you/.env ~/.work4you/profiles/*/.env
```

## Updating the code

`work4you update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `work4you gateway start` after a previous `work4you gateway stop`. The
...
```bash
ps -ef | grep "work4you_cli.*-p <profile>"
cat ~/.work4you/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.work4you.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.work4you.gateway-<profile>.plist

# Linux
systemctl --user restart work4you-gateway-<profile>.service
```

### Health check

```bash
work4you doctor                  # default profile
work4you -p <profile> doctor     # one profile
```
