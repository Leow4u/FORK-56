...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    assert env["WORK4YOU_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_work4you_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see WORK4YOU_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('WORK4YOU_HOME'), "
    ...
    assert result["home"] == str(work4you_home)
