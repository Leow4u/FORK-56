"""Cross-process mutual exclusion for in-flight Work4You updates.

Three different surfaces can start an update of the same install tree:

* ``work4you update`` from a terminal,
* the dashboard's Update button (``POST /api/work4you/update`` →
  ``_spawn_work4you_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``work4you-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``work4you update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <WORK4YOU_HOME>/.work4you-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``work4you update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Work4You is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``work4you-setup`` binary under ``~/.work4you`` is only refreshed by a full
  installer run (``copy_self_to_work4you_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Work4You is still running") on every
  GUI update forever, with no Work4You process actually running.
"""
...
MARKER_NAME = ".work4you-update-in-progress"

# Set by an orchestrating updater (the Tauri `work4you-setup --update` flow) to
# its own pid before spawning `work4you update` as a child stage. The parent
...
HANDOFF_PID_ENV = "WORK4YOU_UPDATE_HANDOFF_PID"
...
# to show "Work4You is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Work4You home (never the context-local profile override):
    the Rust updater resolves ``$WORK4YOU_HOME`` or the platform default, and the
    ...
    return get_process_work4you_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``work4you update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``work4you-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Work4You update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `work4you update` as a stage): we run under ITS claim
