...
sidebar_position: 3
title: "Desktop App"
description: "The native Work4You desktop app — a polished experience for chatting with Work4You, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Work4You desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Work4You core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `work4you` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Work4You has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`work4you`) and **[TUI](./tui.md)** (`work4you --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`work4you dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Work4You Desktop](../getting-started/installation.md).

If you already have Work4You installed, simply run
...
- **The same conversation history** as every other Work4You surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Work4You instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Work4You Desktop to a remote backend](./features/web-dashboard.md#connecting-work4you-desktop-to-a-remote-backend).

#### Repository discovery

Work4You Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Work4You sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `work4you desktop --cwd <path>` (or the `WORK4YOU_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Work4You to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `work4you model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Work4You management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#work4you-cron).
- **Profiles** — switch between [Work4You profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [Work4You profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Work4You cron). The roster lives in the left
...
each other's Bot Chats directly (`work4you -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Work4You, and more.
...
You can do the same from the terminal — `work4you uninstall --gui` for the GUI alone, or `work4you uninstall` / `work4you uninstall --full` for the agent too.

:::note
Running `work4you uninstall --gui` from a **source checkout** (a `work4you desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `work4you desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `work4you desktop`

To launch via the CLI, simply run `work4you desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `work4you update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `WORK4YOU_DESKTOP_CWD`)           |
| `--work4you-root PATH` | Override the Work4You source root the app uses (sets `WORK4YOU_DESKTOP_WORK4YOU_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `work4you` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Work4You runtime into `WORK4YOU_HOME` (`~/.work4you`, or `%LOCALAPPDATA%\work4you` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `WORK4YOU_DESKTOP_WORK4YOU_ROOT`, then a completed managed install, then a probed `work4you` on `PATH` (unless `--ignore-existing` / `WORK4YOU_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `WORK4YOU_DESKTOP_WORK4YOU` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `work4you serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `work4you --tui`. The desktop app is **self-contained**: it runs its own `work4you serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.
...
"Remote backend" means a **`work4you serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `work4you serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
- **OAuth (Work4You Portal) — preferred for anything reachable beyond your own machine.** Logins are verified against your Work4You account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `work4you dashboard register` (or the Portal [`/local-dashboards`](https://portal.work4you.ai/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with Work4You**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Work4You](./features/web-dashboard.md#default-provider-work4you-research).

### On the backend (the remote machine)

Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.work4you/.env` (the secrets file, mode 0600):
...
work4you serve --host 0.0.0.0 --port 9119
```

Keep that `work4you serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `work4you serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.work4you/.env` so the credentials are in the environment at boot.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.work4you/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Work4You Desktop to a remote backend](./features/web-dashboard.md#connecting-work4you-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Work4You Desktop](../reference/environment-variables.md#web-dashboard--work4you-desktop).
...
`$WORK4YOU_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Boot logs land in `WORK4YOU_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.work4you/work4you/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
The Windows installer people download (`Work4You-Setup.exe`) is the electron-builder NSIS pack (`npm run dist:win:nsis`), Authenticode-signed in CI with Work4You's SSL.com eSigner certificate. The macOS installer (`Work4You.dmg`) is the electron-builder DMG (`npm run dist:mac:dmg`), Developer ID-signed and notarized in CI (`CSC_P12_BASE64` / `CSC_KEY_PASSWORD` / `APPLE_*`). Windows-exported OpenSSL 3 `.p12` files do not need `openssl pkcs12 -export -legacy`; CI re-exports them to PBESv1 SHA1+3DES, imports them into a temporary keychain, and passes `CSC_NAME` (not `CSC_LINK`) to electron-builder. `mac.notarize` is `false` so the built-in `@electron/notarize` path is not used. CI skips afterSign notarization and notarizes the public `Work4You.dmg` (not a Windows `.exe`) in a later step: `notarytool submit` (no wait), then `notarytool wait` up to 60 minutes. Apple's queue often exceeds 20 minutes for a large Electron DMG. A manual **Run workflow** on `Release Desktop Installer` with an empty `tag` bumps the public `desktop-v*` GitHub Latest (the URL `work4you.ai/downloads` redirects to). `tag=latest` rebuilds that Latest in place. A run that skips Windows or macOS signing does not publish that unsigned binary to GitHub Releases. Local `npm run dist:win` packs are unsigned development artifacts. Local `npm run dist:mac` signs/notarizes only when `CSC_LINK` and `APPLE_*` are already in the environment.

**Update now** in a packaged Windows or macOS app downloads that same signed installer and runs it (silent NSIS on Windows; opens the DMG on Mac). It does **not** run `work4you update` (git pull + dependency rebuild), which is what source and CLI installs still use and can take many minutes. The installer replaces the desktop app; the local agent checkout under `WORK4YOU_HOME` is unchanged. On Windows the Desktop quits so files unlock, then the installer relaunches Work4You when setup finishes (`--force-run`, plus a handoff that starts the new `.exe` if silent NSIS skipped the finish-page launch).
...
certificate once and tell Work4You to use it:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Work4You Local Signing`, Identity Type: *Self-Signed Root*,
   Certificate Type: **Code Signing**.
3. `work4you config set desktop.macos_signing_identity "Work4You Local Signing"`
...
`tccutil reset All com.work4you.work4you` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `work4you --tui` and the dashboard chat tab
