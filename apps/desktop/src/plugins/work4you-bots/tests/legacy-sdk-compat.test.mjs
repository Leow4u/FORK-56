...
function sdkNamedImports(source) {
  const match = source.match(/import\s+\{([\s\S]*?)\}\s+from '@work4you\/plugin-sdk'/)
...
function writeLegacySdk(root) {
  const packageRoot = join(root, 'node_modules', '@work4you', 'plugin-sdk')
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@work4you/plugin-sdk', type: 'module', exports: './index.js' }, null, 2)}\n`
  )
...
test('legacy SDK without optional capability exports still links Bot Mode', async t => {
  const root = mkdtempSync(join(tmpdir(), 'work4you-bot-mode-legacy-sdk-'))
  ...
  assert.equal(loaded.default.id, 'work4you-bots')
