...
def _summary_output(capsys, provider_ready: bool):
    ...
    with patch("work4you_cli.auth.resolve_provider", resolver), \
         patch.object(setup_mod, "get_work4you_subscription_features") as feats:
...
def test_summary_warns_when_no_provider(capsys):
    ...
    assert "work4you model" in out
    assert "work4you setup --portal" in out
