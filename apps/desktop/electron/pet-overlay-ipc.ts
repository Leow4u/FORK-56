...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('work4you:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('work4you:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('work4you:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('work4you:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('work4you:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('work4you:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('work4you:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('work4you:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('work4you:pet-overlay:control', payload)
  })
}
