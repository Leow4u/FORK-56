"""langfuse — Work4You plugin for Langfuse observability.

Traces Work4You conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Work4You plugin system — standalone plugins only
...
Required env vars (set via ``work4you tools`` or ~/.work4you/.env):
  WORK4YOU_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  WORK4YOU_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  WORK4YOU_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  WORK4YOU_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  WORK4YOU_LANGFUSE_RELEASE     - release/version tag
  WORK4YOU_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  WORK4YOU_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  WORK4YOU_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
  ...
  WORK4YOU_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("WORK4YOU_LANGFUSE_DEBUG")
...
def _capture_mode() -> str:
    ...
    value = _env("WORK4YOU_LANGFUSE_CAPTURE").lower()
    if not value:
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid WORK4YOU_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Work4You plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `work4you tools` and configure Langfuse "
                "Observability to reinstall it."
            )
            _LANGFUSE_CLIENT = _INIT_FAILED
            return None

        public_key = _env("WORK4YOU_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
        secret_key = _env("WORK4YOU_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
        ...
        placeholder_issues = [
            msg
            for msg in (
                _validate_langfuse_key("WORK4YOU_LANGFUSE_PUBLIC_KEY", public_key),
                _validate_langfuse_key("WORK4YOU_LANGFUSE_SECRET_KEY", secret_key),
            )
            if msg
        ]
        if placeholder_issues:
            logger.warning(
                ...
                "or unset WORK4YOU_LANGFUSE_PUBLIC_KEY / WORK4YOU_LANGFUSE_SECRET_KEY to "
        ...
        base_url = _env("WORK4YOU_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
        environment = _env("WORK4YOU_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
        release = _env("WORK4YOU_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
        sample_rate = _env("WORK4YOU_LANGFUSE_SAMPLE_RATE")
        ...
        # short-lived-process gap (kanban workers / work4you chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older Work4You paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("WORK4YOU_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Work4You usage into Langfuse usage and cost maps."""
    ...
    # Work4You estimate rather than summing components because it also includes
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Work4You turn",
                tags=["work4you", "langfuse"],
            ):
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``work4you chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Work4You branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Work4You also has a turn-scoped
    ...
    # messages is a list only for legacy Work4You branches that fired
    # pre_llm_call with API messages directly. Current Work4You fires
...
def register(ctx) -> None:
    ...
    # Work4You versions.  pre_api_request / post_api_request fire per API
