"""``work4you pause`` / ``work4you resume`` — the global emergency stop.

``work4you pause`` writes the ESTOP sentinel at ``$WORK4YOU_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``work4you resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Work4You paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `work4you resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Work4You resumed — dispatch picks up on the next tick.")
    else:
        print(f"Work4You is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`work4you resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `work4you pause`",
