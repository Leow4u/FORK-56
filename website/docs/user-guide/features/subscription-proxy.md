...
description: "Use your Work4You Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Work4You-managed provider subscription as their
...
| Use case | "Use Work4You as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Work4You Portal OAuth flow. Work4You stores
the refresh token in `~/.work4you/auth.json` — the same place all Work4You
...
Model:      Work4You-4-70B    # or Work4You-4.3-36B, Work4You-4-405B
```
...
Currently shipped: `work4you` (Work4You Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `work4you_cli/proxy/adapters/`.
...
If you see `not logged in`, run `work4you portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Work4You
Portal:
...
supports; check `portal.work4you.ai/models`.
...
[portal.work4you.ai](https://portal.work4you.ai).
...
`work4you_cli/proxy/adapters/<provider>.py` and registering it in
