...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `work4you tools`.
...
Work4You generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `work4you tools` and persists in `config.yaml`.
...
If you have a paid [Work4You Portal](https://portal.work4you.ai) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `work4you setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Work4You Subscription** as the image-gen backend via `work4you tools`.
...
Navigate to **🎨 Image Generation**, pick your backend (Work4You Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
`max_parallel_requests` defaults to `4`. Work4You clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Work4You
...
Work4You Portal proxies the chat-completions protocol only, so its picker offers
...
as a tool the chat model may call, and Work4You cannot force the call — the
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / Work4You Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Work4You materializes them to the local cache so delivery still works after expiry
