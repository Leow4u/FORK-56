"""Regression tests for TOCTOU-safe credential file writers in ``work4you_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.work4you/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Work4You shared-credential store write (inside _write_shared_work4you_state)
# ---------------------------------------------------------------------------


def test_shared_work4you_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Work4You shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
    # _work4you_shared_store_path() refuses to touch the real shared store during
    ...
    # home" reference — which currently tracks WORK4YOU_HOME via
    # get_default_work4you_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("WORK4YOU_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from work4you_cli import auth as auth_mod

        state = {
            "access_token": "work4you-access-xxx",
            "refresh_token": "work4you-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_work4you_state(state)
        path = auth_mod._work4you_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared Work4You store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Work4You shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Work4You shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )

    data = json.loads(path.read_text())
    assert data["refresh_token"] == "work4you-refresh-xxx"
