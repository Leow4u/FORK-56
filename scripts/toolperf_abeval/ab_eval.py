...
measured in the production session DB) through `work4you chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    WORK4YOU_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Work4You home with credentials for the
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "WORK4YOU_NEMO_RELAY_ATOF_ENABLED": "1",
                "WORK4YOU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "WORK4YOU_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "WORK4YOU_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "work4you_cli.main", "chat", "--query", q,
