"""``work4you doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from work4you_cli.doctor import WORK4YOU_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (WORK4YOU_HOME / "node" / "bin",
                          WORK4YOU_HOME / "node",
                          WORK4YOU_HOME / "node_modules" / ".bin"):
    ...
    # work4you_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``work4you mcp test``.
    """
