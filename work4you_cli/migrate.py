"""CLI handlers for ``work4you migrate ...``.

Currently exposes only ``work4you migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``work4you migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: work4you migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `work4you migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `work4you doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
