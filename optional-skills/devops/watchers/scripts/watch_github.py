...
Usage (via cron with --no-agent):
    ...
    work4you cron create work4you-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$WORK4YOU_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name work4you-issues --repo Leow4u/FORK-56 --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Work4You .env file
(``${WORK4YOU_HOME:-~/.work4you}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Work4You-Watcher/1.0",
    }
