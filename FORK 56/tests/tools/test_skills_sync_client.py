...
the sync wire contract):
  ...
  * the access gate (Work4You admin) making sync inert
...
# ---------------------------------------------------------------------------
# Access gate (Work4You admin) + per-skill opt-in
# ---------------------------------------------------------------------------
...
class TestDevGate:
    def test_gate_open_with_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ssc, "resolve_work4you_runtime_credentials",
        ...
        monkeypatch.setattr(auth_mod, "resolve_work4you_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["work4you_admin"] is True
    ...
    def test_gate_closed_without_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_work4you_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["work4you_admin"] is False
    ...
    def test_maybe_pull_inert_when_not_logged_in(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_work4you_runtime_credentials", _raise)
...
def synced_env(tmp_path, monkeypatch):
    """A WORK4YOU_HOME with two opted-in skills + a token-carrying identity."""
    import work4you_constants
    home = tmp_path / "work4you"
    skills = home / "skills"
    skills.mkdir(parents=True)
    monkeypatch.setattr(work4you_constants, "get_work4you_home", lambda: home)
...
class TestEndToEnd:
    ...
    def test_push_then_pull_materializes(self, mock_server, synced_env, tmp_path, monkeypatch):
        ...
        dev2 = tmp_path / "work4you2" / "skills"
...
# ---------------------------------------------------------------------------
# Env-var configuration (Work4You Cloud "on by default" via environment)
# ---------------------------------------------------------------------------

class TestEnvConfig:
    def test_base_url_env_wins(self, monkeypatch):
        monkeypatch.setenv("WORK4YOU_SYNC_BASE_URL", "https://plane.example/")
    ...
    def test_base_url_defaults_to_production(self, monkeypatch):
        ...
        monkeypatch.delenv("WORK4YOU_SYNC_BASE_URL", raising=False)
        monkeypatch.setattr("work4you_cli.config.load_config", lambda: {}, raising=False)
    ...
    def test_feature_enabled_env(self, monkeypatch):
        # Default off.
        monkeypatch.delenv("WORK4YOU_SYNC_ENABLED", raising=False)
        monkeypatch.setattr("work4you_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_feature_enabled() is False
        for truthy in ("1", "true", "YES", "on"):
            monkeypatch.setenv("WORK4YOU_SYNC_ENABLED", truthy)
            assert ssc.sync_feature_enabled() is True
        for falsy in ("0", "false", "off"):
            monkeypatch.setenv("WORK4YOU_SYNC_ENABLED", falsy)
            assert ssc.sync_feature_enabled() is False

    def test_default_opt_in_env(self, monkeypatch):
        monkeypatch.delenv("WORK4YOU_SYNC_DEFAULT_OPT_IN", raising=False)
        monkeypatch.setattr("work4you_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_default_opt_in() is False
        monkeypatch.setenv("WORK4YOU_SYNC_DEFAULT_OPT_IN", "true")
    ...
    def test_env_overrides_config_yaml(self, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_SYNC_ENABLED", "false")
        monkeypatch.setattr(
            "work4you_cli.config.load_config",
            lambda: {"sync": {"enabled": True}},
            raising=False,
...
class TestDeviceName:
    def test_default_is_hostname_seeded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("WORK4YOU_SYNC_DEVICE_NAME", raising=False)
        monkeypatch.setattr(
    ...
    def test_existing_file_wins_over_default_and_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_SYNC_DEVICE_NAME", "cloud-seed")
        assert ssc.stable_device_id() == "Explicit Name"

    def test_env_seeds_first_use(self, tmp_path, monkeypatch):
        # Work4You Cloud path: WORK4YOU_SYNC_DEVICE_NAME seeds the first-use label.
        monkeypatch.setattr(ssc, "_skills_dir", lambda: tmp_path)
        monkeypatch.setenv("WORK4YOU_SYNC_DEVICE_NAME", "work4you-cloud-ben-1")
        assert ssc.stable_device_id() == "work4you-cloud-ben-1"
        # persisted so it stays stable even if the env later changes
        assert (tmp_path / ".sync_device_id").read_text() == "work4you-cloud-ben-1"
        monkeypatch.setenv("WORK4YOU_SYNC_DEVICE_NAME", "changed")
        assert ssc.stable_device_id() == "work4you-cloud-ben-1"
...
def _org_identity(role=None, org_id="org-1", owner="owner1"):
    ...
    return {"api_key": token, "base_url": "http://x", "owner": owner,
            "work4you_admin": True, "claims": claims,
