...
- Bare `work4you setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `work4you setup --quick` runs the narrower "fill in missing items" flow.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("work4you_cli.setup.ensure_work4you_home", {}),
        ("work4you_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("work4you_cli.config.is_managed", {"return_value": False}),
        ("work4you_cli.setup.load_config", {"return_value": {}}),
        ("work4you_cli.setup.save_config", {}),
        ("work4you_cli.setup.get_env_value", {"return_value": None}),
        ("work4you_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("work4you_cli.setup._print_setup_summary", {}),
        ("work4you_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("work4you_cli.setup.load_config", {"return_value": {}}),
        ("work4you_cli.setup.save_config", {}),
        ("work4you_cli.auth.get_active_provider", {"return_value": None}),
        ("work4you_cli.setup.get_env_value", {"return_value": None}),
        ("work4you_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `work4you setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="work4you_cli.setup.prompt_choice",
                quick="work4you_cli.setup._run_quick_setup",
                model="work4you_cli.setup.setup_model_provider",
                terminal="work4you_cli.setup.setup_terminal_backend",
                agent="work4you_cli.setup.setup_agent_settings",
                gateway="work4you_cli.setup.setup_gateway",
                tools="work4you_cli.setup.setup_tools",
            )
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "work4you_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["work4you", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
