...
const USER_DATA_OVERRIDE = process.env.WORK4YOU_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.WORK4YOU_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.WORK4YOU_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with WORK4YOU_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[work4you] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[work4you] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. WORK4YOU_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[work4you] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[work4you] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via WORK4YOU_DESKTOP_PASSWORD_STORE, bridged by the
// `work4you desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[work4you] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[work4you] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `work4you desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Work4You sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[work4you] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[work4you] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[work4you] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[work4you] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[work4you] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('work4you:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Work4You burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[work4you] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[work4you] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[work4you] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[work4you] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// WORK4YOU_HOME — the user-facing root for everything Work4You-related. Mirrors
// scripts/install.ps1's $Work4YouHome and scripts/install.sh's $WORK4YOU_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\work4you (matches install.ps1)
//   macOS / Linux: ~/.work4you (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.work4you directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\work4you yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// WORK4YOU_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// WORK4YOU_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.work4you / %LOCALAPPDATA%\work4you.
function resolveWork4YouHome() {
  if (process.env.WORK4YOU_HOME) {
    return normalizeWork4YouHomeRoot(process.env.WORK4YOU_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'work4you-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a WORK4YOU_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\work4you and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('WORK4YOU_HOME')

    if (fromRegistry) {
      return normalizeWork4YouHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'work4you')
    const legacy = path.join(app.getPath('home'), '.work4you')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.work4you setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.work4you')
}

const WORK4YOU_HOME = resolveWork4YouHome()

function pathWithWork4YouManagedNode(...entries) {
  const managed = work4youManagedNodePathEntries(WORK4YOU_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_WORK4YOU_ROOT — the canonical mutable Work4You install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_WORK4YOU_ROOT = path.join(WORK4YOU_HOME, 'work4you')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_WORK4YOU_ROOT, 'venv')
...
// boots, going straight to `resolveWork4YouBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_WORK4YOU_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_WORK4YOU_ROOT, '.work4you-bootstrap-complete')
...
// local backend as. When set, startWork4You() passes `work4you --profile <name>
// dashboard …`, which deterministically pins WORK4YOU_HOME (see
// _apply_profile_override in work4you_cli/main.py) and bypasses the sticky
// ~/.work4you/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors work4you_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// work4youDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under WORK4YOU_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by work4you_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(WORK4YOU_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (work4you_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.WORK4YOU_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.WORK4YOU_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.WORK4YOU_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.WORK4YOU_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.WORK4YOU_DESKTOP_APP_NAME || 'Work4You'
...
// 'work4you:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.work4you.work4you) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.work4you.work4you')
}

// Seed the native About panel with the live Work4You version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `work4you update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveWork4YouVersion(),
  copyright: 'Copyright © 2026 Work4You'
})
...
// startWork4You(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.WORK4YOU_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.WORK4YOU_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startWork4You() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning work4you serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Work4You backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  work4youLog.push(...lines)

  if (work4youLog.length > 300) {
    work4youLog.splice(0, work4youLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-work4you-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('work4you:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the work4you:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('work4you:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_WORK4YOU_ROOT
  })
}
...
// The Tauri updater writes WORK4YOU_HOME/.work4you-update-in-progress for the whole
...
// (`force_kill_other_work4you`, taskkill /IM work4you.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(WORK4YOU_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Work4You will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(WORK4YOU_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Work4You update did not finish',
        `${result.message}\n\nDetails: ${path.join(WORK4YOU_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `work4you` — must not shadow `work4you.cmd`/`work4you.exe`.
...
function unwrapWindowsVenvWork4YouCommand(command, backendArgs) {
  return resolveVenvWork4YouCommand(command, backendArgs, {
...
// spawns `work4you serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `work4you` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'work4you_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `work4you --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, WORK4YOU_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveWork4YouBackend step 4); execFileSync of a .cmd without
...
function isWork4YouSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'work4you_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.WORK4YOU_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Work4You deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the work4you-import
...
// findGitBash — locate bash.exe on Windows. Resolves WORK4YOU_GIT_BASH_PATH
...
// normal WORK4YOU_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\work4you\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'work4you', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'work4you', 'git', 'bin', 'git.exe'))
  }
...
function recentWork4YouLog() {
  return work4youLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's work4you root) ──────
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    isWork4YouSourceRoot(ACTIVE_WORK4YOU_ROOT) ? ACTIVE_WORK4YOU_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_WORK4YOU_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('work4you:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `work4you update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(WORK4YOU_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `work4you update` can write
// fresh entry points. On Windows this is the file the running backend
// `work4you.exe` holds open; on POSIX it's never mandatory-locked.
function venvWork4YouShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'work4you.exe')
    : path.join(updateRoot, 'venv', 'bin', 'work4you')
}
...
// only signals the direct child, so on Windows a backend `work4you.exe` that
// spawned its own grandchildren (a `work4you` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.work4youBackendIdentity = identity

    return identity
  } catch (error) {
    stopBackendChild(child)
    await waitForBackendExit(child)
    throw new Error(`Could not persist ownership for the Work4You backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.work4youBackendIdentity
...
// `work4youProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `work4you.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a work4you
// REPL, a pty terminal, the gateway) keep `work4you.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const work4youProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(work4youProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvWork4YouShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentWork4YouProcess = backendConnectionState.getProcess()

    if (currentWork4YouProcess && Number.isInteger(currentWork4YouProcess.pid)) {
      stragglers.push(currentWork4YouProcess.pid)
    }
...
async function applyPackagedInstallerUpdates() {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'error',
      message: `Could not download the installer: ${message}`,
      percent: null
    })
  ...
  if (!handoffOutcome.ok) {
    const message = IS_WINDOWS
      ? `Installer failed to start: ${handoffOutcome.message}. Work4You will keep running — try again.`
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `work4you update` → relaunch),
      ...
      // in: the WORK4YOU_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `work4you update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `work4you desktop`, never the Tauri installer that self-copies
      // work4you-setup.exe into WORK4YOU_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `work4you` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `work4you update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `work4you update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, work4youRoot: updateRoot }
      }
    ...
    preflightStateDb(WORK4YOU_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // work4you.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running work4you, an unkillable child). Handing off anyway
      ...
      const message =
        ...
        '(a second Work4You window or a terminal running work4you?). Close it and retry.'
    ...
    // Work4You processes running from this venv.  The updater normally refuses
    ...
    // `work4you update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `work4you update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          WORK4YOU_HOME,
          PATH: pathWithWork4YouManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `work4you update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(WORK4YOU_HOME, child.pid)
      }
      ...
      // abort with "Another Work4You update is already running (PID <itself>)" —
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Work4You will keep running — try again, or run \`work4you update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvWork4You = path.join(venvBin, IS_WINDOWS ? 'work4you.exe' : 'work4you')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Work4You launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the work4you
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.work4you-bootstrap-complete')),
      hasVenvWork4You: fileExists(venvWork4You),
...
// Python-level pre-update snapshot inside `work4you update`.
function preflightStateDb(work4youHome, rememberLog) {
  const stateDbPath = path.join(work4youHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(work4youHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(work4youHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(work4youHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `work4you update`, swaps/relaunches the app bundle, and writes
// .work4you-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'work4you update', percent: null })

    return { ok: true, manual: true, command: 'work4you update', work4youRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...process.env,
      WORK4YOU_HOME,
      PATH: pathWithWork4YouManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Work4You — this window will close. Don’t reopen Work4You yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_WORK4YOU_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isWork4YouSourceRoot(ACTIVE_WORK4YOU_ROOT) &&
    fileExists(venvPython) &&
    canImportWork4YouCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_WORK4YOU_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function resolveWebDist() {
  const override = process.env.WORK4YOU_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set WORK4YOU_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: work4you desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: work4you desktop --force-build`
  )
...
function resolveWork4YouCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Work4You.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.WORK4YOU_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveWork4YouCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'work4you_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_WORK4YOU_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Work4You at ${ACTIVE_WORK4YOU_ROOT}`,
    command,
    args: ['-m', 'work4you_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      work4youHome: WORK4YOU_HOME,
      pythonPathEntries: [ACTIVE_WORK4YOU_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_WORK4YOU_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveWork4YouBackend(backendArgs) {
  ...
  //    installed `work4you` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isWork4YouSourceRoot.)
  if (!IS_PACKAGED && isWork4YouSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Work4You source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_WORK4YOU_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\work4you\\work4you (Windows) or ~/.work4you/work4you.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Work4You runtime at ${ACTIVE_WORK4YOU_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `work4you` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    WORK4YOU_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.WORK4YOU_DESKTOP_IGNORE_EXISTING !== '1') {
    let work4youCommand = null
    const work4youOverride = process.env.WORK4YOU_DESKTOP_WORK4YOU

    if (work4youOverride) {
      const resolvedOverride = findOnPath(work4youOverride)

      if (resolvedOverride) {
        work4youCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(work4youOverride, { isWsl: IS_WSL })) {
        work4youCommand = work4youOverride
      } else {
        rememberLog(`Ignoring Windows Work4You override under WSL: ${work4youOverride}`)
      }
    } else {
      work4youCommand = findOnPath('work4you')
    }

    if (work4youCommand) {
      if (looksLikeDesktopAppBinary(work4youCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Work4You CLI: ${work4youCommand}`)
        work4youCommand = null
      }
    }

    if (work4youCommand) {
      const unwrapped = unwrapWindowsVenvWork4YouCommand(work4youCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `work4you` shim
      ...
      const shellForProbe = isCommandScript(work4youCommand)

      // WORK4YOU_DESKTOP_WORK4YOU is an explicit deployment override (used by
      ...
      if (
        ...
        return {
          label: `existing Work4You CLI at ${work4youCommand}`,
          command: work4youCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Work4You CLI at ${work4youCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed work4you_cli module via system Python.
  ...
  if (python) {
    ...
    // having work4you_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\work4you\work4you\venv.
    if (canImportWork4YouCli(python)) {
      return {
        kind: 'python',
        label: `installed work4you_cli module via ${python}`,
        command: python,
        args: ['-m', 'work4you_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: work4you_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveWork4YouBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Work4You not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_WORK4YOU_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveWork4YouBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Work4You recovery was handed off to Work4You Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      work4youHome: WORK4YOU_HOME,
      logRoot: path.join(WORK4YOU_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Work4You install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Work4You bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(WORK4YOU_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startWork4You() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // work4you:bootstrap:reset IPC (renderer's "Reload and retry").
  ...
  if (!isWork4YouSourceRoot(ACTIVE_WORK4YOU_ROOT)) {
    throw new Error(
      `Work4You install at ${ACTIVE_WORK4YOU_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Work4You' terminal tool calls bash.exe
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isWork4YouSourceRoot()
    // plus an importable work4you_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Work4You venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Work4You at ${ACTIVE_WORK4YOU_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Work4You runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----work4you-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Work4You backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Work4You-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Work4You backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Work4You backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Work4You-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('work4you:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveWork4YouCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('work4you:preview-file-changed', payload)
}
...
// waitForWork4YouReady how to read a 401 — rejected session vs gated route.
...
async function waitForWork4You(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForWork4YouReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('work4you:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('work4you:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('work4you:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('work4you:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Work4You...".
function sendPowerResume() {
  ...
  webContents.send('work4you:power-resume')
}
...
// 'work4you:power-battery' push below.
ipcMain.handle('work4you:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('work4you:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('work4you:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('work4you:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('work4you:context-menu-spellcheck', {
...
//
// Hosted Work4You gateways gate the dashboard behind an OAuth provider (e.g.
// Work4You) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``work4you_session_at``),
...
//   * Work4You Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / work4you #37247).
//     Both are set as HttpOnly cookies (``work4you_session_at`` ~15 min,
//     ``work4you_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:work4you-remote-oauth'
...
// "not signed in", which on the initial boot path (startWork4You → the renderer's
// single-shot boot() with no retry) surfaced as the "Work4You couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Work4You
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Work4You Cloud agent…' : 'Sign in to Work4You gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Work4You backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in work4you_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Work4You Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Work4You portal ONCE in
...
// portal session cookie living in `persist:work4you-remote-oauth`:
...
// Canonical Work4You portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (work4you_cli/auth.py DEFAULT_WORK4YOU_PORTAL_URL + the same env names)
// so a single override flips every Work4You surface to the same portal.
...
// Whether the OAuth partition currently holds a live Work4You portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Work4You gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for work4you_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Work4You Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Work4You Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Work4You Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Work4You Cloud session has expired. Sign in to Work4You Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Work4You Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every work4you:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteWork4YouPath`/`remoteProfile` —
...
// a valid stored value (pins the root WORK4YOU_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.WORK4YOU_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.WORK4YOU_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Work4You Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteWork4YouPath: (ssh || savedSsh)?.remoteWork4YouPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by WORK4YOU_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Work4You Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Work4You Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteWork4YouPath: input.sshRemoteWork4YouPath ?? existingBlock.remoteWork4YouPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (work4you #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Work4You gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Work4You gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Work4You gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.WORK4YOU_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteWork4YouPath || '')
    ...
    result = await lifecycle({
      ...
      remoteWork4YouPath: sshConfig.remoteWork4YouPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForWork4You: (baseUrl, token) => waitForWork4You(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    work4youVersion: result.work4youVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.work4youVersion || 'work4you (version unknown)'} at ${result.work4youPath || '?'}`
  )
  ...
  return { ...connection, remoteWork4YouVersion: result.work4youVersion || '' }
}
...
//   2. env override (WORK4YOU_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.WORK4YOU_DESKTOP_REMOTE_TOKEN,
      url: process.env.WORK4YOU_DESKTOP_REMOTE_URL
    },
    profile,
...
// env → global) so a startWork4You() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Work4You
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Work4You" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteWork4YouPath: input.sshRemoteWork4YouPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteWork4YouPath || '')
          let work4youPath
          let work4youVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            work4youPath = runtime.work4youPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.work4youPath])
            work4youVersion = inspection.version
            supported = inspection.supported
          } else {
            work4youPath = await remoteLifecycle.locateWork4You(ssh, sshConfig.remoteWork4YouPath || '')
            work4youVersion = await remoteLifecycle.probeWork4YouVersion(ssh, work4youPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, work4youPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Work4You on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteWork4YouPath: work4youPath,
            remoteWork4YouVersion: work4youVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startWork4You())
  ...
  // connect to Work4You gateway". Mirror the renderer's connect here so the test
...
function resetWork4YouConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const work4youProcess = backendConnectionState.invalidate()
  stopBackendChild(work4youProcess)
...
// startWork4You() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetWork4YouConnection() invalidates it.
  const work4youProcess = backendConnectionState.getProcess()
  const dying = work4youProcess && !work4youProcess.killed ? work4youProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('work4you:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('work4you:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startWork4You()
  ...
  // spawning, so two children never share one profile's WORK4YOU_HOME.
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteWork4YouPath: source.remoteWork4YouPath,
    ...
    return {
      ...
      // is only the routing label. work4you:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: work4youLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForWork4You(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startWork4You() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForWork4You(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited WORK4YOU_HOME env (see _apply_profile_override
  // step 3 in work4you_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveWork4YouBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const work4youCwd = resolveWork4YouCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Work4You backend for profile "${profile}" via ${backend.label}`)

  const parentStartMarker = await desktopParentStartMarker()
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(WORK4YOU_HOME, 'profiles', key))
  )
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        ...parentIdentityEnv,
        WORK4YOU_WEB_DIST: webDist,
        ...(readyFile ? { WORK4YOU_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Work4You backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Work4You backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)

    if (!ready) {
      rejectStart?.(
        new Error(`Work4You backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForWork4You(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Work4You backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Work4You backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_work4you_home() recreates the deleted profile directory.
//
...
async function startWork4You() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Work4You Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startWork4You() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Work4You backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Work4You backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Work4You backend at ${remote.baseUrl}`, 24)
      await waitForWork4You(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Work4You backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.work4you/active_profile file) and
    // resolves WORK4YOU_HOME the same way `work4you -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Work4You runtime', 28)

        return resolveWork4YouBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Work4You backend via ${backend.label}`, 84)
    rememberLog(`Starting Work4You backend via ${backend.label}`)
    ...
    const work4youProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin WORK4YOU_HOME for the child so Python's get_work4you_home()
          // resolves to the SAME location our resolveWork4YouHome() picked. Without
          // this pin, Python falls back to ~/.work4you on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\work4you, which differs from C:\Users\<u>\.work4you.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets WORK4YOU_HOME via setx; the desktop
    ...
    await claimBackendChild(work4youProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, work4youProcess)

    if (!processOwner) {
      stopBackendChild(work4youProcess)
      await waitForBackendExit(work4youProcess)
      releaseBackendChild(work4youProcess)
      throw new Error('Work4You backend start was superseded by a newer connection attempt.')
    }

    work4youProcess.stdout.on('data', rememberLog)
    work4youProcess.stderr.on('data', rememberLog)
    ...
    work4youProcess.once('error', error => {
      releaseBackendChild(work4youProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Work4You backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Work4You backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Work4You backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Work4You backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    work4youProcess.once('exit', (code, signal) => {
      releaseBackendChild(work4youProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Work4You backend exit (${signal || code})`)
      ...
      rememberLog(`Work4You backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Work4You backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Work4You backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentWork4YouLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Work4You backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(work4youProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Work4You backend to become ready', 90)
    await Promise.race([waitForWork4You(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => work4youProcess.exitCode === null && !work4youProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Work4You backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Work4You backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Work4You is
...
// pushes pet state over IPC (work4you:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via work4you:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Work4You app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Work4You icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see work4you:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Work4You in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('work4you:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Work4You can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('work4you:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('work4you:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `work4you:hud:set-bounds`, which flips resizable on for the call — the
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('work4you:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('work4you:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('work4you:quick-entry:shown')
}
...
function createWindow() {
  ...
  startWork4You().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('work4you:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('work4you:connection:for', async (_event, payload) => {
...
// "Starting Work4You…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('work4you:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetWork4YouConnection,
...
ipcMain.handle('work4you:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('work4you:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('work4you:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('work4you:window:openInstance', async () => {
...
ipcMain.handle('work4you:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveWork4YouBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Work4You is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `work4you-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, WORK4YOU_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('work4you:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('work4you:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('work4you:zoom:get', event => {
...
ipcMain.on('work4you:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('work4you:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startWork4You() call restarts the
...
ipcMain.handle('work4you:bootstrap:repair', async () => {
  ...
  // "bypass active runtime" path inside startWork4You() and fall through
  ...
  resetWork4YouConnection()

  return { ok: true }
})
ipcMain.handle('work4you:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('work4you:bootstrap:cancel', async () => {
...
ipcMain.handle('work4you:boot-progress:get', async () => bootProgressState)
ipcMain.handle('work4you:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('work4you:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('work4you:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('work4you:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('work4you:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('work4you:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('work4you:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('work4you:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('work4you:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('work4you:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('work4you:connections:set-launch-mode', async (_event, mode) => {
  const registry = setConnectionLaunchMode(readDesktopConnectionsRegistry(), String(mode || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('work4you:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('work4you:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteWork4YouPath: entry.remoteWork4YouPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startWork4You()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteWork4YouPath: connection.remoteWork4YouPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteWork4YouProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root WORK4YOU_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('work4you:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// work4you:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('work4you:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `work4you update` to every eligible registered connection at once.
...
// /api/work4you/update endpoint (the dashboard updater), which runs
// `work4you update` on THAT machine.
ipcMain.handle('work4you:connections:update-all', async () => {
  const registry = readDesktopConnectionsRegistry()
  ...
  const results = await Promise.all(
    registry.connections.map(async connection => {
      ...
      try {
        ...
        const body: any = await postJsonForBackend(descriptor, '/api/work4you/update', {}, { timeoutMs: 15_000 })
...
// analogue of the work4you:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('work4you:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('work4you:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startWork4You() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('work4you:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('work4you:cloud:logout', async () => {
...
ipcMain.handle('work4you:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('work4you:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('work4you:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('work4you:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new WORK4YOU_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('work4you:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('work4you:requestMicrophoneAccess', async () => {
...
ipcMain.handle('work4you:window:readBelow', async event => {
...
async function handleWork4YouApiRequest(request) {
  ...
  // backend calls ensure_work4you_home() which recreates the profile directory,
...
ipcMain.handle('work4you:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its WORK4YOU_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleWork4YouApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleWork4YouApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('work4you:data-url-read-max:get', () => ({
...
ipcMain.handle('work4you:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('work4you:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('work4you:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('work4you:readFileText', async (_event, filePath) => {
...
ipcMain.handle('work4you:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('work4you:writeClipboard', (_event, text) => {
...
ipcMain.handle('work4you:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('work4you:readClipboard', () => clipboard.readText())

ipcMain.handle('work4you:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('work4you:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('work4you:context-menu:edit', (event, command) => {
...
ipcMain.handle('work4you:context-menu:copy-image', event => {
...
ipcMain.handle('work4you:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('work4you:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('work4you:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('work4you:saveClipboardImage', async () => {
...
ipcMain.handle('work4you:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('work4you:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('work4you:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('work4you:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('work4you:active-work', (event, payload) => {
...
ipcMain.on('work4you:titlebar-theme', (_event, payload) => {
...
ipcMain.on('work4you:native-theme', (_event, mode) => {
...
ipcMain.on('work4you:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('work4you:translucency', (_event, payload) => {
...
ipcMain.on('work4you:keep-awake', (_event, on) => {
...
ipcMain.handle('work4you:quick-entry:settings:get', async () => {
...
ipcMain.handle('work4you:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('work4you:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('work4you:quick-entry:submit', {
...
ipcMain.on('work4you:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('work4you:quick-entry:state', payload)
  }
})

ipcMain.on('work4you:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('work4you:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('work4you:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `work4you:window:openSession`).
...
ipcMain.handle('work4you:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('work4you:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('work4you:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))

ipcMain.handle('work4you:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveWork4YouCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('work4you:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveWork4YouCwd()
}))

ipcMain.handle('work4you:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('work4you:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('work4you:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('work4you:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: work4youLog.slice(-200) }))
...
ipcMain.on('work4you:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (work4you:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('work4you:updates:check', async () =>
...
ipcMain.handle('work4you:updates:apply', async (_event, payload) =>
...
ipcMain.handle('work4you:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Work4You version (the one `release.py` bumps in
// work4you_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Work4You version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveWork4YouVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'work4you_cli', '__init__.py')
...
// Renderer-bundle skew: `work4you update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Work4You version and push it into the native About panel
// just before showing it, so an in-place `work4you update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveWork4YouVersion()} — app build out of date, update the desktop app`
        : resolveWork4YouVersion(),
      copyright: 'Copyright © 2026 Work4You'
    })
    app.showAboutPanel()
  })
...
// the actual removal via `work4you uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (work4you_cli/uninstall.py + work4you_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const fallback = () => ({
    work4you_home: WORK4YOU_HOME,
    agent_installed: isWork4YouSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Work4You agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import work4you_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_WORK4YOU_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_WORK4YOU_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `work4you-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `work4you-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('work4you:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('work4you:uninstall:run', async (_event, payload) => {
...
// work4you://mcp/install?name=NAME&config=B64 — the vendor "Add to Work4You"
// button, or work4you://plugin/install?repo=owner/repo). Dev
// (`WORK4YOU_DESKTOP_DEV_SERVER`) registers work4you-dev:// instead — bare
// Electron or a stale OS handler often owns work4you:// on dev machines.
...
// ---------------------------------------------------------------------------
const WORK4YOU_PROTOCOL = DEV_SERVER ? 'work4you-dev' : 'work4you'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['work4you-dev', 'work4you'] : ['work4you']
...
function handleDeepLink(url) {
  ...
  // work4you://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('work4you:deep-link', payload)
...
ipcMain.handle('work4you:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${WORK4YOU_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(WORK4YOU_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(WORK4YOU_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${WORK4YOU_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `work4you://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startWork4You), whose
...
app.on('before-quit', event => {
  ...
  // quitting Work4You never keeps another app's chord hostage.
