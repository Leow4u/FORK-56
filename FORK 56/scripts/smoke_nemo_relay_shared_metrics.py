"""Run a real Work4You CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_work4you_executable(work4you_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "work4you",
        Path(".venv") / "Scripts" / "work4you.exe",
    ):
        candidate = work4you_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("work4you")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Work4You executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Work4You source checkout containing .venv/bin/work4you",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated WORK4YOU_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["work4you.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['work4you.client.active']}"
        )
    [model] = by_name["work4you.model_route.count"]
    expected_model = {
        "name": "work4you.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['work4you.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["work4you.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['work4you.task_run.started']}"
        )
    [terminal] = by_name["work4you.task_run.finished"]
    ...
    [tool] = by_name["work4you.tool_call.count"]
    ...
    lifecycle = by_name["work4you.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["work4you.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Work4You development environment requires jsonschema"
        ) from exc
    ...
    if metrics["work4you.client.active"] != [
        {
            "name": "work4you.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['work4you.client.active']}"
        )
    [model] = metrics["work4you.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['work4you.model_route.count']}"
        )
    [terminal] = metrics["work4you.task_run.finished"]
    ...
    [tool] = metrics["work4you.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["work4you.skill.lifecycle.count"]
    ...
    loads = metrics["work4you.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    work4you_repo = args.work4you_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    work4you = _resolve_work4you_executable(work4you_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="work4you-relay-shared-metrics-"))
    home = root / "work4you-home"
    ...
    try:
        ...
        python_paths = [str(work4you_repo)]
    ...
    (root / "work4you.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "work4you.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Work4You exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Work4You model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Work4You did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Work4You did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from work4you_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "work4you.shared_metrics.v2.schema.json",
    )

    print("Work4You -> NeMo Relay shared-metrics smoke test passed")
