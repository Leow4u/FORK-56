...
# un-throttled log fires on *every* selection. On Windows several Work4You
...
# readiness handshake ("Timed out connecting to Work4You backend after
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "work4you":
            # Work4You stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._work4you_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): work4you, openai-codex, and xai-oauth rotate the
    ...
    ``work4you_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``work4you model`` / ``work4you auth`` which
        ...
        Mirrors the Work4You/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Work4You process
    ...
    def _sync_work4you_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Work4You pool entry from auth.json if tokens differ.

        Work4You OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_work4you_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Work4You Portal.
        """
        if self.provider != "work4you" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "work4you")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Work4You state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Work4You entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Work4You, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Work4You/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``work4you`` invocation that defaults to the active provider
        (e.g. setup wizard, ``work4you auth status``) would land on
        ...
        # device-code sources (work4you, openai-codex, xAI) use ``device_code``.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Work4You
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``WORK4YOU_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "WORK4YOU_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "WORK4YOU_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("work4you_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Work4You
            ...
            elif self.provider == "work4you":
                synced = self._sync_work4you_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_work4you_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_work4you_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("work4you_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as work4you — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Work4You quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/work4you — another Work4You process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Work4You quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    with self._lock:
                        ...
                        self._persist(removed_ids=removed_ids)
                    return None
            # For work4you: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "work4you":
                synced = self._sync_work4you_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Work4You refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_work4you_refresh_error(exc):
                    logger.debug("Work4You refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "work4you") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_work4you_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_work4you_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Work4You OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.WORK4YOU_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.WORK4YOU_DEVICE_CODE_SOURCE}",
                    }
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "work4you":
            # Work4You refresh can require network access and should happen when
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Work4You profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For work4you entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_work4you_runtime_credentials(), making this entry's
            ...
            # re-authed via `work4you model` / `work4you auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `work4you model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``work4you auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `work4you auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `work4you auth remove <provider> <N>` is stable across all source types.
    try:
        from work4you_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Work4You PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `work4you setup` ("Claude
        ...
        # Claude Code CLI, work4you_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.work4you/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `work4you setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"work4you_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_work4you_oauth_credentials

        for source_name, creds in (
            ("work4you_pkce", read_work4you_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_work4you_credentials(label=...) when the user ran
            # `work4you auth add work4you --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (work4you auth remove copilot gh_cli) must not pay either on
        ...
        # Work4You auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from work4you_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.work4you/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `work4you auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `work4you auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Work4You auth store.  Without this gate the removal is instantly
        ...
        # Work4You owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `work4you auth openai-codex`.
        ...
        # When the user logs in via ``work4you model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``work4you auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with work4you/codex).
            source = "device_code"
...
# Prefer ~/.work4you/.env over os.environ — the user's config file is the
# authoritative source for Work4You credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_work4you_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `work4you auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.work4you/.env.
    ...
    try:
        from work4you_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from work4you_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.work4you/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `work4you auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Work4You
