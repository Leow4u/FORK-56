...
description: "How Work4You stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `work4you_cli/cron.py` | CLI `work4you cron` subcommands |
...
Jobs are stored in `~/.work4you/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `work4you cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks Work4You infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Work4You
...
The flow (the managed scheduler is provided by Work4You; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks Work4You to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Work4You token)
  → at fire time Work4You calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Work4You-minted JWT)
...
Config (all non-secret; on hosted agents Work4You sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | Work4You base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into Work4You,
...
agent↔Work4You wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.work4you/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.work4you/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `work4you cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `work4you cron` CLI provides direct job management:

```bash
work4you cron list                    # Show all jobs
work4you cron create                  # Interactive job creation (alias: add)
work4you cron edit <job_id>           # Edit job configuration
work4you cron pause <job_id>          # Pause a running job
work4you cron resume <job_id>         # Resume a paused job
work4you cron run <job_id>            # Trigger immediate execution
work4you cron remove <job_id>         # Delete a job
```
