...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with work4you)
2. User plugins: ``$WORK4YOU_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``work4you.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$WORK4YOU_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$WORK4YOU_HOME/plugins/model-providers/`` if it exists."""
    try:
        from work4you_constants import get_work4you_home

        d = get_work4you_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple WORK4YOU_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_work4you_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``work4you.plugins``
    ...
    * **Provider targets only.** The ``work4you.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$WORK4YOU_HOME``) keep their
    ...
    try:
        from work4you_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$WORK4YOU_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``work4you.plugins``
    ...
    #    filesystem steps means a bundled or ``$WORK4YOU_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with work4you.
    ...
    # 2. User plugins — under $WORK4YOU_HOME/plugins/model-providers/<name>/.
