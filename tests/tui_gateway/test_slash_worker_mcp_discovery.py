...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    server.write_text(
        textwrap.dedent(
            ...
            @mcp.tool()
            def work4you_61922_profile_probe() -> str:
                return {marker!r}
    ...
    (profile_home / "config.yaml").write_text(
        yaml.safe_dump(
            {
                # CI runners can be slow to cold-start the probe MCP server;
                # the slash worker's default 1.5s discovery cap flakes here.
    ...
    tool_name = "mcp__profileprobe__work4you_61922_profile_probe"

    env = os.environ.copy()
    for key in list(env):
        if key.endswith("_API_KEY") or key.endswith("_TOKEN"):
            env.pop(key)
    env["WORK4YOU_HOME"] = str(profile_home)
    env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
    env["WORK4YOU_SLASH_WATCHDOG_GRACE_S"] = "0"
    env["WORK4YOU_SLASH_WATCHDOG_POLL_S"] = "0.05"
    ...
    try:
        ...
        assert tool_name in response["output"]
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)
