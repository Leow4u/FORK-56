    ...
    [
      ...
      terminal.paste ? (
        <Item
          ...
          onSelect={() =>
            void window.work4youDesktop?.readClipboard().then(text => (text ? terminal.paste?.(text) : undefined))
          }
        />
      ) : null,
      <Item
...
function domSections(open: Extract<OpenContextMenu, { kind: 'dom' }>, t: Translations): ReactNode[][] {
  ...
  const editableCommand = (command: 'copy' | 'cut' | 'paste') => {
    withEditableFocus(() => void window.work4youDesktop?.contextMenuEdit?.(command))
  }
  ...
  const spellcheckAction = (action: { kind: 'add' | 'replace'; word: string }) => {
    withEditableFocus(() => void window.work4youDesktop?.contextMenuSpellcheck?.(action))
  }
    ...
    sections.push(
      [
        ...
        <Item
          ...
          onSelect={() => void window.work4youDesktop?.contextMenuCopyImage?.()}
        />,
        target.imageUrl ? (
          <Item
            icon="copy"
        ...
        target.imageUrl ? (
          <Item
            ...
            onSelect={() => void window.work4youDesktop?.saveImageFromUrl?.(target.imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
      ...
      [
        ...
        imageUrl ? (
          <Item
            ...
            label={copy.image.saveImageAs}
            onSelect={() => void window.work4youDesktop?.saveImageFromUrl?.(imageUrl)}
          />
        ) : null
      ].filter(Boolean)
    )
  ...
  return [
    ...
    [
      <Item
        icon="cloud-download"
        key="shell-update"
        label={t.commandCenter.updateWork4You}
...
export function AppContextMenu() {
  ...
  useEffect(() => window.work4youDesktop?.onContextMenuSpellcheck?.(augmentSpellcheck), [])
