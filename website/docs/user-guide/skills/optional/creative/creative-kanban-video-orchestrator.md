...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Work4You Kanban pipeline that
...
3. **Generates** a setup script that creates Work4You profiles, project workspace, and the initial kanban task
...
For mapping role → which Work4You skills + toolsets it loads, see
...
3. Creates each Work4You profile via `work4you profile create --clone`
...
7. Fires the initial `work4you kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/Leow4u/FORK-56/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
work4you kanban watch --tenant <project-tenant>     # live events
work4you kanban list  --tenant <project-tenant>     # board snapshot
work4you dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/Leow4u/FORK-56/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/Leow4u/FORK-56/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `work4you kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${WORK4YOU_HOME:-~/.work4you}/.env` or the user's secret store.
