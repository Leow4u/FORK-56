...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.work4youDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: Work4YouGateway | null) => void
  refreshWork4YouConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: Work4YouConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Work4You...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Work4You…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Work4You gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshWork4YouConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshWork4YouConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startWork4You re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new Work4YouGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshWork4YouConfig(),
          ...
          // retry it), not brick boot behind the "Work4You couldn't start"
