...
//   · its own WORK4YOU_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · WORK4YOU_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated WORK4YOU_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate WORK4YOU_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = work4youHome ?? mkTemp('work4you-perf-home-')
  const userData = userDataDir ?? mkTemp('work4you-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !work4youHome) {
    seedConfigFrom(join(homedir(), '.work4you'), home)
  }
  ...
  try {
    ...
    // WORK4YOU_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set WORK4YOU_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      WORK4YOU_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.WORK4YOU_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'work4you-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'work4you-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.work4you'), home)
