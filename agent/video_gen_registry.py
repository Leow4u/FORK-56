...
def list_providers(*, scope: Optional[str] = None) -> List[VideoGenProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or work4you_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[VideoGenProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or work4you_home_key(), {}).get(key) or _providers.get(key)
...
def get_active_provider() -> Optional[VideoGenProvider]:
    ...
    # The managed "Work4You Subscription" selection is serviced by the FAL
    # plugin through the managed fal-queue gateway (the plugin's resolver
    # routes managed when the stored selection is "work4you").
    if configured:
        try:
            from tools.tool_backend_helpers import WORK4YOU_MANAGED_PROVIDER

            if configured.lower() == WORK4YOU_MANAGED_PROVIDER:
                configured = "fal"
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(work4you_home_key(), {}))
