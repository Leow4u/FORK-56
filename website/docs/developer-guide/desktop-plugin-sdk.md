...
title: "Desktop Plugin SDK (@work4you/plugin-sdk)"
description: "Extend the native Work4You Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Work4You Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `Work4YouPlugin`.
It imports one module — `@work4you/plugin-sdk` — and gets everything: the app's
...
`$WORK4YOU_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Work4You. This page is the **native
desktop app** (`work4you desktop`) SDK — the `@work4you/plugin-sdk` module and
`$WORK4YOU_HOME/desktop-plugins/`. The **web dashboard** (`work4you dashboard`) has
its own, unrelated plugin system on `window.__WORK4YOU_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Work4You Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$WORK4YOU_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$WORK4YOU_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `Work4YouPlugin` contract, appear in **Settings → Plugins**,
...
Create `$WORK4YOU_HOME/desktop-plugins/hello/plugin.js` (that's `~/.work4you/...`
by default, or `~/.work4you/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.work4you/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@work4you/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Work4You' }),
      jsx('div', {
...
The only importable specifiers are `@work4you/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `Work4YouPlugin`:

```ts
interface Work4YouPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `work4you.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@work4you/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@work4you/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@work4you/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@work4you/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@work4you/plugin-sdk'
...
`window.work4you.send('get-price eth')` (or a declarative
`<button data-work4you-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@work4you/plugin-sdk'
...
for requests; `targetProfile` is the backend Work4You profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Work4You
...
ctx.os.notify({
  ...
  // Body click → focus Work4You + navigate. Same vocabulary as OS deep links:
  activate: 'work4you://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'work4you://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `work4you://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `work4you://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@work4you/plugin-sdk'
...
a package into `~/.work4you/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.work4you/plugins` is not reachable as a filesystem —
...
the `work4you://` scheme — a plain anchor on your website or README:
...
`work4you-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Work4You plugin and declare it in a
`manifest.json`:
...
work4you codebase directly (`work4you_state`, `work4you_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.work4you/`)
...
(`work4you.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `Work4YouPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@work4you/plugin-sdk` alias.
2. It's still lint-fenced to `@work4you/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@work4you/plugin-sdk`, `react`,
...
| Plugin contract | `Work4YouPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `Work4YouOpenTarget`, `Contribution` |
...
### Agents: the `work4you-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`work4you-desktop-plugins`** skill — it carries the same contract as this page in
...
`$WORK4YOU_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `work4you logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@work4you/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.work4you/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.work4you/logs/errors.log` for
