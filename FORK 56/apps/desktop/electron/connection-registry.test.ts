...
// The electron and @work4you/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@work4you/shared'))) as {
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.work4you.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(renamed.org, 'work4you')
  ...
  const ssh = {
    ...
    label: 'Box',
    port: 2222,
    remoteWork4YouPath: '/opt/work4you',
  ...
  assert.equal(labelOnly.remoteWork4YouPath, '/opt/work4you')
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.work4you.cloud', authMode: 'oauth', org: 'work4you' },
    registry
  )

  assert.equal(cloud.kind, 'cloud')
  assert.equal(cloud.org, 'work4you')
  ...
  const input = {
    ...
    connections: [
      ...
      },
      {
        id: 'cloud-1',
        kind: 'cloud',
        label: 'Work4You Cloud',
        url: 'https://a.work4you.cloud',
        authMode: 'oauth',
        org: 'work4you'
      },
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.work4you.cloud', authMode: 'oauth', org: 'work4you' }
  })
