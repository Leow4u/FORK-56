...
const TREE_KEY = 'work4you.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'work4you.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'work4you.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['work4you-bots:pane'], active: 'work4you-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'work4you-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'work4you-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","work4you-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['work4you-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['work4you-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'work4you-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['work4you-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'work4you-bots:pane')!
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'work4you-bots:pane', 'work4you-bots:routines'],
          active: 'work4you-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'work4you-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'work4you-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'work4you-bots:pane'])
    expect(botsGroup.active).toBe('work4you-bots:pane')
    expect(routinesGroup.panes).toEqual(['work4you-bots:routines'])
