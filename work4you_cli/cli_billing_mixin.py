...
This module hosts the Work4You billing/subscription methods lifted out of
``cli.py``'s ``Work4YouCLI`` class. ``Work4YouCLI`` inherits
...
Import discipline mirrors ``work4you_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_work4you_credits_block(self) -> bool:
        """Print the Work4You dollar balance block (two-bar view) when a Work4You account
        ...
        truth). Falls back to the legacy ``work4you_credits_lines`` text only when the
        model is unavailable. Agent-independent (a portal fetch gated on "a Work4You
        ...
        wall-clock-bounded; honors WORK4YOU_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Work4You account is logged
        in (the balance block printed), since both commands are Work4You-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Work4You plan + browser hand-off.
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Work4You Portal.')}")
                print("  Run `work4you portal` to log in, then /subscription.")
            return
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``work4you_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from work4you_cli.work4you_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        step_up_work4you_billing_scope directly (it opens the browser + blocks), then
        ...
        if not getattr(self, "_app", None):
            print("  Run `work4you portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from work4you_cli.auth import step_up_work4you_billing_scope

            granted = step_up_work4you_billing_scope(open_browser=True)
        ...
        try:
            from work4you_cli import work4you_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for Work4You (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Work4You Portal.')}")
                print("  Run `work4you portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Work4You page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Work4You page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Work4You to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `work4you portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `work4you portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Work4You page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `work4you portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Work4You page, then run /topup again.")
        ...
        from work4you_cli.work4you_billing import BillingError, post_charge
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Work4You to charge {card.masked} "
