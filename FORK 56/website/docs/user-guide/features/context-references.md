...
Type `@` followed by a reference to inject content directly into your message. Work4You expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Work4You env: `$WORK4YOU_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$WORK4YOU_HOME/skills/.hub/`
