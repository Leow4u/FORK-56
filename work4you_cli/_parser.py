...
# argparse runs (it sets ``WORK4YOU_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``work4you_cli.relaunch`` should carry over when
...
Examples:
    ...
    work4you --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ...
    work4you debug share             Upload debug report for support
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.work4you/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    # top-level parser: when the user writes `work4you -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/work4you_cli/
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.work4you/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
