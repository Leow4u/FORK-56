"""Core NeMo Relay adapters for physical Work4You provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Work4You knows the API mode."""
...
def _relay_metadata(
    ...
    if not isinstance(relay_metadata, dict):
        relay_metadata = {}
    relay_metadata.setdefault("work4you.provider", provider_name)
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Work4You turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Work4You turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Work4You's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Work4You-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Work4You."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Work4You Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
