"""
`work4you computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Work4You side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Work4You provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Work4You-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `WORK4YOU_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Work4You identity under work4you_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["work4you_identity"] = identity
