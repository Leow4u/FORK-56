...
Build the real image and verify: a config.yaml present in $WORK4YOU_HOME
is migrated by docker_config_migrate.py on boot, running as the work4you
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $WORK4YOU_HOME must be migrated on boot by
    docker_config_migrate.py, running as the work4you user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $WORK4YOU_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/work4you/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by work4you (migration ran as work4you)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "work4you", (
        f"config.yaml not owned by work4you (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
