...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/mcp/fastmcp` |
...
| Related skills | [`work4you`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-work4you), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Work4You or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Work4You. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.work4you/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.work4you/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Work4You integration, either:

- configure the server in `~/.work4you/config.yaml` using the `native-mcp` skill, or
...
### Work4You cannot see the deployed server

The server-building part may be correct while the Work4You config is not. Load the `native-mcp` skill and configure the server in `~/.work4you/config.yaml`, then restart Work4You.
