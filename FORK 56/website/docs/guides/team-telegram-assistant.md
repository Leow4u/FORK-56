...
This tutorial walks you through setting up a Telegram bot powered by Work4You that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Work4You installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.work4you/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Work4You itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Work4You Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_work4you_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Work4You. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.work4you/.env`:
...
```
[Gateway] Starting Work4You Gateway...
...
# View live logs
journalctl --user -u work4you-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `work4you gateway install` to update the plist.
:::
...
```bash
work4you gateway stop && work4you gateway start
```
...
work4you pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.work4you/.env`:
...
Control how much detail the bot shows when using tools. In `~/.work4you/config.yaml`:
...
Customize how the bot communicates by editing `~/.work4you/SOUL.md`:

For a full guide, see [Use SOUL.md with Work4You](/guides/use-soul-with-work4you).
...
```bash
# From the CLI
work4you cron list          # View all scheduled jobs
work4you cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u work4you-gateway` (Linux) or `~/.work4you/logs/gateway.log` (macOS) |
| Cron job output | `~/.work4you/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.work4you/cron/jobs.json` |
| Pairing data | `~/.work4you/pairing/` |
| Session history | `~/.work4you/sessions/` |
