# Contributing to Work4You

Thank you for contributing to Work4You! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Work4You to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Leow4u/FORK-56 "<your terms>"
  gh search prs --repo Leow4u/FORK-56 --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/Leow4u/FORK-56/issues?q=) · [PRs (all states)](https://github.com/Leow4u/FORK-56/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Work4You itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Work4You install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `work4you skills browse` (labeled "official") and install it with `work4you skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Work4You Discord](https://work4you.ai). Users can install it with `work4you skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.work4you/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(work4you_home, config)` for setup-wizard integration
...
- Integrate with `work4you memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Work4You ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.work4you/plugins/`, project `.work4you/plugins/`, or a pip entry point) — see [Build a Work4You Plugin](https://work4you.ai/docs/guides/build-a-work4you-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Work4You Discord](https://work4you.ai) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Work4You venv, wires the `work4you` command, stamps the
install method for `work4you update`, and clones the full git project into
`$WORK4YOU_HOME/work4you` (usually `~/.work4you/work4you`). That keeps your
...
Use this only if you intentionally do not want Work4You' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `work4you` entrypoint from this venv; running the
system `python3 -m work4you_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.work4you/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.work4you/config.yaml
touch ~/.work4you/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.work4you/.env
```

### Run

```bash
# The standard installer already put `work4you` on PATH.
work4you doctor
work4you chat -q "Hello"
```

If you used the manual clone fallback, run `./work4you` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/work4you" ~/.local/bin/work4you
```
...
├── cli.py                    # Work4YouCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (work4you-cli, work4you-telegram, etc.)
├── work4you_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── work4you_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Work4You Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.work4you/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (work4you.ai)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.work4you/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.work4you/`)

| Path | Purpose |
|------|---------|
| `~/.work4you/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.work4you/.env` | API keys and secrets |
| `~/.work4you/auth.json` | OAuth credentials (Work4You Portal) |
| `~/.work4you/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.work4you/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.work4you/state.db` | SQLite session database |
| `~/.work4you/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.work4you/cron/` | Scheduled job data |
| `~/.work4you/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`work4you_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.work4you/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Work4You Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_WORK4YOU_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.work4you`:
...
The user may skip setup and keep loading the skill. Work4You only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `work4you setup` or update `~/.work4you/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Work4You tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Work4You" is the secondary collaborator. If the contributor's commit shows "Work4You" as author because they used Work4You to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Work4You tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `work4you --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Work4You uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.work4you/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `work4you_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `work4you_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Work4You runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the work4you wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `work4you_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `work4you_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `work4you_cli/gateway_windows.py::
...
Work4You has terminal access. Security matters.
...
2. **Test manually**: Run `work4you` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Leow4u/FORK-56/issues)
- Include: OS, Python version, Work4You version (`work4you version`), full error traceback
...
- **Discord**: [work4you.ai](https://work4you.ai) — for questions, showcasing projects, and sharing skills
