...
This guide covers adding a new messaging platform to the Work4You gateway. A platform adapter connects Work4You to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.work4you/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Work4You code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `work4you config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-work4you-config) below).
...
With `provides_tools` declared, Work4You imports only `tools.py` during plugin
...
`work4you tools enable my_platform --platform cli`, or by listing the toolset
...
| `work4you config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `work4you status` | Shows plugin platforms with `(plugin)` tag |
| `work4you gateway setup` | Plugin platforms appear in setup menu |
| `work4you tools` / `work4you skills` | Plugin platforms in per-platform config |
...
direct `work4you send --to ...` and cron `deliver=platform:...` by declaring send
...
Most users set up a platform by dropping env vars into `~/.work4you/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `work4you gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `work4you cron run` separate from `work4you gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `work4you config`

`work4you_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
:::note
This checklist is for adding a platform directly to the Work4You core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`work4you_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`work4you_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`work4you_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`work4you_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`work4you_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`work4you_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"work4you-newplat"` toolset definition with `_WORK4YOU_CORE_TOOLS`
2. **`toolsets.py`** — Add `"work4you-newplat"` to the `"work4you-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | work4you-newplat toolset |
