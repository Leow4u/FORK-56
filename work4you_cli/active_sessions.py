...
def _state_dir() -> Path:
    return Path(get_work4you_home()) / "runtime"
...
class ActiveSessionLease:
    ...
    # root ``WORK4YOU_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile WORK4YOU_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``work4you dashboard`` / ``serve``) never trips that
