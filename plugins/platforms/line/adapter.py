"""
LINE Messaging API platform adapter for Work4You.
...
to Work4You. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``WORK4YOU_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            work4you_home = Path(get_work4you_home()).resolve()
        except Exception:
            work4you_home = Path.home().joinpath(".work4you").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``work4you status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``work4you status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``work4you setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.work4you/.env`` via ``work4you_cli.config``.
    """
    print()
    ...
    try:
        from work4you_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("work4you_cli.config not available; set LINE_* vars manually in ~/.work4you/.env")
        return
