...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as Work4YouGateway | null
  const gatewayRef = useRef<Work4YouGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.work4youDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<Work4YouGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as Work4YouGateway | null
      }),
    []
  )
