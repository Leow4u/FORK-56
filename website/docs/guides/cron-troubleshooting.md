...
description: "Diagnose and fix common Work4You cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`work4you gateway` for foreground, or `work4you gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `work4you cron tick`.
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.work4you/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.work4you/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.work4you/.env` |
...
| `local` | Write access to `~/.work4you/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `work4you cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `work4you skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `work4you skills list`.
...
2. `~/.work4you/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `work4you cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Work4You config directory). Verify:
```bash
...
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `work4you skills install <skill-name>`.
...
Jobs are stored in `~/.work4you/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:
...
1. Run the job with `work4you cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.work4you/logs/agent.log` for scheduler messages and `~/.work4you/logs/errors.log` for warnings
3. Open an issue at [github.com/Leow4u/FORK-56](https://github.com/Leow4u/FORK-56) with:
