...
/**
 * User widget apps — Work4You authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$WORK4YOU_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.work4you/plugins/`: files under WORK4YOU_HOME execute
...
const widgetsDir = () => join(process.env.WORK4YOU_HOME?.trim() || join(homedir(), '.work4you'), 'tui-widgets')
 ...
 *  every change, so a widget Work4You writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Work4You) ever writes must hot-load too — a 10s poll here read
