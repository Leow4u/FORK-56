...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Work4You — no API key required"
---

# MiniMax OAuth

Work4You supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Work4You automatically refreshes your session.
...
# → Work4You opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.work4you/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.work4you/.env
```
...
```bash
work4you auth add minimax-oauth --no-browser
```

Work4You will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Work4You implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Work4You generates a PKCE verifier / challenge pair and a random state value.
...
4. Work4You polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.work4you/auth.json` under the `minimax-oauth` key.
...
```bash
work4you config set model.default MiniMax-M2.7
work4you config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.work4you/config.yaml` will contain entries similar to:
...
```bash
work4you --provider minimax-oauth    # canonical
work4you --provider minimax-portal   # alias
work4you --provider minimax-global   # alias
work4you --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `work4you setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
work4you --provider minimax-oauth
```
...
Work4You refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Work4You marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Work4You marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `work4you auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Work4You raises a timeout error.

**Fix:** re-run `work4you auth add minimax-oauth` (or `work4you model`). The flow starts fresh.

### State mismatch (possible CSRF)

Work4You detected that the `state` value returned by the authorization server does not match what it sent.
...
If `work4you` cannot open a browser window, use `--no-browser`:

```bash
work4you auth add minimax-oauth --no-browser
```

Work4You prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `work4you model` and select MiniMax (OAuth), or run `work4you auth add minimax-oauth`.
...
```bash
work4you auth logout minimax-oauth
```
...
- [work4you doctor](../reference/cli-commands.md)
