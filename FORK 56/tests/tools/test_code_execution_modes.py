...
from tools.code_execution_tool import (
    ...
    _uses_work4you_python_environment,
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("work4you_sandbox_", result["output"])
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The work4you venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_work4you_tools(self):
        """Regression: work4you_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from work4you_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
                "from work4you_tools import terminal\n"
                "r = terminal('echo x')\n"
    ...
    def test_strict_mode_can_still_import_work4you_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_work4you_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the work4you root from
...
class TestUsesWork4YouPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_work4you_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_work4you_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the work4you root for the interpreter Work4You itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_work4you_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_tool._python_environment_prefix",
            ...
            self.assertFalse(_uses_work4you_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_work4you_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        work4you_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=work4you_prefix):
            self.assertTrue(_uses_work4you_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — work4you root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify work4you root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_work4you_python_environment`` directly so these tests are
    ...
    def test_work4you_root_included_when_same_env(self):
        """When interpreter is in the Work4You env, work4you root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._work4you_root(), parts,
                      "work4you root must be in PYTHONPATH for same-env interpreters")

    def test_work4you_root_excluded_when_external_env(self):
        """When interpreter is external, work4you root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._work4you_root(), parts,
                         "work4you root must not leak into an external interpreter's PYTHONPATH")
