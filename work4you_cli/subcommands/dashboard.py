"""``work4you dashboard`` / ``work4you serve`` subcommand parsers.
...
``work4you_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `work4you dashboard` / `work4you serve` cmdlines and SIGTERM them directly —
    # the same path `work4you update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Work4You desktop app shells (<= 0.15.x) spawn the
    # backend as `work4you dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Work4You couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Work4You Desktop app (and headless remote
    ...
    # (cmd_dashboard exports WORK4YOU_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `work4you dashboard register` — register a self-hosted dashboard OAuth
    # client with Work4You Portal and write the client_id into ~/.work4you/.env.
    # Nested subparser so bare `work4you dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Work4You Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Work4You "
            "Portal account. Creates an OAuth client, writes "
            "WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID into ~/.work4you/.env, and prints "
    ...
    dashboard_register_parser.add_argument(
        ...
        default=None,
        help=(
            "Override the Work4You Portal base URL for registration (default: the "
