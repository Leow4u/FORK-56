...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Work4You /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Work4You, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``work4you model`` CLI picker. Adds network calls
      (pricing fetch + Work4You tier check); only set for interactive pickers.
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_work4you_tier=force_fresh_work4you_tier,
    ...
    try:
        from work4you_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_work4you_tier=force_fresh_work4you_tier)
    if capabilities:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``work4you_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`work4you_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from work4you_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from work4you_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `work4you model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Work4You.
    """
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + Work4You tier gating.

    Mutates ``rows`` in-place. For every row whose provider supports live
    pricing (openrouter / work4you / novita) adds::

        row["pricing"] = {model_id: {"input": "$3.00", "output": "$15.00",
                                     "cache": "$0.30" | None, "free": bool}}

    For Work4You additionally adds::
    ...
    # Resolve Work4You free-tier once (cached in models.py for the TTL window).
    work4you_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Work4You Portal-only. Other providers (OpenRouter,
            ...
            if slug == "work4you" and not is_free:
        ...
        if slug == "work4you":
            try:
                if work4you_free_tier is None:
                    work4you_free_tier = check_work4you_free_tier(
                        force_fresh=force_fresh_work4you_tier
                    )
                row["free_tier"] = bool(work4you_free_tier)
                if work4you_free_tier:
                    _selectable, unavailable = partition_work4you_models_by_tier(
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`work4you_cli.model_switch.list_picker_providers`) so the
