...
# Connecting Desktop to Many Work4You Instances

Register every Work4You backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Work4You Cloud instances — in one desktop app,
...
list of Work4You gateways. Its intro says it plainly: *"Manage this device and
every Work4You gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Work4You runtime managed by this app." | automatic |
| **Remote gateway** | "A Work4You gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Work4You install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Work4You Cloud** | "A hosted instance discovered through your Work4You Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Work4You Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Work4You Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Work4You Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Work4You Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `work4you serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the Work4You Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Work4You agents
...
connection is registered) dispatches `work4you update` to every eligible
...
- **Work4You Cloud** instances are skipped with a *"Managed by Work4You Cloud"*
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `work4you serve` is running on the remote host, the
...
- **"Update Work4You Desktop to chat with agents on other connections"** — the
