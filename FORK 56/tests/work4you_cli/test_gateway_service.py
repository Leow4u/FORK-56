...
import work4you_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`work4you gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `work4you gateway restart` in the background; when a
        ...
        assert "work4you gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``WORK4YOU_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../work4you_test`` WORK4YOU_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "work4you-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir WORK4YOU_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="WORK4YOU_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/work4you_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="WORK4YOU_HOME=/tmp/work4you-e2e-41264"\n'
        assert (
            gateway_cli._temp_home_in_service_definition(unit)
            == "/tmp/work4you-e2e-41264"
        )
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="WORK4YOU_HOME={tmp_path}/work4you-home"\n'
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".work4you" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `work4you gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import work4you_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.work4you.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                ...
                "<string>/Users/alice/.work4you</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: tmp_path)
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.work4you.gateway"])
            ),
        )
...
class TestSystemUnitWork4YouHome:
    """WORK4YOU_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".work4you" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".work4you")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_work4you = target_home / ".work4you"
        root_home = tmp_path / "root"
        root_work4you = root_home / ".work4you"
        managed_bin = target_work4you / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_work4you.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("WORK4YOU_HOME", str(root_work4you))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: root_work4you)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "work4you_constants.iter_work4you_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "work4you_constants.work4you_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'WORK4YOU_HOME=/home/alice/.work4you' in unit
        assert '/root/.work4you' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's WORK4YOU_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        work4you_home = str(gateway_cli.get_work4you_home().resolve())
        assert f'WORK4YOU_HOME={work4you_home}' in unit


class TestSystemUnitRefreshSyncsWork4YouHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.work4you."""

    def test_refresh_adopts_unit_work4you_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_work4you = root_home / ".work4you"
        alice_work4you = alice_home / ".work4you"
        root_work4you.mkdir(parents=True)
        alice_work4you.mkdir(parents=True)
        (root_work4you / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_work4you / "config.yaml").write_text(
        ...
        monkeypatch.delenv("WORK4YOU_RESTART_DRAIN_TIMEOUT", raising=False)

        # Correct installed unit (operator's WORK4YOU_HOME + drain timeout).
        monkeypatch.setenv("WORK4YOU_HOME", str(alice_work4you))
        ...
        # Simulate sudo without inherited WORK4YOU_HOME (falls back to root).
        monkeypatch.setenv("WORK4YOU_HOME", str(root_work4you))
        ...
        assert os.environ["WORK4YOU_HOME"] == str(alice_work4you)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned WORK4YOU_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_work4you_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_work4you_home_from_systemd_unit",
...
class TestWork4YouHomeForTargetUser:
    """Unit tests for _work4you_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("WORK4YOU_HOME", raising=False)

        result = gateway_cli._work4you_home_for_target_user("/home/alice")
        assert result == "/home/alice/.work4you"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "work4you gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".work4you" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("WORK4YOU_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'WORK4YOU_HOME={target_home / ".work4you" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".work4you" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WORK4YOU_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".work4you" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".work4you" / "work4you"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".work4you" / "work4you")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".work4you" / "work4you"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("WORK4YOU_HOME", str(root_home / ".work4you"))
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: root_home / ".work4you")
        ...
        # WorkingDirectory is anchored at the target user's WORK4YOU_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.work4you" in unit
        assert "WorkingDirectory=/home/alice/.work4you/work4you" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "work4you-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "work4you-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'work4you gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyWork4YouUnitDetection:
    """Tests for _find_legacy_work4you_units / has_legacy_work4you_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``work4you.service`` unit behind when the service was
    renamed to ``work4you-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``work4you.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``work4you-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Work4You Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m work4you_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "work4you.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "work4you.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_work4you_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m work4you_cli.main gateway run
          - python path/to/work4you_cli/main.py gateway run
          - work4you gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m work4you_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/work4you/work4you_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/work4you gateway run --replace",
            "ExecStart=/venv/bin/python /opt/work4you/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "work4you.service" if i == 0 else "work4you.service"  # same name
            # Test each variant fresh
            (user_dir / "work4you.service").write_text(
                f"[Unit]\nDescription=Old Work4You\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_work4you_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "work4you.service" in out
        assert "work4you gateway migrate-legacy" in out



class TestRemoveLegacyWork4YouUnits:
    """Tests for remove_legacy_work4you_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "work4you.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_work4you_units(interactive=False)
        ...
        assert any("--user stop work4you.service" in c for c in cmds_joined)
        assert any("--user disable work4you.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (work4you-gateway-coder.service)
        ...
        profile_unit = user_dir / "work4you-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "work4you-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `work4you gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `work4you gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import work4you_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "work4you_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_work4you_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "work4you_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_work4you_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_work4you_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_work4you_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_work4you_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_work4you_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "work4you-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "work4you-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "work4you-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``work4you gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "work4you-gateway")
        ...
        assert "sudo systemctl start work4you-gateway" in out
        assert "sudo work4you gateway uninstall --system" in out
        assert "work4you gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``work4you gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (WORK4YOU_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_work4you_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".work4you"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_work4you_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_work4you_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.work4you.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.work4you.gateway"
