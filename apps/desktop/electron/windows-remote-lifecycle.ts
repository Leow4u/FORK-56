...
const READY_RE = /^WORK4YOU_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitWork4YouPath = '') {
  const explicit = psLiteral(explicitWork4YouPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$work4youHome=$env:WORK4YOU_HOME',
    'if(-not $work4youHome){$work4youHome=Join-Path $env:LOCALAPPDATA "work4you"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command work4you.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $work4youHome "work4you\\venv\\Scripts\\work4you.exe")',
    '$candidates+=(Join-Path $HOME "work4you\\.venv\\Scripts\\work4you.exe")',
    '$work4you=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $work4you){throw "Work4You is not installed on the remote Windows host."}',
    'if($explicit -and $work4you -ne $explicit){throw "The configured Work4You path is not an executable file."}',
    '$python=Join-Path (Split-Path $work4you) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Work4You Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;work4youHome=$work4youHome;work4youPath=$work4you;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitWork4YouPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitWork4YouPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'work4you_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.work4youPath === 'string' &&
    typeof lock.work4youHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.work4youPath === runtime.work4youPath &&
    lock.work4youHome === runtime.work4youHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteWork4YouPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.work4youPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Work4You on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.work4youPath = inspection.path
  const work4youVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located work4you at ${runtime.work4youPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, work4youPath: runtime.work4youPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    work4youPath: runtime.work4youPath,
    work4youHome: runtime.work4youHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForWork4You(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Work4You SSH"', 'powershell.exe -NoLogo')
