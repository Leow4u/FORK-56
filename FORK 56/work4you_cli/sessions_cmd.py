"""``work4you sessions`` command — extracted from ``work4you_cli/main.py``.
...
Helpers that stay in ``work4you_cli.main`` (``get_work4you_home``,
...
``work4you_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_work4you_home():
    return _m().get_work4you_home()
...
#: Default age floor for `work4you sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`work4you sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from work4you_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    work4you sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from work4you_cli.session_recovery import (
    ...
    if action == "import":
        from work4you_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from work4you_state import workspace_key as _ws_key
        ...
        from work4you_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from work4you_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_work4you_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from work4you_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_work4you_home() / "session-exports"
        ...
        # `work4you sessions prune`: no time window and no filters at all
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `work4you sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: work4you sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`work4you sessions optimize` later to reclaim freed space.)")
