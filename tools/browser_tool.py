...
supports multiple backends — **Browser Use** (cloud, default for Work4You
...
# means a compromised transitive dependency could read every Work4You secret
...
def _build_browser_env() -> dict:
    ...
    Strips Work4You-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``work4you_subprocess_env`` import is deferred to keep ``browser_tool``
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    work4you_node_bin = str(work4you_home / "node" / "bin")
    work4you_node_root = str(work4you_home / "node")
    work4you_nm_bin = str(work4you_home / "node_modules" / ".bin")
    return [work4you_node_bin, work4you_node_root, work4you_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``work4you doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.work4you/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Work4You profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Work4You gateway or
    ...
    browser plugins (``~/.work4you/plugins/browser/<vendor>/``) participate
    ...
    try:
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            ...
            if provider_key in ("local", "camofox"):
                ...
                _cloud_provider_resolved = True
                return None
            if provider_key == "work4you":
                # Managed "Work4You Subscription" selection is serviced by the
    ...
    if resolved is None and provider_key is None:
        ...
        # was ever written: Browser Use first (managed Work4You gateway or
...
# comparison sites (four here, plus work4you_cli/tools_config.py and
# work4you_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Work4You silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Work4You-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Work4You CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-work4you_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a work4you
...
# Hard ceiling for a daemon whose owning work4you process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed work4you processes — this way every clean work4you exit sweeps
    ...
    # Sweep orphans from other crashed work4you processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live work4you processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current work4you PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live work4you process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    work4you restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning work4you process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced work4you PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent work4you instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-work4you_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live work4you process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Work4You-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Work4You-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_work4you_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Work4You-managed
    ...
    # node_modules binary, which disappears on the next ``work4you update`` and
    ...
    # Build an extended search PATH including Work4You-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_work4you_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_work4you_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_work4you_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``work4you update`` (or ``work4you doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Work4You holds, while running registry-fetched
    npm code on every ``work4you update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `work4you update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this work4you PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_work4you_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = work4you_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Work4You
    ...
    from work4you_constants import get_work4you_dir
    screenshots_dir = get_work4you_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/work4you/work4you:latest")
                else:
