...
def _is_work4you_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Work4You ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Work4You gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``work4you update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_work4you_gateway_run_argv(argv):
        return argv
