"""Implementation of the ``work4you bundles`` CLI subcommand.

Mirrors the structure of ``work4you_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `work4you bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  work4you bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``work4you bundles`` argparse tree.

    Called from ``work4you_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `work4you bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``work4you bundles <subcommand>`` to the right handler."""
