...
import type { Work4YouRepoStatus } from '@/global'
...
const sampleStatus: Work4YouRepoStatus = {
...
const otherStatus: Work4YouRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<Work4YouRepoStatus | null>) {
  ;(window as unknown as { work4youDesktop?: unknown }).work4youDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: Work4YouRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: Work4YouRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
...
describe('repoChangeKindForPath', () => {
  it('does not notify a row when only another path changes', () => {
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'b.ts', untracked: true } as Work4YouRepoStatus['files'][number]]
      }
    })
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'a.ts', untracked: true } as Work4YouRepoStatus['files'][number]]
      }
    })
