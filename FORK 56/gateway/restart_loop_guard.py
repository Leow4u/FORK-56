...
Defenses 1 and 2 (the ``_WORK4YOU_GATEWAY`` guard on ``work4you gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.work4you.gateway")``,
...
State lives in ``<WORK4YOU_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_work4you_home() / "gateway" / "restart_loop.json"
