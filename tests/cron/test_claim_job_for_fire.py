...
These exercise the real store against a temp WORK4YOU_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated WORK4YOU_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_work4you_home() reads the env live.
    yield tmp_path
