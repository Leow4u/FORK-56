...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Work4You peer ID in OpenViking (default: work4you)
...
_DEFAULT_AGENT = "work4you"
_AGENT_PROMPT_LABEL = "Work4You peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "WORK4YOU_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Work4You slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Work4You to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Work4You refused the connection."
        ) from exc
...
def _load_work4you_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("WORK4YOU_HOME", "")).expanduser() if os.environ.get("WORK4YOU_HOME") else Path.home() / ".work4you"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Work4You did not start "
    ...
    try:
        ...
        # The Work4You Desktop backend can include the Work4You venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Work4You venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Work4You venv,
        # aborting `work4you update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Work4You will retry on a later access or when "
        "the config changes."
    )
...
def _save_work4you_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See work4you#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_work4you_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Work4You peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], work4you_home: str) -> None:
    ...
    def post_setup(self, work4you_home: str, config: dict) -> None:
        ...
        work4you_home_path = Path(work4you_home)
        env_path = work4you_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_work4you_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        work4you_home = str(kwargs.get("work4you_home") or "").strip()
        if not work4you_home:
            try:
                from work4you_constants import get_work4you_home
                work4you_home = str(get_work4you_home())
            except Exception:
                work4you_home = str(Path.home() / ".work4you")
        self._work4you_home = work4you_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Work4You will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.work4you/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts work4you (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Work4You will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._work4you_home:
            return None
        return Path(self._work4you_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._work4you_home:
            return None
        return Path(self._work4you_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_work4you_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Work4You' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Work4You canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See work4you#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/work4you/.overview.md).
