...
Work4You automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.work4you/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Work4You stores session history so it can resume conversations, but it does not
...
Work4You explicitly injects for that turn.
...
For example, if a user sends an image and asks Work4You to make a meme from it,
Work4You may inspect that image once with vision and run an image-processing
...
`work4you sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `work4you sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`work4you` or `work4you chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.work4you/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `work4you -c` again in the *same* terminal, Work4You resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
work4you -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
work4you -c "my project"   # → resumes "my project #3"
```
...
work4you --resume 20250305_091523_a1b2c3d4
work4you -r 20250305_091523_a1b2c3d4

# Resume by title
work4you --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
work4you --resume latest

# Or with the chat subcommand
work4you chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `work4you sessions list`.
...
work4you --resume latest --in ./my-project

# Works with the TUI too
work4you --tui --resume latest --in ./my-project
```
...
```bash
work4you --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`work4you sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
work4you sessions list --workspace my-project
work4you sessions list --workspace ~/code/work4you
```

### Conversation Recap on Resume

When you resume a session, Work4You displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Work4You session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.work4you/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `work4you -r "<title>"` from the shell) and pick up where the platform left off.
...
Work4You automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `work4you sessions list` or `work4you sessions browse`.
...
```bash
work4you sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Work4You creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`work4you -c "my project"`), it automatically picks the most recent session in the lineage.
...
Work4You provides a full set of session management commands via `work4you sessions`:
...
# Filter by platform
work4you sessions list --source telegram

# Show more sessions
work4you sessions list --limit 50
```
...
`work4you sessions export` is one surface for every export format, selected with `--format`:
...
work4you sessions export backup.jsonl

# Export sessions from a specific platform
work4you sessions export telegram-history.jsonl --source telegram

# Export a single session
work4you sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
work4you sessions export backup.jsonl --redact
```
...
work4you sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
work4you sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
work4you sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
work4you sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `work4you-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
work4you sessions export --format trace

# One session to a local trace file
work4you sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
work4you sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.work4you/session-exports`).

```bash
# Export one session to Markdown
work4you sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
work4you sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
work4you sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
work4you sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
work4you sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
work4you sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
work4you sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
work4you sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
work4you sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
work4you sessions pin 20250305_091523_a1b2c3d4
work4you sessions pin 20250305 20250306

# Remove the pin
work4you sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
work4you sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
work4you sessions pinned --json > pinned-sessions.json
```
...
work4you sessions prune

# Custom age threshold — bare numbers are days
work4you sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
work4you sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
work4you sessions prune --newer-than 5h

# Explicit window with absolute timestamps
work4you sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
work4you sessions prune --source telegram
work4you sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
work4you sessions prune --newer-than 5h --title "smoke test"   # title substring
work4you sessions prune --older-than 30 --max-messages 3        # tiny sessions
work4you sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
work4you sessions prune --model gpt-5 --older-than 1w           # by model (substring)
work4you sessions prune --provider openrouter --older-than 60   # by billing provider
work4you sessions prune --branch feature/old-experiment         # by git branch
work4you sessions prune --user 12345678 --chat-type group       # by messaging origin
work4you sessions prune --max-tokens 500 --older-than 7         # by token usage
work4you sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
work4you sessions prune --newer-than 5h --dry-run

# Skip confirmation
work4you sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `work4you sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `work4you sessions prune` keeps the 90-day cutoff. Every
...
`work4you sessions archive` takes the same filters as `prune` but soft-hides
...
work4you sessions archive --newer-than 5h

# Archive by title substring, preview first
work4you sessions archive --title "dry run" --dry-run
work4you sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `work4you sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`work4you sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
work4you sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`work4you insights`](/reference/cli-commands#work4you-insights).
...
`work4you sessions repair-routing` finds message-bearing session rows with no
...
work4you sessions repair-routing
...
work4you sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
work4you sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.work4you/state.db ~/.work4you/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Work4You and
continue it here. Work4You reads Claude Code's session logs
...
work4you sessions import

# Limit to one tool, or point at a specific file
work4you sessions import --from claude
work4you sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
work4you --resume @claude
work4you --resume @codex
```

`work4you sessions import` creates a new Work4You session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `work4you --resume <id>` command.
...
When Work4You cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Work4You uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Work4You in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.work4you/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.work4you/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.work4you/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.work4you/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.work4you/sessions/sessions.json` is a **legacy mirror** of it,
...
`work4you sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.work4you/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `work4you sessions list`, the cause is
`state.db` not receiving them — run `work4you sessions repair` and watch for a
...
`*.jsonl` files in `~/.work4you/sessions/`. They are no longer written or
read by Work4You. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Work4You process in the same `WORK4YOU_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.work4you/config.yaml`:
...
work4you sessions delete <session_id>
