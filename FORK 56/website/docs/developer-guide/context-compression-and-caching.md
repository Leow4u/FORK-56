...
Work4You uses a dual compression system and Anthropic prompt caching to
...
Configure via `work4you plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Work4You has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|work4you|off for Codex app-server thread compaction
...
auxiliary:
  compression:
    ...
    provider: auto           # Provider: "auto", "openrouter", "work4you", "main", etc.
...
| `codex_app_server_auto` | `native` | `native`, `work4you`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Work4You raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$WORK4YOU_HOME`
...
```bash
work4you config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
work4you config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Work4You' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Work4You
  records the resulting compaction events (compression counters, session
  events). Set `work4you` to let Work4You' compression threshold initiate
  app-server compaction, or `off` to disable Work4You-initiated automatic
  compaction entirely (codex may still compact natively).

Work4You' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Work4You' summary compressor.
...
encrypted `compaction` output item. Work4You captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Work4You
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Work4You can avoid; user-facing docs for `/model`, fallback providers, and
