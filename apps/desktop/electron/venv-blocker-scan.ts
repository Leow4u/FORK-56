...
const SCAN_MODULE = 'work4you_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Work4You processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'work4you_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Work4You process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Work4You installation is free.\n' +
    '\n' +
    'Close other Work4You windows and terminals, then retry.  If the problem\n' +
    'persists, run `work4you update` in a terminal for detailed diagnostics.'
  )
}
