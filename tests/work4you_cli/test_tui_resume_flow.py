...
def main_mod(monkeypatch):
    import work4you_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_work4you_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import work4you_cli.oneshot as oneshot
        from work4you_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """work4you -z bypasses Work4YouCLI, but recall still needs SessionDB."""
    from work4you_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "work4you_state", mod("work4you_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "work4you_cli.config",
        mod("work4you_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "work4you_cli.models",
        mod("work4you_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "work4you_cli.tools_config",
        mod("work4you_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["WORK4YOU_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="work4you/work4you-test", provider="work4you", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["WORK4YOU_MODEL"] == "work4you/work4you-test"
    assert env["WORK4YOU_INFERENCE_MODEL"] == "work4you/work4you-test"
    assert env["WORK4YOU_TUI_PROVIDER"] == "work4you"
    assert env["WORK4YOU_INFERENCE_PROVIDER"] == "work4you"
    assert env["WORK4YOU_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["WORK4YOU_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("work4you-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_work4you_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("WORK4YOU_TUI_DIR", raising=False)
