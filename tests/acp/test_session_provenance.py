...
def test_root_session_no_compression(db):
    ...
    assert prov["currentWork4YouSessionId"] == "root1"
    assert prov["rootWork4YouSessionId"] == "root1"
    assert prov["parentWork4YouSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_work4you_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentWork4YouSessionId"] == "old"
    assert prov["rootWork4YouSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousWork4YouSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootWork4YouSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"work4you"}
    assert "sessionProvenance" in meta["work4you"]
    assert meta["work4you"]["sessionProvenance"]["currentWork4YouSessionId"] == "root1"
