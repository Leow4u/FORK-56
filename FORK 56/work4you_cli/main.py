...
# IMPORTANT: work4you_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``work4you_bootstrap`` is a
...
# new code references ``work4you_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, work4you crashes on import and the user can't run
...
from work4you_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python work4you_cli/main.py`` (script
# mode — sys.path[0] is work4you_cli/, not the repo root) can import the
# canonical helpers; everything else lives in work4you_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from work4you_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``work4you update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from work4you_cli.env_loader import ...`` / ``from work4you_cli.config import
...
# package directory, so if IT can't import, nothing else in work4you_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from work4you_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from work4you_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'work4you' so tools like 'ps', 'top', and
    ...
    Strategy (try in order):
      ...
      4. No-op on Windows (the .exe name is already ``work4you.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("work4you")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"work4you", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"work4you")
        # Windows: the .exe name is already ``work4you.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* work4you_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("WORK4YOU_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".work4you", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``WORK4YOU_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``work4you … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("WORK4YOU_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``WORK4YOU_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("WORK4YOU_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _active_profile_may_override_home_fast(work4you_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(work4you_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``work4you --version`` before config/logging imports."""
...
from work4you_cli.subcommands.sync import build_sync_parser
from work4you_cli.subcommands.gateway import build_gateway_parser
from work4you_cli.subcommands.profile import build_profile_parser
from work4you_cli.subcommands.model import build_model_parser
from work4you_cli.subcommands.setup import build_setup_parser

from work4you_cli.subcommands.whatsapp import build_whatsapp_parser
from work4you_cli.subcommands.slack import build_slack_parser
from work4you_cli.subcommands.login import build_login_parser
from work4you_cli.subcommands.logout import build_logout_parser
from work4you_cli.subcommands.auth import build_auth_parser
from work4you_cli.subcommands.status import build_status_parser
from work4you_cli.subcommands.pause import build_pause_parser
from work4you_cli.subcommands.webhook import build_webhook_parser
from work4you_cli.subcommands.hooks import build_hooks_parser
from work4you_cli.subcommands.doctor import build_doctor_parser
from work4you_cli.subcommands.verify import build_verify_parser
from work4you_cli.subcommands.security import build_security_parser
from work4you_cli.subcommands.approvals import build_approvals_parser
from work4you_cli.subcommands.dump import build_dump_parser
from work4you_cli.subcommands.debug import build_debug_parser
from work4you_cli.subcommands.backup import build_backup_parser
from work4you_cli.subcommands.import_cmd import build_import_cmd_parser
from work4you_cli.subcommands.import_agent import build_import_agent_parser
from work4you_cli.subcommands.config import build_config_parser
from work4you_cli.subcommands.skin import build_skin_parser
from work4you_cli.subcommands.console import build_console_parser
from work4you_cli.subcommands.version import build_version_parser
from work4you_cli.subcommands.update import build_update_parser
from work4you_cli.subcommands.uninstall import build_uninstall_parser
from work4you_cli.subcommands.dashboard import build_dashboard_parser
from work4you_cli.subcommands.gui import build_gui_parser
from work4you_cli.subcommands.logs import build_logs_parser
from work4you_cli.subcommands.prompt_size import build_prompt_size_parser
from work4you_cli.subcommands.memory import build_memory_parser
from work4you_cli.subcommands.acp import build_acp_parser
from work4you_cli.subcommands.tools import build_tools_parser
from work4you_cli.subcommands.insights import build_insights_parser
from work4you_cli.subcommands.monitoring import build_monitoring_parser
from work4you_cli.subcommands.skills import build_skills_parser
from work4you_cli.subcommands.pairing import build_pairing_parser
from work4you_cli.subcommands.plugins import build_plugins_parser
from work4you_cli.subcommands.mcp import build_mcp_parser
from work4you_cli.subcommands.claw import build_claw_parser
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any work4you module import.
#
# Many modules cache WORK4YOU_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("WORK4YOU_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.work4you/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set WORK4YOU_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Work4You' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".work4you" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # Mirrors work4you_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If WORK4YOU_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.work4you/profiles/coder or
    # /opt/data/profiles/coder).  If WORK4YOU_HOME points to the work4you root
    # instead (e.g. systemd hardcodes WORK4YOU_HOME=/root/.work4you), we must
    # still read active_profile — the user may have switched profiles via
    # `work4you profile use` and the gateway should honour that choice.
    # See issue #22502.
    work4you_home_env = os.environ.get("WORK4YOU_HOME", "")
    if profile_name is None and work4you_home_env:
        if Path(work4you_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the work4you root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as WORK4YOU_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # "the root WORK4YOU_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("WORK4YOU_S6_SUPERVISED_CHILD"):
        try:
            from work4you_constants import get_default_work4you_root

            active_path = get_default_work4you_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set WORK4YOU_HOME
    if profile_name is not None:
        try:
            from work4you_cli.profiles import resolve_profile_env

            work4you_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            work4you_home = _resolve_sudo_user_profile_env(profile_name)
            if not work4you_home:
            ...
            # A bug in profiles.py must NEVER prevent work4you from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["WORK4YOU_HOME"] = work4you_home
...
# Load .env from ~/.work4you/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from work4you_cli.config import get_work4you_home
from work4you_cli.env_loader import load_work4you_dotenv
...
load_work4you_dotenv(
    project_env=PROJECT_ROOT / ".env",
    load_external_secrets=sys.argv[1:2] != ["update"],
)

# Bridge security.redact_secrets from config.yaml → WORK4YOU_REDACT_SECRETS env
# var BEFORE work4you_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves work4you_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from work4you_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_work4you_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "WORK4YOU_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["WORK4YOU_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `work4you` subcommands
...
try:
    from work4you_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from work4you_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if work4you_constants not importable yet
...
# Provider model-selection wizard flows extracted to work4you_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (work4you_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_work4you_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("WORK4YOU_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("WORK4YOU_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`work4you_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from work4you_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from work4you_cli.config import get_env_path, get_work4you_home, load_config
    from work4you_cli.auth import get_auth_status

    # Determine whether Work4You itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from work4you_cli.config import split_model_config_default
        ...
        _model_name = ""
    _has_work4you_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Work4You Portal OAuth credentials
    auth_file = get_work4you_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Work4You has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Work4You to use their tokens.
    if _has_work4you_config:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from work4you_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_browse_picker(sessions: list, session_db=None) -> Optional[str]:
    ...
    def _delete_session(session_id: str) -> bool:
        if session_db is None:
            return False
        try:
            sessions_dir = get_work4you_home() / "sessions"
        except Exception:
            sessions_dir = None
        try:
            return bool(
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``work4you -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, work4you_bin
        cli_args: the original CLI arguments (everything after 'work4you')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    work4you_bin = container_info["work4you_bin"]
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'work4you sessions list' to see available sessions, or "
            ...
            try:
                from work4you_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @work4you/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@work4you" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/work4you-ink/package.json",
    "packages/work4you-ink/index.js",
    "packages/work4you-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``WORK4YOU_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("WORK4YOU_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``WORK4YOU_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("WORK4YOU_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    work4you_home = str(get_work4you_home())
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "WORK4YOU_HOME": work4you_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(work4you_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(work4you_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if work4you_cli_dir is None:
        work4you_cli_dir = Path(__file__).parent
    bundled = work4you_cli_dir / "tui_dist" / "entry.js"
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("WORK4YOU_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (WORK4YOU_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("WORK4YOU_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $WORK4YOU_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Work4You installed,
    ...
    ext_dir = os.environ.get("WORK4YOU_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with WORK4YOU_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset WORK4YOU_TUI_DIR (e.g. `unset WORK4YOU_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # work4you_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from work4you_constants import with_work4you_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_work4you_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Work4You-managed
            ...
            from work4you_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @work4you/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @work4you/ink resolves through
        # packages/work4you-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "work4you-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from work4you_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("WORK4YOU_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["WORK4YOU_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("WORK4YOU_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["WORK4YOU_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("WORK4YOU_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["WORK4YOU_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["WORK4YOU_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a work4you process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from work4you_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="work4you-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["WORK4YOU_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["WORK4YOU_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["WORK4YOU_MODEL"] = model
        env["WORK4YOU_INFERENCE_MODEL"] = model
    if provider:
        env["WORK4YOU_TUI_PROVIDER"] = provider
        env["WORK4YOU_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["WORK4YOU_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["WORK4YOU_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["WORK4YOU_TUI_SKILLS"] = value
    if query:
        env["WORK4YOU_TUI_QUERY"] = query
    if image:
        env["WORK4YOU_TUI_IMAGE"] = image
    if checkpoints:
        env["WORK4YOU_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["WORK4YOU_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["WORK4YOU_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["WORK4YOU_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["WORK4YOU_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["WORK4YOU_ACCEPT_HOOKS"] = "1"
    ...
    # WORK4YOU_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set WORK4YOU_TUI_RESUME themselves.
    env.pop("WORK4YOU_TUI_RESUME", None)
    if resume_session_id:
        env["WORK4YOU_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``WORK4YOU_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("WORK4YOU_KANBAN_BOARD"):
        return
    try:
        from work4you_cli.kanban_db import get_current_board

        os.environ["WORK4YOU_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.work4you/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Work4You — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Work4You still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``work4you … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("WORK4YOU_TUI") == "1":
        return True
    try:
        from work4you_cli.config import load_config
...
def cmd_chat(args):
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: work4you --resume {_imported_id})")
    ...
    try:
        from work4you_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from work4you_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from work4you_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    # rare launch right after `work4you update` changed a bundled skill.
    ...
    if getattr(args, "yolo", False):
        os.environ["WORK4YOU_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.work4you/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["WORK4YOU_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["WORK4YOU_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["WORK4YOU_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from work4you_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from work4you_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    session_dir = get_work4you_home() / "whatsapp" / "session"
...
def cmd_whatsapp_cloud(args):
    ...
    ``work4you_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from work4you_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def select_provider_and_model(args=None):
    ...
    from work4you_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("WORK4YOU_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # PROVIDER_GROUPS in work4you_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``work4you model``
    ...
    # matching against work4you_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "work4you":
    ...
    # a leftover OPENAI_BASE_URL in ~/.work4you/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.work4you/.env if the active provider is not 'custom'.
    ...
    """
    from work4you_cli.config import get_env_value, save_env_value, load_config
...
#
# Work4You uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`work4you_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Work4You Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``work4you model`` flow, then route aux tasks
    to them here.
    """
    from work4you_cli.config import load_config
    from work4you_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from work4you_cli.auth import _prompt_model_selection
    from work4you_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from work4you_cli.setup import _curses_prompt_choice
...
# downstream call sites read `work4you_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `work4you` invocation — including
...
# surface so argparse wiring and test monkeypatches on work4you_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `work4you` invocation, including fast paths like `work4you --version`
...
_LAZY_COMMAND_EXPORTS = {
    ...
    "work4you_cli.dashboard_procs": (
        "_detect_concurrent_work4you_instances",
...
def _self():
    ...
    keeps monkeypatches on work4you_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    recover from a malformed paste without editing ``~/.work4you/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from work4you_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from work4you_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from work4you_constants import display_work4you_home as _dhh_fn
...
def cmd_login(args):
    """Authenticate Work4You CLI with a provider."""
    from work4you_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from work4you_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from work4you_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from work4you_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from work4you_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.work4you/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `work4you sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Work4You Portal — sync is inert.", file=sys.stderr)
        elif not status.get("work4you_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set WORK4YOU_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "WORK4YOU_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        return 1
    if not identity.get("work4you_admin"):
        print(
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="work4you sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="work4you sync now")
...
def cmd_webhook(args):
    ...
    from work4you_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from work4you_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from work4you_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from work4you_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from work4you_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from work4you_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from work4you_cli.verify_cmd import run_verify_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from work4you_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `work4you approvals <subcmd>`."""
    from work4you_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from work4you_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from work4you_cli.config import config_command
...
def cmd_skin(args):
    ...
    from work4you_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Work4You home directory to a zip file."""
    if getattr(args, "quick", False):
        from work4you_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Work4You backup from a zip file."""
    from work4you_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from work4you_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from work4you_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from work4you_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from work4you_cli.uninstall import run_gui_uninstall
    ...
    from work4you_cli.uninstall import run_uninstall
...
# Update pipeline lives in work4you_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on work4you_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in WORK4YOU_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``work4you update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``work4you update``
    ...
    ``work4you`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``work4you update``
    ...
    ``work4you_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $WORK4YOU_HOME."""
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``work4you`` launcher present but ``work4you_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the work4you venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — work4you venv python3, no nix-shell overhead
    ...
    # a work4you venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Work4You-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_work4you_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "work4you_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m work4you_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_work4you_node_path())
    ...
    # as `work4you update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (work4you web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#
# Stamp file: $WORK4YOU_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $WORK4YOU_HOME."""
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Work4You.app/Contents/MacOS/Work4You → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``work4you desktop`` skips the rebuild that would fix
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Work4You.app/Contents/MacOS/Work4You"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Work4You.exe",
            release_dir / "win-ia32-unpacked" / "Work4You.exe",
            release_dir / "win-arm64-unpacked" / "Work4You.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "work4you",
            release_dir / "linux-unpacked" / "Work4You",
            release_dir / "linux-arm64-unpacked" / "work4you",
            release_dir / "linux-arm64-unpacked" / "Work4You",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Work4You.exe
...
#
# The desktop self-update chain (Desktop → work4you-setup --update →
# `work4you update` → `work4you desktop --build-only` → relaunch) rebuilds
# Work4You.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Work4You.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Work4You.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 work4you-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Work4You.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Work4You.exe from backup.")
        print("    Your existing version was kept and still works. Run `work4you desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `work4you desktop --force-build` to rebuild, or re-run the Work4You")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Work4You'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_work4you_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Work4You.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Work4You.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Work4You.exe: Access is
    ...
    an unrelated "Work4You" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Work4You.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Work4You is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Work4You.app/Contents/MacOS/Work4You  ->  app bundle = .../Work4You.app
    app = exe.parents[2]
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `work4you desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the WORK4YOU_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Work4You Desktop (Linux only, best-effort).
    ...
    shell. ``work4you uninstall --gui`` removes it.
    """
    try:
        from work4you_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from work4you_logging import setup_logging as _setup_logging_gui
    ...
    # with_work4you_node_path() copies os.environ when called with no arg.
    env = with_work4you_node_path()
    if getattr(args, "fake_boot", False):
        env["WORK4YOU_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["WORK4YOU_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "work4you_root", None):
        env["WORK4YOU_DESKTOP_WORK4YOU_ROOT"] = str(Path(args.work4you_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["WORK4YOU_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["WORK4YOU_DESKTOP_CWD"] = os.getcwd()
    ...
    # `WORK4YOU_DESKTOP_DISABLE_GPU=... work4you desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "WORK4YOU_DESKTOP_DISABLE_GPU" not in os.environ:
        env["WORK4YOU_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `WORK4YOU_DESKTOP_PASSWORD_STORE=... work4you desktop` keeps working.
    if sys.platform == "linux" and "WORK4YOU_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["WORK4YOU_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Work4You-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → work4you-setup →
            # work4you update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `work4you update` path).
            nixos_env = with_work4you_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Work4You.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Work4You.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Work4You.exe, close any")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Work4You is
                ...
                # success on a Work4You.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Work4You shows up
...
# Dashboard process-hygiene helpers live in work4you_cli/dashboard_procs.py
...
# work4you_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "work4you dashboard",
            "work4you_cli.main dashboard",
            "work4you_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "work4you serve",
            "work4you_cli.main serve",
            "work4you_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "work4you-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Work4You installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``work4you-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/work4you-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but work4you launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``work4you update``.
    ...
    log_path = get_work4you_home() / "logs" / "dashboard-restart.log"
    try:
...
# Install-scoped breadcrumbs live next to the venv (not under $WORK4YOU_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``work4you update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``work4you.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_work4you_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from work4you.exe; applying package-only first aid, "
    ...
    try:
        from work4you_cli import _install_repair as _ir
        ...
        from work4you_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Work4You is still running from the launcher that needs "
                "replacing. Close other Work4You windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\work4you.exe`` is a launcher that runs the interpreter with
    ...
    ``work4you ...`` command holds its own shim, and an editable install run
    ...
    ``work4you.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _work4you_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_work4you_launcher_locked() -> bool:
    """True when a venv ``work4you*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "WORK4YOU_UPDATE_REEXEC"
...
def _reexec_update_off_windows_shim() -> bool:
    ...
    from work4you_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "work4you_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from work4you_constants import project_venv_dir
...
def _venv_scripts_dir() -> Path | None:
    ...
    from work4you_constants import project_venv_dir, venv_bin_dir
...
def _work4you_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"work4you", "work4you", "work4you-acp"}
...
def _quarantine_running_work4you_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``work4you.exe``.
    ...
    when ``work4you update`` runs, ``work4you.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``work4you.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next work4you invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Work4You Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Work4You Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``work4you.exe`` re-runs itself under the venv Python before
    ...
    for shim in _work4you_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Work4You versions queued for our shims.

    Work4You used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _work4you_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_work4you_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``work4you.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``work4you.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``work4you`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_work4you_exe(scripts_dir)
    try:
        ...
        # quarantined aside and `work4you` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``work4you.exe.old.*`` and stale reboot renames left by prior updates.

    Called early on every work4you invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``work4you.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``work4you.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `work4you update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``work4you.exe`` / ``work4you-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_work4you_exe`` for the rationale.
    """
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``work4you.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``work4you.exe`` shim is locked during ``work4you update``, or when uv/distlib
    skips a launcher write. The symptom is ``work4you.exe`` and
    ``work4you-acp.exe`` present but ``work4you.exe`` missing, so ``work4you`` drops
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m work4you_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``work4you.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``work4you`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.work4you/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.work4you/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # work4you_cli.config.get_work4you_home to simulate setup failure.
        from work4you_cli.config import get_work4you_home as _get_work4you_home

        logs_dir = _get_work4you_home() / "logs"
...
def cmd_update(args):
    """Update Work4You to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `work4you update --branch=<x>` would actually pull.
    ...
    # Windows: an update launched through venv\Scripts\work4you.exe holds that
    ...
    from work4you_cli.update_lock import (
...
def cmd_profile(args):
    ...
    from work4you_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    if action is None:
        ...
        from work4you_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_work4you_home()
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.work4you)")
            else:
        ...
        from work4you_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from work4you_constants import get_work4you_home as _hh
        ...
        from work4you_cli import profile_describer as _pd
        ...
        from work4you_cli.profiles import profile_exists, validate_alias_name
        ...
        from work4you_cli.profiles import normalize_profile_name, rename_profile
        ...
        from work4you_cli.profiles import export_profile
        ...
        from work4you_cli.profiles import import_profile
        ...
        import tempfile
        from work4you_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="work4you_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from work4you_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from work4you_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("work4you_requires"):
            print(f"Requires:     Work4You {data['work4you_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from work4you_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.work4you_requires:
        print(f"  Requires: Work4You {mf.work4you_requires}")
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} work4you dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from work4you_cli.web_server import should_require_auth
    ...
    try:
        from work4you_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Work4You Portal (run `work4you dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "  It provisions a Work4You Portal OAuth client and writes "
            "WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID into ~/.work4you/.env for you.\n"
    ...
    try:
        from work4you_cli.config import load_config, save_config
        from work4you_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from work4you_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.work4you/desktop-ssh: a
    # literal "~/.work4you/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of WORK4YOU_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_work4you_home(): a non-default sticky profile (or any WORK4YOU_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_work4you_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".work4you" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``WORK4YOU_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``work4you dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with work4you serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with WORK4YOU_DESKTOP=1 plus
    # WORK4YOU_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # WORK4YOU_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `work4you dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited WORK4YOU_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # WORK4YOU_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (WORK4YOU_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets WORK4YOU_SERVE_HEADLESS below.
    if os.environ.get("WORK4YOU_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("WORK4YOU_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("WORK4YOU_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("WORK4YOU_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → WORK4YOU_HOME points into
    ...
    if (
        ...
        and os.environ.get("WORK4YOU_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "work4you_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: WORK4YOU_HOME is explicitly pinned to the
        ...
        # WORK4YOU_HOME.  We must resolve the root explicitly instead of just
        # dropping WORK4YOU_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV WORK4YOU_HOME=/opt/data`), so an unset
        # WORK4YOU_HOME falls back to $HOME/.work4you = /opt/data/.work4you — an
        ...
        # update-button guard also misfires).  get_default_work4you_root()
        # returns the root for both layouts: ~/.work4you for a standard install
        ...
        try:
            from work4you_constants import get_default_work4you_root
            env["WORK4YOU_HOME"] = str(get_default_work4you_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop WORK4YOU_HOME) rather than block the reroute.
            env.pop("WORK4YOU_HOME", None)
    ...
    from work4you_cli.resource_limits import apply_nofile_soft_limit
    ...
    # the same logs directory as every other Work4You surface.
    try:
        from work4you_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["WORK4YOU_SERVE_HEADLESS"] = "1"
    elif "WORK4YOU_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["WORK4YOU_WEB_DIST"])
            if "WORK4YOU_WEB_DIST" in os.environ
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom WORK4YOU_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "WORK4YOU_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # WORK4YOU_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["WORK4YOU_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ WORK4YOU_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset WORK4YOU_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads WORK4YOU_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["WORK4YOU_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from WORK4YOU_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/work4you) registers BEFORE start_server's
    ...
    from work4you_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Work4You Portal."""
    from work4you_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from work4you_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from work4you_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from work4you_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Work4You log files."""
    from work4you_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Work4You command console."""
    from work4you_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from work4you_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "work4you", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
# the top-level flags declared in ``work4you_cli/_parser.py``.
#
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``work4you <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``work4you photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("WORK4YOU_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that WORK4YOU_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from work4you_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["WORK4YOU_SAFE_MODE"] = "1"
    os.environ["WORK4YOU_IGNORE_USER_CONFIG"] = "1"
    os.environ["WORK4YOU_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    ``work4you`` / ``work4you -w -s foo --yolo`` / ``work4you chat`` don't need the
    ...
    """
    if os.environ.get("WORK4YOU_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from work4you_cli.config import get_container_exec_info
    ...
    from work4you_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("WORK4YOU_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["WORK4YOU_DEFER_AGENT_STARTUP"] = "1"
            os.environ["WORK4YOU_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["WORK4YOU_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from work4you_constants import get_work4you_home, display_work4you_home
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_work4you_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_work4you_home()}/memories/\n")
    else:
        from work4you_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from work4you_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from work4you_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from work4you_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: work4you[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from work4you_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from work4you_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``work4you skills trust [path]`` / ``work4you skills untrust [path]``.
...
def cmd_pairing(args):
    from work4you_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from work4you_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from work4you_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``work4you`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``WORK4YOU`` is the Work4You-specific marker. setdefault: never
    clobber an outer harness (e.g. Work4You running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "work4you")
    os.environ.setdefault("WORK4YOU", "true")


def main():
    """Main entry point for work4you CLI."""
    # Cosmetic: make the process show up as 'work4you' instead of 'python3.11'
    ...
    # Sweep stale ``work4you.exe.old.*`` quarantine files left by previous
    # ``work4you update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_work4you_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (work4you update, manual
    ...
    # =========================================================================
    # model command  (parser built in work4you_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from work4you_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from work4you_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://work4you.ai/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.work4you/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://work4you.ai/docs/user-guide/secrets/"
        ),
    )
    ...
    from work4you_cli import secrets_cli as _secrets_cli
    from work4you_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `work4you proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://work4you.ai/docs/user-guide/egress/iron-proxy"
        ),
    )

    from work4you_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``work4you proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from work4you_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in work4you_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in work4you_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in work4you_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in work4you_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from work4you_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in work4you_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in work4you_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in work4you_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in work4you_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in work4you_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in work4you_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in work4you_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Work4You gateways)
    # =========================================================================
    from work4you_cli.subcommands.peer import build_peer_parser

    build_peer_parser(subparsers)

    # =========================================================================
    # portal command — Work4You Portal status + Tool Gateway routing
    # =========================================================================
    from work4you_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from work4you_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from work4you_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in work4you_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in work4you_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in work4you_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in work4you_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in work4you_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in work4you_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in work4you_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in work4you_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.work4you/checkpoints/",
    ...
    from work4you_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in work4you_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: work4you_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from work4you_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in work4you_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in work4you_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in work4you_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in work4you_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in work4you_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from work4you_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in work4you_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    try:
        from work4you_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from work4you_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from work4you_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in work4you_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in work4you_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from work4you_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("WORK4YOU_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from work4you_cli.tools_config import _cua_driver_env
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by WORK4YOU_CUA_DRIVER_CMD, or unset "
                            "the override and run: work4you computer-use install --upgrade"
                        )
                    else:
    ...
    # =========================================================================
    # mcp command  (parser built in work4you_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def _add_session_filter_args(p, default_older_help):
        ...
        p.add_argument(
            ...
            "(e.g. 'sonnet', 'gpt-5', 'work4you')",
        )
        p.add_argument(
            "--provider",
            help="Only match sessions billed through this provider "
            "(e.g. openrouter, anthropic, work4you)",
        )
        p.add_argument(
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <work4you home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Work4You",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Work4You session store "
            "so it can be resumed with 'work4you --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in work4you_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on work4you_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in work4you_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in work4you_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in work4you_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in work4you_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in work4you_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in work4you_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in work4you_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in work4you_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Work4You-Setup.exe success screen tells users to
    ...
    # =========================================================================
    # gui command  (parser built in work4you_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in work4you_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in work4you_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # --yolo: set WORK4YOU_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``work4you egress start`` refusing when credential_source=bitwarden
