"""Guard: every `work4you update` path that reports user-modified skills must
also tell the user how to find them.

`work4you update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``work4you_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``work4you skills list-modified`` so the count is actionable — otherwise,
...
import work4you_cli.update_cmd as update_mod
...
def _source_lines() -> list[str]:
    # The update pipeline was extracted to work4you_cli/update_cmd.py
