"""``work4you logs`` — view and filter Work4You log files.
...
Usage examples::
    ...
    work4you logs                    # last 50 lines of agent.log
    work4you logs -f                 # follow agent.log in real time
    work4you logs errors             # last 50 lines of errors.log
    work4you logs gateway -n 100    # last 100 lines of gateway.log
    work4you logs gui -f            # follow gui.log (dashboard/pty/ws)
    work4you logs desktop -f        # follow desktop.log (Electron app boot/backend)
    work4you logs --level WARNING    # only WARNING+ lines
    work4you logs --session abc123   # filter by session ID substring
    work4you logs --component tools  # only tool-related lines
    work4you logs --since 1h         # lines from the last hour
    work4you logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_work4you_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Work4You runs — try 'work4you chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_work4you_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_work4you_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_work4you_home()}/logs/")
        return

    print(f"Log files in {display_work4you_home()}/logs/:\n")
    found = False
