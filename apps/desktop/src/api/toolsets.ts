...
export function getToolsets(profile?: ProfileScope): Promise<ToolsetInfo[]> {
  return window.work4youDesktop.api<ToolsetInfo[]>({
...
export function getToolsetConfig(name: string, profile?: ProfileScope): Promise<ToolsetConfig> {
  return window.work4youDesktop.api<ToolsetConfig>({
    ...capabilityScoped(profile),
...
export function getToolsetModels(
  ...
  return window.work4youDesktop.api<ToolsetModelsResponse>({
    ...capabilityScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.work4youDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...capabilityScoped(profile),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Work4You row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Work4You Portal. */
  needs_work4you_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_work4you_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.work4youDesktop.api<SelectToolsetProviderResponse>({
    ...capabilityScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.work4youDesktop.api<ActionResponse & { key: string }>({
    ...capabilityScoped(profile),
...
export function getTerminalBackends(profile?: ProfileScope): Promise<TerminalBackendsResponse> {
  return window.work4youDesktop.api<TerminalBackendsResponse>({
    ...capabilityScoped(profile),
...
export function selectTerminalBackend(
  ...
  return window.work4youDesktop.api<{ ok: boolean; backend: string }>({
    ...capabilityScoped(profile),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return work4youApi<ComputerUseStatus>({
    ...profileScoped(),
