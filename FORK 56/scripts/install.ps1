...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$Work4YouHome = $(if ($env:WORK4YOU_HOME) { $env:WORK4YOU_HOME } else { "$env:LOCALAPPDATA\work4you" }),
    [string]$InstallDir = $(if ($env:WORK4YOU_HOME) { "$env:WORK4YOU_HOME\work4you" } else { "$env:LOCALAPPDATA\work4you\work4you" }),
    ...
    # builds apps/desktop into a launchable Work4You.exe.
    #
    # Why opt-in:
    #   * Work4You-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Work4You.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Work4You.exe
...
# WORK4YOU_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[work4you] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'Work4YouInstall.LongPath').Type) {
            Add-Type -Namespace 'Work4YouInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [Work4YouInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -Work4YouHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('Work4YouHome')) {
    $Work4YouHome = ConvertTo-LongPath $Work4YouHome
} else {
    $Work4YouHome = ConvertTo-LongPath $(
        if ($env:WORK4YOU_HOME) { $env:WORK4YOU_HOME } else { "$env:LOCALAPPDATA\work4you" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:WORK4YOU_HOME) { "$env:WORK4YOU_HOME\work4you" } else { "$env:LOCALAPPDATA\work4you\work4you" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: Work4YouHome=$Work4YouHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:Leow4u/FORK-56.git"
$RepoUrlHttps = "https://github.com/Leow4u/FORK-56.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Work4You.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $Work4YouHome)) {
        New-Item -ItemType Directory -Force -Path $Work4YouHome | Out-Null
    }
    $envFile = Join-Path $Work4YouHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `work4you update`'s npx
    ...
    $prefixDir = Join-Path $Work4YouHome "node"
...
function Install-Uv {
    # Work4You owns its own uv at $Work4YouHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (work4you_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `work4you update` stay in sync.
    $managedUv = Join-Path $Work4YouHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $Work4YouHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $Work4YouHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $Work4YouHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $Work4YouHome "bin"
        ...
        # (work4you_cli/managed_uv.py looks only at $Work4YouHome\bin\uv.exe).
...
# Put the Work4You-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone work4you-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Work4You-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Work4You-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("work4you-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and WORK4YOU_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Work4You-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Work4You setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Work4You uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\work4you\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Work4You needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Work4You copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\work4you\git``
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Work4You-managed PortableGit install instead..."
    }

    # Download PortableGit into $Work4YouHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $Work4YouHome\git\ ..."
...
function Set-GitBashEnvVar {
    ...
    ``WORK4YOU_GIT_BASH_PATH`` (User env scope) so Work4You can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $Work4YouHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $Work4YouHome\git\usr\bin\bash.exe
    $candidates += "$Work4YouHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$Work4YouHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Work4You requires Node >=26)"
    }

    # Prefer a Work4You-managed Node from a previous run over a too-old system one.
    $managedNode = "$Work4YouHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$Work4YouHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$Work4YouHome\node"
        ...
        Update-ManagedNpm "$Work4YouHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $Work4YouHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $Work4YouHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\work4you-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$Work4YouHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$Work4YouHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$Work4YouHome\node") {
                    try {
                        Rename-Item "$Work4YouHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Work4You-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$Work4YouHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $Work4YouHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\work4you-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `work4you
                    # update` path (#4735).
                    ...
                    $stashName = "work4you-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. work4you-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``work4you update`` and
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\work4you-$zipLabel.zip"
                $extractPath = "$env:TEMP\work4you-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `work4you update` (see the notes at the shared clone-path
    ...
    # next `work4you update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Work4You-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running work4you process.
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Work4You_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (work4you.exe) plus its child tree.
            & taskkill /F /T /IM work4you.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM work4you.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m work4you_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not work4you.exe.
            ...
            # Work4You children re-exec through .work4you-runtime, so killing only
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Work4You " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `work4you update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Work4You (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `work4you.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install work4you package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and work4you.exe broken with
    ...
    # here, not 30 seconds later when the user runs `work4you`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Work4You processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Work4You processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Work4You and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register work4you.exe in dist-info/RECORD but fail to
        ...
        # `work4you` command while work4you.exe / work4you-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `work4you dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\work4you_cli\web_server.py" 2>&1 | Out-Null
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: work4you_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        $work4youBin = "$InstallDir"
    } else {
        # Expose ONLY the work4you launchers on PATH -- never the whole
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept work4you via $work4youBin)"
    }
    
    if ($currentPath -notlike "*$work4youBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$work4youBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $work4youBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set WORK4YOU_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\work4you instead
    # of the Unix default ~/.work4you
    $currentWork4YouHome = [Environment]::GetEnvironmentVariable("WORK4YOU_HOME", "User")
    if (-not $currentWork4YouHome -or $currentWork4YouHome -ne $Work4YouHome) {
        [Environment]::SetEnvironmentVariable("WORK4YOU_HOME", $Work4YouHome, "User")
        Write-Success "Set WORK4YOU_HOME=$Work4YouHome"
    }
    $env:WORK4YOU_HOME = $Work4YouHome
    
    # Update current session
    $env:Path = "$work4youBin;$env:Path"
    
    Write-Success "work4you command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.work4you-bootstrap-complete which tells the Work4You
    ...
    # Work4You-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `work4you update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".work4you-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the WORK4YOU_HOME directory structure ($Work4YouHome, default %LOCALAPPDATA%\work4you)
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Work4YouHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$Work4YouHome\.env"
    ...
    # Create config.yaml
    $configPath = "$Work4YouHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Work4You's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$Work4YouHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in work4you_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $Work4YouHome"
    
    # Seed bundled skills into $Work4YouHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $Work4YouHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $Work4YouHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$Work4YouHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $Work4YouHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Work4You-Setup.exe runs each -Stage NAME
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\work4you-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\work4you-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\work4you-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `work4you tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $Work4YouHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Work4You' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Work4You' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'work4you tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `work4you
...
# paths still lazy-install via install_cua_driver() (work4you_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `work4you computer-use install` runs. Bounded
        ...
        # Work4You' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'work4you tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: work4you computer-use install"
            }
        } else {
...
# the final `electron` -> `Work4You` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Work4You.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_work4you_desktop command
    # resolves apps/desktop/release/win-unpacked/Work4You.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Work4You-managed
    ...
    # The --dir mode produces an unpacked Work4You.exe in
    ...
    # are NOT trying to work around). The Work4You icon + product name are
    # stamped onto Work4You.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\work4you-desktop-build-$(Get-Random).log"
    ...
    # 3b. The Work4You icon + identity are stamped onto Work4You.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, work4you#38216).
    ...
    #    Work4You.exe. We deliberately do NOT point them at `work4you desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Work4You.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Work4You.lnk')
        )
        ...
        # with the Work4You icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.work4you/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    # Run work4you setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $work4youCmd = "$InstallDir\venv\Scripts\work4you.exe"
    if (-not (Test-Path $work4youCmd)) {
        $work4youCmd = "work4you"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$Work4YouHome\whatsapp\session\creds.json"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$Work4YouHome\logs\gateway.log"
            Start-Process -FilePath $work4youCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$Work4YouHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$Work4YouHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$Work4YouHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$Work4YouHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$Work4YouHome\work4you\"
    Write-Host ""
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://work4you.ai/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
