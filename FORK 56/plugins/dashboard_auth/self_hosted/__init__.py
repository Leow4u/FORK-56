...
A standards-compliant OpenID Connect Relying Party for the ``work4you dashboard``
OAuth gate. Unlike the bundled ``work4you`` provider (which encodes Work4You Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-work4you-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~work4you_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``work4you_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~work4you_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``work4you``
provider verifies its *access* token because Work4You Portal mints a custom JWT
...
same precedence convention as the ``work4you`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.work4you/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    WORK4YOU_DASHBOARD_OIDC_ISSUER
    WORK4YOU_DASHBOARD_OIDC_CLIENT_ID
    WORK4YOU_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    WORK4YOU_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the work4you
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the work4you plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Work4You does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the work4you provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``WORK4YOU_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "WORK4YOU_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "WORK4YOU_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("WORK4YOU_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.work4you/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "WORK4YOU_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
