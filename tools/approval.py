...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("WORK4YOU_YOLO_MODE", ""))
...
# Work4You session id (observability identity, distinct from the gateway
...
# os.environ["WORK4YOU_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export WORK4YOU_INTERACTIVE.
_work4you_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "work4you_interactive",
    default=None,
)


def set_work4you_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["WORK4YOU_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``WORK4YOU_INTERACTIVE`` env var for legacy callers.
    """
    return _work4you_interactive_ctx.set("1" if interactive else "")


def reset_work4you_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_work4you_interactive_context`."""
    _work4you_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``WORK4YOU_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _work4you_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("WORK4YOU_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Work4You session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("WORK4YOU_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("WORK4YOU_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("WORK4YOU_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("WORK4YOU_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("WORK4YOU_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``work4you chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``WORK4YOU_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("WORK4YOU_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("WORK4YOU_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set WORK4YOU_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind WORK4YOU_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds WORK4YOU_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("WORK4YOU_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``WORK4YOU_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $WORK4YOU_HOME, or by the resolved absolute
# active profile home path such as /home/work4you/.work4you/config.yaml. The
# resolved-absolute form is folded into the ~/.work4you/ patterns at detection
...
# go stale when WORK4YOU_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_WORK4YOU_ENV_PATH = (
    r'(?:~\/\.work4you/|'
    r'(?:\$home|\$\{home\})/\.work4you/|'
    r'(?:\$work4you_home|\$\{work4you_home\})/)'
    r'\.env\b'
)
# ~/.work4you/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _WORK4YOU_ENV_PATH; matches the WORK4YOU_HOME override form as
# well as ~/.work4you/.
_WORK4YOU_CONFIG_PATH = (
    r'(?:~\/\.work4you/|'
    r'(?:\$home|\$\{home\})/\.work4you/|'
    r'(?:\$work4you_home|\$\{work4you_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_WORK4YOU_ENV_PATH}|'
    rf'{_WORK4YOU_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_work4you_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Work4You: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Work4You
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]work4you[^\n]*\.env\b', "access to Work4You secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `work4you` and `gateway` (e.g. `work4you -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bwork4you\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart work4you gateway (kills running agents)"),
    (r'\bwork4you\s+update\b', "work4you update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `work4you gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `work4you ...
    ...
    (r'\b(pkill|killall)\b.*\b(work4you|gateway|cli\.py)\b', "kill work4you/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill work4you` but not
    # `kill -9 $(pgrep -f work4you)` because the substitution is opaque
    ...
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(work4you|ai\.work4you)\b', "stop/restart work4you launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Work4You file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.work4you/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Work4You-managed security file (~/.work4you/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_WORK4YOU_CONFIG_PATH}|{_WORK4YOU_ENV_PATH})', "in-place edit of Work4You config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_WORK4YOU_CONFIG_PATH}|{_WORK4YOU_ENV_PATH})', "in-place edit of Work4You config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_WORK4YOU_CONFIG_PATH}|{_WORK4YOU_ENV_PATH})', "in-place edit of Work4You config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.work4you/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / WORK4YOU_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Work4You home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\work4you), so folding the user home
    # first would eat the prefix the Work4You-home fold needs.
    command = _rewrite_resolved_work4you_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / WORK4YOU_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.work4you``); the matched
...
def _rewrite_resolved_work4you_home(command: str) -> str:
    """Rewrite the resolved absolute Work4You home prefix to ``~/.work4you/``.

    Resolves the active ``WORK4YOU_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.work4you/`` so the static ``_WORK4YOU_CONFIG_PATH`` / ``_WORK4YOU_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/work4you/.work4you/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$WORK4YOU_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from work4you_constants import get_work4you_home
        home = get_work4you_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.work4you")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``work4you send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Work4You ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"work4you-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Work4You YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["WORK4YOU_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "WORK4YOU_SPINNER_PAUSE" in os.environ:
            del os.environ["WORK4YOU_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Work4You approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``WORK4YOU_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            WORK4YOU_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export WORK4YOU_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set WORK4YOU_INTERACTIVE or "
                "WORK4YOU_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set WORK4YOU_INTERACTIVE or "
            "WORK4YOU_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("WORK4YOU_EXEC_ASK"):
        ...
        # can see (WORK4YOU_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``WORK4YOU_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from work4you_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from work4you_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("WORK4YOU_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # WORK4YOU_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from work4you_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (WORK4YOU_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # WORK4YOU_EXEC_ASK (and sometimes a session platform marker) can leak
