...
as ``kind: backend``) or ``~/.work4you/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``work4you tools``
...
def _images_cache_dir() -> Path:
    """Return ``$WORK4YOU_HOME/cache/images/``, creating parents as needed."""
    from work4you_constants import get_work4you_home

    path = get_work4you_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$WORK4YOU_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$WORK4YOU_HOME/cache/images/``.
