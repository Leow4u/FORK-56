...
description: "Run Work4You directly on an Android phone with Termux"
---

# Work4You on Android with Termux
...
Work4You can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Work4You from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
This APT repository is **community-maintained by `@adybag14-cyber` and is not an official Work4You distribution**. Work4You does not build, sign, host, or audit these packages. Enabling the repository means trusting the contributor-operated repository and its signing key. Termux itself remains a Tier 2 / best-effort platform.
:::

For users who prefer a native package-manager install rather than building Python/Rust dependencies on the phone, a community-maintained APT repository is available. The repository bootstrap and packaging sources are published in [`adybag14-cyber/termux-python`](https://github.com/adybag14-cyber/termux-python), with the Work4You package build in [`adybag14-cyber/termux-work4you`](https://github.com/adybag14-cyber/termux-work4you).

Install the repository key/source and Work4You with:
...
APT-managed Work4You installs are marked with install method `apt`. Work4You therefore does not run its Git self-updater against package-owned files; use the package manager instead:
...
Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Work4You runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Work4You now ships a Termux-aware installer path:
...
- links `work4you` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `work4you` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/work4you" "$PREFIX/bin/work4you"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `work4you` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.work4you/.env`.
...
### `work4you doctor` says ripgrep or Node is missing
