...
Translates between Work4You's internal OpenAI-style message format and
...
try:
    import work4you_cli as _work4you_cli

    _WORK4YOU_VERSION = str(_work4you_cli.__version__)
except Exception:
    _WORK4YOU_VERSION = "0.0.0"
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in work4you_cli/runtime_provider.py.
    """
...
# Work4You effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_work4you_portal_endpoint(base_url: str | None) -> bool:
    """Return True for Work4You Portal's Anthropic Messages route.

    Portal serves its ``anthropic/*`` catalog natively at
    ``https://inference-api.work4you.ai/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.work4you.ai``
    2. The operator-set ``WORK4YOU_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.work4you.ai.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.work4you.ai"):
        return True
    try:
        from work4you_cli.auth import _work4you_inference_env_override

        override = _work4you_inference_env_override()
    except Exception:
        return False
    if not override:
        return False
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Work4You
    Portal's Messages route follow this pattern.
    """
    if _is_work4you_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to work4you's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to work4you's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + Work4YouAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://work4you.ai")
        headers.setdefault("X-Title", "Work4You")
        headers.setdefault("User-Agent", f"Work4YouAgent/{_WORK4YOU_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Work4You loads that into the process env from
    # ``~/.work4you/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Work4You notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Work4You historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``work4you models``) never mutates ``~/.work4you/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Work4You)
      ...
      5. Anthropic credential_pool OAuth entry (~/.work4you/auth.json)
    ...
    # 1. Work4You-managed OAuth/setup token env var
    ...
    # 5. Work4You credential_pool OAuth entry.
...
# ── Work4You-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.work4you/.anthropic_oauth.json (our own file).
...
def _get_work4you_oauth_file() -> Path:
    return get_work4you_home() / ".anthropic_oauth.json"
...
def run_work4you_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Work4You-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Work4You with your Claude Pro/Max subscription.")
    print()
...
def read_work4you_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Work4You-managed OAuth credentials from ~/.work4you/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Work4You OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See work4you#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See work4you: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See work4you#13848 (Kimi) and
    work4you#16748 (DeepSeek).

    Work4You Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_work4you_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Work4You Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if not _is_work4you_portal_endpoint(base_url):
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("work4you", "claude-code")
                text = text.replace("Work4You", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Work4You-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Work4You surfaces in its CLI. We
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Work4You Portal's ``x-work4you-credits-*`` balance family
