...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshWork4YouConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshWork4YouConfig — two REST calls (config + defaults) per event, per
