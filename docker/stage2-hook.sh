...
WORK4YOU_HOME="${WORK4YOU_HOME:-/opt/data}"
...
# $WORK4YOU_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass WORK4YOU_UID/WORK4YOU_GID — or the PUID/PGID aliases — which the
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e WORK4YOU_UID=\$(id -u) -e WORK4YOU_GID=\$(id -g) ...
...
# --- Bootstrap WORK4YOU_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid work4you mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom WORK4YOU_HOME paths whose parents only
# root can create (e.g. `WORK4YOU_HOME=/home/work4you/.work4you` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for WORK4YOU_UID/WORK4YOU_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  WORK4YOU_UID/
# WORK4YOU_GID still win when both are set.  See #15290, salvages #25872.
WORK4YOU_UID="${WORK4YOU_UID:-${PUID:-}}"
WORK4YOU_GID="${WORK4YOU_GID:-${PGID:-}}"

if [ -n "${WORK4YOU_UID:-}" ] && validate_uid_gid "$WORK4YOU_UID" && [ "$WORK4YOU_UID" != "$(id -u work4you)" ]; then
    echo "[stage2] Changing work4you UID to $WORK4YOU_UID"
    usermod -u "$WORK4YOU_UID" work4you
fi
if [ -n "${WORK4YOU_GID:-}" ] && validate_uid_gid "$WORK4YOU_GID" && [ "$WORK4YOU_GID" != "$(id -g work4you)" ]; then
    echo "[stage2] Changing work4you GID to $WORK4YOU_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$WORK4YOU_GID" work4you 2>/dev/null || true
fi
...
# When WORK4YOU_UID is remapped or the top-level $WORK4YOU_HOME isn't owned by
# the runtime work4you UID, restore ownership to work4you — but ONLY for the
# directories work4you actually writes to. The full $WORK4YOU_HOME may be a
...
tree_has_non_work4you_owner() {
    target="$1"
    find "$target" \( ! -user work4you -o ! -group work4you \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$WORK4YOU_HOME" 2>/dev/null)" != "$actual_work4you_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $WORK4YOU_HOME (targeted) to work4you ($actual_work4you_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $WORK4YOU_HOME: chown the directory itself (not its contents)
    # so work4you can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$WORK4YOU_HOME"; then
        :
    else
        chown work4you:work4you "$WORK4YOU_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $WORK4YOU_HOME failed (rootless container?) — continuing"
    fi
    # Work4You-owned subdirs: recursive chown is safe here because these are
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$WORK4YOU_HOME/$sub" ] && tree_has_non_work4you_owner "$WORK4YOU_HOME/$sub"; then
            chown_work4you_tree "$WORK4YOU_HOME/$sub"
        fi
    done
fi
...
# $WORK4YOU_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# WORK4YOU_DISABLE_LAZY_INSTALLS=1. Keeping /opt/work4you root-owned and
...
# $WORK4YOU_HOME/lazy-packages dir on the data volume (Dockerfile sets
# WORK4YOU_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $WORK4YOU_HOME/profiles to work4you on every
...
if [ -d "$WORK4YOU_HOME/profiles" ] && tree_has_non_work4you_owner "$WORK4YOU_HOME/profiles"; then
    chown_work4you_tree "$WORK4YOU_HOME/profiles"
fi

# Always reset ownership of $WORK4YOU_HOME/cron on every boot for the same
...
# gate as profiles/).
if [ -d "$WORK4YOU_HOME/cron" ] && tree_has_non_work4you_owner "$WORK4YOU_HOME/cron"; then
    chown_work4you_tree "$WORK4YOU_HOME/cron"
fi

# Always ensure logs/gateways is work4you-owned (#45258). Formerly healed by
...
# top-level $WORK4YOU_HOME is mis-owned, so a warm volume with work4you-owned
# WORK4YOU_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid work4you mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as work4you.
if [ -d "$WORK4YOU_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$WORK4YOU_HOME/logs/gateways"; then
        :
    else
        chown work4you:work4you "$WORK4YOU_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# work4you pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged work4you gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $WORK4YOU_HOME is
...
# and cron/.
if [ -d "$WORK4YOU_HOME/platforms/pairing" ] && tree_has_non_work4you_owner "$WORK4YOU_HOME/platforms/pairing"; then
    chown_work4you_tree "$WORK4YOU_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$WORK4YOU_HOME/pairing" ] && tree_has_non_work4you_owner "$WORK4YOU_HOME/pairing"; then
    chown_work4you_tree "$WORK4YOU_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $WORK4YOU_HOME
...
# sweep so host-owned files in a bind-mounted $WORK4YOU_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$WORK4YOU_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$WORK4YOU_HOME/$f"; then
            :
        else
            chown work4you:work4you "$WORK4YOU_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$WORK4YOU_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$WORK4YOU_HOME/config.yaml"; then
        :
    else
        chown work4you:work4you "$WORK4YOU_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$WORK4YOU_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $WORK4YOU_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_work4you mkdir -p \
    "$WORK4YOU_HOME/backups" \
    "$WORK4YOU_HOME/cron" \
    "$WORK4YOU_HOME/sessions" \
    "$WORK4YOU_HOME/logs" \
    "$WORK4YOU_HOME/logs/gateways" \
    "$WORK4YOU_HOME/hooks" \
    "$WORK4YOU_HOME/memories" \
    "$WORK4YOU_HOME/skills" \
    "$WORK4YOU_HOME/skins" \
    "$WORK4YOU_HOME/plans" \
    "$WORK4YOU_HOME/workspace" \
    "$WORK4YOU_HOME/home" \
    "$WORK4YOU_HOME/pairing" \
    "$WORK4YOU_HOME/platforms/pairing" \
    "$WORK4YOU_HOME/lazy-packages"
...
# $WORK4YOU_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $WORK4YOU_HOME: it is a shared DATA volume, commonly
...
# stale 'docker' stamp from $WORK4YOU_HOME if one is present (the host install's
...
if [ -f "$WORK4YOU_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$WORK4YOU_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$WORK4YOU_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$WORK4YOU_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$WORK4YOU_HOME/$dest"; then
            :
        else
            as_work4you cp "$INSTALL_DIR/$src" "$WORK4YOU_HOME/$dest"
        fi
    fi
}
...
if [ -f "$WORK4YOU_HOME/.env" ] && ! grep -q '^API_SERVER_KEY=..*' "$WORK4YOU_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$WORK4YOU_HOME/.env"; then
        :
    else
        ...
        if [ -n "$_gen_key" ]; then
            ...
            sed -i '/^API_SERVER_KEY=$/d' "$WORK4YOU_HOME/.env" 2>/dev/null || true
            printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$WORK4YOU_HOME/.env"
...
if [ -f "$WORK4YOU_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$WORK4YOU_HOME/.env"; then
        :
    else
        chown work4you:work4you "$WORK4YOU_HOME/.env" 2>/dev/null || true
        chmod 600 "$WORK4YOU_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $WORK4YOU_HOME on the mounted volume. Run the same safe, non-interactive
...
if [ ! -f "$WORK4YOU_HOME/auth.json" ] && [ -n "${WORK4YOU_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$WORK4YOU_HOME/auth.json"; then
        :
    else
        printf '%s' "$WORK4YOU_AUTH_JSON_BOOTSTRAP" > "$WORK4YOU_HOME/auth.json"
        chown work4you:work4you "$WORK4YOU_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$WORK4YOU_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD Work4You bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose Work4You bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.work4you.last_auth_error.relogin_required
...
# supply a freshly-issued session via WORK4YOU_AUTH_JSON_REBOOTSTRAP (distinct
# from the create-only *_BOOTSTRAP var); this helper swaps ONLY the
# providers.work4you entry when the on-disk entry is provably terminal OR the
...
if [ -f "$WORK4YOU_HOME/auth.json" ] && [ -n "${WORK4YOU_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$WORK4YOU_HOME/auth.json"; then
        :
    else
        s6-setuidgid work4you "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_work4you_session.py" \
            "$WORK4YOU_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_work4you_session.py failed; continuing"
    fi
fi
...
if [ ! -f "$WORK4YOU_HOME/gateway_state.json" ] && \
        [ "${WORK4YOU_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$WORK4YOU_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$WORK4YOU_HOME/gateway_state.json"
        chown work4you:work4you "$WORK4YOU_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$WORK4YOU_HOME/gateway_state.json"
    fi
fi
...
# directory. agent-browser (the runtime CLI Work4You spawns for the
