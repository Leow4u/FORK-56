...
Build the real image and verify the actual runtime behavior:
  ...
  1. WORK4YOU_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume seeds
...
def test_seeds_running_state_on_blank_volume(
    ...
    """WORK4YOU_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume must
...
def test_no_seed_when_env_unset(
    ...
    """No WORK4YOU_GATEWAY_BOOTSTRAP_STATE = no seed file written."""
...
def test_non_running_value_ignored(
    ...
    for bogus in ("stopped", "Running", "1", "true", "starting"):
        ...
        _start_container(
            built_image, name,
            f"WORK4YOU_GATEWAY_BOOTSTRAP_STATE={bogus}",
        )
        r = docker_exec_sh(
            name,
            "test -f /opt/data/gateway_state.json && "
