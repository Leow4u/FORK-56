...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'work4you_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'work4you_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'work4you_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Work4You.app/Contents/MacOS/Work4You', 'darwin'),
    '/Applications/Work4You.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Work4You.app/Contents/MacOS/Work4You', 'darwin'),
    '/Users/x/Applications/Work4You.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Work4You\\Work4You.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Work4You'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\work4you-desktop\\Work4You.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\work4you-desktop'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Work4You.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Work4You.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.work4you/work4you/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.work4you/work4you',
    uninstallArgs: ['-m', 'work4you_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/work4you/linux-unpacked',
    work4youHome: '/home/x/.work4you'
  })
  ...
  assert.match(script, /'-m' 'work4you_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/work4you\/linux-unpacked'/)
  assert.match(script, /export WORK4YOU_HOME='\/home\/x\/\.work4you'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.work4you/work4you',
    agentRoot: '/home/x/.work4you/work4you',
    uninstallArgs: ['-m', 'work4you_cli.uninstall', '--mode', 'full'],
    appPath: null,
    work4youHome: '/home/x/.work4you'
  })

  // System python + source on PYTHONPATH so import work4you_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.work4you\/work4you'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'work4you_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  ...
  assert.match(script, /'-m' 'work4you_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\work4you',
    agentRoot: 'C:\\work4you',
    uninstallArgs: ['-m', 'work4you_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Work4You',
    work4youHome: 'C:\\Users\\x\\AppData\\Local\\work4you'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import work4you_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\work4you;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "work4you_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Work4You" >nul 2>&1/)
