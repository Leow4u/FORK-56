---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Work4You"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Work4You"
---
...
Set up Actual Computer (actual.inc) inference in Work4You.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `work4you skills install official/devops/actual-setup` |
| Path | `optional-skills/devops/actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + Work4You |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Work4You inference
...
- User has an `ac_` key and wants Work4You routed through their Actual cluster.
...
- Work4You has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Work4You — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.work4you/.env`.
...
3. Select provider + model:
   ```bash
   work4you config set model.provider actual
   work4you config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   work4you chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Work4You at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.work4you/.env`, then:
   ```bash
   work4you config set model.provider actual
   work4you config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   work4you chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Work4You since the first-class provider).**
   ...
   request still 400s this way on an old Work4You, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Work4You' default toolset
   ...
   servers emit a bare `data: [DONE]` — Work4You reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Work4You the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
work4you chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
work4you chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
