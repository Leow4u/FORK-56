...
//!
//! Driven when the installer is launched as `Work4You-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Work4You desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `work4you update`
//!      or repair bootstrap can race locked files),
//!   2. run `work4you update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in work4you_cli/main.py),
//!   3. run `work4you desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `work4you update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_work4you) and
...
/// `work4you update` exit code meaning "another work4you process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// work4you_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `work4you update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `work4you update` claims
/// the same file (see `work4you_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `work4you update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in work4you_cli/update_lock.py — all three read
...
/// owner that is itself and aborts ("Another Work4You update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `work4you update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(work4youHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    let work4you_home = crate::paths::work4you_home();
    let install_root = work4you_home.join("work4you");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_work4you`
    ...
    //
    // The same marker is the cross-process update lock (work4you_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `work4you update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Work4You update is already running (PID {}, started {} ago). \
    ...
    let work4you = resolve_work4you(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the work4you CLI under {}. Is Work4You installed? \
    ...
    // async on Windows. If it still holds the venv shim, `work4you update`
    ...
    // ---- stage 2: work4you update -----------------------------------------
    // Pass --branch so `work4you update` targets the branch this installer was
    ...
    // without --branch, `work4you update` switches the checkout to `main` (a
    ...
    // --force skips `work4you update`'s Windows running-exe guard (which would
    ...
    // time `work4you update` runs there is no legitimate work4you.exe to protect,
    // and the guard would only produce a false "Work4You is still running" stop.
    //
    ...
    // probe only checks the work4you.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Work4You windows" message.
    ...
    // Retry-once for the update-boundary crash. `work4you update` lazily imports
    ...
    // second `work4you update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Work4You" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Work4You is still running. Close all Work4You windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "work4you update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::work4you_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: work4you desktop --build-only ----------------------------
    // `work4you update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `work4you update` already does above, and `work4you update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `work4you desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Work4You manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_work4you_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Work4You to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend work4you.exe (or the desktop Work4You.exe
            ...
            // force-kill every Work4You.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Work4You still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_work4you();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_work4you(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Work4You.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Work4You.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `work4you.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `work4you.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\work4you.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startWork4You()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// work4you.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named work4you.exe.)
fn force_kill_other_work4you() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `work4you <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv work4you shim under an install root, regardless of existence.
fn venv_work4you(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("work4you.exe")
    } else {
        install_root.join("venv").join("bin").join("work4you")
    }
}

/// Resolve the work4you CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `work4you` on PATH.
fn resolve_work4you(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_work4you(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "work4you.exe" } else { "work4you" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let work4you_home = crate::paths::work4you_home();
    let mut envs = vec![(
        "WORK4YOU_HOME".to_string(),
        work4you_home.as_os_str().to_os_string(),
    )];
    // `work4you update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `work4you update` child claims that SAME lock (work4you_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Work4You is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in work4you_cli/update_lock.py.
    envs.push((
        "WORK4YOU_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        work4you_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_work4you_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Work4You.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.work4you-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.work4you-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::work4you_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_work4you_is_under_install_root() {
        let root = Path::new("/x/work4you");
        let shim = venv_work4you(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/work4you"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/work4you"));
        assert!(
            envs.iter().any(|(k, v)| k == "WORK4YOU_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the work4you update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_work4you(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/work4you");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".work4you-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `work4you update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(work4youHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Work4You.app"]),
            Some(PathBuf::from("/Applications/Work4You.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Work4You.app");
        let tmp = base.join("Work4You.app.work4you-update-new");
        let old = base.join("Work4You.app.work4you-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Work4You.app");
        let tmp = base.join("Work4You.app.work4you-update-new"); // intentionally absent
        let old = base.join("Work4You.app.work4you-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Work4You.app");
        let tmp = base.join("Work4You.app.work4you-update-new"); // absent
        let old = base.join("Work4You.app.work4you-update-old");
        write_marker(&target, "OLD");
