# nix/desktop.nix — Work4You Desktop (Electron) app build + wrapper
#
# `work4youAgent` is the fully-built `.#default` package — it ships the
# `work4you` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `WORK4YOU_DESKTOP_WORK4YOU` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Work4You CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "work4you-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "work4you-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = work4youNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "work4you-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe work4youNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/work4you-desktop $out/bin
    cp -r ${renderer}/* $out/share/work4you-desktop/
    ...
    substituteInPlace $out/share/work4you-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/work4you-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # WORK4YOU_DESKTOP_WORK4YOU to the absolute path of the nix-built `work4you`
    # binary so the desktop's resolver step 4 ("existing Work4You CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/work4you-desktop \
      --add-flags "$out/share/work4you-desktop" \
      --set WORK4YOU_DESKTOP_WORK4YOU "${lib.getExe work4youAgent}" \
      --set ELECTRON_IS_DEV 0

    # XDG launcher entry
    mkdir -p $out/share/applications $out/share/icons/hicolor/1024x1024/apps
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/work4you.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../work4you_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/work4you-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/work4you.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/work4you.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Work4You";
    homepage = "https://github.com/Leow4u/FORK-56";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "work4you-desktop";
  };
}
