"""Unit tests for the generic-OIDC / Work4You-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `work4you gateway enroll` CLI.
...
Three modes:
  ...
  2. Work4You Portal (resolve_work4you_access_token) otherwise — the default.

The HTTP calls and the Work4You resolver are monkeypatched; these prove the mode
