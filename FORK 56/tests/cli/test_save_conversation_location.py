...
Regression: the old implementation wrote ``work4you_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``work4you sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.work4you/sessions/saved/`` and prints
...
def work4you_home(tmp_path, monkeypatch):
    ...
    # Clear any cached work4you_home computation
    import work4you_constants
    if hasattr(work4you_constants, "_work4you_home_cache"):
        work4you_constants._work4you_home_cache = None
    return home
...
def test_save_conversation_writes_under_work4you_home(work4you_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.work4you/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the WORK4YOU_HOME fixture
    ...
    cli.Work4YouCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("work4you_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.work4you/sessions/saved/
    saved_dir = work4you_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("work4you_conversation_*.json"))
...
def test_save_conversation_empty_history_does_nothing(work4you_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "work4you_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(work4you_home, capsys):
    ...
    cli.Work4YouCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(work4you_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "work4you_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.Work4YouCLI.save_conversation(stub, "/save pdf")
