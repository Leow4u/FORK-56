"""
Security advisory checker for Work4You.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``work4you doctor --ack <id>``; the ack is persisted to
...
1. ``work4you doctor`` (and ``work4you doctor --ack <id>``)
...
run in environments where the rest of Work4You failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including work4you when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.work4you/.env (OpenRouter, Anthropic, OpenAI, "
            "Work4You, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: work4you doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Work4You requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'work4you doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.work4you/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_work4you_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``work4you doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `work4you doctor` on the gateway host for details.")
