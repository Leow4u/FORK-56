#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Work4You icon + version metadata onto the
// built Work4You.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Work4You.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `work4you desktop`, the installer's
...
// path (which rebuilds via `work4you desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Work4You.exe>
//
...
// Stamp the Work4You icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Work4You',
      FileDescription: 'Work4You',
      CompanyName: 'Work4You',
      LegalCopyright: 'Copyright (c) 2026 Work4You'
    }
  })

  console.log('[set-exe-identity] done — Work4You icon + identity stamped')
}
