...
def backup_env(monkeypatch, tmp_path):
    """Isolate WORK4YOU_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_work4you_home picks up the env var fresh.
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under WORK4YOU_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level WORK4YOU_DIR picks up the tmp HOME."""
    import work4you_constants
    importlib.reload(work4you_constants)
