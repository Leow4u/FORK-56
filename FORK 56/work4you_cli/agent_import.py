"""work4you import-agent — import Claude Code / Codex CLI setups into Work4You.
...
Usage:
    ...
    work4you import-agent claude-code --dry-run # preview only, no changes
    work4you import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``work4you claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_work4you.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in WORK4YOU_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → WORK4YOU_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in WORK4YOU_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in WORK4YOU_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → WORK4YOU_HOME/skills/codex-imports/<name>/
...
deliberately via ``work4you setup`` or config.yaml.
"""
...
# Same entry delimiter as the Work4You memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`work4you config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_work4you.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Work4You command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Work4You glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Work4You equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        work4you_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → WORK4YOU_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``work4you import-agent`` (invoked from work4you_cli.main)."""
    from work4you_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: work4you import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: work4you import-agent claude-code   or   work4you import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Work4You — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: work4you import-agent "
    ...
    print_info("Secrets:     never imported — run 'work4you setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: work4you import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'work4you setup' "
               "to configure providers, or add them to ~/.work4you/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from work4you_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'work4you setup' or ~/.work4you/.env.")
        print()
