...
// The @mention middleware appends a handoff note whose work4you command the
...
/** Run the note's first work4you command under a stub that echoes each argv
 *  element — proves the shell received the interpolations as LITERALS. */
function runHandoffCommand(noteText) {
  const command = noteText.match(/`work4you -p [^`]*`/)[0].slice(1, -1)
  const script = `work4you() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: a poisoned bot title stays literal in the handoff command', async () => {
  const quoteSentinel = `/tmp/work4you-bot-mode-mention-quote-${process.pid}`
  const subSentinel = `/tmp/work4you-bot-mode-mention-sub-${process.pid}`
...
test('regression: the handoff command quotes the recipient argument', async () => {
  ...
  assert.match(result.text, /`work4you -p 'ops' chat --in ~/)
})

test('behavior: @dixie on a Connections bot stays in this chat and does not work4you -p', async () => {
  const values = new Map()
  const atom = initial => {
    const slot = { get: () => values.get(slot), set: value => values.set(slot, value) }
    values.set(slot, initial)
  ...
  assert.doesNotMatch(result.text, /work4you -p 'dixie'/)
