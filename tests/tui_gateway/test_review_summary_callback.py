    ...
    with patch.dict(
        "sys.modules",
        {
            "work4you_constants": MagicMock(
                get_work4you_home=MagicMock(return_value="/tmp/work4you_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'work4you-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'work4you-release' patched"
    }
