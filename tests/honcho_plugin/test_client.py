...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "work4you"
        assert config.workspace_id == "work4you"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"work4you": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"work4you": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"work4you": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="work4you"
        ):
            result = config.resolve_session_name("/home/user/work4you/subdir")
        assert result == "work4you"


class TestResolveConfigPath:
    def test_prefers_work4you_home_when_exists(self, tmp_path):
        work4you_home = tmp_path / "work4you"
        work4you_home.mkdir()
        local_cfg = work4you_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: WORK4YOU_HOME points at ~/.work4you/profiles/<name>, so
        # _get_default_work4you_home() must resolve back to ~/.work4you — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".work4you"
        ...
        assert _get_default_work4you_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "work4you_coder"
        assert profile_host_key("default") == "work4you"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"WORK4YOU_HONCHO_HOST": "work4you.coder"}):
            assert resolve_active_host() == "work4you.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("WORK4YOU_HONCHO_HOST", None)
            ...
            try:
                assert resolve_active_host() == "work4you"
            finally:
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="work4you_coder")
        assert config.host == "work4you_coder"
        assert config.workspace_id == "work4you"  # shared workspace
        assert config.ai_peer == "work4you_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"work4you": {"enabled": True, "aiPeer": "work4you"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (work4you.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="work4you_work",
            workspace_id="work4you",
            raw={"hosts": {"work4you.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_work4you_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("work4you_cli.profiles.get_active_profile_name", return_value="default"), \
