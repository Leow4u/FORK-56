...
storage discipline as the other indexes). On a populated database, run
    ...
    work4you sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.work4you/config.yaml` to disable. Override
the .so location with `WORK4YOU_FTS5_CJK_SO`.
