...
# trigger its own recovery path. Refs: Leow4u/FORK-56#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: Leow4u/FORK-56#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("WORK4YOU_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "WORK4YOU_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "WORK4YOU_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Work4You-created private-chat topic lanes are marked with
        ...
        queued follow-ups after a gateway restart) prefer the Work4You topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the Work4You
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the Work4You topic thread id when it
    ...
    # older PTB/clients. Streaming edits stay on Work4You' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Work4You must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `work4you update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Work4You or OpenClaw instance is running "
            "with this token, then restart the gateway with 'work4you gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  Work4You defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Work4You process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("WORK4YOU_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("WORK4YOU_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("WORK4YOU_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("WORK4YOU_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("WORK4YOU_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (
                os.getenv("WORK4YOU_TELEGRAM_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if not fallback_ips and not disable_fallback:
                discovery_timeout = self._env_float_clamped(
                    "WORK4YOU_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT",
                    5.0,
                    min_value=0.0,
                )
                logger.warning(
            ...
            _max_connect = 8
            _init_timeout = _env_float("WORK4YOU_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set WORK4YOU_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"WORK4YOU_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``work4you model`` picker via the shared ``group_providers`` fold,
    ...
    # Scripts live in ~/.work4you/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".work4you" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@work4you_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Work4You bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Work4You inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Work4You inbox (#52363).
        #
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("WORK4YOU_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `work4you setup` to install Telegram support.",
