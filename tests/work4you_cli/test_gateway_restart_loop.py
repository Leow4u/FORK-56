...
Covers:
- Defense 1: gateway stop/restart refuse when _WORK4YOU_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.work4you.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.work4you/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l work4you-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.work4you.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.work4you.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.work4you/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated work4you note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "work4you cron list",
        "work4you update",
        "work4you config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `work4you gateway start` is benign — starting a gateway from inside a
        ...
        "work4you gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway work4you
        # services must not be falsely blocked (the old `.*work4you` matched any
        # work4you token).
        "launchctl unload ai.work4you.update-checker.plist",
        "launchctl restart ai.work4you.daemon",
        ...
        "systemctl restart work4you-meta.service",
        "systemctl restart work4you-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_work4you_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade work4you then run work4you gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_work4you_gateway_once.sh). The script must live under
        # WORK4YOU_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / ".work4you"))
        scripts_dir = tmp_path / ".work4you" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify work4you gateway stop/restart refuse when _WORK4YOU_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_WORK4YOU_GATEWAY", "1")
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _WORK4YOU_GATEWAY=1.

    Issue #37453: systemctl --user restart work4you-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart work4you-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.work4you.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-work4you label: label-independent detection is the point
        # (#62891 second reproduction used `ai.work4you.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.work4you.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.work4you.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.work4you.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-work4you systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run work4you gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("work4you gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfework4you gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under WORK4YOU_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.work4you.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("work4you gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/work4you\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 work4you gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_work4you_home()` falls back to Path.home(), which raises
        RuntimeError when neither WORK4YOU_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    work4you_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run work4you gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / ".work4you"))
        (tmp_path / ".work4you").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run work4you gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop work4you-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart work4you-gateway' /var/log/syslog",
        "rg 'work4you gateway restart' /home/user/.work4you/logs/",
        "journalctl -u work4you-gateway --grep 'systemctl restart work4you-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'work4you_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell work4you gateway restart"',
        'psql -c "\\! systemctl restart work4you-gateway"',
        "grep 'systemctl restart work4you-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart work4you-gateway",
        'grep "$(systemctl restart work4you-gateway)" f',
        "grep 'restart' log; systemctl restart work4you-gateway",
        'sqlite3 db "SELECT 1"; work4you gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "work4you gateway restart",
        "sudo systemctl stop work4you-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart work4you-gateway%'\""
        )
