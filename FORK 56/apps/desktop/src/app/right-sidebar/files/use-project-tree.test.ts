...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): Work4YouReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { work4youDesktop: unknown }).work4youDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: Work4YouReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<Work4YouReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { work4youDesktop: unknown }).work4youDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.work4youDesktop is missing', async () => {
    delete (window as unknown as { work4youDesktop?: unknown }).work4youDesktop
