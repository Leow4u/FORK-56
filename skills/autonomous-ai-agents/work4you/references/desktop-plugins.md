...
Write plugins for the Work4You desktop app: statusbar items, layout panes,
...
system (`~/.work4you/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$WORK4YOU_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$WORK4YOU_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Work4You desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$WORK4YOU_HOME/desktop-plugins/` (usually
  `~/.work4you/desktop-plugins/`).

## How to Run

1. Create `$WORK4YOU_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.work4you/...` by default, or `~/.work4you/profiles/<profile>/...` under a
...
The ONLY import surface is `@work4you/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Work4You
  ...
  (`work4you://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.work4you/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@work4you/plugin-sdk`, `react`, and
