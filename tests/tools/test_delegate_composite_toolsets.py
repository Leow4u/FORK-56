...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_work4you_cli_expands_web(self):
        """work4you-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"work4you-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("work4you-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['work4you-cli'] yields ['web']."""
        parent_toolsets = {"work4you-cli"}
