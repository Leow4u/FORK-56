...
and injects content-free wake hints into Work4You' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Work4You only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Work4You turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    ...
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Work4You env
