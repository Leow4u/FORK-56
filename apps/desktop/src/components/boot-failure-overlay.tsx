...
import { connectingPreviewMode, onboardingPreviewMode } from './onboarding/preview'
...
export function BootFailureOverlay() {
  ...
  const suppressed =
    Boolean(onboardingPreviewMode()) ||
    connectingPreviewMode() ||
  ...
  const retry = async () => {
    setBusy('retry')
    await window.work4youDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.work4youDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.work4youDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      await window.work4youDesktop?.oauthLogoutConnectionConfig?.()
  ...
  const openLogs = () => void window.work4youDesktop?.revealLogs().catch(() => undefined)
  ...
  if (view === 'connect') {
    return (
      <div className="fixed inset-0 z-(--z-setup) flex items-center justify-center bg-(--ui-chat-surface-background) p-6">
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-work4you) bg-(--ui-chat-bubble-background) shadow-work4you">
  ...
  return (
    <div className="fixed inset-0 z-(--z-setup) flex items-center justify-center bg-(--ui-chat-surface-background) p-6">
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-work4you) bg-(--ui-chat-bubble-background) shadow-work4you">
