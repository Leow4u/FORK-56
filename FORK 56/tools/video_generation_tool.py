...
provider** — turn on a backend by enabling a plugin (``work4you plugins
enable video_gen/<name>``) and selecting it in ``work4you tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/work4you-dev/references/dynamic-tool-schemas.md.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `work4you tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `work4you tools` → "
...
# `work4you tools` or `/skills`, the schema rebuilds automatically.
