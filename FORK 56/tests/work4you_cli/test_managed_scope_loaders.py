...
work4you_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # work4you_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("WORK4YOU_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import work4you_time

    assert work4you_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (WORK4YOU_TIMEZONE, WORK4YOU_REDACT_SECRETS, WORK4YOU_MAX_ITERATIONS,
    ...
    # writing WORK4YOU_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
