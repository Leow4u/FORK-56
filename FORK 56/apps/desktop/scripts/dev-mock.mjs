...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `work4you-dev-mock-${Date.now()}`))
  ...
  return { root, work4youHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(work4youHome, mockUrl) {
  fs.writeFileSync(
    path.join(work4youHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(work4youHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.work4youHome, mock.url)
  console.log(`  WORK4YOU_HOME: ${sandbox.work4youHome}`)
  ...
  const env = {
    ...
    WORK4YOU_DESKTOP_APP_NAME: `Work4YouDevMock-${Date.now()}`,
  }
