...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__work4youTourEngine) {
    ${driverIife}
    w.__work4youTourHolder = {};
    w.__work4youTourCollect = (${collectTourTargets.toString()});
    w.__work4youTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__work4you-tour-style')) {
    var style = document.createElement('style');
    style.id = '__work4you-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__work4youTourEngine(
    w.driver.js.driver,
    w.__work4youTourHolder,
    ${JSON.stringify(action)},
    w.__work4youTourCollect,
    document
  ));
})()`
}
