...
Work4You middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Work4You'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Work4You calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Work4You retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Work4You guardrails,
...
- `telemetry_schema_version`: currently `work4you.observer.v1`
- `middleware_schema_version`: currently `work4you.middleware.v1`
...
Work4You stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Work4You runs
them as a nested chain in registration order. Middleware failures are fail-open:
Work4You logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Work4You applies middleware in this order:
...
For each tool call, Work4You applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Work4You pre-execution path against the effective arguments:
...
For isolated local testing, use one `WORK4YOU_HOME` for plugin enablement and the
agent run:

```bash
export WORK4YOU_HOME=/tmp/work4you-middleware-test
mkdir -p "$WORK4YOU_HOME"
work4you plugins enable <plugin-name>
work4you chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run work4you plugins enable <plugin-name>
uv run work4you chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["work4you_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/work4you-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Work4You expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Work4You treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Work4You preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Work4You does not convert downstream
