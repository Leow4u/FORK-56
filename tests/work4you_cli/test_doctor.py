...
import work4you_cli.doctor as doctor
import work4you_cli.gateway as gateway_cli
from work4you_cli import doctor as doctor_mod
from work4you_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull work4you/work4you:latest" in hint
        assert "recreate all Work4You containers" in hint
        assert "work4you update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `work4you update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade work4you`" in hint
        assert "work4you update" not in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `work4you doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "WORK4YOU_HOME", work4you_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("WORK4YOU_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("WORK4YOU_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_work4you_home(self, tmp_path, provider=""):
        """Create a minimal WORK4YOU_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_work4you_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "WORK4YOU_HOME", home)
        ...
        try:
            from work4you_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_work4you_auth_status_local", lambda: {})
...
def test_run_doctor_accepts_work4you_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (work4you_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".work4you"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
    ...
    try:
        from work4you_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_work4you_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_xai_oauth_auth_status", lambda: {})
    except Exception:
        pass
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`work4you doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp WORK4YOU_HOME.

        ``run_doctor`` reads the module-level ``WORK4YOU_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("WORK4YOU_HOME")`` alone
        leaves doctor probing the REAL ~/.work4you. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_work4you_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from work4you_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("work4you_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "work4you_cli.doctor", raising=False)
        from work4you_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Work4You / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from work4you_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_work4you_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Work4You / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".work4you"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Work4You Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale WORK4YOU_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir(parents=True)
        (work4you_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"WORK4YOU_MAX_ITERATIONS={ghost}\n")
        (work4you_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "WORK4YOU_HOME", work4you_home)
        monkeypatch.setattr(doctor_mod, "get_work4you_home", lambda: work4you_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("WORK4YOU_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("WORK4YOU_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale WORK4YOU_MAX_ITERATIONS" in out
        env_after = (work4you_home / ".env").read_text(encoding="utf-8")
        assert "WORK4YOU_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_work4you_tool_progress_warning_says_unsupported_since_floor(self):
        """WORK4YOU_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"WORK4YOU_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["WORK4YOU_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"WORK4YOU_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["WORK4YOU_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir(parents=True)
        (work4you_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (work4you_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"WORK4YOU_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: WORK4YOU_TOOL_PROGRESS_MODE" in out
