...
Work4You includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Work4You and does not require `work4you plugins enable
observability/nemo_relay`. Work4You remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Work4You execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `work4you[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Work4You requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Work4You installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Work4You process so configured interceptors can
...
Collection remains off unless Work4You policy enables it:
...
Work4You core owns one Relay host and one isolated Relay session scope per Work4You
session. Core lifecycle producers use
`work4you_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Work4You plugin registration. Shared-metrics marks must
...
```text
Work4You turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Work4You shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Work4You sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Work4You used for the logical call, such as
...
New calls use `work4you.model_route.count`. The previous
`work4you.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `work4you.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Work4You
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Work4You processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `work4you.task_run`, parented to
the owning Work4You session. The start counter contains only bounded execution
...
provider attempts for the same Work4You API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Work4You tool-call
...
cancellations. Active task ownership follows the task ID if Work4You rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`work4you.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Work4You
...
tool or plugin names. Work4You does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `work4you.tool_approval` marks and
...
Successful skill mutations emit `work4you.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `work4you.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Work4You derives reuse and patch-generation
...
```text
$WORK4YOU_HOME/telemetry/shared_metrics/metrics.sqlite3
$WORK4YOU_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Work4You version,
...
schema field name, its current scope is one `WORK4YOU_HOME`, so it is more
...
Deleting `$WORK4YOU_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `WORK4YOU_HOME`. To reset it, stop Work4You
processes and remove `$WORK4YOU_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Work4You CLI turn against the deterministic local model server:
