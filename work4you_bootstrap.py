"""Windows UTF-8 bootstrap for Work4You entry points.
...
should be imported at the very top of every Work4You entry point
(``work4you``, ``work4you``, ``work4you-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``work4you_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``work4you_bootstrap``, never ``work4you_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Work4You modules.

    Work4You ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Work4You source root
    ...
    """
    root = src_root or os.environ.get("WORK4YOU_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``WORK4YOU_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
...
# Apply on import — entry points just need ``import work4you_bootstrap``
# (or ``from work4you_bootstrap import apply_windows_utf8_bootstrap``) at
