...
WORK4YOU_HOME_DIR="${WORK4YOU_HOME:-$HOME/.work4you}"
WORK4YOU_CFG="${WORK4YOU_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Work4You config has twozero_td MCP entry ──
if [[ ! -f "$WORK4YOU_CFG" ]]; then
    echo -e " ${FAIL} Work4You config not found at ${WORK4YOU_CFG}"
    manual_steps+=("Create ${WORK4YOU_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$WORK4YOU_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Work4You config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Work4You config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$WORK4YOU_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${WORK4YOU_CFG} manually"); }
    manual_steps+=("Restart Work4You session to pick up config change")
fi
