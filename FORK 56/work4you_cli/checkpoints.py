"""`work4you checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.work4you/checkpoints/``.  Actions:

    work4you checkpoints               # same as `status`
    work4you checkpoints status        # total size, project count, breakdown
    work4you checkpoints list          # per-project checkpoint counts + workdir
    work4you checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    work4you checkpoints clear [-f]    # nuke the entire base (asks first)
    work4you checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    work4you checkpoints
    work4you checkpoints prune --retention-days 3 --max-size-mb 200
    work4you checkpoints clear -f
...
from work4you_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: work4you checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``work4you checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `work4you checkpoints` → status
