...
const agentConn = (over: Partial<Work4YouConnection> = {}): Work4YouConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as Work4YouConnection
...
const getConnection = vi.fn<(profile?: string | null) => Promise<Work4YouConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<Work4YouConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { work4youDesktop: { getConnection, getConnectionFor } })
})
