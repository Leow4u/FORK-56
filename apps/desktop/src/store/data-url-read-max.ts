 ...
 * (`work4you:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.work4youDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.work4youDesktop?.dataUrlReadMax) {
