"""CLI handlers for the ``work4you proxy`` subcommand."""
...
from work4you_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "work4you proxy requires aiohttp. Run `work4you setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "work4you"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"work4you auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Work4You proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Work4You proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: work4you proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``work4you proxy <subcommand>``."""
    ...
    print(
        "work4you proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  work4you proxy start [--provider work4you|xai] [--host 127.0.0.1] [--port 8645]\n"
