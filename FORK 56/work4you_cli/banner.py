...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `work4you ... > file`) raises
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "work4you-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `work4you update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Work4You update is available.

    Two paths: if ``WORK4YOU_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = work4you_home / ".update_check"
    embedded_rev = os.environ.get("WORK4YOU_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # WORK4YOU_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    if embedded_rev:
        ...
        # $WORK4YOU_HOME/work4you/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = work4you_home / "work4you"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Work4You git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$WORK4YOU_HOME/work4you/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``work4you_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Work4You checkout. Cached per-process. Release URL always points at the
    canonical Leow4u/FORK-56 repo (forks don't get a link).
    """
...
def _format_update_notice(behind: int) -> str:
    ...
    from work4you_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built work4you; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_work4you_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_work4you_home() / ".env"):
            try:
                st = p.stat()
