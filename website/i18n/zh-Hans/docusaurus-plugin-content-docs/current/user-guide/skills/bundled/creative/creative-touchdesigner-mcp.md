...
```
Work4You -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${WORK4YOU_HOME:-$HOME/.work4you}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

脚本将：
1. 检查 TD 是否正在运行
2. 如果尚未缓存，下载 twozero.tox
3. 将 `twozero_td` MCP 服务器添加到 Work4You 配置（如果缺失）
...
3. **重启 Work4You 会话**以加载新的 MCP 服务器
