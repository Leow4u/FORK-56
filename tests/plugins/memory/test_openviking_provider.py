...
def test_openviking_provider_config_loader_uses_readonly_config(monkeypatch):
    ...
    config = openviking_module._load_work4you_openviking_config()
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_work4you_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    from work4you_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(work4you_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Work4You's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Work4You
    venv instead of its own, and on Windows locks Work4You venv DLLs so the
    venv cannot be rebuilt during `work4you update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/work4you/.venv/Lib/site-packages")
    monkeypatch.setenv("WORK4YOU_PROFILE", "test-profile")
    ...
    assert child_env.get("WORK4YOU_PROFILE") == "test-profile"
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Work4You will retry on a later access "
...
def test_tool_add_resource_rejects_work4you_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    work4you_home = tmp_path / "work4you_home"
    work4you_home.mkdir()
    auth_json = work4you_home / "auth.json"
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "work4you"
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (work4you#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "work4you"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for work4you#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._work4you_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_work4you_openviking_config", lambda: {}
    )
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Work4You and
