...
///
/// `work4you_home_override` propagates to the child as $WORK4YOU_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, work4you_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = work4you_home_override {
        cmd.env("WORK4YOU_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, work4you_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = work4you_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_work4you_home() {
