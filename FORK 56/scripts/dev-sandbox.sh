...
unprivileged `work4you` user, giving the layout most people have —
$WORK4YOU_HOME/work4you plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  WORK4YOU_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .work4you-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `work4you setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${WORK4YOU_DEV_SANDBOX_UPSTREAM:-https://github.com/Leow4u/FORK-56.git}"
...
GIT_ROOT="${WORK4YOU_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${WORK4YOU_DEV_SANDBOX_DIR:-.work4you-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t work4you-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t work4you-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/work4you.ai"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/work4you.ai/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/work4you.ai/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://work4you.ai/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/work4you.git update-ref refs/heads/main "$next_main"; then
...
if [ "$RUN_AS_USER" = true ]; then
  SANDBOX_UID=1000
  SANDBOX_GID=1000
  SANDBOX_USER=work4you
  SANDBOX_HOME=/home/work4you
else
  SANDBOX_UID=0
  SANDBOX_GID=0
  SANDBOX_USER=root
...
# A user-level install writes the `work4you` launcher to ~/.local/bin and the
# checkout to $WORK4YOU_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/work4you.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q work4you.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t work4you-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Work4You sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/work4you-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=Work4You dev sandbox CA' \
