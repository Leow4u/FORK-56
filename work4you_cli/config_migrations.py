...
``work4you_cli.config.migrate_config``. Each step is a function
...
``work4you_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``work4you_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``work4you_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``work4you_cli.config`` (e.g.
``patch("work4you_cli.config.read_raw_config", ...)``) keep working, because
...
#: WORK4YOU_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_work4you_home()}/config.yaml and run `work4you setup` to "
...
def _cfg():
    """Return the live ``work4you_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_work4you_home = _c.get_work4you_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$WORK4YOU_HOME/plugins/`` for currently installed user plugins.
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `work4you curator status` has no
    ...
    #   3. Creates `~/.work4you/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_work4you_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_work4you_home() / "logs" / "curator"
...
# behavior set it to true explicitly via `work4you config set`.
