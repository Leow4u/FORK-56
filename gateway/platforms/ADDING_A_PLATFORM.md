...
There are two ways to add a platform to the Work4You gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.work4you/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Work4You code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `work4you gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `work4you_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Work4You core.
...
And add it to the `work4you-gateway` composite:

```python
"work4you-gateway": {
    "includes": [..., "work4you-your-platform"]
}
```
...
## 12. Status Display (`work4you_cli/status.py`)
...
## 13. Gateway Setup Wizard (`work4you_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ work4you_cli/ toolsets.py \
  --include="*.py" -l | sort -u
