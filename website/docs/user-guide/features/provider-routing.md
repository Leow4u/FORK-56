...
When using [OpenRouter](https://openrouter.ai) or [Work4You Portal](/integrations/work4you-portal) as your LLM provider, Work4You supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through Work4You Portal respects the same provider preferences — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.work4you/config.yaml`:
...
:::info
Provider routing only applies when using OpenRouter or Work4You Portal. It has no effect with direct provider connections (e.g., connecting directly to the Anthropic API).
:::
...
Provider routing controls which **sub-providers behind OpenRouter or Work4You Portal** handle your requests. For automatic failover to an entirely different provider when your primary model fails, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
