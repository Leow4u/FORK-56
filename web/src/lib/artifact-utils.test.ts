...
function makeSession(overrides: Partial<ArtifactSession> = {}): ArtifactSession {
  return {
    id: "session-1",
    last_active: 1000,
    preview: null,
...
describe("collectArtifactsForSession", () => {
  ...
  it("keeps explicit generated artifacts from tool output", () => {
    const artifacts = collectArtifactsForSession(
      makeSession({ id: "generated-session" }),
      [
        {
          content: JSON.stringify({
        ...
        {
          content: JSON.stringify({
            artifacts: [{ url: "https://cdn.example.com/generated/data.csv" }],
          }),
          role: "tool",
  ...
  it("keeps an explicit browser screenshot but ignores page assets", () => {
    const payload = JSON.stringify({
      ...
      screenshot_path: "/tmp/work4you-browser/screenshot.png",
    });

    const artifacts = collectArtifactsForSession(
      makeSession({ id: "browser-session" }),
      [
        {
...
${payload}
    ...
    expect(artifacts[0]).toMatchObject({
      kind: "image",
      value: "/tmp/work4you-browser/screenshot.png",
    });
  });

  it("keeps native browser screenshots without indexing embedded image data", () => {
    const artifacts = collectArtifactsForSession(
      makeSession({ id: "native-browser-session" }),
      [
        {
          content: {
            _multimodal: true,
            ...
            meta: {
              screenshot_path: "/tmp/work4you-browser/native-screenshot.png",
            },
        ...
        {
          content:
            "Image attached. Screenshot path: /tmp/work4you browser/summary screenshot.png",
          role: "tool",
          timestamp: 1_781_774_002,
          tool_name: "browser_vision",
        },
        {
          content:
            "Image attached. Screenshot path: C:\\Users\\Example User\\.work4you\\screenshot.png",
          role: "tool",
          timestamp: 1_781_774_003,
          tool_name: "browser_vision",
        },
    ...
    expect(artifacts.map((artifact) => artifact.value)).toEqual([
      "/tmp/work4you-browser/native-screenshot.png",
      "/tmp/work4you browser/summary screenshot.png",
      "C:\\Users\\Example User\\.work4you\\screenshot.png",
    ]);
  });
  ...
  it("normalizes session fallback timestamps and preserves existing milliseconds", () => {
    const fromSession = collectArtifactsForSession(
      makeSession({ last_active: 1_781_774_001 }),
      [
        {
    ...
    const alreadyNormalized = collectArtifactsForSession(
      makeSession({ id: "millisecond-session" }),
      [
        {
  ...
  it("falls back past invalid timestamps without multiplying Date.now", () => {
    ...
    const fromNow = collectArtifactsForSession(
      makeSession({ id: "now-session", last_active: 0, started_at: 0 }),
      [
        {
  ...
  it("resolves gateway-local image artifacts through the dashboard fs API", async () => {
    const path = "/home/me/.work4you/cache/image_generate/out.png";

    await expect(artifactImageSrc(path)).resolves.toBe(
      "data:image/png;base64,QQ==",
    );
    expect(apiMocks.readFsDataUrl).toHaveBeenCalledWith(path);
  });

  it("leaves https image sources untouched", async () => {
    await expect(
      artifactImageSrc("https://cdn.example.com/cat.png"),
    ).resolves.toBe("https://cdn.example.com/cat.png");
    expect(apiMocks.readFsDataUrl).not.toHaveBeenCalled();
  });
});
...
describe("loadArtifactsForSessions", () => {
  it("loads transcripts serially and continues after a session fails", async () => {
    ...
    const result = await loadArtifactsForSessions(sessions, async (session) => {
      ...
      try {
        ...
        if (session.id === "session-2") {
          throw new Error("Session transcript exceeds the safe-load limit");
        }
    ...
    expect(result.artifacts.map((artifact) => artifact.sessionId)).toEqual([
      "session-1",
      "session-3",
    ]);
