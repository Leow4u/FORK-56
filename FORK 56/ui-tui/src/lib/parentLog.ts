...
const logDir = join(process.env.WORK4YOU_HOME?.trim() || join(homedir(), '.work4you'), 'logs')
...
// a real ~/.work4you (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('work4you-tui: parent lifecycle log unavailable\n')
    }
  }
}
