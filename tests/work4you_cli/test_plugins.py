"""Tests for the Work4You plugin system (work4you_cli.plugins)."""
...
from work4you_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<work4you_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<work4you_home>/plugins/``; we derive
    ``<work4you_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Work4You home for this
    plugin isn't necessarily the current ``WORK4YOU_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``WORK4YOU_HOME``/``set_work4you_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <WORK4YOU_HOME>/config.yaml.
        # Config is always read from WORK4YOU_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            work4you_home = Path(home)
        else:
            import os
            work4you_home_str = os.environ.get("WORK4YOU_HOME")
            if work4you_home_str:
                work4you_home = Path(work4you_home_str)
            else:
                work4you_home = base.parent
        work4you_home.mkdir(parents=True, exist_ok=True)
        cfg_path = work4you_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_enabled_portable_plugin_registers_components(
        ...
        from work4you_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("WORK4YOU_HOME", str(home))
        monkeypatch.setenv("WORK4YOU_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "work4you_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you_test"))
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under work4you_plugins.<name>."""
        plugins_dir = tmp_path / "work4you_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you_test"))

        # Clean up any prior namespace module
        sys.modules.pop("work4you_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "work4you_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / "work4you_test"
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``work4you.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "work4you_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $WORK4YOU_HOME/plugins/.
        work4you_home = tmp_path / "work4you_test"
        plugins_dir = work4you_home / "plugins"
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.config.load_config", lambda: {"hooks": {}}
        )
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from work4you_cli import observability
        from work4you_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from work4you_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("work4you_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from work4you_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from work4you_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("work4you_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `work4you plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="work4you_cli.plugins"):
    ...
    def test_plugin_manager_scoped_by_work4you_home_override(self, tmp_path):
        """set_work4you_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``WORK4YOU_HOME`` via a
        context-local ContextVar, which — per
        ``work4you_constants.set_work4you_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``WORK4YOU_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('WORK4YOU_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['WORK4YOU_HOME'] itself (simulating
        # a real plugin like work4you-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_work4you_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_work4you_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``work4you_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``work4you_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `work4you_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("work4you_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("work4you_cli.plugins.PluginContext.dispatch_tool.__module__", "work4you_cli.plugins"):
...
class TestPluginDebugLogging:
    """WORK4YOU_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("WORK4YOU_PLUGINS_DEBUG", raising=False)
        from work4you_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from WORK4YOU_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """WORK4YOU_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """WORK4YOU_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".work4you" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WORK4YOU_HOME", str(prof))
