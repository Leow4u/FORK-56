...
- Hosted: API key from [app.supermemory.ai/integrations?connect=work4you](http://app.supermemory.ai/integrations?connect=work4you)
...
Before running `work4you memory setup`, add the local endpoint to
...
Then run `work4you memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$WORK4YOU_HOME/supermemory.json`
...
| `container_tag` | `work4you` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `work4you-{identity}` → `work4you-coder`). |
...
`https://api.supermemory.ai`. Work4You resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: work4you`, and document writes stamp
`metadata.sm_source: work4you`. This is a **functional routing key, not telemetry**:
it groups Work4You-written memories into a dedicated "Work4You" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Work4You profile:

```json
{
  "container_tag": "work4you-{identity}"
}
```

For a profile named `coder`, this resolves to `work4you-coder`. The default profile resolves to `work4you-default`. Without `{identity}`, all profiles share the same container.
