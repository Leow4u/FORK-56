...
the tool registry or launching Work4You (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``work4you-cli`` to the non-existent ``work4you``. ``resolve_toolset('work4you')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``work4you-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``work4you`` instead of ``work4you-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"work4you-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `work4you tools` to reconfigure."
        )
    return warnings
