#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the Work4You desktop GUI.
...
- ``surface="app"`` — the Work4You desktop app's own DOM (tours of Work4You itself).
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the Work4You desktop app.")
...
TOUR_SCHEMA = {
    "name": "tour",
    "description": (
        "Give a live guided tour in the Work4You desktop GUI: dim the screen, "
        "highlight an element, and attach a popover with your own title/text. "
        "Works on two surfaces — 'app' (the Work4You app itself) and 'preview' "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "surface": {
                "type": "string",
                "enum": list(SURFACES),
                "description": "Where the tour runs: 'app' (Work4You desktop UI, default) or 'preview' (the page in the in-app browser pane).",
            },
            "selector": {
                "type": "string",
