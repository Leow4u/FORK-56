...
//
// When someone is in a hands-free "Hey Work4You" voice chat, the natural way to
...
// phrase (optionally addressed to Work4You), so a real turn that merely contains
...
// Optional address prefixes so "work4you stop" / "ok stop" / "hey work4you, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey work4you', 'hey work4you,', 'work4you', 'work4you,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("work4you") is not a stop command on its own.
      continue
    }
 ...
 * to Work4You). Returns false for anything that merely contains "stop" as part of
