"""Tests for IRC gateway configuration via `work4you setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Work4You install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Work4You install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#work4you")
            monkeypatch.setenv("IRC_NICKNAME", "work4you-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `work4you setup gateway` experience with IRC present."""
