...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/devops/watchers` |
...
All three live in `$WORK4YOU_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$WORK4YOU_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $WORK4YOU_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${WORK4YOU_HOME:-~/.work4you}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $WORK4YOU_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name work4you-issues --repo Leow4u/FORK-56 --scope issues
```

Poll an arbitrary JSON API:

```bash
python $WORK4YOU_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$WORK4YOU_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $WORK4YOU_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $WORK4YOU_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$WORK4YOU_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
