...
``<WORK4YOU_HOME>/state/gateway.lifecycle.json``:
...
def _process_work4you_home() -> Path:
    """WORK4YOU_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<WORK4YOU_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``work4you_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
