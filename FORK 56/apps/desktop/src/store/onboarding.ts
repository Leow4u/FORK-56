...
const CONFIGURED_CACHE_KEY = 'work4you-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'work4you-onboarding-skipped-v1'
const POLL_MS = 2000
...
// mirroring work4you_cli/work4you_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Work4You auto-routes unconfigured tools through the Tool
...
async function fetchProviderDefaultModel(
  ...
  // `work4you model` does (for Work4You it honors the user's free/paid tier, so a
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Work4You could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Work4You still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Work4You still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Work4You Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Work4You still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Work4You still cannot reach ${url}.` }
    }
