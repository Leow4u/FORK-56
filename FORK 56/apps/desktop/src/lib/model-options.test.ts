...
const globalOptions = { model: 'work4you-4', provider: 'work4you', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'work4you-local', provider: 'work4you-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['work4you-local'], name: 'Work4You Local vLLM', slug: 'work4you-local' }]
    }
    ...
    await expect(requestModelOptions({ gateway: gateway as never, sessionId: 'session-1' })).resolves.toEqual({
      ...restPayload,
      model: 'work4you-local',
      provider: 'work4you-local'
    })
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'work4you-local', provider: 'work4you-local', providers: [] }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'Work4You', slug: 'work4you', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'work4you', 'work4you-4')).toBe(false)
  })
