"""Guard: Work4You-owned subprocesses must not resolve managed runtimes by bare PATH.

Work4You installs runtimes for itself — ``uv`` at ``$WORK4YOU_HOME/bin/uv``, Node at
``$WORK4YOU_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Work4You's own
...
* when a system copy also exists, the one Work4You does not own wins — which is
...
``iter_work4you_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Work4You provisions into WORK4YOU_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Work4You-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("work4you_cli/update_cmd.py", "npm"): (
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Work4You-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$WORK4YOU_HOME/bin (uv) and $WORK4YOU_HOME/node are not on an "
        ...
        "  node/npm -> work4you_constants.find_node_executable()\n"
        "  PATH env -> work4you_constants.iter_work4you_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(work4you_constants, helper))


def test_managed_uv_helpers_exist():
    from work4you_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
