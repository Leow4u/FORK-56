"""ACP session manager — maps ACP sessions to Work4You AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.work4you/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Work4You itself is running in WSL.

    Windows ACP clients can launch ``work4you acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from work4you_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Work4You from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["work4you-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Work4You agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Work4You AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Work4You runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.work4you/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``WORK4YOU_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_work4you_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=work4you_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see work4you_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see Work4YouACPAgent._schedule_mcp_late_refresh).
        try:
            from work4you_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        # editor/session cwd instead of the Work4You daemon's process cwd.
