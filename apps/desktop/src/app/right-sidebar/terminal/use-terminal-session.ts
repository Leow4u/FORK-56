...
const WORK4YOU_PATHS_MIME = 'application/x-work4you-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(WORK4YOU_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(WORK4YOU_PATHS_MIME)
  ...
  const getPath = window.work4youDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.work4youDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (work4you --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.work4youDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[work4you-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.work4youDesktop?.terminal?.write(sessionId, `${command}\r`)
