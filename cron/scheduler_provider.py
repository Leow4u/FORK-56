...
def fire_overdue_jobs(
    ...
    *,
    ...
    from cron.jobs import _ensure_aware, _work4you_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global WORK4YOU_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `work4you cron
        ...
        # Heartbeat once before the first sleep so `work4you cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `work4you cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_work4you_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_work4you_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `work4you cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
