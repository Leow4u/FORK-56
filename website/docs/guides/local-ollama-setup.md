---
sidebar_position: 9
title: "Run Work4You Locally with Ollama — Zero API Cost"
description: "Step-by-step guide to running Work4You entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---

# Run Work4You Locally with Ollama — Zero API Cost
...
You'll set up Work4You running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Work4You works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Work4You connected to Ollama as a custom endpoint
...
```bash
# ~/.work4you/.env
WORK4YOU_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Work4You is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Work4You experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Work4You with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Alternatively, edit `~/.work4you/config.yaml` directly:
...
Work4You will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Work4You requires at least 64,000 tokens for agentic work with tools:
...
Then update your Work4You config to use `gemma4-64k` as the model name.
...
Once Work4You works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.

### Telegram

1. Create a bot via [@BotFather](https://t.me/BotFather) and get the token
2. Add to your `~/.work4you/config.yaml`:
...
Work4You sends a fixed payload on every API call — the system prompt plus the tool schemas for all enabled tools — before any of your conversation content. On CPU-only or low-VRAM setups, processing that prompt (the *prefill* phase) dominates the first turn: the model can sit silent for minutes while it works through the prompt, then generate at its normal pace. This is expected behaviour, not a hang. The [Mac local-LLM guide](./local-llm-on-mac.md#timeouts) documents the same effect — during prefill on large contexts, local models may produce no output for minutes while processing the prompt — and Work4You automatically raises its stream read timeout from 120s to 1800s for local endpoints (`WORK4YOU_STREAM_READ_TIMEOUT`).
...
- **Widen the API timeout** — set `WORK4YOU_API_TIMEOUT=1800` in `~/.work4you/.env` (see [What You Need](#what-you-need)).
- **Measure and trim the fixed prompt** — run `work4you prompt-size` for a byte breakdown of the system prompt and tool schemas, then disable unused toolsets with `work4you tools` and uninstall skills you don't need with `work4you skills`.
...
- **Terminal commands** — Work4You wraps the command, runs it, reads output regardless of model
...
- **Long context windows** — cloud models offer 100K–1M tokens; local runtimes often default below Work4You' 64K minimum unless you configure them
