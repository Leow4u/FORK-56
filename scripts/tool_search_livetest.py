#!/usr/bin/env python3
"""Live test harness for Work4You's Tool Search feature.
...
# Force-isolate the test environment BEFORE any work4you imports.
ORIGINAL_HOME = os.environ.get("WORK4YOU_HOME")
ORIGINAL_AUTH = Path.home() / ".work4you" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.work4you/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.work4you/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="work4you_ts_live_"))
    work4you_home = home_dir / ".work4you"
    work4you_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, work4you_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".work4you" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, work4you_home / ".env")
        ...
        load_work4you_dotenv(work4you_home=str(Path.home() / ".work4you"))
    ...
    (work4you_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new WORK4YOU_HOME takes effect."""
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["WORK4YOU_HOME"] = str(home)
...
def main():
    ...
    # Restore original WORK4YOU_HOME
    if ORIGINAL_HOME is not None:
        os.environ["WORK4YOU_HOME"] = ORIGINAL_HOME
    else:
