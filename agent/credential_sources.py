"""Unified removal contract for every credential source Work4You reads from.

Work4You seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.work4you/.env
    claude_code   — ~/.claude/.credentials.json
    work4you_pkce   — ~/.work4you/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (work4you, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `work4you auth add`
...
unify here is **removal**:
    ...
    ``work4you auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, work4you device_code (partial), work4you_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"work4you"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.work4you/.env  → clear it
    ...
    """
    from work4you_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in work4you_cli/config.py. read_text() with no
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Work4You stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `work4you auth add anthropic` to re-enable if needed.",
    ])


def _remove_work4you_pkce(provider: str, removed) -> RemovalResult:
    """~/.work4you/.anthropic_oauth.json is ours — delete it outright."""
    from work4you_constants import get_work4you_home

    result = RemovalResult()
    oauth_file = get_work4you_home() / ".anthropic_oauth.json"
...
def _remove_work4you_device_code(provider: str, removed) -> RemovalResult:
    """Work4You OAuth lives in auth.json providers.work4you — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `work4you auth add work4you` (or any other path that writes providers.work4you)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `work4you auth add work4you` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Work4You: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``work4you auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `work4you model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Work4You auth store is not enough — _seed_from_singletons() would
    ...
    via ``work4you auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `work4you auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `work4you auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Work4You stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `work4you auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="work4you_pkce",
        remove_fn=_remove_work4you_pkce,
        description="~/.work4you/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="work4you", source_id="device_code",
        remove_fn=_remove_work4you_device_code,
        description="auth.json providers.work4you",
    ))
    register(RemovalStep(
