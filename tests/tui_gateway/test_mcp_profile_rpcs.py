...
``WORK4YOU_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def work4you_root(tmp_path, monkeypatch):
    """A temp WORK4YOU_HOME root with two named profiles: 'work' and 'other'.

    Pointing WORK4YOU_HOME at a dir outside ~/.work4you makes it the profile ROOT
    (get_default_work4you_root's Docker/custom branch), so named profiles live at
...
def test_add_lands_in_named_profile_only(work4you_root):
    root = work4you_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(work4you_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(work4you_root):
    root = work4you_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(work4you_root):
    root = work4you_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(work4you_root):
    root = work4you_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(work4you_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(work4you_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(work4you_root):
    root = work4you_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == WORK4YOU_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
