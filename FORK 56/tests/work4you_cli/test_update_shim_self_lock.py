...
``venv\\Scripts\\work4you.exe`` is a launcher that runs the interpreter with the
...
is holding, which Windows refuses — so ``work4you update`` re-runs itself under
...
SHIM_NAMES = ["work4you.exe", "work4you.exe", "work4you-acp.exe", "work4you-gateway.exe"]
...
def venv(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["work4you", "update"])
...
def test_detects_shim_from_zipapp_main_py(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "work4you.exe" / "__main__.py")])
    assert cli_main._windows_shim_in_process_chain() == venv / "work4you.exe"


def test_detects_shim_from_main_module_spec_origin(venv, monkeypatch):
    fake_main = types.SimpleNamespace(
        __file__=None,
        __spec__=types.SimpleNamespace(origin=str(venv / "work4you.exe")),
    )
...
def test_detects_shim_in_ancestor_chain(venv, monkeypatch):
    ...
    _fake_psutil(monkeypatch, [str(venv / "work4you.exe")])
    assert cli_main._windows_shim_in_process_chain() == venv / "work4you.exe"


def test_ignores_work4you_exe_outside_the_project_venv(venv, monkeypatch, tmp_path):
    """A shim from some other install must never trigger a re-exec."""
    other = tmp_path / "other" / "Scripts"
    other.mkdir(parents=True)
    monkeypatch.setattr(sys, "argv", [str(other / "work4you.exe"), "update"])
    _fake_psutil(monkeypatch, [str(other / "work4you.exe")])
...
def test_no_shim_off_windows(venv, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", [str(venv / "work4you.exe"), "update"])
    assert cli_main._windows_shim_in_process_chain() is None
...
def test_reexec_runs_same_args_under_venv_python(venv, monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", [str(venv / "work4you.exe"), "update", "--yes"])
    calls = _capture_popen(monkeypatch)

    assert cli_main._reexec_update_off_windows_shim() is True
    cmd, env, kwargs = calls[0]
    assert cmd == [
        str(venv / "python.exe"), "-m", "work4you_cli.main", "update", "--yes",
    ]
...
def test_reexec_falls_through_when_venv_python_is_missing(venv, monkeypatch, capsys):
    ...
    assert cli_main._reexec_update_off_windows_shim() is False
    assert "-m work4you_cli.main update" in capsys.readouterr().out
...
def test_pending_rename_filter_drops_only_our_shim_pairs():
    shims = [Path(r"C:\work4you\venv\Scripts\work4you.exe")]
    entries = [
        r"\??\C:\other\thing.dll", r"!\??\C:\other\thing.dll.bak",
        r"\??\C:\work4you\venv\Scripts\work4you.exe",
        r"!\??\C:\work4you\venv\Scripts\work4you.exe.old.1755624735000",
    ]
...
def test_pending_rename_filter_keeps_a_shim_pair_with_a_foreign_target():
    shims = [Path(r"C:\work4you\venv\Scripts\work4you.exe")]
    entries = [
        r"\??\C:\work4you\venv\Scripts\work4you.exe", r"!\??\C:\somewhere\else.exe",
    ]
    kept, removed = cli_main._filter_pending_shim_renames(entries, shims)
    assert removed == 0
    assert kept == entries
