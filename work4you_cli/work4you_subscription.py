"""Helpers for Work4You subscription managed-tool capabilities."""
...
# Maps a tools_config provider's ``managed_work4you_feature`` to the tool-pool
# coverage category (work4you_cli.work4you_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `work4you tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_work4you_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"work4you"`` for the managed selection (stored
    ``work4you`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``work4you status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
@dataclass(frozen=True)
class Work4YouSubscriptionFeatures:
    subscribed: bool
    work4you_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, Work4YouFeatureState]
    account_info: Optional[Work4YouPortalAccountInfo] = None

    @property
    def web(self) -> Work4YouFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> Work4YouFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> Work4YouFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> Work4YouFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> Work4YouFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> Work4YouFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> Work4YouFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[Work4YouFeatureState]:
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Work4You-managed Node dirs (Windows installer / POSIX $WORK4YOU_HOME/node)
        ...
        managed_path = with_work4you_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_work4you_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("WORK4YOU_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_work4you_subscription_features(
    ...
    *,
    ...
    provider_is_nous = str(model_cfg.get("provider") or "").strip().lower() == "work4you"

    try:
        if force_fresh:
            account_info = get_work4you_portal_account_info(force_fresh=True)
        else:
            account_info = get_work4you_portal_account_info()
    except Exception:
        account_info = None
    ...
    work4you_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_nous or work4you_auth_present
    ...
    # requires `pip install faster-whisper`. For Work4You subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_work4you_managed_defaults below.
    ...
    # "work4you" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "work4you"
    tts_use_gateway = tts_selected == "work4you"
    stt_use_gateway = stt_selected == "work4you"
    browser_use_gateway = browser_selected == "work4you"
    image_use_gateway = image_selected == "work4you"
    video_use_gateway = video_selected == "work4you"

    # The "work4you" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "work4you" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "work4you" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "work4you" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "work4you" or browser_use_gateway:
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("WORK4YOU_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("WORK4YOU_LOCAL_STT_COMMAND"))
    ...
    features = {
        ...
        "image_gen": Work4YouFeatureState(
            key="image_gen",
            ...
            current_provider="FAL" if (image_selected not in (None, "work4you") or (image_selected is None and direct_fal)) else ("Work4You Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": Work4YouFeatureState(
            key="video_gen",
            ...
            current_provider="FAL" if (video_selected not in (None, "work4you") or (video_selected is None and direct_fal_video)) else ("Work4You Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": Work4YouFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
    ...
    return Work4YouSubscriptionFeatures(
        subscribed=subscribed,
        work4you_auth_present=work4you_auth_present,
...
def apply_work4you_managed_defaults(
    ...
    *,
    ...
    if "tts" in selected_toolsets and not features.tts.explicit_configured and not (
        ...
        tts_cfg["provider"] = "work4you"
        tts_cfg.pop("use_gateway", None)
        changed.add("tts")

    # STT: same pattern as TTS. The DEFAULT_CONFIG seed is "local"
    # (requires `pip install faster-whisper`); for Work4You subscribers we
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "work4you"
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Work4You subscriber or
    is not using Work4You as their provider.
    """
    ...
    # Quick provider check without the heavy get_work4you_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "work4you":
        return [], [], []
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from work4you_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_work4you_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Work4You subscription"
    ...
    if pool_only:
        title = "Your free Work4You tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Work4You subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Work4You Portal login for the Tool Gateway picker (`work4you tools`)
# ---------------------------------------------------------------------------


def ensure_work4you_portal_access(
    *,
    capability: str = "the Work4You Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Work4You Tool Gateway, logging in if
    needed.

    Used by ``work4you tools`` when a user selects a Work4You-managed Tool Gateway
    backend (e.g. "Firecrawl (Work4You Portal)").  Unlike ``work4you model``'s Work4You
    login, this:
    ...
    It only performs the Work4You Portal device-code OAuth (when the user isn't
    ...
    if info is None or not info.logged_in:
        if not _run_work4you_portal_login_only(capability=capability):
            return False
        try:
            info = get_work4you_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_work4you_portal_login_only(*, capability: str) -> bool:
    """Run the Work4You Portal device-code OAuth and persist credentials only.
    ...
    try:
        from work4you_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _work4you_device_code_login,
            _read_shared_work4you_state,
            _save_auth_store,
            _save_provider_state,
            _sync_work4you_pool_from_auth_store,
            _try_import_shared_work4you_state,
            _write_shared_work4you_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Work4You Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Work4You Portal login.")
    try:
        proceed = input("  Log in to Work4You Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Work4You Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Work4You.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Work4You OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_work4you_state(timeout_seconds=15.0)
        ...
        _sync_work4you_pool_from_auth_store()
        print("  Work4You Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _work4you_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Work4You Portal login failed: {exc}")
        return False
