"""Strict tool-provider selection: the `work4you tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "work4you" → managed Work4You Tool Gateway only; a vendor name →
...
selection and pointing at `work4you tools`.
...
MANAGED = SimpleNamespace(
    work4you_user_token="managed-token",
    gateway_origin="https://gateway.work4you.ai",
)
...
class TestReadSelection:
    ...
    def test_work4you_provider_returned(self):
        with self._with_raw({"image_gen": {"provider": "work4you"}}):
            assert tbh.read_selection("image_gen") == "work4you"

    def test_legacy_use_gateway_true_maps_to_nous(self):
        """Old configs stored use_gateway: true beside a vendor name — only
        the managed picker row ever wrote it, so it means 'work4you'."""
        with self._with_raw({"video_gen": {"provider": "fal", "use_gateway": True}}):
            assert tbh.read_selection("video_gen") == "work4you"
...
class TestImageFalStrictSelection:
    def test_work4you_selection_routes_managed_even_with_fal_key(self):
        from tools import image_generation_tool as it

        with patch.object(it, "read_selection", return_value="work4you"), \
    ...
    def test_work4you_selection_unentitled_raises_selection_error(self):
        from tools import image_generation_tool as it
        ...
        assert "image_gen is configured to use work4you" in str(exc.value)
        assert "work4you tools" in str(exc.value)
...
class TestVideoFalStrictSelection:
    def test_work4you_selection_routes_managed_even_with_fal_key(self):
        ...
        with patch("tools.tool_backend_helpers.read_selection", return_value="work4you"), \
...
class TestSttStrictSelection:
    def test_work4you_selection_beats_direct_openai_key(self):
        from tools import transcription_tools as tt

        with patch.object(tt, "_load_stt_config", return_value={"openai": {"api_key": "sk-direct"}}), \
             patch("tools.tool_backend_helpers.read_selection", return_value="work4you"), \
        ...
        assert base_url.startswith("https://gateway.work4you.ai")
...
class TestBrowserUseStrictSelection:
    ...
    def test_work4you_selection_routes_managed_even_with_direct_key(self):
...
class TestWriteProviderConfig:
    ...
    def test_managed_image_row_persists_work4you_provider(self):
        from work4you_cli.tools_config import _write_provider_config

        config = {}
        provider = {"name": "Work4You Subscription", "imagegen_backend": "fal"}
        _write_provider_config(provider, config, managed_feature="image_gen")
        assert config["image_gen"]["provider"] == "work4you"
    ...
    def test_plugin_injected_byok_row_clears_stale_use_gateway(self):
        ...
        config = {"stt": {"provider": "work4you", "use_gateway": True}}
