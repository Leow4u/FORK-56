...
from work4you_cli import config as work4you_config
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(work4you_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(work4you_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(work4you_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(work4you_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(work4you_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(work4you_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(work4you_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(work4you_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(work4you_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    work4you_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_work4you_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import work4you_constants

    monkeypatch.delattr(work4you_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(work4you_constants, "apply_subprocess_home_env")

    work4you_main._reload_updated_runtime_modules()

    assert callable(work4you_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        work4you_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        work4you_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(work4you_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        work4you_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        work4you_main, "_discard_stashed_changes",
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``work4you update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(work4you_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".work4you-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by work4you update (work4you_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "work4you-update-autostash")

    assert marker.exists(), (
        ".work4you-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".work4you-bootstrap-complete" not in status
