...
class TestCacheFileReadBlocking:
    """Internal Work4You cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        work4you_home = tmp_path / ".work4you"
        cache = work4you_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._work4you_home_path", return_value=work4you_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        work4you_home = tmp_path / ".work4you"
        hub = work4you_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_work4you_home(self, tmp_path):
        """The env basename guard does not depend on WORK4YOU_HOME resolution."""
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        work4you_home = tmp_path / ".work4you"
        cache = work4you_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
