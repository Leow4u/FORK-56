"""
Dump command for work4you CLI.

Outputs a compact, plain-text summary of the user's Work4You setup
...
from work4you_cli.config import get_work4you_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.work4you/.env.
    ...
    exports. ``work4you debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.work4you_build_sha`` by the Dockerfile's
    ``WORK4YOU_GIT_SHA`` build-arg (see ``work4you_cli/build_info.py``).
    ...
    try:
        from work4you_cli.build_info import get_build_sha
...
def _redact(value: str) -> str:
    ...
    ``work4you dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from work4you_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(work4you_home: Path) -> int:
...
def _cron_summary(work4you_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = work4you_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"work4you_home:      {display_work4you_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.work4you/.env: a managed
        ...
        # A credential added via `work4you auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `work4you auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["work4you-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(work4you_home)}")
    lines.append(f"  skills:             {_count_skills(work4you_home)}")
