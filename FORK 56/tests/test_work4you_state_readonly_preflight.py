...
- files inside the Work4You home tree are repaired with ``chmod u+rw``
...
from work4you_state import SessionDB, preflight_db_writability
...
def work4you_home(tmp_path, monkeypatch):
    """Isolated WORK4YOU_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, work4you_home):
        db = work4you_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, work4you_home):
        db = work4you_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, work4you_home):
        sub = work4you_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, work4you_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, work4you_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, work4you_home):
        db = work4you_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, work4you_home):
        db_path = work4you_home / "state.db"
    ...
    def test_sessiondb_actionable_error_outside_home(
        ...
        work4you_state._set_last_init_error(None)
        try:
            with pytest.raises(sqlite3.OperationalError) as exc_info:
                SessionDB(db_path)
            msg = str(exc_info.value)
