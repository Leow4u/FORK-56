...
Profile-aware location: ``$WORK4YOU_HOME/logs/dashboard-auth.log``.
...
from work4you_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $WORK4YOU_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="work4you", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="work4you", user_id="u1",
    ...
    assert second["provider"] == "work4you"
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="work4you", access_token="should-not-appear",
