...
``platform="tui"`` / ``chat_id=WORK4YOU_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        work4you_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_work4you_root()``, which resolves the process env and
        ...
        token = set_work4you_home_override(str(other_profile_home))
        try:
