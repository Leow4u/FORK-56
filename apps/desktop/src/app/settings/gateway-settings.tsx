...
// Work4You Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- Work4You Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.work4youDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.work4youDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteWork4YouPath: state.sshRemoteWork4YouPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.work4youDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.work4youDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'work4you-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.work4youDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.work4youDesktop.oauthLoginConnectionConfig(trimmedUrl)
  ...
  const signOut = async () => {
    ...
    try {
      await window.work4youDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.work4youDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.work4youDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.work4youDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.work4youDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.work4youDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.work4youDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Work4You Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.work4you.ai/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/work4you"
        ...
        <SettingsGroup>
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteWork4YouPath: event.target.value }))}
                placeholder={g.sshWork4YouPathPlaceholder}
                value={state.sshRemoteWork4YouPath}
              />
            }
