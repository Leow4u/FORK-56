...
Work4You has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Work4You can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`work4you fallback` reuses the provider picker from `work4you model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.work4you/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Work4You still honors it for back-compat, but `work4you fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Work4You Portal | `work4you` | `work4you setup --portal` (fresh) or `work4you auth add work4you` (OAuth) |
| OpenAI Codex | `openai-codex` | `work4you model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth) |
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `work4you model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `work4you model` (Qwen Portal OAuth; optional: `WORK4YOU_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `work4you model` (MiniMax portal OAuth) |
...
**Work4You Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `work4you fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Work4You uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `work4you kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Work4You first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, Work4You now honors user-configured fallback policy before using the built-in discovery chain:
...
```text
OpenRouter → Work4You Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → Work4You Portal →
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | work4you | codex | main | anthropic
...
Every task above follows the same **provider / model / base_url** pattern. Each task can also declare its own `fallback_chain`; if omitted, `provider: auto` uses the top-level `fallback_providers` chain before Work4You' built-in auxiliary discovery chain.
...
| `"codex"` | Force Codex OAuth | `work4you model` → ChatGPT or Codex Subscription |
...
`base_url` takes precedence over `provider`. Work4You uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Work4You treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Work4You falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Work4You logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Work4You recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Work4You doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | work4you | main
    model: "google/gemini-3-flash-preview"
```
...
If no provider is available for compression, Work4You drops middle conversation turns without generating a summary rather than failing the session.
