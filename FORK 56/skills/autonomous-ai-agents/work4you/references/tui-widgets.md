...
Author widget apps for the Work4You TUI (`work4you --tui`): glanceable ambient
...
- The TUI must be in use (`work4you --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.work4you/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.work4you/logs/tui_gateway_crash.log` if a widget never appears.
