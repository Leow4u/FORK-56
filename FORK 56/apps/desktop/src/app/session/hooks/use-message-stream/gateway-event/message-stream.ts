 ...
 * billing-specific toast — never the generic "Work4You error" — with a smart CTA
 * (Work4You → in-app Settings → Billing, other providers → their billing page).
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    icon: 'credit-card',
    title: block.is_nous
      ? translateNow('billingBlock.titleWork4You')
