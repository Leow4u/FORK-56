...
export async function ensureStripeCustomer(
  ...
  return resolveStripeCustomerId({
    storedId: org.stripeCustomerId,
    retrieve: async (id) => {
      const customer = await getStripe().customers.retrieve(id)
      return { deleted: Boolean('deleted' in customer && customer.deleted) }
    },
    create: () => createAndStoreCustomer(org, email),
  })
}
