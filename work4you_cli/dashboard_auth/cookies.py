...
Three cookies in play:
  - work4you_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - work4you_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Work4You Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / work4you
  ...
  - work4you_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/work4you) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/work4you`` covers
...
setter wrote ``__Secure-work4you_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "work4you_session_at"
SESSION_RT_COOKIE = "work4you_session_rt"
...
# auth plugins are enabled (for example Basic + Work4You OAuth).
SESSION_PROVIDER_COOKIE = "work4you_session_provider"
PKCE_COOKIE = "work4you_session_pkce"
...
SSO_ATTEMPT_COOKIE = "work4you_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /work4you`` we want ``Path=/work4you`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Work4You Portal
    issues a 24h rotating refresh token (work4you #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/work4you``)
