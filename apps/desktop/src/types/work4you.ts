...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Work4You doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in work4you_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `work4you model` uses — instead of
...
export interface Work4YouConfig {
  agent?: {
...
export type Work4YouConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `work4you model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Work4You only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Work4You only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Work4You simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (work4you.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors work4you_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Work4You, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `work4you tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Work4You entitlement ∧
 ...
 *  not Work4You); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/work4you/update/check` — the backend's own update state.
...
/** One Work4You-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Work4You Tool Gateway as a result of
   *  switching the main provider to Work4You. Empty unless provider === 'work4you'
