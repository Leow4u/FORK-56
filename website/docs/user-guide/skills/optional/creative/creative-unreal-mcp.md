...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Work4You MCP catalog. The
...
Two halves, in this order: the editor side must be up before Work4You connects.
...
### One-time, Work4You side

    work4you mcp install unreal-engine
...
`~/.work4you/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Work4You — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Work4You connects
...
2. Start the Work4You session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Work4You
they appear as:

| Work4You tool | Purpose |
|---|---|
...
then happens at `work4you mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Work4You runs batched calls
...
- **Start order matters.** Editor + server up first, then the Work4You
...
- **Timeouts on long operations.** Work4You' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.work4you/config.yaml` for
