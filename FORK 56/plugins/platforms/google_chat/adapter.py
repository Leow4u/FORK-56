...
# ``work4you_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Work4You is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a work4you MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "work4you-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${WORK4YOU_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _work4you_home = _get_work4you_home()
        except (ModuleNotFoundError, ImportError):
            _work4you_home = _Path.home() / ".work4you"
        self._thread_count_store = _ThreadCountStore(
            _work4you_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Work4You is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("WORK4YOU_HOME", str(_Path.home() / ".work4you"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${WORK4YOU_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a work4you MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Work4You is thinking…' marker message.
...
def interactive_setup() -> None:
    ...
    prompt for env vars, persist them to ``~/.work4you/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. work4you-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.work4you/.env")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Work4You plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `work4you setup` to install Google Chat support.",
        ...
        # the "Work4You is thinking..." marker patches and edit overhead.
