...
Chronos (the Greek god of time, alongside Work4You) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Work4You token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Work4You token (the agent
        ...
        if not (_cfg("cron", "chronos", "portal_url") and _cfg("cron", "chronos", "callback_url")):
            return False
        return self._have_work4you_token()

    def _have_work4you_token(self) -> bool:
        """True if the agent has a Work4You Portal login (no network call).

        Checks the stored auth state for a Work4You access token — does NOT refresh
