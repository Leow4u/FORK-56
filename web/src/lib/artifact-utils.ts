...
function untrustedToolPayload(value: string): null | string {
  ...
  return (
    payloadStart === -1 ? wrapped : wrapped.slice(payloadStart + 2)
  ).trim();
}
...
function looksLikeArtifact(value: string): boolean {
  ...
  return (
    ...
    (IMAGE_EXT_RE.test(value) || FILE_EXT_RE.test(value))
  );
}
...
function artifactHref(value: string): string {
  ...
  // Browser cannot open gateway-local file:// paths. Keep the original value;
  // ArtifactsPage opens files through the authenticated `/api/fs` surface.
  return value;
}
...
function normalizeArtifactTimestamp(
...
function collectStringValues(
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) =>
      collectStringValues(entry, `${keyPath}.${index}`, collector),
    );
...
function toolName(message: ArtifactMessage): string {
...
function explicitToolArtifactKey(
  keyPath: string,
  producerTool: boolean,
): boolean {
  return keyPath
    .split(".")
    ...
    .some(
      (segment) =>
        STRONG_TOOL_ARTIFACT_KEY_RE.test(segment) ||
        (producerTool && PRODUCER_TOOL_ARTIFACT_KEY_RE.test(segment)),
    );
}

function structuredToolPayload(message: ArtifactMessage): null | unknown {
  ...
  if (
    ...
    (content as Record<string, unknown>)._multimodal === true
  ) {
...
export async function loadArtifactsForSessions(
  ...
  loadMessages: (session: ArtifactSession) => Promise<ArtifactMessage[]>,
...
export function isHttpArtifact(value: string): boolean {
