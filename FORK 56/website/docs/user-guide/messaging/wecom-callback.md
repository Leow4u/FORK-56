...
Connect Work4You to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Work4You supports two WeCom integration modes:
...
> Run `work4you gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Work4You decrypts the message, queues it for the agent
...
(Use `work4you gateway start` only after `work4you gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Work4You and the token the
...
in `~/.work4you/.env` values around `=` will also break signature checks. After
fixing, restart `work4you gateway run`.
...
Check `work4you gateway run` logs for the bound host/port. If the adapter bound to
