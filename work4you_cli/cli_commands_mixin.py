...
``cli.py``'s ``Work4YouCLI`` class. ``Work4YouCLI`` inherits ``CLICommandsMixin`` so
...
from work4you_constants import display_work4you_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Work4You did)
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Work4You changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Work4You hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Work4You config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_work4you_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from work4you_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `work4you profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: work4you -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `work4you journey`).
        ...
        from cli import _cprint
        from work4you_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from work4you_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from work4you_cli.clipboard import (
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in work4you_cli/colors.py
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from work4you_cli.main import _resolve_session_by_name_or_id
        ...
        # same directory as a startup `work4you -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `work4you -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `work4you -w`.
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from work4you_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from work4you_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to work4you_cli.curator so the CLI and the `work4you curator`
        ...
        try:
            from work4you_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to work4you_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from work4you_cli.skills_hub import handle_skills_slash
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from work4you_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``work4you bundles list`` but renders inside the running
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Work4You-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `work4you cron`.
        """
        from cli import _DIM, _RST, _cprint
        from work4you_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from work4you_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Work4You process runs — use `work4you cron` for "
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from work4you_cli.loops import dispatch_loop_command
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from work4you_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_work4you_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="work4you_prompt_")
        try:
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from work4you_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from work4you_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from work4you_cli.focus_view import format_hidden_line
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Work4You is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                ...
                behavior = "Enter will redirect the current run while Work4You is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from work4you_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from work4you_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug work4you``   → upload to Work4You-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``work4you`` and ``local`` are mutually exclusive; if both are given,
        ...
        work4you = "work4you" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, work4you=work4you, yes=True
        )
    ...
    def _handle_update_command(self) -> bool:
        ...
        ``work4you update`` so the user sees update output directly and gets
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Work4You' hotword listener.
