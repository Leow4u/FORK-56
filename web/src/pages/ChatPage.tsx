/**
 * ChatPage — embeds `work4you --tui` inside the dashboard.
 *
...
const PTY_ATTACH_TOKEN_KEY = "work4you.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__WORK4YOU_SESSION_TOKEN__ &&
    !window.__WORK4YOU_AUTH_REQUIRED__
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("work4you-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("work4you-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__WORK4YOU_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Work4You TUI has enabled xterm mouse-events
    ...
    //      payloads upload to WORK4YOU_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to WORK4YOU_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[work4you-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__WORK4YOU_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets WORK4YOU_HOME pointed at the
    ...
    // forwarding them into Work4You. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="work4you-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
