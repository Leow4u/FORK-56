 ...
 *
 * The root ``package.json`` is installed by ``work4you update`` on every user,
 ...
 *   workspace graph entirely. ``work4you update`` and ``work4you doctor --fix``
 ...
 *   ``work4you tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``work4you update`` for
 ...
 * fails — read the lazy-install guidance in the ``work4you-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in work4you_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`work4you tools` → Browser Automation → Camofox.'
  )
})
