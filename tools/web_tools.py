...
Backend is selected during ``work4you tools`` setup (web.backend in config.yaml).
When available, Work4You can route Firecrawl calls through a Work4You-hosted tool-gateway
for Work4You Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Work4You Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_work4you_access_token as _peek_work4you_access_token,
    read_work4you_access_token as _read_work4you_access_token,
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Work4You config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Work4You' config/.env layer (``work4you config set``, ``work4you tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.work4you/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``work4you tools``). A
    ...
    if configured:
        ...
        # The managed "Work4You Subscription" selection ("work4you") is serviced by
        ...
        if configured == WORK4YOU_MANAGED_PROVIDER:
            return "firecrawl"
        return configured
    ...
    # pre-empted by a Work4You OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `work4you tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_work4you_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Work4You portal on every CLI startup (invoked at
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `work4you plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `work4you tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `work4you plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`work4you plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from work4you_cli.config import get_env_value as _gev
