"""Install and remove the Linux desktop entry (``work4you.desktop``).

``work4you desktop`` builds and launches the Electron app. On Linux, a
...
``work4you uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``work4you desktop`` fails when work4you lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "work4you.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``work4you.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``work4you desktop``.

    Prefer the real ``work4you`` executable (argv[0] or PATH). When Work4You
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from work4you_cli.relaunch import resolve_work4you_bin

    bin_path = resolve_work4you_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "work4you_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Work4You\n"
        "GenericName=Work4You Desktop\n"
        "Comment=Launch Work4You Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Work4You\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Work4You desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "work4you"
