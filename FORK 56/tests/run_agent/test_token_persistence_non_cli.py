...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    work4you_state = ModuleType("work4you_state")
    work4you_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "work4you_state", work4you_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Work4You", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Work4You"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Work4You", "detail": "full"}),
        ),
    )
