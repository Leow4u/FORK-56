...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Work4You profile changed while connecting')
  }
...
export async function refreshProjects(): Promise<void> {
  ...
  let gateway: Work4YouGateway | null = null
...
async function refreshProjectTreeOn(gateway: Work4YouGateway): Promise<void> {
...
const repoScanStates = new WeakMap<Work4YouGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<Work4YouGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getWork4YouConfig(context.profile))
...
export async function listRepoBranches(repoPath: string): Promise<Work4YouGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<Work4YouGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.work4youDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.work4youDesktop?.writeClipboard?.(path)
  }
}
