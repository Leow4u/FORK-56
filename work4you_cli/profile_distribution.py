"""Profile distributions — shareable, packaged Work4You profiles via git.

A distribution is a Work4You profile published as a git repository (or
...
* ``work4you profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``work4you skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``work4you profile``, not a parallel tree):

    work4you profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "work4you_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".work4you_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "work4you", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            work4you_requires=str(data.get("work4you_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.work4you_requires:
            out["work4you_requires"] = self.work4you_requires
        if self.author:
...
def check_work4you_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Work4You {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``work4you profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Work4You profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Work4You distribution."
        )

    # Version check up-front so we fail fast
    check_work4you_requires(manifest.work4you_requires, work4you_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.work4you).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="work4you_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `work4you profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`work4you profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="work4you_dist_update_") as tmp:
        plan = plan_install(
