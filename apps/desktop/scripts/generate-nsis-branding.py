...
The default fallback is NSIS's stock nsis3-metro.bmp (the generic blue
download illustration). This script replaces that with the same green
polyhedron mark the desktop already ships.
"""
...
def _paste_center(base: Image.Image, overlay: Image.Image, cy: int) -> None:
...
def build_sidebar() -> Image.Image:
    ...
    mark = _load_mark(96)
    _paste_center(img, mark, 118)

    title = _font(FONT_BOLD, 16)
    subtitle = _font(FONT_REGULAR, 10)
    name = "Work4You"
    name_box = draw.textbbox((0, 0), name, font=title)
    name_w = name_box[2] - name_box[0]
    draw.text(((SIDEBAR[0] - name_w) / 2, 188), name, font=title, fill=INK)

    line = "for Windows"
    line_box = draw.textbbox((0, 0), line, font=subtitle)
    line_w = line_box[2] - line_box[0]
    draw.text(((SIDEBAR[0] - line_w) / 2, 212), line, font=subtitle, fill=MUTED)

    draw.rectangle((24, SIDEBAR[1] - 18, SIDEBAR[0] - 24, SIDEBAR[1] - 16), fill=ACCENT)
    return img
