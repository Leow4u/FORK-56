...
entirely on the process environment: ``WORK4YOU_HOME`` says *where* to write and
...
# hardcoded ~/.work4you — that root is %LOCALAPPDATA%\work4you on Windows) and
...
root = work4you_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        work4you_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``WORK4YOU_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("WORK4YOU_HOME", "WORK4YOU_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no WORK4YOU_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``WORK4YOU_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{work4you_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert work4you_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["work4you", "gateway", "start"],
            ["/usr/bin/python", "-m", "work4you_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["work4you", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not work4you_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not work4you_state._process_looks_like_pytest(_Denied())
