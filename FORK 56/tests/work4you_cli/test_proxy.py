"""Tests for the `work4you proxy` subcommand and its upstream adapters."""
...
from work4you_cli.proxy.adapters import ADAPTERS, get_adapter
from work4you_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from work4you_cli.proxy.adapters.work4you_portal import Work4YouPortalAdapter
from work4you_cli.proxy.adapters.xai import XAIGrokAdapter
...
# ---------------------------------------------------------------------------
# Work4YouPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(work4you_home: Path, work4you_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given work4you state into a hermetic WORK4YOU_HOME."""
    auth_path = work4you_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"work4you": work4you_state},
    }))
    return auth_path
...
def test_work4you_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "work4you_cli.proxy.adapters.work4you_portal.resolve_work4you_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic WORK4YOU_HOME."""
    auth_path = work4you_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from work4you_cli.proxy.server import create_app  # noqa: E402
