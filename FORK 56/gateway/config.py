...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from work4you_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (work4you_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Work4You is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_work4you_home() / "sessions")
    ...
    # gateway behaves exactly as before — single WORK4YOU_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Work4You Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.work4you/config.yaml (primary user-facing config)
    3. ~/.work4you/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_work4you_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through work4you_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `work4you config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `work4you config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``work4you config set gateway.streaming.*``
