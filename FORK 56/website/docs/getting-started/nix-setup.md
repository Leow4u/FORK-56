...
description: "Install and deploy Work4You with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Work4You ships a Nix flake, a NixOS module, and a Home Manager module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`work4you setup`, `work4you gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage work4you the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `work4you`, `work4you`, and `work4you-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `work4you setup` walks you through provider selection, `work4you gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.work4you/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries work4you might need. if you want a smaller variant, check the other flake outputs. 
...
:::note
This module needs NixOS. Work4You is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
That's it. `nixos-rebuild switch` creates the `work4you` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
Setting `addToSystemPackages = true` does two things: puts the `work4you` CLI on your system PATH **and** sets `WORK4YOU_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `work4you` in your shell creates a separate `~/.work4you/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `work4you` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `work4you chat`, `work4you sessions list`, `work4you version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the work4you codebase, set `WORK4YOU_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.work4you` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `work4you` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo work4you chat` manually.
:::
...
work4you version
work4you config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Work4You defaults to OpenRouter.
:::
...
{ config, ... }: {
  services.work4you = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to WORK4YOU_HOME. Workspace files use
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$WORK4YOU_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."work4you-env".path ]` |
| Give the agent an identity | `work4youHomeFiles."SOUL.md"` | `"You are a terse ops assistant."` |
...
| Override the work4you package | `package` | `inputs.work4you.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/work4you"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$WORK4YOU_HOME/.env` at activation time (`nixos-rebuild switch`). Work4You reads this file on every startup, so changes take effect with a `systemctl restart work4you` — no container recreation needed.
...
```yaml
# secrets/work4you.yaml (encrypted with sops)
work4you-env: |
...
Work4You reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
`work4youHomeFiles` installs into **`WORK4YOU_HOME`**. Work4You reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. Work4You does not load that file as the identity:
...
`work4youHomeFiles` needs no `workingDirectory`, because the module owns the `WORK4YOU_HOME` directory. Most users want `work4youHomeFiles`.
...
:::tip
Environment variables in `env` values are resolved from `$WORK4YOU_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Work4You implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$WORK4YOU_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Work4You prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u work4you` (native):
...
# Ensure: chown work4you:work4you, chmod 0600
```
...
When work4you runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `work4you setup` | Config is declarative — edit `settings` in your Nix config |
| `work4you config edit` | Config is generated from `settings` |
| `work4you config set <key> <value>` | Config is generated from `settings` |
| `work4you gateway install` | The systemd service is managed by NixOS |
| `work4you gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **The `WORK4YOU_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `WORK4YOU_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it work4you work4you config set ...`.
...
The flake also exports `homeManagerModules.default`. Work4You is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.work4you`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:

| | NixOS module | Home Manager module |
|---|---|---|
| Runs as | a system user that you declare, with `user`, `group` and `createUser` | you |
| State directory | `stateDir` and `/.work4you` | `work4youHome`, set directly. The default is `~/.work4you`. |
| Service | `systemd.services` | `systemd.user.services` on Linux, `launchd.agents` on macOS |
| CLI on the PATH | `addToSystemPackages`, which exports `WORK4YOU_HOME` for the full system | `installPackage`, which exports it for your session only |
...
`home-manager switch` makes `~/.work4you`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. Work4You Desktop and the web dashboard connect to a *different* process, which is `work4you serve` or `work4you dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that Work4You Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `WORK4YOU_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
work4you version
work4you config     # shows the configuration that Nix wrote
```
...
When container mode is enabled, work4you runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-work4you-0.1.0  ──►  /nix/store/... (ro)
~/.work4you -> /var/lib/work4you/.work4you       (symlink bridge, per hostUsers)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/work4you gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/work4you` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart work4you` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the work4you package itself do **not** trigger recreation.
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/work4you-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current work4you package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `work4you plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [work4you-lcm](https://github.com/stephenschoettler/work4you-lcm)):
...
Plugins are symlinked into `$WORK4YOU_HOME/plugins/` at activation time. Work4You discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."work4you.plugins"]` (e.g., [rtk-work4you](https://github.com/ogallotti/rtk-work4you)):
...
The package's `site-packages` is added to PYTHONPATH in the work4you wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in work4you's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.
...
:::note
A build-time collision check prevents plugin packages from shadowing core work4you dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # WORK4YOU_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `work4you` and `work4you` binaries exist and `work4you version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `work4you --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `WORK4YOU_MANAGED=true work4you config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `WORK4YOU_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the work4you service |
| `package` | `package` | `work4you` | The work4you package to use |
| `user` | `str` | `"work4you"` | System user |
| `group` | `str` | `"work4you"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/work4you"` | State directory (`WORK4YOU_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `work4you` CLI to system PATH and set `WORK4YOU_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$WORK4YOU_HOME/.env` at activation time |
...
| `work4youHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `WORK4YOU_HOME`. `SOUL.md` and `memories/` must be here, or Work4You does not load them. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `work4you gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the work4you user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$WORK4YOU_HOME/plugins/`. Each must contain `plugin.yaml` |
...
This option runs the process that Work4You Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `work4youHome` | `str` | `"${config.home.homeDirectory}/.work4you"` | `WORK4YOU_HOME` directly. The NixOS module builds it from `stateDir`. |
| `installPackage` | `bool` | `true` | Add the `work4you` CLI to `home.packages`, and export `WORK4YOU_HOME` for your shells |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.work4you` symlink to the service stateDir and are auto-added to the `work4you` group |
...
```
/var/lib/work4you/                     # stateDir (owned by work4you:work4you, 0750)
├── .work4you/                         # WORK4YOU_HOME
│   ├── SOUL.md                      # from work4youHomeFiles: the agent identity
...
├── SOUL.md                          # from work4youHomeFiles
...
├── auth.json                        # OAuth credentials: seeded, then Work4You owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Work4You binary + all Nix deps |
| `/data` | `/var/lib/work4you` | `rw` | All state, config, workspace |
| `/home/work4you` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u work4you` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `work4you version` shows old version | Container not restarted | `systemctl restart work4you` |
| Permission denied on `/var/lib/work4you` | State dir is `0750 work4you:work4you` | Use `docker exec` or `sudo -u work4you` |
| `nix-collect-garbage` removed work4you | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "work4you"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user work4you` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart work4you` |
