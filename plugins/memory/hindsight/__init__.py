...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: work4you)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: work4you)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $WORK4YOU_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "work4you").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Work4You can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``work4you memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``work4you memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'work4you memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $WORK4YOU_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_work4you_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "work4you": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "work4you"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Work4You config."""
    profile = config.get("profile", "work4you")
    return str(profile or "work4you")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Work4You .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Work4You profile name (from agent_identity)
      {workspace} — Work4You workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``work4you-{user}`` with no user becomes ``work4you``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Work4You and the User"
    ...
    def save_config(self, values, work4you_home):
        """Write config to $WORK4YOU_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(work4you_home) / "hindsight"
    ...
    def post_setup(self, work4you_home: str, config: dict) -> None:
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(work4you_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "work4you")
        ...
        self.save_config(provider_config, work4you_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(work4you_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(work4you_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Work4You starter template (best-effort)."""
        from work4you_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "work4you")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "work4you"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: work4you-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Work4You and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "work4you"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "work4you"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/work4you (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "work4you", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "work4you")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Work4You and the User")
        ...
        # Work4You emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                ...
                # Work4You get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See Leow4u/FORK-56#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Work4You created/used that client on the shared async loop,
