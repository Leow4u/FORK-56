...
Regression guard for the bug where `work4you model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`work4you_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in WORK4YOU_OVERLAYS
        overlay = WORK4YOU_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `work4you_cli/main.py` (cmd_model): read
