...
2. An event-loop heartbeat file at ``<WORK4YOU_HOME>/state/gateway.heartbeat`` so
...
def _process_work4you_home() -> Path:
    """WORK4YOU_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<WORK4YOU_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_work4you_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from work4you_logging import drain_log_queue
