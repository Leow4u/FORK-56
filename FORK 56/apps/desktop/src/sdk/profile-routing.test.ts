...
vi.mock('@/work4you', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn() }))
...
const { deleteProfile } = await import('@/work4you')
...
describe('connection-aware plugin host APIs', () => {
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { work4youDesktop: unknown }).work4youDesktop = { getProfileRoutes }
