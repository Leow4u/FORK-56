...
The CDP supervisor closes two long-standing gaps in Work4You' browser tooling:
...
(`work4you-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Work4You `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `work4you-acp`, `work4you-api-server`, and core toolsets (gated on CDP reachability)
- `work4you_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
