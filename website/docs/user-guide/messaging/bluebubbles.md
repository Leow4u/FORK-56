...
Connect Work4You to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Work4You and the BlueBubbles server
...
Or set environment variables directly in `~/.work4you/.env`:
...
By default, Work4You responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Work4You uses conservative defaults for `Work4You` and `@Work4You agent` variants.
...
When someone messages your iMessage, Work4You automatically sends them a pairing code. Approve it with:
```bash
work4you pairing approve bluebubbles <CODE>
```
Use `work4you pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.work4you/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.work4you/.env`):
```bash
...
Work4You will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Work4You
Work4You → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Work4You sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Work4You wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.work4you/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Work4You resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `work4you logs gateway` for webhook errors (or `work4you logs -f` to follow in real-time)
