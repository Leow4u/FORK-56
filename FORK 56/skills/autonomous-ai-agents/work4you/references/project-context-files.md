...
|---|---|---|
| `.work4you.md` / `WORK4YOU.md` | Walks parents up to the git root, stops at git root | You want hierarchical project rules (root + per-package overrides) |
...
`SOUL.md` (in `$WORK4YOU_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.

### Pick the right one

- **Use `.work4you.md`** when you want Work4You-specific behavior that lives above the cwd (root + subtree), or when you want rules to inherit from a parent directory. The parent walk stops at the git root, so a home-level `.work4you.md` won't leak into every project (a git repo's root is the boundary).
...
- **Don't put project rules in `~/.work4you/AGENTS.md`** (or any other home-level location). When Work4You runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$WORK4YOU_HOME`, identity-only) or install a skill via `work4you skills install`.
