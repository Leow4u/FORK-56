"""
work4you fallback — manage the fallback provider chain.
...
Subcommands:
  work4you fallback [list]   Show the current fallback chain (default when no subcommand)
  work4you fallback add      Pick provider + model via the same picker as `work4you model`,
                           then append the selection to the chain
  work4you fallback remove   Pick an entry to delete from the chain
  work4you fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.work4you/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from work4you_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `work4you model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  work4you fallback add")
        print()
        return
    ...
    print("  Docs: https://work4you.ai/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `work4you model`, then append the selection to the chain."""
    from work4you_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `work4you model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `work4you fallback list` to view, or `work4you fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``work4you fallback [subcommand]``."""
