...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.work4you/."""
...
class TestStoreInit:
    ...
    def test_legacy_migration_archives_prev2_repos(
        ...
        (fake_repo / "WORK4YOU_WORKDIR").write_text(str(work_dir) + "\n")
...
class TestSafeRestore:
    """Safe restore: preserve user hand-edits, revert only Work4You-authored changes.
    ...
    def test_safe_restore_skips_file_user_edited_after_agent(self, mgr, work_dir):
        ...
        # Work4You writes the file, then the user modifies it afterwards.
        (work_dir / "main.py").write_text("agent version\n")
        mgr.record_agent_write(str(work_dir / "main.py"))
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # Content no longer matches what Work4You last wrote → preserved.
    ...
    def test_safe_restore_removes_agent_created_file_keeps_user_edit(self, mgr, work_dir):
        ...
        # Work4You creates a brand-new file after the checkpoint...
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # User edit preserved; Work4You-created file removed (not in checkpoint).
...
def _seed_legacy_repo(base: Path, name: str, workdir: Path, mtime: float = None) -> Path:
    ...
    (shadow / "WORK4YOU_WORKDIR").write_text(str(workdir) + "\n")
    (shadow / "info").mkdir()
...
class TestPruneCheckpointsOrphanAllowlist:
    ...
    def test_end_to_end_timing_change_during_confirmation_prompt(self, tmp_path, monkeypatch):
        ...
        `work4you checkpoints prune`: the preview shows one pre-v2 orphan; a
        ...
        be deleted.
        """
        import work4you_cli.checkpoints as checkpoints_cli
