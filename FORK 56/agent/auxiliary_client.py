...
Resolution order for text tasks (auto mode):
  ...
  3. Work4You Portal (~/.work4you/auth.json active provider)
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``WORK4YOU_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `work4you update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Work4You owns auxiliary retry + provider/model fallback policy (the
    ...
    # Work4You sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Work4You control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="work4you-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Work4You triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once Work4You dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Work4You
...
# Work4You catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from work4you_cli.auth import resolve_api_key_provider_credentials
        from work4you_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Work4You returns its compaction pick), so it
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://work4you.ai",
    "X-Title": "Work4You",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``WORK4YOU_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``WORK4YOU_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("WORK4YOU_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("WORK4YOU_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "Work4YouAgent",
}
...
from work4you_cli import __version__ as _WORK4YOU_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://work4you.ai",
    "X-Title": "Work4You",
    "User-Agent": f"Work4YouAgent/{_WORK4YOU_VERSION}",
}

# Work4You Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Work4You Portal.
#
...
from agent.portal_tags import work4you_portal_tags as _work4you_portal_tags


def _work4you_extra_body() -> dict:
    """Return a fresh Work4You Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``work4you_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _work4you_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``WORK4YOU_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_work4you_extra_body()`` or import ``work4you_portal_tags`` directly.
WORK4YOU_EXTRA_BODY = _work4you_extra_body()

# Set at resolve time — True if the auxiliary client points to Work4You Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3.6-flash"
_WORK4YOU_MODEL = "google/gemini-3.6-flash"
_WORK4YOU_DEFAULT_BASE_URL = "https://inference-api.work4you.ai/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_work4you_home() / "auth.json"
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for work4you).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "work4you":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the WORK4YOU resolution.
        from work4you_cli.auth import _work4you_inference_env_override

        env_url = _work4you_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. work4you prefers inference_base_url).
...
def _work4you_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("WORK4YOU_WORK4YOU_MIN_KEY_TTL_SECONDS", "1800")))
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    # is Work4You-internal ladder vocabulary with no wire
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Work4You Portal — a blanket fallback would flip
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from agent.anthropic_adapter import _is_work4you_portal_endpoint

                    if _is_work4you_portal_endpoint(candidate):
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Work4You plumbing (_reasoning_config
...
def _read_work4you_auth() -> Optional[dict]:
    """Read and validate ~/.work4you/auth.json for an active Work4You provider.

    Returns the provider state dict if Work4You is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("work4you")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _WORK4YOU_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "work4you":
            return None
        provider = data.get("providers", {}).get("work4you", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Work4You auth: %s", exc)
        return None


def _work4you_api_key(provider: dict) -> str:
    """Extract a usable Work4You inference JWT from stored auth state."""
    from work4you_cli.auth import _work4you_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _work4you_invoke_jwt_is_usable(
            token,
...
def _work4you_base_url() -> str:
    """Resolve the Work4You inference base URL from env or default."""
    return os.getenv("WORK4YOU_INFERENCE_BASE_URL", _WORK4YOU_DEFAULT_BASE_URL)


def _resolve_work4you_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Work4You auxiliary credentials from the selected pool entry."""
    try:
        from work4you_cli.auth import _agent_key_is_usable

        pool = load_pool("work4you")
    except Exception as exc:
        logger.debug("Auxiliary Work4You pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Work4You pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _work4you_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary Work4You pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _work4you_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _WORK4YOU_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_work4you_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Work4You runtime credentials when available.
    ...
    """
    pooled = _resolve_work4you_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from work4you_cli.auth import resolve_work4you_runtime_credentials

        creds = resolve_work4you_runtime_credentials(
            timeout_seconds=env_float("WORK4YOU_WORK4YOU_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Work4You runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``work4you auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Work4You auth store.
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from work4you_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Work4You —
    # if another session already recorded a 429, skip Work4You entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.work4you_rate_guard import work4you_rate_limit_remaining
        _remaining = work4you_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Work4You Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("work4you", ttl=_remaining)
            return None, None
    except Exception:
        pass

    work4you = _read_work4you_auth()
    runtime = _resolve_work4you_runtime_api(force_refresh=False)
    if runtime is None and not work4you:
        logger.warning(
            "Auxiliary Work4You client unavailable: no Work4You authentication found "
            "(run: work4you auth)."
        )
        _mark_provider_unhealthy("work4you", ttl=60)
        return None, None
    if runtime is None and work4you:
        logger.debug(
            "Auxiliary Work4You: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_nous
    auxiliary_is_nous = True
    logger.debug("Auxiliary client: Work4You Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/work4you/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_work4you_recommended_aux_model
    # auto-detects the caller's tier via check_work4you_free_tier().  Fall back to
    # _WORK4YOU_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _WORK4YOU_MODEL
    if not _aux_probe_active():
        # Availability probes skip the recommended-model lookup: the exact
        # model is irrelevant to "is Work4You resolvable?", and the Portal
        # recommended-models fetch below can hit the network.
        try:
            from work4you_cli.models import get_work4you_recommended_aux_model
            recommended = get_work4you_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
                logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _work4you_api_key(work4you or {})
        if not api_key:
            logger.warning(
                "Auxiliary Work4You client unavailable: no usable inference JWT found "
                "(run: work4you auth add work4you)."
            )
            _mark_provider_unhealthy("work4you", ttl=60)
            return None, None
        base_url = str((work4you or {}).get("inference_base_url") or _work4you_base_url()).rstrip("/")
    return (
...
def _refresh_work4you_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the Work4You Portal's recommended model after a stale-model 404.
    ...
    the Work4You → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_WORK4YOU_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_work4you_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "Work4You recommended-model refresh failed (%s); using default %s",
            exc, _WORK4YOU_MODEL,
        )
    ...
    if _WORK4YOU_MODEL.strip().lower() != stale:
        return _WORK4YOU_MODEL
    return None
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `work4you setup` or `work4you model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import work4you_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=work4you_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second work4you
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``work4you config aux reset``)."""
...
def _work4you_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Work4You account API says paid access is allowed."""
    try:
        ...
        logger.debug("Auxiliary Work4You paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Work4You → OpenRouter catalog. The Work4You
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.work4you.ai"):
        return "work4you"
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Work4You routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.work4you.ai"):
        return "work4you"
    return normalized
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_work4you_fallback_destination", None)
...
def _replan_synchrowork4you_cache_sections(
    messages: list,
    tools: Optional[list],
    *,
...
def _call_fallback_candidate_sync(
    ...
    *,
    ...
    fallback_messages, fallback_tools = _replan_synchrowork4you_cache_sections(
        messages,
        tools,
    ...
    try:
        ...
        if fb_provider not in {"auto", "", None} and _refresh_provider_credentials(fb_provider):
            ...
            if retry_client is not None:
                ...
                retry_messages, retry_tools = _replan_synchrowork4you_cache_sections(
                    messages,
                    tools,
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "work4you": "work4you",
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``work4you_cli.fallback_config.resolve_entry_api_key``).
    """
    from work4you_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._work4you_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Work4You' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Work4You get their chosen chat model; users
      ...
      2. OpenRouter → Work4You → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `work4you model` but the
    #    old OPENAI_BASE_URL lingers in ~/.work4you/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.work4you/.env",
    ...
    # on aggregators (OpenRouter, Work4You) who previously got routed to a
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_work4you_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_work4you_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = work4you_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Work4You owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "work4you", "openai-codex" (or "codex"),
    ...
    #
    # Work4You + vision is the one carve-out: the branch below resolves its model
    ...
    _work4you_portal_vision = provider == "work4you" and is_vision
    if not model and provider != "auto" and not _work4you_portal_vision:
    ...
    if provider == "work4you":
        ...
        if client is None:
            logger.warning("resolve_provider_client: work4you requested "
                           "but Work4You Portal not configured (run: work4you auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        portal_mode = work4you_api_mode(final_model)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: work4you model)")
                return None, None
    ...
    # OpenRouter / Work4You bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: work4you model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``work4you``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: work4you doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(work4you_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "work4you":
            return resolve_provider_client("work4you", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "work4you":
        ...
        return resolve_provider_client("work4you", model, is_vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Work4You → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Work4You — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Work4You is the
        ...
        #   3. Work4You Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "work4you":
                # Work4You resolves its vision model from the Portal's tier-aware
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Work4You Portal product tags when the auxiliary client is backed
    by Work4You Portal. Returns empty dict otherwise.
    """
    return _work4you_extra_body() if auxiliary_is_nous else {}
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    if (not or_key
            and _read_work4you_auth() is None
            and (
...
def _refresh_work4you_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Work4You runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_work4you_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                ...
                pass
        _work4you_on_messages = False
        if _provider_norm in {"work4you", "work4you-portal", "work4you"}:
            from work4you_cli.providers import work4you_api_mode

            _work4you_on_messages = work4you_api_mode(model) == "anthropic_messages"
        if (
            _is_anthropic_compat_endpoint(provider, _effective_base)
            or _work4you_on_messages
            or _is_nvidia_nim
            or _is_moa
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Work4You use
    ...
    if _provider_for_portal in {"work4you", "work4you-portal", "work4you"}:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _work4you_portal_tags()
    ...
    # Anthropic Messages adapters translate Work4You reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _work4you_on_messages = False
        if provider_norm in {"work4you", "work4you-portal", "work4you"}:
            from work4you_cli.providers import work4you_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Work4You reasoning config for direct model calls
    ...
    try:
        ...
        # ── Stale-model self-heal (Work4You Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Work4You → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Work4You-routed calls.
        _heal_is_nous = (
            resolved_provider == "work4you"
            or base_url_host_matches(_base_info, "inference-api.work4you.ai")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_work4you_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Work4You catalog; "
        ...
        # ── Work4You auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "work4you"
            or base_url_host_matches(_base_info, "inference-api.work4you.ai")
        )
        if (
            ...
            and client_is_nous
            and _work4you_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_work4you_auxiliary_client(
                cache_provider=resolved_provider or "work4you",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Work4You runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Work4You runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Work4You
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the Work4You → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_nous = (
            resolved_provider == "work4you"
            or base_url_host_matches(_client_base, "inference-api.work4you.ai")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_work4you_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Work4You catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Work4You runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Work4You runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
