...
| Variable | Meaning |
|----------|---------|
| `WORK4YOU_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `WORK4YOU_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `WORK4YOU_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Work4You itself — they're private to these helpers. The variables Work4You *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `work4you --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:
...
`--dev` compiles from source, so it links `ui-tui/node_modules` from `WORK4YOU_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_work4you_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `WORK4YOU_TUI_DIR` are mutually exclusive
`WORK4YOU_TUI_DIR` points Work4You at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `work4you --tui --dev` exits with an error. Run `unset WORK4YOU_TUI_DIR` before `htui`.
:::
...
|----------|----------------|
| `WORK4YOU_DESKTOP_WORK4YOU_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `work4you`. |
| `WORK4YOU_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `WORK4YOU_DESKTOP_IGNORE_EXISTING` | Ignores any `work4you` on `PATH` so it can't shadow the worktree. |
| `WORK4YOU_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
- [TUI](../user-guide/tui.md) — `work4you --tui --dev` and the `WORK4YOU_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/Leow4u/FORK-56/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `WORK4YOU_*` variable Work4You reads
