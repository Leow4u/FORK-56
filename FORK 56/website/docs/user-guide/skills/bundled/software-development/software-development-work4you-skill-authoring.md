---
title: "Work4You Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Work4You Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Work4You Skill Authoring
...
| Path | `skills/software-development/work4you-skill-authoring` |
| Version | `2.0.0` |
| Author | Work4You |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `work4you`, `conventions`, `skill-md` |
...
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the work4you repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with work4you
...
- Don't use for: personal skills in `~/.work4you/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `work4you skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Work4You
license: MIT
...
- Credit the **human first**, then "Work4You" as secondary collaborator: `Ben Barclay (benbarclay), Work4You`.
- Never `author: Work4You` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Work4You`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.work4you/skills/`.
...
|---|---|
| Work4You tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.work4you/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Work4You` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.work4you.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Work4You tools
