"""
Backup and import commands for work4you CLI.

`work4you backup` creates a zip archive of the entire ~/.work4you/ directory
(excluding the work4you repo and transient files).

`work4you import` restores from a backup zip, overlaying onto the current
WORK4YOU_HOME root.
"""
...
from work4you_constants import get_default_work4you_root, get_work4you_home, display_work4you_home
from utils import (
    _preserve_file_mode,
...
from work4you_cli.sizefmt import format_bytes as _format_size
...
# ``work4you`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/work4you/``
...
# WORK4YOU_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "work4you",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under WORK4YOU_HOME) —
...
# File names that ``work4you import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Work4You portal
...
# Reserved archive subtree for provider state that lives OUTSIDE WORK4YOU_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Work4You backup slot."""
...
@contextmanager
def _backup_operation_lock(work4you_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = work4you_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Work4You backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside WORK4YOU_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to work4you root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``work4you`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/work4you/`` — must be preserved.
        if part == "work4you" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside WORK4YOU_HOME.
...
# to the user as a hung `work4you update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # work4you_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Work4You home directory."""
    work4you_root = get_default_work4you_root()

    if not work4you_root.is_dir():
        print(f"Error: Work4You home directory not found at {work4you_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(work4you_root):
            _run_backup_locked(args, work4you_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, work4you_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"work4you-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"work4you-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_work4you_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(work4you_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(work4you_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``work4you`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "work4you" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(work4you_root)
    ...
    # outside WORK4YOU_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_work4you_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Work4You backup.
    ...
    # Look for telltale files that a work4you home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.work4you/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a work4you dir name
        if prefix in {".work4you", "work4you"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``work4you import``, which is the
    ...
    so without the chown a ``sudo work4you import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``work4you import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Work4You' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_work4you_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (work4you_root / "config.yaml").exists()
        has_env = (work4you_root / ".env").exists()
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under WORK4YOU_HOME.
            ...
            # Work4You portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = work4you_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(work4you_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_work4you_home()}."
            )
        ...
        profiles_dir = work4you_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # work4you_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: work4you profile list  (after installing work4you)")

        # Guidance
        print()
        if not (work4you_root / "work4you").is_dir():
            print("Note: The work4you codebase was not included in the backup.")
        ...
        try:
            from work4you_cli.gateway import ensure_gateway_service, _is_service_running
...
# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and work4you backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to WORK4YOU_HOME).
...
# — `work4you update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $WORK4YOU_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside WORK4YOU_HOME and the entry is silently
...
def _quick_snapshot_root(work4you_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: work4you snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(work4you_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside WORK4YOU_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``work4you update``.
    ...
    Args:
        ...
        work4you_home: Override for the Work4You home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(work4you_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for work4you backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_work4you_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, work4you_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(work4you_root):
            return _write_full_zip_backup_locked(out_path, work4you_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, work4you_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``work4you_root`` to ``out_path``.
...
def _pre_update_backup_dir(work4you_home: Optional[Path] = None) -> Path:
    home = work4you_home or get_work4you_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of WORK4YOU_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<WORK4YOU_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``work4you update``) should continue even if the backup fails.
    """
    work4you_root = work4you_home or get_default_work4you_root()
    if not work4you_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(work4you_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, work4you_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `work4you claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of WORK4YOU_HOME under ``backups/`` before a
    ``work4you claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``work4you import <archive>``.  Writes to
    ``<WORK4YOU_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `work4you import` and the
