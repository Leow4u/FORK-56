"""Regression guard: a genuine Work4You 429 must re-enter the retry loop so the
top-of-loop Work4You rate-limit guard can activate the fallback chain.
...
``record_work4you_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineWork4You429ReentersLoop:
