...
$WORK4YOU_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $WORK4YOU_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"work4you-restart-{safe}"
    volume = f"work4you-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "work4you", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "work4you", "profile", "create", "ghost").check_returncode()
