...
def work4you_home(tmp_path, monkeypatch):
    """Point WORK4YOU_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(work4you_home):
...
def test_verify_checksums_signature_skips_without_gpg(work4you_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(work4you_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(work4you_home, monkeypatch):
...
def test_ca_key_created_with_0o600(work4you_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(work4you_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(work4you_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(work4you_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(work4you_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(work4you_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("work4you-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(work4you_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(work4you_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(work4you_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=work4you_home / "ca.crt",
        ca_key=work4you_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (work4you_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = work4you_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("work4you-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(work4you_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_WORK4YOU_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_WORK4YOU_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(work4you_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(work4you_home, monkeypatch):
