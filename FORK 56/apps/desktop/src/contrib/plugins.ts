 ...
 *    a `Work4YouPlugin` registers automatically (vite glob — drop a folder in).
 *    `work4you-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `work4you-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<work4you home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<work4you home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: Work4YouPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default Work4YouPlugin export — skipped`)
