"""Regression tests for #76414: `work4you honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("work4you.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("work4you_work"). The lookup
...
a real honcho.json (temp WORK4YOU_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "work4you": {"peerName": "alice", "aiPeer": "work4you"},
            "work4you_work": {"peerName": "alice", "aiPeer": "work4you"},
            "work4you_my_profile": {"peerName": "bob", "aiPeer": "work4you"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.profiles.list_profiles",
        ...
        assert host == "work4you_work"  # not "work4you.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'work4you_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("work4you.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["work4you_work"]
        cfg["hosts"]["work4you.work"] = {"peerName": "carol", "aiPeer": "work4you"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "work4you_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'work4you.work' (or '(not set)')."""
        monkeypatch.setattr(
            "work4you_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "work4you.work" not in out
        ...
        assert "alice" in work_line and "work4you" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "work4you.new" not in out  # well-formed key, no dot-form leak
