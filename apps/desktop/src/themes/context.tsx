...
const SKIN_KEY = 'work4you-desktop-theme-v2'
const MODE_KEY = 'work4you-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'work4you-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'work4you-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'work4you-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['work4you-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? work4youTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...work4youTheme.typography, ...theme.typography }
  ...
  root.dataset.work4youTheme = skinName
  root.dataset.work4youMode = rendered
  ...
  const palette: Record<string, string> = {
    ...
    '--ui-composer-selection-background': composerSelectionBackground(rendered)
  }
  ...
  window.work4youDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('work4you-boot-background', chromeBg)
    window.localStorage.setItem('work4you-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.work4youThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.work4youDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Work4You authoring/activating a skin from a
