"""``work4you sync`` subcommand parser — Skill Sync.

Cloned from ``work4you_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    work4you sync status                 show gate/opt-in/head state
    work4you sync pull                   pull and materialize opted-in skills
    work4you sync push                   push opted-in skills
    work4you sync now                    reconcile: pull then push
    work4you sync enable <skill>         opt a skill into sync
    work4you sync disable <skill>        opt a skill out of sync
    work4you sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    work4you sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Work4You token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  work4you sync status            what is synced, and from where\n"
            "  work4you sync enable my-skill   include a skill in your sync\n"
            "  work4you sync now               pull, then push\n"
            "  work4you sync propose my-skill  share a skill with your team\n"
        ),
