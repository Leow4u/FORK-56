...
# Org mirrors live under ~/.work4you/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    WORK4YOU_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``WORK4YOU_KANBAN_TASK`` / ``WORK4YOU_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("WORK4YOU_KANBAN_TASK") or os.getenv("WORK4YOU_KANBAN_BOARD"):
        ...
        # The Work4You Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``WORK4YOU_PLATFORM`` or
            ``WORK4YOU_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("WORK4YOU_PLATFORM")
        or get_session_env("WORK4YOU_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``work4you config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``work4you`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.work4you/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``work4you`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against WORK4YOU_HOME, not cwd
        if not p.is_absolute():
            p = (work4you_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.work4you/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.work4you/skills`` at the git root) are
...
#   <root>/.work4you/skills/   — Work4You-native location
...
# a one-line "run `work4you skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when WORK4YOU_HOME itself lives inside a git checkout:
    ``<root>/.work4you/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``work4you skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`work4you skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under WORK4YOU_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_work4you_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.work4you/skills/`` (including via symlinks) or configured
    ...
    # ~/.work4you/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Work4You can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    work4you = metadata.get("work4you") or {}
    if not isinstance(work4you, dict):
        work4you = {}
    return {
        "fallback_for_toolsets": work4you.get("fallback_for_toolsets", []),
        "requires_toolsets": work4you.get("requires_toolsets", []),
        "fallback_for_tools": work4you.get("fallback_for_tools", []),
        "requires_tools": work4you.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    work4you = metadata.get("work4you")
    if not isinstance(work4you, dict):
        return []
    raw = work4you.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Work4You metadata, VCS, virtualenv/dependency, cache, and skill
