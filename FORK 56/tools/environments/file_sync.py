...
_FORCE_SYNC_ENV = "WORK4YOU_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.work4you") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.work4you to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.work4you", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``WORK4YOU_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, work4you_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.work4you/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(work4you_home=work4you_home)

    def _sync_back_transaction(self, work4you_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .work4you/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (work4you_home or get_work4you_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="work4you-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.work4you/skills/a.md`` →
        ``~/.work4you/skills/a.md``, a new remote file at
        ``/root/.work4you/skills/b.md`` maps to ``~/.work4you/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
