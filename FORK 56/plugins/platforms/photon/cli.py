...
there is no standalone ``login`` verb (matching how every other Work4You
...
# Writable sidecar runtime dir (mirrors to WORK4YOU_HOME on immutable
...
# wiring imports this module for `work4you --help`).
...
# when called later (gateway start, work4you status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Work4You')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Work4You" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.work4you/.env,
    ...
    try:
        from work4you_cli.config import write_platform_config_field
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.work4you/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.work4you/.env)")
