"""OpenAI-compatible shim that forwards Work4You requests to `copilot --acp`.

This adapter lets Work4You treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape Work4You expects from an OpenAI client.
"""
...
def _resolve_args() -> list[str]:
    raw = os.getenv("WORK4YOU_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    WORK4YOU_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set WORK4YOU_HOME explicitly if they
...
def _format_messages_as_prompt(
    ...
    sections: list[str] = [
        "You are being used as the active ACP agent backend for Work4You.",
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float) -> tuple[str, str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"WORK4YOU_COPILOT_ACP_COMMAND / WORK4YOU_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "Work4You ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "  export WORK4YOU_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
