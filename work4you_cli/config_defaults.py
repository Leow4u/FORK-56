"""Default configuration data for Work4You.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from work4you_cli/config.py. Must not import from work4you_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["work4you-cli"],
    # SQLite journal mode used by every Work4You database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Work4You server processes.
    ...
    "session": {
        # Per-terminal `work4you -c`: each CLI session drops a breadcrumb file
        # under $WORK4YOU_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # budget. CLI one-shot equivalent: `work4you chat --run-budget N`.
        ...
        # an interactive `work4you gateway restart` must never block for hours
        ...
        # the Work4You-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Work4You
        ...
        # identity slot (SOUL.md). Empty by default. The WORK4YOU_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Work4You adds a coding operating brief
        ...
        # `work4you status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``WORK4YOU_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             WORK4YOU_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force WORK4YOU_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Work4You auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Work4You sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Work4You
        ...
        # bundled Work4You image, which drops to the `work4you` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Work4You sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Work4You should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``work4you chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.work4you/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: work4you sweeps the checkpoint base at startup
        ...
        # ``work4you checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .work4you.md, or .cursorrules before Work4You applies
    ...
    # Single-query (``work4you -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Work4You truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Work4You' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); work4you =
                                      # Work4You' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Work4You' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Work4You does.
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | work4you | codex | custom
        ...
        # Invoked by ``work4you kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``work4you kanban decompose`` and the kanban
        ...
        # ``work4you profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `work4you model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `work4you` (and `work4you chat`) launches by default:
        ...
        # REPL and `--tui` (or WORK4YOU_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `work4you --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `work4you -c` muscle memory.  Default off so existing
        # users aren't surprised.  WORK4YOU_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `work4you --tui` drops a one-time hint
        ...
        # Work4You credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see work4you_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to WORK4YOU_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/work4you`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `work4you pets`. Disabled until
        ...
        "pet": {
            ...
            # get_work4you_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``WORK4YOU_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.work4you.ai).
        "oauth": {
        ...
        # (``WORK4YOU_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # work4you-account-service at deploy time via the
        # ``WORK4YOU_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``WORK4YOU_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/work4you``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Work4You appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.work4you/cache/piper-voices/
    ...
    # "Hey Work4You" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey work4you",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_work4you" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.work4you/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Work4You, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.work4you/cache/delegation/
    ...
    # response. If not, Work4You feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Work4You auto-pauses the goal and
    ...
    "moa": {
        ...
        # <work4you_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.work4you/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.work4you/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``work4you skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``work4you skills trust`` / ``work4you skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${WORK4YOU_SKILL_DIR} and ${WORK4YOU_SESSION_ID} in SKILL.md
        ...
        # (`work4you skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.work4you/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.work4you/.curator_backups/blobs/. Enables `work4you curator ledger`
        # and single-mutation `work4you curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `work4you curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `work4you curator run --consolidate` overrides
        ...
        # normally restored on every `work4you update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `work4you curator purge` deletes
        ...
        # skipped), snapshot ~/.work4you/skills/ into
        # ~/.work4you/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `work4you curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for work4you-specific overrides; everything else
    ...
    # dangerous command. -q runs export WORK4YOU_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; WORK4YOU_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Work4You' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.work4you/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or WORK4YOU_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .work4you config) always require human
        ...
        # longer trigger the startup banner. Add via `work4you doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``work4you_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Work4You to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $WORK4YOU_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Work4You Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.work4you.ai",
        ...
        # Also overridable via WORK4YOU_CRON_MAX_PARALLEL env var.
        ...
        # WORK4YOU_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # WORK4YOU_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via WORK4YOU_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `work4you -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `work4you` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `work4you kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     work4you's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Work4You tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.work4you/logs/.
    ...
    # curated model lists for OpenRouter and Work4You Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a work4you release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://work4you.ai/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `work4you model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Work4You provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal WORK4YOU_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.work4you/sessions/sessions.json entirely.
        ...
        # the WORK4YOU_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.work4you.gateway` that defenses 1-2 don't
        ...
        # `work4you update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``WORK4YOU_GATEWAY_MAX_STARTS`` / ``WORK4YOU_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.work4you/.env,
        ...
        # behavior: files must live under the Work4You cache, under
        ...
        # user. Bridged to WORK4YOU_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Work4You
        # cache (~/.work4you/cache/{documents,images,audio,video,screenshots})
        ...
        # to WORK4YOU_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # WORK4YOU_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to WORK4YOU_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.work4you/state.db.
    ...
    "sessions": {
        ...
        # the default of ``work4you sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.work4you/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `work4you sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `work4you update` prints a one-line notice with
        ...
        # ~/.work4you/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to WORK4YOU_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # WORK4YOU_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `work4you sessions export`. Checked
    ...
    # ``work4you doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `work4you doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``work4you update`` behaviour.
    "updates": {
        ...
        #     DBs) into <WORK4YOU_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``work4you backup``-style zip
        #     of WORK4YOU_HOME into <WORK4YOU_HOME>/backups/, restorable with
        #     ``work4you import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `work4you update` does with uncommitted local changes to the
        ...
        # When `work4you update` finds the source checkout parked on a feature
    ...
    "lsp": {
        ...
        #              ``<WORK4YOU_HOME>/lsp/bin/`` on first use.
    ...
    # `work4you tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.work4you/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.work4you/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.work4you/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Work4You may use this encrypted
            ...
            # ~/.work4you/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.work4you/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Work4You disables it for our users unless they
        # explicitly opt in here. When false (default), Work4You sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Work4You also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Work4You computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Work4You passes the
    ...
    #
    # Configure with `work4you egress setup`.  Disabled by default — the rest of
    # Work4You works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.work4you/bin/ on
        ...
        # Together, DeepSeek, Work4You).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Work4You Desktop (Electron app) launch options. These only affect
    # `work4you desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the WORK4YOU_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the WORK4YOU_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "WORK4YOU_BASE_URL": {
        "description": "Work4You Portal base URL override",
        "prompt": "Work4You Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Work4You Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Work4You Subscribers only, used to derive vendor hosts, e.g. work4you.ai -> firecrawl-gateway.work4you.ai",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Work4You Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Work4You Subscriber access token for tool-gateway requests (optional; otherwise read from the Work4You auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Work4You' wake word engine (optional; openWakeWord is the free default)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @work4you:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #work4you)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: work4you-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    # WORK4YOU_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to WORK4YOU_TOOL_PROGRESS_MODE for backward
    ...
    # WORK4YOU_TOOL_PROGRESS is fully unsupported since the v12 config support
