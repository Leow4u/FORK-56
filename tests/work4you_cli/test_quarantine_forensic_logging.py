"""Redaction-safe forensic logging at the Work4You OAuth quarantine path.

A NAS-hosted Fly agent's Work4You bootstrap session can take a terminal
...
``_quarantine_work4you_oauth_state`` now emits a WARNING+ forensic record, and — the
...
from work4you_cli.auth import AuthError, _quarantine_work4you_oauth_state


# A distinctive, obviously-fake refresh token so the redaction assertion is
# unambiguous if it ever leaks.
_FAKE_RT = "work4you_rt_LEAK_CANARY_do_not_log_raw_0123456789abcdef"
...
def _make_state(**overrides):
    state = {
        ...
        "access_token": "work4you_at_SECRET_access_token_material",
        "refresh_token": _FAKE_RT,
        "agent_key": "work4you_agent_key_SECRET_material",
...
def test_quarantine_emits_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="work4you_cli.auth"):
        _quarantine_work4you_oauth_state(state, _error(), reason="unit_test_quarantine")
...
def test_raw_refresh_token_never_logged(caplog):
    ...
    state = _make_state()
    with caplog.at_level(logging.DEBUG, logger="work4you_cli.auth"):
        _quarantine_work4you_oauth_state(state, _error(), reason="unit_test_quarantine")
    ...
    assert "work4you_at_SECRET_access_token_material" not in text
    assert "work4you_agent_key_SECRET_material" not in text
