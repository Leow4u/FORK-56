...
class WhatsAppBehaviorMixin:
    ...
    # Keep in sync with scripts/whatsapp-bridge/bridge.js DEFAULT_REPLY_PREFIX.
    DEFAULT_REPLY_PREFIX: str = "*Work4You*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to WORK4YOU_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/work4you), this function
    mirrors the bridge source to a writable WORK4YOU_HOME location and returns that
    ...
    # Try WORK4YOU_HOME location first
    work4you_home = get_work4you_home()
    work4you_home_bridge = work4you_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to WORK4YOU_HOME if needed
    if work4you_home_bridge.exists():
        return work4you_home_bridge

    # Mirror the bridge source to WORK4YOU_HOME
    try:
        work4you_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            work4you_home_bridge,
            dirs_exist_ok=False,
        )
        return work4you_home_bridge
    except Exception:
