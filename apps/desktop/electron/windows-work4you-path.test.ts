// Unit tests for the pure Windows `work4you` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvWork4YouCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `work4you` shim shadows
//      the real work4you.cmd/work4you.exe.
...
//   3. resolveVenvWork4YouCommand() — must probe the venv python via
//      canImportWork4YouCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvWork4You: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvWork4You: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvWork4You: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvWork4You: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvWork4You: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvWork4You: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvWork4YouCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportWork4YouCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    work4youHome: '/fake/work4you-home',
...
test('resolveVenvWork4YouCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvWork4YouCommand('/root/venv/Scripts/work4you.exe', [], deps), null)
})

test('resolveVenvWork4YouCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvWork4YouCommand('/root/venv/Scripts/work4you.cmd', [], deps), null)
})

test('resolveVenvWork4YouCommand: returns null when the basename is not work4you/work4you.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvWork4YouCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvWork4YouCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvWork4YouCommand('/root/venv/bin/work4you.exe', [], deps), null)
})

test('resolveVenvWork4YouCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvWork4YouCommand('/root/venv/Scripts/work4you.exe', [], deps), null)
})

test('resolveVenvWork4YouCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportWork4YouCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvWork4YouCommand('/root/venv/Scripts/work4you.exe', ['serve'], deps)
...
test('resolveVenvWork4YouCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvWork4YouCommand('/root/venv/Scripts/work4you.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'work4you_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvWork4YouCommand: is case-insensitive on work4you.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvWork4YouCommand('/root/venv/Scripts/WORK4YOU.EXE', [], deps))
  assert.ok(resolveVenvWork4YouCommand('/root/venv/SCRIPTS/work4you.exe', [], deps))
})
