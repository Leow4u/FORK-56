"""Tests for the per-profile Projects store (work4you_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Work4You", folders=["/tmp/work4you"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "work4you"
    assert proj.name == "Work4You"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/work4you"
    assert [f.path for f in proj.folders] == ["/tmp/work4you"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "work4you").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' WORK4YOU_HOME.
