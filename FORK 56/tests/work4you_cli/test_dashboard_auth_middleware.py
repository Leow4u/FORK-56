...
from work4you_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``work4you_session_at`` / ``work4you_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``work4you``
    provider first, which tries to reach Work4You Portal's JWKS; if that's
    unreachable ``work4you`` raises ProviderError. The gate must keep trying the
    ...
    def verify_session(self, *, access_token: str):
        from work4you_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["work4you_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
