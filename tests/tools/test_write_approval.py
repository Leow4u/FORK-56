...
@pytest.fixture
def work4you_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="work4you_wa_test_")
    home = os.path.join(d, ".work4you")
    os.makedirs(home)
    monkeypatch.setenv("WORK4YOU_HOME", home)
    yield home
...
def test_default_gate_is_off(work4you_home):
...
def test_invalid_subsystem_is_off(work4you_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(work4you_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(work4you_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(work4you_home, monkeypatch):
...
def test_handle_approve_all(work4you_home):
...
def test_handle_approval_on(work4you_home):
    from work4you_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(work4you_home):
    from work4you_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(work4you_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(work4you_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(work4you_home):
