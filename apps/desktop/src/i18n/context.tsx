...
export interface I18nConfigClient {
  getConfig: () => Promise<Work4YouConfigRecord>
  saveConfig: (config: Work4YouConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.work4youDesktop?.api) {
      return Promise.resolve({})
    }

    return getWork4YouConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.work4youDesktop?.api) {
    ...
    return saveWork4YouConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: Work4YouConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: Work4YouConfigRecord, locale: Locale): Work4YouConfigRecord {
