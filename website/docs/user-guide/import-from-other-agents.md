...
description: "One-command import of a Claude Code (~/.claude) or OpenAI Codex CLI (~/.codex) setup into Work4You — instructions, allowlists, MCP servers, skills, and memories."
---

# Import from Other Agents

`work4you import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Work4You with one command. It follows the same preview-first pattern as [`work4you claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.
...
|---|---|
| `CLAUDE.md` (global instructions) | Memory entries in `~/.work4you/memories/MEMORY.md` |
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.work4you/skills/claude-code-imports/<name>/` |
...
| Codex CLI | Work4You |
|---|---|
| `AGENTS.md` (global instructions) | Memory entries in `~/.work4you/memories/MEMORY.md` |
| `config.toml` → `[mcp_servers.*]` | `mcp_servers` in `config.yaml` |
| `memories/*.md` | Memory entries in `~/.work4you/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.work4you/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `work4you setup` to configure providers, or add secrets to `~/.work4you/.env`.
...
- **Conflicts are skipped by default.** An MCP server or skill that already exists in Work4You is reported as a conflict; pass `--overwrite` to replace it.
...
- Coming from OpenClaw instead? Use [`work4you claw migrate`](../guides/migrate-from-openclaw.md).
