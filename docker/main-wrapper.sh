...
# with-contenv before touching WORK4YOU_HOME / PATH. On the non-PID-1
...
if [ -z "${WORK4YOU_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${WORK4YOU_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export WORK4YOU_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset WORK4YOU_MAIN_WRAPPER_ENV_READY
...
_work4you_orig_cwd="${WORK4YOU_ORIG_CWD:-$PWD}"
...
# Work4You subcommand pass-through.
drop work4you "$@"
