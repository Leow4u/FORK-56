"""work4you-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $WORK4YOU_HOME/config.yaml (profile-scoped):
  plugins:
    work4you-memory-store:
      db_path: $WORK4YOU_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "work4you-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, work4you_home):
        """Write config to config.yaml under plugins.work4you-memory-store."""
        from pathlib import Path
        config_path = Path(work4you_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["work4you-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_work4you_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _work4you_home = str(get_work4you_home())
        _default_db = _work4you_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $WORK4YOU_HOME in user-supplied paths so config values like
        # "$WORK4YOU_HOME/memory_store.db" or "~/.work4you/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$WORK4YOU_HOME", _work4you_home)
            db_path = db_path.replace("${WORK4YOU_HOME}", _work4you_home)
