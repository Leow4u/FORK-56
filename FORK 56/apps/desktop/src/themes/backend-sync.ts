/**
 * Live skin sync from the Work4You backend.
 *
 * The backend resolves the active skin (built-in or `$WORK4YOU_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Work4You
 ...
 */
export function ingestBackendSkin(skin: Work4YouSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (work4you), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → work4you). So we only
