...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update Work4You Desktop and try again.')
    }
...
export interface Work4YouWebSocketUrlOptions {
...
export function buildWork4YouWebSocketUrl(options: Work4YouWebSocketUrlOptions): string {
