import type { SkinBranding, SkinColors } from '@work4you/shared/skin'
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Work4You navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Work4You look was never hand-authored: for years the
 ...
 *  answer when known (cached in WORK4YOU_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.WORK4YOU_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit WORK4YOU_TUI_THEME / COLORFGBG signals above still win,
...
// `WORK4YOU_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `WORK4YOU_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `WORK4YOU_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `WORK4YOU_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Work4You palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.WORK4YOU_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.WORK4YOU_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.WORK4YOU_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into WORK4YOU_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into WORK4YOU_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Work4You navy
  ...
  // the derived value already carries the identity seeds (e.g. Work4You navy).
  const activeRow =
