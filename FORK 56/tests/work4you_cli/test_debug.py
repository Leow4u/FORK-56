"""Tests for ``work4you debug`` CLI command and debug utilities."""
...
@pytest.fixture
def work4you_home(tmp_path, monkeypatch):
    """Set up an isolated WORK4YOU_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from work4you_cli.debug import _upload_paste_rs
        ...
        with patch("work4you_cli.debug.urllib.request.urlopen", return_value=mock_resp):
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("work4you_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("work4you_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, work4you_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = work4you_home / "logs" / "agent.log"
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, work4you_home):
        ...
        (work4you_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `work4you debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, work4you_home):
        from work4you_cli.debug import _capture_log_snapshot

        (work4you_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, work4you_home):
        from work4you_cli.debug import _capture_log_snapshot

        (work4you_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "Work4You Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(work4you_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, work4you_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, work4you_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from work4you_cli.debug import _capture_log_snapshot

        (work4you_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, work4you_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of WORK4YOU_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def work4you_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated WORK4YOU_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".work4you"
        home.mkdir()
        monkeypatch.setenv("WORK4YOU_HOME", str(home))
        ...
        monkeypatch.delenv("WORK4YOU_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, work4you_home_with_secret):
        from work4you_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, work4you_home_with_secret):
        from work4you_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, work4you_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when WORK4YOU_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("WORK4YOU_REDACT_SECRETS", "false")

        from work4you_cli.debug import _capture_log_snapshot

        assert os.environ.get("WORK4YOU_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, work4you_home_with_secret
    ):
        from work4you_cli.debug import _capture_log_snapshot

        log_path = work4you_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, work4you_home_with_secret):
        from work4you_cli.debug import _capture_log_snapshot
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, work4you_home):
        from work4you_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, work4you_home, capsys):
        ...
        with patch("work4you_cli.dump.run_dump"), \
             ...
             patch("work4you_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, work4you_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, work4you_home_with_secret, capsys
    ):
        ...
        with patch("work4you_cli.dump.run_dump"), \
             ...
             patch("work4you_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    ...
    def test_share_subcommand_routes(self, work4you_home):
        from work4you_cli.debug import run_debug
...
class TestScheduleAutoDelete:
    ...
    ``~/.work4you/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``work4you debug``
    invocation.
    """


    def test_records_pending_to_json(self, work4you_home):
    ...
    def test_dedupes_same_url(self, work4you_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, work4you_home):
    ...
    def test_sweep_leaves_future_entries_alone(self, work4you_home):
    ...
    def test_sweep_survives_network_failure(self, work4you_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, work4you_home):
        from work4you_cli.debug import run_debug
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, work4you_home, capsys):
        from work4you_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``work4you debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, work4you_home):
        from work4you_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (work4you_home / "logs" / "agent.log").write_text(
        ...
        with patch("work4you_cli.dump.run_dump"), patch(
            "work4you_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("work4you_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, work4you_home):
        from work4you_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Work4You-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:

    def test_no_redact_omits_banner(self, work4you_home):
    ...
    def test_redaction_keeps_secrets_out(self, work4you_home):
        from work4you_cli.debug import collect_share_bundle
...
class TestBuildWork4YouBundle:
    def test_envelope_shape_and_gzip(self, work4you_home):
        import gzip
        import json as _json

        from work4you_cli.debug import build_work4you_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_work4you_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "work4you-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_work4you_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareWork4You:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_work4you_success_prints_view_url(self, work4you_home, capsys):
        from work4you_cli.debug import run_debug_share
        ...
        with patch("work4you_cli.dump.run_dump"), patch(
            "work4you_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Work4You-INTERNAL" in out
    ...
    def test_work4you_failure_suggests_local(self, work4you_home, capsys):
        from work4you_cli.debug import run_debug_share
        ...
        assert "Work4You upload failed" in err
        assert "--local" in err

    def test_work4you_does_not_touch_pastebin(self, work4you_home):
        from work4you_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("work4you_cli.dump.run_dump"), patch(
            "work4you_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("work4you_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [work4you|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``Work4YouCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def test_bare_debug_defaults_to_paste(self):
        ...
        assert c["work4you"] is False and c["local"] is False
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug WORK4YOU")
        assert c["work4you"] is True
...
class TestShareConsentGate:
    """`work4you debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, work4you=False,
    ...
    def test_non_interactive_requires_yes(self, work4you_home, capsys, monkeypatch):
        ...
        with patch("work4you_cli.dump.run_dump"), \
             patch("work4you_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, work4you_home, capsys, monkeypatch):
