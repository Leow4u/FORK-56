 ...
 * product name (`Work4You`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Work4You'
 *
 * This is a hard failure with no obvious cause for the user — `work4you desktop`
 ...
 * (the mac-unpacked Work4You.app bundle) and Windows (win-unpacked), so we
 ...
 * Work4You.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `work4you desktop --build-only` (work4you_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Work4You.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Work4You'}.exe`
