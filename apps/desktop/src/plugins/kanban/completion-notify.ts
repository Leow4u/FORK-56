 ...
 *    desktop shell fires only while the user is AWAY from Work4You. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@work4you/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Work4You (the toast above covers the foreground case). Isolated:
