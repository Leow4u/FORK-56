...
"""
    ...
    assert "Tyler Williams > Active Projects: Work4You" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``work4you_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_WORK4YOU_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_WORK4YOU_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.work4you/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: work4you-4-405b\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "work4you-4-405b" in real.read_text(encoding="utf-8")
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_WORK4YOU_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # work4you_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.work4you/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "work4you_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Work4You``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Work4You prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Work4You to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Work4You config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Work4You uses tools well"
    # All-lowercase matches → lowercase ``work4you``; this preserves the
    # real filesystem path ``~/.work4you`` (Work4You home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "work4you should always respond concisely"
