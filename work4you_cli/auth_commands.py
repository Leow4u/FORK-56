...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "work4you", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_work4you_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:work4you_pkce",
    ...
    if provider == "work4you":
        # Codex-style auto-import: if a shared Work4You credential lives at
        # <work4you-root>/shared/work4you_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `work4you --profile <name>
        # auth add work4you --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_work4you_state()
        if shared:
            try:
                path = auth_mod._work4you_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Work4You OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_work4you_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_work4you_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._work4you_device_code_login(
        ...
        # Honor `--label <name>` so work4you matches other providers' UX.  The
        # helper embeds this into providers.work4you so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("work4you").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_work4you_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``work4you auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``work4you auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`work4you auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Work4You reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `work4you auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `work4you auth` is called bare."""
