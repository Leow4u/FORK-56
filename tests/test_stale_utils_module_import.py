...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.work4you/work4you/utils.py)
...
2. ``work4you update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``work4you_cli/main.py`` (the ``work4you update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``work4you_constants`` after the
