"""Coding-context awareness — base Work4You, every interactive surface.

When the user runs Work4You inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Work4You shifts into a **coding posture**. This module is the
...
from work4you_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("work4you.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "work4you", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _WORK4YOU_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``WORK4YOU_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Work4You should operate in its coding posture right now."""
