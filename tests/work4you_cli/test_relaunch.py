"""Tests for work4you_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from work4you_cli import relaunch as relaunch_mod


class TestResolveWork4YouBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/work4you"
        ...
        assert relaunch_mod.resolve_work4you_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "work4you"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'work4you' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_work4you_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/work4you" if name == "work4you" else None
        )
        assert relaunch_mod.resolve_work4you_bin() == "/usr/bin/work4you"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_work4you_bin", lambda: "/usr/bin/work4you")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/work4you"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/work4you", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/work4you", ["/usr/bin/work4you", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        work4you).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_work4you_bin", lambda: r"C:\Users\test\work4you.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\work4you.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\work4you.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_work4you_bin", lambda: r"C:\work4you.exe")
...
class TestResolveWork4YouBinWindowsPyGuard:
    """On Windows, resolve_work4you_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a work4you.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\work4you.exe" if name == "work4you" else None,
        )

        bin_path = relaunch_mod.resolve_work4you_bin()
        # Must NOT be the .py — must be the work4you.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\work4you.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "work4you"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_work4you_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_work4you_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND work4you.exe
        ...
        assert relaunch_mod.resolve_work4you_bin() is None
