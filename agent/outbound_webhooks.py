...
(``gateway/platforms/webhook.py``): inbound wakes Work4You when the world
changes; outbound tells the world when Work4You does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Work4You-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``WORK4YOU_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.work4you/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/work4you-events
          ...
          secret_env: WORK4YOU_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `work4you hooks list`
...
Headers::
    ...
    User-Agent:              Work4You-Outbound-Webhook
    X-Work4You-Event:          <hook event name>
    X-Work4You-Delivery:       <delivery_id>
    X-Work4You-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("WORK4YOU_SAFE_MODE"):
        logger.info("WORK4YOU_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Work4You-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Work4You-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
