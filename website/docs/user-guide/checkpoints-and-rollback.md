...
Work4You can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
work4you chat --checkpoints
```

Or enable globally in `~/.work4you/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.work4you/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `work4you checkpoints` | Show total size, project count, per-project breakdown |
| `work4you checkpoints status` | Same as bare `checkpoints` |
| `work4you checkpoints list` | Alias for `status` |
| `work4you checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `work4you checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `work4you checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Work4You detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.work4you/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/work4you/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.work4you/checkpoints/ at startup
  ...
  # `work4you checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `work4you checkpoints prune` manually.
...
Work4You responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.work4you/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/work4you                                        20       2h ago  live
...
Clear with: work4you checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
work4you checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files Work4You itself changed. Every successful
...
Work4You last wrote (you edited it afterwards, or Work4You never touched it) is
...
If the ledger is empty (a store created before this feature, or Work4You hasn't
...
- **Directory scope** — Work4You skips overly broad directories (root `/`, home `$HOME`).
...
~/.work4you/checkpoints/
  ...
  │   ├── refs/work4you/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `work4you checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.work4you/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.work4you/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
work4you checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `work4you chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `work4you checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Work4You session in its own worktree/branch, with checkpoints as an extra layer.
