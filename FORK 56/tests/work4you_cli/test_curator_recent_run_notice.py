...
The notice prints the most recent curator run summary on `work4you update`,
...
session start) so users normally never see the rename map. `work4you update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(work4you_main)
