 ...
 * dir (macOS `~/Library/Application Support/Work4You/connection.json`, Windows
 * `AppData\Roaming\Work4You\connection.json`). Anything that can read the file
 ...
 *      or in WORK4YOU_HOME (logs included).
 ...
 * `X-Work4You-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Work4You desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'work4you-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'Work4YouE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Work4You-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Work4You gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-work4you-session-token']
 ...
 * backend, so we skip spawning `work4you serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startWork4You() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    WORK4YOU_DESKTOP_APP_NAME: STABLE_APP_NAME,
    WORK4YOU_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { work4youDesktop?: Record<string, unknown> }).work4youDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Work4You should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { work4youDesktop: any }).work4youDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // WORK4YOU_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.work4youHome, needles),
      'the gateway session token leaked into a WORK4YOU_HOME file (logs included)',
    ).toEqual([])
