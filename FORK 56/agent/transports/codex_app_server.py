...
projection into Work4You' display, approval bridging, transcript projection into
...
"codex_app_server"`. Work4You' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Work4You secret — gateway
        ...
        spawn_env = work4you_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("WORK4YOU_KANBAN_TASK"):
            kanban_db = spawn_env.get("WORK4YOU_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "WORK4YOU_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("WORK4YOU_HOME", os.path.expanduser("~/.work4you")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "work4you",
        client_title: str = "Work4You",
