"""Tests for work4you_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate WORK4YOU_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from work4you_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "work4you": {
                "metadata": {"display_name": "Work4You Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from work4you_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://work4you.ai/docs/api/model-catalog.json"
    FALLBACK = (
...
class TestCuratedAccessors:
    ...
    def test_work4you_returns_none_when_catalog_empty(self, isolated_home):
        from work4you_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_work4you_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["work4you"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("work4you")
    ...
    def test_shipped_manifest_labels_silent_default(self, isolated_home):
        """Contract with the in-repo manifest: both provider blocks label the
        same default entry the code constant points at."""
        import work4you_cli.model_catalog as model_catalog
        ...
        for provider in ("openrouter", "work4you"):
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in work4you_cli.models."""



    def test_picker_work4you_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["work4you"]``
        list in curated order — matching the ``work4you model`` CLI — not the live
        ...
        # ``WORK4YOU_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` WORK4YOU_HOME directly instead.
        import importlib
        from work4you_cli import model_catalog
        from work4you_cli.models import get_curated_work4you_model_ids
        importlib.reload(model_catalog)
        try:
            from work4you_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["WORK4YOU_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"work4you": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["work4you"] = get_curated_work4you_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("work4you_cli.models.check_work4you_free_tier", return_value=False), patch(
                "work4you_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "work4you_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_work4you_model_ids()
                picker = list_picker_providers(
                    current_provider="work4you", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        work4you_row = next((r for r in picker if r["slug"] == "work4you"), None)
        assert work4you_row is not None, "work4you row must appear when authed"
        assert work4you_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_work4you_model_ids()
                full = list_picker_providers(current_provider="work4you", max_models=None)
                one = list_picker_providers(current_provider="work4you", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="work4you", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _nous(rows):
            return next((r for r in rows if r["slug"] == "work4you"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated work4you list"
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["work4you"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['work4you'] / OPENROUTER_MODELS. "
