...
function sessionsBotsTree() {
  ...
  registerPane('work4you-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'work4you-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('work4you-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('work4you-bots:pane')
    expect($hiddenStripTabs.get()).toContain('work4you-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('work4you-bots:pane')

    expect(setStripTabHidden('work4you-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('work4you-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('work4you-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('work4you-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('work4you.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('work4you-bots:pane')
    ...
    revealTreePane('work4you-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('work4you-bots:pane')
    expect(window.localStorage.getItem('work4you.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'work4you-bots:pane', title: 'work4you-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'work4you-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
