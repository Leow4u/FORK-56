"""Comprehensive tests for work4you_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and WORK4YOU_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.work4you/profiles)
    * WORK4YOU_HOME  -> tmp_path/.work4you  (so get_work4you_home() agrees)
    * Creates the bare-minimum ~/.work4you directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".work4you"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("WORK4YOU_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_work4you_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".work4you"
...
class TestNoSkillsOptOut:
    """Tests for `work4you profile create --no-skills` and the opt-out marker."""
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `work4you update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".work4you" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("work4you_cli.profiles._cleanup_gateway_service"), \
             patch("work4you_cli.profiles.time.sleep"), \
             patch("work4you_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "work4you_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "work4you_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "work4you_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "work4you_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_work4you_shim(self, profile_env, monkeypatch):
        """A `work4you` console-script shim spawned directly (e.g. Electron's
        findOnPath('work4you') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "work4you" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "work4you".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/work4you", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/work4you", "--profile", "other", "serve"]),
            # Non-work4you script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_work4you_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "work4you" (e.g.
        work4you-notes.py, work4you-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "work4you" -- only the actual known
        console-script entry points (work4you, work4you, work4you-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/work4you-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/work4you-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (work4you,
        work4you-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "work4you"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/work4you",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/work4you-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".work4you" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".work4you" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom WORK4YOU_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("WORK4YOU_HOME", str(custom))
        # With Docker-aware roots, a custom WORK4YOU_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("work4you_cli.profiles.shutil.which", lambda name: "/opt/work4you/bin/work4you")
        from work4you_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/work4you/bin/work4you -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from work4you_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from work4you_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from work4you_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".work4you" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("work4you_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".work4you" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".work4you" / "honcho.json"
        ...
        assert "work4you.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["work4you_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["work4you_heimdall"]["peerName"] == "user-peer"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultWork4YouHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_work4you_home()."""



    def test_default_work4you_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_work4you_home() returns WORK4YOU_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WORK4YOU_HOME", str(docker_home))
        home = _get_default_work4you_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under WORK4YOU_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WORK4YOU_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="work4you gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with WORK4YOU_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("WORK4YOU_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_work4you_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured WORK4YOU_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-work4you"
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("WORK4YOU_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No WORK4YOU_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("WORK4YOU_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_work4you_home()
