"""Harness: dashboard opt-in via WORK4YOU_DASHBOARD.

Today (tini): dashboard starts once when WORK4YOU_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    """Without WORK4YOU_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "work4you dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without WORK4YOU_DASHBOARD"
    )
...
# whenever `WORK4YOU_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled work4you
    provider activates whenever ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``work4you`` in the
    ...
    start_container(
        ...
        "WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "work4you" in provider_names, (
        "Bundled dashboard_auth/work4you provider should register when "
        f"WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and WORK4YOU_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``WORK4YOU_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
