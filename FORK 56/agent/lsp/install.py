...
appropriate.  All installs go to a Work4You-owned bin staging dir,
``<WORK4YOU_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``work4you lsp
  status``.
...
# ``<WORK4YOU_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `work4you lsp status` reports the host's presence.
...
def work4you_lsp_bin_dir() -> Path:
    """Return the Work4You-owned bin staging dir for LSP servers."""
    from work4you_constants import get_work4you_home

    p = get_work4you_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(work4you_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $WORK4YOU_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Work4You installed and
    ...
    if npm is None:
        ...
        return None
    staging = work4you_lsp_bin_dir().parent  # <WORK4YOU_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = work4you_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a work4you-owned target dir.
    ...
    console script.
    """
    pip_target = work4you_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = work4you_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``work4you lsp status`` CLI to give users a quick
