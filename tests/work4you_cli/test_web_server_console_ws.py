"""Dashboard Work4You Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_work4you_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from work4you_cli.console_engine import ConsoleResult, Work4YouConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(Work4YouConsoleEngine, "execute", slow_execute)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert complete["prompt"] == "work4you> "
