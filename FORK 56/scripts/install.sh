...
# Python-driven tool session (e.g. Work4You terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Leow4u/FORK-56.git"
REPO_URL_HTTPS="https://github.com/Leow4u/FORK-56.git"
WORK4YOU_HOME="${WORK4YOU_HOME:-$HOME/.work4you}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${WORK4YOU_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$WORK4YOU_INSTALL_DIR"
...
#   code at /usr/local/lib/work4you, command at /usr/local/bin/work4you,
#   data still at /root/.work4you (WORK4YOU_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --work4you-home PATH  Data directory (default: ~/.work4you, or \$WORK4YOU_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Work4You installs the code under"
            echo "  /usr/local/lib/work4you and links the command into"
            ...
            echo "  Existing installs at \$WORK4YOU_HOME/work4you are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Work4You Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Work4You.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `work4you update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Work4You-Setup) passes it so
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $WORK4YOU_HOME/work4you
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $WORK4YOU_HOME/work4you
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/work4you
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $WORK4YOU_HOME/work4you — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in WORK4YOU_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$WORK4YOU_HOME/work4you/.git" ]; then
            INSTALL_DIR="$WORK4YOU_HOME/work4you"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/work4you)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/work4you"
        ...
        log_info "  Data:    $WORK4YOU_HOME (unchanged)"
...
# Point a Work4You-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $WORK4YOU_HOME/node/bin — which is NOT on
...
# Work4You's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Work4You-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$WORK4YOU_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$WORK4YOU_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$WORK4YOU_HOME/node/etc/npmrc"
}

get_work4you_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://work4you.ai/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Work4You owns its own uv at $WORK4YOU_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (work4you_cli/managed_uv.py) looks in the same
    # place, so install.sh and `work4you update` stay in sync.
    local _managed_uv="$WORK4YOU_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $WORK4YOU_HOME/bin ..."
    mkdir -p "$WORK4YOU_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/work4you-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/work4you-uv-installer.$$.sh")"
    ...
    # directly into $WORK4YOU_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$WORK4YOU_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Work4You-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Work4You-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Work4You-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Work4You-managed Node from a previous run over a too-old system one.
    if [ -x "$WORK4YOU_HOME/node/bin/node" ] && [ -x "$WORK4YOU_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$WORK4YOU_HOME/node/bin/node" --version)"; then
        export PATH="$WORK4YOU_HOME/node/bin:$PATH"
        log_success "Node.js $("$WORK4YOU_HOME/node/bin/node" --version) found (Work4You-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Work4You-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Work4You requires Node >=26) — installing Work4You-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.work4you/node/..."
    ...
    # Place into ~/.work4you/node/ and symlink binaries into the same bin dir
    # the work4you command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$WORK4YOU_HOME/node"
    mkdir -p "$WORK4YOU_HOME"
    mv "$extracted_dir" "$WORK4YOU_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$WORK4YOU_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$WORK4YOU_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$WORK4YOU_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$WORK4YOU_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.work4you/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Work4You install may complete, but web search and dependency downloads can fail."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `work4you update` path
                # (#4735).
                ...
                local stash_name
                stash_name="work4you-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``work4you update`` and reset to the
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        WORK4YOU_BIN="$INSTALL_DIR/venv/bin/python"
        WORK4YOU_ENTRYPOINT="$INSTALL_DIR/work4you"
    else
        WORK4YOU_BIN="$(which work4you 2>/dev/null || echo "")"
        if [ -z "$WORK4YOU_BIN" ]; then
    ...
    if [ ! -x "$WORK4YOU_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$WORK4YOU_ENTRYPOINT" ]; }; then
        log_warn "Work4You launcher prerequisites not found"
    ...
    # Create a user-facing shim for the work4you command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $WORK4YOU_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$WORK4YOU_BIN"` self-recurse. (#21454)
...
unset PYTHONPATH
unset PYTHONHOME
exec "$WORK4YOU_BIN" "$WORK4YOU_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/work4you" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    # Also expose `work4you`. The `work4you` console script declared in
...
unset PYTHONPATH
unset PYTHONHOME
exec "$WORK4YOU_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/work4you" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$WORK4YOU_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/work4you"
    log_success "Installed work4you launcher → $command_link_display_dir/work4you"

    # Also expose `work4you-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `work4you-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Work4You as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/work4you-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/work4you-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$WORK4YOU_BIN" "$WORK4YOU_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/work4you-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$WORK4YOU_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/work4you-acp"
    log_success "Installed work4you-acp launcher → $command_link_display_dir/work4you-acp"
...
copy_config_templates() {
    ...
    # Create ~/.work4you directory structure (config at top level, code in subdir)
    mkdir -p "$WORK4YOU_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.work4you/.env (top level, easy to find)
    if [ ! -f "$WORK4YOU_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$WORK4YOU_HOME/.env"
            log_success "Created ~/.work4you/.env from template"
        else
            touch "$WORK4YOU_HOME/.env"
            log_success "Created ~/.work4you/.env"
        fi
    else
        log_info "~/.work4you/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$WORK4YOU_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.work4you/config.yaml (top level, easy to find)
    if [ ! -f "$WORK4YOU_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$WORK4YOU_HOME/config.yaml"
            log_success "Created ~/.work4you/config.yaml from template"
        fi
    else
        log_info "~/.work4you/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in work4you_cli/default_soul.py — the
    ...
    if [ ! -f "$WORK4YOU_HOME/SOUL.md" ]; then
        cat > "$WORK4YOU_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.work4you/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.work4you/"

    # Seed bundled skills into ~/.work4you/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$WORK4YOU_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $WORK4YOU_HOME/.no-bundled-skills"
        log_info "  Future 'work4you update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.work4you/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.work4you/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$WORK4YOU_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$WORK4YOU_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.work4you/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$WORK4YOU_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Work4You browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
install_node_deps() {
    ...
    # Keep the checkout clean so `work4you update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `work4you tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$WORK4YOU_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Work4You' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$WORK4YOU_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'work4you tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `work4you tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `work4you computer-use install` on their own).
    ...
    # Same upstream installer `work4you computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Work4You install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Work4You'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'work4you tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: work4you computer-use install"
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$WORK4YOU_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                WORK4YOU_CMD="$(get_work4you_command_path)"
                $WORK4YOU_CMD whatsapp || true
            fi
        else
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $WORK4YOU_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $WORK4YOU_CMD gateway start 2>/dev/null; then
            ...
            nohup $WORK4YOU_CMD gateway > "$WORK4YOU_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.work4you/logs/gateway.log"
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.work4you-bootstrap-complete, which tells the Work4You
    ...
    local marker_path="$INSTALL_DIR/.work4you-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $WORK4YOU_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $WORK4YOU_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $WORK4YOU_HOME/cron/, sessions/, logs/"
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        echo -e "${YELLOW}⚡ Reload your shell to use 'work4you' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$WORK4YOU_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$WORK4YOU_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$WORK4YOU_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$WORK4YOU_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Work4You` rename
...
# _purge_electron_build_cache() used by `work4you desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Work4You-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `work4you desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Work4You" ]; then
            app="$desktop_dir/release/linux-unpacked/Work4You"
        elif [ -x "$desktop_dir/release/linux-unpacked/work4you" ]; then
            app="$desktop_dir/release/linux-unpacked/work4you"
        fi
    else
        local cand
        for cand in \
            ...
            "$desktop_dir/release/mac/Work4You.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `work4you desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading WORK4YOU_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if WORK4YOU_HOME="$WORK4YOU_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from work4you_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `work4you update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Work4You-managed Node
            # provisioned during prerequisites/node-deps (at $WORK4YOU_HOME/node/bin)
        ...
        complete)
            ...
            # $WORK4YOU_HOME. $WORK4YOU_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $WORK4YOU_HOME.
    # $WORK4YOU_HOME is a shared data dir (it can be bind-mounted into a Docker
