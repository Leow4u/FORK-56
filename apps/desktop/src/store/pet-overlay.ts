 ...
 * app's bounds and stays visible while Work4You is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'work4you.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'work4you.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.work4youDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.work4youDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.work4youDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.work4youDesktop?.petOverlay?.close()
}
