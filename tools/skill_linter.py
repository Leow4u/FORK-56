...
Design contract (matches the Work4You "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    work4you_meta = meta.get("work4you") if isinstance(meta, dict) else None
    if not isinstance(work4you_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.work4you.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in work4you_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.work4you.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("work4you", "agent", "work4you") and (
        author != "Work4You"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Work4You' (proper caps) "
