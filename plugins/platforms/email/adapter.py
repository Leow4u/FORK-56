"""
Email platform adapter for the Work4You gateway.

Allows users to interact with Work4You by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "work4you" "version" "{_work4you_version}" '
            '"vendor" "Work4You" '
            '"support-email" "noreply@work4you.ai")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `work4you config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `work4you gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Work4You")
        ...
        msg_id = f"<work4you-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# work4you_cli/gateway.py, and the _send_email dispatch in
