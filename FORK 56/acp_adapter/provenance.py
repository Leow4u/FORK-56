...
This is an additive Work4You extension surfaced under ACP ``_meta.work4you`` so
...
compression rotates the internal Work4You head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.work4you.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_work4you_session_id: The live internal Work4You DB session id
            (``state.agent.session_id``).
        previous_work4you_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"work4you": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_work4you_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_work4you_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_work4you_session_id}
    ...
    rotated = bool(
        ...
        and previous_work4you_session_id != current_work4you_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentWork4YouSessionId": current_work4you_session_id,
        "rootWork4YouSessionId": root_id,
        "parentWork4YouSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_work4you_session_id:
        provenance["previousWork4YouSessionId"] = previous_work4you_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"work4you": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"work4you": {"sessionProvenance": prov}}
