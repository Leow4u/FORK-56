...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Work4You to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Work4You to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.work4you/.env`:
...
All standard Work4You commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.work4you/logs/gateway.log`
...
4. Consider enabling verbose logging: `work4you gateway run -vv`
...
Yuanbao has a maximum message size. Work4You automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
work4you chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
