...
**three** code paths in Work4You that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Work4You — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Work4You
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Work4You is
   refusing my value."
...
work4you config get delegation.max_concurrent_children

# 2. Did Work4You' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.work4you/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with work4you on path)
...
appears, **the cap is the model**, not Work4You.
