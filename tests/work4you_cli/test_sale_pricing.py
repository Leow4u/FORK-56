...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # Work4You Portal opts in via include_sale_original=True.
...
def test_resolve_work4you_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set WORK4YOU_INFERENCE_BASE_URL — pricing must follow it.

    Without this, anonymous/failed-auth fallback hits prod and sale
    ``pricing.original`` never reaches Desktop/CLI pickers.
    """
    monkeypatch.setenv(
        "WORK4YOU_INFERENCE_BASE_URL",
        "https://stg-inference-api.work4you.ai/v1",
    )
    ...
    api_key, base_url = models_mod._resolve_work4you_pricing_credentials()
    assert api_key == ""
    assert base_url == "https://stg-inference-api.work4you.ai/v1"
