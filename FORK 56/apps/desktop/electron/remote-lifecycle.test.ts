...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    work4youPath: '~/.local/bin/work4you',
    work4youHome: '~/.work4you',
...
test('listRemoteWork4YouProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/WORK4YOU_HOME/, '/Users/zillajr/.work4you\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteWork4YouProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteWork4YouProfiles rejects a hostile WORK4YOU_HOME', async () => {
  const ssh = fakeSsh([[/WORK4YOU_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteWork4YouProfiles(ssh),
    (err: any) => {
...
test('locateWork4You prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/work4you/, 'OK']])
  assert.equal(await locateWork4You(ssh, '/opt/work4you'), '/opt/work4you')
})

test('locateWork4You throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong work4you" bug.
  const ssh = fakeSsh([
    [/command -v work4you/, '/home/u/.local/bin/work4you\n'],
    [/\[ -x .*\.local\/bin\/work4you/, 'OK']
  ])

  await assert.rejects(
    () => locateWork4You(ssh, '/bad/path/work4you'),
    (err: any) => {
      assert.equal(err.kind, 'work4you-not-found')
      assert.match(err.message, /\/bad\/path\/work4you/)
...
test('locateWork4You falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v work4you/, '/home/u/.local/bin/work4you\n'],
    [/\[ -x .*\.local\/bin\/work4you/, 'OK']
  ])

  assert.equal(await locateWork4You(ssh, ''), '/home/u/.local/bin/work4you')
})

test('locateWork4You preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$WORK4YOU_BIN" "$WORK4YOU_ENTRYPOINT" "$@",
  // where $WORK4YOU_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.work4you/work4you/venv/bin/python\n']
  ])

  assert.equal(await locateWork4You(ssh, ''), '/home/u/.local/bin/work4you')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateWork4You must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateWork4You returns an explicit remoteWork4YouPath unchanged', async () => {
  // The override half of #74411: an explicit remoteWork4YouPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/work4you changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/work4you/, 'OK'],
    [/python3 -c/, '/home/u/.work4you/work4you/venv/bin/python\n']
  ])
...
test('locateWork4You falls back to ~/.local/bin/work4you when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateWork4You(ssh, ''), '~/.local/bin/work4you')
})

test('locateWork4You tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/work4you/, 'OK']])
  assert.equal(await locateWork4You(ssh, ''), '~/.work4you/work4you/venv/bin/work4you')
})

test('locateWork4You throws a work4you-not-found error with an install hint', async () => {
...
test('locateWork4You uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v work4you/, '/x/work4you'],
    [/\[ -x/, 'OK']
  ])

  await locateWork4You(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.work4you/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.work4you/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.work4you/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/work4you'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/work4you serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/work4you'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/work4you', '/Users/cd9c/.work4you', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /work4you.*venv.*bin.*work4you/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'work4you wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'work4you')
    const launcher = path.join(temp, 'work4you launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/work4you-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/work4you', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('WORK4YOU_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { work4youPath: '/x/work4you', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('WORK4YOU_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('WORK4YOU_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nWORK4YOU_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.work4you\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/print\("OWNED"/, 'OWNED\n'],
    [/kill 333/, ''],
    [/--version/, 'Work4You v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile work4youPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ work4youPath: '/old/stale/work4you', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteWork4YouPath: '/new/work4you', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes work4youHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/WORK4YOU_HOME/, '/home/alice/.work4you\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"work4youHome":"\/home\/alice\/\.work4you"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'WORK4YOU_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/work4you'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.work4you/work4you/venv/bin/work4you'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/work4you'))
  assert.doesNotThrow(() => validateRemotePath('~/.work4you/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/work4you'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('work4you'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/work4you'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/work4you\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/work4you\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/work4you\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/work4you', '/usr/`id`/work4you', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.work4you/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/work4you')
  assert.ok(result.includes('/usr/local/bin/work4you'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/work4you')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('WORK4YOU_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/work4you', 'work', {
    tokenFilePath: `~/.work4you/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.work4you\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/work4you', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Work4You', () => {
  const cmd = buildSpawnCommand('/x/work4you', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env WORK4YOU_DESKTOP=1/)
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.work4you/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.work4you/unrelated.log' }))
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/work4you'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/work4you'), false)
})
