import type {
  Work4YouGitBaseBranch,
  Work4YouGitBranch,
  Work4YouGitWorktree,
  Work4YouRepoPullRequests,
  Work4YouRepoStatus,
  Work4YouReviewList,
  Work4YouReviewShipInfo
} from '@/global'
...
// (window.work4youDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['work4youDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.work4youDesktop

  if (!desktop) {
    throw new Error('Work4You Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: Work4YouGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: Work4YouGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: Work4YouGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<Work4YouRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    ...
    shipInfo: repoPath => gitGet<Work4YouReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<Work4YouRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.work4youDesktop?.git
}
