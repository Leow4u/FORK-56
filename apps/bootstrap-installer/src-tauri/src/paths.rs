...
//!
//! Mirrors `work4you_constants.get_work4you_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\work4you
//!   macOS:   ~/.work4you
//!   Linux:   ~/.work4you  (override via $WORK4YOU_HOME)
//!
//! NOTE (macOS): Python's get_work4you_home(), scripts/install.sh, and the
//! Electron desktop's resolveWork4YouHome() ALL use ~/.work4you on macOS — there
...
/// Returns the canonical Work4You home directory, respecting $WORK4YOU_HOME if set.
pub fn work4you_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("WORK4YOU_HOME") {
    ...
    {
        // %LOCALAPPDATA%\work4you — matches scripts/install.ps1's $Work4YouHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("work4you");
        }
    }

    // macOS + Linux + fallback: ~/.work4you (matches Python get_work4you_home(),
    // install.sh, and the Electron desktop's resolveWork4YouHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".work4you");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".work4you")
}

pub fn log_dir() -> PathBuf {
    work4you_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    work4you_home().join("bootstrap-cache")
}
...
/// WORK4YOU_HOME so it survives repo checkout deletion (unlike anything under
/// work4you/).
///
/// On Windows this is `%LOCALAPPDATA%\work4you\work4you-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_work4you`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under WORK4YOU_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves WORK4YOU_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    work4you_home().join(".work4you-update-in-progress")
}
...
pub fn copy_self_to_work4you_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to WORK4YOU_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_WORK4YOU_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".work4you-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under WORK4YOU_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[work4you-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("WORK4YOU_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_work4you_home() -> String {
    work4you_home().to_string_lossy().into_owned()
}
