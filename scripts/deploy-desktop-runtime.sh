...
usage() {
  echo "Usage: deploy-desktop-runtime.sh --bundle-dir DIR --work4you-home DIR [--install-stamp FILE] [--pinned-commit SHA] [--pinned-branch NAME] [--skip-import-probe]" >&2
}
...
' "$INSTALL_DIR" "$cfg" "$PYTHON_HOME" "$executable" 2>/dev/null || {
    # Bundle python may not be importable yet; rewrite without the module.
    {
