...
Small Node helper that bridges Work4You to Photon's Spectrum SDK
(`spectrum-ts`).  Work4You is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Work4You-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Work4You can receive inbound messages
...
The Work4You plugin's `work4you photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Work4You is Python, so replies go through this sidecar
