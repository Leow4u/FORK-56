...
from agent.auxiliary_client import (
    ...
    _refresh_work4you_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "WORK4YOU_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("work4you_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("work4you_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp WORK4YOU_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "work4you-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "work4you", "model": "work4you-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "work4you",
                                    "model": "work4you-4-405b",
                                },
                            },
                        },
                    }
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("work4you", "work4you-4", "https://inference-api.work4you.ai/v1", "max_tokens"),
        ],
    )
...
class TestWork4YouTagsScoping:
    def test_tags_injected_when_provider_is_nous(self, monkeypatch):
        ...
        assert kwargs["extra_body"]["tags"] == aux._work4you_portal_tags()
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``work4you auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
        monkeypatch.delenv("WORK4YOU_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
        monkeypatch.setenv("WORK4YOU_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``work4you model``, no per-task override is
    ...
    fallback chain (OpenRouter / Work4You / etc.), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_skips_paid_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("work4you_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = _try_openrouter()
        assert client is None
        assert model is None
    ...
    def test_free_only_allows_free_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("work4you_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("work4you_cli.config.load_config_readonly", return_value={"auxiliary": {}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_work4you_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_work4you_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_work4you_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_work4you_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("work4you_cli.models.get_work4you_recommended_aux_model", return_value=None),
        ):
    ...
    def test_call_llm_retries_work4you_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.work4you.ai/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale work4you key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.work4you.ai/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("work4you", "work4you-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "work4you-model")),
            ...
            patch("agent.auxiliary_client._resolve_work4you_runtime_api", return_value=("fresh-agent-key", "https://inference-api.work4you.ai/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
...
class TestIsModelNotFoundError:
    ...
    def test_work4you_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Work4You → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshWork4YouRecommendedModel:
    """_refresh_work4you_recommended_model picks a fresh model after a stale 404."""
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.models.get_work4you_recommended_aux_model", _boom)
        out = _refresh_work4you_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _WORK4YOU_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.models.get_work4you_recommended_aux_model",
            lambda **kw: _WORK4YOU_MODEL,
        )
        out = _refresh_work4you_recommended_model(
            vision=False, stale_model=_WORK4YOU_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "work4you", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_nous", return_value=(mock_client, "work4you-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "work4you-model"
        assert label == "work4you"
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Work4You/custom/api-key all fail, payment-fallback returns None —
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Work4You auth / credential pool."""
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Work4You owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("work4you_cli.config.load_config", return_value=config), patch("work4you_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("work4you_cli.config.load_config", return_value=config), patch("work4you_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Work4You reasoning.

    MoA slot reasoning is carried through call_llm as a Work4You
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/Leow4u/FORK-56/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/Leow4u/FORK-56/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "Work4YouAgent"
...
def test_pool_runtime_base_url_uses_work4you_env_override(monkeypatch):
    entry = SimpleNamespace(
        provider="work4you",
        runtime_base_url="https://inference-api.work4you.ai/v1",
        inference_base_url="https://inference-api.work4you.ai/v1",
        base_url="https://inference-api.work4you.ai/v1",
    )
    monkeypatch.setenv("WORK4YOU_INFERENCE_BASE_URL", "https://ai.wildebeest-newton.ts.net/v1")
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        work4you_client = MagicMock()
        work4you_resp = MagicMock()
        work4you_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        work4you_client.chat.completions.create.return_value = work4you_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(work4you_client, "n-model", "work4you")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("work4you_cli.config.load_config", return_value=fake_config), patch("work4you_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("work4you_cli.config.load_config", return_value={"model": {}}), patch("work4you_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestSynchrowork4youFallbackCachePlans:
    @staticmethod
...
class TestAsynchrowork4youFallbackCachePlans:
    @pytest.mark.asyncio
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("work4you_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("work4you") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("work4you_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("work4you") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("work4you_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("work4you") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("work4you_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("work4you") == "openai/gpt-10-mini"
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            ...
            "work4you_cli.models.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("work4you")
