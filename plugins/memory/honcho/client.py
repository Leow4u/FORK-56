...
Resolution order for config file:
  1. $WORK4YOU_HOME/honcho.json  (instance-local, enables isolated Work4You instances)
...
from work4you_cli.profiles import _get_default_work4you_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Work4You profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Work4You profile.

    Resolution order:
      1. WORK4YOU_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``work4you_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"work4you"`` (default profile)
    """
    explicit = os.environ.get("WORK4YOU_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Work4You
    # profile share that same host.  Keep named profiles isolated; only the
    # default Work4You profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $WORK4YOU_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.work4you/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_work4you_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_work4you_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.work4you
    ...
    # resolve_config_path()/get_work4you_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $WORK4YOU_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Work4You profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.work4you block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Work4You-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Work4You session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Work4You session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_work4you_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_work4you_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'work4you honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.work4you_home) if config.work4you_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/work4you_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'work4you honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `work4you honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                work4you_cfg = load_config()
                honcho_cfg = work4you_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
