...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.work4you.ai"
...
def test_resolve_managed_tool_gateway_is_inactive_without_work4you_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "work4you.ai",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "work4you.ai"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_work4you_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "work4you-token",
        )
...
def test_read_work4you_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.auth.resolve_work4you_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_work4you_access_token() == "fresh-token"
...
def test_managed_vendor_endpoints_pin_the_deployed_gateway_url():
    ...
    with patch.dict(
        os.environ,
        {"TOOL_GATEWAY_DOMAIN": "work4you.ai", "TOOL_GATEWAY_SCHEME": "https"},
        clear=False,
    ):
    ...
    assert endpoints == {
        "origin": "https://tool-gateway.work4you.ai",
        "base_url": "https://tool-gateway.work4you.ai/api/bfl",
...
def test_managed_vendor_endpoints_do_not_consult_entitlement():
    ...
    assert endpoints["base_url"] == "https://tool-gateway.work4you.ai/api/bfl"
...
def test_managed_gateway_auth_headers_carry_the_bearer():
    with patch.object(managed_tool_gateway, "managed_work4you_tools_enabled", return_value=True):
    ...
    assert headers == {"Authorization": "Bearer work4you-token"}


def test_managed_gateway_auth_headers_reflect_a_rotated_token():
    # Read fresh on every call: a Work4You access token expires within the hour,
...
class TestManagedMediaUploader:
    """The presign -> PUT -> ``work4you-upload:<token>`` protocol.
    ...
    def _uploader(self, **kwargs):
        return managed_tool_gateway.build_managed_media_uploader(
            ...
            token_reader=kwargs.pop("token_reader", lambda: "work4you-token"),
        )
    ...
    def test_presign_declares_the_exact_type_and_length_the_put_then_sends(self):
        ...
        assert calls["presign"][0]["headers"]["Authorization"] == "Bearer work4you-token"
        ...
        assert calls["result"] == "work4you-upload:tok-1"
    ...
    def test_a_missing_credential_fails_before_any_request(self):
        ...
        with patch.object(managed_tool_gateway, "managed_work4you_tools_enabled", return_value=True), \
            ...
            with pytest.raises(RuntimeError, match="no Work4You credential"):
