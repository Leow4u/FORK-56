"""Tests for ``work4you dashboard register``.
...
The portal HTTP call and the Work4You token resolution are both mocked — this
...
import work4you_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        err = AuthError("not logged in", provider="work4you", relogin_required=True)
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "work4you_cli.auth.resolve_work4you_access_token", side_effect=err
            ), patch("work4you_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Work4You Portal" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.work4you.ai",
        ...
        # (idempotency key) and once for WORK4YOU_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "work4you_cli.auth.resolve_work4you_access_token", return_value=account_token
        ), patch("work4you_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "work4you_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "work4you_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "WORK4YOU_DASHBOARD_PORTAL_URL" not in saved
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / WORK4YOU_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        WORK4YOU_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "WORK4YOU_DASHBOARD_PORTAL_URL":
                return existing_portal
            return None

        with patch(
            "work4you_cli.auth.resolve_work4you_access_token", return_value="tok"
        ), patch("work4you_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "work4you_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "work4you_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
        ):
            # The ambient process env may carry WORK4YOU_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("WORK4YOU_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists WORK4YOU_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://work4you.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    WORK4YOU_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as WORK4YOU_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        WORK4YOU_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "WORK4YOU_DASHBOARD_PUBLIC_URL":
        ...
        with patch(
            ...
            dr, "_resolve_portal_base_url", return_value="https://portal.work4you.ai"
        ), patch(
            "work4you_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "work4you_cli.config.save_env_value", side_effect=fake_save
    ...
    def test_no_redirect_flag_does_not_overwrite_existing(self, capsys):
        ...
        assert "WORK4YOU_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://work4you.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "work4you_cli.config.get_env_value", return_value=None
        ), patch(
            "work4you_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://work4you.example.com/auth/callback",
                )
            )
        assert saved["WORK4YOU_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["WORK4YOU_DASHBOARD_PUBLIC_URL"] == "https://work4you.example.com"
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-work4you.ai"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-work4you.ai"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.work4you.ai/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
