#!/usr/bin/env python3
"""Telephony helper for the Work4You optional telephony skill.

Capabilities:
- Persist telephony provider credentials to the Work4You .env file ($WORK4YOU_HOME/.env)
...
def _work4you_home() -> Path:
    return Path(os.environ.get("WORK4YOU_HOME", "~/.work4you")).expanduser()


def _env_path() -> Path:
    return _work4you_home() / ".env"


def _config_path() -> Path:
    return _work4you_home() / "config.yaml"


def _state_path() -> Path:
    return _work4you_home() / "telephony_state.json"
...
def _load_root_config() -> dict[str, Any]:
    ...
    if not path.exists():
        return {}
    try:
        import yaml  # optional dependency; Work4You already ships PyYAML
    except Exception:
        return {}
    try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Work4You telephony helper")
