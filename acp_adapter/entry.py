"""CLI entry point for the work4you ACP adapter.

Loads environment variables from ``~/.work4you/.env``, configures logging
...
try:
    ...
    # shadowing Work4You's own modules — ``work4you acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from WORK4YOU_HOME (default ``~/.work4you``)."""
    from work4you_cli.env_loader import load_work4you_dotenv

    work4you_home = get_work4you_home()
    loaded = load_work4you_dotenv(work4you_home=work4you_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", work4you_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--version", action="store_true", help="Print Work4You version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.work4you/node/ "
...
def _print_version() -> None:
    ...
    print(work4you_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import Work4YouACPAgent  # noqa: F401

    print("Work4You ACP check OK")


def _run_setup() -> None:
    from work4you_cli.main import main as work4you_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "work4you", "model"]
        work4you_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting work4you ACP adapter")
    ...
    import acp
    from .server import Work4YouACPAgent
    ...
    if os.environ.get("WORK4YOU_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
