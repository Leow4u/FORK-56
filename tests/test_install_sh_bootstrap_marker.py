...
The marker at ``$INSTALL_DIR/.work4you-bootstrap-complete`` is what the desktop
...
def test_marker_matches_the_schema_the_desktop_validates(tmp_path):
    ...
    marker = install_dir / ".work4you-bootstrap-complete"
...
def test_marker_publish_leaves_no_temp_sibling(tmp_path):
    ...
    assert (install_dir / ".work4you-bootstrap-complete").is_file()
    assert not (install_dir / ".work4you-bootstrap-complete.tmp").exists()
...
def test_no_marker_written_when_head_cannot_be_resolved(tmp_path):
    ...
    assert not (install_dir / ".work4you-bootstrap-complete").exists()
