...
(/opt/work4you/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable WORK4YOU_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_work4you_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable WORK4YOU_HOME."""
    ...
    monkeypatch.setattr(
        "work4you_constants.get_work4you_home", lambda: work4you_home
    )
    ...
    expected = work4you_home / "scripts" / "whatsapp-bridge"
