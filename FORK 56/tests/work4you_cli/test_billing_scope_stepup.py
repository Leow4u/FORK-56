...
from work4you_cli.auth import (
    WORK4YOU_BILLING_MANAGE_SCOPE,
    work4you_token_has_billing_scope,
    step_up_work4you_billing_scope,
)
...
def _stub_persist(monkeypatch):
    ...
    monkeypatch.setattr(auth, "_write_shared_work4you_state", lambda *a, **kw: None)
    monkeypatch.setattr(auth, "_sync_work4you_pool_from_auth_store", lambda: None)
...
def test_step_up_requests_billing_scope_and_reuses_prior_urls(monkeypatch, _stub_persist):
    ...
    monkeypatch.setattr(auth, "_work4you_device_code_login", _fake_login)

    granted = step_up_work4you_billing_scope()
    assert granted is True
    # Requested scope must include billing:manage, preserving prior scopes.
    assert WORK4YOU_BILLING_MANAGE_SCOPE in captured["scope"].split()
    ...
    assert captured["client_id"] == "work4you-cli"
...
def test_device_login_fires_on_verification_before_polling(monkeypatch):
    ...
    try:
        auth._work4you_device_code_login(open_browser=False, on_verification=_cb)
    except Exception:
        pass
