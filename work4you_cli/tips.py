...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or work4you -c.",
    ...
    "Ctrl+Z suspends Work4You to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "work4you -c resumes your most recent CLI session. work4you -c \"project name\" resumes by title.",
    "work4you -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "work4you -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "work4you chat -t web,terminal enables only specific toolsets for a focused session.",
    "work4you chat -s github-pr-workflow preloads a skill at launch.",
    "work4you chat -q \"query\" runs a single non-interactive query and exits.",
    "work4you chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "work4you chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "work4you --yolo bypasses all dangerous command approval prompts for the entire session.",
    "work4you chat --source telegram tags the session for filtering in work4you sessions list.",
    "work4you -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "work4you doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "work4you dump outputs a compact setup summary — great for bug reports.",
    "work4you config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "work4you config edit opens config.yaml in your default editor.",
    "work4you config check scans for missing or stale configuration options.",
    "work4you sessions browse opens an interactive session picker with search.",
    "work4you sessions stats shows session counts by platform and database size.",
    "work4you sessions prune --older-than 30 cleans up old sessions.",
    "work4you skills search react --source skills-sh searches the skills.sh public directory.",
    "work4you skills check scans installed hub skills for upstream updates.",
    "work4you skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "work4you skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "work4you mcp add github --command npx adds MCP servers from the command line.",
    "work4you mcp serve runs Work4You itself as an MCP server for other agents.",
    "work4you auth add lets you add multiple API keys for credential pool rotation.",
    "work4you completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "work4you logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "work4you backup creates a zip backup of your entire Work4You home directory.",
    "work4you profile create coder creates an isolated profile that becomes its own command.",
    "work4you profile create work --clone copies your current config and keys to a new profile.",
    "work4you update syncs new bundled skills to ALL profiles automatically.",
    "work4you gateway install sets up Work4You as a system service (systemd/launchd).",
    "work4you memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "work4you webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: work4you tools disables unused tools, work4you skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "work4you models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as work4you -w).",
    ...
    "WORK4YOU_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Work4You tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'work4you profile create coder' creates the 'coder' command.",
    "work4you profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Work4You prints a resume command with session ID and stats.",
    "work4you sessions export backup.jsonl exports all sessions for backup or analysis.",
    "work4you -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "The plan skill saves markdown plans under .work4you/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: work4you cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Work4You runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "work4you gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Work4You via a local macOS server.",
    ...
    "Context files (.work4you.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Work4You loads project context from .work4you.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "work4you mcp opens an interactive picker of Work4You-approved MCPs you can install in one keystroke.",
    "work4you mcp catalog lists Work4You-approved MCP servers shipped with the repo.",
    "work4you mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — work4you tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.work4you/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "work4you chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "work4you plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "work4you acp runs Work4You as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "WORK4YOU_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "work4you auth add work4you or work4you auth add openai-codex sets up OAuth-based providers.",
    ...
    "work4you status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.work4you/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Work4You your own.",
    ...
    "Before a gateway session reset, Work4You auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.work4you/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Work4You state via WORK4YOU_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "WORK4YOU_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in WORK4YOU_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.work4you/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'work4you profile create ls' would be rejected.",
    "work4you profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "work4you claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "WORK4YOU_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via work4you tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.work4you/.env.",
    "Skills from trusted repos (Work4You) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.work4you/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Work4You auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Work4You config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Work4You is working.',
    ...
    '/reload reloads ~/.work4you/.env into the running session — pick up new API keys without restarting.',
    ...
    "WORK4YOU_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.work4you/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.work4you/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'work4you curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "work4you curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'work4you curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'work4you auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Work4You subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'WORK4YOU_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "WORK4YOU_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'work4you dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'work4you dashboard embeds the full Work4You TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.work4you/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.work4you/dashboard-plugins/ — no npm build required.',
    ...
    'WORK4YOU_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'WORK4YOU_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'WORK4YOU_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'WORK4YOU_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.work4you/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.work4you/skills/ so drop-in skills appear without restarting the session.',
    ...
    'work4you -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'work4you chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'work4you chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'work4you chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "work4you chat --source tool tags programmatic chats so they don't clutter work4you sessions list.",
    'work4you dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'work4you sessions rename <ID> "new title" renames any past session; work4you sessions delete <ID> removes one.',
    'work4you import restores a full Work4You backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'work4you fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'work4you pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'work4you setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'work4you status --deep runs the full health sweep across every component; plain work4you status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'WORK4YOU_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'WORK4YOU_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.work4you/plugins/ — trust-gated by design.',
    "WORK4YOU_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'WORK4YOU_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'WORK4YOU_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'WORK4YOU_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'WORK4YOU_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'WORK4YOU_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'WORK4YOU_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'WORK4YOU_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'WORK4YOU_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'WORK4YOU_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'WORK4YOU_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Work4You block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from work4you sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.work4you/dashboard-plugins/.',
]
