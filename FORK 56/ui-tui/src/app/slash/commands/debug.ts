...
import { terminalBackgroundHex } from '@work4you/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $WORK4YOU_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see WORK4YOU_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['WORK4YOU_TUI_BACKGROUND', process.env.WORK4YOU_TUI_BACKGROUND ?? '(unset)'],
            ['WORK4YOU_TUI_THEME', process.env.WORK4YOU_TUI_THEME ?? '(unset)'],
