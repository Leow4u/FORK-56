/**
 * windows-work4you-path.ts
 *
 * Pure, dependency-injected pieces of Windows `work4you` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvWork4YouCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `work4you` shim shadowed the real
 *      work4you.cmd/work4you.exe; the shim then failed the --version probe and
 ...
 *      and Work4You launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvWork4YouCommand() — unwrapWindowsVenvWork4YouCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `work4you`) must
 * not shadow `work4you.cmd`/`work4you.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Work4You launcher that drives `work4you update`.
...
export interface ResolveVenvWork4YouCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportWork4YouCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `work4you`/`work4you.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/work4you(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m work4you_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportWork4YouCli(). A
 * venv whose update died mid-`pip install` still has python.exe + work4you.exe
 ...
 *
 * Returns null when `command` is not a venv work4you shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvWork4YouCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvWork4YouCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^work4you(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Work4You at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Work4You Python at ${python}`,
    command: python,
