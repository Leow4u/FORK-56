...
``kind: backend``) or ``~/.work4you/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`work4you_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.work4you/.env``) so credentials set
    through Work4You' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``work4you tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``work4you tools`` paint.
        """
