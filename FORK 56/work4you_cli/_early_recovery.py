"""Dependency-light venv recovery that runs BEFORE work4you_cli.main's imports.

The ``work4you`` console entry point is ``work4you_cli.main:main``.  Importing
``work4you_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``work4you_cli.env_loader``, ``yaml`` via ``work4you_cli.config``, ...).  In
...
a corrupted venv.  ``work4you_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``work4you_cli.main`` to become
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Work4You already vendors
    (``~/.work4you/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".work4you" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``work4you acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``work4you_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``work4you_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``work4you update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `work4you update`: CLI and Desktop retries preserve
...
# every `work4you` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``work4you update`` (or the self-lock preflight, #83569) left the
    ...
    ``work4you_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``work4you acp`` included).
    ...
    """
    try:
        from work4you_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; work4you will keep working from "
