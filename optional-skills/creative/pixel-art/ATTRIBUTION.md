...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Work4You uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Work4You has `image_generate`
...
- License: MIT (inherits from work4you repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Work4You contributors
