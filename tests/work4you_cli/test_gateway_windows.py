"""Tests for work4you_cli.gateway_windows."""
...
import work4you_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda work4you_home: "")
    monkeypatch.setattr("work4you_cli.config.get_work4you_home", lambda: str(work4you_home))
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "work4you_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda: (argv, cwd, {"WORK4YOU_GATEWAY_DETACHED": "1"}),
    )
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            argv,
            cwd,
            {"WORK4YOU_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("work4you_cli.config.get_work4you_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_work4you_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".work4you"
        home.mkdir()
        monkeypatch.setattr("work4you_cli.config.get_work4you_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".work4you"
        project = tmp_path / "checkout"
        monkeypatch.setattr("work4you_cli.config.get_work4you_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Work4You_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Work4You_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Work4You_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Work4You\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Work4You\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Work4You_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Work4You_Gateway_alice")
    ...
    assert "Work4You_Gateway_alice.vbs" in xml_seen["text"]
...
# resume_pending=True) never fires when `work4you gateway stop` kills the
