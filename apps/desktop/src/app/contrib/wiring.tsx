...
import { getLatestSessionMessages } from '@/work4you'
...
import { useWork4YouConfig } from '../session/hooks/use-work4you-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshWork4YouConfig, sttEnabled, voiceMaxRecordingSeconds } = useWork4YouConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('work4you:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('work4you:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshWork4YouConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshWork4YouConfig])
  ...
  // The REAL titlebar tool clusters (sidebar/flip toggles, HUD, right sidebar)
  ...
  // Count every button the static cluster actually renders: HUD and the
  // always-present right-sidebar toggle (see titlebar-controls.tsx).
  // A shared width that under-counts leaves the find bar, the titlebar
  // header padding, and the pane-cluster anchor overlapping those buttons.
