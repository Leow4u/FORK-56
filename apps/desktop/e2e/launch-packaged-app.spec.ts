...
/**
 * E2E smoke tests for the packaged Work4You desktop app.
 *
 ...
 * (credential stripping, isolated WORK4YOU_HOME + userData, unique app name).
 *
...
test('window opens with the Work4You title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      work4youDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).work4youDesktop?.hud?.open({ sessionId: null })
  )
