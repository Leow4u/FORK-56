"""``work4you approvals suggest`` — mine approval history into allowlist proposals.

Work4You has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.work4you/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, work4you self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/work4you home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_work4you_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_work4you_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  work4you approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.work4you/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``work4you approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``work4you approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from work4you_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: work4you approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `work4you approvals <subcommand> -h` for details."
    )
    return 1
