"""Unified provider-credential lifecycle across every store Work4You reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.work4you/.env``                     — the canonical secret store
    2. ``~/.work4you/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.work4you/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `work4you auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``work4you auth add``.
    """
    from work4you_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from work4you_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from work4you_cli.config import load_env, remove_env_value
