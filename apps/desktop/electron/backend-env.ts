...
/**
 * Work4You-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\work4you\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$WORK4YOU_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_work4you_node_dirs()` in work4you_constants.py, which the Electron main
 * process cannot import.
 */
function work4youManagedNodePathEntries(
  work4youHome,
  ...
  const root = pathModule.join(work4youHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const work4youNodeDirs = work4youManagedNodePathEntries(work4youHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([work4youNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeWork4YouHomeRoot(work4youHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!work4youHome) {
    return work4youHome
  }

  const resolved = pathModule.resolve(String(work4youHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). work4you_bootstrap sets this inside the child too, but only
