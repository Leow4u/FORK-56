...
describe('ToolsetConfigPanel', () => {
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out Work4You Subscription row and a
  ...
  describe('managed Work4You provider activation', () => {
    const work4youBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
    ...
    it('surfaces a sign-in notice when the PUT reports needs_work4you_auth', async () => {
      ...
      // reports needs_work4you_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(work4youBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'Work4You Subscription (Browser Use cloud)',
        needs_work4you_auth: true,
        feature: 'browser'
      })
      ...
      // The single Work4You row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /Work4You Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'Work4You Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing Work4You OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        verification_url: 'https://portal.work4you.ai/device?user_code=WORK4YOU-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('work4you'))
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('work4you', 'sess-1'), { timeout: 8000 })
