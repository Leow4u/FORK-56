...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: Work4YouConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('WORK4YOU_API_KEY')).toBe('Work4You Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // WORK4YOU_QWEN_ shares the WORK4YOU_ stem with other integrations.
      expect(providerGroup('WORK4YOU_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: Work4YouConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: Work4YouConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: Work4YouConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: Work4YouConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: Work4YouConfigRecord = { toolsets: ['memory'] }
      const next: Work4YouConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: Work4YouConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: Work4YouConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: Work4YouConfigRecord = { toolsets: [] }
      const next: Work4YouConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: Work4YouConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: Work4YouConfigRecord = { model: 'b', toolsets: ['memory'] }
