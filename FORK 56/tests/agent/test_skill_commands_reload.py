...
def work4you_home(monkeypatch):
    """Isolate WORK4YOU_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``WORK4YOU_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="work4you-reload-skills-")
    monkeypatch.setenv("WORK4YOU_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "WORK4YOU_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, work4you_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(work4you_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, work4you_home):
