...
function buildManageUrl(s: SubscriptionStateResponse, tierId?: string): string | null {
  ...
  // on the Python side (e.g. https://portal.work4you.ai/billing). Strip any
...
export const subscriptionCommands: SlashCommand[] = [
  {
    help: 'View or change your Work4You subscription plan',
    name: 'subscription',
    aliases: ['upgrade'],
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<SubscriptionStateResponse>(s => {
            if (!s.logged_in) {
              sys('Not logged into Work4You Portal — run /portal to log in, then /subscription.')
