#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `work4you kanban list` and
...
def work4you_available() -> bool:
    return shutil.which("work4you") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed work4you CLI."""
    try:
        out = subprocess.run(
            ["work4you", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `work4you kanban list`
    out = subprocess.run(
        ["work4you", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["work4you", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not work4you_available():
        print("ERROR: 'work4you' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
