...
Before the fix, ``work4you whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``work4you gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    work4you = home / ".work4you"
    work4you.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you))
...
def _env_value(work4you_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from work4you_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("work4you_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``work4you whatsapp`` with an existing paired session and
