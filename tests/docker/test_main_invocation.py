...
def test_no_args_starts_work4you(built_image: str) -> None:
    """``docker run <image>`` should start work4you cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``work4you chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> work4you batch ...`` returns a
