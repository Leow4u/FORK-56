...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._work4you_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-Work4You-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host WORK4YOU_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("work4you_cli.config.load_config", return_value={}), patch("work4you_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
    ...
    def test_prompt_caching_disabled_by_falsy_cache_ttl(self, falsy_value):
        ...
        with (
            ...
            patch(
                "work4you_cli.config.load_config_readonly",
                return_value={"prompt_caching": {"cache_ttl": falsy_value}},
            ),
        ):
            a = AIAgent(
...
class TestBuildSystemPrompt:
    ...
    def test_includes_work4you_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_work4you_subscription_prompt", lambda tool_names: "WORK4YOU SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "WORK4YOU SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("work4you_cli.lifecycle.has_hook", lambda name: True)
    ...
    def test_interrupt_skips_remaining(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.lifecycle.has_hook", lambda name: True)
        monkeypatch.setattr(
            "work4you_cli.lifecycle.invoke_hook",
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from work4you_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "work4you_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "work4you_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "work4you_cli.plugins._dispatch_pre_tool_call_hooks",
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "Work4You tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["Work4You tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because work4you' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "work4you_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "work4you_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Work4You tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "work4you_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("work4you_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("work4you_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("work4you_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_work4you_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "work4you"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_work4you_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like work4you-lcm can
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_KANBAN_TASK", "t_test_task_123")
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when WORK4YOU_KANBAN_TASK
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Work4You Portal fronting Anthropic) returns ``message.refusal``
...
class TestWork4YouCredentialRefresh:
    """Verify Work4You credential refresh rebuilds the runtime client."""

    def test_try_refresh_work4you_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "work4you_cli.auth.resolve_work4you_runtime_credentials", _fake_resolve
        )
        ...
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_work4you_client_credentials(force=True)
        ...
        assert rebuilt["kwargs"]["api_key"] == "new-work4you-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.work4you.ai/v1"
        )
        assert "default_headers" not in rebuilt["kwargs"]
        assert isinstance(agent.client, _RebuiltClient)

    def test_try_refresh_work4you_client_credentials_rebuilds_anthropic_client(
        self, agent, monkeypatch
    ):
        ...
        agent.api_key = "stale-work4you-key"
        agent.base_url = "https://inference-api.work4you.ai/v1"
        agent._anthropic_api_key = "stale-work4you-key"
        agent._anthropic_base_url = "https://inference-api.work4you.ai/v1"
        ...
        assert agent.base_url == "https://inference-api.work4you.ai/v1"
        assert agent._anthropic_api_key == "fresh-portal-jwt"
        assert agent._anthropic_base_url == (
            "https://inference-api.work4you.ai/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_work4you_gpt5_stays_on_chat_completions(self, agent):
        """Work4You serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "work4you"
        agent.base_url = "https://inference-api.work4you.ai/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Work4You" in agent._cached_system_prompt
