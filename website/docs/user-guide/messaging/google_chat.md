...
sidebar_position: 12
title: "Google Chat"
description: "Set up Work4You as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Work4You to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Work4You
...
> Run `work4you gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Work4You can read (e.g.,
`~/.work4you/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a work4you restart)
...
- Principal: `work4you-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Work4You calls
...
- **App name**: whatever you want users to see ("Work4You" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/work4you-chat-events`.
...
`ADDED_TO_SPACE` event that Work4You uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Work4You

Add the Google Chat section to `~/.work4you/.env`:
...
Send "hola" in the test DM. The bot posts a "Work4You is thinking…" marker, then
...
`~/.work4you/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Work4You is thinking…").
...
Thread support: when a user replies inside a thread, Work4You detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Work4You session.
...
3. Download the JSON. Move it onto the host that runs Work4You.
4. Register the client with Work4You (run under the profile you want it scoped to):
...
work4you -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Work4You home (e.g.
`~/.work4you/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.work4you/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.work4you/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Work4You isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `work4you gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Work4You will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.work4you/google_chat_user_tokens/<sanitized_email>.json` (filesystem
