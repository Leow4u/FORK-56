...
def _root() -> Path:
    ...
    # helper's token, including custom WORK4YOU_HOME layouts.
    return get_default_work4you_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, work4you_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(work4you_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the work4you exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs work4you_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "work4you_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "work4you_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, work4you_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, work4you_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves work4you_cli's own location and prepends its parent, because the
    ...
    # the work4you_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the work4you_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["work4youPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Work4You path must be absolute")
    work4you_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(work4you_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Work4You Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('work4you_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_work4you(work4you_path: str) -> dict[str, Any]:
    path = os.path.abspath(work4you_path)
    if not os.path.isabs(work4you_path) or not os.path.isfile(path):
        raise ValueError("Work4You path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "work4youHome": str(get_default_work4you_root()), "python": sys.executable}
    ...
    if operation == "inspect" and len(argv) == 2:
        return inspect_work4you(argv[1])
