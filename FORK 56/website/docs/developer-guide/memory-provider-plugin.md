...
description: "How to build a memory provider plugin for Work4You"
---

# Building a Memory Provider Plugin

Memory provider plugins give Work4You persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `work4you plugins`.
:::

## Installation Layouts

Work4You discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Work4You. Closed to new providers — see [CONTRIBUTING](https://github.com/Leow4u/FORK-56/blob/main/CONTRIBUTING.md). |
| User | `$WORK4YOU_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.work4you/plugins/<name>/` | Opt-in via `WORK4YOU_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `work4you.memory_providers` entry point | `pip install`, nothing to copy. |
...
Work4You, in `$WORK4YOU_HOME/plugins/<name>/` when installed by a user, or in
`./.work4you/plugins/<name>/` for a project-local one:
...
`work4you.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."work4you.memory_providers"]
...
package layout. No copy under `$WORK4YOU_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Work4You reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `work4you <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          work4you_home (str): Active WORK4YOU_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `work4you memory setup` | **Yes** |
| `save_config(values, work4you_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `work4you memory setup`:
...
Every field in `get_config_schema()` is prompted during `work4you memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$WORK4YOU_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, work4you_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(work4you_home) / "my-provider.json"
...
the `messages` parameter; Work4You will continue calling them with the legacy
signature.
...
All storage paths **must** use the `work4you_home` kwarg from `initialize()`, not hardcoded `~/.work4you`:

```python
# CORRECT — profile-scoped
from work4you_constants import get_work4you_home
data_dir = get_work4you_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.work4you/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `work4you my-provider status`, `work4you my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `work4you <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `work4you --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: work4you my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the work4you my-provider argparse tree.
