"""Generic managed-tool gateway helpers for Work4You-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "work4you.ai"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_WORK4YOU_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    work4you_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the Work4You auth store path, respecting WORK4YOU_HOME overrides."""
    return get_work4you_home() / "auth.json"


def _read_work4you_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        work4you_provider = providers.get("work4you", {})
        if isinstance(work4you_provider, dict):
            return work4you_provider
    except Exception:
        pass
    return None
...
def peek_work4you_access_token() -> Optional[str]:
    """Cheap probe for a Work4You gateway token without triggering refresh.

    Availability scans (`work4you tools`, banner/status paint, provider
    ...
    that call :func:`read_work4you_access_token`.
    """
    explicit = _read_user_token_override()
    if explicit:
        return explicit

    work4you_provider = _read_work4you_provider_state() or {}
    access_token = work4you_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_work4you_access_token() -> Optional[str]:
    """Read a Work4You Subscriber OAuth access token from auth store or env override."""
    explicit = _read_user_token_override()
    if explicit:
        return explicit
    work4you_provider = _read_work4you_provider_state() or {}
    cached_token = peek_work4you_access_token()

    if cached_token and not _access_token_is_expiring(
        work4you_provider.get("expires_at"),
        _WORK4YOU_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token
    ...
    try:
        ...
        refreshed_token = resolve_work4you_access_token(
            refresh_skew_seconds=_WORK4YOU_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Work4You access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_work4you_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    work4you_user_token = resolved_token_reader()
    if not gateway_origin or not work4you_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        work4you_user_token=work4you_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Work4You token are present.

    Defaults to :func:`peek_work4you_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_work4you_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_work4you_access_token,
    ) is not None
...
# managed vendor's gateway URL is pinned: adding one is a Work4You release, and
...
#
# The gateway exposes a Work4You-owned REST contract per vendor; it names the
...
def is_managed_work4you_gateway_url(
    url: object,
    gateway_builder: Optional[Callable[[str], str]] = None,
) -> bool:
    """True when ``url`` is on the Work4You tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Work4You access token expires
    ...
    """
    if not is_managed_work4you_gateway_url(url, gateway_builder):
        return {}
...
# the tool argument carries an opaque `work4you-upload:<token>` reference instead.
#
# The protocol lives HERE rather than in a vendor tool module: the presign
# request shape, the response contract, and the `work4you-upload:` scheme are Work4You
...
def build_managed_media_uploader(
    ...
    Work4You URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``work4you-upload:<token>`` for the tool argument. The token is
       bound to this Work4You principal and is redeemable only through the
       gateway, so it is inert anywhere else it might end up.
    """
    if not is_managed_work4you_gateway_url(server_url, gateway_builder):
        return None
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Work4You credential is available for the upload")
        ...
        # is_managed_work4you_gateway_url) plus the pinned upload_path — the same
        ...
        return f"work4you-upload:{token}"
