...
minimal ``Work4YouCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def work4you_home(tmp_path, monkeypatch):
    """Isolated WORK4YOU_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".work4you"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("WORK4YOU_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves WORK4YOU_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal Work4YouCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, work4you_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, work4you_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, work4you_home):
        ...
        src = inspect.getsource(Work4YouCLI.chat)
