...
// registry, and the work4you:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // WORK4YOU_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.WORK4YOU_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Work4You' TUI into
    ...
    env.TERM_PROGRAM = 'Work4You'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a work4you/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from WORK4YOU_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.WORK4YOU_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `work4you:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('work4you:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('work4you:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('work4you:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('work4you:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('work4you:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
