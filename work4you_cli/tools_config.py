...
`work4you tools` and `work4you setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.work4you/config.yaml under
...
from work4you_cli.work4you_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    Work4YouSubscriptionFeatures,
    apply_work4you_managed_defaults,
    get_work4you_subscription_features,
)
from work4you_cli.work4you_account import format_work4you_portal_entitlement_message
from tools.tool_backend_helpers import WORK4YOU_MANAGED_PROVIDER, fal_key_is_configured
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``work4you tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`work4you_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from work4you_cli.cli_output import (  # noqa: E402 — late import block
...
# Platform-scoped toolsets: only appear in the `work4you tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``work4you tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from work4you_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from work4you_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``work4you tools`` checklist actually offers
    for ``platform``.
    ...
    so ``work4you tools`` never claims to add or remove a toolset the user was
...
from work4you_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Work4You Subscription" — managed Firecrawl billed via Work4You
        #     subscription (requires_work4you_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Work4You Subscription" — managed FAL billed via the Work4You
        #     subscription (requires_work4you_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_work4you_auth": True,
                "managed_work4you_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Work4You Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Work4You Portal.  Plugin-backed
    ...
    "x_search": {
        ...
        "setup_note": (
            "Work4You routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Work4You Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Work4You Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Work4You subscription (requires_work4you_auth +
        ...
        "providers": [
            ...
            {
                "name": "Work4You Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_work4you_auth": True,
                "managed_work4you_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set WORK4YOU_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "WORK4YOU_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "WORK4YOU_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "WORK4YOU_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "WORK4YOU_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "WORK4YOU_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `work4you model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("WORK4YOU_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``WORK4YOU_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Work4You telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Work4You' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $WORK4YOU_HOME/bin is never on PATH, so a bare which()
    # misses the uv Work4You installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``work4you update`` and
      by ``work4you computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``work4you update`` sets
    ...
    a further ~600s wait on Windows). ``work4you computer-use install
    ...
    ``work4you update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `work4you update` calls this
    ...
    override = os.environ.get("WORK4YOU_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    WORK4YOU_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Work4You' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Work4You' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `work4you update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Work4You cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("WORK4YOU_CUA_DRIVER_CMD", "").strip():
            _print_info(
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `work4you computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `work4you update` (require_confirmed_update=True) keeps the
    ...
    # `work4you computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: work4you computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: work4you computer-use doctor")
            return False
    if ok and before:
        try:
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `work4you update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `work4you update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.work4you/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Work4You process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Work4You-managed cloud rows all execute through
    ...
    check — only the Work4You-managed ``$WORK4YOU_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Work4You installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Work4You-managed install)
        ...
        # (PATH -> Homebrew/Work4You-managed node -> npx) rather than a bare
        # shutil.which — Work4You-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/work4you/work4you:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Work4You-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Work4You-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.work4you/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.work4you/config.yaml")
        ...
        # Run the full `work4you auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.work4you/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from work4you_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: work4you auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `work4you auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: work4you auth spotify")
        except Exception as exc:
        ...
        try:
            from work4you_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: work4you plugins enable observability/langfuse")
        _print_info("    Restart Work4You for tracing to take effect.")
        _print_info("    Verify: work4you plugins list")
        ...
        try:
            from work4you_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: work4you auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `work4you auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: work4you auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: work4you auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``work4you tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``work4you tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: work4you tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``work4you-discord``, which contains those
...
#:
#: Saving ``work4you tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[work4you-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``work4you tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"work4you-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (work4you-acp, work4you-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``work4you-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"work4you-{platform}"
    ...
    # explicit config like ``[work4you-cli, a2a]`` must survive filtering just
    ...
    # "work4you-cli" (which include all _WORK4YOU_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[work4you-cli, spotify]`` after enabling Spotify via ``work4you
        ...
        # (e.g. "work4you-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    # otherwise saving via `work4you tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"work4you-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("work4you-")}
    ...
    # shipped as a bundled plugin but user must opt in via `work4you tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `work4you tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by work4you tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `work4you config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `work4you` instead of
    # `work4you-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `work4you update`/`work4you doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `work4you tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (work4you-cli, work4you-telegram, etc.)
    ...
    # Opening `work4you tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[Work4YouSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_work4you_subscription_features(
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from work4you_constants import work4you_home_key

    scope = work4you_home_key()
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from work4you_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
# ("Work4You Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Work4You Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Work4You-managed Tool Gateway rows (``managed_work4you_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Work4You Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_work4you_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Work4You-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_work4you_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_work4you_auth")
            and not provider.get("managed_work4you_feature")
            and not features.work4you_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_work4you_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Work4You Subscription.
    ...
    # remaining hardcoded rows ("Work4You Subscription", "Firecrawl
    ...
    # the hardcoded "Work4You Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_work4you_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Work4You Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Work4You-managed rows were filtered out for unentitled
    ...
    (``ensure_work4you_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `work4you tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``work4you tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``work4you tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from work4you_cli.work4you_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``work4you tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from work4you_cli.work4you_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Work4You Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Work4You Subscription rows).

    ``features`` (a ``Work4YouSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_work4you_feature")
    if provider.get("requires_work4you_auth") or managed_feature:
        if features is None:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_work4you_message = _hidden_work4you_gateway_message(
        cat,
        config,
        f"the Work4You Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_work4you_message:
            for line in hidden_work4you_message.splitlines():
        ...
        # When the user is logged into Work4You, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Work4You.
        try:
            _work4you_logged_in = bool(
                get_work4you_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).work4you_auth_present
            )
        except Exception:
            _work4you_logged_in = False
        ...
        for p in providers:
            ...
            # Mark Work4You-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Work4You Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_work4you_feature"):
                if _work4you_logged_in:
                    sub_marker = "  ★ Included with your Work4You subscription"
                else:
                    sub_marker = "  ★ via Work4You Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_work4you_feature"):
        # Managed (Work4You-subscription) entries fall through to the
    ...
    if video_plugin_name and not provider.get("managed_work4you_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if configured_provider not in {None, "", "fal", WORK4YOU_MANAGED_PROVIDER}:
                    return False
                if (
                    configured_provider != WORK4YOU_MANAGED_PROVIDER
        ...
        if provider.get("tts_provider"):
            return (
                ...
                in {provider["tts_provider"], WORK4YOU_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_nous
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], WORK4YOU_MANAGED_PROVIDER}
            )
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Work4You-managed
    flow: the stored selection becomes ``image_gen.provider: work4you`` (the
    ...
    img_cfg["provider"] = WORK4YOU_MANAGED_PROVIDER if use_gateway else plugin_name
...
# Kept in sync with the dashboard selects (work4you_cli/web_server.py
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: work4you``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = WORK4YOU_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Work4You auth, or run interactive model pickers.
    ...
    category. Managed "Work4You Subscription" rows write ``work4you``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ work4you) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Work4You Subscription" rows carry only
    # managed_work4you_feature) still persist the "work4you" selection.
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = WORK4YOU_MANAGED_PROVIDER
            section.pop("use_gateway", None)
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Work4You Portal auth, or run interactive model
    ...
    # flow). Managed picks store the "work4you" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    # Work4You-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Work4You
    ...
    # "enable all tools" prompt (that lives in `work4you model`).
    if managed_feature:
        from work4you_cli.work4you_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_work4you_portal_access,
        )

        if not ensure_work4you_portal_access(
            capability=f"{provider.get('name', 'the Work4You Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Work4You Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_work4you_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_work4you_auth") and not managed_feature:
        features = get_work4you_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.work4you_auth_present or not entitled:
            message = format_work4you_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Work4You Subscription')}",
            )
            _print_warning(
                f"  {message or 'Work4You Subscription is only available after logging into Work4You Portal.'}"
            )
            return
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Work4You subscription.")
        ...
        if backend:
            ...
            # explicit selection: "work4you" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    WORK4YOU_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
    ...
    # Work4You-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Work4You.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_work4you_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_work4you_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_work4you_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.work4you_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Work4You Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``work4you model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``work4you model`` aux-task
    ...
    """
    try:
        from work4you_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `work4you model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Work4You Portal login + entitlement gate as _configure_provider:
    ...
    # provider string per category — "work4you" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ work4you) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = WORK4YOU_MANAGED_PROVIDER
            _print_success(f"  Browser cloud provider set to: {bp or 'work4you'}")
        elif bp == "local":
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            WORK4YOU_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (Work4You Subscription) row also carries
                    # imagegen_backend="fal" — store the "work4you" selection
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a Work4You-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    WORK4YOU_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `work4you model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `work4you tools` and `work4you setup tools`.
    ...
    print(color("  Guide: https://work4you.ai/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_work4you_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Work4You subscription defaults", Colors.GREEN))
    ...
    print(color(f"  Tool configuration saved to {display_work4you_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'work4you' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, work4you mcp
