...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$WORK4YOU_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   Work4You-owned caches (images, audio, documents, ...) instead of littering
...
STORAGE_DIR = "/tmp/work4you-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "WORK4YOU_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $WORK4YOU_HOME/cache/spillover as a Path (not created)."""
    from work4you_constants import get_work4you_home

    return get_work4you_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $WORK4YOU_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/work4you-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"WORK4YOU_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $WORK4YOU_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other Work4You-owned
