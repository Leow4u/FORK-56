...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Work4You CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Work4You CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.work4you/skins/mytheme.yaml
```

Or set the default skin in `~/.work4you/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Work4You — gold and kawaii | `Work4You` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar hero banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Work4You` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Work4You` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Work4You` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Work4You` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
Create YAML files under `~/.work4you/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.work4you/skins/mytheme.yaml
...
## Work4You Mod — Visual Skin Editor

[Work4You Mod](https://github.com/cocktailpeanut/work4you-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Work4You Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/work4you-mod/master/work4you.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Work4You skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.work4you/skins/`
- Activates a skin by updating `~/.work4you/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/work4you-mod.git
cd work4you-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.work4you/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Work4You Mod respects the `WORK4YOU_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `work4you_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.work4you/skins/` take precedence over built-in skins with the same name.
