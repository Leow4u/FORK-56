...
Supersedes: PR #31781 (prompt_toolkit `work4you profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `work4you profile wizard` in the terminal.
...
needs, and a profile is just a WORK4YOU_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.work4you/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — WORK4YOU_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with WORK4YOU_HOME override |
...
### Seam #1 — hub-skill install cannot use the WORK4YOU_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = WORK4YOU_HOME / "skills"` at **module
import time**. The context-local `set_work4you_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_work4you_action`
runs `python -m work4you_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_work4you_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's WORK4YOU_HOME bound
...
spawned detached (`_spawn_work4you_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, WORK4YOU_HOME override)
3. write selected MCP servers (synchronous, WORK4YOU_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `work4you -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, WORK4YOU_HOME override
...
page afterward. Mirror `_write_profile_model`'s WORK4YOU_HOME-override helper for
...
- Backend E2E with isolated WORK4YOU_HOME: POST a full create body
