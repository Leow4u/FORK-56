"""``work4you debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    work4you debug share              Upload debug report (asks for confirmation)
    work4you debug share --yes        Skip confirmation (for scripts/CI)
    work4you debug share --lines 500  Include more log lines
    work4you debug share --expire 30  Keep paste for 30 days
    work4you debug share --local      Print report locally (no upload)
    work4you debug share --no-redact  Disable upload-time secret redaction
    work4you debug share --work4you       Upload to Work4You-internal storage (private)
    work4you debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Work4You-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Work4You staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'work4you debug share'",
    )
