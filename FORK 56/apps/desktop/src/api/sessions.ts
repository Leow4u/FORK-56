...
import { work4youApi } from './client'
...
export async function listSessions(
  ...
  const result = await work4youApi<PaginatedSessions>({
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await work4youApi<SidebarSessionsResponse>({
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return work4youApi<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return work4youApi<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(
  ...
  return work4youApi<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return work4youApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
