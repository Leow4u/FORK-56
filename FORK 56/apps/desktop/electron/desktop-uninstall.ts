 ...
 * the three user-facing uninstall modes to the `work4you uninstall` CLI flags,
 ...
 *              `work4you uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `work4you uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `work4you uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `work4you` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m work4you_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `work4you_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'work4you_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Work4You.app/Contents/MacOS/Work4You  → …/Work4You.app
 *   Windows: …\Work4You\Work4You.exe                 → …\Work4You  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Work4You.app/Contents/MacOS/Work4You → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Work4You.app
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/work4you
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import work4you_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, work4youHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export WORK4YOU_HOME=${q(work4youHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import work4you_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Work4You install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "WORK4YOU_HOME=${String(work4youHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
