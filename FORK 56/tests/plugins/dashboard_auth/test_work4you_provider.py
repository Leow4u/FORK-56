"""Tests for the bundled Work4You dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.work4you/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.work4you as work4you_plugin
...
def _patched_jwks(provider: work4you_plugin.Work4YouDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(work4you_plugin.Work4YouDashboardAuthProvider)

    def test_name_and_display(self):
        p = work4you_plugin.Work4YouDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "work4you"
        assert p.display_name == "Work4You"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            work4you_plugin.Work4YouDashboardAuthProvider(
                client_id="work4you-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("WORK4YOU_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        work4you_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID" in work4you_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: WORK4YOU_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Work4You Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("WORK4YOU_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        work4you_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, work4you_plugin.Work4YouDashboardAuthProvider)
        assert registered._portal_url == "https://portal.work4you.ai"
        # Skip reason cleared on successful registration.
        assert work4you_plugin.LAST_SKIP_REASON == ""
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("WORK4YOU_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        work4you_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.work4you.ai"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID``
    and ``WORK4YOU_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``work4you_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "work4you_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        work4you_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in work4you_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {work4you_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return work4you_plugin.Work4YouDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://work4you.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://work4you.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "work4you_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["work4you_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["work4you_session_pkce"] != b.cookie_payload[
            "work4you_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://work4you.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.work4you.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "work4you"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.work4you.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=work4you_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between WORK4YOU_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.work4you"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.work4you.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-work4you-refresh-token"] == "rt_old_value"
