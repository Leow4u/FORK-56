"""``work4you project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$WORK4YOU_HOME/projects.db`` store
(see :mod:`work4you_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Work4You'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``work4you project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: work4you project <action> [options]\n"
                "Run 'work4you project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `work4you project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `work4you project add-folder`.",
            file=sys.stderr,
        )
        return 1
