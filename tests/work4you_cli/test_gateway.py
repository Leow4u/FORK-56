"""Tests for work4you_cli.gateway."""
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /work4you-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``WORK4YOU_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``WORK4YOU_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(*, replace, verbosity):
            assert "_WORK4YOU_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["WORK4YOU_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "WORK4YOU_HOME": str(tmp_path),
            "WORK4YOU_GATEWAY_DETACHED": "1",
            "WORK4YOU_GATEWAY_EXIT_DIAG": "1",
            "WORK4YOU_GATEWAY_MAX_STARTS": "0",
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["WORK4YOU_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "WORK4YOU_HOME": str(tmp_path),
        "WORK4YOU_GATEWAY_EXIT_DIAG": "0",
        "WORK4YOU_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("WORK4YOU_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("work4you_constants.is_container", lambda: True)
    monkeypatch.setattr("work4you_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("work4you_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_work4you_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test WORK4YOU_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="WORK4YOU_HOME=/home/alice/.work4you"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_work4you_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_work4you_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "work4you-gateway.service"
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Work4You Desktop opens (``work4you serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Work4You Desktop opens
    (``work4you serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `work4you gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "work4you-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "work4you-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "work4you-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "work4you-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "work4you_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """Work4You_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "Work4YouGateway" would leave the guard dormant on every
        # standard install (task name is Work4You_Gateway / Work4You_Gateway_<p>).
        import work4you_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "Work4You_Gateway_testprof"
        )
        queried = []
        ...
        assert killed_pids == []
        assert queried == ["Work4You_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(self, monkeypatch):
        ...
        assert gateway._windows_scheduled_task_running("Work4YouGateway") is False
        assert gateway._windows_scheduled_task_supervises("Work4YouGateway") is False
        assert gateway._windows_scheduled_task_state("Work4YouGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("Work4You_Gateway") is expected, state
            assert gateway._windows_scheduled_task_running("Work4You_Gateway") is (state == "Running")
