...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``work4you tools`` repaint, tool-registration scans,
    ...
    2. ``~/.work4you/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``work4you auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_work4you_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.work4you/.env`` first, then ``os.environ``.
    ...
    """
    try:
        from work4you_cli.config import get_env_value as _work4you_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _work4you_get_env_value(name)
    return value if value is not None else default


def work4you_xai_user_agent() -> str:
    """Return a stable Work4You-specific User-Agent for xAI HTTP calls."""
    try:
        from work4you_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Work4You/{__version__}"


def work4you_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Work4You,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": work4you_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Work4You config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Work4You home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_work4you_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``WORK4YOU_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`work4you_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("WORK4YOU_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Work4You-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``work4you_cli.config.get_env_value`` so keys
    stored in ``~/.work4you/.env`` (the standard Work4You location) are honored —
    ...
    honors ``WORK4YOU_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
