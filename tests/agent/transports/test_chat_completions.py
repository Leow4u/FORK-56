...
class TestChatCompletionsBasic:
    ...
    @pytest.mark.parametrize("provider", ["work4you", "openrouter"])
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Work4You-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_work4you_tags(self, transport):
        from agent.portal_tags import work4you_portal_tags
        from providers import get_provider_profile
        profile = get_provider_profile("work4you")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(model="gpt-4o", messages=msgs, provider_profile=profile)
        assert kw["extra_body"]["tags"] == work4you_portal_tags()
    ...
    def test_work4you_omits_disabled_reasoning(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("work4you")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # Work4You rejects enabled=false; reasoning omitted entirely
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (Work4You, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. Work4You portal tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _work4you_profile(self):
        from providers import get_provider_profile
        return get_provider_profile("work4you")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            None,
            provider_profile=self._work4you_profile(),
    ...
    def test_tags_preserved_on_work4you_endpoint(self, transport):
        kw = transport.build_kwargs(
            "work4you-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
...
class TestPromptCacheKeyCapability:
    ...
    def test_stale_profile_without_supports_prompt_cache_key_does_not_crash(self, transport):
        ...
        Regression: 'Work4YouProfile' object has no attribute
