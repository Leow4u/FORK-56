...
// bug that drives the REAL useGatewayBoot hook + REAL Work4YouGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshWork4YouConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { work4youDesktop?: unknown }).work4youDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  window.localStorage.removeItem('work4you.desktop.workspace-cwd')
  $currentCwd.set('')
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForWork4You) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startWork4You()'s remote branch awaits waitForWork4You() for 45s before it
    ...
    ;(window as { work4youDesktop?: unknown }).work4youDesktop = desktop
    ...
    // After ~45s waitForWork4You gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Work4You backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Work4You couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Work4You')
    expect($currentCwd.get()).toBe('C:\\Work4You')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startWork4You() rejects with a
