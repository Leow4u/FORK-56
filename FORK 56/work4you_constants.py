...
_UNSET = object()
_WORK4YOU_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_WORK4YOU_HOME_OVERRIDE", default=_UNSET
)
...
def set_work4you_home_override(path: str | Path | None) -> Token:
    """Set a context-local Work4You home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _WORK4YOU_HOME_OVERRIDE.set(value)


def reset_work4you_home_override(token: Token) -> None:
    """Restore the previous context-local Work4You home override."""
    _WORK4YOU_HOME_OVERRIDE.reset(token)


def get_work4you_home_override() -> str | None:
    """Return the active context-local Work4You home override, if any."""
    override = _WORK4YOU_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_work4you_home() -> Path:
    """Return the platform-native default Work4You home path."""
...
def _work4you_home_from_env() -> Path:
    """Resolve WORK4YOU_HOME from the process environment only.

    Reads the ``WORK4YOU_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_work4you_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_work4you_home`
    and :func:`get_process_work4you_home` so the two never drift.
    """
    val = os.environ.get("WORK4YOU_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_work4you_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``WORK4YOU_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_work4you_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[WORK4YOU_HOME fallback] WORK4YOU_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass WORK4YOU_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_work4you_home() -> Path:
    """Return the Work4You home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_work4you_home_override`) → ``WORK4YOU_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``WORK4YOU_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``WORK4YOU_HOME`` explicitly (see the systemd
    template in ``work4you_cli/gateway.py`` and the kanban dispatcher in
    ``work4you_cli/kanban_db.py``).  See https://github.com/Leow4u/FORK-56/issues/18594.
    """
    override = get_work4you_home_override()
    if override:
        return Path(override)

    if not os.environ.get("WORK4YOU_HOME", "").strip():
        _warn_profile_fallback_once()

    return _work4you_home_from_env()


def work4you_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Work4You home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_work4you_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_work4you_home() -> Path:
    """Return the Work4You home for the running process, ignoring task overrides.

    Unlike :func:`get_work4you_home`, this never follows the context-local
    override set by :func:`set_work4you_home_override`.  It resolves only the
    process ``WORK4YOU_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_work4you_root(). The function resolves
# WORK4YOU_HOME against the native home on every call (~80us of path
...
# Its result depends only on (WORK4YOU_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates WORK4YOU_HOME mid-process.
_default_work4you_root_memo: "tuple[str, str, Path] | None" = None


def get_default_work4you_root() -> Path:
    """Return the root Work4You directory for profile-level operations.

    In standard deployments this is the platform-native Work4You home
    (``~/.work4you`` on POSIX, ``%LOCALAPPDATA%\\work4you`` on native Windows).

    In Docker or custom deployments where ``WORK4YOU_HOME`` points outside
    ``~/.work4you`` (e.g. ``/opt/data``), returns ``WORK4YOU_HOME`` directly
    — that IS the root.

    In profile mode where ``WORK4YOU_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.work4you/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_work4you_root_memo
    native_home = _get_platform_default_work4you_home()
    env_home = os.environ.get("WORK4YOU_HOME", "")
    if _default_work4you_root_memo is not None:
        memo_native, memo_env, memo_result = _default_work4you_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # WORK4YOU_HOME is under ~/.work4you (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — WORK4YOU_HOME itself is the root
                result = env_path
    _default_work4you_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``WORK4YOU_OPTIONAL_SKILLS``.
    """
    override = os.getenv("WORK4YOU_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_work4you_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Work4You-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``WORK4YOU_OPTIONAL_MCPS``.
    """
    override = os.getenv("WORK4YOU_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_work4you_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``WORK4YOU_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<WORK4YOU_HOME>/skills`` last-resort
    """
    override = os.getenv("WORK4YOU_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_work4you_home() / "skills"


def get_work4you_dir(
    new_subpath: str,
    old_name: str,
    *,
    home: Path | None = None,
) -> Path:
    """Resolve a Work4You subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to WORK4YOU_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to WORK4YOU_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Work4You home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local WORK4YOU_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_work4you_home()
...
def iter_work4you_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Work4You-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\work4you\\node``. POSIX installs use
    ``$WORK4YOU_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_work4you_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with work4youManagedNodePathEntries() in
...
_WORK4YOU_NODE_TARGET_MAJOR = int(os.environ.get("WORK4YOU_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Work4You-managed Node trees live under ``$WORK4YOU_HOME/node`` (or a profile's
    ``WORK4YOU_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_work4you_node_executable``
    used to trust file presence alone, so ``work4you update`` would pick that
...
def work4you_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Work4You-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_work4you_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Work4You-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `work4you update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%WORK4YOU_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the work4you.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_WORK4YOU_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_WORK4YOU_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$WORK4YOU_HOME/node`` on POSIX.
    ...
    ``WORK4YOU_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_work4you_managed_node() -> str | None:
    """Install a Work4You-managed Node tree and return its npm path.
    ...
    Work4You never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$WORK4YOU_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_work4you_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_work4you_node_dirs():
...
def heal_work4you_managed_node() -> bool:
    """Redownload Work4You-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not work4you_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "WORK4YOU_HOME": str(get_work4you_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_work4you_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_WORK4YOU_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_work4you_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _WORK4YOU_NODE_TARGET_MAJOR
    return False


def find_work4you_node_executable(command: str) -> str | None:
    """Return a Work4You-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_WORK4YOU_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_work4you_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Work4You-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Work4You-managed installs.

    This is for Work4You-owned subprocesses that should not be broken by a bad,
    ...
    if managed:
        return managed
    if work4you_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_work4you_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Work4You-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``work4you update`` — leaving a **dangling symlink** that ``which`` still
...
def display_work4you_home() -> str:
    """Return a user-friendly display string for the current WORK4YOU_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.work4you``
        profile:  ``~/.work4you/profiles/coder``
        custom:   ``/opt/work4you-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.work4you``.  For code that needs a real ``Path``, use
    :func:`get_work4you_home` instead.
    """
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``WORK4YOU_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/Leow4u/FORK-56/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{WORK4YOU_HOME}/home`` when the profile-home directory exists."""
    work4you_home = get_work4you_home_override() or (env or {}).get("WORK4YOU_HOME") or os.getenv("WORK4YOU_HOME")
    if not work4you_home:
        return None
    profile_home = os.path.join(work4you_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("WORK4YOU_REAL_HOME") or os.getenv("WORK4YOU_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Work4You profile HOME.

    ``WORK4YOU_HOME`` scopes Work4You state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={WORK4YOU_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{WORK4YOU_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{WORK4YOU_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Work4You' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["WORK4YOU_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Work4You itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under WORK4YOU_HOME.

    Replaces the ``get_work4you_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, work4you_logging.py, work4you_time.py, etc.).
    """
    return get_work4you_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under WORK4YOU_HOME."""
    return get_work4you_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under WORK4YOU_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_work4you_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._work4you_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``work4you_cli`` modules using three different Windows
    ...
    A few sites outside ``work4you_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``work4you_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Work4You itself. An ImportError
...
# `work4you_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Work4You.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("work4you_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `work4you update` is exactly
    ...
    return [
        ...
        "    work4you update",
        "If that also fails, reinstall: https://work4you.ai",
    ]
