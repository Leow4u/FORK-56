"""Centralized logging setup for Work4You.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.work4you/logs/`` (profile-aware via ``get_work4you_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``work4you_cli.web_server``,
    ``work4you_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Work4You (TUI, gateway, ``hy_memory`` server, MCP
...
from work4you_constants import get_config_path, get_work4you_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_work4you_session_injector", False):
    ...
    _session_record_factory._work4you_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``work4you logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``work4you logs --component gateway``.
    "gateway": ("gateway", "work4you_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("work4you_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "work4you_cli.web_server",
        "work4you_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Work4You logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    work4you_home
        Override for the Work4You home directory.  Falls back to
        ``get_work4you_home()`` (profile-aware).
    log_level
    ...
    home = work4you_home or get_work4you_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_work4you_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._work4you_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the work4you group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from work4you_cli.config import is_managed
...
# the module header): when several Work4You processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._work4you_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="work4you-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_work4you_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse work4you_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when work4you_cli.config isn't importable (bare
        # work4you_logging consumers).
        try:
            from work4you_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
