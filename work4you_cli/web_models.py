"""Pydantic request/response models for the Work4You dashboard web server.

Extracted verbatim from ``work4you_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from work4you_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``work4you model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `work4you import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`work4you -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the WORK4YOU_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under WORK4YOU_HOME.
    output: str = ""
