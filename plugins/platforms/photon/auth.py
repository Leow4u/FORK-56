...
Credential storage mirrors every other Work4You channel:

    * runtime SDK creds  -> ``~/.work4you/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.work4you/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a work4you dependency
...
# registers Work4You as its own client_id.
...
# Default name of the project Work4You provisions for the operator.
DEFAULT_PROJECT_NAME = "Work4You"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of work4you.

def _auth_json_path() -> Path:
    """Resolve ``~/.work4you/auth.json`` honouring the active Work4You profile."""
    try:
        from work4you_constants import get_work4you_home
        return Path(get_work4you_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.work4you")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # work4you_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from work4you_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.work4you/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.work4you/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.work4you/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: work4you_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `work4you photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `work4you photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `work4you photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `work4you photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `work4you photon setup`)"
