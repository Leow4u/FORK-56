...
``_load_auth_store`` and the Codex/Work4You shared-store readers previously called
...
def work4you_home(tmp_path, monkeypatch):
    """Point WORK4YOU_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.work4you/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, work4you_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, work4you_home, windows_default_encoding):
        ...
        assert not (work4you_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, work4you_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, work4you_home):
        auth_path = work4you_home / "auth.json"
...
# --- sibling readers of the same ~/.work4you/auth.json in other modules -------
#
# _load_auth_store lives in work4you_cli/auth.py, but several other modules read
# the same ~/.work4you/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, work4you_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(work4you_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_work4you_provider_reads_non_ascii_store(self, work4you_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Work4You-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling Work4You as
        ...
        store = {
            ...
            "providers": {"work4you": {"agent_key": "k", "label": "工作账号"}},
        }
        _write_utf8(work4you_home / "auth.json", store)
        ...
        # WORK4YOU_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", work4you_home / "auth.json")

        # _read_work4you_auth consults the credential pool FIRST and returns early
        ...
        provider = aux._read_work4you_auth()
        assert provider is not None
        assert provider.get("agent_key") == "k"

    def test_read_shared_work4you_state_reads_non_ascii_store(
        self, tmp_path, monkeypatch, windows_default_encoding
    ):
        """work4you_cli.auth._read_shared_work4you_state must read a non-ASCII store.

        The shared Work4You store (``work4you_auth.json``) is written as UTF-8. A
        ...
        _write_utf8(shared_dir / "work4you_auth.json", payload)

        provider = auth._read_shared_work4you_state()
        assert provider is not None
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, work4you_home, monkeypatch, windows_default_encoding
    ):
        """work4you_cli.main._has_any_provider_configured reads auth.json.
        ...
        # work4you_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_work4you_state(
        self, work4you_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_work4you_provider_state reads auth.json.

        The Work4You provider entry can carry a non-ASCII label. Under the
        ...
        from tools.managed_tool_gateway import _read_work4you_provider_state

        work4you = _read_work4you_provider_state()
        assert work4you is not None
        assert work4you.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert work4you.get("label") == "工作账号"
