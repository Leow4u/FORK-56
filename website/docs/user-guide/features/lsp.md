...
Work4You runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Work4You
...
1. Work4You captures a baseline of current diagnostics for the file.
...
…). Work4You auto-detects the binary on PATH or in
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Work4You at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<WORK4YOU_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`work4you lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Work4You installs both packages together when you
run `work4you lsp install typescript` or auto-install fires on first
use.

## CLI

```
work4you lsp status          # service state + per-server install status
work4you lsp list            # registry, optionally --installed-only
work4you lsp install <id>    # eagerly install one server
work4you lsp install-all     # try every server with a known recipe
work4you lsp restart         # tear down running clients
work4you lsp which <id>      # print resolved binary path
```

`work4you lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <WORK4YOU_HOME>/lsp/bin
...
When `install_strategy: auto`, Work4You installs binaries into
`<WORK4YOU_HOME>/lsp/bin/`. NPM packages land in
`<WORK4YOU_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Work4You-owned and is
...
**`work4you lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<WORK4YOU_HOME>/lsp/bin/`. Run
`work4you lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `work4you lsp status`**
...
When `work4you lsp status` shows a `Backend warnings` section, install
...
Check `~/.work4you/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `work4you lsp restart` to clear the set;
