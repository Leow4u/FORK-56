...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("work4you_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_SESSION_KEY", session_key)
        monkeypatch.setenv("WORK4YOU_EXEC_ASK", "1")
        monkeypatch.delenv("WORK4YOU_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("work4you-verify-", "work4you-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "work4you-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/work4you-verify-example.py",
            "rm -f /tmp/work4you-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../work4you-verify-example.py",
            "rm -f /tmp/a/../../tmp/work4you-verify-example.py",
            "rm -f /var/tmp/work4you-verify-example.py",
            "rm -f /tmp/work4you-verify-*",
            "rm -f /tmp/work4you-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/work4you-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/work4you-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\work4you-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\work4you-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\work4you-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"WORK4YOU_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.work4you/.env",
            "echo x | tee $WORK4YOU_HOME/.env",
            'echo x | tee "$WORK4YOU_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestWork4YouConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.work4you/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.work4you/config.yaml",
            "echo '  mode: off' >> ~/.work4you/config.yaml",
            "echo x | tee ~/.work4you/config.yaml",
            "echo x | tee $WORK4YOU_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.work4you/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Work4You absolute config.yaml is
        # handled by the project patterns, not the Work4You-home rule.
        for cmd in (
            "cat ~/.work4you/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.work4you/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Work4You-home prefixes must fold to ~/ and
    ~/.work4you/ in dangerous-command detection.
    ...
    SSH, and Work4You config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_work4you_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/WORK4YOU_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("WORK4YOU_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.work4you/work4you && source venv/bin/activate && python -m work4you_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m work4you_cli.main gateway run --replace &",
            "nohup python -m work4you_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart work4you-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Work4You security config via IFS.
            "sed${IFS}-i ~/.work4you/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep work4you) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "work4you.*gateway")',
            "kill -9 `pgrep work4you`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof work4you)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof work4you_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof work4you`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Work4You service
    label achieves the same effect as `work4you gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_work4you_label_detected(self):
        for cmd in (
            "launchctl stop ai.work4you.gateway",
            "launchctl kickstart -k system/ai.work4you.gateway",
            "launchctl bootout system/ai.work4you.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.work4you.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Work4You labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("WORK4YOU_GATEWAY_SESSION", "WORK4YOU_CRON_SESSION",
                      "WORK4YOU_YOLO_MODE",
                      "WORK4YOU_SESSION_KEY", "WORK4YOU_INTERACTIVE")
        }
        os.environ.pop("WORK4YOU_YOLO_MODE", None)
        os.environ.pop("WORK4YOU_INTERACTIVE", None)
        # WORK4YOU_CRON_SESSION takes priority over WORK4YOU_GATEWAY_SESSION in
        ...
        os.environ.pop("WORK4YOU_CRON_SESSION", None)
        os.environ["WORK4YOU_GATEWAY_SESSION"] = "1"
        os.environ["WORK4YOU_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("work4you_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"WORK4YOU_INTERACTIVE": "1"}, clear=False):
