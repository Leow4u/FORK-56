...
late-binding seam in :mod:`work4you_cli.web_deps` (``late`` for callables,
...
from work4you_cli.web_models import (
...
_spawn_work4you_action = late("_spawn_work4you_action")
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `work4you tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `work4you computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``work4you tools`` picker shows
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                # re-probing per row.
                features = get_work4you_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        "requires_work4you_auth": bool(prov.get("requires_work4you_auth")),
                        "is_active": is_active,
                        # Honest server-side readiness. The GUI's old client-side
                        # heuristic showed "Ready" for every zero-env-var row —
                        # including logged-out Work4You Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `work4you tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``work4you tools``
    ...
    Managed Work4You rows (``managed_work4you_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_work4you_portal_access`` (inline login), but the GUI has no inline
    ...
    ``needs_work4you_auth: true`` + ``feature`` so the client can drive the
    existing Work4You Portal OAuth flow (``POST /api/providers/oauth/work4you/start``)
    and refetch.
    """
    from work4you_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Work4You rows — mirrors the gate the CLI
            # applies via ensure_work4you_portal_access at selection time. This hits
            ...
            managed_feature = (row or {}).get("managed_work4you_feature")
            if managed_feature:
                features = get_work4you_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_work4you_auth"] = True
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.work4you/.env`` via ``save_env_value`` —
    the same store ``work4you tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``work4you tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``work4you -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same WORK4YOU_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``work4you computer-use permissions grant`` as a background action.
