...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@work4you\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Work4You". Regression:
  ...
  assert.equal(name(active, undefined), 'Work4You')
})

test('botHandle: precomputed multi-source handle wins; default stays work4you', () => {
  ...
  assert.equal(botHandle('default'), 'work4you')
})
...
test('displayName: local default stays Work4You; remote default uses the device label', () => {
...
test('resolveRosterMentions: @work4you in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'work4you' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @work4you and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
