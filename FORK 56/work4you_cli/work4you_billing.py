"""Work4You Portal Remote Spending HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``work4you_cli/work4you_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Work4You already holds for inference
  (``get_provider_auth_state("work4you")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``WORK4YOU_PORTAL_BASE_URL`` → ``WORK4YOU_PORTAL_BASE_URL`` → the
...
DEFAULT_PORTAL_BASE_URL = "https://portal.work4you.ai"
...
# work4you_cli.auth.WORK4YOU_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``WORK4YOU_PORTAL_BASE_URL`` → ``WORK4YOU_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("WORK4YOU_PORTAL_BASE_URL") or os.getenv("WORK4YOU_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base). `resolve_work4you_access_token`
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Work4You Portal — run `work4you portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_work4you_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable Work4You session at all.
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Work4You Portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Work4You Portal: timed out", error="network_error"
        ) from exc
