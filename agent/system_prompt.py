...
upstream prefix cache warm.  See ``work4you-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, work4you subscription block, tool-use
...
from work4you_constants import get_default_work4you_root, get_work4you_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- work4you-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_work4you_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``WORK4YOU_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("WORK4YOU_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the WORK4YOU_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from work4you_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from work4you_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound WORK4YOU_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_work4you_root` (NOT ``get_work4you_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_work4you_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Work4You never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want WORK4YOU_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the WORK4YOU_HOME ContextVar
    ...
    # Pointer to the work4you skill + docs for user questions about Work4You itself.
    stable_parts.append(WORK4YOU_HELP_GUIDANCE)
    ...
    # WORK4YOU_KANBAN_TASK env var). Normal chat sessions never see
    ...
    work4you_subscription_prompt = _r.build_work4you_subscription_prompt(agent.valid_tool_names)
    if work4you_subscription_prompt:
        stable_parts.append(work4you_subscription_prompt)
    ...
    # Coding posture (base Work4You, any interactive coding surface in a code
    ...
    # `work4you -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['work4you-bots']).
    ...
    # Active-profile hint — names the Work4You profile the agent is running
    # under so it doesn't conflate ~/.work4you/skills/ (default profile) with
    # ~/.work4you/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient WORK4YOU_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_work4you_root(): get_work4you_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_work4you_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_work4you_root())
    else:
        _home_str = _root_str = str(get_work4you_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_work4you_root()), which in ambient
        # profile mode is NOT get_work4you_home().
    ...
    if not agent.skip_context_files:
        ...
        # (developing Work4You). Every other surface (desktop chat panel,
    ...
    from work4you_time import get_timezone as _work4you_tz, now as _work4you_now
    now = _work4you_now()
