...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Work4You'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``WORK4YOU_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Work4You
...
def _snapshot_store_path() -> Path:
    return get_work4you_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.work4you"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_work4you = (
            "/.work4you"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.work4you"
        )
        archive_member = remote_work4you.lstrip("/")
        remote_tar = f"/tmp/.work4you_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
