...
export function getLogs(params: {
  ...
  return work4youApi<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getWork4YouConfig(profile?: string): Promise<Work4YouConfig> {
  return work4youApi<Work4YouConfig>({
...
export function getWork4YouConfigRecord(profile?: ProfileScope): Promise<Work4YouConfigRecord> {
  return window.work4youDesktop.api<Work4YouConfigRecord>({
    ...capabilityScoped(profile),
    path: '/api/config'
  })
}

export function getWork4YouConfigDefaults(): Promise<Work4YouConfigRecord> {
  return work4youApi<Work4YouConfigRecord>({
    ...profileScoped(),
...
export function getWork4YouConfigSchema(profile?: null | string): Promise<ConfigSchemaResponse> {
  return work4youApi<ConfigSchemaResponse>({
    ...profileScoped(profile),
    path: '/api/config/schema'
  })
}

export function saveWork4YouConfig(config: Work4YouConfigRecord, profile?: null | string): Promise<{ ok: boolean }> {
...
export function revealEnvVar(key: string, profile?: ProfileScope): Promise<{ key: string; value: string }> {
  return window.work4youDesktop.api<{ key: string; value: string }>({
    ...capabilityScoped(profile),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function validateProviderCredential(
  ...
  return work4youApi<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return work4youApi<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return work4youApi<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return work4youApi<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string, profile?: ProfileScope): Promise<OAuthStartResponse> {
  return window.work4youDesktop.api<OAuthStartResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return work4youApi<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(
  ...
  return window.work4youDesktop.api<OAuthPollResponse>({
    ...capabilityScoped(profile),
