"""Bounded product contract for the first Work4You shared-metrics slice."""
...
SCHEMA_KEY = "work4you.metrics.schema_version"
SCHEMA_VERSION = "work4you.metrics.event.v2"
MODEL_CALL_SCOPE = "work4you.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "work4you.task_run"
TOOL_CALL_SCOPE = "work4you.tool_call"
CLIENT_ACTIVE_MARK = "work4you.client.active"
TOOL_APPROVAL_MARK = "work4you.tool_approval"
SKILL_LIFECYCLE_MARK = "work4you.skill.lifecycle"
SKILL_LOAD_MARK = "work4you.skill.load"
SUBSCRIBER_NAME = "work4you.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "work4you.client.active"
LEGACY_MODEL_CALL_METRIC = "work4you.model_call.count"
MODEL_ROUTE_METRIC = "work4you.model_route.count"
TASK_STARTED_METRIC = "work4you.task_run.started"
TASK_FINISHED_METRIC = "work4you.task_run.finished"
TOOL_CALL_METRIC = "work4you.tool_call.count"
TOOL_APPROVAL_METRIC = "work4you.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "work4you.skill.lifecycle.count"
SKILL_LOAD_METRIC = "work4you.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Work4You installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(work4you_version or "").strip()
    ...
    return {
        ...
        "work4you_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("work4you_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Work4You logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("work4you.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from work4you_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Work4You terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Work4You registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "work4you-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Work4You tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Work4You."""
