"""Work4You Portal ``anthropic/*`` models route on the native Messages wire.

Portal serves its ``anthropic/*`` catalog at
``https://inference-api.work4you.ai/v1/messages`` alongside the
...
# hostname, so Portal behaviour must key off provider=work4you.
...
class TestApiModeRouting:
    """``work4you_api_mode`` / ``determine_api_mode`` split the Portal catalog."""
    ...
    def test_anthropic_prefixed_models_use_the_messages_wire(self, model):
        assert work4you_api_mode(model) == "anthropic_messages"
    ...
    def test_a_claude_model_without_the_vendor_prefix_is_not_rerouted(self):
        ...
        assert work4you_api_mode("claude-opus-4.8") == "chat_completions"
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the Work4You
        overlay alone advertises openai_chat for every Work4You model."""
        ...
        assert (
            determine_api_mode("work4you", PORTAL_URL, model="work4you-4-405b")
            == "chat_completions"
        )
        # No model → historical OpenAI-wire default (safer than guessing).
        assert determine_api_mode("work4you", PORTAL_URL) == "chat_completions"
...
class TestRuntimeResolution:
    ...
    def _stub_portal_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "work4you")
    ...
    def test_anthropic_model_resolves_to_the_messages_wire(self, monkeypatch):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="work4you", target_model="anthropic/claude-opus-5"
        )

        assert resolved["provider"] == "work4you"
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "work4you", "default": "work4you-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="work4you", target_model="anthropic/claude-opus-4.8"
        )
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        from agent.anthropic_adapter import (
            _is_work4you_portal_endpoint,
            _requires_bearer_auth,
        )

        spoofed = "https://inference-api.work4you.ai.attacker.test/v1"
        assert not _is_work4you_portal_endpoint(spoofed)
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Work4You loads that env from ~/.work4you/.env, so
...
class TestPortalBodyFields:
    ...
    They are produced by the Work4You provider profile, which only the OpenAI-wire
    transport consults — so the Anthropic branch has to merge them in itself.
    """

    def _build(self, provider="work4you", session_id="sess-abc123"):
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import work4you_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=work4you" in tags
        assert work4you_client_tag() in tags
    ...
    def test_helper_merge_is_a_no_op_for_non_nous(self):
        from agent.chat_completion_helpers import (
            _merge_work4you_portal_messages_extra_body,
        )

        kwargs = {"model": "claude-opus-4-8", "messages": []}
        agent = SimpleNamespace(provider="anthropic", session_id="s")
        assert _merge_work4you_portal_messages_extra_body(agent, kwargs) is kwargs
...
class TestPortalThinkingReplay:
    ...
    def test_staging_host_with_env_override_keeps_signed_thinking(
        ...
        monkeypatch.setenv("WORK4YOU_INFERENCE_BASE_URL", STAGING_URL)
...
class TestAuxiliaryDualWire:
    """``resolve_provider_client('work4you', …)`` must wrap Claude onto Messages."""
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                ...
                return_value=(plain, "work4you-4-405b"),
            ),
            patch(
            ...
            client, model = resolve_provider_client("work4you", "work4you-4-405b")

        assert model == "work4you-4-405b"
