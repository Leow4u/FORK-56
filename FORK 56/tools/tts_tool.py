...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.work4you/config.yaml``. Work4You
...
Configuration is loaded from ~/.work4you/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``work4you auth add <provider_id>``).
...
from tools.xai_http import work4you_xai_user_agent
...
# The managed OpenAI audio gateway (Work4You portal proxy) only proxies these speech
...
# Base URL now resolved via work4you_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_work4you_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.work4you/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.work4you/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``work4you tools``); otherwise the historical Edge backend remains active.

    The managed "Work4You Subscription" selection (``tts.provider: work4you``) is
    ...
    if provider == WORK4YOU_MANAGED_PROVIDER:
        return "openai"
    return provider
...
# custom voice-cloning scripts, etc.) into Work4You without any Python code
...
#           output_format: wav
#
# Work4You writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__WORK4YOU_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Work4You secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Work4You secrets (salvage of #56332) while still
    ...
    scrubbed = work4you_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from work4you the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``work4you_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `work4you setup` to configure, "
    ...
    from work4you_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `work4you model` or set XAI_API_KEY.")
    ...
    # output_format when Work4You actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": work4you_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Work4You client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Work4You caches Piper voice models.

    Resolves to ``~/.work4you/cache/piper-voices/`` under the active
    WORK4YOU_HOME so voice downloads follow profile boundaries.
    """
    from work4you_constants import get_work4you_dir
    root = Path(get_work4you_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("WORK4YOU_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `work4you setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run work4you setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'work4you setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'work4you tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the Work4You managed audio
    ...
    string):
    - ``"work4you"`` (or legacy ``use_gateway: true``) → managed gateway ONLY;
    ...
    if selected == WORK4YOU_MANAGED_PROVIDER:
        ...
        if managed_gateway is None:
            raise ValueError(selection_error(
                "tts",
                WORK4YOU_MANAGED_PROVIDER,
                "the Work4You Tool Gateway is not available (not entitled or "
                "unreachable)",
            ))
        return (
            managed_gateway.work4you_user_token,
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_work4you_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
