...
it is automatically downloaded from GitHub releases to $WORK4YOU_HOME/bin/tirith.
...
def _get_work4you_home() -> str:
    """Return the Work4You home directory, respecting WORK4YOU_HOME env var."""
    return str(get_work4you_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_work4you_home(), ".tirith-install-failed")
...
def _work4you_bin_dir() -> str:
    """Return $WORK4YOU_HOME/bin, creating it if needed."""
    d = os.path.join(_get_work4you_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $WORK4YOU_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_work4you_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $WORK4YOU_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $WORK4YOU_HOME/bin/tirith
    ...
    work4you_bin = os.path.join(_work4you_bin_dir(), "tirith")
    if os.path.isfile(work4you_bin) and os.access(work4you_bin, os.X_OK):
        _resolved_path = work4you_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return work4you_bin
