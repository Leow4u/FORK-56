...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      work4you = self'.packages.default;
      work4youVenv = work4you.work4youVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "work4you-check";
                homeDirectory = "/home/work4you-check";
                stateVersion = "24.11";
              };
            }
            { services.work4you = settings; }
      ...
      # services.work4you. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.work4you);
      ...
      configKeys = pkgs.runCommand "work4you-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${work4youVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "work4you-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "work4you-build-package" { } ''
          echo "PASS: package built at ${work4you}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "work4you-build-devshell" { } ''
        ...
        home-manager-module =
          let
            enabled = evalHomeModule {
              ...
              environment.WORK4YOU_TEST = "1";
              environmentFiles = [ "/run/secrets/work4you-env" ];
              work4youHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "work4you" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "work4you" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.work4youAgentSetup.data;

            failures =
              lib.optional (names != [
                "work4you"
                "work4you-backend"
              ]) "expected work4you + work4you-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/work4you gateway" (argvOf "work4you")
              ) "gateway process does not run `work4you gateway`: ${argvOf "work4you"}"
              ++ lib.optional (
                !lib.hasInfix "bin/work4you serve" (argvOf "work4you-backend")
              ) "backend process does not run `work4you serve`: ${argvOf "work4you-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "work4you-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/work4you-check/.work4you" (envOf n)) names
              ) "gateway and backend must share one WORK4YOU_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.WORK4YOU_HOME or null != "/home/work4you-check/.work4you"
              ) "installPackage must export WORK4YOU_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "work4you-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/work4you-check/.work4you/SOUL.md" activation
              ) "work4youHomeFiles must install into WORK4YOU_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads WORK4YOU_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "WORK4YOU_MANAGED=home-manager" (envOf n)) names
              ) "processes must report WORK4YOU_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "work4you-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "work4you-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/work4you-check";
            ...
            cases = [
              ...
              {
                name = "work4youHomeFiles needs no directory";
                ok = accepts { work4youHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "work4you-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "work4you-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "work4you" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."work4you-setup".text;

            failures =
              lib.optional (names != [
                "work4you"
                "work4you-backend"
              ]) "expected work4you + work4you-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/work4you gateway" (execOf "work4you")
              ) "gateway unit does not run `work4you gateway`: ${execOf "work4you"}"
              ++ lib.optional (
                !lib.hasInfix "bin/work4you dashboard" (execOf "work4you-backend")
              ) "backend unit does not run `work4you dashboard`: ${execOf "work4you-backend"}"
              ++ lib.optional (
                units.work4you.environment.WORK4YOU_HOME != units.work4you-backend.environment.WORK4YOU_HOME
              ) "gateway and backend must share one WORK4YOU_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/work4you/.work4you/SOUL.md" activation
              ) "work4youHomeFiles must install into WORK4YOU_HOME";
          ...
          in
          pkgs.runCommand "work4you-nixos-module" { } (
            if failures != [ ] then
        ...
        # $WORK4YOU_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                WORK4YOU_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "work4you-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'WORK4YOU_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--work4you-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "work4you-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "work4you-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${work4you}/bin/work4you || (echo "FAIL: work4you binary missing"; exit 1)
          test -x ${work4you}/bin/work4you || (echo "FAIL: work4you binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${work4you}/bin/work4you version 2>&1 | grep -qi "work4you" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "work4you-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in work4you work4you work4you-acp; do
            test -x ${work4you}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "work4you-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking work4you --help ==="
          ${work4you}/bin/work4you --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${work4you}/bin/work4you --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "work4you-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${work4you}/share/work4you/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${work4you}/share/work4you/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "WORK4YOU_BUNDLED_SKILLS" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: WORK4YOU_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${work4you}/share/work4you/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${work4you}/share/work4you/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "WORK4YOU_OPTIONAL_SKILLS" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, WORK4YOU_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "work4you-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${work4you}/share/work4you/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${work4you}/share/work4you/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "WORK4YOU_BUNDLED_PLUGINS" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: WORK4YOU_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "work4you-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${work4you}/share/work4you/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${work4you}/share/work4you/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${work4you}/share/work4you/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "WORK4YOU_BUNDLED_LOCALES" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: WORK4YOU_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + WORK4YOU_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && WORK4YOU_BUNDLED_LOCALES=${work4you}/share/work4you/locales \
            ${work4youVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with WORK4YOU_BUNDLED_LOCALES set"; exit 1)
        ...
        # WORK4YOU_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "work4you-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${work4you}/share/work4you/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${work4you}/share/work4you/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "WORK4YOU_OPTIONAL_MCPS" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: WORK4YOU_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${work4you}/bin/work4you mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: work4you mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "work4you-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${work4you}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${work4you}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "WORK4YOU_TUI_DIR" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: WORK4YOU_TUI_DIR set in wrapper"
        ...
        # Verify WORK4YOU_NODE is set in wrapper and points to Node 26+
        # (Work4You pins its toolchain to Node 26 everywhere)
        work4you-node = pkgs.runCommand "work4you-node-version" { } ''
          set -e
          echo "=== Checking WORK4YOU_NODE in wrapper ==="
          grep -q "WORK4YOU_NODE" ${work4you}/bin/work4you || \
            (echo "FAIL: WORK4YOU_NODE not set in wrapper"; exit 1)
          echo "PASS: WORK4YOU_NODE present in wrapper"

          WORK4YOU_NODE=$(sed -n "s/^export WORK4YOU_NODE='\(.*\)'/\1/p" ${work4you}/bin/work4you)
          test -x "$WORK4YOU_NODE" || (echo "FAIL: WORK4YOU_NODE=$WORK4YOU_NODE not executable"; exit 1)
          echo "PASS: WORK4YOU_NODE executable at $WORK4YOU_NODE"

          NODE_MAJOR=$("$WORK4YOU_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Work4You requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All WORK4YOU_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify WORK4YOU_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "work4you-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(WORK4YOU_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking WORK4YOU_MANAGED guards ==="
          check_blocked "config set" ${work4you}/bin/work4you config set model foo
          check_blocked "config edit" ${work4you}/bin/work4you config edit
        ...
        extra-python-packages = let
          ...
          work4youWithExtra = work4you.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "work4you-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${work4youWithExtra}/bin/work4you || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${work4youWithExtra}/bin/work4you || \
          ...
          if grep -q "PYTHONPATH" ${work4you}/bin/work4you; then
        ...
        extra-dependency-groups = let
          work4youWithGroups = work4you.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "work4you-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${work4youWithGroups}"
          echo "venv: ${work4youWithGroups.work4youVenv}"
        ...
        messaging-variant = pkgs.runCommand "work4you-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.work4youVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "work4you-config-roundtrip" {
          ...
          merge_and_load() {
            local work4you_home="$1"
            export WORK4YOU_HOME="$work4you_home"
            ${configMergeScript} ${nixSettings} "$work4you_home/config.yaml"
            ${work4youVenv}/bin/python3 -c '
import json, sys
