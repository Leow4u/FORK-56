...
`work4you config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.work4you/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `WORK4YOU_HOME`, secret scope, and
