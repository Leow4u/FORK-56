"""Tests for --yolo (WORK4YOU_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When WORK4YOU_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With WORK4YOU_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """WORK4YOU_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("WORK4YOU_YOLO_MODE") is None or True  # no-op, documents intent
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("WORK4YOU_YOLO_MODE", value)
        monkeypatch.setenv("WORK4YOU_INTERACTIVE", "1")
        monkeypatch.setenv("WORK4YOU_SESSION_KEY", "test-session")
