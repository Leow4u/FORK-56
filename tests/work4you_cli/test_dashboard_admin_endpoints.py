...
def _client():
    ...
    # Keep the state DB under the isolated WORK4YOU_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_work4you_home):
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from work4you_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.work4you/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `work4you auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `work4you auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_work4you_home):
        ...
        (get_work4you_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_work4you_home() / "memories"
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_work4you_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_work4you_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "work4you_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("work4you-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # work4you-index source advertises availability + featured search.
            idx = _Src("work4you-index")
        ...
        assert {"official", "github", "work4you-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/work4you/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/work4you/update`` runs ``work4you update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/work4you/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/work4you/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import work4you_cli.debug as dbg
        ...
        monkeypatch.setattr("work4you_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `work4you tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_work4you_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_work4you_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _WORK4YOU_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_work4you_action stores
    ...
    ws._spawn_work4you_action(["gateway", "restart"], "gateway-restart")

    assert "_WORK4YOU_GATEWAY" not in captured["env"]
    assert captured["env"]["WORK4YOU_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under WORK4YOU_DESKTOP=1 so the stale orphan is cleared first.
    """
    import work4you_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from work4you_cli.gateway, so patch it
    # on that module.
    import work4you_cli.gateway as g
