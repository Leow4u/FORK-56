...
The web dashboard is a browser-based UI for managing your Work4You installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Work4You Portal OAuth; if you also want the dashboard to talk to a real backend, `work4you setup --portal` wires up the model and tool gateway too. See [Work4You Portal](/integrations/work4you-portal).
:::
...
child with the selected profile's `WORK4YOU_HOME`, so the conversation runs
...
processes (manage them via `work4you -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `work4you` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.work4you/work4you && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.work4you/work4you && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `work4you dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `work4you dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.work4you` is nearly full
...
- Node.js (same requirement as `work4you --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.work4you/work4you && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `work4you config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Work4You instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `work4you profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.work4you/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `work4you skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `work4you mcp` reads from.
...
**Catalog:** browse the Work4You-approved MCP servers (the bundled `optional-mcps/`
...
`work4you mcp catalog` / `work4you mcp install` use.
...
Connect Work4You to any messaging platform from the browser — full parity with
`work4you setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.work4you/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Work4You versions, CPU core count + utilization, memory, disk usage of the Work4You home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Work4You version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `work4you update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Work4You Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `work4you portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `work4you curator`.
...
This re-reads `~/.work4you/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`WORK4YOU_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.work4you`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/mcp/catalog` | Browse the Work4You-approved MCP catalog |
...
| `GET /api/work4you/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Work4You Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Work4You engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (Work4You Portal)](#default-provider-work4you-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Work4You Desktop connection](#connecting-work4you-desktop-to-a-remote-backend). Every login is verified against your Work4You account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No Work4You Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `work4you dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `work4you dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Work4You plugin, no custom plugin), `work4you dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `work4you dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Work4You doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `work4you dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: Work4You

The bundled `plugins/dashboard_auth/work4you` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `work4you` when a client ID is configured.

Because every login is verified against Work4You Portal and protected by your Work4You account, **the Work4You provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Work4You provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `work4you dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Work4You login (run `work4you setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID` into `~/.work4you/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.work4you.ai/local-dashboards) in the Work4You Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
#### Worked example: Work4You

From a logged-in Work4You install to a Work4You-gated dashboard in three steps.

**1. Log in and register the dashboard.** `work4you dashboard register` uses your existing Work4You login to provision an OAuth client and writes `WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID` into `~/.work4you/.env` for you:
...
**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `work4you` provider:

```bash
work4you dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Work4You** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: work4you`). For an internet-facing host, register with `--redirect-uri https://work4you.example.com/auth/callback` and set `WORK4YOU_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Work4You provider](#default-provider-work4you-research) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the Work4You provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `WORK4YOU_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `WORK4YOU_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.work4you/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Work4You](#default-provider-work4you-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Work4You Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Work4You provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `WORK4YOU_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `WORK4YOU_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `WORK4YOU_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-work4you.json` — it defines a `work4you` realm, a **public PKCE client** (`work4you-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
`http://localhost:8080/realms/work4you`). The admin console is at
...
`WORK4YOU_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → work4you-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `WORK4YOU_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `WORK4YOU_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `WORK4YOU_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Work4You Portal OAuth contract v1](https://github.com/Work4You/work4you-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Work4You" button → `/auth/login?provider=work4you`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.work4you.ai/oauth/authorize?…`.
4. User authenticates with Portal, lands at `/auth/callback?code=…&state=…`.
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `work4you_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `work4you_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `work4you_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `work4you_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via work4you` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$WORK4YOU_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-Work4You OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:
...
Work4You Desktop can drive a Work4You backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Work4You Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Work4You Portal)** (register it with [`work4you dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Work4You*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Work4You](#default-provider-work4you-research).
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/work4you` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `WORK4YOU_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.
...
- **Signed out on every restart** — set `WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `work4you_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `work4you update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `work4you dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Work4You Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Work4You Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Work4You Blue** (`work4you-blue`) | Work4You-branded blue accents with airy spacing |
