...
SessionDB.session_gateway_runtime (work4you_state.py) — the round trip that
makes `work4you --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare Work4YouCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.Work4YouCLI)
...
def test_session_gateway_runtime_falls_back_to_top_level_keys():
    ...
    meta = _row(model_config={"provider": "work4you", "api_mode": "chat_completions"})
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "work4you", "api_mode": "chat_completions"}
...
def test_session_gateway_runtime_explicit_provider_wins_over_billing():
    ...
    meta = _row(model_config={"provider": "work4you"})
    meta["billing_provider"] = "minimax"
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "work4you"}
