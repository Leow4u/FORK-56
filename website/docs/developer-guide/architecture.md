...
This page is the top-level map of Work4You internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
Three discovery sources: `~/.work4you/plugins/` (user), `.work4you/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `work4you plugins` or `config.yaml`.
...
Exposes Work4You as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`work4you -p <name>`) gets its own WORK4YOU_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
