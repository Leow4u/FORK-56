...
Approvals = auto-bypassed (WORK4YOU_YOLO_MODE=1 is set for the call).
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to WORK4YOU_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("WORK4YOU_INFERENCE_MODEL", "").strip()
...
def _create_session_db_for_oneshot():
    ...
    Oneshot bypasses ``Work4YouCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when work4you is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from work4you_cli.config import load_config
    from work4you_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("WORK4YOU_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from work4you_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("WORK4YOU_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # Work4YouCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                WORK4YOU_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via WORK4YOU_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via WORK4YOU_YOLO_MODE=1
