...
Tests cover:
- managed_work4you_tools_enabled() subscription-based gate
...
from work4you_cli.work4you_account import Work4YouPaidServiceAccessInfo, Work4YouPortalAccountInfo
...
# managed_work4you_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedWork4YouToolsEnabled:
    """Subscription-based gate: True for paid Work4You subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "work4you_cli.work4you_account.get_work4you_portal_account_info",
            lambda: Work4YouPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_work4you_tools_enabled() is False
...
class TestWork4YouToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        ...
        message = work4you_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, work4you_enabled=False):
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, work4you_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, work4you_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, work4you_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_work4you_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, work4you_enabled=False)
        assert result["selected_backend"] is None
