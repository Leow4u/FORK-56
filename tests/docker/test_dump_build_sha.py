...
``$WORK4YOU_GIT_SHA`` build-arg to ``/opt/work4you/.work4you_build_sha`` and
...
* If ``/opt/work4you/.work4you_build_sha`` exists in the image, assert that
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/work4you/.work4you_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/work4you/.work4you_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``WORK4YOU_GIT_SHA``, dump must surface it.
