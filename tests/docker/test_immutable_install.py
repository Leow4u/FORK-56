...
Build the real image and verify at runtime:
  ...
  2. PYTHONDONTWRITEBYTECODE and WORK4YOU_DISABLE_LAZY_INSTALLS are set
  3. /opt/work4you/.install_method contains "docker" (code-scoped stamp)
  4. $WORK4YOU_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $WORK4YOU_HOME is healed (removed) on boot
"""
...
def test_work4you_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    WORK4YOU_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$WORK4YOU_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"WORK4YOU_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $WORK4YOU_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $WORK4YOU_HOME was not healed on boot: "
        f"{r.stdout}"
    )
