...
const UPDATE_TOAST_SNOOZE_KEY = 'work4you:update-toast-snooze-until'
...
// v7: requires GET /api/connectors/directory (Work4You Apps MCP store).
...
const SKEW_TOAST_SNOOZE_KEY = 'work4you:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'work4you:install-method-toast-snooze-until'
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.work4youDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.work4youDesktop?.updates
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== work4you-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateWork4You()

    if (!started.ok) {
      const message = (started as { message?: string }).message || translateNow('updates.applyStatus.notAvailable')
      const command = (started as { update_command?: string }).update_command || 'work4you update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkWork4YouUpdate(true)
