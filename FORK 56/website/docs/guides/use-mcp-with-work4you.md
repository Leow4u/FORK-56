---
sidebar_position: 6
title: "Use MCP with Work4You"
description: "A practical guide to connecting MCP servers to Work4You, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Work4You

This guide shows how to actually use MCP with Work4You in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Work4You tool
- you want Work4You to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Work4You to internal APIs, databases, or company systems without modifying Work4You core

Do not use MCP when:
- a built-in Work4You tool already solves the job well
...
- Work4You remains the agent
- MCP servers contribute tools
- Work4You discovers those tools at startup or reload time
...
If you installed Work4You with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Work4You banner/status should show MCP integration when configured
- ask Work4You what tools it has available
...
## WSL2: bridge Work4You in WSL to Windows Chrome

This is the practical setup when:

- Work4You runs inside WSL2
...
In this setup, Work4You does **not** connect to Chrome directly. Instead:

- Work4You runs in WSL
- Work4You starts a local stdio MCP server
...
```text
Work4You (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Work4You stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Work4You core browser transport
...
```bash
work4you mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
work4you mcp test chrome-devtools-win
```

Then start a fresh Work4You session or run:
...
Once loaded, Work4You can use the MCP-prefixed browser tools directly. For example:
...
If Work4You runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Work4You from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Work4You from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Work4You:
...
2. Work4You-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Work4You will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Work4You to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Work4You to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Work4You remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
work4you mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
work4you mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `work4you mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Work4You to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Work4You to merge, publish, deploy, or spawn runtimes.
...
Now Work4You can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Work4You core.
...
Because Work4You now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
