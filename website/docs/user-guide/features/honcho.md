...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Work4You's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Work4You instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
work4you memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$WORK4YOU_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Work4You at a self-hosted Honcho server, `work4you honcho setup` (and `work4you memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `work4you` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Work4You syncs them back at session init.
...
The `work4you honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `work4you memory setup honcho` (or run `work4you memory setup` and pick it from the list); the `work4you honcho` subcommand then appears on the next invocation.

```bash
work4you memory setup honcho    # Configure Honcho directly (works before activation)
work4you honcho status          # Connection status, config, and key settings
work4you honcho setup           # Redirects to `work4you memory setup` (post-activation alias)
work4you honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
work4you honcho peer            # Show or update peer names + dialectic reasoning level
work4you honcho mode            # Show or set recall mode (hybrid/context/tools)
work4you honcho tokens          # Show or set token budget for context and dialectic
work4you honcho identity        # Seed or show the AI peer's Honcho identity
work4you honcho sync            # Sync Honcho config to all existing profiles
work4you honcho peers           # Show peer identities across all profiles
work4you honcho sessions        # List known Honcho session mappings
work4you honcho map             # Map current directory to a Honcho session name
work4you honcho enable          # Enable Honcho for the active profile
work4you honcho disable         # Disable Honcho for the active profile
work4you honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `work4you honcho`

If you previously used the standalone `work4you honcho setup`:
...
No re-login or re-setup needed. Run `work4you memory setup` and select "honcho" — the wizard detects your existing config.
