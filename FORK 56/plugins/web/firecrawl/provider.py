...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Work4You tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Work4You Subscriber token are available.

    Reads ``peek_work4you_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_work4you_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_work4you_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `work4you tools`
    ...
    existing tests and the ``work4you tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_work4you_tools_enabled():
        return ""
    return (
        ", or use the Work4You Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_work4you_tools_enabled():
        message += (
            " With your Work4You subscription you can also use the Tool Gateway. "
            "run `work4you tools` and select Work4You Subscription as the web provider."
        )
    else:
        message += " " + _wt.work4you_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    - ``"work4you"`` (or legacy ``use_gateway: true``) → managed Tool Gateway
    ...
    functions (``resolve_managed_tool_gateway``, ``_read_work4you_access_token``,
    ...
    def _managed_kwargs():
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_work4you_access_token
        )
        if managed_gateway is None:
            return None
        kwargs = {
            "api_key": managed_gateway.work4you_user_token,
    ...
    if selected == WORK4YOU_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the Work4You "
