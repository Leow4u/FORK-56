...
One ``CDPSupervisor`` runs per Work4You ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Work4You logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "work4you-dialog-bridge.invalid"
...
(() => {
  if (window.__work4youDialogBridgeInstalled) return;
  window.__work4youDialogBridgeInstalled = true;
  const ENDPOINT = "http://work4you-dialog-bridge.invalid/";
