/**
 * Shared E2E fixtures for the Work4You desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → work4you serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `work4you desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `work4you-e2e-${prefix}-${Math.random()}`))
  const work4youHome = path.join(root, 'work4you-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(work4youHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(work4youHome, 'config.yaml')
 ...
 */
export function writeEnvFile(work4youHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(work4youHome, '.env')
 ...
 */
function writeEmptyConfig(work4youHome: string): void {
  const configPath = path.join(work4youHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - WORK4YOU_HOME → sandbox work4you-home (isolated config/sessions)
 *  - WORK4YOU_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - WORK4YOU_DESKTOP_IGNORE_EXISTING=1 → don't pick up `work4you` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - WORK4YOU_DESKTOP_WORK4YOU_ROOT → REPO_ROOT (dev checkout resolution)
 *  - WORK4YOU_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    WORK4YOU_HOME: sandbox.work4youHome,
    WORK4YOU_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    WORK4YOU_DESKTOP_IGNORE_EXISTING: '1',
    WORK4YOU_DESKTOP_WORK4YOU_ROOT: REPO_ROOT,
    WORK4YOU_DESKTOP_APP_NAME: `Work4YouE2E-${Date.now()}`,
    ...
    WORK4YOU_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated WORK4YOU_HOME + userData
 * @param env      - the process environment (already has WORK4YOU_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.work4youHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.work4youHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.work4youHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via WORK4YOU_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`work4you serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.work4youHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { WORK4YOU_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Work4You backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Work4You.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Work4You.app', 'Contents', 'MacOS', 'Work4You')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'work4you')
}
 ...
 * progress without spawning a real Work4You backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * WORK4YOU_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    WORK4YOU_DESKTOP_BOOT_FAKE: '1',
    WORK4YOU_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + work4you-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).WORK4YOU_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).WORK4YOU_DESKTOP_WORK4YOU
  delete (env as Record<string, string | undefined>).WORK4YOU_DESKTOP_WORK4YOU_ROOT
