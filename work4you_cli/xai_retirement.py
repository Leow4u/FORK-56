...
Pure logic: walks a Work4You config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `work4you doctor` and a future `work4you migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Work4You config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Work4You config and return retirement issues.
...
def apply_migration(
    ...
    from work4you_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `work4you migrate xai` reports nothing to migrate
