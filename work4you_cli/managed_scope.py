...
A system-level directory (default ``/etc/work4you``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.work4you/config.yaml`` and ``~/.work4you/.env`` on a per-leaf-key basis.

This is DISTINCT from ``work4you_cli.config.is_managed()`` / ``WORK4YOU_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/work4you")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/work4you`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``WORK4YOU_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$WORK4YOU_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/work4you`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("WORK4YOU_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through work4you_cli.config.load_config) to honor managed
    scope. Mirrors work4you_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from work4you_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
