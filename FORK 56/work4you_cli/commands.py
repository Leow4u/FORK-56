"""Slash command definitions and autocomplete for the Work4You CLI.
...
from work4you_constants import INDICATOR_STYLES
...
# _work4you_auth_status_cache). Falls back to a fresh load when the file cannot
# be stat'ed.
...
class CommandDef:
    ...
    # Names a key in ``work4you_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``work4you_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Work4You config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Work4You works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Work4You' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Work4You is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.work4you/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Work4You plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Work4You balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Work4You to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Work4You version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[work4you|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`work4you_cli.plugins.PluginContext.register_command`. They behave
...
# Telegram allows up to 100 BotCommands. Work4You ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Work4You
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``work4you skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``work4you skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``work4you skills list`` and
...
# unrelated command landed. These claim their slots right after /work4you,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /work4you <command>).
...
# "Slack-via-/work4you" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /work4you topup on Slack.
...
#   - egress: Docker-only proxy status; reachable as /work4you egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /work4you init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /work4you version on
...
#   - diff: git working-tree diff; reached via /work4you diff on Slack so it
...
#     /work4you update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /work4you heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /work4you refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /work4you pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /work4you whoami on Slack.
...
#     /work4you platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_WORK4YOU_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    avoidance. ``/work4you`` is always reserved as the first entry so the
    legacy ``/work4you <subcommand>`` form keeps working for anything that
    ...
    # Reserve /work4you as the catch-all top-level command.
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_WORK4YOU_ONLY:
            # Intentionally Slack-via-/work4you only (see _SLACK_VIA_WORK4YOU_ONLY).
            return
    ...
    # everything except /work4you, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://work4you.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /work4you handler.

    Maps both canonical names and aliases so /work4you bg do stuff works
    the same as /work4you background do stuff.

    Plugin-registered slash commands are included so ``/work4you <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from work4you_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via work4you_cli.personality."""
        try:
            ...
            # freshness-correct (same pattern as load_env / _work4you_auth_status_cache).
