...
Scans three surfaces a Work4You user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Work4You venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.work4you/plugins``
...
def _discover_plugins(work4you_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.work4you/plugins``.
...
def _discover_components(
    *,
    ...
    home = work4you_home or Path(get_work4you_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `work4you security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, work4you_home=home
    )
