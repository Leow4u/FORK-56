...
_DEFAULT_CONTAINER_TAG = "work4you"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=work4you"
...
def _load_supermemory_config(work4you_home: str) -> dict:
    config = _default_config()
    config_path = Path(work4you_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, work4you_home: str) -> None:
    config_path = Path(work4you_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "work4you"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Work4You writes into the "Work4You" Space in the Supermemory
        ...
        merged = {"sm_source": "work4you", **(metadata or {})}
...
def _resolve_container_tag_for_setup(work4you_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(work4you_home)
...
def _probe_supermemory_connection(api_key: str, work4you_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(work4you_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(work4you_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `work4you memory setup`.
        # All other options are documented for $WORK4YOU_HOME/supermemory.json
    ...
    def save_config(self, values, work4you_home):
        ...
        _save_supermemory_config(sanitized, work4you_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, work4you_home)
    ...
    def post_setup(self, work4you_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(Path(work4you_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from work4you_constants import get_work4you_home
        self._work4you_home = kwargs.get("work4you_home") or str(get_work4you_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._work4you_home)
