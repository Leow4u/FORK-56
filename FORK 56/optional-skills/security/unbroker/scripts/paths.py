...
All per-subject data lives under PDD_DATA_DIR (default: $WORK4YOU_HOME/unbroker),
which is the same trust boundary Work4You uses for .env and OAuth tokens.
"""
...
def work4you_home() -> Path:
    return Path(os.environ.get("WORK4YOU_HOME") or (Path.home() / ".work4you"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else work4you_home() / "unbroker"
