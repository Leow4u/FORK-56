...
#
# Installs a real, earlier Work4You the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `work4you`.
#
...
# Keep sandbox state out of the default .work4you-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".work4you-sandbox-e2e-$ROUTE"
export WORK4YOU_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/work4you/.work4you/work4you"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/work4you.git"
...
LOG_DIR="${WORK4YOU_E2E_LOG_DIR:-$(mktemp -d -t work4you-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED work4you accept FLAG on `work4you update`?
#
...
# that silently rots. `work4you update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "work4you update --help 2>&1" | grep -qF -- "$flag"
}
...
require_work4you_works() {
  local when="$1" out
  out="$(in_sandbox "work4you --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "work4you --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "work4you runs $when"
}

# ── install the earlier Work4You ─────────────────────────────────────────────
...
require_work4you_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: work4you update'
    ...
    # work4you which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="work4you update --yes"
    else
      update_cmd="work4you update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "work4you update failed ($update_cmd)"
    fi
    require_landed_on_target 'work4you update'
    require_work4you_works 'after work4you update'
    ;;
  installer)
    ...
    require_work4you_works 'after installer re-run'
    ;;
esac
