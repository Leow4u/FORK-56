 ...
 * `Work4YouPlugin`; it never touches the registry directly — it receives a
 ...
 *  under `work4you.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.work4youDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Work4You — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `work4you://index-network/intent/1`),
...
export interface Work4YouPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `work4you.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.work4youDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.work4youDesktop
