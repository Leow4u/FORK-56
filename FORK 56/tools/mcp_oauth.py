...
URL of Work4You' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``Work4YouTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Work4You"
...
_USER_SKIPPED_SENTINEL = "__work4you_user_skipped__"
...
def _get_token_dir(work4you_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses WORK4YOU_HOME so each profile gets its own OAuth tokens.
    Layout: ``WORK4YOU_HOME/mcp-tokens/``
    """
    from work4you_constants import get_work4you_home

    base = Path(work4you_home) if work4you_home is not None else Path(get_work4you_home())
...
def _cached_redirect_port(storage: "Work4YouTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Work4You restarts and chooses a
...
def _cached_redirect_uri(storage: "Work4YouTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``work4you mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `work4you mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# Work4YouTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class Work4YouTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        WORK4YOU_HOME/mcp-tokens/<server_name>.json         -- tokens
        WORK4YOU_HOME/mcp-tokens/<server_name>.client.json   -- client info
        WORK4YOU_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        WORK4YOU_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, work4you_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._work4you_home = Path(work4you_home) if work4you_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._work4you_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._work4you_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._work4you_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._work4you_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Work4You records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``work4you mcp login`` /
        ``work4you mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._work4you_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Work4You.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://work4you.ai/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Work4You' Client ID Metadata Document ({cimd_url}); set "
...
Work4YouOAuthClientProvider: Any = None


def _get_work4you_oauth_provider_class() -> type | None:
    global Work4YouOAuthClientProvider
    if Work4YouOAuthClientProvider is not None:
        return Work4YouOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _Work4YouOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._work4you_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_work4you_token_user_agent", None)
            if ua:
    ...
    _Work4YouOAuthClientProvider.__name__ = "Work4YouOAuthClientProvider"
    _Work4YouOAuthClientProvider.__qualname__ = "Work4YouOAuthClientProvider"
    Work4YouOAuthClientProvider = _Work4YouOAuthClientProvider
    return Work4YouOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = Work4YouTokenStorage(server_name, work4you_home=work4you_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Work4You only decides whether a
...
# work4you.ai/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://work4you.github.io/work4you/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Work4You picks
...
def _has_cached_client_info(storage: "Work4YouTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "Work4YouTokenStorage | None") -> bool:
    ...
    Work4You has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "Work4YouTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Work4You would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Work4You" / "Work4You" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Work4You")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Work4You is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: work4you mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Work4You auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Work4You is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = Work4YouTokenStorage(server_name)
    ...
    provider_class = _get_work4you_oauth_provider_class()
