...
Lets the LLM write a Python script that calls Work4You tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `work4you_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `work4you_tools.py` with file-based RPC stubs
...
# available on every platform Work4You itself runs on.
...
# match a safe prefix, the operational WORK4YOU_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Work4You
# code can lose the DB-layer Kanban mutation guard while still inheriting
# WORK4YOU_HOME.
#
# NB: the broad "WORK4YOU_" prefix was deliberately removed (#27303) — it leaked
# WORK4YOU_*-named config that lacks a secret substring (e.g. WORK4YOU_BASE_URL,
# WORK4YOU_KANBAN_DB, WORK4YOU_*_WEBHOOK).  The child only needs the few
# location/profile vars in _WORK4YOU_CHILD_ALLOWED below; WORK4YOU_RPC_SOCKET /
# WORK4YOU_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational WORK4YOU_* vars the child legitimately needs by exact name — these
...
_WORK4YOU_CHILD_ALLOWED = frozenset({
    "WORK4YOU_HOME",
    "WORK4YOU_PROFILE",
    "WORK4YOU_CONFIG",
    "WORK4YOU_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational WORK4YOU_* vars (_WORK4YOU_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret WORK4YOU_* vars dropped by the tightened allowlist (#27303). The
    # broad "WORK4YOU_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like WORK4YOU_KANBAN_DB / WORK4YOU_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _WORK4YOU_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("WORK4YOU_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped WORK4YOU_* var.
            _dropped_work4you.append(k)
    if _dropped_work4you:
        logger.debug(
            "execute_code: dropped %d non-allowlisted WORK4YOU_* var(s) from the "
            ...
            len(_dropped_work4you),
            ", ".join(sorted(_dropped_work4you)),
        )
...
# ---------------------------------------------------------------------------
# work4you_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Work4You-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Work4You-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    work4you_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'work4you_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_work4you_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the work4you_tools.py stub module.
...
def _connect():
    ...
    WORK4YOU_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["WORK4YOU_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("WORK4YOU_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
_RPC_DIR = os.environ.get("WORK4YOU_RPC_DIR") or os.path.join(tempfile.gettempdir(), "work4you_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated work4you_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/work4you_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_work4you_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/work4you_tools.py", tools_src)
        ...
        env_prefix = (
            f"WORK4YOU_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"WORK4YOU_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("WORK4YOU_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with work4you_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="work4you_sandbox_")
    ...
    # selector from WORK4YOU_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"work4you_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated work4you_tools module.
        ...
        tools_src = generate_work4you_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "work4you_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  WORK4YOU_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["WORK4YOU_RPC_SOCKET"] = rpc_endpoint
        child_env["WORK4YOU_RPC_TOKEN"] = rpc_token
        ...
        # WORK4YOU_TIMEZONE is an internal Work4You setting and must not leak
        # into child processes.
        _tz_name = os.getenv("WORK4YOU_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("WORK4YOU_TIMEZONE", None)
        ...
        # ``work4you_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Work4You's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Work4You's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Work4You-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Work4You-
        ...
        _strip_work4you_owned_pythonpath(child_env)
        _work4you_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_work4you_python_environment(_child_python):
            _pp_parts.append(_work4you_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import work4you_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Work4You "
                "environment; work4you root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.work4you/.env')).
...
def _load_config() -> dict:
    ...
    ``work4you --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (work4you's python). Reproducible and the
...
# Interpreter paths already reported as outside the Work4You environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the work4you root from every
...
def _uses_work4you_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Work4You's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``work4you tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and work4you's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.work4you/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Work4You tools programmatically. "
        ...
        f"Available via `from work4you_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from work4you_tools import {import_str}` "
