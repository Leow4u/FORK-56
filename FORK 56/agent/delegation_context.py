...
The parent Work4You process may itself be a Kanban dispatcher worker with
WORK4YOU_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's WORK4YOU_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "work4you_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "WORK4YOU_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``WORK4YOU_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$WORK4YOU_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``WORK4YOU_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``WORK4YOU_KANBAN_*`` vars while losing the ContextVar in
