...
def server():
    ...
    # e.g. work4you_cli.active_sessions would bind the mocked get_work4you_home
    ...
    with patch.dict("sys.modules", {
        "work4you_constants": MagicMock(get_work4you_home=MagicMock(return_value="/tmp/work4you_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `WORK4YOU_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("WORK4YOU_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("WORK4YOU_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_roundtrip(server, tmp_path):
    server._work4you_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/work4you-dev": {"name": "work4you-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "work4you-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient WORK4YOU_HOME
    ...
    # The gateway's own WORK4YOU_HOME (the test-isolation tempdir, no
...
def test_voice_and_wake_handlers_are_pool_routed(voice_or_wake_method, server):
    ...
    STT provider auto-detect → a SYNCHROWORK4YOU faster-whisper lazy install (uv/pip
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `work4you config set` does)
    ...
    import work4you_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_work4you_home", lambda: tmp_path)
    ...
    # Activate midnight, as `work4you config set display.skin midnight` would.
