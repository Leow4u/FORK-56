...
def _enable(work4you_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<work4you_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        work4you_home = Path(os.environ["WORK4YOU_HOME"])  # set by hermetic conftest fixture
        user_plugins = work4you_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(work4you_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        work4you_home = Path(os.environ["WORK4YOU_HOME"])  # set by hermetic conftest fixture
        _write_plugin(work4you_home / "plugins", ["p1"])
        _enable(work4you_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(work4you_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(work4you_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(work4you_home, "bad-img-plugin")
