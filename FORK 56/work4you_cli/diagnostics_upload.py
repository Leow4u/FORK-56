"""Client for uploading ``work4you debug share`` bundles to Work4You-internal S3.

This is the opt-in (``--work4you``) destination for ``work4you debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Work4You-owned
S3 bucket via a short-lived signed URL minted by the Work4You account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Work4You staff (and allowlisted Discord mods) through a
...
# Base URL of the Work4You account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "WORK4YOU_DIAGNOSTICS_BASE_URL", "https://portal.work4you.ai"
)
...
_USER_AGENT = "work4you/debug-share"
...
def share_to_nous(report_bundle: bytes) -> dict:
    """Orchestrate the full Work4You-S3 upload of a gzipped *report_bundle*.
