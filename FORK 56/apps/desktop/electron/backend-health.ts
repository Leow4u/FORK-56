...
export interface Work4YouReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForWork4YouReady`).
   */
...
export async function waitForWork4YouReady(baseUrl: string, options: Work4YouReadyOptions): Promise<void> {
  ...
  throw new Error(`Work4You backend did not become ready: ${detail}`)
}
