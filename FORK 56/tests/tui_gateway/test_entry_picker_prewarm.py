...
guard is covered in ``tests/work4you_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``work4you_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from work4you_cli.model_switch,
