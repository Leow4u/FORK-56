...
# Work4You WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: WORK4YOU_TINI_SHIM_TARGET overrides the `/init` path (and
# WORK4YOU_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${WORK4YOU_TINI_SHIM_TARGET:-/init}"
WRAPPER="${WORK4YOU_TINI_SHIM_WRAPPER:-/opt/work4you/docker/main-wrapper.sh}"
