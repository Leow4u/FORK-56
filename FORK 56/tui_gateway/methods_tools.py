...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.work4you/.env`` into the gateway process via
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        r = subprocess.run(
            ...
            env=work4you_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind WORK4YOU_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_work4you_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("WORK4YOU_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("WORK4YOU_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_work4you_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off WORK4YOU_HOME, so scoping the
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``work4you journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `work4you mcp` offers, so
...
# (work4you_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes WORK4YOU_HOME via set_work4you_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses work4you_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from work4you_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.work4youDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``work4you mcp login`` uses
    ...
    try:
        ...
        work4you_home = str(get_work4you_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(work4you_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``work4you plugins`` / the dashboard, so the three surfaces
    ...
    plugins live under each profile's WORK4YOU_HOME, so a client can list or
