...
``<work4you home>/plugins/<name>/``. That tree is the plugin *install* dir —
``work4you plugins remove`` deletes it and ``work4you plugins update`` git-pulls
...
``<work4you home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Work4You.
...
# Mirrors the plugin-name shape `work4you plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<work4you home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`work4you_constants.get_work4you_home` on every call. Don't cache the
    result across profile switches.
    """
    from work4you_constants import get_work4you_home

    root = get_work4you_home() / "plugin-data" / _validate_name(name)
