import { JsonRpcGatewayClient } from '@work4you/shared'

import type { Work4YouApiRequest } from '@/global'
...
// "Timed out connecting to Work4You backend" that hangs the UI (#48504).
//
...
export class Work4YouGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
...
// process to route them to that host (work4you:api's registry branch). Null
...
export function work4youApi<T>(request: Work4YouApiRequest): Promise<T> {
  return window.work4youDesktop.api<T>({ ...connectionScoped(), ...request })
}
