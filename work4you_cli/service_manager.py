...
the existing module-level functions in work4you_cli.gateway and
work4you_cli.gateway_windows directly. This protocol is a thin facade
...
``work4you gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``work4you gateway start/stop/restart``).
    """
    ...
    # `work4you gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged work4you
    ...
    detection always fails. Since every Work4You runtime call inside the
    container drops to work4you via ``s6-setuidgid``, that silent failure
...
# in ``work4you_cli.gateway`` (systemd/launchd) and ``work4you_cli.gateway_windows``
...
# active (set via the ``work4you -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in work4you_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from work4you_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from work4you_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from work4you_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from work4you_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in work4you_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from work4you_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from work4you_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from work4you_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from work4you_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``work4you_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `work4you profile create`
# runs inside the container (Phase 4). Static services (main-work4you, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's WORK4YOU_HOME blindly;
    derive the shared profile root from the current WORK4YOU_HOME and map the
    ...
    work4you_home = Path(os.environ.get("WORK4YOU_HOME", "/opt/data"))
    if work4you_home.parent.name == "profiles":
        root = work4you_home.parent.parent
    else:
        root = work4you_home
...
# UID/GID of the in-image ``work4you`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the work4you user.
    ...
    (the ``work4you`` user — UID 10000 — running every Work4You runtime
    ...
    is dispatched as the work4you user.
    ...
    with work4you ownership before triggering ``s6-svscanctl -a``,
    ...
    slot's supervise/ was work4you-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _WORK4YOU_UID, _WORK4YOU_GID)
        except PermissionError:
            # Running as the work4you user already — directory is work4you-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _WORK4YOU_UID, _WORK4YOU_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is work4you-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _WORK4YOU_UID, _WORK4YOU_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``work4you -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`work4you profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-work4you, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources WORK4YOU_HOME (and any extra env) via with-contenv —
             so e.g. ``-e WORK4YOU_HOME=/data/work4you`` is honored at run
          ...
          4. Drops to the work4you user and exec's
             ``work4you -p <profile> gateway run`` (or just ``work4you
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``work4you gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root WORK4YOU_HOME profile" (the implicit profile that exists at
        the top of $WORK4YOU_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``work4you_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$WORK4YOU_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``WORK4YOU_HOME``,
        ...
        allocator in ``work4you_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export WORK4YOU_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's WORK4YOU_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``work4you gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-WORK4YOU_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The WORK4YOU_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own WORK4YOU_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${WORK4YOU_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the WORK4YOU_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e WORK4YOU_HOME=/data/work4you``
        gets its logs under /data/work4you/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``work4you logs`` reads and what
        ...
        return (
            ...
            f': "${{WORK4YOU_HOME:=/opt/data}}"\n'
            f'log_dir="$WORK4YOU_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as work4you when
            # this script starts as root. Never chown or unlink work4you-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is work4you-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded work4you-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid work4you mkdir -p "$log_dir"\n'
            f'  s6-setuidgid work4you rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``work4you gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``work4you -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with work4you ownership
            ...
            # the work4you user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is work4you-owned (see ``_seed_supervise_skeleton``), and on
