...
export const $work4youHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.work4you/logs/bootstrap-installer.log')
    $work4youHome.set('~/.work4you')
  ...
  try {
    const [logPath, work4youHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_work4you_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $work4youHome.set(work4youHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Work4You" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Work4You-Setup.exe --update);
...
export async function launchWork4YouDesktop(): Promise<void> {
  ...
  await invoke('launch_work4you_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Work4You repository' },
