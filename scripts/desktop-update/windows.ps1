...
# (%WORK4YOU_HOME%\work4you-setup.exe). That binary has no self-update path --
# copy_self_to_work4you_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `work4you update` refreshes
...
#     -InstallRoot <path>   repo checkout (WORK4YOU_HOME\work4you)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Work4You.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .work4you-update-in-progress in place (tests)
#
...
# .work4you-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim WORK4YOU_HOME\.work4you-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately). work4you_cli/update_lock.py's ancestry rule lets our
# `work4you update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Work4You.exe pid.
try {
    Add-Type -Namespace Work4YouHandoff -Name Win32 -MemberDefinition @'
...
$Work4YouHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $Work4YouHome ".work4you-update-in-progress"
$LogDir = Join-Path $Work4YouHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $Work4YouHome ".work4you-update-result.json"
$script:Ui = $null
$script:UiStage = "Work4You will open once done."   # until the first gate; matches ui.html
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-Work4YouStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("work4you-update-ui-{0}" -f $PID)
                $edgeArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [Work4YouHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `work4you debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"work4you debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [Work4YouHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Work4You themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Work4You.exe before writing the replacement,
    ...
    # DO NOT spawn Work4You.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [Work4YouHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [Work4YouHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [Work4YouHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
...
function Invoke-Work4YouStep([string]$Exe, [string[]]$Work4YouArgs, [string]$Tag) {
    ...
    # afterwards, where `work4you debug share` picks it up.
    ...
    $psi.Arguments = ($Work4YouArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # work4you update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; WORK4YOU_SELFTEST_FAIL=1 exercises the
# error state, WORK4YOU_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
...
try {
    ...
    Publish-UiProgress "Waiting for Work4You to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Work4You window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Work4You fully and try again."
    ...
    Publish-UiProgress "Preparing Work4You files"
    $shim = Join-Path $InstallRoot "venv\Scripts\work4you.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Work4You install open (venv\Scripts\work4you.exe locked after 20s). Nothing was changed. Close other Work4You windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the work4you.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\work4you.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_work4you_exe must first rename the running work4you.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that work4you.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Work4You window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Work4You.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m work4you_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Work4You installer or `work4you doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "work4you_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    Write-HandoffLog ("running: python " + ($updateArgs -join " "))
    Publish-UiProgress "Updating code and dependencies"
    $res = Invoke-Work4YouStep $pythonExe $updateArgs "update"
    Write-HandoffLog "work4you update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Work4You windows") is not retryable.
    ...
    # `work4you update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "work4you update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-Work4YouStep $pythonExe @("-m", "work4you_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `work4you desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `work4you debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Work4You is back",
    ...
    if ($finalCode -ne 0) {
        ...
        Publish-UiProgress "Opening Work4You"
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Work4You to finish (it could not restart itself)."
