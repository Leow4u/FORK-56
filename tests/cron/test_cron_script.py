...
- Path resolution (absolute, relative to WORK4YOU_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp WORK4YOU_HOME."""
    work4you_home = tmp_path / ".work4you"
    work4you_home.mkdir()
    (work4you_home / "cron").mkdir()
    (work4you_home / "cron" / "output").mkdir()
    (work4you_home / "scripts").mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "WORK4YOU_DIR", work4you_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", work4you_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", work4you_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", work4you_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Work4You provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _WORK4YOU_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_WORK4YOU_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.work4you/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("WORK4YOU_SESSION_PLATFORM") is None
        assert os.environ.get("WORK4YOU_SESSION_CHAT_ID") is None
        assert os.environ.get("WORK4YOU_SESSION_CHAT_NAME") is None
