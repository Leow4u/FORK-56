# Themes / Skins — Author a Work4You Color Theme

Author a Work4You **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`work4you_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.work4you/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Work4You itself.
...
- Write access to the Work4You home dir — `~/.work4you` by default, or `$WORK4YOU_HOME`
  / the active profile's dir. Skins live in `<work4you-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `work4you config set`).
...
3. `write_file` it to `<work4you-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `work4you config set display.skin default` (or they can `/skin default`).
...
```
work4you skin set <key> <hex>      # e.g. work4you skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `work4you skin set background "#08201f"` changes
only the background; `work4you skin use <name>` / `work4you skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.work4you`** when a profile is active — resolve the real home
  from `$WORK4YOU_HOME` first, falling back to `~/.work4you`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `work4you`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `work4you config set
...
- **You apply it, not the user.** `work4you config set display.skin <name>` is
...
- `read_file` the written `<work4you-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `work4you config get display.skin` and confirm it reports `<name>`.
