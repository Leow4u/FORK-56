...
# xAI's public API is OpenAI-compatible for the endpoints Work4You commonly
# uses. The Responses endpoint is included because Work4You' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Work4You-managed OAuth credentials."""

    auth_hint = "work4you auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`work4you auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`work4you auth reset xai-oauth` or re-authenticate with "
                    "`work4you auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `work4you auth add xai-oauth --type oauth`."
            )
