...
The ACP adapter wraps Work4You' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
- reject options -> Work4You `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Work4You tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Work4You' runtime resolver:

- `acp_adapter/auth.py`
- `work4you_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Work4You provider/credentials. It also always advertises a terminal setup auth method (`work4you-setup`, args `--setup`) so first-run ACP clients can open Work4You' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.work4you/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `work4you-acp` toolset definition
- `work4you_cli/main.py` — `work4you acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `work4you-acp` script
