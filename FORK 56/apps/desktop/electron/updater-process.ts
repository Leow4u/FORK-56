 ...
 *
 * The staged Tauri `work4you-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `work4you update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `work4you update`, swaps/relaunches
 * the app, and writes .work4you-update-result.json. With the app gone before
 * the update starts, the WORK4YOU_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `work4you update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into WORK4YOU_HOME on *every* platform
 * (`work4you-setup.exe` on Windows, `work4you-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_work4you_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `work4you update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(work4youHome, 'work4you-setup.exe')
 ...
 *
 * `copy_self_to_work4you_home` deliberately no-ops during `--update`
 ...
 * "Another Work4You update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `work4you update`, which
 ...
 * the installer is written to WORK4YOU_HOME at install/repair time, making mtime
