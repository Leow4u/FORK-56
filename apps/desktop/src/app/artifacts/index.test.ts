...
import type { SessionInfo, SessionMessage } from '@/types/work4you'
...
${payload}
    ...
    const artifacts = collectArtifactsForSession(makeSession({ id: 'native-browser-session' }), [
      {
        content: {
          ...
          meta: { screenshot_path: '/tmp/work4you-browser/native-screenshot.png' },
      ...
      {
        content: 'Image attached. Screenshot path: /tmp/work4you browser/summary screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_002,
        tool_name: 'browser_vision'
      },
      {
        content: 'Image attached. Screenshot path: C:\\Users\\Example User\\.work4you\\screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_003,
        tool_name: 'browser_vision'
      }
  ...
  it('resolves local file image artifacts through the desktop fs bridge', async () => {
    ...
    vi.stubGlobal('window', { work4youDesktop: { readFileDataUrl } })
  ...
  it('resolves remote image artifact thumbnails through the desktop fs bridge', async () => {
    ...
    vi.stubGlobal('window', { work4youDesktop: { api } })
    $connection.set({ baseUrl: 'https://gw', mode: 'remote', token: 'secret' } as never)

    const path = '/Users/me/.work4you/skills/work-esab/references/images/manual-step03.jpeg'

    await expect(artifactImageSrc(path)).resolves.toBe('data:image/jpeg;base64,cmVtb3Rl')

    expect(api).toHaveBeenCalledWith({
      path: '/api/fs/read-data-url?path=%2FUsers%2Fme%2F.work4you%2Fskills%2Fwork-esab%2Freferences%2Fimages%2Fmanual-step03.jpeg'
    })
  })
})
