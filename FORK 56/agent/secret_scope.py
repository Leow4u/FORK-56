...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the WORK4YOU_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Work4You runtime / deployment
    "WORK4YOU_HOME", "WORK4YOU_PROFILE", "WORK4YOU_GATEWAY_LOCK_DIR",
    "WORK4YOU_MAX_ITERATIONS", "WORK4YOU_MAX_TOKENS", "WORK4YOU_API_TIMEOUT",
    "WORK4YOU_REDACT_SECRETS", "WORK4YOU_WORK4YOU_TIMEOUT_SECONDS",
    "_WORK4YOU_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "WORK4YOU_KANBAN_DB", "WORK4YOU_KANBAN_WORKSPACES_ROOT", "WORK4YOU_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "WORK4YOU_KANBAN_",
    "WORK4YOU_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Work4You writes
    ...
    ``work4you_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Work4You parser: save_env_value
    ...
    from work4you_cli.config import _parse_env_value
...
def build_profile_secret_scope(work4you_home: Path) -> Dict[str, str]:
    ...
    try:
        from work4you_cli.env_loader import get_secret_source_values
