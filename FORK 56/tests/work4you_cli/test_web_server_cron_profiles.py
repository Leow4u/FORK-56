...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_work4you_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_work4you_home()
    ...
    outer_token = set_work4you_home_override(default_home)
    try:
        ...
        assert scheduler._get_work4you_home() == default_home
    finally:
...
def test_create_registers_scheduler_inside_target_profile(
    ...
    class RecordingProvider(CronScheduler):
        ...
        def register_job(self, job):
            captured["job"] = job
            captured["runtime_home"] = get_work4you_home()
            captured["jobs_file"] = cron_jobs._current_cron_store().jobs_file
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.work4you`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from work4you_cli import web_server

    monkeypatch.setenv(
        "WORK4YOU_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """WORK4YOU_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from work4you_cli import web_server

    monkeypatch.setenv("WORK4YOU_HOME", str(isolated_profiles["default"]))
