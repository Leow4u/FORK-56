...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Work4You core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Work4You native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Work4You native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Work4You native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Work4You usage patterns
...
- Do not silently choose Parallel when Work4You native tools are already sufficient.
