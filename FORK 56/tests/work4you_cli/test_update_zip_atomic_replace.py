...
which broke ``work4you --tui`` because ``ui-tui/`` had vanished.
...
from work4you_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.work4you-update-staging").exists()
    assert not (dst.parent / "ui-tui.work4you-update-old").exists()
