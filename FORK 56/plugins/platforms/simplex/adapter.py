"""SimpleX Chat platform adapter (Work4You plugin).
...
This adapter ships as a Work4You platform plugin under
``plugins/platforms/simplex/``. The Work4You plugin loader scans the
...
Optional environment variables:
    ...
    WORK4YOU_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``work4you setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Work4You — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("WORK4YOU_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``work4you setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.work4you/.env`` via
    ``work4you_cli.config``.
    """
    print()
