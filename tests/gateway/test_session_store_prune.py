...
def test_session_store_default_db_uses_runtime_work4you_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime WORK4YOU_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    work4you_state before a fixture redirected WORK4YOU_HOME used to pin every
    default SessionDB() at the developer's real ~/.work4you/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_work4you_home"
    fake_home.mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "work4you sessions list" in raw["_README"]
