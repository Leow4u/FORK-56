 ...
 *     `X-Work4You-Session-Token` header; WS uses `?token=`.
...
// work4you_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - work4you_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - work4you_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / work4you #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-work4you_session_at', '__Secure-work4you_session_at', 'work4you_session_at']
const RT_COOKIE_VARIANTS = ['__Host-work4you_session_rt', '__Secure-work4you_session_rt', 'work4you_session_rt']

// The Work4You portal (NAS) does NOT use Work4You gateway session cookies — it is a
...
/** Which Work4You profile the remote SSH dashboard should actually run as.
...
// Work4You Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteWork4YouPath = String(entry.remoteWork4YouPath || '').trim()

  if (remoteWork4YouPath) {
    out.remoteWork4YouPath = remoteWork4YouPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Work4You installation. Preserve an explicit mapping
  // when it is a valid Work4You profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_work4you_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `WORK4YOU_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `work4you:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://work4you.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `work4you:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a work4you session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live Work4You PORTAL (Privy) session — a non-empty
 ...
 * the Work4You gateway session cookies, so cloud sign-in / discovery liveness
