# Work4You Observer Hooks

Work4You observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Work4You' planner, model providers, memory, tool registry,
...
Work4You also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "work4you.observer.v1"
```

Hook callbacks are fail-open. Work4You catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Work4You only builds
