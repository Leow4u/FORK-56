#!/usr/bin/env bash
# Canonical test runner for work4you. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.work4you/...)
#
# Usage:
...
# (WORK4YOU_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.work4you/work4you/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.work4you/work4you/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${WORK4YOU_PYTHON:-}" ] && [ -x "$WORK4YOU_PYTHON" ] \
    && "$WORK4YOU_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: WORK4YOU_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `work4you` binary rather
  # than the devShell hook.
  PYTHON="$WORK4YOU_PYTHON"
  echo "▶ no local venv — using Nix dev venv via WORK4YOU_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and WORK4YOU_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.work4you/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.work4you"
...
#
#   * WORK4YOU_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * WORK4YOU_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     WORK4YOU_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# WORK4YOU_RUN_SLOW_PET_TESTS / WORK4YOU_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no WORK4YOU_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in WORK4YOU_TEST_IMAGE WORK4YOU_TEST_WORKERS WORK4YOU_TEST_PATHS \
  WORK4YOU_TEST_FILE_TIMEOUT WORK4YOU_TEST_FILE_RETRIES WORK4YOU_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${WORK4YOU_RUN_SLOW_PET_TESTS:+WORK4YOU_RUN_SLOW_PET_TESTS="$WORK4YOU_RUN_SLOW_PET_TESTS"} \
  ${WORK4YOU_E2E_BROWSER:+WORK4YOU_E2E_BROWSER="$WORK4YOU_E2E_BROWSER"} \
