...
Work4You can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `work4you model` menu entries
...
1. `work4you_cli/auth.py` decides how credentials are found.
2. `work4you_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `work4you_cli/models.py` and `work4you_cli/main.py` make the provider show up in the CLI. (`work4you_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`work4you_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `work4you setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$WORK4YOU_HOME/plugins/model-providers/` both get picked up.
...
7. `work4you model` menu includes the provider
8. `work4you setup` wizard delegates to `main.py` automatically
...
User plugins at `$WORK4YOU_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Work4You Portal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `work4you model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `work4you_cli/auth.py`
- `_PROVIDER_LABELS` in `work4you_cli/models.py`
- `_PROVIDER_ALIASES` in both `work4you_cli/auth.py` and `work4you_cli/models.py`
- CLI `--provider` choices in `work4you_cli/main.py`
...
## Step 2: Add auth metadata in `work4you_cli/auth.py`
...
- OAuth / auth-store path: Work4You, OpenAI Codex

Questions to answer here:

- What env vars should Work4You check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `work4you_cli/models.py`
...
## Step 4: Resolve runtime data in `work4you_cli/runtime_provider.py`
...
Be careful with API-key precedence. Work4You already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `work4you_cli/main.py`

A provider is not discoverable until it shows up in the interactive `work4you model` flow.

Update these in `work4you_cli/main.py`:
...
:::tip
`work4you_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `work4you model` and `work4you setup` automatically.
:::
...
When you add a native provider, double-check that Work4You is only sending fields that provider actually understands.
...
- `tests/work4you_cli/test_model_switch_custom_providers.py` (and adjacent `tests/work4you_cli/test_model_switch_*.py`)
...
- [ ] `ProviderConfig` added in `work4you_cli/auth.py`
- [ ] aliases added in `work4you_cli/auth.py` and `work4you_cli/models.py`
- [ ] model catalog added in `work4you_cli/models.py`
- [ ] runtime branch added in `work4you_cli/runtime_provider.py`
- [ ] CLI wiring added in `work4you_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `work4you model` but not `work4you setup`
