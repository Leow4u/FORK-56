...
``{title, url, description, position}`` rows every other Work4You web
provider produces.
...
prefers Work4You-managed xAI Grok OAuth (via ``work4you auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.work4you/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``work4you tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `work4you auth` to sign in with "
