...
```bash
work4you teams-pipeline validate
```
...
```bash
work4you teams-pipeline token-health
work4you teams-pipeline token-health --force-refresh
```
...
```bash
work4you teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
work4you teams-pipeline maintain-subscriptions
work4you teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Work4You cron (recommended if you already run the Work4You gateway)

Work4You ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.work4you/scripts/`. First create the script:

```bash
mkdir -p ~/.work4you/scripts
cat > ~/.work4you/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec work4you teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.work4you/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
work4you cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/work4you-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Work4You Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=work4you
EnvironmentFile=/etc/work4you/env
ExecStart=/usr/local/bin/work4you teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/work4you-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Work4You Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now work4you-teams-pipeline-maintain.timer
systemctl list-timers work4you-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/work4you teams-pipeline maintain-subscriptions >> /var/log/work4you/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.work4you/.env` at the top of a wrapper script that crontab calls.
...
```bash
work4you teams-pipeline subscriptions   # should show expirationDateTime advanced
work4you teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
work4you teams-pipeline list
work4you teams-pipeline list --status failed
work4you teams-pipeline show <job-id>
```

### Replay a stored job

```bash
work4you teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
work4you teams-pipeline fetch --meeting-id <meeting-id>
work4you teams-pipeline fetch --join-web-url "<join-url>"
work4you teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `work4you teams-pipeline maintain-subscriptions --dry-run`
- inspect `work4you teams-pipeline list --status failed`
...
- run `work4you teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `work4you teams-pipeline run`
...
- [ ] `work4you teams-pipeline validate` returns an OK snapshot
- [ ] `work4you teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Work4You cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
