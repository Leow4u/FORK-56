        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.work4youDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Work4You to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.work4youDesktop?.watchPreviewFile ||
      !window.work4youDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.work4youDesktop.onPreviewFileChanged(payload => {
    ...
    void window.work4youDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.work4youDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.work4youDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.work4youDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.work4youDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.work4youDesktop?.contextMenuCopyImage?.(),
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {isWebPreview && !isRemoteHtml && (
          <PreviewBrowserBar
            ...
            onOpenExternal={() => void window.work4youDesktop?.openExternal(currentUrl)}
