...
def curator_env(tmp_path, monkeypatch):
    """Isolated WORK4YOU_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from WORK4YOU_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.work4you once monkeypatch
    ...
    # thread here, while WORK4YOU_HOME is still pinned to this test's tmp home
...
def test_run_review_synchrowork4you_invokes_llm_stub(curator_env, monkeypatch):
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
# `work4you model` → auxiliary picker, the dashboard Models tab, and the full
