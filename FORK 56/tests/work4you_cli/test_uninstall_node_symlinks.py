"""Tests for work4you_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $WORK4YOU_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Work4You-managed node dir.
"""
...
def _make_work4you_node(work4you_home: Path) -> Path:
    """Create a fake $WORK4YOU_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = work4you_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    work4you_home = fake_home / ".work4you"
    _make_work4you_node(work4you_home)
    ...
    removed = uninstall.remove_node_symlinks(work4you_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    work4you_home = fake_home / ".work4you"
    node_bin = _make_work4you_node(work4you_home)
