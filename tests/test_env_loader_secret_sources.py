...
These cover the small public surface that lets `work4you model` / `work4you setup`
...
def test_cold_profile_hydration_seeds_op_env_bootstrap(tmp_path, monkeypatch):
    ...
    load_work4you_dotenv() reads <home>/.op.env for OP_SERVICE_ACCOUNT_TOKEN
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_work4you_dotenv()`` is called at module-import time from several
    ...
    runs exactly once per WORK4YOU_HOME per process.
    """
