...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>Work4You Work4You</p>')
    performScopedFind(surface, 'work4you', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'work4you', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>work4you</p>')
    performScopedFind(surface, 'work4you', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">Work4You</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>work4you</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'work4you')).toBe(true)
  })
