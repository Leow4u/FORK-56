...
@pytest.fixture()
def isolated_work4you_home():
    """Per-test WORK4YOU_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_work4you_home):
    from work4you_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_work4you_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_work4you_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_work4you_home):
    from work4you_cli.config import read_raw_config_readonly

    cfg = isolated_work4you_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
