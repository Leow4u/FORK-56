...
def _reset_work4you_time_cache():
...
class TestWork4YouTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_work4you_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["WORK4YOU_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["WORK4YOU_TIMEZONE"] = "UTC"
        result = work4you_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["WORK4YOU_TIMEZONE"] = "Europe/London"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When WORK4YOU_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) WORK4YOU_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("WORK4YOU_TIMEZONE=" + os.environ.get("WORK4YOU_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "WORK4YOU_TIMEZONE=NOT_SET" in result["output"], (
            "WORK4YOU_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When WORK4YOU_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("WORK4YOU_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=work4you_tz) which shifted
        absolute time when system-local tz != Work4You tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Work4You tz
        ...
        # Use a Work4You timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _work4you_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["WORK4YOU_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_work4you_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Work4You timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["WORK4YOU_TIMEZONE"] = "US/Eastern"
        _reset_work4you_time_cache()
