...
def test_resolve_stdio_command_falls_back_to_work4you_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"WORK4YOU_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$WORK4YOU_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Work4You Docker image
    ...
    # the other candidates ($WORK4YOU_HOME/node/bin/npx and ~/.local/bin/npx)
