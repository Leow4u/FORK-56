...
function routeIdentity(route: StoredRoute): null | string {
  if (route.kind === 'ssh') {
    ...
    return stableValue({
      ...
      kind: 'ssh',
      port: ssh.port || 22,
      remoteWork4YouPath: ssh.remoteWork4YouPath || '',
...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'WORK4YOU_DESKTOP_REMOTE_URL is set but WORK4YOU_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Work4You backend.'
      )
    }
