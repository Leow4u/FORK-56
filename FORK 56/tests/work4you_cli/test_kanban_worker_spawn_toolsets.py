...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    WORK4YOU_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_work4you_argv", lambda: ["work4you"])
    ...
    assert pid == 4242
    assert captured["env"]["WORK4YOU_HOME"] == str(profile)
    assert captured["env"]["WORK4YOU_KANBAN_TASK"] == "t_spawn_tools"
