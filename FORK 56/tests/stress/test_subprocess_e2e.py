...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves work4you_cli.kanban_db on its own
  ...
  - worker logs are captured to WORK4YOU_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "WORK4YOU_KANBAN_TASK": task.id,
            "WORK4YOU_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="work4you_e2e_")
    os.environ["WORK4YOU_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from work4you_cli import kanban_db as kb

    # Point the `work4you` CLI child processes will run at the worktree
    # work4you_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "work4you")
...
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/work4you_e2e_*/worker_*.log
    import glob
