...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Work4You's live source checkout "
        ...
        "checkout, stop Work4You, run the command externally, then restart "
        "Work4You."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(work4you_home).expanduser() if work4you_home else Path.home() / ".work4you"
