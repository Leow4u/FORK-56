...
const WORK4YOU_DIRECTIVE_RE = referenceRe()
...
export const work4youDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(WORK4YOU_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...work4youDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Work4You directives (`@file:...`, `@image:...`) as
