...
vi.mock('@/work4you', () => ({
  ...
  saveWork4YouConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye work4you', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye work4you')
  })
