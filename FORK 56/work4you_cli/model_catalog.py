...
The Work4You docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Work4You Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/Leow4u/FORK-56/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_work4you_home() / "cache" / "model_catalog.json"
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # this via get_curated_work4you_model_ids on every open) never block on the
...
def get_curated_work4you_models() -> list[str] | None:
    """Return Work4You Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("work4you")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Work4You silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`work4you update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``work4you update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.work4you/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``work4you model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
