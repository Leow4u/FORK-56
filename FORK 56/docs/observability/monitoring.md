...
Work4You gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Work4You-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `work4you.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `work4you.platform.up/degraded` with bounded `error_code` attributes |
...
`work4you.gateway.active_agents`, `work4you.gateway.background_work`, and
`work4you.gateway.background_delegations` are complementary. `active_agents`
...
```bash
work4you monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'work4you[otlp]'`),
...
a plain `work4you gateway run`: the exporter lives in the gateway process, so
...
`work4you.gateway.up`, `work4you.platform.up`, and `work4you.platform.degraded`.
...
absent_over_time(work4you_gateway_up[5m])

# Locally owned bridge is explicitly down.
work4you_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
work4you_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
work4you_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
work4you_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(work4you_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `work4you.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
work4you.status = failed|unknown
work4you.delivery_outcome = failed|not_configured
work4you.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `work4you.job_key`;
...
Work4You-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/work4you_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/work4you_otel_capture.jsonl --wait 8
...
`work4you.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
work4you monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `work4you monitoring` CLI intentionally exposes `status` only. This first
release covers only Work4You-owned service-health and operational-diagnostic
signals, including Work4You-owned Relay transport health. Team Gateway's
