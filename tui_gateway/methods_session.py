...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind WORK4YOU_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `WORK4YOU_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_work4you_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_work4you_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_work4you_home_override(init_home_token)
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds WORK4YOU_HOME (the
        ...
        # atexit.register(_drain_token_queue_at_exit) (work4you_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``work4you --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``work4you gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Work4You credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Work4You
    # or on any portal hiccup.
    try:
        from agent.account_usage import work4you_credits_lines

        credits = work4you_credits_lines()
        if credits:
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Work4You Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    from work4you_cli.work4you_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from work4you_cli.work4you_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from work4you_cli.work4you_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from work4you_cli.work4you_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from work4you_cli.work4you_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from work4you_cli.work4you_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from work4you_cli.work4you_billing import BillingError
        ...
        granted = step_up_work4you_billing_scope(
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Work4You profile home
    # (~/.work4you/sessions/saved/) rather than the project/workspace CWD, and
    ...
    path = saved_dir / f"work4you_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_work4you_home_override(parent_home) if parent_home else None
        )
