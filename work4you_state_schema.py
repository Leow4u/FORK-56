...
``work4you_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import work4you_state (cycle) — shared
module-level constants live in work4you_state_common.
"""
...
from work4you_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_CJK_STALE_KEY,
    FTS_STALE_KEY,
    FTS_SQL,
...
# Moved methods logged under the "work4you_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("work4you_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``work4you_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `work4you update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._work4you_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._work4you_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from work4you_constants import get_work4you_home
            cache_path = get_work4you_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `work4you sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `work4you db optimize`) must keep its EXISTING
