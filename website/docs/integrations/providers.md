...
This page covers setting up inference providers for Work4You — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Work4You.

## Inference Providers

You need at least one way to connect to an LLM. Use `work4you model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Work4You Portal** | `work4you model` (OAuth, subscription-based) |
| **OpenAI Codex** | `work4you model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `work4you model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `work4you model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `work4you model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.work4you/.env` |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.work4you/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.work4you/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.work4you/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.work4you/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.work4you/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.work4you/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.work4you/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.work4you/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **Actual Computer** | `ACTUAL_API_KEY` in `~/.work4you/.env` for the hosted relay, or `ACTUAL_BASE_URL=http://127.0.0.1:8080` for the local daemon — no key needed on loopback (provider: `actual`; aliases: `actual-computer`, `actualcomputer`, `aci`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.work4you/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.work4you/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.work4you/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `work4you model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.work4you/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.work4you/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.work4you/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.work4you/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.work4you/.env` (provider: `opencode-zen`) |
| **CommandCode** | `COMMANDCODE_API_KEY` in `~/.work4you/.env` (provider: `commandcode`, alias: `commandcode-chat`; Claude models via `commandcode-anthropic`, alias: `commandcode-claude`). Works with GOAT/Pro/Max/Provider plans (not the $1 Go plan — no API access). |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.work4you/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.work4you/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.work4you/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.work4you/.env` (provider: `gemini`) |
| **Google Vertex AI** | `work4you model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.work4you/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `work4you model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `work4you model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.work4you/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `work4you model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `work4you model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `work4you model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.work4you/.env` (provider: `stepfun`) |
| **LM Studio** | `work4you model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `work4you model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[Work4You Portal](https://portal.work4you.ai) is Work4You's unified subscription gateway and **the recommended way to run Work4You**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) — billed against your Work4You subscription instead of separate per-provider accounts.

```bash
work4you setup --portal     # fresh install — OAuth + provider + gateway in one command
work4you model              # existing install — pick "Work4You Portal" from the list
work4you portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [portal.work4you.ai/manage-subscription](https://portal.work4you.ai/manage-subscription).

**For full details:** see the dedicated [Work4You Portal integration page](/integrations/work4you-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Work4You with Work4You Portal guide](/guides/run-work4you-with-work4you-portal).

**Client identification.** Every Portal request from Work4You carries a `client=work4you-client-v<version>` tag (e.g. `client=work4you-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Work4You traffic from other clients. No config required; the tag updates automatically when you `work4you update`.

**JWT auth (automatic).** Work4You prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Work4You stores the resulting credentials in its own auth store under `~/.work4you/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Work4You marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `work4you auth add openai-codex` (or `work4you model` → **ChatGPT or Codex Subscription**) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Work4You Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Work4You routes these tasks to your **main chat model** — the same model you picked in `work4you model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Work4You Tool Gateway
Paid Work4You Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `work4you setup --portal` logs you in, sets Work4You as your provider, and turns the gateway on in one command. Existing users can enable it from `work4you model` or per-tool from `work4you tools`. Inspect routing at any time with `work4you portal info`.
:::

### Two Commands for Model Management

Work4You has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`work4you model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Work4You chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `work4you model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `work4you model`, complete the provider setup, then start a new session.


### Subscription plans: what your plan pays for

Several providers let you sign in to Work4You with a **consumer subscription** (Claude Max, ChatGPT, SuperGrok / X Premium+, …) instead of an API key. What that subscription actually pays for — and what it doesn't — differs per provider, and it's the single most common source of billing surprises. The table below is the short version; each provider's own section has the details.

> Cells marked *not currently documented* mean exactly that: Work4You docs do not yet specify the behavior. Don't assume — check your provider's billing dashboard, and treat these as open questions.

| Plan / path | Can Work4You use it? | What gets consumed | What does NOT get consumed | Common surprise |
|---|---|---|---|---|
| **Anthropic — Claude Max + OAuth** | ✅ Yes — `work4you model` → Anthropic OAuth. Requires Max **and** purchased extra usage credits | The **extra/overage credits** you've added on top of the Max plan | The **base Max plan allowance** (the usage included in Claude Code by default) | All Work4You usage bills as "extra usage" even while your included Max allowance sits untouched |
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `work4you model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
| **Google — Gemini consumer plan (Google AI Pro / Ultra)** | ❌ No documented path — the `gemini` provider is API-key only (`GOOGLE_API_KEY` / `GEMINI_API_KEY`); Vertex AI uses GCP billing | Your **API key's quota** (free tier or billing-enabled Google Cloud project) — *consumer-plan consumption not currently documented* | *Not currently documented* | Free-tier keys can be exhausted after a handful of agent turns, because Work4You may make several model calls per user turn |

**Anthropic.** The OAuth path routes as Claude Code against your Anthropic account and **only works on a Claude Max plan with purchased extra usage credits** — the base Max allowance is never consumed by Work4You, only the extra/overage credits on top. Claude Pro subscribers cannot use this path; the supported alternative is an `ANTHROPIC_API_KEY`, billed pay-per-token against that key's organization at standard API pricing. See [Anthropic (Native)](#anthropic-native) below.

**OpenAI Codex.** Work4You authenticates via ChatGPT device-code OAuth, stores credentials in `~/.work4you/auth.json`, and can import existing Codex CLI credentials from `~/.codex/auth.json`. Which ChatGPT plan tiers are eligible, and how Work4You usage counts against your plan's Codex limits, are **not currently documented** — the Codex note under [Work4You Portal](#work4you-portal) covers authentication and token-refresh behavior only.
...
**Google Gemini.** There is currently no way to sign in to Work4You with a consumer Gemini subscription — the `gemini` provider takes an API key, and [Google Vertex AI](#google-vertex-ai) bills to your GCP project. A billing-enabled Google Cloud project is recommended for agent use; free-tier quotas are too small for long-running agent sessions. See the [Google Gemini guide](/guides/google-gemini).

:::tip One subscription instead of five
If you'd rather not track per-provider plan semantics at all, [Work4You Portal](#work4you-portal) covers 300+ models under a single subscription with one OAuth login.
:::
...
When you authenticate via `work4you model` → Anthropic OAuth (or via `work4you auth add anthropic --type oauth`), Work4You routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Work4You — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
# Preferred: authenticate through `work4you model`
# Work4You will use Claude Code's credential store directly when available
work4you model
...
work4you chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `work4you model`, Work4You prefers Claude Code's own credential store over copying the token into `~/.work4you/.env`. That keeps refreshable Claude credentials refreshable.
...
Work4You supports GitHub Copilot as a first-class provider with two modes:
...
If no token is found, `work4you model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `work4you model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `work4you model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Work4You
Work4You sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Work4You now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Work4You therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
| `WORK4YOU_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `WORK4YOU_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
# Requires: FIREWORKS_API_KEY in ~/.work4you/.env

# NovitaAI Model API
work4you chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.work4you/.env

# z.ai / ZhipuAI GLM
work4you chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.work4you/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
work4you chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.work4you/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
work4you chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.work4you/.env

# MiniMax (global endpoint)
work4you chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.work4you/.env

# MiniMax (China endpoint)
work4you chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.work4you/.env

# Qwen Cloud / DashScope (Qwen models)
work4you chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.work4you/.env

# Xiaomi MiMo
work4you chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.work4you/.env

# Tencent TokenHub (Hy3 Preview)
work4you chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.work4you/.env

# Arcee AI (Trinity models)
work4you chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.work4you/.env

# Meta Model API (Muse Spark family)
work4you chat --provider meta-ai --model muse-spark-1.2
# Requires: MODEL_API_KEY in ~/.work4you/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
work4you chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.work4you/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `work4you model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, Work4You automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.work4you/.env` and pick xAI in `work4you model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `work4you model`, or run `work4you auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Work4You runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Work4You automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `work4you tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `work4you doctor` and `work4you chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `work4you migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
work4you migrate xai          # preview replacements
work4you migrate xai --apply  # rewrite ~/.work4you/config.yaml in place
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `work4you model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Work4You auto-discovers the available models.
...
work4you model   # → "Google Vertex AI" → project → region → model
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Work4You lazy-installs `google-auth` on first use; run `work4you setup` if the managed install needs repair. See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `work4you model`, sign in through the browser, and Work4You persists the refresh token.

```bash
work4you model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.work4you/auth.json

work4you chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `WORK4YOU_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Work4You exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `work4you model`, sign in through the browser, and Work4You persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
work4you chat   # uses api.minimax.io/anthropic endpoint
```
...
# Requires: NVIDIA_API_KEY in ~/.work4you/.env
...
Work4You automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
Your own hardware as a private inference cluster via [Actual Computer](https://actual.inc). Two serving modes, both OpenAI-compatible (Work4You uses the Responses API transport):
...
- **Local daemon** — on-device at `http://127.0.0.1:8080`, fully offline. No API key needed: Work4You detects the loopback base URL and authenticates with an internal placeholder automatically.

```bash
# Hosted relay (ACTUAL_API_KEY in ~/.work4you/.env)
work4you chat --provider actual --model <model-id-from-your-cluster>

# Local daemon (ACTUAL_BASE_URL=http://127.0.0.1:8080 in ~/.work4you/.env, no key)
...
Notes:
- Model IDs come from your cluster's `GET /v1/models` — discover with `work4you model` or `curl -s https://api.actual.inc/v1/models -H "Authorization: Bearer $ACTUAL_API_KEY"`.
...
- Small local models: Work4You' full default toolset plus the system prompt can exceed a 32k context window, producing an empty-stream error from llama.cpp-family servers. Restrict the toolset (`-t file,web`) or load the model with a larger context. The optional `actual-setup` skill (`work4you skills install official/devops/actual-setup`) covers setup and troubleshooting in detail.
...
# Requires: STEPFUN_API_KEY in ~/.work4you/.env
```
...
# Requires: HF_TOKEN in ~/.work4you/.env
...
Work4You works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Work4You at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `work4you model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `work4you setup` or config migration.
:::
...
**`work4you model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Work4You chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `work4you model`, set up the new provider, then start a new session.
:::
...
When switching providers, Work4You persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Work4You requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Work4You.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Work4You) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `work4you` (Qwen 2.5, Work4You 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** Work4You preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Work4You. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below Work4You' minimum per active session.

Then configure Work4You to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Work4You won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Work4You 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Work4You preserves the context of an already-loaded LM Studio instance. For an unloaded model in the default explicit mode, Work4You omits `context_length` unless you configured one in Work4You, so LM Studio can apply its own model setting. Work4You then uses only the context length LM Studio reports after loading.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip Work4You' explicit preload step:
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Work4You) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Work4You requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Work4You config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Work4You config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Work4You.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Work4You inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser work4you` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Work4You's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Work4You thinks the context is
...
Work4You auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Work4You uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Work4You with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Work4You.
...
Then configure Work4You with `work4you model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Work4You with `work4you model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `work4you model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Work4You uses this to decide when to compress history and to validate API requests.
...
Work4You uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
7. **Work4You Portal** — suffix-matches Work4You model IDs against OpenRouter metadata
...
`work4you model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
providers:
  local:
    ...
    # api_key omitted — Work4You uses "no-key-required" for keyless local servers
  work:
    api: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    transport: chat_completions   # set explicitly by `work4you model` → Custom Endpoint wizard; auto-detection still happens as a fallback
  anthropic-proxy:
...
:::note Legacy format
Older configs used a top-level `custom_providers:` list instead. It still works — Work4You reads both — and `work4you update` auto-migrates it to the `providers:` dict (config v12). Field names differ slightly in the dict format: legacy `model` is `default_model`, and legacy `api_mode` is `transport`.
:::

Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Work4You will merge it into each chat-completions request for that endpoint:
...
The `work4you model` → Custom Endpoint wizard now prompts for the API mode explicitly and persists your answer to `config.yaml` (as `transport` on the provider entry). URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Work4You routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `work4you model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under the `providers:` dict. Three worked recipes follow. Each drops into `~/.work4you/config.yaml` and the matching API key goes in `~/.work4you/.env`.
...
Together's `/v1/models` endpoint works, so `work4you model` can auto-discover available models.
...
:::tip Troubleshooting
- `work4you doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `work4you model` will persist the model with a warning rather than hard-reject — see #15136.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or Work4You Portal |
...
:::tip
You can switch between providers at any time with `work4you model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Work4You at your instance (no API key needed):
   ```bash
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.work4you/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.work4you/config.yaml`:
...
Configure a chain of backup providers Work4You tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
Supported providers: `openrouter`, `work4you`, `novita`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `qwen-oauth`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `bedrock`, `ai-gateway`, `azure-foundry`, `opencode-zen`, `opencode-go`, `commandcode`, `commandcode-anthropic`, `kilocode`, `xiaomi`, `arcee`, `gmi`, `actual`, `stepfun`, `lmstudio`, `alibaba`, `alibaba-coding-plan`, `tencent-tokenhub`, `custom`.

:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `work4you fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
