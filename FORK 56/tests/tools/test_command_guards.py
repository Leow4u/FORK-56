...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects WORK4YOU_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("WORK4YOU_INTERACTIVE", "WORK4YOU_GATEWAY_SESSION", "WORK4YOU_EXEC_ASK", "WORK4YOU_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["WORK4YOU_INTERACTIVE"] = "1"
        session_key = os.getenv("WORK4YOU_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No WORK4YOU_INTERACTIVE or WORK4YOU_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
