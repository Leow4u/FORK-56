...
Covers:
- :class:`Work4YouTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.Work4YouMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _WORK4YOU_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# Work4YouTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = Work4YouTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
        storage = Work4YouTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (Work4YouMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: Work4YouTokenStorage, **context_attrs):
    ...
    provider = _WORK4YOU_PROVIDER_CLS.__new__(_WORK4YOU_PROVIDER_CLS)
    provider._work4you_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
        storage = Work4YouTokenStorage("mgr-srv")
        ...
        with patch.object(
            _WORK4YOU_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
        storage = Work4YouTokenStorage("flow-srv")
        ...
        with patch.object(
            _WORK4YOU_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
