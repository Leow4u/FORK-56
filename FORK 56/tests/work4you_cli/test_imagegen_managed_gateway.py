...
(non-managed) routing. When a user picked FAL through the Work4You-subscription
...
the user's personal FAL_KEY instead of the Work4You Tool Gateway (real incident:
...
exactly ONE provider string per category — ``image_gen.provider: work4you`` for
the managed "Work4You Subscription" row, ``image_gen.provider: fal`` for the
BYOK FAL row — and any legacy ``use_gateway`` key is popped so the
read-time shim (use_gateway: true ⇒ work4you) cannot override the fresh pick.
...
def test_image_gen_selector_preserves_managed_selection(monkeypatch):
    """Managed pick: the 'work4you' provider string must survive the selector."""
    _quiet(monkeypatch)
    config = {}
...
def test_image_and_video_selectors_share_the_selection_contract(monkeypatch):
    ...
    for use_gateway, expected in ((True, "work4you"), (False, "fal")):
        config = {
...
def _quiet_reconfigure(monkeypatch):
    ...
    monkeypatch.setattr(ns, "ensure_work4you_portal_access", lambda **k: True)
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a Work4You Subscription user re-entering the
...
def test_reconfigure_direct_fal_row_writes_vendor_selection(monkeypatch):
    ...
    legacy use_gateway key so the read-time shim can't resurrect 'work4you'."""
    _quiet_reconfigure(monkeypatch)
    import work4you_cli.tools_config as tc
