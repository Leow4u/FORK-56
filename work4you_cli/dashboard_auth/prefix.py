...
prefix (e.g. ``mission-control.tilos.com/work4you/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /work4you`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``WORK4YOU_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``work4you.example.com`` instead of
    ``https://work4you.example.com``) — the single most common cause of
    "I set WORK4YOU_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "work4you.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/work4you"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# WORK4YOU_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``WORK4YOU_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("WORK4YOU_DASHBOARD_PUBLIC_URL", "")
