...
_WORK4YOU_MD_NAMES = (".work4you.md", "WORK4YOU.md")


def _find_work4you_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.work4you.md`` or ``WORK4YOU.md``.
    ...
    # could pick up a .work4you.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _WORK4YOU_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Work4You, an intelligent AI assistant created by Work4You. "
...
WORK4YOU_HELP_GUIDANCE = (
    "You run on Work4You (by Work4You). When the user needs help with "
    "Work4You itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://work4you.ai/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `work4you` skill with skill_view(name='work4you') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.work4you/kanban.db`. Your task id is in "
    "`$WORK4YOU_KANBAN_TASK`; your workspace is `$WORK4YOU_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $WORK4YOU_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $WORK4YOU_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${WORK4YOU_KANBAN_BRANCH:-wt/$WORK4YOU_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$WORK4YOU_KANBAN_BRANCH` a deterministic "
...
#
# The work4you runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to work4you's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Work4You YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Work4You launches that runtime.\n\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Work4You floating over another application, so the
    user is rarely asking about Work4You — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Work4You "
        ...
        "rather than anything inside Work4You. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "desktop": (
        "You are chatting inside the Work4You desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.work4you.send(\"prompt\") — or a "
        "data-work4you-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Work4You WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Work4You itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Work4You runs on.
    """
...
def build_environment_hints() -> str:
    ...
    # Embedder-supplied environment description. Lets a host that wraps Work4You
    ...
    extra = (os.getenv("WORK4YOU_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_work4you_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("WORK4YOU_PLATFORM") or os.environ.get("WORK4YOU_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.work4you/skills/`` directory.  External dirs
    ...
    # WORK4YOU_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_work4you_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.work4you/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_work4you_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the `work4you` skill "
...
def build_work4you_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Work4You subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Work4You subscription helper: %s", exc)
        return ""

    if not managed_work4you_tools_enabled():
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via Work4You subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.work4you_auth_present:
            return f"- {feature.label}: included with Work4You subscription, not currently selected"
        if feature.key == "modal" and features.work4you_auth_present:
            return f"- {feature.label}: optional via Work4You subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Work4You Subscription",
        "Work4You subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
    ...
    lines.extend(
        [
            ...
            "If the user is not subscribed and asks for a capability that Work4You subscription would unlock or simplify, suggest Work4You subscription as one option alongside direct setup or local alternatives.",
...
def load_soul_md(
    ...
    """Load SOUL.md from WORK4YOU_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the WORK4YOU_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from work4you_cli.config import ensure_work4you_home
        ensure_work4you_home()
    except Exception as e:
        logger.debug("Could not ensure WORK4YOU_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_work4you_home()
...
def _load_work4you_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.work4you.md / WORK4YOU.md — walk to git root."""
    work4you_md_path = _find_work4you_md(cwd_path)
    if not work4you_md_path:
        return ""
    try:
        content = work4you_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = work4you_md_path.name
        try:
            rel = str(work4you_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".work4you.md", context_length=context_length,
            read_path=str(work4you_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", work4you_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .work4you.md / WORK4YOU.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from WORK4YOU_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Work4You install/source
    ...
    # explicitly configured cwd is honored verbatim — the Work4You tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Work4You in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Work4You install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_work4you_md(cwd_path, context_length)
    ...
    # SOUL.md from WORK4YOU_HOME only — skip when already loaded as identity
    if not skip_soul:
