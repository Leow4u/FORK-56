"""Regression tests for the parked-branch guard in ``work4you update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``work4you
...
def _no_config(monkeypatch):
    ...
    import work4you_cli.config as work4you_config

    monkeypatch.setattr(work4you_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && work4you update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(work4you_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(work4you_main, "PROJECT_ROOT", repo)
    monkeypatch.setattr(work4you_main, "_resolve_update_branch", lambda args: "main")
    ...
    monkeypatch.setattr(work4you_main, "_discard_lockfile_churn", lambda *a, **k: None)
    ...
    monkeypatch.setattr(work4you_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    monkeypatch.setattr(work4you_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(work4you_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        work4you_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(work4you_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(work4you_main, "_capture_active_tool_dependencies", lambda: [])
