"""Migrate Work4You' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Work4You' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Work4You mcp_servers.<n>.command/args/env  → codex stdio transport
  Work4You mcp_servers.<n>.url/headers       → codex streamable_http transport
  Work4You mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Work4You mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Work4You-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Work4You' own
...
_KEYS_DROPPED_WITH_WARNING = {
    # Work4You' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, work4you_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Work4You MCP server config to the codex inline-table dict
    ...
    if not isinstance(work4you_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(work4you_cfg.get("command"))
    has_url = bool(work4you_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (WORK4YOU_HOME, PYTHONPATH) could
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Work4You' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Work4You' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Work4You
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="work4you-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a WORK4YOU_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed work4you-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.work4you`` symlink that happens to live under ``/private/var/folders``
...
def _build_work4you_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Work4You' own
    ...
    so a work4you installed under /opt/, /usr/local/, or a venv all work.
    WORK4YOU_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # WORK4YOU_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_work4you_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever WORK4YOU_HOME its launcher sets
    ...
    # — that would override the launcher's WORK4YOU_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("WORK4YOU_HOME", tmp_path) would
    ...
    work4you_home = os.environ.get("WORK4YOU_HOME") or ""
    if work4you_home and _looks_like_test_tempdir(work4you_home):
        work4you_home = ""
    if work4you_home:
        env["WORK4YOU_HOME"] = work4you_home
    # PYTHONPATH passes through so a worktree-launched work4you finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["WORK4YOU_QUIET"] = "1"
    env["WORK4YOU_REDACT_SECRETS"] = env.get("WORK4YOU_REDACT_SECRETS", "true")
...
def migrate(
    work4you_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_work4you_tools: bool = True,
) -> MigrationReport:
    """Translate Work4You mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        work4you_config: full ~/.work4you/config.yaml dict
        ...
        expose_work4you_tools: when True (default), register Work4You' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Work4You for tools
    ...
    work4you_servers = (work4you_config or {}).get("mcp_servers") or {}
    if not isinstance(work4you_servers, dict):
    ...
    for name, cfg in work4you_servers.items():
    ...
    # Inject Work4You' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Work4You for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/work4you_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_work4you_tools:
        translated["work4you-tools"] = _build_work4you_tools_mcp_entry()
        if "work4you-tools" not in report.migrated:
            report.migrated.append("work4you-tools")
