...
Ledger location: `$WORK4YOU_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `WORK4YOU_CITATION_LEDGER`.

## How to Run

```bash
S=~/.work4you/skills/research/grounded-citations/scripts/sources.py
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `WORK4YOU_CITATION_LEDGER`) if their
