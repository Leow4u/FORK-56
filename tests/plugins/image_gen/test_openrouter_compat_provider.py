#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Work4You)."""
...
_RUNTIME = "work4you_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "work4you"}
    ...
    def test_display_names(self):
        ...
        assert by_name["work4you"].display_name == "Work4You Portal"
    ...
    def test_work4you_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        work4you = {p.name: p for p in _build_providers()}["work4you"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert work4you._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestLiveCatalog:
    ...
    def test_work4you_portal_picker_excludes_image_api_catalog(self):
        """Work4You Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        work4you = {p.name: p for p in _build_providers()}["work4you"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in work4you.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """Work4You routes to its own base URL — proves the same code serves both."""
        work4you_runtime = _runtime_ok(
            provider="work4you", base_url="https://inference.work4you.ai/v1", api_key="work4you-tok"
        )
        with patch(_RUNTIME, return_value=work4you_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            work4you = {p.name: p for p in _build_providers()}["work4you"]
            result = work4you.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "work4you"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.work4you.ai/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_work4you_never_uses_the_image_api(self):
        """Work4You Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        with patch(_RUNTIME, return_value=work4you_runtime), \
             ...
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            work4you = {p.name: p for p in _build_providers()}["work4you"]
            result = work4you.generate(prompt="a pet", model="openai/gpt-image-2")

        assert result["success"] is True
        assert mock_post.call_args[0][0] == "https://inference.work4you.ai/v1/chat/completions"
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        work4you_ids = {m["id"] for m in by_name["work4you"].list_models()}
        assert "openai/gpt-image-2" in openrouter_ids
        assert set(_IMAGE_API_MODELS) <= openrouter_ids
        assert not (set(_IMAGE_API_MODELS) & work4you_ids)
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "work4you"}
    ...
    def test_both_are_reference_capable_for_pets(self):
        ...
        assert "work4you" in _REF_CAPABLE
