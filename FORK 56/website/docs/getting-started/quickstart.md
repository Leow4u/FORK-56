...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Work4You — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Work4You setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Work4You Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Work4You Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Work4You for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Work4You working on my machine | `work4you setup` | Run a real chat and verify it responds |
| I already know my provider | `work4you model` | Save the config, then start chatting |
| I want a bot or always-on setup | `work4you gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `work4you model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `work4you model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Work4You cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Work4You
### With the Work4You Desktop installer on macOS or Windows (recommended)
...
The single most important setup step. Use `work4you model` to walk through the choice interactively:

```bash
work4you model
```

:::tip Easiest path: Work4You Portal
...
That logs you in, sets Work4You as your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `work4you setup` offers three modes:

- **Quick Setup (Work4You Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `work4you update`. Re-enable anything later with `work4you tools`, seed skills with `work4you skills opt-in --sync`, or tune settings with `work4you setup agent`.
:::
...
|----------|-----------|---------------|
| **Work4You Portal** | Subscription-based, zero-config | OAuth login via `work4you model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `work4you model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `work4you model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `work4you_cli/models.py` may change between releases) | `work4you model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `work4you model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `work4you model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `work4you model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `work4you model` (requires `copilot` CLI + `copilot login`) |
...
Work4You requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `work4you model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Work4You separates secrets from normal config:

- **Secrets and tokens** → `~/.work4you/.env`
- **Non-secret settings** → `~/.work4you/config.yaml`
...
```bash
work4you            # classic CLI
work4you --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Work4You ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `work4you` vs `work4you --tui`.
:::
...
- Work4You replies without error
...
```bash
work4you --continue    # Resume the most recent session
work4you -c            # Short form
```
...
```bash
work4you gateway setup    # Interactive platform configuration
```
...
- `work4you tools` — tune tool access per platform
- `work4you skills` — browse and install reusable workflows
...
```bash
work4you config set terminal.backend docker    # Docker isolation
work4you config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `work4you egress setup && work4you egress start`; `work4you setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Work4You install directory (the curl installer placed it at
# ~/.work4you/work4you on Linux/macOS or %LOCALAPPDATA%\work4you\work4you on Windows):
...
Skills are on-demand instruction documents that teach Work4You how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Work4You ships with a catalog of bundled skills already installed in `~/.work4you/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
work4you skills browse                      # list everything available
work4you skills search kubernetes           # find skills by keyword
work4you skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `work4you skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Work4You ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.work4you/work4you && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Work4You opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `work4you model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `work4you gateway setup` and check `work4you gateway status` |
| `work4you --continue` can't find old session | Switched profiles or session never saved | Check `work4you sessions list` and confirm you're in the right profile |
...
| `work4you doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `work4you tools` | Configure which tools are enabled per platform |
| `work4you setup` | Full setup wizard (configures everything at once) |
| `work4you doctor` | Diagnose issues |
| `work4you update` | Update to latest version |
| `work4you gateway` | Start the messaging gateway |
| `work4you --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-work4you-to-another-machine)** — `work4you backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
