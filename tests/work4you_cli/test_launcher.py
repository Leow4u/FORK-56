"""Tests for the top-level `./work4you` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./work4you` should use `work4you_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "work4you"
    called = []

    fake_main_module = types.ModuleType("work4you_cli.main")

    def fake_main():
        called.append("work4you_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "work4you_cli.main", fake_main_module)
    ...
    assert called == ["work4you_cli.main"]
