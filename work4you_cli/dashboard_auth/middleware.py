...
from work4you_cli.dashboard_auth.base import (
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /work4you``, the
    ``login_url`` is prefixed (``/work4you/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from work4you_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["work4you.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``work4you_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``work4you_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `work4you` provider first, which
        # tries to reach Work4You Portal's JWKS; if that's unreachable it raises,
