"""
Gateway subcommand for work4you CLI.

Handles: work4you gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_work4you_home is imported lazily at call sites to avoid ImportError
# when work4you_constants is cached from a pre-update version during `work4you update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "work4you CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `work4you update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``work4you gateway status`` from falsely counting the ``work4you`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``work4you gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_work4you_home().resolve())
    # Forward slashes on both sides of the WORK4YOU_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"work4you_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. WORK4YOU_HOME
        ...
        # WORK4YOU_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "work4you_home=" in command_lc
            and f"work4you_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``work4you update`` hung exactly there
            ...
            # watchdog probe.
            from work4you_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``work4you update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Work4You profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Work4You profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "work4you_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m work4you_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``work4you update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Work4You's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose WORK4YOU_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``work4you update`` users
    # with no running gateway until they re-invoke ``work4you gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (WORK4YOU_HOME,
    ...
    if sys.platform == "win32":
        try:
            from work4you_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / WORK4YOU_HOME) the windowless
        # base interpreter needs to import work4you_cli.  Empty on POSIX, where
...
def _work4you_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``WORK4YOU_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("WORK4YOU_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_work4you_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``WORK4YOU_HOME``.

    Under ``sudo``, ``WORK4YOU_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_work4you_home` falls back to ``/root/.work4you`` — the wrong
    profile. The unit file pins ``WORK4YOU_HOME`` for the actual gateway
    ...
    unit_home = (_work4you_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("WORK4YOU_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("WORK4YOU_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["WORK4YOU_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from work4you_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``work4you gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from work4you_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (work4you_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Work4You_Gateway /
            # Work4You_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `work4you gateway install` deployment.
            from work4you_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Work4You_Gateway"
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (WORK4YOU_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Work4You Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``work4you gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``WORK4YOU_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("WORK4YOU_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from work4you_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current WORK4YOU_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (WORK4YOU_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_work4you_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary WORK4YOU_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(work4you_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when WORK4YOU_HOME is a named profile.

    For ``~/.work4you/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        work4you_home: Optional explicit WORK4YOU_HOME path. Defaults to the current
            ``get_work4you_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Work4You root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_work4you_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from work4you_constants import get_default_work4you_root

    home = Path(work4you_home or str(get_work4you_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_work4you_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(work4you_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".work4you"
    try:
        Path(work4you_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(work4you_home, default_root=target_root)
    except ValueError:
        return _profile_arg(work4you_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this WORK4YOU_HOME.

    Default ``~/.work4you`` returns ``work4you-gateway`` (backward compatible).
    Profile ``~/.work4you/profiles/coder`` returns ``work4you-gateway-coder``.
    Any other WORK4YOU_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Work4You installs that predate the
# work4you-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (work4you-gateway-*.service) and unrelated third-party
# "work4you" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("work4you.service",)
...
def _find_legacy_work4you_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Work4You gateway units.

    Detects unit files installed by older Work4You versions that used a
    different service name (e.g. ``work4you.service`` before the rename to
    ``work4you-gateway.service``). When both a legacy unit and the current
    ``work4you-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``work4you-gateway-coder.service`` and unrelated third-party
      ``work4you-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``work4you.service`` running an unrelated
...
def has_legacy_work4you_units() -> bool:
    """Return True when any legacy Work4You gateway unit files exist."""
    return bool(_find_legacy_work4you_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Work4You gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_work4you_units()
    if not legacy:
        return
    print_warning("Legacy Work4You gateway unit(s) detected from an older install:")
...
def remove_legacy_work4you_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Work4You gateway unit files.

    Iterates over whatever ``_find_legacy_work4you_units()`` returns — which is
    ...
    legacy = _find_legacy_work4you_units()
    if not legacy:
        print("No legacy Work4You gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Work4You gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``work4you setup`` (end of wizard)
    and ``work4you import``: if this host supports a service manager and no
    ...
    ``work4you gateway install --system`` explicitly (that path prompts and
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: work4you gateway")
        return False
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from work4you_cli import gateway_windows
        ...
        print_info("  You can retry manually: work4you gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Work4You often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.work4you`` → ``ai.work4you.gateway.plist`` (backward compatible).
    Profile ``~/.work4you/profiles/coder`` → ``ai.work4you.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # work4you_constants cached from before this symbol existed. See
            # _reload_work4you_constants() in work4you_cli/managed_uv.py.
            from work4you_cli.managed_uv import _reload_work4you_constants

            venv_python_path = _reload_work4you_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.work4you/work4you  -> /home/alice/.work4you/work4you
...
def _work4you_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current WORK4YOU_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_work4you_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.work4you                    → /home/alice/.work4you
      /root/.work4you/profiles/coder     → /home/alice/.work4you/profiles/coder
      /opt/custom-work4you               → /opt/custom-work4you  (kept as-is)
    """
    current_work4you_raw = os.environ.get("WORK4YOU_HOME", "").strip()
    current_work4you = (
        Path(current_work4you_raw).expanduser()
        if current_work4you_raw
        else get_work4you_home()
    )
    ...
    # different WORK4YOU_HOME into the generated service unit.
    current_default = Path.home() / ".work4you"
    target_default = Path(target_home_dir) / ".work4you"

    # Default ~/.work4you → remap to target user's default
    if current_work4you == current_default:
        return str(target_default)

    # Profile or subdir of ~/.work4you → preserve the relative structure
    try:
        relative = current_work4you.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.work4you) — keep as-is
        return str(current_work4you)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    work4you_node = work4you_home / "node" / "bin"
    if _is_dir(work4you_node):
        candidates.append(str(work4you_node))
    work4you_nm = work4you_home / "node_modules" / ".bin"
    if _is_dir(work4you_nm):
        candidates.append(str(work4you_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m work4you_cli.main``, so module
    ...
    dir, or a clone that ``work4you update`` later relocates/removes — the path
    ...
    ``WORK4YOU_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if WORK4YOU_HOME
...
def _systemd_watchdog_seconds(work4you_home: str | Path | None = None) -> int:
    ...
    if work4you_home is not None:
        ...
        override_token = set_work4you_home_override(work4you_home)
        reset_home_override = reset_work4you_home_override
    try:
...
def _systemd_watchdog_service_fields(
    work4you_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(work4you_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], work4you_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Work4You-managed Node under ``$WORK4YOU_HOME/node`` goes first when it
    ...
    *work4you_root* is the Work4You home the unit will run against. System units
    ...
    """
    from work4you_constants import (
        work4you_managed_node_tree_present,
        iter_work4you_node_dirs,
    )

    managed_node_present = work4you_managed_node_tree_present(work4you_root)
    for directory in iter_work4you_node_dirs(work4you_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target Work4You home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Work4You home is known — probing here would stat the calling
    ...
    if system:
        ...
        work4you_home = _work4you_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            work4you_home
        )
        profile_arg = _profile_arg_for_target_user(work4you_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's WORK4YOU_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(work4you_home) if work4you_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped work4you_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(work4you_home) if work4you_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m work4you_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="WORK4YOU_HOME={work4you_home}"
Restart=always
RestartSec=5
...
"""
    ...
    work4you_home = str(get_work4you_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        work4you_home
    )
    profile_arg = _profile_arg(work4you_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m work4you_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="WORK4YOU_HOME={work4you_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned WORK4YOU_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo work4you gateway … --system``, WORK4YOU_HOME is often stripped
    # and falls back to ``/root/.work4you``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / WORK4YOU_HOME comparisons
    ...
    _sync_work4you_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir WORK4YOU_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved WORK4YOU_HOME
    ...
    exported a throwaway ``WORK4YOU_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``work4you gateway restart``
    with ``WORK4YOU_HOME=/tmp/work4you-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="WORK4YOU_HOME=..."`` lines and launchd
    ``<key>WORK4YOU_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'WORK4YOU_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>WORK4YOU_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp WORK4YOU_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: WORK4YOU_HOME resolves to a "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single WORK4YOU_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only WORK4YOU_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp WORK4YOU_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../work4you_test`` path into
    # ``Environment="WORK4YOU_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/work4you_test"' in new_unit
        or "/work4you_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # WORK4YOU_HOME into the unit (manual E2E homes like /tmp/work4you-e2e-NNN
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("WORK4YOU_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("WORK4YOU_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (work4you.service from pre-rename installs)
    # before installing the new work4you-gateway.service. If both remain, they
    ...
    if has_legacy_work4you_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_work4you_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin WORK4YOU_HOME; adopt it before any
    ...
    # ``sudo work4you gateway install --system --force`` would bake /root/.work4you
...
def systemd_start(system: bool = False):
    ...
    # WORK4YOU_HOME sync happens inside refresh_systemd_unit_if_needed's
...
# process invocation (each ``work4you gateway start/stop/status`` call).
...
#      background process (the `nohup work4you gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_work4you_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_work4you_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m work4you_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active WORK4YOU_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "work4you_cli.main"]
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `work4you gateway start`)
    ...
    nofile_block = ""
    try:
        from work4you_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>WORK4YOU_HOME</key>
        <string>{work4you_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.work4you/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See work4you-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_work4you_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_stop():
    ...
    # `work4you gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `work4you update`
...
def launchd_status(deep: bool = False):
    ...
    # Work4You PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Work4You PID file).
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``WORK4YOU_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``work4you gateway run`` is always a misconfiguration, so we
    ...
    try:
        from work4you_constants import get_default_work4you_root
        default_root = get_default_work4you_root()
    ...
    try:
        ...
        from work4you_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``work4you gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``work4you gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other WORK4YOU_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("WORK4YOU_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'work4you' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/work4you/docker/entrypoint.sh before the Work4You command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `work4you gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set WORK4YOU_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("WORK4YOU_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `work4you gateway restart` which already
    ...
    # `work4you gateway start/restart` — leaving the gateway vulnerable to
    ...
    # WORK4YOU_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("WORK4YOU_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``WORK4YOU_GATEWAY_MAX_STARTS`` /
    # ``WORK4YOU_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("WORK4YOU_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("WORK4YOU_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``work4you gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. work4you) and copy the bot token",
...
def _all_platforms() -> list[dict]:
    ...
    ``work4you setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run work4you under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.work4you/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_work4you_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from work4you_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from work4you_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    print_info(
        "  3. Work4You will store the returned account_id/token in ~/.work4you/.env."
    )
    print_info(
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_work4you_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "Work4YouAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``work4you_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from work4you_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.work4you/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.work4you/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_work4you_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info(
                    "  For persistence:   tmux new -s work4you 'work4you gateway run'"
                )
                print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`work4you_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from work4you_cli.service_manager import (
    ...
    if profile is None:
        ...
        # WORK4YOU_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``work4you gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``work4you gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``work4you gateway run`` are unaffected.
      2. ``WORK4YOU_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``work4you gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``WORK4YOU_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("WORK4YOU_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("WORK4YOU_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${WORK4YOU_HOME}/logs/gateways/<profile>/current,
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s work4you 'work4you gateway run'"
                )
                print()
            ...
            from work4you_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        # slot — start them individually via `work4you -p <name> gateway
        ...
        if os.getenv("_WORK4YOU_GATEWAY") == "1":
            print_error(
        ...
        # Stop, disable, and remove legacy Work4You gateway unit files from
        # pre-rename installs (e.g. work4you.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_work4you_units(interactive=not yes, dry_run=dry_run)
