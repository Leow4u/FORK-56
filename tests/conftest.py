"""Shared fixtures for the work4you test suite.
...
2. **Isolated WORK4YOU_HOME.** WORK4YOU_HOME points to a per-test tempdir so
   code reading ``~/.work4you/*`` via ``get_work4you_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".work4you"`` instead of the canonical
   ``get_work4you_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No WORK4YOU_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox WORK4YOU_HOME before ANY test module is imported ──────────────────
# `work4you_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_work4you_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.work4you/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes WORK4YOU_HOME, but fixtures run
...
# the REAL Work4You root — capture it BEFORE the sandbox rewires WORK4YOU_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.work4you (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("WORK4YOU_KANBAN_HOME", "").strip()
_PRE_SANDBOX_WORK4YOU_HOME = os.environ.get("WORK4YOU_HOME", "")


def _work4you_home_points_at_production(value: str) -> bool:
    """True when a pre-set WORK4YOU_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    WORK4YOU_HOME=~/.work4you``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``work4you_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.work4you`` — the escape vector that landed pytest
    ...
    WORK4YOU_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".work4you").resolve()
    except Exception:
        return True
...
if _work4you_home_points_at_production(os.environ.get("WORK4YOU_HOME", "")):
    _SESSION_WORK4YOU_HOME = tempfile.mkdtemp(prefix="work4you-test-home-")
    os.environ["WORK4YOU_HOME"] = _SESSION_WORK4YOU_HOME
    atexit.register(shutil.rmtree, _SESSION_WORK4YOU_HOME, True)
...
# like a real CLI") — which used to disarm work4you_state's live-DB guard in
# the child at the same moment the child lost the WORK4YOU_HOME redirect.
# WORK4YOU_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# work4you_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export WORK4YOU_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["WORK4YOU_TEST_ISOLATION"] = os.environ.get("WORK4YOU_HOME", "") or "1"

#: WORK4YOU_HOME as it stood when conftest was imported - i.e. before any test
...
WORK4YOU_HOME_AT_CONFTEST_IMPORT = os.environ.get("WORK4YOU_HOME", "")
...
# WORK4YOU_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_WORK4YOU_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``work4you_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Work4You agent session inherits them and
    # work4you_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "WORK4YOU_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "WORK4YOU_INTERACTIVE",
    "WORK4YOU_QUIET",
    "WORK4YOU_TOOL_PROGRESS",
    "WORK4YOU_TOOL_PROGRESS_MODE",
    "WORK4YOU_MAX_ITERATIONS",
    "WORK4YOU_SESSION_PLATFORM",
    "WORK4YOU_SESSION_CHAT_ID",
    "WORK4YOU_SESSION_CHAT_NAME",
    "WORK4YOU_SESSION_CHAT_TYPE",
    "WORK4YOU_SESSION_THREAD_ID",
    "WORK4YOU_SESSION_SOURCE",
    "WORK4YOU_SESSION_KEY",
    "WORK4YOU_GATEWAY_SESSION",
    "WORK4YOU_CRON_SESSION",
    "_WORK4YOU_GATEWAY",
    "WORK4YOU_PLATFORM",
    "WORK4YOU_MODEL",
    "WORK4YOU_INFERENCE_MODEL",
    "WORK4YOU_INFERENCE_PROVIDER",
    "WORK4YOU_TUI_PROVIDER",
    "WORK4YOU_MANAGED",
    "WORK4YOU_MANAGED_DIR",
    "WORK4YOU_DEV",
    "WORK4YOU_CONTAINER",
    "WORK4YOU_EPHEMERAL_SYSTEM_PROMPT",
    "WORK4YOU_TIMEZONE",
    "WORK4YOU_REDACT_SECRETS",
    "WORK4YOU_BACKGROUND_NOTIFICATIONS",
    "WORK4YOU_EXEC_ASK",
    "WORK4YOU_HOME_MODE",
    "WORK4YOU_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.work4you/kanban.db instead of the per-test WORK4YOU_HOME.
    "WORK4YOU_KANBAN_DB",
    "WORK4YOU_KANBAN_BOARD",
    "WORK4YOU_KANBAN_HOME",
    "WORK4YOU_KANBAN_WORKSPACES_ROOT",
    "WORK4YOU_KANBAN_LOGS_ROOT",
    "WORK4YOU_KANBAN_TASK",
    "WORK4YOU_KANBAN_WORKSPACE",
    "WORK4YOU_KANBAN_RUN_ID",
    "WORK4YOU_KANBAN_CLAIM_LOCK",
    "WORK4YOU_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "WORK4YOU_DELEGATED_CHILD_CONTEXT",
    "WORK4YOU_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "work4you" host.
    "WORK4YOU_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `work4you` plugin auto-registers itself on plugin discovery,
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and WORK4YOU_HOME to per-test tempdirs so code that
    reads ``~/.work4you/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral WORK4YOU_* vars that could change test semantics.
    for name in _WORK4YOU_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("WORK4YOU_HONCHO_HOST", "work4you")

    # 3. Redirect WORK4YOU_HOME to a per-test tempdir. Code that reads
    #    ``~/.work4you/*`` via ``get_work4you_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.work4you/*`` via
    #    ``Path.home() / ".work4you"`` instead of ``get_work4you_home()``
    #    is a bug to fix at the callsite.
    fake_work4you_home = tmp_path / "work4you_test"
    fake_work4you_home.mkdir()
    (fake_work4you_home / "sessions").mkdir()
    (fake_work4you_home / "cron").mkdir()
    (fake_work4you_home / "memories").mkdir()
    (fake_work4you_home / "skills").mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(fake_work4you_home))
    ...
    # work4you_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("WORK4YOU_TEST_ISOLATION", str(fake_work4you_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("WORK4YOU_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. work4you_state computes ``DEFAULT_DB_PATH = get_work4you_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from work4you_state import ...``) that
    ...
    work4you_state_mod = sys.modules.get("work4you_state")
    if work4you_state_mod is not None and hasattr(work4you_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            work4you_state_mod, "DEFAULT_DB_PATH", fake_work4you_home / "state.db"
        )
    ...
    monkeypatch.setenv("WORK4YOU_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.work4you/plugins/ (which, per step 3, is now empty — but the
    ...
    try:
        ...
        # previous test's tmp_path WORK4YOU_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_work4you_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from work4you_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.work4you. This autouse
...
# allow-list because test-level fixtures legitimately move WORK4YOU_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session WORK4YOU_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``WORK4YOU_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Work4You root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_WORK4YOU_HOME and not _work4you_home_points_at_production(
        _PRE_SANDBOX_WORK4YOU_HOME
    ):
        # WORK4YOU_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_work4you_root().resolve()
    # No pre-existing WORK4YOU_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".work4you").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.work4you`` captured at import time. Hermetic tests that legitimately
    move WORK4YOU_HOME to sibling tempdirs are unaffected.

    Only patches when ``work4you_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("work4you_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import work4you_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.work4you. See #69283."
        )
...
# ``work4you_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Work4You root. This fixture wires the
# test-side knobs:
...
#     installs where WORK4YOU_HOME is not ~/.work4you) into the guard's
...
# so subprocess children that import work4you_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("work4you_state")
    ...
    if _PRE_SANDBOX_WORK4YOU_HOME and not _work4you_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_WORK4YOU_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Work4You home override redirects every later
    # ``get_work4you_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from work4you_constants import get_work4you_home_override, set_work4you_home_override

        if get_work4you_home_override() is not None:
            set_work4you_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal work4you config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``work4you-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> work4you-gateway``
...
def _wal_is_usable() -> bool:
    """True when Work4You will actually put a database into WAL mode here.

    Work4You refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Work4You can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Work4You managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``work4you_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_work4you_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_work4you_home`` fixture
    redirects ``WORK4YOU_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.work4you/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``work4you_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["WORK4YOU_VOICE_TTS"] = "1"``. The
...
#      ``work4you_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``work4you_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``work4you_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``work4you_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Work4You runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.work4you``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Work4You falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Work4You uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart work4you-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting work4you/python patterns are also blocked.
    """
    ...
    def _matches_work4you_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _WORK4YOU_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_work4you_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch work4you-themed patterns + a
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live work4you-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting work4you/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `work4you update` (or the
        ...
        if "update" in low and (
            # work4you update / work4you update --gateway / setsid bash -c ... work4you update
            ("work4you" in low and "update" in low.split())
            or
            # python -m work4you_cli.main update --gateway
            ("work4you_cli" in low and "update" in low.split())
            or
            # venv/bin/work4you update  (absolute path variant used in tests)
            (".venv/bin/work4you" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`work4you update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_WORK4YOU_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import work4you_cli.voice as _voice
    except Exception:
