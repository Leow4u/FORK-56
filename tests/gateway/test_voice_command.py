...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        event.source.chat_name = "Work4You Server / #general"
        runner.adapters[event.source.platform] = mock_adapter
    ...
    async def test_input_reuses_bound_source_metadata(self, runner):
        ...
        bound_source = SessionSource(
            chat_id="123",
            chat_name="Work4You Server / #general",
            chat_type="group",
            user_id="user1",
            user_name="user1",
        ...
        assert event.source.chat_name == "Work4You Server / #general"
...
class TestDiscordVoiceChannelMethods:
    ...
    def test_discord_voice_timeout_config_loaded(self):
        ...
        with patch("work4you_cli.config.read_raw_config", return_value={
            "discord": {
...
class TestVoiceChannelAwareness:
    ...
    def _make_adapter(self):
        ...
        adapter._voice_receivers = {}
        adapter._client = MagicMock()
        adapter._client.user = SimpleNamespace(id=99999, name="Work4YouBot")
        return adapter
    ...
    def test_returns_info_with_members(self):
        ...
        bot_member = self._make_member(99999, "Work4YouBot", is_bot=True)
        ...
        assert "Bob" in names
        assert "Work4YouBot" not in names
