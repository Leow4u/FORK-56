...
def test_required_frontmatter_fields(p):
    ...
    work4you = (fm.get("metadata") or {}).get("work4you") or {}
    if not (work4you.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.work4you.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    work4you = (fm.get("metadata") or {}).get("work4you") or {}
    dangling = [
        rs for rs in (work4you.get("related_skills") or []) if rs not in _all_names()
    ]
