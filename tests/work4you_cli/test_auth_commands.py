...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (work4you_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from work4you_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "work4you" / "auth.json").read_text())
...
def test_auth_add_work4you_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("work4you@example.com")
    monkeypatch.setattr(
        "work4you_cli.auth._work4you_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["work4you"]
    device_code_entries = [
    ...
    # `work4you auth add work4you` must also populate providers.work4you so the
    # 401-recovery path (resolve_work4you_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Work4You is not logged into Work4You Portal" and the agent dies.
...
def test_auth_add_work4you_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`work4you auth add work4you --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_work4you_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("work4you@example.com")
    ...
    pool_entry = payload["credential_pool"]["work4you"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-work4you"

    # … and survives in providers.work4you so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["work4you"]["label"] == "my-work4you"


def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``work4you auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``work4you auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("work4you_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from work4you_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from work4you_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """work4you auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``work4you auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "work4you" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`work4you logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (work4you_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from work4you_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Work4You reads from has a
...
def test_seed_from_singletons_respects_work4you_pkce_suppression(tmp_path, monkeypatch):
    """anthropic work4you_pkce must not re-seed from ~/.work4you/.anthropic_oauth.json when suppressed."""
    work4you_home = tmp_path / "work4you"
    work4you_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    import yaml
    (work4you_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (work4you_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["work4you_pkce"]},
    }))

    # Stub the readers so only work4you_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_work4you_oauth_credentials", lambda: {
    ...
    # work4you_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "work4you_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.work4you/.anthropic_oauth.json",
        "auth.json providers.work4you",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from work4you_cli.auth import is_source_suppressed
    from work4you_cli.auth_commands import auth_remove_command
    ...
    with patch(
        ...
        "work4you_cli.copilot_auth.get_copilot_api_token",
