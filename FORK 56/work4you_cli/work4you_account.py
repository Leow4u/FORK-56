"""Normalized Work4You Portal account entitlement helpers."""
...
Work4YouAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (work4you-account-service
...
@dataclass(frozen=True)
class Work4YouPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class Work4YouPortalAccountInfo:
    logged_in: bool
    source: Work4YouAccountInfoSource
    fresh: bool
    ...
    work4you_client: Optional[str] = None
    ...
    subscription: Optional[Work4YouPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[Work4YouPaidServiceAccessInfo] = None
    tool_access: Optional[Work4YouToolAccessInfo] = None
...
def work4you_portal_billing_url(account_info: Optional[Work4YouPortalAccountInfo] = None) -> str:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_WORK4YOU_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def work4you_portal_topup_url(account_info: Optional[Work4YouPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = work4you_portal_billing_url(account_info)  # {base}/billing
...
def format_work4you_portal_entitlement_message(
    account_info: Optional[Work4YouPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Work4You tool-gateway entitlement.
    ...
    """
    billing_url = work4you_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Work4You Portal "
    ...
    if account_info is None:
        return (
            f"Work4You could not verify your Work4You Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Work4You inference credentials are configured, but Work4You cannot verify "
                f"your Work4You Portal paid access for {capability}. Log in with "
                f"`work4you model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Work4You Portal to use {capability}: run `work4you model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Work4You could not verify your Work4You Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Work4You could not find a Work4You Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `work4you model` to "
            f"authenticate again; if the problem persists, contact Work4You support."
        )
    ...
    return (
        f"Your Work4You Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: Work4YouPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Work4You Portal access is paused because you've exceeded the"
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Work4You Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Work4You Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Work4You Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Work4You Portal account has no usable paid credits{credit_detail}, so "
...
def reset_work4you_portal_account_info_cache() -> None:
...
def get_work4you_portal_account_info(
    *,
    ...
    """Return normalized Work4You Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from work4you_cli.auth import get_provider_auth_state, resolve_work4you_access_token

        access_token = resolve_work4you_access_token()
        ...
        payload = _fetch_work4you_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[Work4YouPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Work4You keys."""
    try:
        entry = _select_work4you_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_work4you_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_work4you_pool_entry() -> Optional[Any]:
...
def _fetch_work4you_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from work4you_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = Work4YouPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return Work4YouPortalAccountInfo(
        ...
        work4you_client=_coerce_str(claims.get("work4you_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[Work4YouToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`Work4YouToolAccessInfo`. Fails closed: a non-object value
    ...
    return Work4YouToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[Work4YouPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return Work4YouPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[Work4YouPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return Work4YouPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
