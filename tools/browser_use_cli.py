...
_PRIVATE_BROWSER_SENTINEL = "_WORK4YOU_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# work4you: pin this named session to its own tab (once per daemon process)
def _work4you_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "work4you-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_work4you_ensure_own_tab()
del _work4you_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Work4You's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Work4You's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_work4you_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `work4you tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Work4You' own bin dir ($WORK4YOU_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from work4you_constants import get_work4you_home

        return str(Path(get_work4you_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Work4You' own ``$WORK4YOU_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Work4You-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Work4You' managed uv (bootstrapped on demand via
    ``work4you_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$WORK4YOU_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_work4you_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    3. A configured cloud browser provider (Browserbase, Firecrawl, Work4You
    ...
    # Work4You-gateway variant (use_gateway: true) DOES resolve through the
    ...
    try:
        ...
        return (
            ...
            "switch backends via `work4you tools` → Browser Automation."
        )
...
def browser_exec(
    ...
    # Work4You gateway, CDP override, local Chrome, …). Named sessions compose
