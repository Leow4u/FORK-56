import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@work4you/shared'
...
const isOpen = (gateway: Work4YouGateway | null): boolean => gateway?.connectionState === 'open'

interface RegistryConfig {
  onEvent: (event: GatewayEvent) => void
  onActiveConnectionInvalidated?: (fallbackProfile: string, activationEpoch: number) => void
  onActiveConnectionChanged?: (connection: Work4YouConnection) => void
  /**
...
interface Secondary {
  ...
  connection: Work4YouConnection | null
...
interface GatewayRegistryState {
  ...
  primaryGateway: Work4YouGateway | null
  primaryProfile: string
  activeKey: string
  activationEpoch: number
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<Work4YouGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('work4you.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<Work4YouGateway | null>(null),
...
export function activeGateway(): Work4YouGateway | null {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the work4you module (null
...
function publishActiveConnection(connection: Work4YouConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new Work4YouGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: Work4YouGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Work4You gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.work4youDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Work4You Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<Work4YouGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Work4You gateway is not connected" on the Sessions `+` action (#88880).
