"""Meta Model API (Muse Spark) provider plugin for Work4You.
...
Bundled from https://github.com/albertodepaola/work4you-meta-provider. Work4You'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to work4you' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Work4You' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in work4you_cli/providers.py and the retention hint in
