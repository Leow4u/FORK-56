"""ensure_work4you_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".work4you"
    second = tmp_path / "b" / ".work4you"

    monkeypatch.setenv("WORK4YOU_HOME", str(first))
    cfg.ensure_work4you_home()

    # Profile switch: WORK4YOU_HOME moves → the new path is ensured too.
    monkeypatch.setenv("WORK4YOU_HOME", str(second))
    cfg.ensure_work4you_home()
