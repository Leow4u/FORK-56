...
| | |
|---|---|
| Source | Optional — install with `work4you skills install official/productivity/airtable` |
| Path | `optional-skills/productivity/airtable` |
| Version | `1.1.0` |
| Author | community |
| License | MIT |
...
4. Store the token in `${WORK4YOU_HOME:-~/.work4you}/.env` (or via `work4you setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Work4You. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${WORK4YOU_HOME:-~/.work4you}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
