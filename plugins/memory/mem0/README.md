...
```bash
work4you memory setup    # select "mem0"
```
...
Behavioral settings live in `$WORK4YOU_HOME/mem0.json` (set them via `work4you memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.work4you/.env`.
...
| `user_id` | `work4you-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Work4You session and call `mem0_search` — it connects to your server.
...
```bash
work4you memory setup mem0 --mode oss \
...
```bash
work4you memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$WORK4YOU_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.work4you/mem0_qdrant"}}
  }
}
```
...
```bash
work4you memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$WORK4YOU_HOME/mem0.json`).
