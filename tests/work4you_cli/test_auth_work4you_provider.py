"""Regression tests for Work4You OAuth refresh and inference JWT interactions."""
...
from work4you_cli.auth import AuthError, get_provider_auth_state, resolve_work4you_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from work4you_cli.auth import _resolve_verify
...
def _setup_work4you_auth(
    ...
    *,
    ...
    (work4you_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_work4you_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    _setup_work4you_auth(
        work4you_home,
        access_token=token,
        scope=auth_mod.DEFAULT_WORK4YOU_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    creds = auth_mod.resolve_work4you_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.WORK4YOU_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.WORK4YOU_AUTH_PATH_INVOKE_JWT

    payload = json.loads((work4you_home / "auth.json").read_text())
    singleton = payload["providers"]["work4you"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["work4you"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.WORK4YOU_DEVICE_CODE_SOURCE


def test_resolve_work4you_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import work4you_cli.auth as auth_mod
    ...
    auth_store = {
        ...
        "providers": {
            "work4you": {
                "portal_base_url": "https://portal.work4you.ai",
                "inference_base_url": "https://inference-api.work4you.ai/v1",
                "client_id": "work4you-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_WORK4YOU_SCOPE,
                "access_token": token,
    ...
    auth_path = work4you_home / "auth.json"
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_work4you_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["work4you"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_work4you_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import work4you_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_work4you_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((work4you_home / "auth.json").read_text())
    assert payload["providers"]["work4you"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("work4you")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
    monkeypatch.setenv("WORK4YOU_USE_LEGACY_SESSION_KEYS", "true")

    creds = auth_mod.resolve_work4you_runtime_credentials()

    assert creds["api_key"] == token
    payload = json.loads((work4you_home / "auth.json").read_text())
    assert payload["providers"]["work4you"]["agent_key"] == token
    ...
    result = auth_mod._work4you_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_WORK4YOU_SCOPE]
    assert result["agent_key"] == login_token


def test_work4you_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.DEBUG, logger="work4you_cli.auth")
    auth_mod.resolve_work4you_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_work4you_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_work4you_auth_status() should find Work4You credentials in the pool
    even when the auth store has no Work4You provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from work4you_cli.auth import get_work4you_auth_status

    work4you_home = tmp_path / "work4you"
    work4you_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Work4You provider entry
    (work4you_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    # Seed the credential pool with a Work4You entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("work4you")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("work4you", {
        "access_token": token,
    ...
    status = get_work4you_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_work4you_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_work4you_auth_status() returns logged_in=False when both pool
...
class TestLoginWork4YouSkipKeepsCurrent:
    """When a user runs `work4you model` → Work4You Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "work4you" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return work4you_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import work4you_cli.work4you_subscription as ns

        fake_auth_state = {
            "access_token": "fake-work4you-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.work4you.ai",
            "portal_base_url": "https://portal.work4you.ai",
            "refresh_token": "fake-refresh",
            "token_expires_at": 9999999999,
        }
        monkeypatch.setattr(
            auth_mod, "_work4you_device_code_login",
        ...
        def _check_work4you_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_work4you_free_tier", _check_work4you_free_tier)
        monkeypatch.setattr(
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Work4You creds still saved."""
        import argparse
        import yaml
        from work4you_cli.auth import PROVIDER_REGISTRY, _login_nous

        work4you_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_nous(args, PROVIDER_REGISTRY["work4you"])
        ...
        # auth.json: active_provider restored to openrouter, but Work4You creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "work4you" in auth_after["providers"]
        assert auth_after["providers"]["work4you"]["access_token"] == "fake-work4you-token"
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a Work4You model → provider flips to work4you with that model."""
        import argparse
        import yaml
        from work4you_cli.auth import PROVIDER_REGISTRY, _login_nous
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as work4you."""
        import argparse
        import yaml
        from work4you_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        auth_path = work4you_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "work4you" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Work4You creds are still saved
        assert "work4you" in auth_after.get("providers", {})


# =============================================================================
# persist_work4you_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================
...
def test_persist_work4you_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.work4you AND providers.work4you.

    Regression guard: before this helper existed, `work4you auth add work4you`
    wrote only the pool. After the Work4You agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_work4you_runtime_credentials
    which reads providers.work4you, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from work4you_cli.auth import persist_work4you_credentials, WORK4YOU_DEVICE_CODE_SOURCE
    ...
    entry = persist_work4you_credentials(state)

    assert entry is not None
    assert entry.provider == "work4you"
    assert entry.source == WORK4YOU_DEVICE_CODE_SOURCE

    payload = json.loads((work4you_home / "auth.json").read_text())

    # providers.work4you populated with the full state (new behaviour)
    singleton = payload["providers"]["work4you"]
    ...
    # credential_pool.work4you has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["work4you"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == WORK4YOU_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_work4you_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.work4you and lets seeding
    ...
    persist_work4you_credentials(first)
    ...
    persist_work4you_credentials(second)

    payload = json.loads((work4you_home / "auth.json").read_text())

    # providers.work4you reflects the latest write (singleton semantics)
    assert payload["providers"]["work4you"]["access_token"] == second_token
    assert payload["providers"]["work4you"]["agent_key"] == second_token

    # credential_pool.work4you has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["work4you"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == WORK4YOU_DEVICE_CODE_SOURCE
...
def test_persist_work4you_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_work4you_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.work4you when the caller didn't supply one.
    payload = json.loads((work4you_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["work4you"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Work4You Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Work4You must surface an
    ...
    re-authenticate" string led users to report this as a Work4You persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from work4you_cli.auth import _refresh_access_token
    ...
    assert "work4you auth add work4you" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Work4You refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-work4you-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Work4You token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect WORK4YOU_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Work4You store — the
    ...
    monkeypatch.setenv("WORK4YOU_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without WORK4YOU_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.work4you/shared/`` across CI runs.
    """
    from work4you_cli.auth import _work4you_shared_store_path

    monkeypatch.delenv("WORK4YOU_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Work4You auth store"):
        _work4you_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_work4you_state()
...
def test_persist_work4you_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_work4you_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `work4you auth add work4you
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from work4you_cli.auth import (
        _work4you_shared_store_path,
        _read_shared_work4you_state,
        persist_work4you_credentials,
    )

    work4you_home = tmp_path / "work4you"
    work4you_home.mkdir(parents=True, exist_ok=True)
    (work4you_home / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    persist_work4you_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((work4you_home / "auth.json").read_text())
    assert "work4you" in payload.get("providers", {})
    ...
    assert str(_work4you_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_work4you_credentials() needs.
    """
    from work4you_cli import auth as auth_mod

    auth_mod._write_shared_work4you_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_work4you_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_work4you_state()
    ...
    assert result["client_id"] == "work4you-cli"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_work4you_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from work4you_cli.auth import _load_auth_store, DEFAULT_WORK4YOU_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "work4you": {
                    "portal_base_url": "https://api.work4you.ai",
        ...
        work4you = store["providers"]["work4you"]
        assert work4you["portal_base_url"] == DEFAULT_WORK4YOU_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_file = work4you_home / "auth.json"
        store = json.loads(auth_file.read_text())
        store["providers"]["work4you"]["portal_base_url"] = (
            "http://portal.work4you.ai"
        )
        ...
        auth_mod.resolve_work4you_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_WORK4YOU_PORTAL_URL]
...
class TestWork4YouDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from work4you_cli.auth import _work4you_device_auth_timeout_message

        msg = _work4you_device_auth_timeout_message("https://portal.work4you.ai")
        assert "CAPTCHA" in msg
        assert "work4you portal" in msg
        assert "https://portal.work4you.ai/login" in msg
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from work4you_cli.auth import (
            ...
            _work4you_device_auth_timeout_message,
        )

        msg = _work4you_device_auth_timeout_message("")
        assert f"{DEFAULT_WORK4YOU_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server._work4you_poller,
...
def test_work4you_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.work4you.ai/manage-subscription",
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._work4you_device_auth_timeout_message(
