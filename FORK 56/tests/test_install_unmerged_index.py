...
The ``work4you update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m work4you_cli.main gateway run`` — image name
    ``pythonw``, not ``work4you.exe`` — so the ``taskkill /IM work4you.exe`` guard
    ...
    # The work4you.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM work4you.exe' in text
