...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Work4You"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^work4you"]}
        )
        assert adapter._message_matches_mention_patterns("work4you please help") is True
        assert adapter._message_matches_mention_patterns("please work4you help") is False
