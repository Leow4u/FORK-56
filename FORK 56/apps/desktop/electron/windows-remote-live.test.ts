...
//   WORK4YOU_WIN_SSH_HOST   ssh alias/host of the Windows box
//   WORK4YOU_WIN_SSH_USER   remote user
//   WORK4YOU_WIN_SSH_WORK4YOU absolute path to the remote work4you.exe under test
const liveHost = process.env.WORK4YOU_WIN_SSH_HOST || ''
const liveUser = process.env.WORK4YOU_WIN_SSH_USER || ''
const configuredWork4You = process.env.WORK4YOU_WIN_SSH_WORK4YOU || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Work4You-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredWork4You)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredWork4You.replace('work4you.exe', 'python.exe')}' -m work4you_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredWork4You.replace('work4you.exe', 'python.exe')
        const terminate = `& '${python}' -m work4you_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.work4youPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m work4you_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m work4you_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
