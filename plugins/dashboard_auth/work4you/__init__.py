"""Work4YouDashboardAuthProvider — Work4You Portal OAuth (authorization-code + PKCE).

Implements ``work4you-account-service/docs/agent-dashboard-oauth-contract.md``
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      WORK4YOU_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``work4you-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``work4you_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via WORK4YOU_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.work4you.ai"
...
class Work4YouDashboardAuthProvider(DashboardAuthProvider):
    """Work4You Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"work4you_session_pkce\"]``
        ...
        cookie_payload = {
            "work4you_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Work4You Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-work4you-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-work4you-refresh-
                ...
                headers={
                    ...
                    "x-work4you-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between WORK4YOU_DASHBOARD_PORTAL_URL / WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Work4You Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-work4you: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``WORK4YOU_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("WORK4YOU_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    Registers ``Work4YouDashboardAuthProvider`` only when a client_id is
    configured (either via ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``work4you_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "Work4You Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Work4You "
        ...
        logger.debug("dashboard-auth-work4you: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Work4You Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-work4you: %s", LAST_SKIP_REASON)
        return

    try:
        provider = Work4YouDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"Work4YouDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-work4you: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-work4you: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
