...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset(
    {"todo", "session_search", "memory", "clarify", "read_terminal", "read_preview", "drive_preview", "read_window_below", "setup_mcp", "tour", "delegate_task"}
)
...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Work4You) which
    ...
    if (
        provider_lower in {"work4you", "work4you-portal", "work4you"}
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Work4You-Session-Id header), and an
        ...
        if env_var_enabled("WORK4YOU_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # Work4You Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_work4you_portal = base_url_host_matches(eff_base_url, "work4you.ai")
    ...
    # canonicalized only after Work4You sends the request. In that case model
    ...
    if is_anthropic_wire or _litellm_openai_wire:
        try:
            from work4you_cli.config import get_custom_provider_model_capability
    ...
    if (
        (is_openrouter or is_work4you_portal)
        and (is_claude or is_kimi)
        and not is_anthropic_wire
    ):
        return True, False
    # Work4You Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_work4you_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to work4you's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # Pass model so dual-wire providers (Work4You Portal anthropic/* → Messages)
    ...
    try:
        ...
        from work4you_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from work4you_cli.plugins import _dispatch_pre_tool_call_hooks
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: work4you' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Work4You' keepalive client (#10324 / ``_build_keepalive_http_client``) and
