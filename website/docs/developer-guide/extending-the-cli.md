...
Work4You exposes protected extension hooks on `Work4YouCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
Called after Work4You registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
