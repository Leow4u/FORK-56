#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Work4You desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Work4You window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Work4You desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Work4You desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Work4You "
        "windows are skipped: the nearest non-Work4You window is reported. "
