...
path via ``work4you chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp WORK4YOU_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point WORK4YOU_HOME at a temp dir and turn moa.save_traces on."""
    work4you_home = tmp_path / ".work4you"
    work4you_home.mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    # save_moa_turn reads config via work4you_cli.config.load_config; stub it to
    ...
    return work4you_home / "moa-traces"
