 ...
 * `work4you://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `work4you://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `work4you://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type Work4YouOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const WORK4YOU_PROTOCOL = 'work4you:'
...
export function normalizeWork4YouOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('work4you://') || trimmed.startsWith(`${WORK4YOU_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // work4you://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // work4you://index-network/intent/1 → /index-network/intent/1
...
export function resolveWork4YouOpenPath(target: Work4YouOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeWork4YouOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeWork4YouOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeWork4YouOpenString(target.path)
...
export function pathFromWork4YouDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  ...
  if (kind === 'open') {
    return resolveWork4YouOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveWork4YouOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `work4you://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromWork4YouDeepLink('open', name, params)
}
