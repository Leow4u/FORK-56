"""``work4you peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Work4You gateway (any machine: homelab, Spark, Work4You
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    work4you peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    work4you peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``work4you -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.work4you/.env`` as
  ``WORK4YOU_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"WORK4YOU_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _ensure_bot_chat(base: str, key: str) -> str:
    ...
    # Real api_server wraps the row: {"object": "work4you.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (work4you peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.work4you/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  work4you peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.work4you/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: work4you peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: work4you peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: work4you peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.work4you/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: work4you peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Work4You gateways)",
        description=(
            "Register other Work4You gateways as peers and message their agents. "
            "'work4you peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'work4you -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.work4you/.env."
        ),
        epilog=(
            "Examples:\n"
            "  work4you peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  work4you peer list\n"
            '  work4you peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  work4you peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  work4you peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.work4you/.env)")
