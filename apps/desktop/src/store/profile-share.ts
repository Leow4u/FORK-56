 ...
 *
 * The archive is the CLI's own `work4you profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/work4you'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Work4You profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.work4youDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.work4youDesktop?.selectPaths?.({
