...
plugin SDK (``window.__WORK4YOU_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Work4You-Session-Token`` header) and gated OAuth
(``work4you_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__WORK4YOU_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__WORK4YOU_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + work4you-achievements are bundled today; assert at least one is
