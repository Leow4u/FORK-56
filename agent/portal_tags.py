"""Centralized Work4You Portal request tags.

Every Work4You request that hits the Work4You Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Work4You can attribute usage to Work4You and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=work4you",
        "client=work4you-client-v<__version__>",
    ]
...
# active conversation id here and ``work4you_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "work4you_portal_conversation_id", default=None
)
...
def _work4you_version() -> str:
    """Return the current Work4You release version, e.g. ``"0.13.0"``.
...
def work4you_client_tag() -> str:
    """Return the ``client=...`` tag for Work4You Portal requests.

    Format: ``client=work4you-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=work4you-client-v{_work4you_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Work4You session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Work4You conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.work4you/sessions/`` storage, session logs, and lineage.
...
def work4you_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Work4You Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(work4you_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Work4You
    ...
    """
    tags = ["product=work4you", work4you_client_tag()]
