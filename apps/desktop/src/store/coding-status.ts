...
const EMPTY_WORKTREES: Work4YouGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, Work4YouRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, Work4YouGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<Work4YouRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<Work4YouGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<Work4YouRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<Work4YouGitWorktree[]>>()
const $noRepoStatus = atom<Work4YouRepoStatus | null>(null)
const $noWorktrees = atom<Work4YouGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<Work4YouRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<Work4YouGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: Work4YouRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: Work4YouGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<Work4YouRepoStatus | null>
  seq: number
  target: string
}
