...
import { formatRefValue, work4youDirectiveFormatter } from './directive-text'
...
describe('work4youDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = work4youDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = work4youDirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = work4youDirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...work4youDirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = work4youDirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...work4youDirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
