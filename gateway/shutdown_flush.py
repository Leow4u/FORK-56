...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<work4you_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active WORK4YOU_HOME."""
    from work4you_constants import get_work4you_home

    flush_dir = get_work4you_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<work4you_home>/pending_messages/``), so a runtime cap rotation no
