...
closure the PR changed, against a real temp ``WORK4YOU_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_work4you_home() / config.yaml`` — point it here.
    monkeypatch.setattr("work4you_constants.get_work4you_home", lambda: work4you_home)
    monkeypatch.setattr("work4you_cli.config.get_work4you_home", lambda: work4you_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_work4you_home", default_home)
    _stub_picker_dependencies(monkeypatch)
