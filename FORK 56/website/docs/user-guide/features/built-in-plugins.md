...
description: "Plugins shipped with Work4You that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Work4You ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.work4you/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Work4You Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.work4you/plugins/<name>/`
3. **Project** — `./.work4you/plugins/<name>/` (requires `WORK4YOU_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `work4you.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `work4you memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `work4you plugins list` and the interactive `work4you plugins` UI), but none load until you explicitly enable them:

```bash
work4you plugins enable disk-cleanup
```

Or via `~/.work4you/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Work4You. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
work4you plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `work4you plugins enable <name>`.
...
| `work4you-achievements` | dashboard tab | Steam-style collectible badges generated from your real Work4You session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `work4you memory` and `work4you plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `WORK4YOU_HOME` or `/tmp/work4you-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under WORK4YOU_HOME | always | Never |
...
**State** — everything lives at `$WORK4YOU_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `WORK4YOU_HOME` or `/tmp/work4you-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `work4you plugins enable disk-cleanup` (or check the box in `work4you plugins`).

**Disabling again:** `work4you plugins disable disk-cleanup`.
...
**Enabling:** `work4you plugins enable security-guidance` (or check the box in `work4you plugins`).

**Disabling again:** `work4you plugins disable security-guidance`.
...
Traces Work4You turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Work4You' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `work4you logs`.
...
```bash
work4you tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Work4You and the next turn ships a trace.
...
Then put the credentials in `~/.work4you/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Work4You turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `WORK4YOU_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Work4You session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `work4you chat` session lives under one Langfuse session.

**Verify:**

```bash
work4you plugins list                 # observability/langfuse should show "enabled"
work4you chat -q "hello"              # check the Langfuse UI for a "Work4You turn" trace
```
...
| `WORK4YOU_LANGFUSE_RELEASE` | — | Release/version tag |
| `WORK4YOU_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `WORK4YOU_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `WORK4YOU_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Work4You-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Work4You-prefixed wins when both are set.
...
**Disabling:** `work4you plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### observability/nemo_relay

Relays Work4You execution boundaries — sessions, turns, LLM calls, and tool invocations — to an [NVIDIA NeMo Relay](https://docs.nvidia.com/nemo/relay/about-nemo-relay/overview) endpoint. Work4You core owns the Relay session/turn/LLM/tool scopes; the plugin configures exporters (ATOF JSONL, ATIF trajectories, OpenTelemetry) and adds observer marks for approvals and delegated subagents. Full exporter setup lives in the plugin's `README.md` under `plugins/observability/nemo_relay/`.

**Enabling:**

```bash
work4you plugins enable observability/nemo_relay
```
...
Rotated segments keep the same `session_id` attribute and add `work4you.session.segment` (0-based index) plus `work4you.session.segment_reason` (`compaction` or `max_turns`), so dashboards that group on `session_id` are unaffected. Rotation happens exclusively at turn boundaries and rides the same bounded scope-op executor as every other native Relay call — a wedged exporter costs one segment span, never the agent.
...
- Post-meeting artifacts (transcript, status) saved under `~/.work4you/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
work4you plugins enable google_meet
work4you meet setup   # preflight: playwright, chromium, auth file
work4you meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `work4you plugins disable google_meet`. Any saved transcripts stay in `~/.work4you/workspace/meetings/` until you remove them.

### work4you-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Work4You session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Work4You feature changes.

**How it works:**

- Scans your entire `~/.work4you/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$WORK4YOU_HOME/plugins/work4you-achievements/state.json`
...
| Secret | Hidden until Work4You detects the first related signal in your history |

**API** — routes mount under `/api/plugins/work4you-achievements/`:
...
**State files** — live under `$WORK4YOU_HOME/plugins/work4you-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Work4You updates. |
...
**Enabling:** Nothing to enable — `work4you-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `work4you dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/work4you-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.work4you/plugins/work4you-achievements/` that ships no dashboard. The plugin's state files under `$WORK4YOU_HOME/plugins/work4you-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Work4You plugin — see [Build a Work4You Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.work4you/plugins/<name>/`
- Manifest source is reported as `bundled` in `work4you plugins list`
