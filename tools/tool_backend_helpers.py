...
def managed_work4you_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Work4You Tool Gateway.

    Entitlement is paid Work4You Portal service access OR a live free tool pool
...
def work4you_tool_gateway_unavailable_message(
    capability: str = "the Work4You Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Work4You Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `work4you model` to refresh your "
        "Work4You Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_work4you_tools_enabled()
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``work4you auth add
    ...
    2. The environment / ``~/.work4you/.env``. Under a multiplexed gateway turn
       ...
       ``work4you_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``work4you auth
    ...
    when omitted, ``work4you_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `work4you auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``work4you auth add openai-api``) via
...
# The provider value the managed "Work4You Subscription" picker rows write for
# every category (image_gen.provider: work4you, web.backend: work4you,
# browser.cloud_provider: work4you, ...). Runtime dispatch is a plain switch on
# the stored string: "work4you" → managed gateway client; any vendor name → that
# vendor direct with the user's own credentials; no key ever written →
# legacy credential autodetect.
WORK4YOU_MANAGED_PROVIDER = "work4you"
...
def read_selection(section: str) -> str | None:
    """Return the stored `work4you tools` provider string for a config section.

    THE single runtime read of the persisted selection. Returns:
    - ``"work4you"`` — the managed Work4You Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "Work4You
    Subscription" row, so it maps to ``"work4you"`` regardless of the name key;
    ...
    if "use_gateway" in raw and is_truthy_value(raw.get("use_gateway"), default=False):
        return WORK4YOU_MANAGED_PROVIDER
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via work4you "
        f"tools), but {failure}. Run 'work4you tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.work4you/.env`` (via
