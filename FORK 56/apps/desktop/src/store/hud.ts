 ...
 * composer with the reply scrolling above it, so Work4You can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.work4youDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.work4youDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.work4youDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.work4youDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
