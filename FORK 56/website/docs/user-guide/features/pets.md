...
Work4You can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<WORK4YOU_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Work4You renders the sprite at full fidelity when your
...
work4you pets list
work4you pets list cat

# Install a pet and make it active in one step
work4you pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
work4you pets show

# Check your setup
work4you pets doctor
```

## `work4you pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `work4you pets list [query] [--limit N]` |
| List installed pets | `work4you pets list --installed` |
| Install a pet | `work4you pets install <slug> [--select] [--force]` |
...
| Preview/animate | `work4you pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `work4you pets off` |
| Remove an installed pet | `work4you pets remove <slug>` |
| Diagnose setup | `work4you pets doctor` |

`work4you pets show` flags:
...
Beyond installing pre-made pets from the gallery, Work4You can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `work4you pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Work4You state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Work4You Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Work4You run a quality-first model chain by default.

- Resolution order prefers Work4You Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `work4you tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `WORK4YOU_PET_IMAGE_PROVIDER` env var (e.g. `WORK4YOU_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Work4You is
...
Run `work4you pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Work4You uses its own
  profile-scoped `<WORK4YOU_HOME>/pets/` instead — install through `work4you pets`.

## See also

- The [`work4you` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-work4you.md)
