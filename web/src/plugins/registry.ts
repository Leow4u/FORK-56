 ...
 *
 * Plugins call window.__WORK4YOU_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__WORK4YOU_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__WORK4YOU_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__WORK4YOU_PLUGIN_SDK__ = {
    ...
    // window.__WORK4YOU_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Work4You DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
