...
Prior to this change, aggregator users (OpenRouter / Work4You Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_work4you_portal(seen: dict):
        """Stub the Work4You network boundary, keeping the resolution chain real.
        ...
        work4you_client.api_key = "jwt-test"
        work4you_client.base_url = "https://inference-api.work4you.ai/v1"

        def fake_try_nous(vision=False):
            seen["vision"] = vision
            return work4you_client, (
                "stepfun/step-3.7-flash:free" if vision else "tencent/hy3:free"
            )

        return work4you_client, fake_try_nous

    def test_work4you_main_vision_uses_portal_pick_not_text_chat_model(self):
        """Work4You main → vision runs the Portal's vision slot, not the chat model.

        A Work4You chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the Work4You network boundary
        ...
        """
        seen: dict = {}
        work4you_client, fake_try_nous = self._stub_work4you_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="work4you",
        ), patch(
    ...
    def test_work4you_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: work4you`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("work4you", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_nous", side_effect=fake_try_nous,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_work4you_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a Work4You main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("work4you")
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / Work4You,
