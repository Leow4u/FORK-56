...
``CommandDef.execute`` (work4you_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/work4you_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``work4you_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_work4you_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: work4you bundles create <name> --skill <s1> --skill <s2>\n"
