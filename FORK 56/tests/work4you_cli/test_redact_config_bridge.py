...
`WORK4YOU_REDACT_SECRETS` at module-import time. `work4you_cli/main.py` at
...
in .env), the toggle was silently ignored in both `work4you chat` and
`work4you gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `WORK4YOU_REDACT_SECRETS`
env var in `work4you_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports work4you_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("WORK4YOU_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import work4you_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('WORK4YOU_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["WORK4YOU_HOME"] = str(work4you_home)
    env.pop("WORK4YOU_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env WORK4YOU_REDACT_SECRETS takes precedence over config.yaml."""
    work4you_home = tmp_path / ".work4you"
    work4you_home.mkdir()
    (work4you_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (work4you_home / ".env").write_text("WORK4YOU_REDACT_SECRETS=true\n")
