"""Guards for work4you_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from work4you_cli.config import ...`` silently makes `work4you --version`
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import work4you_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"work4you_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    env.pop("WORK4YOU_DEV", None)
    return subprocess.run(
        [sys.executable, "-m", "work4you_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version({"WORK4YOU_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Work4You v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"WORK4YOU_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "Work4You v" in result.stdout
