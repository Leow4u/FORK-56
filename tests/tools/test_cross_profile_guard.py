...
def fake_work4you(tmp_path, monkeypatch):
    """Build a two-profile Work4You layout and point WORK4YOU_HOME at
    the work4you-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_work4you):
        from tools.file_tools import write_file_tool
        target = fake_work4you["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_work4you):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_work4you["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "work4you-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_work4you_path_unaffected(self, fake_work4you, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_work4you):
    ...
    def test_cross_profile_patch_bypass(self, fake_work4you):
        from tools.file_tools import patch_tool
        target = fake_work4you["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_work4you):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_work4you, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_work4you["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up WORK4YOU_HOME (set in
        ...
        assert "not found in active profile 'work4you-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.work4you/profiles/<name>/."""
        # Don't set WORK4YOU_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_work4you_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_work4you_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_work4you):
        """When active profile is 'work4you-security', the prompt warns
        ...
        assert "~/.work4you/profiles/" in src
