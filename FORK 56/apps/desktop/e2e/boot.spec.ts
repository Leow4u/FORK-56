 ...
 * packaged binary) with a real `work4you serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → work4you serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Work4You title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Work4You')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { work4youDesktop?: Record<string, unknown> }).work4youDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → work4you serve is spawned → WS connects → config
