...
#
# `services.work4you` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  work4youHome, installPackage, home.activation,
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "work4you-config-attrs";
    description = "Work4You YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $WORK4YOU_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Work4You";
      ...
      package = mkOption {
        ...
        description = "The work4you package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Work4You configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `work4you config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $WORK4YOU_HOME/.env. Work4You reads that file at each start, with
          load_work4you_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."work4you/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Work4You refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Work4You reads SOUL.md and memories/ from WORK4YOU_HOME, so put those
          files in `work4youHomeFiles`.
      ...
      work4youHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into WORK4YOU_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Work4You reads SOUL.md and the memory files from WORK4YOU_HOME and not
          from the working directory. Declare those files here, or Work4You
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the work4you plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Work4You discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          work4you.plugins entry-point group. Each package must be built
          with the same Python interpreter as work4you (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in work4you's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `work4you gateway`.";
      };
      ...
      # `work4you serve` and `work4you dashboard` are the same entry point,
      # work4you_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Work4You Desktop
      ...
      # controls an external gateway, with `work4you gateway restart`. It does
        ...
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Work4You
                            Desktop connects to.
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "work4you-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "work4you-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "work4you-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "work4you-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "work4you: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # WORK4YOU_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.work4youHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Work4You make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${work4youHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Work4You
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${work4youHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${work4youHome}/config.yaml
            ${run}chmod ${modes.config} ${work4youHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "work4you-managed" managedSystem} ${work4youHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${work4youHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${work4youHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${work4youHome}/auth.json"
        else
          ''
            if [ ! -e ${work4youHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${work4youHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree work4youHome cfg.work4youHomeFiles}
      ...
      ${run}find ${work4youHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "work4you: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${work4youHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/work4you"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Work4You web dashboard and desktop backend"
    else
      "Work4You backend for Work4You Desktop";

  # The environment that each Work4You process needs, from either module.
  #
  # managedSystem gives the value of WORK4YOU_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      WORK4YOU_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give Work4You an identity and a memory, use
          ${optionPath}.work4youHomeFiles instead. Those files go to
          WORK4YOU_HOME. Work4You reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of WORK4YOU_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
