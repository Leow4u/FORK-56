...
machine as Work4You itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Work4You ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Work4You's own uv call sites: this
    ...
    # local.py puts the Work4You-managed $WORK4YOU_HOME/bin there, so a managed-only
