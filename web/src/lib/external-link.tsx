...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|work4you):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.work4youDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.work4youDesktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of Work4You, and it is the surface the agent
