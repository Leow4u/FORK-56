...
Maps each role archetype to the Work4You skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public work4you
...
## Work4You skills relevant to video production

### Visual / rendering skills (`work4you/skills/creative/`)
...
### Design / pre-production skills (`work4you/skills/creative/`)
...
### Audio / media skills (`work4you/skills/creative/` + `skills/media/`)
...
These are **not** Work4You skills but external CLIs / APIs that profiles invoke.
...
## Built-in Work4You tools for media review

These are native Work4You tools — not invoked via terminal but through their own
...
key is present in `${WORK4YOU_HOME:-~/.work4you}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Work4You profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${WORK4YOU_HOME:-~/.work4you}/.env` → environment variable.
...
When a new Work4You-public video skill ships:
