#!/usr/bin/env python3
"""Build the Work4You Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["work4you"]``) and writes them to a JSON manifest that the
Work4You CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Work4You release.
...
# Ensure WORK4YOU_HOME is set for imports that touch it at module level.
os.environ.setdefault("WORK4YOU_HOME", os.path.join(os.path.expanduser("~"), ".work4you"))

from work4you_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def _work4you_entry(mid: str) -> dict:
    entry: dict = {"id": mid}
    if mid == PREFERRED_SILENT_DEFAULT_MODEL:
        entry["default"] = True
    return entry
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "work4you repo",
            "docs": "https://work4you.ai/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Work4You '
            ...
            "work4you": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_work4you_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Work4You '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _work4you_entry(mid)
                    for mid in _PROVIDER_MODELS.get("work4you", [])
                ],
            },
        },
    }
