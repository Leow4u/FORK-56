...
2. Spawn: _default_spawn sets the WORK4YOU_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (work4you kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """work4you kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from work4you_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("work4you_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("work4you_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("work4you_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("work4you_cli.kanban._worker_run_id_for", lambda _: None)
