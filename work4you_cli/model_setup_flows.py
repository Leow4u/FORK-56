"""Per-provider model-selection wizard flows for ``work4you setup`` / ``work4you model``.

Extracted from ``work4you_cli/main.py`` as part of the god-file decomposition
campaign (``~/.work4you/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from work4you_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``work4you_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from work4you_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``work4you_cli.main`` at import time -> no import cycle.
"""
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from work4you_cli.auth import _resolve_api_key_provider_secret
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.work4you/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `work4you setup` from scratch. OpenRouter
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.work4you/.env by hand.
    ...
    from work4you_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from work4you_cli.auth import _save_model_choice, deactivate_provider
    from work4you_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `work4you moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    """Work4You Portal provider: ensure logged in, then pick model."""
    from work4you_cli.auth import (
        get_provider_auth_state,
        _prompt_model_selection,
        _save_model_choice,
        _update_config_for_provider,
        resolve_work4you_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_nous,
        PROVIDER_REGISTRY,
    ...
    from work4you_cli.work4you_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("work4you")
    if not state or not state.get("access_token"):
        print("Not logged into Work4You Portal. Starting login...")
        print()
        try:
            ...
            _login_nous(mock_args, PROVIDER_REGISTRY["work4you"])
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Work4You Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_work4you_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _work4you_portal_url = ""
    try:
        _work4you_state = get_provider_auth_state("work4you")
        if _work4you_state:
            _work4you_portal_url = _work4you_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Work4You Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from work4you_cli.auth import DEFAULT_WORK4YOU_PORTAL_URL

            _url = (_work4you_portal_url or DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_work4you_portal_url,
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Work4You as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("work4you", inference_url)
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `work4you auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``work4you auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from work4you_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from work4you_cli.auth import (
    ...
    from work4you_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from work4you_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from work4you_cli.auth import _save_model_choice, deactivate_provider
    from work4you_cli.config import (
    ...
    from work4you_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `work4you model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from work4you_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from work4you_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from work4you_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from work4you_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from work4you_cli.auth import _save_model_choice, deactivate_provider
    from work4you_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_copilot(config, current_model=""):
    ...
    from work4you_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from work4you_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from work4you_cli.config import save_env_value, load_config, save_config
    from work4you_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from work4you_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from work4you_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from work4you_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from work4you_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from work4you_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `work4you auth`)
    from work4you_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from work4you_cli.config import load_config, save_config, get_env_value
    from work4you_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.work4you/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``work4you_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from work4you_cli.main import _prompt_provider_choice
    from work4you_cli.auth import ZAI_ENDPOINTS
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Work4You."
                )
                print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from work4you_cli.main import _run_anthropic_oauth_flow
    from work4you_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'work4you model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
