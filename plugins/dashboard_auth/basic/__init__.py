...
into the same ``DashboardAuthProvider`` framework as the Work4You OAuth
...
mirroring the Work4You provider's precedence convention:
  ...
  Environment overrides::
      ...
      WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET
      WORK4YOU_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
Skip reasons:
  Like the Work4You provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "WORK4YOU_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "WORK4YOU_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "WORK4YOU_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )
    ...
    # WORK4YOU_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
