"""Upload a Work4You session transcript to Hugging Face as an agent trace.

Work4You stores sessions in its own SQLite store (``work4you_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``work4you trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Work4You' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "work4you-traces"
_WORK4YOU_VERSION = "work4you"
...
# ---------------------------------------------------------------------------
# Conversion: Work4You OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Work4You' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Work4You conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.work4you/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `work4you trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/work4you-traces`` dataset. Returns a
