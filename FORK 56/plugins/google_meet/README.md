...
Let the work4you join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where work4you runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `work4you meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `work4you meet` CLI |
...
| `cli.py` | `work4you meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$WORK4YOU_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `work4you meet node {run,list,approve,remove,status,ping}` |
...
work4you meet install                                      # pip + Chromium
work4you meet setup                                        # preflight
work4you meet auth                                         # optional
work4you meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
work4you meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.work4you/.env
work4you meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
work4you meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
work4you meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.work4you/.env
work4you meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, work4you will **not** switch your system audio input automatically — the
...
work4you plugins enable google_meet
work4you meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
work4you meet node ping my-mac
