 ...
 *   (`@work4you/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default Work4YouPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<work4you home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<work4you home>/plugins/<name>/desktop/
...
import { createPluginContext, type Work4YouPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.work4you/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@work4you/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @work4you/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: Work4YouPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default Work4YouPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of work4you-bots predating its adoption in-tree) must
...
//  - `<work4you home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<work4you home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's work4you_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.work4you/plugins` is installed-but-inert until the
