...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.work4you to the active profile home."""
    ...
    if raw.startswith("~/.work4you/"):
        return work4you_home / raw.removeprefix("~/.work4you/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return work4you_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under WORK4YOU_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from work4you_constants import get_work4you_home

    scripts_root = (get_work4you_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.work4you" or raw_text.startswith("~/.work4you/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__work4you_ignore__") is True
        ):
            return False, None
