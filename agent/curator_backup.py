...
A pre-run snapshot of ``~/.work4you/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.work4you/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.work4you/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_work4you_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.work4you/cron/jobs.json``)."""
    return get_work4you_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.work4you/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.work4you/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.work4you/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.work4you/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `work4you curator rollback --list` to see available snapshots)",
            None,
        )
