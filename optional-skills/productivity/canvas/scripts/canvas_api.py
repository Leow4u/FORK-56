#!/usr/bin/env python3
"""Canvas LMS API CLI for Work4You.
...
def _check_config():
    ...
    if missing:
        work4you_env = os.path.join(
            os.environ.get("WORK4YOU_HOME", os.path.expanduser("~/.work4you")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {work4you_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Work4You"
    )
