"""Per-terminal session breadcrumbs for ``work4you -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$WORK4YOU_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``work4you -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_work4you_home() / "terminal-sessions"
