...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.work4you/.env``. See Leow4u/FORK-56#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'work4you memory status'. Note: "
        "systemd/gateway services do not inherit ~/.work4you/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from work4you_cli.providers import WORK4YOU_OVERLAYS, get_provider

        overlay = WORK4YOU_OVERLAYS.get(provider)
    ...
    try:
        ...
        from work4you_cli.models import normalize_provider as normalize_model_provider
        from work4you_cli.providers import normalize_provider as normalize_registry_provider
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$WORK4YOU_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_work4you_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$WORK4YOU_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .work4you.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / WORK4YOU_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.work4you/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"work4you", "work4you-portal", "work4you"}:
        ...
        agent.api_mode = work4you_api_mode(agent.model)
    else:
        ...
        try:
            from work4you_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from work4you_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # x-work4you-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind WORK4YOU_DEV_CREDITS.
    ...
    # both live under ~/.work4you/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from work4you_logging import setup_logging, setup_verbose_logging
    setup_logging(work4you_home=_ra()._work4you_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that work4you_logging.setup_logging() does not
        ...
        # handler level inside work4you_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from work4you_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = work4you_xai_default_headers()
    ...
    # A multiplexed gateway may enter a different WORK4YOU_HOME after
    ...
    # worker (kanban_show tool is present iff WORK4YOU_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["WORK4YOU_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.work4you/sessions/ alongside gateway sessions
    work4you_home = get_work4you_home()
    agent.logs_dir = work4you_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.work4you/sessions/session_{sid}.json)
    ...
    try:
        from work4you_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `work4you --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from work4you_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Work4You behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "work4you"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Work4You' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "work4you", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, work4you, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from work4you_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from work4you_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from work4you_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Work4You.  Choose a model with at least "
    ...
    # Work4You Work4You 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from work4you_cli.model_switch import _check_work4you_model_warning

            _work4you_warn = _check_work4you_model_warning(agent.model or "")
            if _work4you_warn:
                _user_msg = (
                    "⚠ Work4You Work4You 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_work4you_warn)
        except Exception:
            pass
