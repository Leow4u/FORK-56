...
import { api, WORK4YOU_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "work4you:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${WORK4YOU_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${WORK4YOU_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?work4you_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-work4you-plugin", manifest.name);
