...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("work4you_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated WORK4YOU_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="work4you_api_content_")
