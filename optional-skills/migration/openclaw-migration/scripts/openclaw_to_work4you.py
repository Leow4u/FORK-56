#!/usr/bin/env python3
"""OpenClaw -> Work4You migration helper.
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Work4You.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Work4You memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Work4You memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Work4You-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Work4You-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Work4You command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.work4you/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.work4you/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Work4You .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Work4You .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Work4You .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Work4You .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Work4You .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Work4You config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Work4You config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Work4You.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Work4You MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Work4You config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'work4you cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Work4You config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Work4You session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Work4You custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Work4You config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Work4You config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Work4You config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `work4you config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`work4you config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.work4you/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Work4You config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Work4You in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Work4You`` (prose), but lowercase matches
# like ``openclaw`` → ``work4you`` (so filesystem paths like ``~/.openclaw``
# become ``~/.work4you`` — the real Work4You home — not the broken ``~/.Work4You``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Work4You'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Work4You'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Work4You'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Work4You`` but maps ``openclaw`` → ``work4you`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.work4you/config.yaml`` (the real Work4You home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Work4You.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Work4You memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Work4You Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-work4you/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Work4You config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-work4you/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Work4You session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Work4You config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Work4You
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Work4You-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Work4You-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: work4you config set",
                        )
                    continue
        ...
        for oc_key, work4you_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and work4you_key not in secret_additions:
                secret_additions[work4you_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, work4you_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and work4you_key not in secret_additions:
                        secret_additions[work4you_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        work4you_config = load_yaml_file(destination)
        current_model = work4you_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = work4you_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                work4you_config["model"] = {"default": model_str}
            dump_yaml_file(destination, work4you_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Work4You still uses "edge"
        ...
        work4you_config = load_yaml_file(destination)
        existing_tts = work4you_config.get("tts", {})
        ...
        if self.execute:
            ...
            work4you_config["tts"] = merged_tts
            dump_yaml_file(destination, work4you_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Work4You destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Work4You-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Work4You-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        work4you_cfg_path = self.target_root / "config.yaml"
        work4you_cfg = load_yaml_file(work4you_cfg_path)
        existing_mcp = work4you_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Work4You config")
                continue

            work4you_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                work4you_srv["command"] = srv["command"]
                if srv.get("args"):
                    work4you_srv["args"] = srv["args"]
                if srv.get("env"):
                    work4you_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    work4you_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                work4you_srv["url"] = srv["url"]
                if srv.get("headers"):
                    work4you_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    work4you_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                work4you_srv["enabled"] = False
            if srv.get("timeout"):
                work4you_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                work4you_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                work4you_srv["tools"] = {}
                if tools_cfg.get("include"):
                    work4you_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    work4you_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                work4you_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = work4you_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(work4you_cfg_path)
            work4you_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(work4you_cfg_path, work4you_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'work4you cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = work4you_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Work4You reasoning_effort
        ...
        if compaction:
            compression = work4you_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = work4you_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            work4you_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = work4you_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'work4you gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("WORK4YOU_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        work4you_cfg_path = self.target_root / "config.yaml"
        work4you_cfg = load_yaml_file(work4you_cfg_path)
        sr = work4you_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        work4you_cfg_path = self.target_root / "config.yaml"
        work4you_cfg = load_yaml_file(work4you_cfg_path)
        custom_providers = work4you_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(work4you_cfg_path)
            work4you_cfg["custom_providers"] = custom_providers
            dump_yaml_file(work4you_cfg_path, work4you_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Work4You config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            work4you_cfg_path = self.target_root / "config.yaml"
            work4you_cfg = load_yaml_file(work4you_cfg_path)
            discord_work4you = work4you_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_work4you["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_work4you["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                work4you_cfg["discord"] = discord_work4you
                dump_yaml_file(work4you_cfg_path, work4you_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        work4you_cfg_path = self.target_root / "config.yaml"
        work4you_cfg = load_yaml_file(work4you_cfg_path)
        browser_work4you = work4you_cfg.get("browser") or {}
        changed = False

        # Map fields that have Work4You equivalents
        if browser.get("cdpUrl"):
            browser_work4you["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_work4you["headless"] = browser["headless"]
            changed = True

        if changed:
            work4you_cfg["browser"] = browser_work4you
            if self.execute:
                self.maybe_backup(work4you_cfg_path)
                dump_yaml_file(work4you_cfg_path, work4you_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            work4you_mode = mode_map.get(mode, "manual")
            work4you_cfg.setdefault("approvals", {})["mode"] = work4you_mode
            if self.execute:
                self.maybe_backup(work4you_cfg_path)
                dump_yaml_file(work4you_cfg_path, work4you_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{work4you_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Work4You Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Work4You. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Work4You Config Not Overwritten)",
                "",
                "These items already existed in your Work4You config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Work4You agent discovers these",
            "directories, it may read/write to them instead of the Work4You state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `work4you claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `work4you claw cleanup` to fix it.",
            "",
            "## Work4You-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `work4you claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `work4you setup` to configure any remaining settings",
            "- Run `work4you mcp list` to verify MCP servers were imported correctly",
        ])
        ...
        notes.extend([
            "- Run `work4you gateway install` if you need the gateway service",
            "- Review `~/.work4you/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("WORK4YOU_HOME") or str(Path.home() / ".work4you"), help="Work4You home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Work4You targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Work4You-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Work4You Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.work4you/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.work4you/config.yaml")
        print("    2. Run: work4you mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: work4you cron")
