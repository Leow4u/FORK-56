...
Config files are stored in ~/.work4you/ for easy access:
- ~/.work4you/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.work4you/.env         - API keys and secrets

This module provides:
- work4you config          - Show current configuration
- work4you config edit     - Open config in editor
- work4you config get      - Print a resolved configuration value
- work4you config set      - Set a specific value
- work4you config unset    - Remove a user configuration value
- work4you config wizard   - Re-run setup wizard
"""
...
from work4you_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``work4you config set`` (which rewrites
    ...
    ``config.yaml`` in place: work4you never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.work4you/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``work4you logs``
...
#   the next ``subprocess.run([...])`` Work4You makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Work4You itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``work4you update``, the TUI build.
...
#   on every plugin install / ``work4you update``.
...
# * ``WORK4YOU_HOME`` / ``WORK4YOU_PROFILE`` / ``WORK4YOU_CONFIG`` /
#   ``WORK4YOU_ENV`` — Work4You runtime location flags. Writing these into
...
#
# IMPORTANT: ``WORK4YOU_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (WORK4YOU_LANGFUSE_PUBLIC_KEY,
# WORK4YOU_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONWORK4YOUERSITE",
    # Node
    ...
    # Work4You runtime location — never via dashboard env writer.
    # NOT a WORK4YOU_* blanket: integration credentials (WORK4YOU_GEMINI_*,
    # WORK4YOU_LANGFUSE_*, WORK4YOU_SPOTIFY_*, ...) ARE allowed.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # WORK4YOU_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `work4you plugins enable
    # observability/langfuse` or `work4you tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "WORK4YOU_LANGFUSE_ENV",
    "WORK4YOU_LANGFUSE_RELEASE",
    "WORK4YOU_LANGFUSE_SAMPLE_RATE",
    "WORK4YOU_LANGFUSE_MAX_CHARS",
    "WORK4YOU_LANGFUSE_CAPTURE",
    "WORK4YOU_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "WORK4YOU_ACP_AUTH_METHOD",
    "WORK4YOU_ACP_AUTO_APPROVE",
    "WORK4YOU_COPILOT_ACP_COMMAND",
    "WORK4YOU_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from work4you_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set WORK4YOU_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("WORK4YOU_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_work4you_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # WORK4YOU_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Work4You is running in package-manager-managed mode.

    Two signals: the WORK4YOU_MANAGED env var (set by the systemd service),
    or a .managed marker file in WORK4YOU_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Work4You through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/work4you`` inside the published image. It is a property of
    the running interpreter, NOT of ``$WORK4YOU_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$WORK4YOU_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. WORK4YOU_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.work4you``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$WORK4YOU_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.work4you:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$WORK4YOU_HOME``, a
    ...
    ``docker`` and ``work4you update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``work4you/work4you`` image bakes a ``docker``
    ...
    #    host install whose shared $WORK4YOU_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``work4you update``.
    try:
        method = (
            (get_work4you_home() / ".install_method")
    ...
    # detect Nix installs that don't set WORK4YOU_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``work4you_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$WORK4YOU_HOME``, so that two installs sharing one data directory
...
# Long-form text for ``work4you update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# work4you_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Work4You, it doesn't update
...
Work4You runs as a published image (work4you/work4you), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull work4you/work4you:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate work4you
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm work4you/work4you:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/work4you/work4you/tags
  • Your config and session history live under ``$WORK4YOU_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from WORK4YOU_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, work4you_bin
    or None if container mode is not active, we're already inside the
    container, or WORK4YOU_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("WORK4YOU_DEV") == "1":
        return None
    ...
    container_mode_file = get_work4you_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "work4you")
    exec_user = info.get("exec_user", "work4you")
    work4you_bin = info.get("work4you_bin", "/data/current-package/bin/work4you")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "work4you_bin": work4you_bin,
    }
...
# Re-export from work4you_constants — canonical definition lives there.
from work4you_constants import get_work4you_home, get_process_work4you_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_work4you_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_work4you_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_work4you_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the WORK4YOU_UID / WORK4YOU_GID env vars set by Docker deployments.

    Docker containers running Work4You commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level WORK4YOU_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("WORK4YOU_UID", "").strip()
    gid_str = os.environ.get("WORK4YOU_GID", "").strip()
    try:
...
def _chown_to_work4you_uid(path) -> None:
    """Chown ``path`` to ``WORK4YOU_UID:WORK4YOU_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_work4you_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the work4you group can
    share state with the gateway service.

    The mode can be overridden via the WORK4YOU_HOME_MODE environment variable
    (e.g. WORK4YOU_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse WORK4YOU_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``WORK4YOU_UID``/``WORK4YOU_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("WORK4YOU_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_work4you_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Work4You runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("WORK4YOU_CONTAINER") or os.environ.get("WORK4YOU_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set WORK4YOU_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into WORK4YOU_HOME, upgrading legacy empty templates.
...
# ensure_work4you_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_WORK4YOU_HOME_ENSURED: set = set()


def ensure_work4you_home():
    """Ensure ~/.work4you directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``WORK4YOU_HOME`` we only re-run
    ...
    ``get_work4you_home()`` and therefore re-run for the new path.
    """
    home = get_work4you_home()
    key = str(home)

    if key in _WORK4YOU_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``work4you profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old WORK4YOU_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_work4you_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _WORK4YOU_HOME_ENSURED.add(key)


def _ensure_work4you_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"WORK4YOU_HOME {home} does not exist."
        )
...
from work4you_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Work4You Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `work4you config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.work4you.config`` entries, then checks
    ...
    try:
        ...
        # should never break `work4you update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Work4You' own config writer has historically emitted it
    ...
    # Work4You (``_save_discovered_models_to_config``), not hand-curated.
    # Older Work4You versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Work4You versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``work4you_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (work4you_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (work4you_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (work4you_cli/setup.py)
    "known_plugin_toolsets", # written/read by work4you_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, work4you send) so
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_work4you_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "work4you update / work4you -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from work4you_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ work4you config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in work4you_cli.config_migrations as a
    ...
    # `work4you update`, but only the version bump is persisted.
    ...
    # metadata.work4you.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.work4you/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_work4you_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``Work4YouCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``work4you config set``
    ...
    backends) — while display paths (``work4you status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _work4you_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in work4you_cli.personality.
from work4you_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`work4you_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from work4you_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from work4you_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``WORK4YOU_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`work4you_cli.personality` (single owner).
    """
    from work4you_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.work4you/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_work4you_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.work4you/config.yaml.
    ...
    (which change ``WORK4YOU_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Work4You host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/work4you/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_work4you_dotenv()
...
#   openai-codex (OAuth — work4you auth) — OpenAI Codex
#   work4you         (OAuth — work4you auth) — Work4You Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.work4you/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Work4You writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.work4you/.env.
    ...
    interactive menu render (`work4you tools`, `work4you setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `work4you tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.work4you/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.work4you/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``WORK4YOU_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"WORK4YOU_CUSTOM_{slug}_API_KEY" if slug else "WORK4YOU_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.work4you/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.work4you/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Work4You — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Work4You vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.work4you/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.work4you/.env`` over ``os.environ``.

    Used for Work4You-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from work4you_cli.auth import get_anthropic_key
    ...
    # Warn on stale WORK4YOU_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("WORK4YOU_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale WORK4YOU_MAX_ITERATIONS={_env_ghost} "
    ...
    try:
        from work4you_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``WORK4YOU_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("WORK4YOU_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `work4you cron list`, then pin the "
        "intended values with `work4you cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (work4you_cli/plugins_cmd.py, work4you_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `work4you config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``work4you config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``work4you config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``work4you config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (work4you config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        if value.lower() in {'true', 'yes', 'on'}:
            ...
            #   work4you config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   work4you config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``work4you config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``work4you config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    work4you config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    work4you config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `work4you config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_work4you_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `work4you config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `work4you config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: work4you config get <key> [--json]")
            print()
            print("Examples:")
            print("  work4you config get model")
            print("  work4you config get terminal.backend")
            print("  work4you config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: work4you config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: work4you config unset <key>")
            print()
            print("Examples:")
            print("  work4you config unset model")
            print("  work4you config unset terminal.backend")
            print("  work4you config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  work4you config get <key>          Print a resolved config value")
        print("  work4you config set <key> <value>   Set a config value")
        print("  work4you config unset <key>        Remove a config value")
...
# ``_inject_profile_env_vars`` surfaces them in ``work4you config`` UI so users
