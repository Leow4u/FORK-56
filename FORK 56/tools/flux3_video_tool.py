"""Native BFL FLUX 3 video generation tools, backed by the Work4You tool gateway.
...
there is no Work4You sign-in to call it with — never on entitlement, which is the
...
(sandbox confinement, credential guard) and delivered via the Work4You upload
protocol (presign, direct PUT to storage, ``work4you-upload:<token>`` reference).
...
from tools.managed_tool_gateway import (
    ...
    peek_work4you_access_token,
    read_work4you_access_token,
)
...
_SIGN_IN_MESSAGE = (
    "BFL video generation needs a Work4You Portal sign-in. "
    "Ask the user to run `work4you model` and sign in to Work4You, then retry."
)
...
def _warm_work4you_token() -> None:
    """Refresh the Work4You token once, before any parallel upload needs it.

    ``read_work4you_access_token`` takes no lock and, when a refresh fails, falls
    ...
    try:
        ...
        logger.debug("Work4You token warm-up failed before parallel uploads: %s", exc)
...
async def _deliver_media(value, permitted: tuple, task_id: Optional[str]):
    """Replace a local path with a ``work4you-upload:`` reference; pass URLs through.
...
def _default_directory():
    ...
    is not: an operator running WORK4YOU_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_work4you_dir("cache/videos", "video_cache")
        except Exception:
...
def _has_work4you_credential() -> bool:
    """True when a Work4You bearer is on hand, without spending a refresh to learn it.

    Two lookups, because the transport itself has two.
    ``peek_work4you_access_token`` covers the env override and the active store's
    ...
    fallback ``resolve_work4you_access_token`` takes when the transport refreshes.
...
def check_bfl_requirements() -> bool:
    """Visible to anyone signed in to Work4You; the gateway rules on the rest.
    ...
    A sign-in is still required, because the gateway takes a Work4You bearer and
    ...
    try:
        if _endpoints() is None:
            return False
        return _has_work4you_credential()
    except Exception:
        return False
...
_MEDIA_SENTENCE = (
    "Media fields accept a local file path (uploaded automatically to Work4You-managed temporary "
