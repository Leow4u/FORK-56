...
selectable via ``work4you tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Work4You Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Work4You
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Work4You Subscription)
# ---------------------------------------------------------------------------
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `work4you tools` selection.

    Dispatch is a plain switch on the stored ``image_gen`` provider string:
    - ``"work4you"`` (or legacy ``use_gateway: true``) → managed fal-queue
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.work4you_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Work4You Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Work4You Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `work4you tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.work4you"
    ...
    # Work4You cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `work4you tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `work4you tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Work4You tools are enabled)
    ...
    if managed_work4you_tools_enabled():
        ...
        gateway_message = work4you_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_work4you_tools_enabled():
        lines.append(
            "  2. Sign in to a Work4You account that has the managed FAL "
            "gateway enabled (`work4you setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `work4you tools` "
        "→ Image Generation (run `work4you plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    if not configured or configured in ("fal", WORK4YOU_MANAGED_PROVIDER):
        return False
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", WORK4YOU_MANAGED_PROVIDER):
        # Unset/explicit FAL keeps the legacy FAL path; "work4you" (managed
        # Work4You Subscription selection) also runs the legacy pipeline, which
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `work4you plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `work4you tools` → Image Generation."
                ),
...
def _maybe_route_managed_krea(
    ...
    # (other than the managed "work4you" selection, which IS a managed-mode
    ...
    if configured_provider is not None and configured_provider != WORK4YOU_MANAGED_PROVIDER:
        return None
...
# model/provider via `work4you tools` or `/skills`.
