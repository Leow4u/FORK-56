...
# Cache validation (mirrors work4you_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.work4you/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set WORK4YOU_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped WORK4YOU_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from WORK4YOU_HOME/.env."""
    env_path = get_work4you_home() / ".env"
    ...
    # the canonical readers in work4you_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set WORK4YOU_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (WORK4YOU_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("WORK4YOU_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("WORK4YOU_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_work4you_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.work4you/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``WORK4YOU_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``WORK4YOU_PLATFORM`` environment variable
    3. ``WORK4YOU_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from work4you_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("WORK4YOU_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.work4you/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``work4you skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile WORK4YOU_HOME; the module-level
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `work4you skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.work4you/skills/): {skill_md}")
        ...
        # Check metadata.work4you.* first (agentskills.io convention), fall back to top-level
        work4you_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            work4you_meta = metadata.get("work4you", {}) or {}

        tags = _parse_tags(work4you_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            work4you_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `work4you sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
