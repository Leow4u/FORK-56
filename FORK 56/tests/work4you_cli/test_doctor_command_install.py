"""Tests for the Command Installation check in work4you doctor."""
...
import work4you_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal WORK4YOU_HOME + PROJECT_ROOT for doctor tests."""
    ...
    work4you_bin = venv_bin_dir / "work4you"
    work4you_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    work4you_bin.chmod(0o755)
    ...
    return home, project, work4you_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, work4you_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "work4you"
        wrong_target = tmp_path / "wrong_work4you"
        ...
        assert cmd_link.resolve() == work4you_bin.resolve()
