...
`mcp_tool.py`, and `work4you_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# Work4YouMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_work4you_provider_class() -> Optional[type]:
    ...
    class Work4YouMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._work4you_server_name = server_name
            self._work4you_home = ""
            ...
            self._work4you_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`Work4YouTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, Work4YouTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._work4you_server_name, exc,
                    )
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Work4You' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._work4you_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import Work4YouTokenStorage
                        if isinstance(storage, Work4YouTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._work4you_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import Work4YouTokenStorage
            if not isinstance(storage, Work4YouTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``work4you mcp reauth``.
            ...
            back to ``work4you mcp reauth``.
            """
            try:
                if self._work4you_preregistered:
                    return
                ...
                # walking back into the same refusal. `work4you mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._work4you_server_name, cimd_url,
                    )
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._work4you_server_name,
                    work4you_home=self._work4you_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return Work4YouMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_WORK4YOU_PROVIDER_CLS: Optional[type] = _make_work4you_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._work4you_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`Work4YouMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _WORK4YOU_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `work4you mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``work4you mcp remove <name>`` and (indirectly) by
        ``work4you mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, work4you_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, work4you_home=work4you_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, work4you_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, work4you_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, work4you_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(work4you_home) / f"{_safe_filename(server_name)}.json"
            try:
