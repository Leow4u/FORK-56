"""Regression test: ``work4you mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``work4you_cli/_parser.py``).  The dispatcher in ``work4you_cli/main.py``
...
result, ``work4you mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the work4you parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`work4you mcp add foo --url ...` must keep args.command == "mcp".
