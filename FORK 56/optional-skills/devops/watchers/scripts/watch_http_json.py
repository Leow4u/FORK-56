...
Usage (via cron with --no-agent):
    ...
    work4you cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$WORK4YOU_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Work4You-Watcher/1.0"})
