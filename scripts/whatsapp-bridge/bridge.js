 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.work4you/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.work4you', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (WORK4YOU_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.WORK4YOU_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.work4you', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.WORK4YOU_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.work4you', 'document_cache');
const AUDIO_CACHE_DIR = process.env.WORK4YOU_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.work4you', 'audio_cache');
...
// `work4you update` updates bridge.js on disk but a long-lived bridge process
...
// Keep in sync with gateway/platforms/whatsapp_common.py DEFAULT_REPLY_PREFIX.
const DEFAULT_REPLY_PREFIX = '*Work4You*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Work4You itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Work4You and trigger
      ...
      // Ignore Work4You' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `work4you_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `work4you_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
