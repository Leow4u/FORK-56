...
Supports OAuth device code flows (Work4You Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.work4you/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and work4you_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("work4you_cli.auth.httpx.Client", ...) keeps working in tests.
...
# Work4You Portal defaults
DEFAULT_WORK4YOU_PORTAL_URL = "https://portal.work4you.ai"
DEFAULT_WORK4YOU_INFERENCE_URL = "https://inference-api.work4you.ai/v1"
DEFAULT_WORK4YOU_CLIENT_ID = "work4you-cli"
WORK4YOU_INFERENCE_INVOKE_SCOPE = "inference:invoke"
WORK4YOU_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_WORK4YOU_SCOPE = WORK4YOU_INFERENCE_INVOKE_SCOPE
WORK4YOU_DEVICE_CODE_SOURCE = "device_code"
WORK4YOU_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
WORK4YOU_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from work4you_cli import __version__ as _WORK4YOU_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _WORK4YOU_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"work4you-cli/{_WORK4YOU_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://work4you.ai/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://work4you.ai/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Work4You' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "work4you": ProviderConfig(
        id="work4you",
        name="Work4You Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_WORK4YOU_PORTAL_URL,
        inference_base_url=DEFAULT_WORK4YOU_INFERENCE_URL,
        client_id=DEFAULT_WORK4YOU_CLIENT_ID,
        scope=DEFAULT_WORK4YOU_SCOPE,
    ),
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Work4You finding a setup-token
...
def get_anthropic_key() -> str:
    ...
    ``~/.work4you/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from work4you_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from work4you_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.work4you/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``work4you auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.code == "subscription_required":
        if error.provider == "work4you":
            return _format_work4you_entitlement_auth_error(error)
...
def _format_work4you_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        account_info = get_work4you_portal_account_info(force_fresh=True)
        message = format_work4you_portal_entitlement_message(
            account_info,
            capability="Work4You model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Work4You Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("WORK4YOU_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.work4you/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_work4you_home() / "auth.json"
    # Seat belt: if pytest is running and WORK4YOU_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch WORK4YOU_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".work4you" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set WORK4YOU_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom WORK4YOU_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_work4you_root()
    except Exception:
        return None
    profile_home = get_work4you_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``_work4you_auth_status_cache``): read_credential_pool() -> load_pool() runs
    this once per provider row in the /model picker, and the path resolution
    (``_global_auth_file_path()`` -> ``get_default_work4you_root()``) + JSON
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".work4you" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Work4You store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_work4you_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Work4You lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_work4you_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "work4you_portal" in systems:
            providers["work4you"] = systems["work4you_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "work4you" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Work4You refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``work4you``) that were only authenticated at
    global scope. Once the user runs ``work4you auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``work4you auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``work4you auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Work4You device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Work4You
    ...
    # not by the user explicitly configuring anthropic in Work4You.
    ...
    # the user initiated inside Work4You (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "work4you_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `work4you logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from work4you_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    # Auto-detect an OpenRouter credential added via `work4you auth add openrouter`
    ...
    # `work4you auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.work4you/.env isn't silently
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.work4you/.env.",
...
_WORK4YOU_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.work4you.ai",
})

# Allowlist of valid Work4You Portal hosts. A portal_base_url outside this
...
_WORK4YOU_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.work4you.ai",
    "localhost",
    "127.0.0.1",
})
...
def _migrate_stale_work4you_portal_url(providers: Dict[str, Any]) -> None:
    work4you = providers.get("work4you")
    if not isinstance(work4you, dict):
        return
    stored = (work4you.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _WORK4YOU_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale work4you portal_base_url %s -> %s",
                stored, DEFAULT_WORK4YOU_PORTAL_URL,
            )
            work4you["portal_base_url"] = DEFAULT_WORK4YOU_PORTAL_URL


# Allowlist of hosts the Work4You Portal proxy is willing to forward inference
...
# (WORK4YOU_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_WORK4YOU_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.work4you.ai",
})


def _validate_work4you_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``WORK4YOU_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "work4you: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_WORK4YOU_INFERENCE_HOSTS:
        logger.warning(
            "work4you: refusing inference URL host %r from Portal response "
...
def _work4you_inference_env_override() -> Optional[str]:
    """Return the user-set ``WORK4YOU_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("WORK4YOU_INFERENCE_BASE_URL"))


def _work4you_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_work4you_inference_env_override()``: ``WORK4YOU_PORTAL_BASE_URL`` /
    ``WORK4YOU_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Work4You at a non-production Work4You Portal (e.g. a hosted agent
    provisioned on work4you-account-service's `staging` environment, which stamps
    ``WORK4YOU_PORTAL_BASE_URL=https://portal.staging-work4you.ai`` into
    ...
    gated by ``_WORK4YOU_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("WORK4YOU_PORTAL_BASE_URL") or os.getenv("WORK4YOU_PORTAL_BASE_URL")
    )
...
def _work4you_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = WORK4YOU_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if WORK4YOU_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _work4you_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_work4you_inference_jwt_usable(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
) -> None:
    token = state.get("access_token") if access_token is None else access_token
    reason = _work4you_invoke_jwt_status(
        token,
    ...
    if reason is None:
        return
    raise AuthError(
        "Work4You Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: work4you auth add work4you",
        provider="work4you",
        code=reason,
...
def _log_work4you_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.debug("Work4You inference auth: using NAS invoke JWT")
    _oauth_trace(
        "work4you_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _work4you_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_work4you_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _work4you_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_work4you_invoke_jwt(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
    sequence_id: Optional[str] = None,
    ...
    _set_work4you_agent_key_from_invoke_jwt(state)
    _log_work4you_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_WORK4YOU_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _work4you_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _WORK4YOU_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("WORK4YOU_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `work4you model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.work4you/auth.json
# =============================================================================
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("WORK4YOU_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("WORK4YOU_SPOTIFY_ACCOUNTS_BASE_URL"),
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Work4You / xAI-OAuth / Codex-OAuth / MiniMax pattern.
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.work4you/.env, and return it.
    ...
    print("       App name:     anything (e.g. work4you)")
    ...
    # Persist so subsequent `work4you auth spotify` runs skip the wizard.
    ...
    print()
    print("Saved WORK4YOU_SPOTIFY_CLIENT_ID to ~/.work4you/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "WORK4YOU_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.work4you/auth.json (not ~/.codex/)
#
# Work4You maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Work4You auth store (~/.work4you/auth.json).
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``work4you setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``work4you auth add openai-codex``
      ...
      ``work4you auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Work4You auth store (~/.work4you/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``work4you auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Work4You auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Work4You auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run work4you auth" prompt (see issue #32790).
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Work4You auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Work4You keeps its OWN
        ...
        # the Codex CLI (or another Work4You process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Work4You's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("WORK4YOU_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("WORK4YOU_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Work4You processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Work4You persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.work4you/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.work4you/auth.json even when WORK4YOU_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``work4you auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.work4you/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``WORK4YOU_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `work4you model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Work4You (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `work4you model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``work4you model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("WORK4YOU_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("WORK4YOU_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("WORK4YOU_CA_BUNDLE")
...
def _work4you_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for Work4You device-code login failures.
    ...
    """
    portal = (portal_base_url or DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/")
    return (
...
def _poll_for_token(
    ...
    # the CLI login (_work4you_device_code_login) and the dashboard/desktop
    # poller (web_server._work4you_poller, which surfaces str(e) to the UI).
    raise TimeoutError(_work4you_device_auth_timeout_message(portal_base_url))


# =============================================================================
# Work4You Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Work4You token store — lets OAuth credentials persist across profiles
# so a new `work4you --profile <name> auth add work4you --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${WORK4YOU_SHARED_AUTH_DIR}/work4you_auth.json, defaulting to
# ``<work4you-root>/shared/work4you_auth.json`` where ``<work4you-root>`` is what
# ``get_default_work4you_root()`` returns — ``~/.work4you`` on Linux/macOS,
# ``%LOCALAPPDATA%\work4you`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's WORK4YOU_HOME so named profiles (which
# typically live under ``<work4you-root>/profiles/<name>/``) all see the
# same file.
#
...
WORK4YOU_SHARED_STORE_FILENAME = "work4you_auth.json"
_work4you_shared_lock_holder = threading.local()


def _work4you_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Work4You token store.

    Honors ``WORK4YOU_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<work4you-root>/shared/``, where ``<work4you-root>`` is what
    :func:`work4you_constants.get_default_work4you_root` returns — so
    Linux/macOS classic installs land at ``~/.work4you/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\work4you\\shared\\``, and
    Docker / custom ``WORK4YOU_HOME`` deployments at
    ``<WORK4YOU_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("WORK4YOU_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from work4you_constants import get_default_work4you_root
    return get_default_work4you_root() / "shared"


def _work4you_shared_store_path() -> Path:
    path = _work4you_shared_auth_dir() / WORK4YOU_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Work4You root, refuse rather than silently corrupt cross-profile
    # state. Tests must set WORK4YOU_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from work4you_constants import get_default_work4you_root
        real_home_shared = (
            get_default_work4you_root() / "shared" / WORK4YOU_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Work4You auth store during test run: "
                f"{path}. Set WORK4YOU_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _work4you_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Work4You OAuth store.
    ...
    ``_try_import_shared_work4you_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _work4you_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No WORK4YOU_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _work4you_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Work4You auth lock",
    ):
        yield


def _merge_shared_work4you_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Work4You state."""
    shared = _read_shared_work4you_state()
    if not shared:
        return False
...
def _write_shared_work4you_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Work4You OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_WORK4YOU_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_WORK4YOU_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_WORK4YOU_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_WORK4YOU_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _work4you_shared_store_lock():
            path = _work4you_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Work4You
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "work4you_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Work4You auth store: %s", exc)


def _read_shared_work4you_state() -> Optional[Dict[str, Any]]:
    """Return the shared Work4You OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Work4You auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_work4you_state(reason: str) -> None:
    """Remove the shared Work4You OAuth store after a terminal token failure."""
    try:
        with _work4you_shared_store_lock():
            path = _work4you_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("work4you_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Work4You auth store: %s", exc)


def _is_terminal_work4you_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Work4You refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "work4you"
...
def _quarantine_work4you_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Work4You
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Work4You state; provenance is client_id +
    ...
    logger.warning(
        "Work4You OAuth state quarantined (terminal auth death): %s",
    ...
    _clear_shared_work4you_state(reason)
    invalidate_work4you_auth_status_cache()


def _quarantine_work4you_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Work4You pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("work4you")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {WORK4YOU_DEVICE_CODE_SOURCE, f"manual:{WORK4YOU_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["work4you"] = retained
        _oauth_trace(
            "work4you_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_work4you_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Work4You OAuth state from the shared store.
    ...
    for ``persist_work4you_credentials()``.
    ...
    """
    try:
        with _work4you_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_work4you_state()
            if not shared:
                return None

            # Build a full state dict so refresh_work4you_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_WORK4YOU_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_WORK4YOU_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_WORK4YOU_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_WORK4YOU_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_work4you_state(updated_state)

            refreshed = refresh_work4you_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_work4you_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "work4you_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_work4you_refresh_error(exc):
            _clear_shared_work4you_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Work4You import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "work4you_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-work4you-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="work4you", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="work4you", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Work4You portal
    ...
    # called POST /api/oauth/token with Work4You's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Work4You's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Work4You Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Work4You install sharing "
            "~/.work4you/auth.json) called POST /api/oauth/token with Work4You's "
            "refresh token without persisting the rotated token back.\n"
            "Work4You refresh tokens are single-use — only Work4You may call the "
            "refresh endpoint. For health checks, use `work4you auth status` "
            "instead.\n"
            "Re-authenticate with: work4you auth add work4you"
        )
        relogin = True

    raise AuthError(description, provider="work4you", code=code, relogin_required=relogin)


def fetch_work4you_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Work4You inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="work4you", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Work4You models — they're not reliable for agentic tool-calling
            if "work4you" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _work4you_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
# Per-process memo for resolve_work4you_access_token. Startup runs
...
def resolve_work4you_access_token(
    *,
    ...
    """Resolve a refresh-aware Work4You Portal access token for managed tool gateways."""
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("work4you") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Work4You is not logged into Work4You Portal.",
                provider="work4you",
                relogin_required=True,
            )

        # WORK4YOU_PORTAL_BASE_URL / WORK4YOU_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors WORK4YOU_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _work4you_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_WORK4YOU_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _WORK4YOU_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_WORK4YOU_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _work4you_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_work4you_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Work4You Portal login.",
                    provider="work4you",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "work4you", state, state_source_path)
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_work4you_refresh_error(exc):
                        _quarantine_work4you_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_work4you_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
...
def refresh_work4you_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_WORK4YOU_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh Work4You OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_WORK4YOU_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_WORK4YOU_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_WORK4YOU_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _work4you_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Work4You Portal.",
                    provider="work4you",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_work4you_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_WORK4YOU_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_work4you_inference_jwt_usable(state)
        _select_work4you_invoke_jwt(state)

    return state


def refresh_work4you_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Work4You OAuth from a state dict. Thin wrapper around refresh_work4you_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_work4you_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "work4you-cli"),
        state.get("portal_base_url", DEFAULT_WORK4YOU_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_WORK4YOU_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_WORK4YOU_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_work4you_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Work4You OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Work4You credentials are read at runtime from two independent locations:

    - ``providers.work4you``: singleton state read by
      ``resolve_work4you_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.work4you``: used by the runtime ``pool.select()`` path.

    Historically ``work4you auth add work4you`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.work4you``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.work4you`` then calls ``load_pool("work4you")`` so
    ...
    ``work4you auth add work4you --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("work4you")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "work4you", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `work4you auth add work4you --type oauth`. Best-
...
def _sync_work4you_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.work4you changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("work4you")
    except Exception as exc:
        logger.debug("Failed to sync Work4You credential pool from auth store: %s", exc)


def resolve_work4you_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Work4You inference credentials for runtime use.
    ...
    with _provider_state_transaction("work4you") as (
        ...
        if not state:
            raise AuthError("Work4You is not logged into Work4You Portal.",
                            provider="work4you", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _WORK4YOU_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_WORK4YOU_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_work4you_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_WORK4YOU_INFERENCE_URL
            )
            effective_inference_url = (
                _work4you_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_WORK4YOU_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Work4You auth-status cache warm during read paths.
            if (
                _work4you_effective_provider_state(state)
                == _work4you_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "work4you_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_work4you_state.
            _write_shared_work4you_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "work4you_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _work4you_shared_store_lock(
                    ...
                    if _merge_shared_work4you_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Work4You Portal login.",
                                provider="work4you", relogin_required=True)

            invoke_jwt_status = _work4you_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _work4you_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_work4you_oauth_pure):
                        ...
                        # persisted to auth.json below. The WORK4YOU_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_work4you_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_WORK4YOU_INFERENCE_URL
                        inference_base_url = (
                            _work4you_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_work4you_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_work4you_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_work4you_runtime_credentials_final")

    if state_persisted:
        _sync_work4you_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Work4You inference API key",
                        provider="work4you", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": WORK4YOU_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": WORK4YOU_AUTH_PATH_INVOKE_JWT,
...
def _empty_work4you_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_work4you_pool_status() -> Dict[str, Any]:
    """Best-effort status from the credential pool.

    This is a fallback only. The auth-store provider state is the runtime source
    of truth because it is what ``resolve_work4you_runtime_credentials()`` refreshes.
    """
    try:
        from agent.credential_pool import load_pool
        ...
        if not pool or not pool.has_credentials():
            return _empty_work4you_auth_status()
...
# ── Process-level memo for get_work4you_auth_status() ──
# get_work4you_auth_status() validates state by calling resolve_work4you_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.work4you.ai. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`work4you tools`, status panels,
# subscription-feature checks) call it many times per render — `work4you tools` → "All Platforms"
...
# `work4you auth login/logout/add/remove` invalidate naturally on the next call.
_WORK4YOU_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_work4you_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None

# mtime-keyed memo for _load_global_auth_store(): (path, mtime_ns, store).
# Same invalidation contract as _work4you_auth_status_cache — the global auth
...
def invalidate_work4you_auth_status_cache() -> None:
    """Clear the get_work4you_auth_status() process-level memo.

    Call this from any code path that mutates Work4You auth state without going
    through resolve_work4you_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _work4you_auth_status_cache
    _work4you_auth_status_cache = None


def get_work4you_auth_status() -> Dict[str, Any]:
    """Status snapshot for Work4You auth.
    ...
    ``invalidate_work4you_auth_status_cache()`` explicitly.
    """
    global _work4you_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _work4you_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _WORK4YOU_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_work4you_auth_status()
    _work4you_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_work4you_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_work4you_auth_status(). See that function."""
    state = get_provider_auth_state("work4you")
    if state:
        base_status = {
        ...
        try:
            creds = resolve_work4you_runtime_credentials()
            refreshed_state = get_provider_auth_state("work4you") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_work4you_pool_status()


def get_work4you_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Work4You auth snapshot for read-only display surfaces.

    Unlike :func:`get_work4you_auth_status`, this NEVER calls
    ``resolve_work4you_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_work4you_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _work4you_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``work4you_session_valid``.
...
WORK4YOU_SESSION_VALID = "valid"
WORK4YOU_SESSION_TERMINAL = "terminal"
WORK4YOU_SESSION_UNKNOWN = "unknown"


def get_work4you_session_validity() -> str:
    """Classify the Work4You bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Work4You credential is present (login healthy).
      - ``"terminal"`` — the Work4You session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Work4You provider state, or a transient/
    ...
    # into the Work4You provider state when it clears dead tokens (the exact path
    ...
    try:
        ...
        return WORK4YOU_SESSION_UNKNOWN
    ...
    if isinstance(last_err, dict) and last_err.get("relogin_required"):
        ...
        if not (state.get("access_token") or state.get("refresh_token")):
            return WORK4YOU_SESSION_TERMINAL

    if _work4you_invoke_jwt_status(
        state.get("access_token"),
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    ) is None:
        return WORK4YOU_SESSION_VALID
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `work4you auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `work4you auth` and
    # `work4you model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("WORK4YOU_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("WORK4YOU_COPILOT_ACP_ARGS", "").strip()
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "work4you":
        return get_work4you_auth_status()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``work4you doctor`` runs the live
    ...
    try:
        from work4you_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Work4You' "
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from work4you_cli.copilot_auth import (
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `work4you logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"work4you", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`work4you_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    sale_chrome = (confirm_provider or "").strip().lower() == "work4you"

    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (work4you_cli.model_selection_guards): the cost
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/")
    ...
    from work4you_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from work4you_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'work4you model' or 'work4you setup' instead."""
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.work4you/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Work4You-owned credentials
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("WORK4YOU_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    # Run a fresh device code flow — Work4You gets its own OAuth session
    print()
    ...
    # Save tokens to Work4You auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from work4you_constants import display_work4you_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``work4you auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``work4you auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Work4You auth store (~/.work4you/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Work4You login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Work4You / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``work4you status`` that just want to know
...
def _work4you_device_code_login(
    *,
    ...
    """Run the Work4You device-code flow and return full OAuth state without persisting."""
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("WORK4YOU_INFERENCE_BASE_URL")
    ...
    print(f"Starting Work4You login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_work4you_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
...
def work4you_token_has_billing_scope() -> bool:
    """Return True if the currently-held Work4You token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return WORK4YOU_BILLING_MANAGE_SCOPE in scope.split()


def step_up_work4you_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``WORK4YOU_PORTAL_BASE_URL`` set
    ...
    """
    prior = get_provider_auth_state("work4you") or {}
    pconfig = PROVIDER_REGISTRY["work4you"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [WORK4YOU_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if WORK4YOU_BILLING_MANAGE_SCOPE not in requested:
        requested.append(WORK4YOU_BILLING_MANAGE_SCOPE)
    scope = " ".join(requested)

    auth_state = _work4you_device_code_login(
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "work4you", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_nous.
    try:
        _write_shared_work4you_state(auth_state)
    except Exception:
        pass
    try:
        _sync_work4you_pool_from_auth_store()
    ...
    return isinstance(granted, str) and WORK4YOU_BILLING_MANAGE_SCOPE in granted.split()


def _login_nous(args, pconfig: ProviderConfig) -> None:
    """Work4You Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Work4You credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_work4you_state()
        if shared:
            try:
                shared_path = _work4you_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Work4You OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Work4You OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Work4You session from shared credentials...")
                auth_state = _try_import_shared_work4you_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "work4you".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=work4you but model
        ...
        try:
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("work4you")
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_work4you_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_work4you_portal_entitlement_message(
                                _account_info,
                                capability="paid Work4You models",
                            )
                            or ""
                        )
                    except Exception:
            ...
            if model_ids:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="work4you",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/")
                print("No free models currently available.")
                print(unavailable_message or f"Upgrade at {_url} to access paid models.")
            else:
                print("No curated models available for Work4You Portal.")
        except Exception as exc:
        ...
        # them to Work4You with a mismatched model.  The Work4You OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "work4you".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Work4You credentials saved for future use.")
            print("  Run `work4you model` again to switch to Work4You Portal.")
            return

        config_path = _update_config_for_provider(
            "work4you", inference_base_url, default_model=selected_model,
        )
        if selected_model:
