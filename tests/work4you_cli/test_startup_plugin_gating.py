...
``work4you_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``work4you --help``, ``work4you version``,
``work4you logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``work4you --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from work4you_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["work4you", "--help"]
    buf = io.StringIO()
    try:
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``work4you photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain work4you-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level work4you <name> CLI command via PluginContext.
