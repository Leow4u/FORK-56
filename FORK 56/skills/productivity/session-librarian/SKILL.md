...
| List/filter by metadata (age, source, cost, tokens, workspace) | `work4you sessions list` / `stats` via terminal |
| Rename | `work4you sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `work4you sessions archive <filters>` |
| Delete (destructive) | `work4you sessions delete` / `work4you sessions prune <filters>` |
| Export before deleting anything valuable | `work4you sessions export --session-id <id> --format md` |
...
`work4you sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `work4you sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `work4you sessions list`
