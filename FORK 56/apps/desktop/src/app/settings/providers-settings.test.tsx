...
import type { EnvVarInfo, OAuthProvider } from '@/types/work4you'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `work4you auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'work4you' ? 'Work4You Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'work4you' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('work4you', true), provider('minimax-oauth', false)]
  })
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Work4You Portal' })
    await act(async () => {
      fireEvent.click(remove)
    })

    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('work4you'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Work4You Portal'))
    })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `work4you auth add qwen-oauth` or that provider's CLI to remove it.",
