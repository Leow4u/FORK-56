...
def work4you_home(tmp_path):
    """Create a minimal WORK4YOU_HOME with config."""
...
class TestGetExternalSkillsDirs:
    def test_empty_config(self, work4you_home):
        (work4you_home / "config.yaml").write_text("skills:\n  external_dirs: []\n")
    ...
    def test_valid_dir_returned(self, work4you_home, external_skills_dir):
...
class TestGetAllSkillsDirs:
    def test_local_always_first(self, work4you_home, external_skills_dir):
        (work4you_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        with patch.dict(os.environ, {"WORK4YOU_HOME": str(work4you_home)}):
        ...
        assert result[0] == work4you_home / "skills"
        assert result[1] == external_skills_dir.resolve()


class TestExternalSkillsInFindAll:
    def test_external_skills_found(self, work4you_home, external_skills_dir):
        (work4you_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = work4you_home / "skills"
        with (
            patch.dict(os.environ, {"WORK4YOU_HOME": str(work4you_home)}),
    ...
    def test_local_takes_precedence(self, work4you_home, external_skills_dir):
...
class TestExternalSkillView:
    def test_skill_view_finds_external(self, work4you_home, external_skills_dir):
        (work4you_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = work4you_home / "skills"
