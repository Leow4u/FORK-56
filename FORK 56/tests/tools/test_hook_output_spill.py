...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("work4you_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="work4you-spill-test-")
    ...
    def test_default_directory_uses_work4you_home(self):
        """When no directory override, spill under WORK4YOU_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="work4you-home-")
        try:
            with patch.dict(os.environ, {"WORK4YOU_HOME": test_home}):
                # Also patch get_work4you_home to the env var to mirror production.
            ...
            # ~/.work4you/hook_outputs depending on get_work4you_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.work4you/hook_outputs/sess")),
            ]
