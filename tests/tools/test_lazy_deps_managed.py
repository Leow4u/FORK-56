...
A package-manager install (NixOS, and anything else shipping Work4You from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.work4you/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("work4you_cli.config.get_managed_system", lambda: "nixos")
...
def test_unmanaged_install_is_not_blocked_by_the_guard(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.config.get_managed_system", lambda: None)
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets WORK4YOU_MANAGED *and* a writable target.

    Dockerfile sets WORK4YOU_LAZY_INSTALL_TARGET and the NixOS container module
    passes WORK4YOU_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("work4you_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.config.get_managed_system", _raise)
