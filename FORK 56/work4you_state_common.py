...
Extracted verbatim from work4you_state.py so the SessionDB mixin modules
(work4you_state_search / work4you_state_schema / work4you_state_portability) can
reference them without importing work4you_state (which would be a cycle).
work4you_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``work4you sessions optimize-storage``. A legacy DB sits at
...
# `work4you sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `work4you db optimize`. This is
