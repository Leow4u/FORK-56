...
Scoped strictly to `$WORK4YOU_HOME` and `/tmp/work4you-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `WORK4YOU_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `WORK4YOU_HOME` or `/tmp/work4you-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$WORK4YOU_HOME/disk-cleanup/` is itself excluded
- `$WORK4YOU_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
