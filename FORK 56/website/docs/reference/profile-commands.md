...
This page covers all commands related to [Work4You profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `work4you profile` without a subcommand shows help.
...
## `work4you profile use`

```bash
work4you profile use <name>
```

Sets `<name>` as the active profile. All subsequent `work4you` commands (without `-p`) will use this profile.
...
work4you profile use default
```

## `work4you profile create`

```bash
work4you profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `work4you profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `work4you update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.work4you`), use `work4you skills opt-out` / `work4you skills opt-in`. |
...
work4you profile create work2 --clone-from work

# Clone everything from a specific profile
work4you profile create work2-backup --clone-from work --clone-all
```

## `work4you profile describe`

```bash
work4you profile describe [<name>] [options]
```
...
work4you profile describe researcher

# Set it explicitly
work4you profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
work4you profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
work4you profile describe --all --auto
```

## `work4you profile delete`

```bash
work4you profile delete <name> [options]
```
...
```bash
work4you profile delete mybot
work4you profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.work4you`) cannot be deleted — use `work4you uninstall` to remove everything.
:::

## `work4you profile show`

```bash
work4you profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Work4You home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ work4you profile show work
Profile: work
Path:    ~/.work4you/profiles/work
...
## `work4you profile alias`

```bash
work4you profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Work4You installation.
...
```bash
work4you profile alias work
# Creates/updates ~/.local/bin/work

work4you profile alias work --name mywork
# Creates ~/.local/bin/mywork

work4you profile alias work --remove
# Removes the wrapper script
```

## `work4you profile rename`

```bash
work4you profile rename <old-name> <new-name>
```
...
```bash
work4you profile rename mybot assistant
# ~/.work4you/profiles/mybot → ~/.work4you/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `work4you profile export`

```bash
work4you profile export <name> [options]
```
...
```bash
work4you profile export work
# Creates work.tar.gz in the current directory

work4you profile export work -o ./work-2026-03-29.tar.gz
```
...
## `work4you profile import`

```bash
work4you profile import <archive> [options]
```
...
```bash
work4you profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

work4you profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `work4you -p telemetry`). |
...
work4you profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
work4you profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
work4you profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
work4you profile install ./telemetry/
```

### `work4you profile update`

```bash
work4you profile update <name> [--force-config] [--yes]
```
...
### `work4you profile info`

```bash
work4you profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Work4You version, author, env var requirements, the source URL/path, and
...
`work4you profile list` also shows the distribution name and version in a
`Distribution` column, and `work4you profile show <name>` / `delete <name>`
...
work4you profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
work4you profile install https://github.com/your-org/internal-assistant.git
```
...
`work4you_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Work4You version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Work4You can clone from.
3. Tell recipients to run `work4you profile install <your-repo-url>`.
...
## `work4you -p` / `work4you --profile`

```bash
work4you -p <name> <command> [options]
work4you --profile <name> <command> [options]
```

Global flag to run any Work4You command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
work4you -p work chat -q "Check the server status"
work4you --profile dev gateway start
work4you -p personal skills list
work4you -p work config edit
```

## `work4you completion`

```bash
work4you completion <shell>
```
...
- `work4you profile <TAB>` — subcommands (list, use, create, etc.)
- `work4you profile use <TAB>` — profile names
- `work4you -p <TAB>` — profile names
