...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate WORK4YOU_HOME for each test.

    The global hermetic fixture already redirects WORK4YOU_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    WORK4YOU_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "work4you_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "work4you_plugins" not in sys.modules:
        ns = types.ModuleType("work4you_plugins")
        ns.__path__ = []
        sys.modules["work4you_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "work4you_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["work4you_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_work4you_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_work4you_home(self, _isolate_env):
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, work4you_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from work4you_cli import plugins as pmod
