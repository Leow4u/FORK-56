...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'work4you-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Work4You Test')
    git('config', 'user.email', 'work4you@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/Leow4u/FORK-56.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/Leow4u/FORK-56/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/Leow4u/FORK-56.git',
