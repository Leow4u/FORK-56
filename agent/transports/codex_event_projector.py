"""Projects codex app-server events into Work4You' messages list.

The translator that lets Work4You' memory/skill review keep working under the
...
Work4You' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Work4You
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Work4You' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Work4You stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Work4You already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Work4You' messages store text only.
        text_parts: list[str] = []
