...
Storage
-------
Bundles live in ``~/.work4you/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``WORK4YOU_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``work4you bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under WORK4YOU_HOME.

    Honors ``WORK4YOU_BUNDLES_DIR`` for tests; falls back to
    ``<WORK4YOU_HOME>/skill-bundles``.
    """
    override = os.environ.get("WORK4YOU_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_work4you_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `work4you bundles` CLI subcommand.
# ---------------------------------------------------------------------------
