...
description: "How to build a model provider (inference backend) plugin for Work4You"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Work4You can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$WORK4YOU_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with Work4You
2. **User plugins** — `$WORK4YOU_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$WORK4YOU_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `work4you plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `work4you_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `work4you_cli/main.py` | Accepts `acme-inference` |
| `work4you model` picker | `work4you_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `work4you doctor` | `work4you_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `work4you setup` | `work4you_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `work4you_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `work4you model` picker |
...
| `plugins/model-providers/work4you/` | Attribution tags, "omit reasoning when disabled" |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.work4you/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Work4You picks one based on:
...
| `oauth_external` | User signs in elsewhere, tokens land in `auth.json` | Anthropic OAuth, MiniMax OAuth, Qwen Portal, Work4You Portal |
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Work4You' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `WORK4YOU_HOME` at a temp directory so you don't pollute your real config:

```bash
export WORK4YOU_HOME=/tmp/work4you-plugin-test
mkdir -p $WORK4YOU_HOME/plugins/model-providers/my-provider
cat > $WORK4YOU_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
work4you -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `work4you plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$WORK4YOU_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Model providers can ship as a pip package. Expose an entry point in the
`work4you.plugins` group in your `pyproject.toml`:

```toml
[project.entry-points."work4you.plugins"]
acme-inference = "acme_work4you_plugin:register"
```
...
- **Lowest precedence.** Entry-point plugins are discovered **before**
  ...
  bundled or `$WORK4YOU_HOME` profile of the same name always overrides a
...
See [Building a Work4You Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a Work4You Plugin](/developer-guide/plugins) — general plugin authoring
