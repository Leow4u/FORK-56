...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Work4You tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Work4You Docker layout, `/opt/data` is `WORK4YOU_HOME`, but Work4You tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Work4You-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Work4You tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Work4You subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Work4You adaptation: reformatted for Work4You skill conventions; safety guardrails preserved verbatim.
