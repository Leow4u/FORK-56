...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.work4you/credentials/a.json"),
            (str(src_b), "/root/.work4you/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.work4you/credentials/a.json" in names
            assert "root/.work4you/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.work4you/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.work4you/skills/b.py").read()
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.work4you/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.work4you/large.bin" in names
