...
/**
 * Explicit-confirm gate for `work4you://mcp/install` deep links. The payload is
 ...
 * confirming lets Work4You spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getWork4YouConfigRecord())
      ...
      setWork4YouConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
