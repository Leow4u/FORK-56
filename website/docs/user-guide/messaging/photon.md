...
Connect Work4You to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Work4You — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Work4You runs it in a
...
work4you photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Work4You` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.work4you/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.work4you/.env`
...
(device token, dashboard project id) lives in `~/.work4you/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Work4You
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Work4You replies with a pairing code. Approve it with:

```bash
work4you pairing approve photon <CODE>
```

Use `work4you pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.work4you/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.work4you/.env`):
...
By default Work4You responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Work4You` and
`@Work4You agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Work4You will reply.

## Status & troubleshooting

```bash
work4you photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Work4You uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run work4you photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `work4you photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `work4you photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `work4you photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Work4You sends images, voice
  ...
  media.
- **Native polls are supported.** Work4You sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Work4You sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `work4you send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<work4you-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Work4You is set
...
| `PHOTON_MENTION_PATTERNS` | Work4You wake words  | JSON list / comma / newline regex patterns for group mentions |
