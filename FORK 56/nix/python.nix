...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Work4You derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            work4you = prev.work4you.overrideAttrs (_old: {
              WORK4YOU_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $WORK4YOU_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$WORK4YOU_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        work4you = prev.work4you.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "work4you-env" {
    work4you = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "work4you-editable-env" {
    work4you = dependency-groups;
  };
}
