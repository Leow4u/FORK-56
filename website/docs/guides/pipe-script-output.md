...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `work4you send`."
---

# Pipe Script Output to Messaging Platforms

`work4you send` is a small, scriptable CLI that pushes a message to any
messaging platform Work4You is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | work4you send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `work4you
...
work4you send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
work4you send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
work4you send --list

# Filter by platform
work4you send --list telegram
```
...
Any platform Work4You ships adapters for works as a target:
...
`work4you send` resolves the message body in this order:

1. **Positional argument** — `work4you send --to telegram "hi"`
2. **`--file PATH`** — `work4you send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | work4you send --to telegram`

When stdin is a TTY (no pipe), Work4You does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  work4you send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `work4you send` reuses your Work4You config, the same script works on
any host where Work4You is installed — no need to export bot tokens into
...
`work4you send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  work4you send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | work4you send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.work4you/bin/work4you send \
...
```bash
./train.py --epochs 200 && \
  work4you send --to telegram "training done" || \
  work4you send --to telegram "training failed (exit $?)"
```
...
work4you send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(work4you send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `work4you send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `work4you send` calls
the platform's REST endpoint directly using credentials from
`~/.work4you/.env` and `~/.work4you/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `work4you gateway start` and retry.
...
# Machine-readable
work4you send --list --json
```

The listing is built from `~/.work4you/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`work4you gateway
...
| Approach | Multi-platform | Reuses Work4You creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `work4you send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`work4you send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `work4you send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `work4you send` shares with cron delivery.
