...
"No LLM provider configured. Run `work4you model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real WORK4YOU home.
...
# provider (e.g. Work4You) but who switched THIS session to a self-hosted model
...
# found" (the b200/work4you-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-work4you-model", "provider": "work4you"},
    "providers": {
        "work4you-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["work4you-ultra-sft"],
        }
    },
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("work4you-ultra-sft")
            == "custom:work4you-ultra"
        )
