...
from work4you_constants import (
    ...
    find_work4you_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_work4you_root,
    get_work4you_dir,
    get_work4you_home,
    get_process_work4you_home,
    heal_work4you_managed_node,
    ...
    with_work4you_node_path,
)


class TestGetDefaultWork4YouRoot:
    """Tests for get_default_work4you_root() — Docker/custom deployment awareness."""

    def test_no_work4you_home_returns_native(self, tmp_path, monkeypatch):
        """When WORK4YOU_HOME is not set, returns ~/.work4you."""
        monkeypatch.delenv("WORK4YOU_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_work4you_root() == tmp_path / ".work4you"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (WORK4YOU_HOME=<root>/profiles/<name>),
        ...
        assert get_default_work4you_root() == docker_root

    @pytest.mark.windows_only
    def test_no_work4you_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\work4you, not ~/.work4you."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("WORK4YOU_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_work4you_root() == local_appdata / "work4you"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; WORK4YOU_HOME / home changes invalidate.

        get_default_work4you_root() resolves WORK4YOU_HOME against the native
        ...
        (native home, WORK4YOU_HOME) compared for free each call.
        """
        # WORK4YOU_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_work4you_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            work4you_constants, "_default_work4you_root_memo", None, raising=False
        )

        first = get_default_work4you_root()
        ...
        # WORK4YOU_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("WORK4YOU_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_work4you_root() == docker_root
        assert resolve_calls["n"] > before, (
            "WORK4YOU_HOME change must force a fresh resolution"
        )
...
class TestGetWork4YouHome:
    """Tests for get_work4you_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When WORK4YOU_HOME is unset on Windows, use %LOCALAPPDATA%\\work4you."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("WORK4YOU_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_work4you_home() == local_appdata / "work4you"


class TestGetProcessWork4YouHome:
    """Tests for get_process_work4you_home() — process launch scope.
    ...
    via set_work4you_home_override().
    """
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_work4you_home() == home




class TestWork4YouManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_work4you_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        assert find_work4you_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(work4you_constants, "heal_work4you_managed_node", lambda: False)
        monkeypatch.setattr(
            work4you_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert work4you_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Work4You-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(work4you_constants, "heal_work4you_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = work4you_constants._WORK4YOU_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = work4you_constants.find_work4you_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert work4you_constants.find_work4you_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert work4you_constants.find_work4you_node_executable("node") == str(node)
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from work4you_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".work4you"
...
class TestGetWork4YouDir:
    """Tests for ``get_work4you_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_work4you_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_work4you_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert work4you_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert work4you_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert work4you_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert work4you_constants.windows_path_to_wsl("/home/alex") is None
        assert work4you_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(work4you_constants, "is_wsl", lambda: True)
        assert work4you_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert work4you_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert work4you_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(work4you_constants.sys, "platform", "darwin")
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
        assert work4you_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert work4you_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(home))
        monkeypatch.setenv(
            "WORK4YOU_NODE_TARGET_MAJOR",
            str(work4you_constants._WORK4YOU_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(work4you_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            work4you_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            work4you_constants,
        ...
        monkeypatch.setattr(
            work4you_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(work4you_constants._WORK4YOU_NODE_TARGET_MAJOR)
        ...
        result = work4you_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(work4you_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_work4you_managed_node() is False
        assert work4you_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_work4you_managed_node() is False
        assert work4you_constants._managed_node_heal_attempted is True
