...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    ...
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Work4You'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Work4You.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Work4You'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default WORK4YOU_HOME for non-sandboxed runs -- matches main.ts's
// resolveWork4YouHome(). On Windows it's %LOCALAPPDATA%\work4you; elsewhere
// it's ~/.work4you. The fresh-install sandbox launchFresh() sets its own
// WORK4YOU_HOME and never touches this.
const DEFAULT_WORK4YOU_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'work4you')
  }
  return path.join(os.homedir(), '.work4you')
})()
const VENV_ROOT = path.join(DEFAULT_WORK4YOU_HOME, 'work4you', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'work4you-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.WORK4YOU_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Work4You-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Work4You-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Work4You-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Work4You-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.WORK4YOU_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.WORK4YOU_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const work4youHome = path.join(sandbox, 'work4you-home')
  ...
  env.WORK4YOU_DESKTOP_CWD = cwd
  env.WORK4YOU_DESKTOP_IGNORE_EXISTING = '1'
  env.WORK4YOU_DESKTOP_TEST_MODE = 'fresh-install'
  env.WORK4YOU_DESKTOP_USER_DATA_DIR = userDataDir
  env.WORK4YOU_HOME = work4youHome
  delete env.WORK4YOU_DESKTOP_WORK4YOU
  delete env.WORK4YOU_DESKTOP_WORK4YOU_ROOT
  ...
  console.log(`  WORK4YOU_HOME: ${work4youHome}`)
  console.log(`  cwd: ${cwd}`)
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of work4you_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'work4you', 'work4you_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Work4You
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + WORK4YOU_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  WORK4YOU_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
