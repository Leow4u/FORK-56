...
Every aux picker (``work4you model`` → Configure auxiliary models, the
``work4you tools`` vision picker, and any future one) must route through
``work4you_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A WORK4YOU_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from work4you_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("work4you_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    """
    import work4you_cli.main as main
    ...
    with (
        patch("work4you_cli.model_switch.list_authenticated_providers", _direct),
        patch("work4you_cli.inventory.build_aux_picker_rows", _substrate),
        patch("work4you_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through work4you_cli.inventory.build_aux_picker_rows so custom "
