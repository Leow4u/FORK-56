...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`work4you pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                def _collect():
                    ...
                    # WORK4YOU_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `work4you kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours WORK4YOU_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("WORK4YOU_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via WORK4YOU_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Work4You profile
            ...
            PATH, missing venv, credential loss for a real Work4You profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from work4you_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("WORK4YOU_KANBAN_BOARD")
                try:
                    os.environ["WORK4YOU_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("WORK4YOU_KANBAN_BOARD", None)
                    else:
                        os.environ["WORK4YOU_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`work4you pause`): skip auto-decompose
