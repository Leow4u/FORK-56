...
import { type ResolvedTranslations, useI18n } from "@/i18n";
...
type AgentsCopy = ResolvedTranslations["agents"];
...
function streamGlyph(entry: SubagentStreamEntry): ReactNode {
  ...
  if (entry.kind === "tool") {
    return (
      <span
        aria-hidden
        className="mt-0.5 size-1.5 shrink-0 rounded-full bg-foreground/55"
      />
    );
  }
  ...
  if (entry.kind === "summary") {
    return (
      <CheckCircle2
        aria-hidden
        className="mt-0.5 size-3 shrink-0 text-emerald-600/85 dark:text-emerald-400/85"
      />
    );
  }
  ...
  if (entry.kind === "thinking") {
    return (
      <span
        aria-hidden
        className="font-mono text-[0.7rem] leading-none text-muted-foreground/70"
      >
        …
      </span>
    );
  ...
  return (
    <span
      aria-hidden
      className="mt-0.5 size-1 shrink-0 rounded-full bg-muted-foreground/55"
    />
  );
}

const fmtDuration = (seconds: number | undefined, a: AgentsCopy) => {
...
const fmtTokens = (value: number | undefined, a: AgentsCopy) =>
  value ? a.tokens(compactNumber(value)) : "";

const fmtAge = (updatedAt: number, nowMs: number, a: AgentsCopy) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {running ? (
          <ActivityTimerText
            className="mt-1 shrink-0 text-[0.6rem]"
...
export default function AgentsPage() {
  ...
  // Aggregate every session, matching the chat-header count — a subagent
  // running in a background session must still be visible here.
  const tree = useMemo(
    () => buildSubagentTree(allSubagents(subagentsBySession)),
    [subagentsBySession],
  );
  ...
  useEffect(() => {
    ...
    setAfterTitle(
      <p className="text-xs text-muted-foreground/75">{t.agents.subtitle}</p>,
    );
    return () => {
      setTitle(null);
      setAfterTitle(null);
    };
  }, [setAfterTitle, setTitle, t.agents.subtitle, t.agents.title]);

  if (tree.length === 0) {
    return (
      <div
        className="grid min-h-[16rem] place-items-center gap-3 py-12 text-center"
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col py-4" data-testid="agents-tree">
