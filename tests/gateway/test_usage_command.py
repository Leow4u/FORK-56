...
class TestUsageAccountSection:
    ...
    async def test_usage_command_uses_persisted_provider_when_agent_not_running(self, monkeypatch):
        ...
        async def _fake_to_thread(fn, *args, **kwargs):
            ...
            # with the provider positionally) and the Work4You credits fetch
            # (work4you_credits_lines, markdown-only) through to_thread — record every
        ...
        # The credits block routes through the shared work4you_credits_lines() helper;
        # stub it so this account-section test stays hermetic (no portal/auth lookup).
        monkeypatch.setattr("agent.account_usage.work4you_credits_lines", lambda markdown=False: [])
    ...
    async def test_usage_command_prefers_dominant_persisted_route(self, monkeypatch):
        ...
        runner._session_db = AsyncSessionDB(MagicMock())
        runner._session_db._db.get_session.return_value = {
            "billing_provider": "work4you",
            "billing_base_url": "https://inference-api.work4you.ai/v1/",
        }
