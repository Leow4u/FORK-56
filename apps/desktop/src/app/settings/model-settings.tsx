...
// backend now surfaces the full `work4you model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in work4you_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile = null }: ModelSettingsProps) {
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateWork4YouConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `work4you model` curation). Best-effort — fall through to the refreshed
