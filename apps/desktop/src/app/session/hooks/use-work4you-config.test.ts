...
import { useWork4YouConfig } from './use-work4you-config'

vi.mock('@/work4you', () => ({
  getWork4YouConfig: vi.fn(),
  getWork4YouConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'work4you.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getWork4YouConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getWork4YouConfig>>)

describe('useWork4YouConfig refreshWork4YouConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Work4You' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useWork4YouConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshWork4YouConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useWork4YouConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getWork4YouConfig>>>()
    vi.mocked(getWork4YouConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useWork4YouConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshWork4YouConfig(true)
    })
    expect(getWork4YouConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getWork4YouConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getWork4YouConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getWork4YouConfig>>>()
    vi.mocked(getWork4YouConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshWork4YouConfig(true)
      refreshC = result.current.refreshWork4YouConfig(true)
    })
