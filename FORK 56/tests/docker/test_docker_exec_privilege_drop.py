...
The shim (docker/work4you-exec-shim.sh, installed at /opt/work4you/bin/work4you)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> work4you login` would write /opt/data/auth.json as
...
failed: Work4You is not logged into Work4You Portal" on every message.

These tests verify:

1. ``docker exec <c> work4you …`` (defaulting to root) gets dropped to the
   work4you user before the real binary runs.
2. ``docker exec --user work4you <c> work4you …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $WORK4YOU_HOME from a ``docker exec`` session land
   as work4you:work4you — the actual user-visible invariant.
4. The WORK4YOU_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-work4you-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown work4you:work4you`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to work4you:work4you,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$WORK4YOU_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (work4you_cli.container_boot), which s6 runs
    *strictly after* ``01-work4you-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_work4you_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `work4you` with a Python-style `-c` shim equivalent — there's no
    pure-work4you "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `work4you` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `work4you`
    do anything that writes to disk, then check the file's owner.

    Use `work4you config set` which writes config.yaml under WORK4YOU_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "work4you", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by work4you, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "work4you:work4you", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected work4you:work4you. "
        "The shim did not drop privileges before invoking work4you."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/work4you/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid work4you work4you <args>` which resolves `work4you`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> work4you login`` (root) wrote
    ...
    as a parse failure, and resolve_work4you_runtime_credentials raised
    "Work4You is not logged into Work4You Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `work4you config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is work4you-readable.

    Specifically: pretend the operator ran `work4you login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the work4you UID. We use `work4you auth list` since that touches the
    ...
    # `work4you auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `work4you config set` first to
    # provoke a write into WORK4YOU_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "work4you", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # WORK4YOU_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "work4you", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the work4you user "
