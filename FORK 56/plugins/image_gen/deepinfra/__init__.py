...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=work4you``
where each entry's ``metadata.tags`` declares its surface (``image-gen``
here). ``list_models()`` filters that catalog via
:func:`work4you_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``work4you tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from work4you the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `work4you tools` → Image "
                    "Generation → DeepInfra to configure, or `work4you setup` "
                    "to add the key."
                ),
        ...
        from work4you_cli.models import deepinfra_base_url
