...
is typically only in `~/.work4you/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Work4You,
...
`work4you-custom-providers` skill, pitfall 2). Work4You hits this because it forwards
