...
This rewrites files Work4You does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
from work4you_cli import uninstall
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``WORK4YOU_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("WORK4YOU_HOME", str(home / ".work4you"))
    return home


class TestHappyPath:
    def test_work4you_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/WORK4YOU_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Work4You" not in real.read_text(encoding="utf-8")
