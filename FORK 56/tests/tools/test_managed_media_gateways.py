...
@pytest.fixture(autouse=True)
def _enable_managed_work4you_tools(monkeypatch):
    """Patch the source modules so managed_work4you_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr(
        "work4you_cli.work4you_account.get_work4you_portal_account_info",
        lambda: Work4YouPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
def test_managed_fal_submit_uses_gateway_origin_and_work4you_token(monkeypatch):
    captured = {}
    ...
    assert captured["client_key"] == "work4you-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "work4you.ai")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "work4you-token")
    ...
    assert captured["api_key"] == "work4you-token"
    assert captured["base_url"] == "https://openai-audio-gateway.work4you.ai/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    # The managed audio route is the stored "work4you" selection (strict model);
    ...
    transcription_tools._load_stt_config = lambda: {"provider": "work4you"}
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.work4you.ai/v1"
