"""work4you hooks — inspect and manage shell-script hooks.

Usage::

    work4you hooks list
    work4you hooks test <event> [--for-tool X] [--payload-file F]
    work4you hooks revoke <command>
    work4you hooks doctor

Consent records live under ``~/.work4you/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.work4you/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``work4you hooks`` — dispatches to the requested action."""
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.work4you/config.yaml.")
    ...
    if not specs:
        print("No shell hooks configured in ~/.work4you/config.yaml.")
    else:
...
# stdin a script sees under `work4you hooks test` and `work4you hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `work4you hooks doctor` would execute
