#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Work4You.
...
# Ensure sibling modules (_work4you_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _work4you_home import display_work4you_home, get_work4you_home

WORK4YOU_HOME = get_work4you_home()
TOKEN_PATH = WORK4YOU_HOME / "google_token.json"
CLIENT_SECRET_PATH = WORK4YOU_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = WORK4YOU_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Work4You profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Work4You Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Work4You Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  work4you setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Work4You home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_work4you_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Work4You")
