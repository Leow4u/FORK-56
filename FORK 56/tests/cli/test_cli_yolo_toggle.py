...
Pre-fix bug (issue #33925): ``cli.Work4YouCLI._toggle_yolo`` mutated only
``os.environ["WORK4YOU_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``work4you --yolo`` (process-start env), ``WORK4YOU_YOLO_MODE=1``,
and ``work4you config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``Work4YouCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``Work4YouCLI.__init__`` imports a lot of optional
...
def _clear_approval_state(monkeypatch):
    ...
    # Work4You-driven test runs may inherit WORK4YOU_YOLO_MODE=1 from the parent
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``Work4YouCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``Work4YouCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_enables_session_bypass(self):
        ...
        with patch("cli._cprint"):
            Work4YouCLI._toggle_yolo(stand_in)
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            Work4YouCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            Work4YouCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
    ...
    def test_two_independent_sessions_are_isolated(self):
        ...
        try:
            with patch("cli._cprint"):
                Work4YouCLI._toggle_yolo(cli_a)
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert Work4YouCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            Work4YouCLI._toggle_yolo(stand_in)

        assert Work4YouCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``work4you --yolo`` sets ``WORK4YOU_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                Work4YouCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            Work4YouCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        Work4YouCLI._transfer_session_yolo(stand_in, "", "new")
        Work4YouCLI._transfer_session_yolo(stand_in, "old", "")
