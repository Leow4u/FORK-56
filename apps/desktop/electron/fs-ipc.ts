...
// hardening, WORK4YOU_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('work4you:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('work4you:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('work4you:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('work4you:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<WORK4YOU_HOME>/desktop-plugins`,
  // resolved from the main-process WORK4YOU_HOME (see resolveWork4YouHome) — NOT from
  // the connected backend. A remote backend reports its own `work4you_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped work4you_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(work4youHome, 'profiles', profile) : work4youHome
  ...
  ipcMain.handle('work4you:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL agent-plugin root (`<WORK4YOU_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('work4you:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('work4you:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('work4you:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('work4you:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('work4you:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('work4you:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
