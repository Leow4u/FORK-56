...
Automates what a user otherwise does by hand: open the Work4You Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.work4you/.env``
as ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Work4You Portal access token from the existing login
     (``~/.work4you/auth.json``), refreshing it if needed. Fails fast with a
  ...
  3. Writes ``WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``WORK4YOU_DASHBOARD_PORTAL_URL`` into ``~/.work4you/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``WORK4YOU_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the Work4You login — this is the
    ...
    try:
        from work4you_cli.auth import DEFAULT_WORK4YOU_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Work4You Portal rejected the access token (401). "
                "Try `work4you auth add work4you` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach Work4You Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from work4you_cli.config import get_env_path
    ...
    print()
    print(
        "  Heads up — Work4You login only *engages* on a non-loopback bind. A plain\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Work4You login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Work4You login (e.g. exposing on your LAN or a public host):")
...
def cmd_dashboard_register(args) -> None:
    ...
    from work4you_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID
    ...
    # 1. Resolve a fresh Work4You access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_work4you_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Work4You Portal.")
            print("  Run `work4you setup` (or `work4you auth add work4you`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Work4You Portal access token: {exc}")
        sys.exit(1)
    ...
    # WORK4YOU_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    #      WORK4YOU_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.work4you.ai"
    existing_portal = None
    try:
        existing_portal = get_env_value("WORK4YOU_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    # the portal, e.g. https://work4you.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking WORK4YOU_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as WORK4YOU_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("WORK4YOU_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("WORK4YOU_DASHBOARD_PUBLIC_URL", public_url)
