"""
Profile management for multiple isolated Work4You instances.

Each profile is a fully independent WORK4YOU_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.work4you/profiles/<name>/`` by default.

The "default" profile is ``~/.work4you`` itself — backward compatible,
zero migration needed.

Usage::

    work4you profile create coder          # fresh profile + bundled skills
    work4you profile create coder --clone  # also copy config, .env, SOUL.md, skills
    work4you profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    work4you -p coder chat                 # or via flag
    work4you profile use coder             # set as sticky default
    work4you profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See work4you_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `work4you backup` archives
...
# Marker file written by `work4you profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `work4you
...
# `work4you skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Work4You infrastructure directories that only the default profile
         (``~/.work4you``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_work4you_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.work4you) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "work4you",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when WORK4YOU_HOME equals the
...
# into the export. The set below identifies the *known Work4You profile
# artifacts* at the root of WORK4YOU_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``work4you_constants``.
...
_RESERVED_NAMES = frozenset({
    "work4you", "default", "test", "tmp", "root", "sudo",
})

# Work4You subcommands that cannot be used as profile names/aliases
_WORK4YOU_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the work4you root, NOT to the current WORK4YOU_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where WORK4YOU_HOME points outside
    ``~/.work4you``, profiles live under ``WORK4YOU_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_work4you_home() / "profiles"


def _get_default_work4you_home() -> Path:
    """Return the default (pre-profile) WORK4YOU_HOME path.

    In standard deployments this is ``~/.work4you``.
    In Docker/custom deployments where WORK4YOU_HOME is outside ``~/.work4you``
    (e.g. ``/opt/data``), returns WORK4YOU_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_work4you_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its WORK4YOU_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_work4you_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, work4you subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _WORK4YOU_SUBCOMMANDS:
        return f"'{canon}' conflicts with a work4you subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            work4you_exe = shutil.which("work4you") or "work4you"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(work4you_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Work4You. If we
    ...
    try:
        ...
        from work4you_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `work4you doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``work4you -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``work4you profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``work4you -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``work4you profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``WORK4YOU_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Work4You config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_work4you_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, work4you_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own WORK4YOU_HOME. When
    ...
    The returned ``work4you_home`` is the path to pass to
    ``set_work4you_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_work4you_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``work4you update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.work4you)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.work4you/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `work4you -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `work4you update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`work4you profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Work4You (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `work4you profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `work4you -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``work4you profile create
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_work4you_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Work4You *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Work4You backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``WORK4YOU_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `work4you -p <canon> profile
    ...
    work4you_markers = ("work4you_cli.main", "work4you-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('work4you')` resolution) spawns the
    ...
    # interpreter, not "work4you", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _WORK4YOU_CONSOLE_SCRIPT_NAMES = frozenset({"work4you", "work4you", "work4you-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Work4You process: either an entrypoint marker in argv, a
            # resolved executable named `work4you`, or a python interpreter
            # directly exec'ing a `work4you`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_work4you = (
                any(marker in joined for marker in work4you_markers)
                or exe_name == "work4you"
                or exe_name.startswith("work4you")
            )
            if not is_work4you and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: work4you, work4you,
                # work4you-acp) rather than a bare `startswith("work4you")` --
                ...
                # "work4you-notes.py" or "work4you-unrelated-tool" as the shim,
                ...
                is_work4you = script_stem in _WORK4YOU_CONSOLE_SCRIPT_NAMES
            if not is_work4you:
                continue
            ...
            # ...or by WORK4YOU_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("WORK4YOU_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.work4you).\n"
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_work4you_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``WORK4YOU_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from work4you_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set WORK4YOU_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("WORK4YOU_HOME")
    try:
        os.environ["WORK4YOU_HOME"] = str(profile_dir)
        from work4you_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["WORK4YOU_HOME"] = old_home
        elif "WORK4YOU_HOME" in os.environ:
            del os.environ["WORK4YOU_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.work4you/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from WORK4YOU_HOME.

    Returns ``"default"`` if WORK4YOU_HOME is not set or points to ``~/.work4you``.
    Returns the profile name if WORK4YOU_HOME points into ``~/.work4you/profiles/<name>``.
    Returns ``"custom"`` if WORK4YOU_HOME is set to an unrecognized path.
    """
    from work4you_constants import get_work4you_home
    work4you_home = get_work4you_home()
    resolved = work4you_home.resolve()

    default_resolved = _get_default_work4you_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      WORK4YOU_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Work4You file the
      user may have lying alongside WORK4YOU_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``work4you sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``WORK4YOU_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.work4you itself — its parent is ~/ and its
        # directory name is ".work4you", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.work4you itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.work4you). "
    ...
    with tempfile.TemporaryDirectory(prefix="work4you_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"work4you_{old_name}"
    legacy_old_host = f"work4you.{old_name}"
    new_host = f"work4you_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_work4you_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("work4you_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a WORK4YOU_HOME path string.

    Called early in the CLI entry point, before any work4you modules
    are imported, to set the WORK4YOU_HOME environment variable.

    When WORK4YOU_HOME is already set, the configured spelling IS the
