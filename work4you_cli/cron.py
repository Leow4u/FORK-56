"""
Cron subcommand for work4you CLI.
...
# commands at execution time when ``_WORK4YOU_GATEWAY=1``.
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `work4you cron status` and the gateway log for details.")
        return 1
    return 0
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec work4you "
                        "work4you cron ...`). Fix ownership to match the "
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (WORK4YOU_SESSION_KEY);
...
def cron_notepad(args) -> int:
    """Handle ``work4you cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: work4you cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: work4you cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: work4you cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: work4you cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
