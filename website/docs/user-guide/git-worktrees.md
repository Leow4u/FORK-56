...
title: "Git Worktrees"
description: "Run multiple Work4You agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Work4You is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Work4You so each session has a clean, isolated working directory.

## Why Use Worktrees with Work4You?

Work4You treats the **current working directory** as the project root:

- CLI: the directory where you run `work4you` or `work4you chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.work4you/config.yaml`
...
Work4You creates `.worktrees/my-experiment/` inside the repo (branch
`work4you/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `work4you-<id>`
...
git worktree add ../repo-feature feature/work4you-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/work4you-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Work4You there:

```bash
cd ../repo-feature

# Start Work4You in the worktree
work4you
```
...
git worktree add ../repo-experiment-a feature/work4you-a
git worktree add ../repo-experiment-b feature/work4you-b
```
...
Each Work4You process:

- Works on its own branch (`feature/work4you-a` vs `feature/work4you-b`).
...
- Work4You checkpoint data under `~/.work4you/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Work4You experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/work4you-checkpoints-docs`, `feature/work4you-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Work4You from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `work4you -w` (Automatic Worktree Mode)

Work4You has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `work4you/work4you-<hash>`).
...
```bash
work4you -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `work4you -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Work4You session its own clean checkout.
