"""work4you memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``work4you memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_work4you_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as work4you tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``work4you update`` heals the active memory provider
    ...
    # data volume (WORK4YOU_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(work4you_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.work4you/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like work4you-cli).
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.work4you/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.work4you/plugins/")
