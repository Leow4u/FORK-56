  ...
  if (target.tierOrder > current.tierOrder) {
    ...
    if (org.stripeSubscriptionId) {
      try {
        ...
        // Fork/CLI preview is USD cents. After BRL Prices, Stripe amount_due
        // is presentment cents — do not pass it through as dollars.
        if (
          getStripePresentmentCurrency() === 'usd' &&
          typeof upcoming.amount_due === 'number'
        ) {
          amountDueNowCents = upcoming.amount_due
        } else {
