...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "work4you_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # WORK4YOU_HOME.  For a standard install (WORK4YOU_HOME unset) that root is
        # the platform-native default (~/.work4you), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from work4you_constants import get_default_work4you_root
        assert env.get("WORK4YOU_HOME") == str(get_default_work4you_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (WORK4YOU_DESKTOP=1) must NOT
