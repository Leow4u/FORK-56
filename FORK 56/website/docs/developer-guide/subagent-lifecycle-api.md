...
Plugins can launch and supervise fresh Work4You child sessions without importing
...
active agent turn fails closed with `No active Work4You parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Work4You can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Work4You's existing unsafe-tool block remains enforced.
