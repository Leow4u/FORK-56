...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where work4you previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Work4You.
...
_work4you_config_resolved: str | None = None
_work4you_config_resolved_loaded = False


def _get_work4you_config_resolved() -> str | None:
    """Return the resolved absolute path of the Work4You config file (cached)."""
    global _work4you_config_resolved, _work4you_config_resolved_loaded
    if _work4you_config_resolved_loaded:
        return _work4you_config_resolved
    _work4you_config_resolved_loaded = True
    try:
        from work4you_cli.config import get_config_path
        _work4you_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _work4you_config_resolved = str(Path(_expand_tilde("~/.work4you/config.yaml")).resolve())
        except Exception:
            _work4you_config_resolved = None
    return _work4you_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Work4You config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    work4you_config = _get_work4you_config_resolved()
    if work4you_config and (resolved == work4you_config or normalized == work4you_config):
        return (
            f"Refusing to write to Work4You config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.work4you/config.yaml directly or use 'work4you config' instead."
        )
    return None
...
# .cursorrules (or a project-local .work4you config tree) outlives the current
...
_real_work4you_home_cached: str | None = None
_real_work4you_home_loaded = False


def _get_real_work4you_home() -> str | None:
    """Return the realpath of the authoritative Work4You home (cached)."""
    global _real_work4you_home_cached, _real_work4you_home_loaded
    if _real_work4you_home_loaded:
        return _real_work4you_home_cached
    _real_work4you_home_loaded = True
    try:
        from work4you_constants import get_work4you_home
        _real_work4you_home_cached = os.path.realpath(str(get_work4you_home()))
    except Exception:
        try:
            _real_work4you_home_cached = os.path.realpath(_expand_tilde("~/.work4you"))
        except Exception:
            _real_work4you_home_cached = None
    return _real_work4you_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from work4you_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.work4you home is governed by its own guards
    ...
    # ``.work4you`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_work4you_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .work4you config dirs (e.g. <repo>/.work4you/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.work4you`` — matching
        # any ancestor named .work4you would gate every write inside a
        # checkout that happens to live under ~/.work4you (e.g. the
        # work4you repo itself at ~/.work4you/work4you).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".work4you":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Work4You mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Work4You
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Work4You state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.work4you/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Work4You process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.work4you/…`` path.

    Returns ``None`` when the write is in-scope or outside Work4You scope.
    ...
    # in a session that cd'd into ``~/.work4you/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Work4You internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under WORK4YOU_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == WORK4YOU_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Work4You-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Work4You-profile guard for
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with work4you_tools.write_file() for very "
            "large files."
        )
