...
Authentication via ``FAL_KEY`` or the managed Work4You gateway. Output is an
...
def _resolve_managed_fal_video_gateway():
    ...
    image FAL resolver: ``"work4you"`` (or legacy ``use_gateway: true``) →
...
def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `work4you tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "or sign in to Work4You (`work4you setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
