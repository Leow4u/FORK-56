...
export function credentialKeyElementId(key: string) {
...
const PROVIDER_GROUPS: { prefix: string; name: string; priority: number }[] = [
  // Work4You Portal first
  { prefix: "WORK4YOU_", name: "Work4You Portal", priority: 0 },
...
function EnvVarRow({
  ...
  if (compact && !info.is_set && !isEditing) {
    return (
      <div
        id={credentialKeyElementId(varKey)}
...
// Mirror of the backend env-name guard (work4you_cli/config.py _ENV_VAR_NAME_RE).
...
/* ------------------------------------------------------------------ */
/*  Panel views (desktop Settings parity)                              */
/* ------------------------------------------------------------------ */

export type EnvCredentialsView =
  | "operator"
  | "providers-accounts"
  | "providers-api-keys"
  | "keys-tools"
...
function isUserFacingEnvKey(key: string, view: EnvCredentialsView): boolean {
  ...
  if (isCapabilitiesVendorCredentialHidden(key)) return false;
  return !key.startsWith("WORK4YOU_");
}

export interface EnvCredentialsPanelProps {
  view: EnvCredentialsView;
  /** Deep-link scroll target (?key= on Settings → Tools & Keys). */
...
export function EnvCredentialsPanel({
  view,
  highlightKey = null,
}: EnvCredentialsPanelProps) {
  ...
  // Scroll-to sub-nav in the page header (operator /env only).
  ...
  }, [vars, sections, setAfterTitle, view]);

  useEffect(() => {
    if (!highlightKey || !vars) return;
    const id = credentialKeyElementId(highlightKey);
  ...
  const showOperatorHeader = view === "operator";
  const showOAuth = view === "operator" || view === "providers-accounts";
  const showProviderKeys =
    view === "operator" || view === "providers-api-keys";
  const showToolKeys = view === "keys-tools";
  const showSettingKeys = view === "keys-settings";
  const showOperatorCategories = view === "operator";
  const showCustom = view === "custom-keys" || view === "operator";

  const toolSection = nonProviderGrouped.find((s) => s.category === "tool");
  const settingsSections = nonProviderGrouped.filter((s) =>
    s.category === "setting" || s.category === "messaging",
  );

  return (
    <div className="flex flex-col gap-6">
      {(view === "operator" || view.startsWith("providers-") || view.startsWith("keys-") || view === "custom-keys") && (
      ...
      {showToolKeys &&
        toolSection &&
        toolSection.totalEntries > 0 && (
          <EnvCategoryCard key={toolSection.category} section={toolSection} {...rowProps} />
        )}
