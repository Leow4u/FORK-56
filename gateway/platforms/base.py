...
# Audio file extensions Work4You recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Work4You but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Work4You' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Work4You-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_work4you_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``WORK4YOU_SESSION_PLATFORM`` contextvar — that contextvar is
...
from work4you_constants import get_default_work4you_root, get_work4you_dir, get_work4you_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.work4you/.env manually."
)
...
IMAGE_CACHE_DIR = get_work4you_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_work4you_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_work4you_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from work4you_cli.config import load_config_readonly as _load_config
...
AUDIO_CACHE_DIR = get_work4you_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_work4you_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_work4you_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_work4you_dir("cache/screenshots", "browser_screenshots")
...
_WORK4YOU_ROOT = get_default_work4you_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "WORK4YOU_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "WORK4YOU_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "WORK4YOU_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "WORK4YOU_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _WORK4YOU_HOME / "image_cache",
    _WORK4YOU_HOME / "audio_cache",
    _WORK4YOU_HOME / "video_cache",
    _WORK4YOU_HOME / "document_cache",
    _WORK4YOU_HOME / "browser_screenshots",
    ...
    _WORK4YOU_HOME / "cache" / "images",
    _WORK4YOU_HOME / "cache" / "audio",
    _WORK4YOU_HOME / "cache" / "videos",
    _WORK4YOU_HOME / "cache" / "documents",
    _WORK4YOU_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Work4You root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* WORK4YOU_HOME's cache, so a gateway running at
    the root (e.g. ``WORK4YOU_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when WORK4YOU_HOME is symlinked under a
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("WORK4YOU_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _WORK4YOU_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Work4You profile and shared Work4You root both contain control
    ...
    # WORK4YOU_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.work4you
    ...
    for work4you_root in (_WORK4YOU_HOME, _WORK4YOU_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(work4you_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Work4You
    secrets (``~/.work4you/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Work4You cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.work4you/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Work4You cache dirs (``/root/.work4you/...``), the default
    ...
    # In-process gateways (Desktop backend, `work4you serve`) may not have
    ...
    # are longer prefixes, so /root/.work4you/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.work4you/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.work4you"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``WORK4YOU_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Work4You-managed cache, under an operator-allowlisted root
    (``WORK4YOU_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Work4You root (~/.work4you/.env,
    ...
    # ``MEDIA:~/.work4you/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``WORK4YOU_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Work4You commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-WORK4YOU_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("WORK4YOU_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "WORK4YOU_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "WORK4YOU_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Work4You sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-WORK4YOU_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(work4you --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Work4You home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Work4You home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.work4you/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          WORK4YOU_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          WORK4YOU_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          WORK4YOU_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("WORK4YOU_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("WORK4YOU_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("WORK4YOU_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_work4you_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # WORK4YOU_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _work4you_run_generation is set on
    ...
    def build_source(
        ...
        per-profile WORK4YOU_HOME isolation.
        """
