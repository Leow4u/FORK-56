...
transport, discovers their tools, and registers them into the work4you
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.work4you/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.work4you/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with Work4You' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Work4You' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Work4You explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Work4You and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            work4you_home = os.path.expanduser(
                os.getenv(
                    "WORK4YOU_HOME", os.path.join(os.path.expanduser("~"), ".work4you")
                )
            )
            candidates = [
                os.path.join(work4you_home, "node", "bin", resolved_command),
                ...
                # image (which the Work4You Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Work4You MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Work4You gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Work4You document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`work4you_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Work4You profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Work4You' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (WORK4YOU_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit WORK4YOU_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Work4You' logging (agent.log via work4you_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `work4you setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Work4You process (kill -9, crash, force-quit)
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"work4you-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Work4You' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Work4You is fully
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``work4you mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Work4You' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"file under ~/.work4you/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Work4You processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_work4you_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local WORK4YOU_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local WORK4YOU_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_work4you_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Work4You config file.
    ...
    ``os.environ`` (which includes ``~/.work4you/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Work4You' MEDIA: tag convention and out to messaging
            ...
            # Work4You' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Work4You's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Work4You uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Work4You registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``work4you tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Work4You registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple work4you
