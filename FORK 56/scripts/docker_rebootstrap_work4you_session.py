#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Work4You bootstrap session.

Background
----------
A Work4You bootstrap session (client_id ``work4you-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.work4you.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``WORK4YOU_AUTH_JSON_BOOTSTRAP`` only
...
``WORK4YOU_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Work4You
...
Design constraints
------------------
- Pure stdlib, no work4you_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.work4you`` in the existing auth.json, leaving
...
# from WORK4YOU_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Work4You entry on an existing volume.
REBOOTSTRAP_ENV = "WORK4YOU_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "work4you-cli-vps"


def _work4you_entry_is_terminal(work4you_state: Any) -> bool:
    """True iff the on-disk Work4You provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``work4you_cli.auth.get_work4you_session_validity``:
    ...
    """
    if not isinstance(work4you_state, dict):
        return False
    last_err = work4you_state.get("last_auth_error")
    ...
    if work4you_state.get("access_token") or work4you_state.get("refresh_token"):
        return False
    return True


def _extract_work4you_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.work4you`` block out of a WORK4YOU_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    WORK4YOU_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(work4you, dict) or not work4you:
        return None
    if work4you.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(work4you.get("access_token"), str)
        and work4you["access_token"].strip()
        and isinstance(work4you.get("refresh_token"), str)
        and work4you["refresh_token"].strip()
    ):
        return None
    return work4you
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no work4you entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             WORK4YOU_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_nous = _extract_work4you_from_seed(seed_raw)
    ...
    local_nous = providers.get("work4you")
    terminal = _work4you_entry_is_terminal(local_nous)
    ...
    # Surgical replacement: swap ONLY providers.work4you, preserve everything else.
    providers["work4you"] = seed_nous
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Work4You bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Work4You bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
