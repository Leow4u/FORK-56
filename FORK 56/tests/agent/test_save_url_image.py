...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate WORK4YOU_HOME under tmp_path."""
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / ".work4you"))
    (tmp_path / ".work4you").mkdir()

    # Force the constants/image cache helpers to re-read WORK4YOU_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("work4you_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_work4you_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under WORK4YOU_HOME — gateway cleanup
