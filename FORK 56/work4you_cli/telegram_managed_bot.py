...
manual BotFather token copy-paste. Work4You talks only to the Work4You onboarding
...
# Default pairing API base URL (Work4You-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.work4you.ai"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The Work4You-hosted manager bot username (without @). The backend returns the
# actual deep link, so this is only used by local helpers/tests.
DEFAULT_MANAGER_BOT = "Work4YouSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``work4you_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Work4You Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
