"""Tests for agent/work4you_rate_guard.py — cross-session Work4You Portal rate limit guard."""
...
class TestRecordWork4YouRateLimit:
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_work4you_rate_limit(headers=None, default_cooldown=120.0)
...
class TestWork4YouRateLimitRemaining:
    """Test checking remaining rate limit time."""


    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.work4you_rate_guard import record_work4you_rate_limit, work4you_rate_limit_remaining

        record_work4you_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = work4you_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.work4you_rate_guard import work4you_rate_limit_remaining, _state_path
        ...
        assert work4you_rate_limit_remaining() is None
        # File should be cleaned up
        assert not os.path.exists(_state_path())



class TestClearWork4YouRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_work4you_rate_limit(headers={"retry-after": "600"})
        assert work4you_rate_limit_remaining() is not None
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.work4you_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.work4you_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_work4you_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.work4you_rate_guard import record_work4you_rate_limit

        # Record a rate limit
        record_work4you_rate_limit(headers={"retry-after": "600"})

        # Mock _read_work4you_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_work4you_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.work4you.test/v1",
        })

        result = aux._try_nous()
        assert result == (None, None)

    def test_try_work4you_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_nous should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_work4you_auth", lambda: None)
        result = aux._try_nous()
        assert result == (None, None)


class TestIsGenuineWork4YouRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    Work4You Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Work4You).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.work4you_rate_guard import is_genuine_work4you_rate_limit
        ...
        assert is_genuine_work4you_rate_limit(headers=headers) is True
    ...
    def test_bare_429_with_no_headers_is_upstream(self):
        ...
        assert is_genuine_work4you_rate_limit(headers=None) is False
        assert is_genuine_work4you_rate_limit(headers={}) is False
        assert is_genuine_work4you_rate_limit(
    ...
    def test_last_known_state_all_healthy_stays_upstream(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="work4you")
        assert is_genuine_work4you_rate_limit(
