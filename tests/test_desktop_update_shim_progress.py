...
def progress(tmp_path):
    ...
    status = tmp_path / "work4you-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Work4You to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `work4you update`, and reports the stage that was on screen
...
FAKE_WORK4YOU = """#!/bin/bash
n="$(cat "$WORK4YOU_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$WORK4YOU_TEST_CALLS"
for f in "$TMPDIR"/work4you-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$WORK4YOU_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$WORK4YOU_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "work4you"
    (install_root / "venv" / "bin").mkdir(parents=True)
    work4you = install_root / "venv" / "bin" / "work4you"
    work4you.write_text(FAKE_WORK4YOU)
    work4you.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "WORK4YOU_TEST_CAPTURE": str(capture),
        "WORK4YOU_TEST_CALLS": str(calls),
        "WORK4YOU_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".work4you-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`work4you update` is the longest wait in the hand-off and the one the
