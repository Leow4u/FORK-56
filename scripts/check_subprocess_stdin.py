...
When Work4You runs in TUI mode, the gateway child process communicates with
...
# ``get_work4you_home() / "plugins"`` (user) and ``./.work4you/plugins/`` (project,
# gated behind ``WORK4YOU_ENABLE_PROJECT_PLUGINS``) — see
# ``work4you_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``work4you_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import work4you_constants (this script
    ...
    # plugins in get_work4you_home()/plugins/ and project plugins in
    # ./.work4you/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_work4you_home() / "plugins"]
    if os.environ.get("WORK4YOU_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".work4you" / "plugins")
