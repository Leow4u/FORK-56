...
const UPSTREAM_URL = 'https://github.com/Leow4u/FORK-56.git'
...
const LEGACY_STORAGE_KEY = 'work4you.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'work4you.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.work4youDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `work4you-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const work4youHome = join(sandbox, 'work4you-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(work4youHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(work4youHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        WORK4YOU_DESKTOP_APP_NAME: `Work4YouShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        WORK4YOU_DESKTOP_WORK4YOU_ROOT: target.targetRoot,
        WORK4YOU_DESKTOP_IGNORE_EXISTING: '1',
        WORK4YOU_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'work4you-short-session-ab-'))
  const prepared = []
