...
on Japanese locales, etc.).  Work4You's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``work4you_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Work4You is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``WORK4YOU_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("WORK4YOU_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Work4You-managed Git install directories so
    ...
    # already-running shells don't see the change — which means work4you
    ...
    # install without Work4You-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Work4You's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Work4You's
    ``work4you config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Work4You (User env var in Windows Settings, or
    export in a PowerShell profile) and Work4You picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Work4You-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\work4you\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``work4you`` from right after install) retain their old PATH.

    Any subprocess Work4You spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Work4You-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "work4you", "git", "cmd"),
        os.path.join(local_appdata, "work4you", "git", "bin"),
        os.path.join(local_appdata, "work4you", "git", "usr", "bin"),
        # Work4You venv Scripts directory — host of the work4you.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes work4you, but harmless to include.
        os.path.join(local_appdata, "work4you", "work4you", "venv", "Scripts"),
