...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad WORK4YOU_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: WORK4YOU_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_work4you_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "WORK4YOU_HOME": "/h", "WORK4YOU_PROFILE": "p",
        "WORK4YOU_CONFIG": "/c.yaml", "WORK4YOU_ENV": "/e",
        "WORK4YOU_DELEGATED_CHILD_CONTEXT": "1",
        # other WORK4YOU_* → dropped (broad prefix removed)
        "WORK4YOU_BASE_URL": "https://x", "WORK4YOU_INTERACTIVE": "1",
        "WORK4YOU_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "WORK4YOU_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "WORK4YOU_BASE_URL", "WORK4YOU_INTERACTIVE", "WORK4YOU_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped WORK4YOU_* vars."""
    import logging
