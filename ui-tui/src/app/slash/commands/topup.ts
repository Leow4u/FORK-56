import { driveChargeSettlement, type SettlementOutcome } from '@work4you/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Work4You page; this is NOT a per-terminal stop.
      sys("Remote spending is off for this account — a billing admin can turn it on from the portal's Work4You page.")
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into Work4You Portal — run /portal to log in, then /topup.')
