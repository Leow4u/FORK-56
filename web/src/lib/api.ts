import { buildWork4YouWebSocketUrl } from "@work4you/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/work4you/). The Python backend
// injects ``window.__WORK4YOU_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__WORK4YOU_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const WORK4YOU_BASE_PATH = readBasePath();
const BASE = WORK4YOU_BASE_PATH;
...
declare global {
  interface Window {
    __WORK4YOU_SESSION_TOKEN__?: string;
    __WORK4YOU_BASE_PATH__?: string;
    ...
    __WORK4YOU_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Work4You-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``work4you update``, ``work4you gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__WORK4YOU_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__WORK4YOU_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__WORK4YOU_SESSION_TOKEN__`` is valid, so the next
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__WORK4YOU_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Work4You-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``work4you_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__WORK4YOU_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildWork4YouWebSocketUrl({
...
export const api = {
  ...
  updateWork4You: () =>
    fetchJSON<ActionResponse>("/api/work4you/update", { method: "POST" }),
  checkWork4YouUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/work4you/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Work4You Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** work4you-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["work4you"]``).
   ...
   * updates are handled by the outer launcher instead of ``work4you update``. */
  can_update_work4you?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the WORK4YOU_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the WORK4YOU_HOME volume — no staleness dimension, so no
 * sampled_at. */
...
export interface ToolsetProvider {
  ...
  requires_work4you_auth: boolean;
  is_active: boolean;
}
