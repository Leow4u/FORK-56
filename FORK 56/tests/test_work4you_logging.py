...
def _reset_logging_state():
    ...
    work4you_logging._reset_queued_handlers()
...
def work4you_home(tmp_path, monkeypatch):
    """Provide an isolated WORK4YOU_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_work4you_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, work4you_home):
        log_dir = work4you_logging.setup_logging(work4you_home=work4you_home)
        assert log_dir == work4you_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in work4you_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home)
        work4you_logging.setup_logging(work4you_home=work4you_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home)

        test_logger = logging.getLogger("test_work4you_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        work4you_logging.flush_log_queue()

        agent_log = work4you_home / "logs" / "agent.log"
    ...
    def test_explicit_params_override_config(self, work4you_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (work4you_home / "config.yaml").write_text(yaml.dump(config))

        work4you_logging.setup_logging(work4you_home=work4you_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, work4you_home):
        ...
        gw_log = work4you_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, work4you_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home, mode="gui")
        ...
        gui_log = work4you_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, work4you_home):
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home)
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_work4you_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in work4you_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, work4you_home):
        level, max_size, backup = work4you_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, work4you_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        work4you_logging.setup_logging(work4you_home=work4you_home, mode="gateway")
        gw_path = work4you_home / "logs" / "gateway.log"
        rotated = work4you_home / "logs" / "gateway.log.1"
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, work4you_home):
        work4you_logging.setup_logging(work4you_home=work4you_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_work4you_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in work4you_logging.rotating_file_handlers()
        )
