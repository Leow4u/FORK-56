...
declare global {
  interface Window {
    work4youDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<Work4YouConnection>
      ...
      // Resume this session in the user's own terminal emulator (`work4you --tui
      ...
      // Work4You Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new WORK4YOU_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: Work4YouApiRequest) => Promise<T>
      notify: (payload: Work4YouNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<Work4YouReadFileTextResult>
      selectPaths: (options?: Work4YouSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<Work4YouPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<Work4YouPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<Work4YouPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: Work4YouActiveWork) => void
      setTitleBarTheme?: (payload: Work4YouTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<Work4YouReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<WORK4YOU_HOME>/desktop-plugins),
      ...
      // Local AGENT-plugin root (<WORK4YOU_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<Work4YouGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<Work4YouGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<Work4YouGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<Work4YouRepoStatus | null>
        ...
        review: {
          ...
          shipInfo: (repoPath: string) => Promise<Work4YouReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<Work4YouRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<Work4YouPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: Work4YouTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<Work4YouTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: Work4YouWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: Work4YouPreviewFileChanged) => void) => () => void
...
export interface Work4YouTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface Work4YouTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `work4you update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  work4youRoot?: string
...
export interface Work4YouConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface Work4YouTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface Work4YouActiveWork {
  count: number
  titles: string[]
}

export interface Work4YouWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Work4You Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Work4You Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Work4You Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live Work4You portal (Privy) session — the
...
export interface Work4YouApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface Work4YouReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface Work4YouPreviewWatch {
  id: string
  path: string
}
...
export interface Work4YouGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface Work4YouGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface Work4YouGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface Work4YouRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface Work4YouRepoStatus {
  branch: null | string
...
export interface Work4YouReviewPr {
  url: string
  state: string
  number: number
}
...
export interface Work4YouBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface Work4YouRepoPullRequests {
  ghReady: boolean
  prs: Work4YouBranchPullRequest[]
}
...
export interface Work4YouPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface Work4YouReviewShipInfo {
  ghReady: boolean
  pr: Work4YouReviewPr | null
}

export interface Work4YouReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface Work4YouReadDirResult {
  entries: Work4YouReadDirEntry[]
  error?: string
}

export interface Work4YouPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface Work4YouSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
