...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.work4youDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.work4youDesktop?.openWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.work4youDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.work4youDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.work4youDesktop.openWindow(), 'Could not open a new window')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.work4youDesktop.openSessionInTerminal(sessionId, opts),
