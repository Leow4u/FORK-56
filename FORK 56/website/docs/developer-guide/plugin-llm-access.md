...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Work4You already has applies. The plugin never sees a
...
work4you itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Work4You provider id (e.g. "openrouter")
...
using Work4You built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Work4You provider
        # (must be one Work4You already knows about — same names as
        # `work4you model` and config.yaml model.provider).
        ...
        # plugin sends — Work4You does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.work4you/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Work4You' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Work4You subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Leow4u/FORK-56/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Leow4u/FORK-56/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/Work4You/work4you-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/Work4You/work4you-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
