...
before ``prompt.submit`` installs the session's profile WORK4YOU_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".work4you" / "profiles" / "coder"
...
def test_launch_home_fallback_when_no_profile(tmp_path):
    ...
    launch_home = tmp_path / ".work4you"
    session = {}

    with patch("tui_gateway.server._work4you_home", launch_home):
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".work4you" / "profiles" / "a"
    home_b = tmp_path / ".work4you" / "profiles" / "b"
