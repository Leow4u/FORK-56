#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Work4You Skills Hub.
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _work4you_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            work4you_meta = metadata.get("work4you", {})
            if isinstance(work4you_meta, dict):
                tags = work4you_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            work4you_meta = metadata.get("work4you", {})
            if isinstance(work4you_meta, dict):
                raw_tags = work4you_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Work4You) but not activated
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``work4you update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                work4you_meta = meta_block.get("work4you", {})
                if isinstance(work4you_meta, dict):
                    tags = work4you_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``work4you_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Work4You centralized index source
# ---------------------------------------------------------------------------

WORK4YOU_INDEX_URL = "https://work4you.ai/docs/api/skills-index.json"
WORK4YOU_INDEX_TTL = 6 * 3600  # 6 hours


def _work4you_index_cache_file() -> Path:
    return _index_cache_dir() / "work4you-index.json"


def _load_work4you_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for WORK4YOU_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    work4you_index_cache_file = _work4you_index_cache_file()
    if work4you_index_cache_file.exists():
        try:
            age = time.time() - work4you_index_cache_file.stat().st_mtime
            if age < WORK4YOU_INDEX_TTL:
                return json.loads(work4you_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Work4You index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Work4You index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Work4You index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        work4you_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        work4you_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class Work4YouIndexSource(SkillSource):
    """Skill source backed by the centralized Work4You Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_work4you_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "work4you-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "work4you-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        Work4YouIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "work4you-index"
