...
`~/.work4you/config.yaml`, `~/.work4you/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/work4you`:
...
The location can be relocated with the `WORK4YOU_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `WORK4YOU_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Work4You.
...
:::warning
A user who can set `WORK4YOU_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `work4you doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/work4you/config.yaml` (managed) | `/etc/work4you/.env` (managed) |
| 2 | `~/.work4you/config.yaml` (user) | `~/.work4you/.env` (user) |
...
If you try to change a managed value, Work4You refuses and names the source:
...
The same applies to managed secrets — `work4you config set` / setup will not write
...
Changes take effect on the next Work4You start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`work4you doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Work4You as `root`), managed scope is advisory.
