---
sidebar_position: 30
title: "Work4You Relay"
description: "Connect Work4You to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Work4You Relay (Connector)
...
Work4You Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Work4You Portal access token from your existing login
   (`~/.work4you/auth.json`) — this proves which Work4You org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no Work4You Portal involved): with
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.work4you/.env`:
...
`work4you gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.work4you/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Work4You Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `work4you auth add work4you` (or `work4you setup`) and retry.
...
startup. Restart it (`work4you gateway restart`).
