...
work4you meet install                 # pip deps + Chromium (transcribe only)
work4you meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
work4you meet auth                    # optional; skips guest-lobby wait
work4you meet setup                   # preflight checks
```

`work4you meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or WORK4YOU_MEET_REALTIME_KEY in ~/.work4you/.env
```
...
pip install playwright websockets && python -m playwright install chromium
work4you plugins enable google_meet
work4you meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
work4you meet node approve my-mac ws://<mac-ip>:18789 <token>
work4you meet node ping my-mac                   # confirm reachable
```

Run `work4you meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Work4You agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `work4you meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `WORK4YOU_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `work4you_meet_src`). |
...
Local:
```
$WORK4YOU_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$WORK4YOU_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `work4you meet node approve`.
