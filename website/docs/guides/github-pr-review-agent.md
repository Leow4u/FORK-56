...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Work4You instantly when PRs are opened or updated.
:::
...
- **Work4You installed** — see the [Installation guide](/getting-started/installation)
...
Use `deliver: "local"` to save reviews to `~/.work4you/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Work4You can access GitHub. Start a chat:
...
Still in the chat, ask Work4You to review a real PR:
...
A skill gives Work4You consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.work4you/skills/code-review
```

Create `~/.work4you/skills/code-review/SKILL.md`:
...
Verify it loaded — start `work4you` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Work4You your team's standards:
...
2. Teach Work4You your conventions via memory (Step 4)
