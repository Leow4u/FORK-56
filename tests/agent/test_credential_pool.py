...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "work4you" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "work4you" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "work4you" / "auth.json").read_text())
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://work4you/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://work4you/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    write_credential_pool("work4you", [
        {
            "id": "work4you-device",
            "label": "device-code",
            "auth_type": "oauth",
    ...
    persisted = json.loads((tmp_path / "work4you" / "auth.json").read_text())["credential_pool"]["work4you"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.work4you/.env when
    ...
    # User edited ~/.work4you/.env with the fresh key
    (work4you_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.work4you/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (work4you_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_mirrors_work4you_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you"))
    ...
    auth_payload = json.loads((tmp_path / "work4you" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["work4you"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_work4you_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `work4you setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Work4You PKCE
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_work4you_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `work4you_pkce` into auth.json) and later switches to the API key at
    `work4you setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("work4you_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_work4you_oauth_credentials", lambda: None)
    monkeypatch.setattr("agent.anthropic_adapter.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `work4you setup`.  When ANTHROPIC_TOKEN is set (and
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_work4you_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_custom_endpoint_pool_seeds_from_config(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "work4you" / "config.yaml"
    import yaml
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``work4you auth remove copilot gh_cli``.  The gate must run
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you"))
    from work4you_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("work4you_cli.copilot_auth.resolve_copilot_token", _boom)
...
def test_work4you_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.work4you`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-work4you-AAAA"
...
# ── PR #10160 salvage: Work4You OAuth cross-process sync tests ─────────────────
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "work4you" / "auth.json").read_text())
