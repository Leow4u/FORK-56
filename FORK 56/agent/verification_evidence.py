...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("work4you-verify-", "work4you-ad-hoc-")
...
def _db_path() -> Path:
    return get_work4you_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "work4you verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``work4you verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``work4you verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="work4you verify",
        kind="verify",
