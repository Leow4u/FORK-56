...
React + Ink terminal UI for Work4You. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `WORK4YOU_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `work4you setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Work4You remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.work4you/.work4you_history` or under `WORK4YOU_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Work4You remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Work4You credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `work4you setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/work4you-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external work4you setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Work4You remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.work4you/tui-parent.log
    ...
    types/
      work4you-ink.d.ts            type declarations for @work4you/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent Work4YouCLI subprocess for slash commands
```
