 ...
 * runtime, remote gateways, Work4You Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@work4you/shared`) — tsconfig project
...
/** Turn `ls ~/.work4you/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Work4You Cloud
 * instances are platform-managed — we never run `work4you update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteWork4YouPath: input.remoteWork4YouPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Work4You-managed names; an empty result stores nothing.
 ...
 * `remoteWork4YouPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Work4You Cloud' : 'Remote gateway'),
