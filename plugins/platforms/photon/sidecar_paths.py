...
under an immutable ``/opt/work4you`` (read-only for the work4you user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$WORK4YOU_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".work4you-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_work4you_home() / "photon" / "sidecar"
    try:
