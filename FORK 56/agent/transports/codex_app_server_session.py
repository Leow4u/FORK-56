...
Owns one Codex thread per Work4You session. Drives `turn/start`, consumes
...
# Work4You' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_WORK4YOU_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Work4You/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Work4You session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _WORK4YOU_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("WORK4YOU_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="work4you",
            client_title="Work4You",
            client_version=_get_work4you_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Work4You' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Work4You' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own work4you-tools callback we
            # auto-accept — the user already approved Work4You' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Work4You
        approval-mode/timeout logic. The Work4You-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Work4You approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Work4You approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Work4You returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Work4You approval mode/timeout resolution
    ...
    # "prompt expired"; the Work4You-side messaging already distinguishes them.
    return "decline"
...
def _get_work4you_version() -> str:
    """Best-effort Work4You version string for codex's userAgent line."""
    try:
