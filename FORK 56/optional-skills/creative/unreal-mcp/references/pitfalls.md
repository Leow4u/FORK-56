...
### 1. Start order: editor first, Work4You second

Work4You probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Work4You session.
...
restart the editor, restart the Work4You session.
...
server fails to bind (Output Log) or Work4You' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.work4you/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Work4You

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Work4You' connection
lives in `~/.work4you/config.yaml` via `work4you mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Work4You — just don't
mistake it for the Work4You setup step.
...
explicitly warns against overlapping calls. Work4You executes same-turn tool
...
### 9. Timeouts: Work4You gives up before Unreal does

Work4You' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Work4You runs elsewhere (SSH backend,
