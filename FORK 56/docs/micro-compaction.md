...
has to be thrown away or summarized. Work4You has always done this in one batch:
...
Work4You folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Work4You's normal session flush is
...
With `micro_compact` unset or `false` Work4You behaves exactly as it always has:
...
Defaults to `$WORK4YOU_HOME/logs/agent.log`. It reports passes, outcome mix, net
