...
``gateway run --replace`` only kills same-WORK4YOU_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "work4you_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(work4you --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_work4you_home(adapter):
    """Locks written before the profile field existed still attribute via work4you_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "work4you_home": "/opt/data/profiles/lead-gen-outreach",
