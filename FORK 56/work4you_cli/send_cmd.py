...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    # setup used only for outbound `work4you send`) would otherwise be
    ...
    if not platforms:
        ...
        print("channel discovery can populate ~/.work4you/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_work4you_env() -> None:
    """Populate ``os.environ`` from ``~/.work4you/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.work4you/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.work4you/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.work4you/.env and ~/.work4you/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.work4you/.env + ~/.work4you/config.yaml) — no LLM, "
