...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'work4you-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/work4you-runner-test',
    sourceRepoRoot: null,
    work4youHome: '/tmp/work4you-runner-test',
    logRoot: '/tmp/work4you-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'work4you', 'scripts')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/work4you',
      work4youHome: '/tmp/work4you'
    }),
    ['--dir', '/tmp/work4you', '--work4you-home', '/tmp/work4you', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/work4you', '--work4you-home', '/tmp/work4you', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/work4you',
      work4youHome: '/tmp/home'
    }),
    ['--dir', '/tmp/work4you', '--work4you-home', '/tmp/home', '--branch', 'main']
  )
})
