...
``auth.json``.  They intentionally have no dependency on ``work4you_cli.auth`` so
...
# Sources Work4You owns and can intentionally persist in auth.json.  Everything
...
_PERSISTABLE_PROVIDER_SOURCES = frozenset({
    ("anthropic", "work4you_pkce"),
    ("minimax-oauth", "oauth"),
    ("work4you", "device_code"),
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Work4You-owned OAuth/device-code state)
