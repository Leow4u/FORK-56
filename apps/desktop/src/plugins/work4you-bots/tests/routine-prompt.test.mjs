...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /work4you -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('work4you '), prompt.lastIndexOf('\n\nIf the command'))
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/work4you-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/work4you-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `work4you -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('work4you '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `work4you() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /work4you -p/)
})
