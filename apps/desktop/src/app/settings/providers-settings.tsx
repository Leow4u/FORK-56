...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.work4youDesktop?.terminal)
...
//      in work4you_cli/provider_catalog.py) — the SAME provider identity
//      `work4you model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Work4You can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Work4You can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Work4You never deletes creds another tool owns behind a silent API call.
