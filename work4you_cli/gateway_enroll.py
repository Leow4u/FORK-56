"""``work4you gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Work4You Portal access token from the existing login
     (``~/.work4you/auth.json``) — the same path ``work4you dashboard register``
     uses (``resolve_work4you_access_token``). This proves *which Work4You org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.work4you/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Work4You Portal).
    ...
    self-hosted-IdP path; otherwise Work4You Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Work4You Portal "
                "token could not be verified — try `work4you auth add work4you` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `work4you gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Work4You Portal); otherwise the Work4You Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.work4you/.env (0600 dir) and never logs.
    to_write = {
