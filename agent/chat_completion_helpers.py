...
def _merge_work4you_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    """Merge Portal ``tags`` / ``session_id`` onto an Anthropic Messages kwargs dict.

    The Work4You provider profile is only consulted by the OpenAI-wire transport;
    ...
    """
    if getattr(agent, "provider", None) not in {"work4you", "work4you-portal", "work4you"}:
        return anthropic_kwargs
    try:
        from providers import get_provider_profile

        work4you_profile = get_provider_profile("work4you")
        if work4you_profile is not None:
            anthropic_kwargs.setdefault("extra_body", {}).update(
                work4you_profile.build_extra_body(
                    session_id=getattr(agent, "session_id", None)
                )
            )
    except Exception as exc:  # noqa: BLE001 — never block a turn on tagging
        logger.debug("Work4You Portal extra_body merge failed: %s", exc)
    return anthropic_kwargs
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("WORK4YOU_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("WORK4YOU_STREAM_STALE_TIMEOUT", 180.0)
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``WORK4YOU_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via WORK4YOU_CODEX_TTFB_TIMEOUT_SECONDS and
    # WORK4YOU_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via WORK4YOU_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("WORK4YOU_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # WORK4YOU_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("WORK4YOU_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("WORK4YOU_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("WORK4YOU_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set WORK4YOU_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("WORK4YOU_CODEX_TTFB_MAX_SECONDS", 120.0)
    ...
    _codex_idle_timeout = _env_float(
        "WORK4YOU_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        # Work4You Portal reads ``tags`` and ``session_id`` as top-level body fields
        ...
        return _merge_work4you_portal_messages_extra_body(agent, anthropic_kwargs)
    ...
    _is_nous = base_url_host_matches(agent._base_url_lower, "work4you.ai")
    ...
    # like write_file/patch.  OpenRouter/Work4You were the only routes covered
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "work4you",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects WORK4YOU_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
        from agent.redact import redact_sensitive_text
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    try:
        ...
        return f"work4you_auth_unreadable:{type(exc).__name__}"
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Work4You-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Work4You bookkeeping,
        ...
        _is_nous = "work4you" in agent._base_url_lower
        ...
        if _is_nous:
            from agent.portal_tags import work4you_portal_tags as _portal_tags
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "anthropic_messages":
                ...
                _ant_kw = _merge_work4you_portal_messages_extra_body(agent, _ant_kw)
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                _ant_kw2 = _merge_work4you_portal_messages_extra_body(agent, _ant_kw2)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
        ...
        try:
            ...
            while t.is_alive():
                ...
                if _stale_elapsed > _bedrock_stale_timeout:
                    ...
                    # crosses WORK4YOU_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the WORK4YOU_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("WORK4YOU_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # WORK4YOU_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("WORK4YOU_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode WORK4YOU_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Work4You. Preserve only
        ...
        if agent.provider == "moa":
            # Work4You interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Work4You preserves
        ...
        #      Work4You dedicated endpoint) stall for minutes during large
    ...
    def _call():
        ...
        _max_stream_retries = env_int("WORK4YOU_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("WORK4YOU_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set WORK4YOU_STREAM_STALE_TIMEOUT; override the
    # local ceiling with WORK4YOU_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``WORK4YOU_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("WORK4YOU_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
