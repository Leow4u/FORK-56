...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``WORK4YOU_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("WORK4YOU_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_work4you_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Work4You.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/Work4You minimax/minimax-m3).
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.work4you.ai": "work4you",
    ...
    # (often 8K) makes it unusable for Work4You' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Work4You minimum.
...
def _resolve_endpoint_context_length(
    ...
    if not matched:
        if len(endpoint_metadata) == 1:
            ...
            # a 32K embedding model on the Work4You portal) and poison the
...
def _get_context_cache_path() -> Path:
    ...
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Work4You phrasing of the same condition.
    ...
    # OpenRouter/Work4You format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Work4You
            and "maximum context length" in error_lower)
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Work4You grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Work4You uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_work4you_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Work4You Portal model context length.

    Tries the live Work4You inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Work4You model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Work4You /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        resolve_context_length) so the Work4You portal path does not short-circuit it.
        """
        ...
        if ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info(
                ...
                "(known 32K underreport, Work4You path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Work4You URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Work4You: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # Drop the stale entry and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Work4You Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "work4you":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Work4You portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from work4you_cli.models import get_copilot_model_context
    ...
    if effective_provider == "work4you":
        ctx, source = _resolve_work4you_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Work4You URLs.
    ...
    # the dedicated Work4You/Copilot/GMI branches above.
    ...
    # ``Work4You-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Work4You sends to providers:
