...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@work4you/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.WORK4YOU_TUI_THEME : undefined

  writeBootTheme(theme, process.env.WORK4YOU_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to WORK4YOU_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit WORK4YOU_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the WORK4YOU_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.WORK4YOU_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.WORK4YOU_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a WORK4YOU_TUI_THEME the user
    ...
    delete process.env.WORK4YOU_TUI_THEME
  }
 ...
 * is cached into WORK4YOU_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * WORK4YOU_TUI_LIGHT / WORK4YOU_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.WORK4YOU_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.WORK4YOU_TUI_THEME || process.env.WORK4YOU_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.WORK4YOU_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.WORK4YOU_TUI_BACKGROUND ||
      process.env.WORK4YOU_TUI_THEME ||
      process.env.WORK4YOU_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.WORK4YOU_TUI_BACKGROUND || process.env.WORK4YOU_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.WORK4YOU_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Work4You" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `work4you -c` /
    // `work4you --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey Work4You": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: work4you -p ${wakeProfile} --tui`)
