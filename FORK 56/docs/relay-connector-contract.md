...
This document is the formal interface between the **Work4You gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Work4You/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `work4you gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`work4you gateway enroll` → connector `/relay/enroll`, or
...
(`Work4You/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `work4you` CLI.
