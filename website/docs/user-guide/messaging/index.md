...
description: "Chat with Work4You from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Work4You](/guides/use-voice-mode-with-work4you).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Work4You Portal](/integrations/work4you-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Work4You Gateway"]
...
For group chats, hooks, and automation flows, Work4You supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Work4You keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Work4You does not hide failures just because the text resembles a silence token.
...
```bash
work4you gateway setup        # Interactive setup for all messaging platforms
```
...
work4you gateway setup        # Configure messaging platforms interactively
work4you gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo work4you gateway install --system   # Linux only: install a boot-time system service
work4you gateway start        # Start the default service
work4you gateway stop         # Stop the default service
work4you gateway status       # Check default service status
work4you gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.work4you/config.yaml"
gateway:
...
```bash
work4you gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Work4You sends heartbeats
...
| `/update` | Update Work4You to the latest version |
...
with the `session_reset` section in `~/.work4you/config.yaml`:
...
Configure per-platform overrides in `~/.work4you/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.work4you/gateway-config.yaml`:
...
work4you pairing approve telegram XKGH5N7P

# Other pairing commands:
work4you pairing list          # View pending + approved users
work4you pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Work4You appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.work4you/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.work4you/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `WORK4YOU_HOME`:

- `~/.work4you/status_phrases.yaml` or any `*.yaml` in `~/.work4you/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to WORK4YOU_HOME
...
Off by default. When enabled, Work4You prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.work4you/config.yaml`:
...
```bash
WORK4YOU_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
work4you gateway install               # Install as user service
work4you gateway start                 # Start the service
work4you gateway stop                  # Stop the service
work4you gateway status                # Check status
journalctl --user -u work4you-gateway -f  # View logs
...
sudo work4you gateway start --system
sudo work4you gateway status --system
journalctl -u work4you-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Work4You installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit work4you-gateway` (or `sudo systemctl edit work4you-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `work4you update`. When `work4you update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart work4you-gateway` command (it never blocks on an interactive password prompt).
...
```bash
work4you gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `work4you update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo work4you update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/work4you-gateway`:

```
work4you ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed work4you-gateway*, /usr/bin/systemctl --no-ask-password start work4you-gateway*, /usr/bin/systemctl --no-ask-password restart work4you-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Work4You will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Work4You installations on the same machine (with different `WORK4YOU_HOME` directories), each gets its own systemd service name. The default `~/.work4you` uses `work4you-gateway`; other installations use `work4you-gateway-<hash>`. The `work4you gateway` commands automatically target the correct service for your current `WORK4YOU_HOME`.
:::

### macOS (launchd)

```bash
work4you gateway install               # Install as launchd agent
work4you gateway start                 # Start the service
work4you gateway stop                  # Stop the service
work4you gateway status                # Check status
tail -f ~/.work4you/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.work4you.gateway.plist`. It includes three environment variables:
...
- **WORK4YOU_HOME** — scopes the gateway to your Work4You installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `work4you gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::
...
|----------|---------|--------------|
| CLI | `work4you-cli` | Full access |
| Telegram | `work4you-telegram` | Full tools including terminal |
| Discord | `work4you-discord` | Full tools including terminal |
| WhatsApp | `work4you-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `work4you-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `work4you-slack` | Full tools including terminal |
| Google Chat | `work4you-google_chat` | Full tools including terminal |
| Signal | `work4you-signal` | Full tools including terminal |
| SMS | `work4you-sms` | Full tools including terminal |
| Email | `work4you-email` | Full tools including terminal |
| Home Assistant | `work4you-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `work4you-mattermost` | Full tools including terminal |
| Matrix | `work4you-matrix` | Full tools including terminal |
| DingTalk | `work4you-dingtalk` | Full tools including terminal |
| Feishu/Lark | `work4you-feishu` | Full tools including terminal |
| WeCom | `work4you-wecom` | Full tools including terminal |
| WeCom Callback | `work4you-wecom-callback` | Full tools including terminal |
| Weixin | `work4you-weixin` | Full tools including terminal |
| BlueBubbles | `work4you-bluebubbles` | Full tools including terminal |
| QQBot | `work4you-qqbot` | Full tools including terminal |
| Yuanbao | `work4you-yuanbao` | Full tools including terminal |
| Microsoft Teams | `work4you-teams` | Full tools including terminal |
| API Server | `work4you-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `work4you-webhook` | Full tools including terminal |
| Raft | `work4you-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.work4you/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
