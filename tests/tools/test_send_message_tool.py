...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "work4you-in"})
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from work4you"
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@work4you:matrix.org", "@work4you:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``work4you cron``
    runs separately from ``work4you gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``WORK4YOU_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``WORK4YOU_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``WORK4YOU_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
