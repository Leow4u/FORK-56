"""Work4You update pipeline — extracted from ``work4you_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``work4you_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``work4you_cli.main`` reference — so existing call sites and test monkeypatches
that target ``work4you_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``work4you_cli.main``.
...
Imports are one-way: ``work4you_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``work4you_cli.main`` reference.

    Lets callers keep patching ``work4you_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``work4you_cli.main`` -> ``work4you_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``work4you update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``work4you update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``work4you_cli.config_defaults``,
    ``work4you_cli.config``, and ``work4you_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``work4you_cli._subprocess_compat`` and
    ``work4you_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "work4you_cli.config_defaults",
        "work4you_cli.config",
        "work4you_cli.config_migrations",
        "work4you_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from work4you_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from work4you_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that Work4You must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "work4you_cli/main.py",
    "work4you_cli/config.py",
    "work4you_cli/__init__.py",
    "work4you_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``work4you`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="work4you-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``work4you update`` can be driven by a
    ...
    """
    from work4you_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from work4you_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('work4you_'):\n"
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://work4you.ai/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `work4you update`.
    ...
    try:
        ...
        return
    db_path = get_work4you_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `work4you sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    work4you sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  work4you sessions optimize-storage")
    print(
...
def _reload_process_scan_modules() -> None:
    ...
    ``from work4you_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``work4you_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.work4you-update-staging"
    backup = f"{dst}.work4you-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `work4you update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``work4you_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``work4you update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && work4you update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("WORK4YOU_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== work4you-update completed {action_id} ===")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `work4you update "
    ...
    try:
        ...
        print(
            "  Re-run `work4you update` to retry; if the agent won't start, "
            "reinstall from https://work4you.ai"
        )
        _m().sys.exit(1)
    finally:
    ...
    from work4you_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from work4you_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from work4you_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_work4you_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_work4you_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "work4you-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes work4you completely unrunnable (SyntaxError on import).
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Leow4u/FORK-56.git",
    "git@github.com:Leow4u/FORK-56.git",
    "https://github.com/Leow4u/FORK-56",
    "git@github.com:Leow4u/FORK-56",
}

OFFICIAL_REPO_URL = "https://github.com/Leow4u/FORK-56.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_work4you_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_work4you_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Work4You repository.")
        print("  This means you may miss updates from Leow4u/FORK-56.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Leow4u/FORK-56.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Leow4u/FORK-56.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    default_home = get_default_work4you_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "work4you.exe"
    lines = ["✗ Another work4you.exe is running:"]
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``work4you tools``."""
    try:
        from work4you_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Work4You Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``work4you tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Work4You Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Work4You Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `work4you update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `work4you update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Work4You' editable-install extras or
    ...
    try:
        from work4you_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from work4you_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$WORK4YOU_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `work4you update` is exactly the step
...
def _npm_lockfile_changed(work4you_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `work4you
    ...
    try:
        ...
        cache_file = work4you_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(work4you_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    work4you update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `work4you update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `work4you update`.")
            failed = []
            if any(
    ...
    # Work4You profile using this checkout. Keep one per-checkout cache under the
    # shared Work4You root rather than rerunning npm once per named profile.
    shared_work4you_root = get_default_work4you_root()
    ...
    # `work4you update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_work4you_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `work4you update`.")
        return list(labels)
    ...
    nixos_env = with_work4you_node_path(_m()._nixos_build_env())
    ...
    # `work4you update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_work4you_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.work4you/logs/update.log`` only, never the terminal.
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``work4you update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from work4you_cli.config import format_docker_update_message
    ...
    from work4you_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from work4you_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    ``work4you`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/work4you.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v work4you'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/work4you, code at /usr/local/lib/work4you).
    fhs_link = Path("/usr/local/bin/work4you")
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``work4you-acp`` launcher next to the ``work4you`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``work4you update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``work4you-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Work4You as not installed even when it is.

    The shim simply delegates to the sibling ``work4you`` launcher with the
    ...
    No-op on Windows (install.ps1 copies ``work4you.exe`` + ``work4you-acp.exe``
    ...
    so ``work4you-acp.exe`` already resolves) and wherever a ``work4you-acp`` is
    already present next to the ``work4you`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        work4you_cmd = bin_dir / "work4you"
        acp_cmd = bin_dir / "work4you-acp"
        try:
            if not (work4you_cmd.is_file() or work4you_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of WORK4YOU_HOME under
      ``backups/`` (restorable via ``work4you import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from work4you_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from work4you_constants import
        # get_work4you_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from work4you_cli.config import get_work4you_home as _get_home
    ...
    try:
        from work4you_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from work4you_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_work4you_home so the user sees ~/.work4you/...
    try:
        from work4you_constants import get_work4you_home, display_work4you_home

        home = get_work4you_home()
        try:
            display_path = f"{display_work4you_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        # dev may run work4you from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.work4you-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".work4you-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The work4you.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m work4you_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m work4you_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "work4you_cli.main" in cmdline_low:
        ...
        # the `-m work4you_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``work4you_cli.main`` used to import
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `work4you` launch will complete it")
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``work4you update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from work4you_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning work4you-setup, and the update-in-progress marker
    ...
    the update with "Work4You is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Work4You backend (``work4you_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.work4you-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "work4you_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.work4you-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    work4you.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from work4you_cli.gateway import (
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m work4you_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from work4you_cli import gateway_windows
        from work4you_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``work4you-gateway*.service``/``work4you-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("work4you-serve")`` alone would also accept the
        # unrelated ``work4you-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "work4you-gateway.service"
            or unit.startswith("work4you-gateway-")
            or unit == "work4you-serve.service"
            or unit.startswith("work4you-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``work4you-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``work4you-serve*`` units (#83438) don't, so sending them
    ...
    (``work4you-gateway-helper`` is fine — profile units are
    ``work4you-gateway-<profile>`` — but ``work4you-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "work4you-gateway" or svc_name.startswith("work4you-gateway-")
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``work4you_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``work4you_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``work4you update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``work4you-setup.exe``) executes
    ``$WORK4YOU_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_work4you_home()) / "bootstrap-cache"
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``work4you update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `work4you desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "work4you_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Work4You-managed Node on PATH: when
    # `work4you update` runs inside the desktop updater chain (Desktop →
    # work4you-setup → work4you update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from work4you_constants import with_work4you_node_path

    build_env = with_work4you_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `work4you desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another work4you.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_work4you_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `work4you serve` backend) will keep .pyd
    ...
    # the work4you.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .work4you-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Work4You is still
    ...
    # process by design (a CLI `work4you update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Work4You gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in work4you_cli/config.py bricked
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``work4you
        ...
        # Before this guard, ``work4you update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``work4you doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && work4you update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_work4you_home added to work4you_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``work4you`` launch finishes the install
        ...
        # symbols in work4you_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `work4you` again "
        ...
        if not import_ok:
            ...
            print("    Run `work4you update` again — if it persists, reinstall:")
            print("    https://work4you.ai")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_work4you_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.work4you/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit WORK4YOU_HOME so
        # it is not affected by sync_skills()'s module-level WORK4YOU_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's WORK4YOU_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from work4you_cli.profiles import backfill_profile_envs
        ...
        # ``work4you update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``work4you_cli.config`` and
        # ``work4you_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from work4you_cli.config import
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'work4you config migrate' manually to configure."
                    )
                    response = "n"
        ...
        # it and warn loudly.
        try:
            from work4you_cli.backup import restore_cron_jobs_if_emptied
        ...
        # Self-heal the work4you-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Work4You on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("work4you-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                from work4you_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `work4you update`
                # must stay fast; `work4you computer-use install --upgrade`
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... work4you-gateway*`` also qualifies
            ...
            try:
                from work4you_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all work4you-gateway* units (default + profiles) plus
            # work4you-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # work4you-serve has no such handler (it isn't
            ...
            if failed_or_stale_units:
                ...
                if gateway_mode:
                    _exit_code_path = get_work4you_home() / ".update_exit_code"
                    try:
        ...
        # Warn if legacy Work4You gateway unit files are still installed.
        # When both work4you.service (from a pre-rename install) and the
        # current work4you-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `work4you update` surfaces the issue until the user migrates.
        try:
            from work4you_cli.gateway import (
                has_legacy_work4you_units,
                _find_legacy_work4you_units,
                supports_systemd_services,
            )
            ...
            if supports_systemd_services() and has_legacy_work4you_units():
                ...
                for name, path, is_sys in _find_legacy_work4you_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (work4you.service) fight the current")
        ...
        #
        # Forward the systemd units restarted above (includes work4you-serve*,
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``work4you_cli.gateway`` no longer imports,
