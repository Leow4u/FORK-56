...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the Work4You
     skill-authoring standards (description <=60 chars, the modern section
     order, Work4You-tool framing, no invented commands). Small sources get one
...
# (HARDLINE)" and the work4you-dev new-skill salvage reference. Embedded in
...
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.work4you.tags: a few Capitalized, Relevant, Tags.
