...
Data resolution order:
  ...
  2. Disk cache (~/.work4you/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Work4You ↔ models.dev
# ---------------------------------------------------------------------------

# Work4You provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Work4You' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Work4You ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_work4you_ids(mdev_id: str) -> List[str]:
    """Return the Work4You provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for work4you_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(work4you_id)
...
def _get_cache_path() -> Path:
    ...
    return get_work4you_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``work4you config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Work4You provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Work4You id.
    for work4you_id in _models_dev_to_work4you_ids(provider_key):
        if work4you_id != provider_key:
            candidates.append(work4you_id)
...
def _get_provider_models(
    ...
    """Resolve a Work4You provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``work4you model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from work4you_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Work4You agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Work4You provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Work4You ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Work4You or models.dev provider ID.  Tries exact match then
