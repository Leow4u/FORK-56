...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "work4you" else ""
...
def _local_config_path() -> Path:
    ...
    Always returns $WORK4YOU_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_work4you_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        work4you_host.get("pinUserPeer"),
        work4you_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in work4you_host:
        aliases_src = work4you_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in work4you_host:
        prefix_src = work4you_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(work4you_host: dict) -> None:
...
def _apply_runtime_prefix(
    work4you_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        work4you_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(work4you_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = work4you_host.get("userPeerAliases")
    prefix = work4you_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(work4you_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(work4you_host)
    work4you_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        work4you_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        work4you_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/work4you (NS-605).
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = work4you_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            work4you_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(work4you_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'work4you honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'work4you honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                ...
                return
            print(" approved")
            _apply_grant_to_host(work4you_host, cred)
    ...
    # --- 3. Identity ---
    current_peer = work4you_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        work4you_host["peerName"] = new_peer

    current_ai = work4you_host.get("aiPeer") or cfg.get("aiPeer", "work4you")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        work4you_host["aiPeer"] = new_ai

    current_workspace = work4you_host.get("workspace") or cfg.get("workspace", "work4you")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        work4you_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, work4you_host)
    ...
    if run_mapping:
        peer_target = work4you_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(work4you_host)
            work4you_host["pinUserPeer"] = False
            if prior_aliases:
                work4you_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                work4you_host, current_prefix, prefix_from_root,
            ...
            if merged:
                work4you_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                work4you_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                work4you_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = work4you_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        work4you_host["observationMode"] = new_obs
    else:
        work4you_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(work4you_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        work4you_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        work4you_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = work4you_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        work4you_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = work4you_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        work4you_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                work4you_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(work4you_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            work4you_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        work4you_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        work4you_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        work4you_host["dialecticReasoningLevel"] = new_reasoning
    else:
        work4you_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = work4you_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        work4you_host["sessionStrategy"] = new_strat

    work4you_host["enabled"] = True
    work4you_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        work4you_config = load_config()
        work4you_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(work4you_config)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if work4you_cli internals move."""
    try:
        from work4you_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(work4you_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    work4you_host["apiKey"] = cred.access_token
    work4you_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        work4you_host["peerName"] = cred.consent_peer_name
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("work4you.work") stay readable per the README's back-compat
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        work4you = hosts.get(_host_key(), {})
        user = work4you.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = work4you.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = work4you.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = work4you.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Work4You' identity in Honcho. Seed with 'work4you honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = work4you.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = work4you.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = work4you.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
