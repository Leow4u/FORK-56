...
``CommandSecretsProvider`` (work4you-desktop ``src/main/secrets/commandProvider.ts``)
...
* The requested key is passed to the child ONLY via the ``WORK4YOU_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``WORK4YOU_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# work4you_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``WORK4YOU_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["WORK4YOU_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``WORK4YOU_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # WORK4YOU_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Work4You discards helper stderr so diagnostics can't leak "
                "secret material."
            )
