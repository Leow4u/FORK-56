"""`setup_path()` must also install a `work4you-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `work4you-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Work4You as not installed. `setup_path()` therefore
writes a `work4you-acp` launcher next to the `work4you` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/work4you-acp".*?'
    r'log_success "Installed work4you-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the work4you-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    work4you_bin = tmp_path / "venv" / "bin" / "python"
    work4you_bin.parent.mkdir(parents=True)
    work4you_bin.write_text("#!/bin/sh\n", encoding="utf-8")
    entrypoint = tmp_path / "work4you"
    ...
    return command_link_dir / "work4you-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `work4you-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "work4you-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/work4you-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/work4you-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# work4you launcher regression (#74819)
# ---------------------------------------------------------------------------

WORK4YOU_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/work4you".*?'
    r'log_success "Installed work4you launcher[^\n]*\n)',
    re.S,
)


def _extract_work4you_shim_block() -> str:
    match = WORK4YOU_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
        "could not locate the work4you launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)


def _run_work4you_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted work4you block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: work4you is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"work4you shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "work4you"


def test_venv_install_writes_executable_work4you_launcher(tmp_path):
    """venv install must write a user-executable work4you launcher."""
    shim = _run_work4you_block(tmp_path, "true")
...
def test_work4you_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove work4you alongside
    work4you and work4you-acp."""
    from work4you_cli.uninstall import remove_wrapper_script

    # Simulate a work4you wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "work4you"
    ...
    assert local_shim in removed, "local work4you wrapper must be removed"
