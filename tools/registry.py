"""Central registry for all work4you tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (work4you_constants itself is stdlib-only, so no cycle).
        from work4you_constants import get_work4you_home

        return Path(get_work4you_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``work4you tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Work4You-home override for every
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``work4you tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved WORK4YOU_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("work4you_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``work4you_plugins.{name}``), not the exact module string.
                # A handler defined in ``work4you_plugins.pkg.handlers`` is
                # still owned by the ``work4you_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``work4you tools enable foo``)
