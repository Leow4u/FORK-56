...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.WORK4YOU_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Work4You on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via WORK4YOU_TUI_TRUECOLOR=1.
