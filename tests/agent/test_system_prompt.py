...
def _captured_context_cwd(agent):
    ...
    with (
        ...
        patch("run_agent.build_work4you_subscription_prompt", return_value=""),
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_work4you_home``,
    ``get_default_work4you_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``WORK4YOU_HOME`` and no resolver mocks.
    """

    def test_real_work4you_home_under_profiles_renders_correct_paths(
        ...
        assert get_work4you_home() == profile_home
        assert get_default_work4you_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Work4You profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """WORK4YOU_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".work4you"
        ...
        assert "Active Work4You profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "WORK4YOU_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_work4you_home", lambda: Path("/work4you"))

    expected_profile = (
        "Active Work4You profile: default. Other profiles (if any) live "
        "under /work4you/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("work4you_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("work4you_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under WORK4YOU_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from work4you_cli import config as _cfgmod
