...
import { FilterGroup, Segmented } from "@work4you/ui/ui/components/segmented";
...
export interface LogsPanelProps {
  /** When true, omit plugin slots (Settings embed). */
  embedded?: boolean;
}

/** Shared log tail UI — used by /logs (operator) and Settings → Portal. */
export function LogsPanel({ embedded = false }: LogsPanelProps) {
  ...
  const logBodyMaxHeight = embedded
    ? "max-h-[min(420px,50vh)]"
    : "max-h-[calc(100vh-220px)]";

  return (
    <div className="flex min-w-0 max-w-full flex-col gap-4">
      {!embedded && <PluginSlot name="logs:top" />}
      <div
        role="toolbar"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <Label
            htmlFor={embedded ? "logs-auto-refresh-settings" : "logs-auto-refresh"}
          ...
          <Switch
            ...
            id={embedded ? "logs-auto-refresh-settings" : "logs-auto-refresh"}
          />
          <Button
            type="button"
            ghost
      ...
      <Card className="min-w-0 max-w-full overflow-hidden">
        ...
        <CardContent className="p-0">
          ...
          <div
            ref={scrollRef}
            className={`max-w-full min-h-[240px] ${logBodyMaxHeight} overflow-auto p-4 font-mono-ui text-xs leading-5 break-words`}
          >
      ...
      {!embedded && <PluginSlot name="logs:bottom" />}
    </div>
  );
}
