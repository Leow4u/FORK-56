...
def _work4you_home_path() -> Path:
    """Resolve the active WORK4YOU_HOME (profile-aware) without circular imports."""
    try:
        from work4you_constants import get_work4you_home  # local import to avoid cycles
        return get_work4you_home()
    except Exception:
        return Path(os.path.expanduser("~/.work4you"))


def _work4you_root_path() -> Path:
    """Resolve the Work4You root dir (always the parent of any profile, never per-profile)."""
    try:
        from work4you_constants import get_default_work4you_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    work4you_home = _work4you_home_path()
    work4you_root = _work4you_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(work4you_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(work4you_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(work4you_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(work4you_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(work4you_home / "cache" / "bws_cache.enc.json"),
            str(work4you_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved WORK4YOU_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("WORK4YOU_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    work4you_dirs = []
    for base in (_work4you_home_path(), _work4you_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in work4you_dirs:
                work4you_dirs.append(real)
        except Exception:
            continue

    for base_real in work4you_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside WORK4YOU_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Work4You path.

    Three categories are blocked:

      * Internal Work4You cache files under ``WORK4YOU_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under WORK4YOU_HOME and the global Work4You
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.work4you/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active WORK4YOU_HOME (profile-aware) AND the global
    # Work4You root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (WORK4YOU_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    work4you_dirs: list[Path] = []
    for base in (_work4you_home_path(), _work4you_root_path()):
        try:
            real = base.resolve()
            if real not in work4you_dirs:
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Work4You read (see
...
#
# Work4You profiles are separate WORK4YOU_HOME dirs under
...
#
# Reference: May 2026 incident where a work4you-security profile session
# edited skills under both ``~/.work4you/profiles/work4you-security/skills/``
# AND ``~/.work4you/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under WORK4YOU_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from WORK4YOU_HOME.

    ``~/.work4you``              -> ``"default"``
    ``~/.work4you/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _work4you_home_path().resolve()
        root_real = _work4you_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Work4You scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Work4You entirely. Caller is expected to surface the warning to the
...
#
#   <WORK4YOU_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.work4you/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.work4you/…`` segment and warns
# regardless of which Work4You profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.work4you/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.work4you`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.work4you/…`` and returns the
    index where the inner Work4You-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .work4you / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".work4you":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Work4You state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.work4you``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.work4you`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Work4You resolver to
    succeed, so it works correctly even when called from contexts where
    WORK4YOU_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real WORK4YOU_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.work4you/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.work4you/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host WORK4YOU_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Work4You state.
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real WORK4YOU_HOME. Use the "
