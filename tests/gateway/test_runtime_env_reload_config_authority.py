...
WORK4YOU_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env WORK4YOU_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    (work4you_home / ".env").write_text(
        "WORK4YOU_MAX_ITERATIONS=90\nOPENROUTER_API_KEY=fresh-key\n",
        encoding="utf-8",
    )

    monkeypatch.setattr(gateway_run, "_work4you_home", work4you_home)
    monkeypatch.setenv("WORK4YOU_MAX_ITERATIONS", "9000")
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["OPENROUTER_API_KEY"] == "fresh-key"
    assert os.environ["WORK4YOU_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``work4you config get terminal.backend`` still says local.
    """
    work4you_home = tmp_path / ".work4you"
    work4you_home.mkdir()
    (work4you_home / "config.yaml").write_text(
    ...
    (work4you_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
