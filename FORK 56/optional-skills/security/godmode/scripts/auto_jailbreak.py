...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("WORK4YOU_HOME", os.path.expanduser("~/.work4you")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("WORK4YOU_HOME", Path.home() / ".work4you")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Work4You config paths
# ═══════════════════════════════════════════════════════════════════

WORK4YOU_HOME = Path(os.getenv("WORK4YOU_HOME", Path.home() / ".work4you"))
CONFIG_PATH = WORK4YOU_HOME / "config.yaml"
PREFILL_PATH = WORK4YOU_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Work4You/Work4You models — already uncensored, just needs clean prompt
    "work4you": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "work4you" in model_lower or "work4you" in model_lower:
        return "work4you"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Work4You config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.work4you/prefill.json."""
