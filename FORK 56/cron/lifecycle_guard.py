...
``work4you gateway restart`` (or ``launchctl kickstart ai.work4you.gateway``
or ``systemctl restart work4you-gateway``).  When the cron fires, the
...
``work4you cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``work4you gateway``, ``launchctl ... work4you-gateway``,
``systemctl ... work4you-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_WORK4YOU_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``work4you gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `work4you gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:work4you\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a work4you-gateway label. macOS launchd
    # labels look like `ai.work4you.gateway` / `work4you-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated work4you services (e.g.
    # `launchctl unload ai.work4you.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.work4you.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a work4you-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bwork4you[.\-]?gateway)"
    # Branch D: pkill / kill targeting the work4you gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.work4you.gateway-... \` / `  -- ...`,
...
# for `systemctl restart work4you-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart work4you-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.work4you.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart work4you-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart work4you-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".work4you"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<WORK4YOU_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.work4you/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_work4you_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_work4you_home() falls back to Path.home(), which raises when
        # neither WORK4YOU_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "work4you gateway restart"
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.work4you/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `work4you gateway restart` embedded in a .py script is still
