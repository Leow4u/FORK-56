"""Tests for the get_work4you_auth_status() process-level cache.

The cache avoids re-validating Work4You credentials on every menu paint —
`work4you tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.work4you.ai during one render. The cache is keyed
...
also call invalidate_work4you_auth_status_cache().
"""
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test WORK4YOU_HOME.
...
def test_get_work4you_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    auth_mod.invalidate_work4you_auth_status_cache()
    ...
    with patch.object(auth_mod, "_compute_work4you_auth_status", side_effect=fake_compute):
        first = auth_mod.get_work4you_auth_status()
        second = auth_mod.get_work4you_auth_status()
        third = auth_mod.get_work4you_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_work4you_auth_status was called {call_count['n']}× — "
        "cache is not deduplicating within TTL."
    )
    # Each call returns a copy so callers can't mutate the cached dict.
    assert first == second == third
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_work4you_auth_status()

    auth_mod.invalidate_work4you_auth_status_cache()


def test_get_work4you_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too — that's where the cost was.

    Teknium's case: ~31 cache misses per `work4you tools` "All Platforms"
    ...
    with patch.object(auth_mod, "_compute_work4you_auth_status", side_effect=fake_compute):
        for _ in range(10):
            auth_mod.get_work4you_auth_status()
