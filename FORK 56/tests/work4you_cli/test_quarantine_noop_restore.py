...
``work4you*.exe`` shims aside (``work4you.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``work4you`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("work4you", "work4you", "work4you-acp", "work4you-gateway"):
...
# ---------------------------------------------------------------------------
# work4you_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "work4you.exe" in names, "work4you.exe must be restored after a no-op install"
    assert "work4you-acp.exe" in names
    assert "work4you-gateway.exe" in names
    assert (scripts / "work4you.exe").read_bytes() == b"MZ-old-work4you"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "work4you.exe").read_bytes() == b"MZ-new-work4you"
...
# ---------------------------------------------------------------------------
# work4you_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "work4you.exe" in names, "work4you.exe must be restored after a no-op recovery install"
    assert (scripts / "work4you.exe").read_bytes() == b"MZ-old-work4you"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "work4you.exe").write_bytes(b"MZ-new-work4you")
