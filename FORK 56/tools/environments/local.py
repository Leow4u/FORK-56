...
from work4you_constants import get_process_work4you_home
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Work4You was already launched from (for example ``work4you``
    while the process cwd is ``~/.work4you/work4you``), passing it through
    unchanged makes the wrapper run ``cd work4you`` *inside* the project
    ...
    # Common recovery for config values like ``work4you`` when Work4You was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./work4you``; use the current directory instead.
...
# Work4You-internal env vars that should NOT leak into terminal subprocesses.
_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX = "_WORK4YOU_FORCE_"

# Work4You-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Work4You inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    blocked.update({
        ...
        "WORK4YOU_DASHBOARD_SESSION_TOKEN",
    ...
    # Work4You-managed inference credential — Claude subscription auth is not a
    # working Work4You provider path.  Stripping it broke agent-spawned
...
_WORK4YOU_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Work4You venv path instead of the project's own
# ``.venv`` — silently clobbering the Work4You environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Work4You venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Work4You venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Work4You itself treats PYTHONHOME as contamination in its own
...
# _strip_work4you_owned_pythonpath() which removes only Work4You-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_work4you_internal_secret(key: str) -> bool:
    """Return True for Work4You-internal secrets injected under *dynamic* names.

    ``_WORK4YOU_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Work4You-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`work4you_subprocess_env` helper all call it, so the
...
def _inject_context_work4you_home(env: dict) -> None:
    """Bridge the context-local Work4You home override into subprocess env."""
    try:
        from work4you_constants import get_work4you_home_override

        value = get_work4you_home_override()
        if value:
            env["WORK4YOU_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (WORK4YOU_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Work4You-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_work4you_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _WORK4YOU_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_work4you_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_work4you_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _WORK4YOU_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_work4you_home(sanitized)
    ...
    # the separate Work4You runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_work4you_owned_pythonpath_and_runtime_markers(sanitized)
...
# legitimate child Work4You spawns needs them, and they are the highest-value
...
# narrow subset of _WORK4YOU_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in work4you_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_work4you_internal_secret, but _ID has no secret suffix, so it must be
...
def work4you_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_WORK4YOU_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Work4You spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_WORK4YOU_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Work4You-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_work4you_internal_secret`.
    for key in list(env):
        if key.startswith(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_work4you_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _WORK4YOU_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_work4you_home(env)
    from work4you_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_work4you_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose WORK4YOU_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's WORK4YOU_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`work4you_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``WORK4YOU_HOME`` / subprocess ``HOME`` contract) and the
    Work4You secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_work4you_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``WORK4YOU_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Work4You home override into ``WORK4YOU_HOME`` and apply the
      subprocess HOME contract (``work4you_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``WORK4YOU_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs WORK4YOU_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("WORK4YOU_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale WORK4YOU_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\work4you\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\work4you\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _work4you_portable_git = os.path.join(_local_appdata, "work4you", "git") if _local_appdata else ""
    if _work4you_portable_git:
        for candidate in (
            os.path.join(_work4you_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_work4you_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # WORK4YOU_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\work4you\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "WORK4YOU_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Work4You. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``work4you`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_WORK4YOU_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_work4you_bin_dir() -> str | None:
    """Return the directory holding the ``work4you`` console-script, or None.
    ...
    work4you install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``work4you``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``work4you`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``work4you`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("work4you")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``work4you`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _WORK4YOU_BIN_DIR
    if _WORK4YOU_BIN_DIR is not _SENTINEL:
        return _WORK4YOU_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("work4you")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "work4you" or base.startswith("work4you."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "work4you.exe" if _IS_WINDOWS else "work4you"
    ...
    _WORK4YOU_BIN_DIR = candidate
    return candidate


def _prepend_work4you_bin_dir(existing_path: str) -> str:
    """Prepend the work4you install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_work4you_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Work4You-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Work4You installs for
    itself, so on a machine where Work4You provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$WORK4YOU_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$WORK4YOU_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_work4you_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_work4you_managed_node``, a first browser install).
    """
    try:
        from work4you_constants import get_work4you_home, iter_work4you_node_dirs

        candidates = [*iter_work4you_node_dirs(), get_work4you_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Work4You-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Work4You
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_WORK4YOU_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_work4you_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_work4you_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _WORK4YOU_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the work4you install dir is reachable so plugins can shell out
        # to bare ``work4you`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_work4you_bin_dir(new_path)

    _inject_context_work4you_home(run_env)
    ...
    _strip_work4you_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_work4you_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by Work4You launchers.

    ``gateway_windows._preserve_work4you_home_path`` maps a physical path under
    the resolved WORK4YOU_HOME back onto the configured WORK4YOU_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of WORK4YOU_HOME as Work4You-owned.
    ...
    # lexically the same way get_default_work4you_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of WORK4YOU_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\work4you\work4you
    # -> C:\...\work4you) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Work4You-owned.  Fail-closed: a
...
# --- Work4You venv / repo-root detection (module-level, computed once) ---

#: The Work4You repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import work4you_cli``,
#: etc.  Subprocesses that are NOT the Work4You backend don't need it and it
#: can shadow local packages.
_work4you_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Work4You launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Work4You-owned paths under
#: the configured WORK4YOU_HOME spelling (which may be a junction to another
#: drive — see ``work4you_cli/gateway_windows.py::_preserve_work4you_home_path``).
...
_work4you_repo_root_aliases: tuple[Path, ...] = _build_work4you_repo_root_aliases(
    _work4you_repo_root,
...
#: subprocess spawn.
_work4you_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Work4You repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _work4you_repo_root_aliases):
        return None
...
def _get_work4you_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Work4You runtime.
    ...
    """
    global _work4you_site_packages
    if _work4you_site_packages is not None:
        result = list(_work4you_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _work4you_site_packages = list(result)
...
def _strip_work4you_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Work4You-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_work4you_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_work4you_owned_pythonpath(env: dict) -> None:
    """Remove Work4You-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Work4You repo root and the Work4You venv's
    ...
    user entries, so only entries proven Work4You-owned are removed:
    ...
    work4you_site_packages = _get_work4you_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Work4You-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Work4You venv site-packages ---
        ...
        for sp in work4you_site_packages:
        ...
        # resolved and unresolved (WORK4YOU_HOME/junction) spellings count as
        # Work4You-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _work4you_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped Work4You-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Work4You trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``WORK4YOU_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under WORK4YOU_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_work4you_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "work4you_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._work4you_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_work4you_pgid", None)
                    if pgid is None:
                        raise
