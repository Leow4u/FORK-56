 ...
 * and the Firecrawl gateway.
 *
 * fal_client sends the Portal access token as `Authorization: Key <token>`.
 * Bearer is also accepted (static keys / curl). Default login scope is
 * inference:invoke; device step-up also requests tool:invoke. Either
 * scope is enough here so an existing Portal login already works.
 */
...
/**
 * Paid Portal access always covers FAL image gen. Free tool-pool coverage
 * is the JWT `tool_access.coverage.fal` flag (same map as
...
export function isFalCovered(
  ...
  if (rec.enabled === true) {
    ...
    if (coverage && typeof coverage === 'object') {
      const fal = (coverage as Record<string, unknown>).fal
      if (fal === false) return false
    }
    return true
  }
...
export function extractInvokeToken(
  authorization: string | undefined,
): string | null {
  if (!authorization) return null
  const v = authorization.trim()
  const lower = v.toLowerCase()
  if (lower.startsWith('bearer ')) return v.slice('Bearer '.length).trim()
  if (lower.startsWith('key ')) return v.slice('Key '.length).trim()
  return null
}
...
export async function verifyInvokeBearer(
  ...
  if (!isFalCovered(payload, paidAccess)) {
    throw Object.assign(new Error('fal_not_covered'), { status: 403 })
  }
