...
describe('DesktopInstallOverlay first-run setup', () => {
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    expect(screen.queryByText('Local installation could not start. Restart Work4You Desktop and try again.')).toBeTruthy()
  })
  ...
  it('requires a successful token connection test before applying remote config', async () => {
    ...
    desktop.probeConnectionConfig.mockResolvedValue({
      authMode: 'token',
      baseUrl: 'https://gateway.example.com/work4you',
      error: null,
      providers: [],
      reachable: true,
      version: '0.17.0'
    ...
    await waitFor(() => {
      expect(desktop.testConnectionConfig).toHaveBeenCalledWith({
        ...
        remoteUrl: 'https://gateway.example.com/work4you'
      })
    })

    await screen.findByText('Connected to https://gateway.example.com/work4you (0.17.0).')
  ...
  it('does not enable Apply when credentials change during a connection test', async () => {
    ...
    await act(async () => {
      resolveTest?.({ baseUrl: 'https://gateway.example.com/work4you', ok: true, version: '0.17.0' })
      await pendingTest
    })

    expect(screen.queryByText('Connected to https://gateway.example.com/work4you (0.17.0).')).toBeNull()
    expect(apply.disabled).toBe(true)
  })
  ...
  it('signs in, tests, and applies a password-style remote gateway', async () => {
    ...
    await waitFor(() => {
      expect(desktop.oauthLoginConnectionConfig).toHaveBeenCalledWith('https://gateway.example.com/work4you')
    })
    ...
    await screen.findByText('Connected to https://gateway.example.com/work4you.')
    const apply = screen.getByText('Apply and reconnect').closest('button') as HTMLButtonElement
    expect(apply.disabled).toBe(false)
    fireEvent.click(apply)
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('https://gateway.example.com/work4you'), {
      target: { value: 'https://gateway.example.com/work4you' }
    })
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('Work4You needs a one-time install')).toBeNull()
  })
})
