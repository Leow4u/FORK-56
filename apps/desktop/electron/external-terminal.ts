// Launching the Work4You TUI in the user's OWN terminal emulator.
//
...
// `work4you --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Work4You runtime is often a venv Python invoked
//     as `python -m work4you_cli.main`, not a `work4you` on PATH — so the command
...
export function terminalScriptEnv(
  ...
  if (work4youHome) {
    out.WORK4YOU_HOME = work4youHome
  }
