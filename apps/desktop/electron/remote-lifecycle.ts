 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Work4You dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Work4You install on the remote (login-shell probe),
 ...
 *     `WORK4YOU_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForWork4You in.
 *
 * The minted WORK4YOU_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^WORK4YOU_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.work4you/desktop-ssh'
...
// Resolve the remote work4you executable. An EXPLICIT path is honored strictly
...
async function locateWork4You(ssh, remoteWork4YouPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The work4you binary or wrapper script
    ...
    //     the Work4You version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/Leow4u/FORK-56/issues/74411
    return candidate
  }
  ...
  if (remoteWork4YouPath) {
    if (await isExecutable(remoteWork4YouPath)) {
      return resolveLauncher(remoteWork4YouPath)
    }

    const err: any = new Error(
      `The Work4You path you set is not an executable on the remote host: "${remoteWork4YouPath}". ` +
        'Check the path (it must be the full path to the `work4you` binary on the remote, e.g. ' +
        '~/work4you/.venv/bin/work4you), or clear it to auto-detect.'
    )

    err.kind = 'work4you-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v work4you')}`)).trim()
  ...
  candidates.push('~/.local/bin/work4you')
  candidates.push('/usr/local/bin/work4you')
  candidates.push('~/.work4you/work4you/venv/bin/work4you')
  ...
  const err: any = new Error(
    'Work4You is not installed on the remote host (could not find a `work4you` executable). ' +
      'Install it on the remote with:  curl -fsSL https://work4you.ai/install.sh | sh  ' +
      '— or set the Work4You path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<work4you> --version`,
// e.g. "Work4You v0.18.2 ..."), or '' on failure. Surfaces WHICH work4you a
// connection uses, so a stale/unexpected install is visible.
async function probeWork4YouVersion(ssh, work4youPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(work4youPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Work4You Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The WORK4YOU_HOME the remote dashboard will use (explicit env wins, else
// ~/.work4you). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteWork4YouHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${WORK4YOU_HOME:-$HOME/.work4you}"')).trim().split('\n').pop()

    return out || '~/.work4you'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Work4You home.')
...
async function listRemoteWork4YouProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteWork4YouHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Work4You profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Work4You home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'work4youPath', 'work4youHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !work4youPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(work4youPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/work4you`, leaving neither
      // launcher nor WORK4YOU_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `work4you_home=os.path.expanduser(${shq(work4youHome)}) if ${shq(work4youHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if work4you_home:\n' +
      ' expected_entries.add(os.path.join(work4you_home,"work4you","venv","bin","work4you"))\n' +
...
function buildSpawnCommand(work4youPath, profile, opts: any = {}) {
  const work4you = expandRemotePath(work4youPath)
  ...
  const dashCmd =
    ...
    `exec env WORK4YOU_DESKTOP=1 ${work4you} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, work4youPath) {
  const work4you = expandRemotePath(work4youPath)

  const out = await ssh.exec(
    `help="$(${work4you} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { work4youPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, work4youPath))) {
    const err: any = new Error(
      'The remote Work4You install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Work4You on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(work4youPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForWork4You, a token-gated
...
async function connect(deps) {
  ...
  const work4youPath = await locateWork4You(ssh, remoteWork4YouPath)
  log(`located work4you at ${work4youPath}`)
  const work4youVersion = await probeWork4YouVersion(ssh, work4youPath)

  if (work4youVersion) {
    log(`remote work4you version: ${work4youVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const work4youHome = await probeRemoteWork4YouHome(ssh)
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.work4youPath === work4youPath &&
      lock.work4youHome === work4youHome
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForWork4You(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
...
export {
  ...
  pidIsOurDashboard,
  probeRemotePlatform,
  probeRemoteWork4YouHome,
  probeWork4YouVersion,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
  REMOTE_LOCK_DIR,
