"""OpenRouter-compatible image generation backend (OpenRouter + Work4You Portal).

Both OpenRouter and the Work4You Portal inference endpoint speak the same
...
Work4You Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~work4you_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the Work4You OAuth device-code
...
The Image API is OpenRouter-only: Work4You Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for Work4You.
"""
...
# image_gen.model from ``work4you tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Work4You Portal). The two differ only
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: Work4You Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``work4you tools``. Work4You
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``work4you tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `work4you tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Work4You Portal).
            "HTTP-Referer": "https://github.com/Leow4u/FORK-56",
            "X-Title": "Work4You",
        }
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="work4you",
            display_name="Work4You Portal",
            runtime_name="work4you",
            config_key="work4you",
            model_env_var="WORK4YOU_IMAGE_MODEL",
            setup_schema={
                "name": "Work4You Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Work4You Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Work4You Portal image gen providers."""
