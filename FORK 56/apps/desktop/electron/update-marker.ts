...
export function markerPath(work4youHome) {
  return path.join(work4youHome, '.work4you-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(work4youHome)
  let raw
 ...
 *
 * The Tauri-based work4you-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(work4youHome, pid, { now = Date.now } = {}) {
  const file = markerPath(work4youHome)
 ...
 * Work4You to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(work4youHome, opts)
