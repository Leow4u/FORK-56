...
of fields, and Work4You schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Work4You asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.work4you.blueprint` block in its
