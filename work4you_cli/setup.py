...
from tools.tool_backend_helpers import managed_work4you_tools_enabled
...
_DOCS_BASE = "https://work4you.ai/docs"
...
# display_work4you_home imported lazily at call sites (stale-module safety during work4you update)
...
from work4you_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from work4you_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``WORK4YOU_NONINTERACTIVE=1`` (see ``work4you_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("WORK4YOU_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``WORK4YOU_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, work4you_home):
    ...
    try:
        from work4you_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Work4You cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    work4you model            (pick any provider/model)")
        print_info("    work4you setup --portal   (Work4You Portal OAuth, no API key)")
    ...
    tool_status = []
    subscription_features = get_work4you_subscription_features(config)
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Work4You subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Work4You Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Work4You), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Work4You subscription)", True, None))
    ...
    # Video generation — opt-in via `work4you tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via Work4You subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from work4you_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Work4You subscription)", True, None))
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        tool_status.append(("Speech-to-Text (OpenAI via Work4You subscription)", True, None))
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'work4you tools' → Speech-to-Text")
            )

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Work4You subscription)", True, None))
    ...
    elif managed_work4you_tools_enabled() and subscription_features.work4you_auth_present:
        tool_status.append(("Modal Execution (optional via Work4You subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via work4you auth spotify — check auth.json, not env vars)
    try:
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {work4you_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('work4you', Colors.GREEN)}              Start chatting")
...
# between `work4you tools` and `work4you setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``work4you model``)
    ...
    provider added to ``work4you model`` is automatically available here.
    ...
    # Delegate to the shared work4you model flow — handles provider picker,
    ...
    # on demand via `work4you auth add`, `work4you setup` vision, and
    # `work4you setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from work4you_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``work4you model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_work4you_tools_enabled() and subscription_features.work4you_auth_present:
        choices.append("Work4You Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("work4you-openai")
    choices.extend(
        [
    ...
    selected_via_nous = selected == "work4you-openai"
    if selected == "work4you-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Work4You gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.work4you/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Work4You auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'work4you setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `work4you setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `work4you setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `work4you egress setup` then `work4you egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `work4you egress setup`."
            )
        ...
        managed_modal_available = bool(
            managed_work4you_tools_enabled()
            and
            get_work4you_subscription_features(config).work4you_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Work4You gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'work4you[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Work4You installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'work4you[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into WORK4YOU_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("WORK4YOU_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # WORK4YOU_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.work4you/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from work4you_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_work4you_home(Path(get_work4you_home()))
    except Exception:
        pass
...
def _profile_name_from_work4you_home(work4you_home) -> str | None:
    """Return the active profile name when WORK4YOU_HOME is a profile dir."""
    if work4you_home.parent.name == "profiles":
        return work4you_home.name
    return None
...
def _setup_qqbot():
    ...
    from work4you_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://work4you.ai/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://work4you.ai/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from work4you_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    # via `work4you import` or `work4you setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `work4you setup tools` and `work4you tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Work4You profile and are not uploaded.")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from work4you_cli.config
    ...
    if section_key == "model":
        ...
        from work4you_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_work4you.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_work4you migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_work4you", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Work4You.
...
def _offer_openclaw_migration(work4you_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=work4you_home.resolve(),
    ...
    # Execute the migration — overwrite=False so existing Work4You configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Work4You config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Work4You Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``work4you setup --portal`` and ``work4you portal``. This is the
    Work4You-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Work4You session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``work4you setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``work4you model`` runs
    when you pick Work4You. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``work4you portal`` always offers a model picker,
    and there is a single source of truth for the Work4You onboarding steps.
    """
    ...
    print(color("│     ⚕ Work4You Setup — Work4You Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Work4You Portal sub.")
    print()
    print_info("  Sign up: https://portal.work4you.ai/manage-subscription")
    print()
    ...
    # Work4You model picker), then offers the Tool Gateway opt-in and sets
    # provider=work4you via the login/model save. This is the same routine quick
    # setup calls, so `work4you portal` == quick setup's Work4You step.
    try:
        ...
        logger.debug("_model_flow_nous error during `work4you portal`: %s", exc)
        print()
        print_error(f"  Work4You Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `work4you portal`.")
        return
    ...
    print_info("  Run `work4you` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      work4you setup           — full or quick (auto-detected)
      work4you setup model     — just model/provider
      work4you setup tts       — just text-to-speech
      work4you setup terminal  — just terminal backend
      work4you setup gateway   — just messaging platforms
      work4you setup tools     — just tool configuration
      work4you setup telemetry — just local shared metrics
      work4you setup agent     — just agent settings
    """
    from work4you_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Work4You Portal setup. Skips the rest of the wizard.
    ...
    print(
        color(
            "│             ⚕ Work4You Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Work4You installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, work4you_home)
            return
    ...
    print_info(f"Data folder:  {work4you_home}")
    ...
    # Tune later with `work4you setup agent`.
    if not is_existing:
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from work4you_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    _print_setup_summary(config, work4you_home)


def _run_first_time_quick_setup(config: dict, work4you_home, is_existing: bool):
    """Streamlined first-time setup via Work4You Portal: OAuth, model, terminal & messaging.

    Routes straight to the Work4You Portal provider — runs the device-code OAuth
    login, picks a Work4You model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``work4you setup <section>``
    or switch providers with ``work4you model``.
    """
    from work4you_cli.config import load_config

    # Step 1: Work4You Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Work4You model picker). Provider is set to "work4you" by the login/model save.
    print()
    print_header("Work4You Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.work4you.ai/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Work4You Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Work4You Portal setup encountered an error: {exc}")
        print_info("You can try again later with: work4you model")
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `work4you import` from another machine).
        from work4you_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``work4you tools`` (which
    ...
    try:
        ...
        from work4you_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``work4you setup agent`` /
    ``work4you config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, work4you_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `work4you update` runs don't re-inject them.
        try:
    ...
    _blank_slate_walkthrough(config, work4you_home)


def _blank_slate_walkthrough(config: dict, work4you_home):
...
def _run_quick_setup(config: dict, work4you_home):
