...
The bootstrap module is imported at the top of every Work4You entry point
(work4you, work4you, work4you-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Work4You entry point imports work4you_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Work4You CLI
...
class TestEntryPointsImportBootstrap:
    """Every Work4You entry point must import work4you_bootstrap as its
    ...
    # Entry points that invoke Work4You as a process.  Each one must
    # import work4you_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "work4you_cli/main.py",   # work4you CLI (console_script)
        "run_agent.py",          # work4you (console_script)
        "acp_adapter/entry.py",  # work4you-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``work4you update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``work4you_bootstrap`` as a top-level module) leaves work4you
        ...
        """
        # Resolve relative to the work4you repo root.  Tests live
        # at tests/test_work4you_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # work4you start even when work4you_bootstrap hasn't been
...
class TestHardenImportPath:
    ...
    directory from shadowing Work4You's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("WORK4YOU_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["WORK4YOU_PYTHON_SRC_ROOT"] = env
            elif "WORK4YOU_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["WORK4YOU_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/work4you")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WORK4YOU_PYTHON_SRC_ROOT", None)
            else:
                os.environ["WORK4YOU_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/work4you", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Work4You root.  The guard must relocate Work4You to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/work4you"])
        assert result[0] == "/opt/work4you"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Work4You root.
        assert result.index("/opt/work4you") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["WORK4YOU_PYTHON_SRC_ROOT"] = "/env/work4you"
            hb.harden_import_path()
            assert sys.path[0] == "/env/work4you"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WORK4YOU_PYTHON_SRC_ROOT", None)
