...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Work4You' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Work4You")

The wake word turns Work4You into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Work4You listens in the background for a spoken
trigger phrase. Say it, and Work4You starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Work4You backend (for example a
...
Work4You supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey work4you"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey work4you"** — a model for it ships with Work4You, so
...
```bash
cd ~/.work4you/work4you && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `work4you` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.work4you/config.yaml`,
...
wake_word:
  ...
  phrase: "hey work4you"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_work4you         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey work4you" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey work4you" utterances are missed.
...
default) to let Work4You choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Work4You installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Work4You surfaces, and `surface` picks which one
...
| `cli` | Only the classic `work4you` CLI. |
| `tui` | Only `work4you --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Work4You surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Work4You does not silently fail over to another open surface.
...
"Hey Work4You" works out of the box — the bundled openWakeWord model
(`model: hey_work4you`) is the default. To wake on something else, the easiest
...
command (`work4you -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.work4you/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("work4you" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Work4You" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey work4you"
  porcupine:
    keyword: ~/.work4you/wakewords/hey_work4you.ppn
```

Set your access key in `~/.work4you/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `work4you tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `work4you[wake]`).
...
but the phrase never fires. Work4You detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Work4You
backend (it may appear as your terminal, `python`, or Work4You), then toggle the
...
```bash
work4you config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
work4you config set wake_word.input_device null
```
