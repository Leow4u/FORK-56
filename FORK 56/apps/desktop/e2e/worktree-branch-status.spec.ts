...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Work4You E2E'], { cwd: repo })
...
function configureRepoCwd(work4youHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(work4youHome, mockUrl)
  fs.appendFileSync(path.join(work4youHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(work4youHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.work4youHome, mock.url, repo)
