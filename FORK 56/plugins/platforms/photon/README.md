...
TypeScript-only, Work4You runs it inside a small supervised Node sidecar and
...
`work4you photon setup` does, in order:
...
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.work4you/.env` so the
...
There is no separate `login` command; like every other Work4You channel,
...
Run `work4you photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.work4you/.env` (the same place every other
...
Management metadata lives in `~/.work4you/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `work4you photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Work4You
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Work4You coalesces
...
- **Native polls are supported.** Work4You posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `work4you send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<work4you-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `work4you photon status`, a DM and a group roundtrip,
