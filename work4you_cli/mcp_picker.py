"""MCP picker — interactive `work4you mcp picker` (also the default `work4you mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``work4you mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `work4you plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from work4you_cli.cli_output import prompt_yes_no
from work4you_cli.curses_ui import curses_single_select
from work4you_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from work4you_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `work4you mcp catalog`."""
    if not rows:
        print()
    ...
    # Surface manifest-version warnings so users know when their Work4You is
...
def show_catalog() -> None:
    """`work4you mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`work4you mcp picker` (and default `work4you mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`work4you mcp install <name>` — non-interactive entry-point.
