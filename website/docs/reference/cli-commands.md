...
description: "Authoritative reference for Work4You terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Work4You profile to use for this invocation. Overrides the sticky default set by `work4you profile use`. |
...
| `--ignore-user-config` | Ignore `~/.work4you/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `WORK4YOU_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `work4you chat` | Interactive or one-shot chat with the agent. |
| `work4you model` | Interactively choose the default provider and model. |
| `work4you moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `work4you fallback` | Manage fallback providers tried when the primary model errors. |
| `work4you gateway` | Run or manage the messaging gateway service. |
| `work4you proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `work4you egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `work4you lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `work4you setup` | Interactive setup wizard for all or part of the configuration. |
| `work4you whatsapp` | Configure and pair the WhatsApp bridge. |
| `work4you whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `work4you whatsapp` (Baileys personal-account bridge). |
| `work4you slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `work4you auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Work4You/Anthropic. |
| `work4you login` / `logout` | **Deprecated** — use `work4you auth` instead. |
| `work4you send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `work4you peer` | Register peer Work4You gateways on other machines and DM their agents' canonical Bot Chats (`work4you peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `work4you secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.work4you/.env`. |
| `work4you migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `work4you status` | Show agent, auth, and platform status. |
| `work4you cron` | Inspect and tick the cron scheduler. |
| `work4you kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `work4you project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `work4you webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `work4you hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `work4you doctor` | Diagnose config and dependency issues. |
| `work4you security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `work4you approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `work4you dump` | Copy-pasteable setup summary for support/debugging. |
| `work4you prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `work4you debug` | Debug tools — upload logs and system info for support. |
| `work4you backup` | Back up Work4You home directory to a zip file. |
| `work4you checkpoints` | Inspect / prune / clear `~/.work4you/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `work4you import` | Restore a Work4You backup from a zip file. |
| `work4you logs` | View, tail, and filter agent/gateway/error log files. |
| `work4you config` | Show, edit, migrate, and query configuration files. |
| `work4you skin` | List, switch, and tweak display skins. |
| `work4you console` | Open the safe Work4You command console. |
| `work4you pairing` | Approve or revoke messaging pairing codes. |
| `work4you skills` | Browse, install, publish, audit, and configure skills. |
| `work4you bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `work4you curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `work4you journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `work4you memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `work4you honcho`) register automatically when their provider is active. |
| `work4you acp` | Run Work4You as an ACP server for editor integration. |
| `work4you mcp` | Manage MCP server configurations and run Work4You as an MCP server. |
| `work4you plugins` | Manage Work4You plugins (install, enable, disable, remove). |
| `work4you portal` | Work4You Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `work4you tools` | Configure enabled tools per platform. |
| `work4you computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `work4you pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `work4you sessions` | Browse, export, prune, rename, and delete sessions. |
| `work4you insights` | Show token/cost/activity analytics. |
| `work4you claw` | OpenClaw migration helpers. |
| `work4you import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `work4you dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `work4you serve` | Start the Work4You backend server (headless; powers the desktop app and remote backends). |
| `work4you desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `work4you profile` | Manage profiles — multiple isolated Work4You instances. |
| `work4you completion` | Print shell completion scripts (bash/zsh/fish). |
| `work4you version` | Show version information. |
| `work4you update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `WORK4YOU_HOME` snapshot. |
| `work4you uninstall` | Remove Work4You from the system. |
...
| `--ignore-user-config` | Ignore `~/.work4you/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Work4You itself. |
...
### `work4you -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `work4you -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.
...
Per-run overrides (no mutation to `~/.work4you/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `WORK4YOU_INFERENCE_MODEL` | Override the model for this run |
...
Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `work4you chat -q` instead; `-z` is explicitly for "I only want the final answer".

#### `--usage-file` — JSON usage report for pipelines

`work4you -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Work4You chat session.
...
- log into OAuth-backed providers (Anthropic, Copilot, Codex, Work4You Portal)
- enter or update API keys
- pick from provider-specific model lists
- configure a custom/self-hosted endpoint
- save the new default into config

:::warning work4you model vs /model — know the difference
**`work4you model`** (run from your terminal, outside any Work4You session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Work4You chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Work4You session first (`Ctrl+C` or `/quit`), then run `work4you model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `work4you model` from the terminal.
:::
...
| `migrate-legacy` | Remove legacy `work4you.service` units left over from pre-rename installs. Profile units (`work4you-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Work4You Relay](/user-guide/messaging/relay). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `WORK4YOU_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `work4you update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `WORK4YOU_GATEWAY_NO_SUPERVISE=1`. |
...
`work4you gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `work4you gateway run` instead of `work4you gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s work4you 'work4you gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-work4you-gateway-start-fails) for details.
:::
...
**Easiest path:** `work4you setup --portal` — OAuth into Work4You Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `work4you setup` on an existing install now does this by default. |
| `--portal` | One-shot Work4You Portal setup: log in via OAuth, set Work4You as the inference provider, and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md). Skips the rest of the wizard. |
...
Inspect Work4You Portal auth, Tool Gateway routing, and reach the subscription page. Subcommand-less invocation runs `status`.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Portal auth state + per-tool Tool Gateway routing summary. Also shown when no subcommand is given. |
| `open` | Open `portal.work4you.ai/manage-subscription` in your default browser. |
| `tools` | List every Tool Gateway partner (Firecrawl, FAL, OpenAI TTS, Browser Use, Modal) and which are routed via Work4You. |

For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `work4you setup --portal` above.
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$WORK4YOU_HOME/slack-manifest.json`. |
| `--name NAME` | `Work4You` | Bot display name in Slack. |
...
Run `work4you slack manifest --write` again after `work4you update` to pick
up any new commands.
...
Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.work4you/.env` + `~/.work4you/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `work4you send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `work4you send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
Bot-to-bot DMs across machines. Register another Work4You gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`work4you peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`work4you -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `WORK4YOU_PEER_<NAME>_KEY` in `~/.work4you/.env`. |
...
includes the peer roster and the `work4you peer dm` pattern, so agents discover
...
Pull API keys from an external secret manager at process startup instead of storing them in `~/.work4you/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `work4you claw migrate` (one-shot import of OpenClaw configuration into Work4You) — `work4you migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Work4You venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.work4you/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Work4You Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `work4you login` / `work4you logout` *(Deprecated)*

:::caution
`work4you login` has been removed. Use `work4you auth` to manage OAuth credentials, `work4you model` to select a provider, or `work4you setup` for full interactive setup.
:::
...
`$WORK4YOU_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.work4you/kanban.db` for back-compat; additional boards live at `~/.work4you/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `work4you kanban boards switch`, the `WORK4YOU_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `work4you kanban`. Workers have `WORK4YOU_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.work4you/kanban/current`). |
...
Board resolution order (highest precedence first): `--board <slug>` flag → `WORK4YOU_KANBAN_BOARD` env var → `~/.work4you/kanban/current` file → `default`.

All actions are also available as a slash command in the gateway (`/kanban …`), with the same argument surface — including `boards` subcommands and the `--board` flag.

For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/work4you-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/user-guide/features/kanban).
...
| `--script` | Filter/transform script under `~/.work4you/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |

Subscriptions persist to `~/.work4you/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
Outputs a compact, plain-text summary of your entire Work4You setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Work4You version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, WORK4YOU_HOME path |
...
:::tip
`work4you dump` is specifically designed for sharing. For interactive diagnostics, use `work4you doctor`. For a visual overview, use `work4you status`.
:::
...
| `--work4you` | Upload to Work4You-internal diagnostics storage instead of a public paste service. Use this when Work4You support asks for a private diagnostic bundle. |
| `--local` | Print the report locally instead of uploading. |
| `--no-redact` | Disable upload-time secret redaction. By default, uploads are redacted. |

The report includes system info (OS, Python version, Work4You version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.

Default uploads use public paste services tried in order: paste.rs, dpaste.com. `--work4you` uploads the same debug bundle to private Work4You diagnostics storage instead; the returned viewer link is for the Work4You team and auto-deletes after 14 days.
...
Create a zip archive of your Work4You configuration, skills, sessions, and data. The backup excludes the work4you codebase itself.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/work4you-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Work4You is running (WAL-mode safe).

**What's excluded from the zip:**

- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite's WAL / shared-memory / journal sidecars. The `*.db` file already got a consistent snapshot via `sqlite3.backup()`; shipping the live sidecars alongside it would let a restore see a half-committed state.
- `checkpoints/` — per-session trajectory caches. Hash-keyed and regenerated per session; wouldn't port cleanly to another install anyway.
- The `work4you` code itself (this is a user-data backup, not a repo snapshot).
...
Inspect and manage the shadow git store at `~/.work4you/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `work4you checkpoints` is equivalent. |
...
Restore a previously created Work4You backup into your Work4You home directory. All files in the archive overwrite existing files in your Work4You home; `--force` only skips the confirmation prompt that fires when the target already has a Work4You installation.
...
View, tail, and filter Work4You log files. All logs are stored in `~/.work4you/logs/` (or `<profile>/logs/` for non-default profiles).
...
Work4You uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `work4you logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how Work4You layers
...
enabled. To shrink the prompt, disable unused toolsets (`work4you tools`) or
uninstall skills you don't need (`work4you skills`). Context files (AGENTS.md,
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `work4you config get model.default`). `--json` emits machine-readable output. |
...
- `--source well-known` lets you point Work4You at a site exposing `/.well-known/skills/index.json`.
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.work4you/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.work4you/skill-bundles/` and report added/removed bundles |
...
| `backup` | Take a manual tar.gz snapshot of `~/.work4you/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.work4you/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `work4you update`. Use `work4you curator run --dry-run` to preview before that happens.
...
`work4you moa configure` reuses Work4You' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
| `add` | Pick a provider + model (same picker as `work4you model`) and append to the chain |
...
Inspect shell-script hooks declared in `~/.work4you/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.work4you/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `work4you <provider>` command for provider-specific management (e.g. `work4you honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `work4you --help` to see what's currently wired in.
:::
...
Starts Work4You as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Work4You as an MCP server.

| Subcommand | Description |
|------------|-------------|
| *(none)* or `picker` | Interactive catalog picker — browse Work4You-approved MCPs and install/enable/disable. |
| `catalog` | List Work4You-approved MCPs (plain text, scriptable). |
| `install <name>` | Install a catalog entry (e.g. `work4you mcp install n8n`). |
| `serve [-v\|--verbose]` | Run Work4You as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Work4You](../guides/use-mcp-with-work4you.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-work4you-as-an-mcp-server).
...
Unified plugin management — general plugins, memory providers, and context engines in one place. Running `work4you plugins` with no subcommand opens a composite interactive screen with two sections:
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Work4You Plugin](../developer-guide/plugins/index.md).
...
`work4you computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`work4you tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Work4You checks its version and runtime
...
installer. Work4You never replaces a custom binary selected through
`WORK4YOU_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`work4you computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Work4You integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Work4You and links Cua's
skill pack into the Work4You skills directory automatically.

Permission mode, capability-manifest approval, and the existing-profile grant
belong to runtime launch. In bounded mode Work4You passes Cua's canonical
...
`work4you update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Work4You update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Work4You shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to Work4You. Reads from `~/.openclaw` (or a custom path) and writes to `~/.work4you`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Work4You files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.work4you/` (by default a single restore-point archive is written to `~/.work4you/backups/pre-migration-*.zip` before apply; restorable with `work4you import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Work4You equivalents or **archived** for manual review.
...
Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Work4You. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.work4you/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Start the Work4You **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `work4you dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `work4you serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `work4you dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`work4you serve`](#work4you-serve) above.) Requires `cd ~/.work4you/work4you && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.work4you/work4you && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `work4you dashboard` processes and exit. |
| `--status` | — | List running `work4you dashboard` processes and exit. |

### `work4you dashboard register`

Register this install as a self-hosted dashboard with your Work4You Portal account. Creates an OAuth client, writes `WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID` into `~/.work4you/.env`, and prints how to engage the login gate. Requires being logged in (`work4you setup`).

| Option | Description |
|--------|-------------|
| `--name` | Human-readable label for the dashboard (default: auto-generated). |
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://work4you.example.com/auth/callback`). Omit for localhost-only use. |
| `--portal-url` | Override the Work4You Portal base URL for registration (default: the portal you logged into). Also settable via `WORK4YOU_DASHBOARD_PORTAL_URL`. |
...
Manage profiles — multiple isolated Work4You instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Work4You commands, subcommands, and profile names.
...
Pulls the latest `work4you` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`work4you update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Work4You may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `WORK4YOU_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `work4you config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Work4You attempts to restart all running gateway profiles automatically so they pick up the new code. Use `work4you gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, Work4You makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `work4you update`.
- **Pairing data snapshot.** Even when `--backup` is off, `work4you update` takes a lightweight snapshot of `~/.work4you/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `work4you backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `work4you.service` warning.** If Work4You detects a pre-rename `work4you.service` systemd unit (instead of the current `work4you-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `work4you version` | Print version information. |
| `work4you update` | Pull latest changes and reinstall dependencies. |

| `work4you uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Work4You, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
