"""Dashboard process-hygiene helpers — extracted from ``work4you_cli/main.py``.
...
``_detect_concurrent_work4you_instances``) are lifted verbatim. References to
helpers that STAY in ``work4you_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``work4you_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``work4you_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``work4you_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``work4you-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Work4You Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``work4you serve`` as
    ...
    environment variable ``WORK4YOU_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`work4you serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "work4you.*dashboard"`) keeps us consistent
            # with `work4you_cli.gateway._scan_gateway_pids` and avoids the
...
def _work4you_home_for_pid(pid: int) -> str | None:
    """Best-effort ``WORK4YOU_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("WORK4YOU_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"WORK4YOU_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Work4You Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``work4you update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], work4you_home: str | None = None
) -> str:
    """Stable owner key: ``WORK4YOU_HOME`` when known, else ``--profile`` / ``-p``.

    ``WORK4YOU_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.work4you`` roots) keep a resolved ``home:`` key so
    ...
    if work4you_home:
        try:
            home_path = Path(work4you_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(work4you_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(work4you_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``work4you update``.

    Each candidate is ``(pid, argv, work4you_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``WORK4YOU_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``WORK4YOU_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``work4you update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, work4you_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, work4you_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``work4you dashboard`` / ``work4you serve`` processes.
    ...
    When ``restart_managed`` is true (the ``work4you update`` path), a detected
    ``work4you-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``work4you-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``work4you update``'s systemd
    fleet-restart loop, which restarts ``work4you-serve*`` units before this
    ...
    # When the Work4You Desktop Electron app spawns this dashboard as a
    # backend child, it sets WORK4YOU_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("WORK4YOU_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `work4you update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot WORK4YOU_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _work4you_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. work4you update's
    ...
    #    remote backend (work4you serve) under Restart=on-failure never comes
...
def _detect_concurrent_work4you_instances(
    ...
    The Work4You Desktop Electron app spawns ``work4you.EXE`` as a backend child,
    ...
    shims (``work4you.exe`` / ``work4you-gateway.exe``) and returns ``(pid,
    ...
    launcher (``work4you.exe``) is a separate process from the Python
    ...
    for shim in _m()._work4you_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated work4you.exe launcher is a separate native process
    ...
    #      work4you.exe sitting *under* a non-Work4You parent (e.g. a Work4You
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        work4you serve --host 127.0.0.1 --port 0
        work4you serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "work4you" not in cmd and "work4you_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (WORK4YOU_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("WORK4YOU_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``work4you serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``work4you serve`` on a Mac
...
def _work4you_home_dir() -> Path:
    """Resolved Work4You home (WORK4YOU_HOME override or ~/.work4you)."""
    override = os.environ.get("WORK4YOU_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "work4youPath", "work4youHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated WORK4YOU_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{work4you_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _work4you_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local work4you serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``work4you serve`` backends with no parent.
    ...
    running under WORK4YOU_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never WORK4YOU_DESKTOP_CHILD_PID entries
