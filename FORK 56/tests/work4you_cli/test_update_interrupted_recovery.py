...
``work4you update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr("work4you_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to work4you_cli._install_repair (shared between
    ...
    import work4you_cli._install_repair as ir
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(m, "_work4you_exe_shims", lambda d: [shim])
    monkeypatch.setattr(
        m,
