...
This page is for adding a **built-in Work4You tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Work4You core, use the plugin route instead:
...
2. **`toolsets.py`** — add tool name to `_WORK4YOU_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `work4you_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `work4you_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `work4you chat -q "Use the weather tool for London"`
