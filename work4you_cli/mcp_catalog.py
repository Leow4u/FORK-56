"""MCP catalog — curated, Work4You-approved MCP servers shipped with the repo.
...
entries via ``work4you mcp catalog`` or the interactive ``work4you mcp picker``,
and install them with ``work4you mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into work4you. Presence in the
  ``optional-mcps/`` directory = Work4You approval. No community tier, no trust
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``work4you mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.work4you/.env`` (the
...
from work4you_constants import get_work4you_home, get_optional_mcps_dir
...
from work4you_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.work4you/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Work4You install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from work4you_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Work4You is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Work4You
        understands. Update Work4You to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_work4you_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.work4you/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.work4you/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from work4you_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from work4you_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``work4you mcp configure <name>``.
    """
    print()
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `work4you mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `work4you auth <provider>` flow. Surface guidance
    ...
    from work4you_cli.mcp_config import _save_mcp_server
