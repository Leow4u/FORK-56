"""Tests for `work4you memory setup [provider]` routing.
...
``work4you memory setup honcho``) without the interactive picker — which matters
because the per-provider ``work4you <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "work4you memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets WORK4YOU_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"WORK4YOU_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("work4you_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("work4you_cli.tools_config.subprocess.run", fake_run):
