"""CLI for the Work4You Kanban board — ``work4you kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/work4you-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence(
    work4you_home: Optional[Path] = None,
) -> tuple[bool, str]:
    """Return ``(running, message)``.

    - ``running=True``: a gateway is alive for this WORK4YOU_HOME and its
    ...
    Used by ``work4you kanban create`` (and callers) to warn when a task
    ...
    ``work4you_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different WORK4YOU_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=work4you_home, use_cache=False
        )
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`work4you kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            ...
            "See https://work4you.ai/docs/user-guide/features/kanban "
            "or docs/work4you-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # WORK4YOU_KANBAN_BOARD env var, then the persisted current-board
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `work4you project list`.")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $WORK4YOU_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `work4you gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.work4you/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``work4you kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: work4you kanban <action> [options]\n"
                "Run 'work4you kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # work4you_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using WORK4YOU_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `work4you kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # WORK4YOU_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `work4you kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("WORK4YOU_PROFILE_NAME", "WORK4YOU_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("WORK4YOU_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (work4you kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``work4you kanban boards <action>``.
    ...
    DB, so a fresh WORK4YOU_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `work4you kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `work4you kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `work4you kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `work4you kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.work4you/profiles/.")
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `work4you -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from work4you_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``work4you kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "work4you kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    work4you gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`work4you kanban list --status ready` / "
                    f"`work4you kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Work4You profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from work4you_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from work4you_cli import kanban_decompose as decomp
