...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("work4you/work4you-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("work4you-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned work4you/* and pr-* branches."""

    def test_prunes_orphaned_work4you_branch(self, git_repo):
        """work4you/work4you-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "work4you/work4you-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "work4you/work4you-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "work4you/work4you-deadbeef" in result.stdout
        ...
        assert "work4you/work4you-deadbeef" in orphaned
        ...
        assert "work4you/work4you-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"work4you/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"work4you pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_unlocked_clean_stale_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-nolock", pid=None)
        cli._prune_stale_worktrees(str(git_repo))
        assert not wt.exists(), "clean unlocked stale worktree should be reaped"

    def test_dirty_survives_over_72h(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "work4you-dirty72", pid=None, dirty=True, age_h=100)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "work4you-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "work4you/work4you-x", "HEAD"],
    ...
    def test_foreign_lock_reason_returns_dead(self, git_repo):
        import cli
        p = self._mk_locked(git_repo, "work4you-foreign", "some other tool")
...
class TestWidenedPruner:
    ...
    Previously only ``work4you-*`` directories were considered, so salvage/
    ...
    # -- named (non work4you-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "work4you-merged", commit=True, age_h=100)
    ...
    def test_merged_predicate_fails_safe_without_upstream(self, git_repo_no_remote):
        ...
        p = repo / ".worktrees" / "work4you-noremote"
        (repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
...
class TestMergeVerdictCache:
    ...
    dominates ``work4you -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "work4you-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "work4you-moves", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestPruneParallelEquivalence:
    ...
    def _kinds(survivors):
        ...
        for n in survivors:
            for kind in ("merged", "unpushed", "dirty", "fresh"):
                if n.startswith(f"work4you-{kind}"):
                    out.add(kind)
        return out
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "work4you-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestShallowCloneDeepening:
    ...
    (real incident: 21 of 25 work4you-* trees stuck on a default install).
    ...
    def _stuck_worktree(self, tmp_path):
        ...
        wt = clone / ".worktrees" / "work4you-shallowstuck"
        (clone / ".worktrees").mkdir()
        self._run(
            ["git", "worktree", "add", str(wt), "-b", "work4you/work4you-shallowstuck", "HEAD"],
            clone,
        )
