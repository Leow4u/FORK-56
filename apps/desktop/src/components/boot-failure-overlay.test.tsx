...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Work4You gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.work4youDesktop
  Object.defineProperty(window, 'work4youDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'work4youDesktop', { configurable: true, value: original })
}
