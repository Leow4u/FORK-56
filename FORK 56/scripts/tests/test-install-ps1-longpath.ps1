...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[work4you] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[work4you`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[work4you`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        Work4YouHome = $(if ($paths) { $paths.work4you_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\work4you\work4you'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-work4you'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-work4you') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
