...
def _is_work4you_provider_credential(name: str) -> bool:
    """True if ``name`` is a Work4You-managed provider credential (API key,
    token, or similar) per ``_WORK4YOU_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Work4You API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Work4You credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Work4You-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_work4you_internal_secret(name):
        return True
    return name in _WORK4YOU_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Work4You-managed provider credentials (from
    ``_WORK4YOU_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Work4You-managed
    ...
    Non-Work4You third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_work4you_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Work4You provider "
                "credential %r (blocked by _WORK4YOU_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Work4You-managed provider credentials must not be passed
                ...
                if _is_work4you_provider_credential(name):
                    logger.warning(
                        ...
                        "by _WORK4YOU_PROVIDER_ENV_BLOCKLIST). Operator "
