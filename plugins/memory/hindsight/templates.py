...
the ``work4you`` integration, and applies a chosen manifest to the user's bank
...
from work4you_cli.urllib_security import open_credentialed_url
...
def fetch_work4you_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``work4you`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "work4you" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_work4you_templates()
