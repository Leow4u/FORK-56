...
Batch processing lets you run the Work4You agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** — producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. A [Work4You Portal](/user-guide/features/tool-gateway) subscription bundles model access plus web search, image gen, TTS, and cloud browsers under one bill — useful when you want stable cost-per-trajectory without juggling rate limits across five vendor accounts. Set up with `work4you setup --portal`, then point `--model` at a Work4You model.
:::
