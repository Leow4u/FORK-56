...
Instructions for AI coding assistants and developers working on the work4you codebase.

**Never give up on the right solution.**

## What Work4You Is

Work4You is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Work4You ships a **lot** — most merges are bug fixes to
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `WORK4YOU_HOME`. Mocks hide integration bugs.
...
- **New `WORK4YOU_*` env vars for non-secret config.** `.env` is for secrets
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.work4you/plugins/` (or via a
  pip entry point), and promote them in the Work4You Discord
...
one behind a plain URL + token, or Work4You Cloud. Only the first two are spawned
by us and carry `WORK4YOU_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
Work4You desktop app."*

The pattern that works:

- **The toolset is the surface gate.** Keep the tools off `_WORK4YOU_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `WORK4YOU_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`work4you --tui` against that same backend) is
...
`$HOME/.work4you/work4you/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _WORK4YOU_CORE_TOOLS list
├── cli.py                # Work4YouCLI class — interactive CLI orchestrator (~11k LOC)
├── work4you_state.py       # SessionDB — SQLite session store (FTS5 search)
├── work4you_constants.py   # get_work4you_home(), display_work4you_home() — profile-aware paths
├── work4you_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
...
│   ├── work4you-achievements/  # Gamified achievement tracking
...
`gateway.log` when running the gateway. Profile-aware via `get_work4you_home()`.
Browse with `work4you logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Work4You: desktop, TUI, website, and future TS packages.
...
- `process_command()` is a method on `Work4YouCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
...
```
2. Add handler in `Work4YouCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `work4you --tui` or `WORK4YOU_TUI=1`.
...
A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@work4you/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `work4you serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `WORK4YOU_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `work4you` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `work4you --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Work4You built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Work4You core.
...
core Work4You tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_WORK4YOU_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_WORK4YOU_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_work4you_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `WORK4YOU_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_work4you_home()` for the base directory — never `Path.home() / ".work4you"`. This ensures each profile gets its own state.
...
- `default` — Classic Work4You gold/kawaii (the current look)
...
Work4You has two plugin surfaces. Both live under `plugins/` in the repo so
...
bundled, `$WORK4YOU_HOME/plugins/`, `./.work4you/plugins/` (opt-in via
`WORK4YOU_ENABLE_PROJECT_PLUGINS`), and `work4you.memory_providers` entry
...
`post_setup(work4you_home, config)` for setup-wizard integration.
...
don't own. Promote standalone plugins in the Work4You Discord
...
2. User: `$WORK4YOU_HOME/plugins/model-providers/<name>/`
...
[`work4you-example-plugins`](https://github.com/Work4You/work4you-example-plugins)
...
`metadata.work4you.tags`, `metadata.work4you.category`,
`metadata.work4you.related_skills`, `metadata.work4you.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.work4you.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Work4You tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   used Work4You to draft the skill), replace it with their actual name
...
lives in the `work4you-dev` skill at
...
`"messaging"`); `_WORK4YOU_CORE_TOOLS` is the default bundle most
...
Work4You returns a delegation id immediately and the result re-enters the
...
- **Board** is the hard boundary — workers are spawned with
  `WORK4YOU_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Work4You ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `WORK4YOU_BACKGROUND_NOTIFICATIONS` env var):
...
Work4You supports **profiles** — multiple fully isolated instances, each with its own
`WORK4YOU_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `work4you_cli/main.py` sets
`WORK4YOU_HOME` before any module imports. All `get_work4you_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_work4you_home()` for all WORK4YOU_HOME paths.** Import from `work4you_constants`.
   NEVER hardcode `~/.work4you` or `Path.home() / ".work4you"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".work4you" / "config.yaml"
   ```

2. **Use `display_work4you_home()` for user-facing messages.** Import from `work4you_constants`.
   This returns `~/.work4you` for default or `~/.work4you/profiles/<name>` for profiles.
   ```python
   # GOOD
   from work4you_constants import display_work4you_home
   print(f"Config saved to {display_work4you_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.work4you/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_work4you_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_work4you_home()`,
   not `Path.home() / ".work4you"`.

4. **Tests that mock `Path.home()` must also set `WORK4YOU_HOME`** — since code now uses
   `get_work4you_home()` (reads env var), not `Path.home() / ".work4you"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"WORK4YOU_HOME": str(tmp_path / ".work4you")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not WORK4YOU_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".work4you" / "profiles"`, NOT `get_work4you_home() / "profiles"`.
   This is intentional — it lets `work4you -p coder profile list` see all profiles regardless
...
### DO NOT hardcode `~/.work4you` paths
Use `get_work4you_home()` from `work4you_constants` for code paths. Use `display_work4you_home()`
for user-facing print/log messages. Hardcoding `~/.work4you` breaks profiles — each profile
has its own `WORK4YOU_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `work4you_cli/curses_ui.py`. See `work4you_cli/tools_config.py` for an example.
...
with actual imports (not mocks) against a temp `WORK4YOU_HOME`.

### Tests must not write to `~/.work4you/`
The `_isolate_work4you_home` autouse fixture in `tests/conftest.py` redirects `WORK4YOU_HOME` to a temp dir. Never hardcode `~/.work4you/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_work4you_home()` resolve within the temp dir.
...
subprocess (`--file-retries`, default 1; `WORK4YOU_TEST_FILE_RETRIES=0` to
...
Work4You supports Linux, macOS and native Windows, and plenty of its behaviour
