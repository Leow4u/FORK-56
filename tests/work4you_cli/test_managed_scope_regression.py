...
def work4you_home(tmp_path, monkeypatch):
    ...
    # /etc/work4you on the dev/CI box can't influence the test.
    monkeypatch.setenv("WORK4YOU_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(work4you_home, monkeypatch):
...
def test_env_expansion_in_user_config(work4you_home, monkeypatch):
    from work4you_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_work4you_dotenv(work4you_home=str(home))
