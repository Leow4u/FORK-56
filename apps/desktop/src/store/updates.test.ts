...
const checkWork4YouUpdateSpy = vi.fn()
const updateWork4YouSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/work4you', () => ({
  checkWork4YouUpdate: (...args: unknown[]) => checkWork4YouUpdateSpy(...args),
  updateWork4You: (...args: unknown[]) => updateWork4YouSpy(...args),
...
describe('maybeNotifyUpdateAvailable', () => {
  ...
  it('notifies for slim chrome updates even without a target SHA', () => {
    maybeNotifyUpdateAvailable(status({ targetSha: undefined, channel: 'chrome', updateAvailable: true, behind: null }))
    expect(notifySpy).toHaveBeenCalledTimes(1)
  })
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkWork4YouUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkWork4YouUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'work4you update',
      message: null,
    ...
    expect(checkWork4YouUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkWork4YouUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateWork4YouSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkWork4YouUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      work4youDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      work4youDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'work4you update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('work4you update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Work4You to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateWork4YouSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== work4you-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateWork4YouSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'work4you-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== work4you-update started now ===', `step ${attempt}`],
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateWork4YouSpy.mockResolvedValue({ ok: true, name: 'work4you-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== work4you-update started now ===', '✓ Already up to date!'],
      name: 'work4you-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== work4you-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'work4you-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== work4you-update completed ${'c'.repeat(32)} ===`],
        name: 'work4you-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== work4you-update completed ${actionId} ===`],
        name: 'work4you-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== work4you-update completed ${actionId} ===`],
      name: 'work4you-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('work4you-update', 2000)
  })

  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    $backendUpdateStatus.set({
      behind: 2,
      commits: [{ at: 1, author: 'Work4You', sha: 'requested-target', summary: 'target' }],
      fetchedAt: 1,
      supported: true,
    ...
    checkWork4YouUpdateSpy
      ...
      .mockResolvedValueOnce({
        behind: 1,
        can_apply: true,
        commits: [{ at: 2, author: 'Work4You', sha: 'newer-commit', summary: 'newer' }],
        current_version: '0.18.2',
        install_method: 'git',
        message: null,
        update_available: true,
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkWork4YouUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkWork4YouUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== work4you-update started now ===', 'still running'],
        name: 'work4you-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== work4you-update started now ===', 'Update complete!'],
        name: 'work4you-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateWork4YouSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'work4you-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== work4you-update started now ===', '✓ Already up to date!'],
      name: 'work4you-update',
      pid: 1,
      running: false
    })
    ...
    expect(updateWork4YouSpy).toHaveBeenCalledTimes(1)
  })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== work4you-update started now ===', 'update failed'],
      name: 'work4you-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateWork4YouSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkWork4YouUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
