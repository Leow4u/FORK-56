...
redirected ``get_work4you_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with WORK4YOU_HOME on the root.
    ...
    # The suite-wide fixture in conftest re-points ``work4you_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_work4you_home()``,
    # which is what production does.  ``WORK4YOU_HOME`` above still keeps that
...
def test_db_handle_follows_the_active_profile_scope(multiplex_homes):
    ...
    token = set_work4you_home_override(str(profile))
    try:
