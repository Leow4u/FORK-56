...
pytestmark = pytest.mark.skipif(
    os.environ.get("WORK4YOU_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "WORK4YOU_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "work4you": _FakeImgProvider("work4you")}
    ...
    # Listed in preference order: Work4You Portal before OpenAI.
    assert [p["name"] for p in listed] == ["work4you", "openai"]
