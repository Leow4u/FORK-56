...
Canonical owners:
  ...
  * ``work4you_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._work4you_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/work4you/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "work4you_cli/managed_scope.py",
