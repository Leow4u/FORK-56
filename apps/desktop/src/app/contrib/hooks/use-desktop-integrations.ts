...
import { pathFromWork4YouDeepLink, resolveWork4YouOpenPath } from '@/lib/work4you-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.work4youDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.work4youDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // work4you://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future work4youDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveWork4YouOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (work4you://index-network/intent/1, work4you://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromWork4YouDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.work4youDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.work4youDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
