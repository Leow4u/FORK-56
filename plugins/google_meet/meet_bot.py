...
``$WORK4YOU_HOME/workspace/meetings/<meeting-id>/``. The main work4you process
...
# Filenames the bot reads/writes in ``WORK4YOU_MEET_OUT_DIR``.
...
# and exposes ``window.__work4youMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__work4youMeetInstalled) return;
  window.__work4youMeetInstalled = true;
  window.__work4youMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__work4youMeetQueue.push({
      ts: Date.now(),
  ...
  window.__work4youMeetDrain = () => {
    const out = window.__work4youMeetQueue.slice();
    window.__work4youMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "work4you_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("WORK4YOU_MEET_URL", "").strip()
    out_dir_env = os.environ.get("WORK4YOU_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("WORK4YOU_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("WORK4YOU_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("WORK4YOU_MEET_GUEST_NAME", "Work4You")
    duration_s = _parse_duration(os.environ.get("WORK4YOU_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when WORK4YOU_MEET_MODE=realtime.
    mode = os.environ.get("WORK4YOU_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("WORK4YOU_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("WORK4YOU_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("WORK4YOU_MEET_REALTIME_INSTRUCTIONS", "")
    # WORK4YOU_MEET_REALTIME_KEY is set explicitly by process_manager.start(),
    ...
    realtime_api_key = os.environ.get("WORK4YOU_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("WORK4YOU_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__work4youMeetDrain && window.__work4youMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__work4youMeetInstalled) {
