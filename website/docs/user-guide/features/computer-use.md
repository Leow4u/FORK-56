...
Work4You can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Work4You integration. It
...
**Fresh installs already have the driver.** The Work4You installer
...
- **`work4you tools`** → pick `🖱️  Computer Use` — installs the driver
...
on macOS/Linux, `install.ps1` on Windows. Use `work4you computer-use
status` to verify the install.

Already have cua-driver? Work4You reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `work4you update`, and the first
`computer_use` call of a session, Work4You checks the local version and
...
selected with `WORK4YOU_CUA_DRIVER_CMD` stays
under your control, so Work4You reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Work4You autodetection is a
planned cua-driver follow-up, so currently point Work4You at that directory or
...
interface. The built-in toolset provides Work4You actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `work4you computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Work4You host identity. |
...
```
work4you -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.work4you/config.yaml`.

## Permission modes and logged-in browser profiles

Work4You maps its existing approval UX onto cua-driver's immutable runtime
...
| Work4You session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Work4You approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Work4You risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Work4You then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.work4you/cua-manifest.yaml
```
...
for the format). Work4You launches a private runtime with
...
resetting or closing the Work4You session, cancellation cleanup, or process exit
closes that transport session. Work4You also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Work4You
...
embedded service under the Work4You host identity.
...
## `work4you computer-use doctor` — your first triage stop

`work4you computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ work4you computer-use doctor
...
Work4You run declares a public cua-driver **session name** (something like
`work4you-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Work4You keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Work4You-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Work4You
autodetection is a planned cua-driver follow-up, so currently point Work4You at
...
These are **platform deep dives, not duplicates of the Work4You skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Work4You
...
Work4You applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.work4you/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Work4You applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Work4You agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Work4You agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Work4You warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Work4You from inside the RDP / console
...
```
WORK4YOU_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
WORK4YOU_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Work4You disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Work4You sets
...
When it's on, `work4you computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Work4You at a binary you built from source instead
of the published release. Work4You resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`WORK4YOU_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Work4You testing (see notes).
...
### Option B — point Work4You straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`WORK4YOU_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
WORK4YOU_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
WORK4YOU_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Work4You is using your build

- `work4you computer-use status` prints the resolved binary path and
  version.
- `work4you computer-use doctor` confirms the binary is reachable and
...
- **Work4You spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Work4You YOLO, Work4You instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Work4You
  change needed either way.
- **Local builds skip the version check.** Work4You warns when the
...
**First action when anything's off: run `work4you computer-use doctor`.**
...
Run `work4you computer-use install` to fetch the cua-driver binary, or
run `work4you tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `work4you computer-use doctor` will
...
- **Work4You-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Work4You `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Work4You autodetection is a
  planned follow-up; currently point Work4You at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Work4You among them)
