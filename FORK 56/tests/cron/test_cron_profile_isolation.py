...
profile's WORK4YOU_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.work4you/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active WORK4YOU_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever WORK4YOU_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    WORK4YOU_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import work4you_constants

    monkeypatch.setattr(
        work4you_constants, "_get_platform_default_work4you_home", lambda: root
    )
    monkeypatch.setenv("WORK4YOU_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile WORK4YOU_HOME (<root>/profiles/<name>), the cron store
    ...
    assert work4you_constants.get_work4you_home().resolve() == profile_home.resolve()
    assert work4you_constants.get_default_work4you_root().resolve() == root.resolve()

    # cron/jobs.py computes WORK4YOU_DIR from get_work4you_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.WORK4YOU_DIR.resolve() == profile_home.resolve()
        assert (
