...
A credential added via `work4you auth add openrouter` lives in the credential
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `work4you auth add openrouter <token>` — a manual pool entry."""
...
def test_auto_detects_openrouter_from_pool(tmp_path, monkeypatch):
    ...
    (tmp_path / "work4you").mkdir(parents=True, exist_ok=True)
