# Work4You Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Work4You Portal subscription without me
...
## Layer 1 — Is this thing a Work4You plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Work4You process**. Its LLM calls go through Work4You's already-configured provider. | Already uses Portal if user's Work4You is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.work4you.ai`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Work4You. That LLM call is already authenticated
via Work4You's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.work4you.ai/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.work4you.ai → API Keys`, or
...
"Sign in with Work4You Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Work4You has that *feels* like OAuth — `work4you login --provider work4you`
opening a browser, user signs in, token lands in `~/.work4you/auth.json` — is a
**Work4You-specific browser flow**. Under the hood it produces a credential
Work4You uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Work4You's existing Portal credential out of `~/.work4you/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.work4you.ai/v1` with that
   bearer attached.
...
rides on the credential Work4You already holds.

Where this could live in Work4You:
...
Only when the consumer is *another first-party Work4You thing* (a future SDK, a
Work4You-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Work4You itself.
...
1. First decide: Work4You plugin (runs inside Work4You) or separate app? If plugin,
   it already uses Portal via Work4You's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.work4you.ai/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Work4You agent itself") is fully supported. Work4You treats agent
...
pitch in the Tool Gateway announcement is specifically about Work4You's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Work4You is configured to use Portal as its provider. It is
