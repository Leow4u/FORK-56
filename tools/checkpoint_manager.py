...
-----------------------------------------------------------------------------
    ...
    ~/.work4you/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/work4you/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_work4you_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/work4you"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Work4You convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("WORK4YOU_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Work4You wrote
...
def _git_env(
    ...
    The shared store is internal Work4You infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``work4you checkpoints clear-legacy``.
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "work4you@local"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "WORK4YOU_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``WORK4YOU_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at WORK4YOU_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "WORK4YOU_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Work4You just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Work4You last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Work4You last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Work4You'
        last write or that Work4You never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Work4You has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Work4You never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Work4You wrote it: restoring it back is
                # safe — its last content was Work4You-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Work4You changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Work4You changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Work4You' last write — per the agent-write ledger —
        are left untouched, and only Work4You-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Work4You-created files absent from it (delete to restore state).
...
# ---------------------------------------------------------------------------
# Public helpers for `work4you checkpoints` CLI
# ---------------------------------------------------------------------------
