 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.work4you.send(prompt)` (or declarative
 * `data-work4you-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'work4you-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'work4you-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `work4you.send(prompt)`.
 ...
 *  wires `data-work4you-send` so declarative HTML works with zero script:
 *  `<button data-work4you-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.work4you={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-work4you-send]"):null;' +
    'if(el)send(el.getAttribute("data-work4you-send")||"")},true)})()</script>'
  )
}
...
function InlineHtmlFrame({
  ...
  useEffect(() => {
    ...
    void Promise.resolve(window.work4youDesktop?.readFileText(path))
      .then(result => {
        if (!alive) {
          return
        }
