...
The Python launcher (`work4you --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Work4You session
...
from work4you_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["work4you", "--tui"])
        monkeypatch.delenv("WORK4YOU_TUI", raising=False)
        monkeypatch.setenv("WORK4YOU_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["work4you", "--tui"])
        monkeypatch.delenv("WORK4YOU_TUI", raising=False)
        monkeypatch.delenv("WORK4YOU_TUI_NO_EARLY_DISABLE", raising=False)
