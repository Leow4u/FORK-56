...
export type { Work4YouOpenTarget }
...
const STORAGE_KEY = 'work4you:native-notifications'
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `work4you://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `work4you://index-network/intent/1`). */
  activate?: Work4YouOpenTarget
 ...
 *  user is away from Work4You — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveWork4YouOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveWork4YouOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
