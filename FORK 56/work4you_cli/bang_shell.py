...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("WORK4YOU_GATEWAY_SESSION"):
        return False
    if env_var_enabled("WORK4YOU_CRON_SESSION"):
        return False
    if (os.getenv("WORK4YOU_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Work4You-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Work4You session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
