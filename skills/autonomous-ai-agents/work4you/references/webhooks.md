...
Add to `${WORK4YOU_HOME:-~/.work4you}/.env`:
```bash
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.work4you/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.work4you/scripts/` (path traversal is blocked).

```bash
work4you webhook subscribe todoist-work4you \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-work4you-label.py" \
  --deliver telegram --deliver-chat-id "12345"
```

Full filter syntax: https://work4you.ai/docs/user-guide/messaging/webhooks#payload-filters
