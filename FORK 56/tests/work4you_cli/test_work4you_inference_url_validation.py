"""Regression tests for Work4You Portal inference_base_url host-allowlist validation.
...
client. ``_validate_work4you_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``WORK4YOU_INFERENCE_BASE_URL``) is NOT
...
from work4you_cli.auth import (
    DEFAULT_WORK4YOU_INFERENCE_URL,
    _ALLOWED_WORK4YOU_INFERENCE_HOSTS,
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="work4you_cli.auth"):
            assert (
                _validate_work4you_inference_url_from_network("https://attacker.com/v1")
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_WORK4YOU_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.work4you.ai``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_work4you_inference_url_from_network(DEFAULT_WORK4YOU_INFERENCE_URL)
            == DEFAULT_WORK4YOU_INFERENCE_URL.rstrip("/")
        )
...
class TestCallSiteWiring:
    ...
    ``_validate_work4you_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_work4you_runtime_credentials`` and one
    ...
    def _read_auth_source(self):
        import work4you_cli.auth as _auth_mod
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_work4you_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_work4you_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_work4you_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The Work4You proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import work4you_cli.proxy.adapters.work4you_portal as _work4you_adapter
        source = Path(_work4you_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_work4you_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``WORK4YOU_INFERENCE_BASE_URL`` is read by ``resolve_work4you_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_work4you_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "WORK4YOU_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_work4you_inference_url_from_network" not in line, (
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.work4you.ai`` persisted before the
    ...
    staging endpoint (real incident: opus-4.8 routed to work4you, work4you pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api.work4you.ai/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "work4you-cli",
            "portal_base_url": auth.DEFAULT_WORK4YOU_PORTAL_URL,
        ...
        monkeypatch.setattr(auth, "_work4you_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(auth, "_assert_work4you_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_work4you_invoke_jwt", lambda *a, **k: None)

        result = auth.refresh_work4you_oauth_from_state(state, force_refresh=True)

        assert result["inference_base_url"] == auth.DEFAULT_WORK4YOU_INFERENCE_URL, (
...
class TestEnvOverrideWins:
    """``WORK4YOU_INFERENCE_BASE_URL`` must win over the stored value for the
    ...
    STAGING = "https://stg-inference-api.work4you.ai/v1"

    def _patch_no_refresh(self, monkeypatch, auth, state):
        import contextlib

        # No refresh fires: the stored access token is a usable invoke JWT.
        monkeypatch.setattr(auth, "_work4you_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(auth, "_write_shared_work4you_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_work4you_pool_from_auth_store", lambda *a, **k: None)
    ...
    def test_no_refresh_env_override_not_persisted(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_WORK4YOU_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.setenv("WORK4YOU_INFERENCE_BASE_URL", self.STAGING)

        auth.resolve_work4you_runtime_credentials()

        assert state["inference_base_url"] == auth.DEFAULT_WORK4YOU_INFERENCE_URL, (
    ...
    def test_no_refresh_heals_poisoned_stored_without_env(self, monkeypatch):
        ...
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.delenv("WORK4YOU_INFERENCE_BASE_URL", raising=False)

        result = auth.resolve_work4you_runtime_credentials()
        assert result["base_url"] == auth.DEFAULT_WORK4YOU_INFERENCE_URL, (
...
class TestProxyAdapterEnvOverride:
    """The Work4You proxy adapter is the second chokepoint: it re-validates the
    base_url returned by resolve_work4you_runtime_credentials() against the prod
    allowlist. That re-validation must not clobber a legitimate
    WORK4YOU_INFERENCE_BASE_URL staging override.
    """
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_work4you_adapter.__file__).read_text(encoding="utf-8")
        assert "_work4you_inference_env_override()" in source, (
