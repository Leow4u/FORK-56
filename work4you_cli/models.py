...
Add, remove, or reorder entries here — both `work4you setup` and
`work4you` provider-selection will pick up the change automatically.
"""
...
from work4you_cli.urllib_security import open_credentialed_url, url_origin
...
_WORK4YOU_USER_AGENT = f"work4you-cli/{_WORK4YOU_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from work4you_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `work4you model`
    flow without maintaining a separate static list.
    """
    from work4you_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $WORK4YOU_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Work4You-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $WORK4YOU_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see work4you_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Work4You Portal free-model helper
# ---------------------------------------------------------------------------
# The Work4You Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Work4You Portal account tier detection
# ---------------------------------------------------------------------------
def is_work4you_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_work4you_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Work4You models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/work4you/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["work4you"]`` list happens to contain or whether the
    ...
    older Work4You that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_work4you_models_by_tier` keeps it.

    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged.
    """
    try:
        payload = fetch_work4you_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/work4you/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["work4you"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Work4You that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_work4you_models_by_tier` keeps free models selectable;
...
# ---------------------------------------------------------------------------
# Work4You Portal recommended models
#
...
WORK4YOU_RECOMMENDED_MODELS_PATH = "/api/work4you/recommended-models"
_WORK4YOU_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_work4you_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _work4you_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "cache" / "work4you_recommended_cache.json"


def _read_work4you_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_work4you_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_work4you_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _work4you_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "work4you recommended-models disk cache write failed: %s", exc
        )


def fetch_work4you_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Work4You Portal's curated recommended-models payload.

    Hits ``<portal>/api/work4you/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_WORK4YOU_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$WORK4YOU_HOME/cache/work4you_recommended_cache.json``) as last-known-good.
    ...
    now = time.monotonic()
    cached = _work4you_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _WORK4YOU_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{WORK4YOU_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _work4you_recommended_cache[base] = (data, now)
        _write_work4you_recommended_disk(base, data)
        return data
    ...
    disk = _read_work4you_recommended_disk(base)
    if disk:
        _work4you_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_work4you_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from work4you_cli.auth import (
            DEFAULT_WORK4YOU_PORTAL_URL,
        ...
        return str(DEFAULT_WORK4YOU_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.work4you.ai"
...
def get_work4you_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_work4you_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_work4you_portal_url()
    payload = fetch_work4you_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_work4you_free_tier()
        except Exception:
...
# this list:  work4you model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `work4you model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `work4you model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("work4you",           "Work4You Portal",              "Work4You Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Work4You provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `work4you model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``work4you model``, the
...
# In-repo fallback for the model Work4You silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from work4you_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Work4You Portal, OpenRouter) order
...
# ``work4you model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in work4you_cli/web_server.py and
# ``partition_work4you_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"work4you", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``work4you auth add openai-codex`` without
    ``work4you model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``work4you model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    work4you is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Work4You Portal, private mirrors, older catalog snapshots) don't populate
...
# work4you_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from work4you_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — work4you
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Work4You lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_work4you_model_ids() -> list[str]:
    """Return the curated Work4You Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["work4you"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from work4you_cli.model_catalog import get_curated_work4you_models
        remote = get_curated_work4you_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("work4you", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from work4you_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Work4You Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Work4You fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Work4You Portal).

    When *include_sale_original* is true (Work4You Portal only) and the gateway
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _WORK4YOU_USER_AGENT,
    }
    if api_key:
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Work4You Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return work4you-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; work4you's picker expects
...
_DEFAULT_WORK4YOU_INFERENCE_BASE = "https://inference-api.work4you.ai"


def _resolve_work4you_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Work4You Portal pricing.

    The Work4You inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``WORK4YOU_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _work4you_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_WORK4YOU_INFERENCE_BASE).rstrip("/")
    return (api_key, base_url)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, work4you, ai-gateway, novita)."""
    ...
    if normalized == "work4you":
        api_key, base_url = _resolve_work4you_pricing_credentials()
        if base_url:
            # Work4You base_url typically looks like https://inference-api.work4you.ai/v1
            ...
            return fetch_models_with_pricing(
                ...
                base_url=stripped,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Work4You Portal-only.
                include_sale_original=True,
            )
    return {}
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from work4you_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``work4you model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from work4you_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        work4you:work4you-3                           →  ("work4you", "work4you-3")
...
def _get_ollama_request_headers() -> dict[str, str]:
    ...
    try:
        from work4you_cli.config import normalize_extra_headers
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _WORK4YOU_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Work4You, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"work4you", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from work4you_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model work4you` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Work4You Portal),
            # entry [0] is the priciest flagship, and typing ``/model work4you``
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Work4You' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``work4you_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Work4You should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``work4you auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.work4you/.env``.
...
#   - "work4you": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from work4you_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Work4You),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Work4You release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Work4You release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "work4you":
        # Try live Work4You Portal /models endpoint
        try:
            from work4you_cli.auth import fetch_work4you_models, resolve_work4you_runtime_credentials
            creds = resolve_work4you_runtime_credentials()
            if creds:
                live = fetch_work4you_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["work4you"] snapshot — so newly
        # added Portal models still surface without a Work4You release.
        manifest_ids = get_curated_work4you_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``work4you model`` shows.
            from work4you_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $WORK4YOU_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, work4you portal), the
    relevant tokens live in ``$WORK4YOU_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `work4you config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _WORK4YOU_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Work4You' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Work4You' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _WORK4YOU_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"work4you/{_WORK4YOU_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=work4you"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from work4you_constants import get_work4you_home
    return get_work4you_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `work4you moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Work4You will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Work4You will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Work4You cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Work4You actually ships.  (#46850)
            #
