...
This module is imported by ``work4you_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no work4you_cli.config,
...
``work4you_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(work4you_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect WORK4YOU_HOME?"""
    active_profile = os.path.join(work4you_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    work4you_home = os.environ.get("WORK4YOU_HOME", "").strip()
    if work4you_home:
        return work4you_home
    return os.path.join(os.path.expanduser("~"), ".work4you")
...
def container_mode_may_be_active() -> bool:
    ...
    work4you_home = os.environ.get("WORK4YOU_HOME", "").strip()
    if work4you_home:
        if os.path.exists(os.path.join(work4you_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(work4you_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(work4you_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".work4you")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect WORK4YOU_HOME.
    """
...
def print_fast_version_info() -> None:
    from work4you_cli import __release_date__, __version__
    ...
    print("Run 'work4you version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``work4you --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the WORK4YOU_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("WORK4YOU_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
