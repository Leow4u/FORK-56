...
//! WORK4YOU_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for WORK4YOU_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub work4you_home: Option<String>,
}
...
/// Spawn the locally-built Work4You desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_work4you_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    ...
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Work4You
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_work4you_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Work4You.exe"),
            ("win-arm64-unpacked", "Work4You.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Work4You.app/Contents/MacOS", "Work4You"),
            ("mac-arm64/Work4You.app/Contents/MacOS", "Work4You"),
        ]
    } else {
        &[("linux-unpacked", "work4you")]
    };
...
pub(crate) fn resolve_work4you_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_work4you_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Work4You.app/Contents/MacOS/Work4You -> .../Work4You.app
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). work4you_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".work4you-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_work4you_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Work4You desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_work4you_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::work4you_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $Work4YouHome which Stage-Repository clones
    // the repo INTO at $Work4YouHome\work4you. Mirrors work4you_constants.
    let work4you_home = args
        .work4you_home
        .clone()
        .unwrap_or_else(|| crate::paths::work4you_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&work4you_home).join("work4you");
    ...
    // Copy ourselves to WORK4YOU_HOME/work4you-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_work4you_home() {
        tracing::warn!(?err, "failed to copy installer into WORK4YOU_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    work4you_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, work4you_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Work4You"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Work4You.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Work4You.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("work4you");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Work4You.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_work4you_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_work4you_desktop_app(&root)
    ...
    #[test]
    fn resolve_work4you_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_work4you_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".work4you-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".work4you-bootstrap-complete");
        let tmp_path = root.join(".work4you-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    #[test]
    fn work4you_is_installed_treats_marker_existence_as_sufficient() {
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".work4you-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".work4you-bootstrap-complete.tmp").exists(),
