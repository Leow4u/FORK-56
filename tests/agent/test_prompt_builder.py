...
from work4you_cli.work4you_subscription import Work4YouFeatureState, Work4YouSubscriptionFeatures
...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.config.load_config", default_load_config)
        monkeypatch.setattr("work4you_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("work4you_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("work4you_cli.config.load_config", lambda: {})
        monkeypatch.setattr("work4you_cli.config.load_config_readonly", lambda: {})
...
class TestBuildWork4YouSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_work4you_tools_enabled", lambda: True)
        monkeypatch.setattr(
            "work4you_cli.work4you_subscription.get_work4you_subscription_features",
            lambda config=None: Work4YouSubscriptionFeatures(
                subscribed=True,
                work4you_auth_present=True,
                provider_is_nous=True,
                features={
                    "web": Work4YouFeatureState("web", "Web tools", True, True, True, True, False, True, "firecrawl"),
                    "image_gen": Work4YouFeatureState("image_gen", "Image generation", True, True, True, True, False, True, "Work4You Subscription"),
                    "video_gen": Work4YouFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": Work4YouFeatureState("tts", "OpenAI TTS", True, True, True, True, False, True, "OpenAI TTS"),
                    "stt": Work4YouFeatureState("stt", "Speech-to-text", True, True, True, True, False, True, "OpenAI Whisper"),
                    "browser": Work4YouFeatureState("browser", "Browser automation", True, True, True, True, False, True, "Browser Use"),
                    "modal": Work4YouFeatureState("modal", "Modal execution", False, True, False, False, False, True, "local"),
                },
            ),
        )

        prompt = build_work4you_subscription_prompt({"web_search", "browser_navigate"})
    ...
    def test_non_subscriber_prompt_includes_relevant_upgrade_guidance(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda config=None: Work4YouSubscriptionFeatures(
                ...
                features={
                    "web": Work4YouFeatureState("web", "Web tools", True, False, False, False, False, True, ""),
                    "image_gen": Work4YouFeatureState("image_gen", "Image generation", True, False, False, False, False, True, ""),
                    "video_gen": Work4YouFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": Work4YouFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                    "stt": Work4YouFeatureState("stt", "Speech-to-text", True, False, False, False, False, True, ""),
                    "browser": Work4YouFeatureState("browser", "Browser automation", True, False, False, False, False, True, ""),
                    "modal": Work4YouFeatureState("modal", "Modal execution", False, False, False, False, False, True, ""),
                },
            ),
        )

        prompt = build_work4you_subscription_prompt({"image_generate"})

        assert "suggest Work4You subscription as one option" in prompt
        assert "Do not mention subscription unless" in prompt

    def test_feature_flag_off_returns_empty_prompt(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_work4you_tools_enabled", lambda: False)

        prompt = build_work4you_subscription_prompt({"web_search"})
...
class TestBuildContextFilesPrompt:
    ...
    def test_work4you_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".work4you.md").write_text("Work4You-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Work4You-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path / "work4you_home"))
        work4you_home = tmp_path / "work4you_home"
        work4you_home.mkdir()
        (work4you_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .work4you.md / WORK4YOU.md discovery ---
...
# =========================================================================
# .work4you.md helper functions
# =========================================================================


class TestFindWork4YouMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".work4you.md").write_text("rules")
        assert _find_work4you_md(tmp_path) == tmp_path / ".work4you.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".work4you.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_work4you_md(sub) == tmp_path / ".work4you.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .work4you.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """WORK4YOU_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("WORK4YOU_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
