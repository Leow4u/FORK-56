...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(WORK4YOU_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Work4You-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.work4you/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'work4you-desktop-ssh')
  }

  return path.join(os.homedir(), '.work4you', 'desktop-ssh')
}
