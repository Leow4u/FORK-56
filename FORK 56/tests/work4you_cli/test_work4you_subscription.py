"""Tests for Work4You subscription feature detection."""

import shutil
import sys

from work4you_cli.work4you_account import Work4YouPortalAccountInfo, Work4YouToolAccessInfo
from work4you_cli import work4you_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> Work4YouPortalAccountInfo:
...
def _pool_account() -> Work4YouPortalAccountInfo:
    ...
    return Work4YouPortalAccountInfo(
        ...
        tool_access=Work4YouToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_work4you_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_work4you_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_work4you_subscription_features({"web": {"backend": "exa"}})
...
def test_local_browser_unavailable_without_chromium(monkeypatch):
    ...
    features = ns.get_work4you_subscription_features(
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import work4you_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "work4you_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_pool_offers_covered_tools_only(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_work4you_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
    ...
    config = {"model": {"provider": "work4you"}}
...
def test_apply_work4you_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_work4you_managed_defaults must store the managed 'work4you' selection
    when a Work4You subscriber selects video_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_work4you_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_work4you_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "work4you"}}
    changed = ns.apply_work4you_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_work4you_portal_access — inline login gate for `work4you tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Work4You detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.Work4YouSubscriptionFeatures(
        subscribed=True,
        work4you_auth_present=True,
        provider_is_nous=True,
        account_info=account_info,
        features={
            key: ns.Work4YouFeatureState(
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("work4you_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_work4you_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Work4You Node dir even when
    ...
    monkeypatch.setattr(
        "work4you_constants.with_work4you_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "work4you_constants.agent_browser_runnable",
