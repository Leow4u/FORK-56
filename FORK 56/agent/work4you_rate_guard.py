"""Cross-session rate limit guard for Work4You Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Work4You Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Work4You triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Work4You retries), and every one of those calls counts
...
_STATE_FILENAME = "work4you.json"


def _state_path() -> str:
    """Return the path to the Work4You rate limit state file."""
    try:
        from work4you_constants import get_work4you_home
        base = get_work4you_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".work4you")
...
def record_work4you_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Work4You Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Work4You rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Work4You rate limit state: %s", exc)


def work4you_rate_limit_remaining() -> Optional[float]:
    """Check if Work4You Portal is currently rate-limited.
...
def clear_work4you_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Work4You request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Work4You rate limit state: %s", exc)
...
def is_genuine_work4you_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Work4You Portal is a real account rate limit.

    Work4You Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Work4You, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Work4You is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Work4You.

    Tripping the cross-session breaker on (b) blocks ALL Work4You requests
    (and all models, since Work4You is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Work4You emits
