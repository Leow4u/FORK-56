"""``work4you slack ...`` CLI subcommands.

Today only ``work4you slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ work4you slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Work4You. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Work4You deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Work4You on Slack's older Assistant messaging
    ...
    """
    from work4you_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Work4You in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
...
def slack_manifest_command(args) -> int:
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "work4you slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"work4you slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "work4you slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "work4you slack manifest: long description must be at most "
