/**
 * @work4you/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__WORK4YOU_PLUGIN_SDK__` and maps the import to it, so a
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Work4You profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.work4youDesktop?.getAgentRoster
...
//
// ensureGatewayProfile awaits the store's dial, and Work4YouGateway.connect() has
...
export const host = {
  ...
  connections: async () => {
    const bridge = window.work4youDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Work4You Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.work4youDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Work4You Desktop.')
    }
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Work4You Desktop connection routing unavailable')
    }
