...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `work4you setup --portal`. See [Work4You Portal](/integrations/work4you-portal).
:::
...
2. **Work4You spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.work4you/cron/output/` and you can read them anytime.
:::
...
Work4You will search the web, read through results, and produce something like:
...
Before creating cron jobs, ensure Work4You has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Work4You what you want:
...
Work4You will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Work4You to delegate each topic to a sub-agent:
...
Work4You will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
