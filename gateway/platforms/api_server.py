...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Work4You-Session-Id header; opt-in long-term memory scoping via X-Work4You-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Work4You-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists work4you and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Work4You sessions
- POST /api/sessions               — create an empty Work4You session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to work4you
...
def _work4you_version() -> str:
    """Return the canonical Work4You version string.

    ``work4you_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``work4you-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from work4you_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("WORK4YOU_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``work4you``)
    ...
    ``direct_model_requests`` config value here, while Work4You-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Work4You-aware and is
    always honored.
    """
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_work4you_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted WORK4YOU_HOME (same filesystem
...
def _derive_chat_session_id(
    ...
    the same Work4You session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through work4you's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Work4You-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`work4you_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from work4you_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "work4you")
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_work4you_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from work4you_cli.profiles import get_profile_dir
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Work4You-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Work4You-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Work4You-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Work4You-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Work4You-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``work4you sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_work4you_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_work4you_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``work4you``) means "use
        ...
        400s ("work4you is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "work4you") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "work4you_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "work4you_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the work4you-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Work4You-Session-Key``).  Unlike ``session_id``
        ...
        # work4you_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `work4you auth add openai-codex` without `work4you model`),
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "work4you", "version": _work4you_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list work4you and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Work4You provider/model inventory.
        ...
        Work4You provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Work4You version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Work4You AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Work4You-Session-Id",
                "session_key_header": "X-Work4You-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Work4You sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Work4You session row.
        ...
        # (e.g. "provider::work4you") are split, and the virtual model
        # alias (self._model_name, e.g. "work4you") is nulled out
        # there — a bare "work4you" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "work4you.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "work4you.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "work4you.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "work4you.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Work4You-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Work4You-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "work4you.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Work4You-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Work4You-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "work4you.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Work4You-Session-Key.  This
        # is independent of X-Work4You-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Work4You-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Work4You-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Work4You-Session-Id rejected: "
            ...
            # conversation map to the same Work4You session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``work4you.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Work4You-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Work4You-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["work4you"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Work4You-Completed"] = "false"
            response_headers["X-Work4You-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Work4You-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Work4You-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Work4You-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Work4You-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Work4You-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: work4you.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="work4you.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Work4You-Session-Id header.  Without this,
        ...
        response_headers = {"X-Work4You-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Work4You-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Work4You agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Work4You-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_work4you_api_runtime", {}) or {})
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Work4You-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "work4you.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "work4you.run.steer", "run_id": run_id, "accepted": True})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Work4You-Relay
            ...
            # that is the exact surface the work4you-0day campaign abused to write
            # ~/.work4you/config.yaml and plant persistence. Sandboxing (Docker /
