...
Optional Work4You observability plugin that configures exporters and maps
Work4You-specific observer hooks to NeMo Relay marks and ATIF state. Work4You core
...
CLI UX. Work4You core emits NeMo Relay lifecycle events for provider and tool
...
- Export the Relay scopes and LLM/tool lifecycles emitted by Work4You core.
- Add Work4You session, turn, approval, and subagent mark events.
...
The `WORK4YOU_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `WORK4YOU_HOME` that the
agent run will use:

```bash
env WORK4YOU_HOME=/tmp/work4you-nemo-relay-test \
  work4you plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`WORK4YOU_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`WORK4YOU_HOME` is the Work4You profile/config home used by both
`work4you plugins enable ...` and the later `work4you chat ...` run. If unset,
Work4You uses the user's default home, usually `~/.work4you`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export WORK4YOU_HOME=/tmp/work4you-nemo-relay-test
work4you plugins enable observability/nemo_relay
work4you chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `work4you` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run work4you plugins enable observability/nemo_relay
uv run work4you chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Work4You installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/work4you-*.whl
work4you plugins enable observability/nemo_relay
```
...
behavior; it does not control Work4You shared client metrics.
...
`work4you.session.segment` / `work4you.session.segment_reason`
...
The plugin can configure exporters directly from `WORK4YOU_NEMO_RELAY_*`
...
```bash
export WORK4YOU_NEMO_RELAY_ATOF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export WORK4YOU_NEMO_RELAY_ATIF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `WORK4YOU_NEMO_RELAY_ATOF_FILENAME`
- `WORK4YOU_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `WORK4YOU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `WORK4YOU_NEMO_RELAY_ATIF_AGENT_NAME`
- `WORK4YOU_NEMO_RELAY_ATIF_AGENT_VERSION`
- `WORK4YOU_NEMO_RELAY_ATIF_MODEL_NAME`
- `WORK4YOU_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Work4You at it:

```bash
export WORK4YOU_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `WORK4YOU_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`WORK4YOU_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`WORK4YOU_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Work4You does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Work4You keeps the direct env-var fallbacks active for that run.

Work4You core routes provider and tool execution through NeMo Relay managed APIs
...
Work4You identifies managed LLM operations by provider wire protocol, not by the
physical service routing the request. This keeps Relay middleware and codec
selection stable when providers such as OpenRouter or Work4You expose an
OpenAI-compatible API:

| Work4You API mode | Relay operation |
| --- | --- |
| `chat_completions` | `openai.chat_completions` |
| `codex_responses` | `openai.responses` |
| `anthropic_messages` | `anthropic.messages` |

Middleware written against earlier Work4You builds might match physical provider
names such as `openrouter`, `work4you`, or `anthropic` as the operation. Match the
canonical operation instead. Work4You retains the original provider in the
managed invocation metadata field `work4you.provider`. Unknown or custom API
modes continue to use the physical provider name as their operation.

### Dynamic Plugins

Work4You uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Work4You-owned
...
with this Work4You-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Work4You rejects
...
Work4You activates these plugins before registering its managed LLM and tool
...
For the full generic Work4You middleware contract, see
...
with Work4You and a local Ollama model served through the OpenAI-compatible API.

```bash
export WORK4YOU_HOME=/tmp/work4you-nemo-relay-docs/work4you-home
mkdir -p "$WORK4YOU_HOME"

cat > "$WORK4YOU_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Work4You session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export WORK4YOU_NEMO_RELAY_ATOF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/work4you-nemo-relay-docs/subagent/atof
export WORK4YOU_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export WORK4YOU_NEMO_RELAY_ATOF_MODE=overwrite
export WORK4YOU_NEMO_RELAY_ATIF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/work4you-nemo-relay-docs/subagent/atif
export WORK4YOU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export WORK4YOU_NEMO_RELAY_ATIF_AGENT_NAME='Work4You E2E'
export WORK4YOU_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export WORK4YOU_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"work4you.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
message. Work4You dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/work4you-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/work4you-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/work4you-nemo-relay-docs/workdir/beta.txt
cd /tmp/work4you-nemo-relay-docs/workdir

export WORK4YOU_NEMO_RELAY_ATOF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/work4you-nemo-relay-docs/parallel/atof
export WORK4YOU_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export WORK4YOU_NEMO_RELAY_ATOF_MODE=overwrite
export WORK4YOU_NEMO_RELAY_ATIF_ENABLED=1
export WORK4YOU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/work4you-nemo-relay-docs/parallel/atif
export WORK4YOU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export WORK4YOU_NEMO_RELAY_ATIF_AGENT_NAME='Work4You E2E'
export WORK4YOU_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Work4You sessions map to `agent` scopes.
- Work4You core managed provider calls map to `llm` scope start/end events.
- Work4You core managed tool calls map to `tool` scope start/end events.
...
Work4You core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Work4You-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Work4You calls the
...
Enable it for Work4You:

```bash
export WORK4YOU_NEMO_RELAY_PLUGINS_TOML=/tmp/work4you-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
Work4You provider call
  -> nemo_relay.llm.execute(...)
    -> Work4You provider adapter callback(...)

Work4You tool call
  -> nemo_relay.tools.execute(...)
    -> Work4You authorization and dispatch callback(...)
```
...
middleware for a local Work4You run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export WORK4YOU_HOME=/tmp/work4you-middleware-test/work4you-home
mkdir -p "$WORK4YOU_HOME" /tmp/work4you-middleware-test/nemo-relay
...
cat > /tmp/work4you-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/work4you-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/work4you-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Work4You Middleware E2E"
agent_version = "local"
...
export WORK4YOU_NEMO_RELAY_PLUGINS_TOML=/tmp/work4you-middleware-test/nemo-relay/plugins.toml
