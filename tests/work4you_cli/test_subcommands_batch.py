...
def test_config_get_unset_subcommands_parse():
    """`work4you config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="work4you")
...
# ── deprecated `work4you login` fails gracefully, not with argparse error ────
#
# `work4you login` is a removed command; its handler (`login_command` in
# `work4you_cli/auth.py`) prints a deprecation notice pointing at `work4you auth` /
# `work4you model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `work4you --help`.
