"""CLI subcommand: `work4you curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`work4you curator adopt <name>` hands one over"
    )
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `work4you curator adopt <name>`, "
          "or all with `work4you curator adopt --all-unmanaged`")
    return 0
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.work4you/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `work4you curator rollback <id>`; "
        "whole-tree snapshots remain available via `work4you curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `work4you curator ledger` for entry ids, or use "
    ...
    print(
        "\nThis will replace the current ~/.work4you/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.work4you/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.work4you/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `work4you curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`work4you curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by work4you_cli.main fallthrough)."""
