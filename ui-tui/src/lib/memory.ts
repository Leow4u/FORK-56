...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because WORK4YOU_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.WORK4YOU_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.work4you', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `work4you-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.WORK4YOU_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.WORK4YOU_HEAPDUMP_MAX_BYTES?.trim()
