...
The canonical transcript store is SQLite via `SessionDB` (from `work4you_state`). The
...
Counts consecutive restarts via a JSON file (`{WORK4YOU_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `work4you update`, `work4you gateway restart`,
or `/restart`.
