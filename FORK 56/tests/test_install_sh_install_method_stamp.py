...
($INSTALL_DIR), not into the shared $WORK4YOU_HOME.

Background (shared-$WORK4YOU_HOME bug)
------------------------------------
$WORK4YOU_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.work4you:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$WORK4YOU_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``work4you update``).
...
``~/.work4you/work4you``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$WORK4YOU_HOME/\.install_method"', text), (
        "install.sh must not stamp $WORK4YOU_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `work4you update`"
    )
