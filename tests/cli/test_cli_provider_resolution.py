...
from work4you_cli import main as work4you_main
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "work4you"},
    )
    config = {
        "model": {"default": "tencent/hy3:free", "provider": "work4you"},
        "providers": {
            "gmk-lan": {
                "name": "GMK Local",
    ...
    monkeypatch.setattr("work4you_cli.config.load_config", lambda: config)
    monkeypatch.setattr("work4you_cli.runtime_provider.load_config", lambda: config)

    shell = cli.Work4YouCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "work4you_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_work4you_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in Work4YouCLI.__init__")

    monkeypatch.setattr("work4you_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("work4you_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.Work4YouCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("work4you_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_work4you_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    import yaml

    config_home = tmp_path / "work4you"
    config_home.mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "work4you_cli.auth.get_provider_auth_state",
        lambda provider: {
            "access_token": "work4you-token",
            "portal_base_url": "https://portal.example.com",
        },
    )
    monkeypatch.setattr(
        "work4you_cli.auth.resolve_work4you_runtime_credentials",
        lambda *args, **kwargs: {
            "base_url": "https://inference-api.work4you.ai/v1",
            "api_key": "work4you-key",
        },
    )
    monkeypatch.setattr(
        "work4you_cli.models.get_curated_work4you_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("work4you_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("work4you_cli.models.check_work4you_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "work4you_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "work4you_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "work4you_cli.work4you_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    work4you_main._model_flow_nous(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "work4you"
    assert model["default"] == selected_model
    assert model["base_url"] == "https://inference-api.work4you.ai/v1"
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.Work4YouCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("work4you_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("work4you_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("work4you_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("work4you_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("work4you_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("work4you_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    work4you_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("work4you_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "work4you_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("work4you_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("work4you_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "work4you_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "work4you_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("work4you_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    work4you_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_work4you_login_tls_options(monkeypatch):
    monkeypatch.setattr(work4you_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "work4you_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "work4you"}},
    )
    monkeypatch.setattr("work4you_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("work4you_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("work4you_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("work4you_cli.auth.resolve_provider", lambda requested, **kwargs: "work4you")
    monkeypatch.setattr("work4you_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(work4you_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("work4you_cli.auth._login_nous", _fake_login)

    work4you_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.work4you.ai",
            inference_url="https://inference.work4you.ai/v1",
            client_id="work4you-local",
...
def test_auto_provider_name_localhost():
    from work4you_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from work4you_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "work4you_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("work4you_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("work4you_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="WORK4YOU_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "WORK4YOU_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from work4you_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
