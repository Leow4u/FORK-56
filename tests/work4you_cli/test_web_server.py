"""Tests for work4you_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_work4you_home):
    """Drop the example-dashboard fixture into the per-test WORK4YOU_HOME
    ...
    isolated ``WORK4YOU_HOME``.

    The user-plugin source is preferred over a transient
    ``WORK4YOU_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, work4you-achievements, model providers
    ...
    user_plugins_dir = get_work4you_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `work4you plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down WORK4YOU_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Work4You vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints WORK4YOU_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("WORK4YOU_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("WORK4YOU_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_work4you_home):
        """Create a TestClient and isolate the state DB under the test WORK4YOU_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `work4you update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `work4you update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(work4you_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = work4you_state.SessionDB.__init__
        ...
        # the class on work4you_state covers every open the helper makes.
        monkeypatch.setattr(work4you_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        WORK4YOU_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_work4you_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from work4you_constants import get_work4you_home
        from work4you_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide WORK4YOU_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from work4you_constants import get_work4you_home

        path = get_work4you_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_work4you_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["work4you"]["workspace"] == "myws"
        assert cfg["hosts"]["work4you"]["peerName"] == "eri"
        assert cfg["hosts"]["work4you"]["environment"] == "local"
        assert cfg["hosts"]["work4you"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["work4you"]["apiKey"] == "hch-test-key"
    ...
    def test_update_work4you_returns_docker_guidance_without_spawning(self, monkeypatch):
        import work4you_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn work4you update")
        ...
        monkeypatch.setattr(web_server, "_spawn_work4you_action", fail_spawn)
        web_server._ACTION_PROCS.pop("work4you-update", None)
        web_server._ACTION_RESULTS.pop("work4you-update", None)

        resp = self.client.post("/api/work4you/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "work4you-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull work4you/work4you:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/work4you-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull work4you/work4you:latest" in line for line in status_data["lines"])

    def test_update_work4you_returns_apt_guidance_without_spawning(self, monkeypatch):
        import work4you_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn work4you update")
        ...
        assert data["update_command"] == "pkg upgrade work4you"
        assert spawned is False
    ...
    def test_update_work4you_spawns_with_action_id(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_work4you_action", fake_spawn)
        web_server._ACTION_PROCS.pop("work4you-update", None)
        web_server._ACTION_RESULTS.pop("work4you-update", None)
        ...
        assert calls == [
            (["update"], "work4you-update", {"WORK4YOU_ACTION_ID": "a" * 32})
        ]

    def test_update_work4you_reuses_running_action(self, monkeypatch):
        import work4you_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["work4you-update"] = Proc()
        web_server._ACTION_IDS["work4you-update"] = "b" * 32

        try:
            resp = self.client.post("/api/work4you/update")
        finally:
            web_server._ACTION_PROCS.pop("work4you-update", None)
            web_server._ACTION_IDS.pop("work4you-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/work4you_cli/test_setup_hidden_env.py.
        """
        from work4you_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Work4You provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from work4you_cli.config import save_env_value
        from work4you_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "work4you_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/Work4YouSetupBot/work4you_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/Work4YouSetupBot/work4you_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "work4you_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_work4you_action", fail_spawn_action)
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from work4you_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``work4you model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from work4you_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from work4you_cli import profiles as profiles_mod
        from work4you_cli.config import custom_endpoint_key_env
        from work4you_constants import get_work4you_home

        default_home = get_work4you_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from work4you_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from work4you_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from work4you_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from work4you_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from work4you_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from work4you_cli.work4you_subscription import Work4YouSubscriptionFeatures

        calls = 0
        features = Work4YouSubscriptionFeatures(
            subscribed=False,
            work4you_auth_present=False,
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Work4You Subscription rows, xAI TTS without Grok OAuth, and
        ...
        # Logged out of Work4You Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "work4you_cli.work4you_subscription.get_work4you_portal_account_info",
            lambda *a, **k: Work4YouPortalAccountInfo(
        ...
        assert by_name["Work4You Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_work4you_provider_reports_needs_work4you_auth(self, monkeypatch):
        """Selecting a managed Work4You row while logged out flags needs_work4you_auth.
        ...
        (ensure_work4you_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Work4You OAuth flow.
        """
        from work4you_cli.work4you_account import Work4YouPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Work4You Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_work4you_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'work4you' provider string (the runtime
        # maps it to the Browser Use cloud through the Work4You Tool Gateway).
        from work4you_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "work4you"
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "work4you"
        save_config(config)
        ...
        assert "work4you@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from work4you_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from work4you_cli.web_server import _denormalize_config_from_web
...
class TestStatusInstallId:
    ...
    Work4You home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_work4you_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """WORK4YOU_HOME=<root> and WORK4YOU_HOME=<root>/profiles/<name> resolve to
        the same id file — profiles share one physical install identity."""
        import work4you_cli.web_server as ws

        root = tmp_path / "work4you-root"
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from work4you_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.work4you/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_work4you_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(work4you_home, name="ocean"):
        themes_dir = work4you_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="work4you-theme-bootstrap">')
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``work4you_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in work4you_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_work4you_home):
        ...
        # Pin the SessionDB to the isolated WORK4YOU_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_work4you_home, _install_example_plugin):
        ...
        installs it into the per-test ``WORK4YOU_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into WORK4YOU_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, work4you-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (work4you-achievements is loaded by default).
        resp = self.client.get("/api/plugins/work4you-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        WORK4YOU_HOME override scopes a request to another profile."""
        from work4you_constants import (
            reset_work4you_home_override,
            set_work4you_home_override,
        )
        ...
        monkeypatch.setenv("WORK4YOU_HOME", str(launch_home))
        from work4you_cli import web_server
        token = set_work4you_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets WORK4YOU_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the work4you root's plugins/ directory."""
        root = tmp_path / "work4you-root"
...
# or ``sh -c 'printf …'``) instead of the real ``work4you --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"work4you-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "WORK4YOU_CWD": str(tmp_path),
            "WORK4YOU_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["WORK4YOU_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from work4you_cli.pty_bridge import PtyUnavailableError
        ...
        import work4you_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from work4you_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import work4you_cli.main as cli_main
    import work4you_cli.web_server as ws
    ...
    gateway_url = env.get("WORK4YOU_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_work4you_home, _install_example_plugin):
        ...
        the per-test ``WORK4YOU_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_work4you_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("WORK4YOU_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under WORK4YOU_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("WORK4YOU_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/work4you"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/work4you/ds-assets/bg.png)" in prefixed.text
        assert "url(/work4you/fonts-terminal/x.woff2)" in prefixed.text
