 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Work4You by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.work4you-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(work4youHome) {
  return path.join(work4youHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.work4you/work4you. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(work4youHome) {
  if (!work4youHome) {
    return null
  }

  const candidate = path.join(work4youHome, 'work4you', 'scripts', installScriptName())
...
function cachedScriptPath(work4youHome, commit) {
  return path.join(bootstrapCacheDir(work4youHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Leow4u/FORK-56/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(work4youHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(work4youHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, work4youHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass WORK4YOU_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          WORK4YOU_HOME: work4youHome || process.env.WORK4YOU_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, work4youHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, work4youHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--work4you-home', work4youHome]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(work4youHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, work4youHome, emit })
