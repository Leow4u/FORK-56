...
Copies bundled skills from the repo's skills/ directory into ~/.work4you/skills/
...
# cron, kanban workers) import this module once under the launch WORK4YOU_HOME
# and later scope requests to a different profile via
# set_work4you_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped WORK4YOU_HOME on every call.
_WORK4YOU_HOME_AT_IMPORT = WORK4YOU_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _work4you_home() -> Path:
    """Return the active profile's WORK4YOU_HOME at call time."""
    configured = Path(WORK4YOU_HOME)
    if configured != _WORK4YOU_HOME_AT_IMPORT:
        return configured
...
# Marker file written by `work4you profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.work4you profile).
# When present in WORK4YOU_HOME, sync_skills() is a no-op so neither the
# installer, `work4you update`, nor a direct sync re-injects bundled skills.
...
def _get_bundled_dir() -> Path:
    ...
    Checks WORK4YOU_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.work4you/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.work4you/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``work4you skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `work4you skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    """
    # Opt-out: a profile (named or the default ~/.work4you) that wrote the
    ...
    if (_work4you_home() / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.work4you/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `work4you update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `work4you skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``WORK4YOU_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.work4you/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``WORK4YOU_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `work4you skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `work4you update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``work4you update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``work4you update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``work4you skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `work4you skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes WORK4YOU_HOME/.no-bundled-skills so the
    installer, ``work4you update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``work4you skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _work4you_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _work4you_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
            ...
            changed = existed
            message = (
                "Opted back in. The next `work4you update` (or `work4you skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_work4you_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.work4you/skills/ ...")
