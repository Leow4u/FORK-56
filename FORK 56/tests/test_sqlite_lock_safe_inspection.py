"""POSIX advisory locks must survive Work4You' own database inspection.
...
Work4You used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `work4you sessions
...
out across Work4You' inspection calls.
"""
...
def clean_registry():
    ...
    import work4you_cli.sqlite_safe_read as mod
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_work4you_fail_close", False):
    ...
    conn._work4you_fail_close = True
    ...
    conn._work4you_fail_close = False
    conn.close()
    assert not has_live_connection(db)
    assert read_header_bytes_preopen(db, length=16) is not None
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import work4you_cli.sqlite_safe_read as ssr
