...
from work4you_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import work4you_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $WORK4YOU_HOME/node is
...
def _has_work4you_browser() -> bool:
    ...
    # install.sh installs globally into $WORK4YOU_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-Work4YouHome", str(get_work4you_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = work4you_subprocess_env(inherit_credentials=False)
