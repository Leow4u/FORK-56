...
#
# The whole job: wait for the Desktop to exit, run `work4you update`, tell the
...
# the WORK4YOU_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (WORK4YOU_HOME/work4you)
...
WORK4YOU_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
WORK4YOU_HOME="${WORK4YOU_HOME:-${TMPDIR:-/tmp}}"
MARKER="$WORK4YOU_HOME/.work4you-update-in-progress"
LOG_DIR="$WORK4YOU_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$WORK4YOU_HOME/.work4you-update-result.json"
STATUS="${TMPDIR:-/tmp}/work4you-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Work4You update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Work4You update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Work4You update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Work4You update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
start_ui() {
  ...
  # teardown TERM killed the shim ~1s into `work4you update` (2026-08-14 16:44,
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/work4you-update-ui-$$" \
...
linux_gate() {
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Work4You to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Work4You.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Work4You.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Work4You (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Work4You also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Work4You to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${WORK4YOU_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${WORK4YOU_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `work4you update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Work4You while the
...
# so the shim server and the later `work4you update` subprocess both inherit
...
# update_lock.py (the `work4you update` child adopts it via process ancestry).
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Work4You window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Work4You fully and try again."
...
WORK4YOU_BIN="$INSTALL_ROOT/venv/bin/work4you"
[ -x "$WORK4YOU_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $WORK4YOU_BIN is missing. The install needs repair (run the Work4You installer or work4you doctor)."; log "$FINAL_MSG"; exit 3; }

# Run FROM the install root: `work4you update` resolves the tree it mutates
...
log "running: work4you update --yes --gateway --branch $BRANCH"
publish_stage "Updating code and dependencies"
OUT="$("$WORK4YOU_BIN" update --yes --gateway --branch "$BRANCH" 2>&1)"; CODE=$?
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "work4you update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Work4You windows") is not retryable.
...
# Truthful completion: `work4you update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside work4you update; retrying build"
  publish_stage "Rebuilding Desktop"
  "$WORK4YOU_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run work4you desktop --force-build from a terminal to retry."
    exit 6
  }
fi

if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run work4you debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
