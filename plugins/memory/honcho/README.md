...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/work4you>
...
```bash
work4you memory setup honcho   # configure Honcho directly (works on a fresh install)
work4you memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
work4you config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.work4you/.env
```

> `work4you honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `work4you memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `work4you model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$WORK4YOU_HOME/honcho.json` | Profile-local (isolated Work4You instances) |
| 2 | `~/.work4you/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Work4You profile: `work4you` (default) or `work4you_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `work4you honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Work4You session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`work4you`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`work4you`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-work4you`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening work4you in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Work4You session ID (timestamp + hex). Every `work4you` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Work4You profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`work4you`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Work4You profile: `work4you` (default) or `work4you_<profile>` (e.g. `work4you -p coder` -> host key `work4you_coder`). Older `work4you.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `WORK4YOU_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `work4you`) |
...
|---------|-------------|
| `work4you memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `work4you honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `work4you memory setup`) |
| `work4you honcho status` | Show resolved config for active profile |
| `work4you honcho enable` / `disable` | Toggle Honcho for active profile |
| `work4you honcho mode <mode>` | Change recall or observation mode |
| `work4you honcho peer --user <name>` | Update user peer name |
| `work4you honcho peer --ai <name>` | Update AI peer name |
| `work4you honcho tokens --context <N>` | Set context token budget |
| `work4you honcho tokens --dialectic <N>` | Set dialectic max chars |
| `work4you honcho map <name>` | Map current directory to a session name |
| `work4you honcho sync` | Create host blocks for all Work4You profiles |
