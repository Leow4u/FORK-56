...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'work4you serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/work4you serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m work4you_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/work4you --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Work4You Runtime\\work4you.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('work4you chat --query serve'), false)
