...
# identify Work4YouAgent traffic — matching other Work4You outbound surfaces
# that already set ``Work4YouAgent/<version>`` for platform-partner attribution.
try:
    from work4you_cli import __version__ as _WORK4YOU_VERSION
except Exception:
    _WORK4YOU_VERSION = "unknown"
_WORK4YOU_SLACK_USER_AGENT_PREFIX = f"Work4YouAgent/{_WORK4YOU_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `work4you gateway setup` "
                "or in the active profile's ~/.work4you/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `work4you gateway setup` "
                "or add it to your active profile's ~/.work4you/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `work4you gateway setup` "
                "or in the active profile's ~/.work4you/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `work4you gateway setup` "
                "or add it to your active profile's ~/.work4you/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_work4you_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_WORK4YOU_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Work4You has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Work4You can "
                    "be removed from the Slack app manifest via "
                    "`work4you slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/work4you$")

            @self._app.command(_slash_pattern)
            async def handle_work4you_command(ack, command):
            ...
            for _action_id in (
                "work4you_approve_once",
                "work4you_approve_session",
                "work4you_approve_always",
                "work4you_deny",
            ):
            ...
            for _action_id in (
                "work4you_confirm_once",
                "work4you_confirm_always",
                "work4you_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("work4you_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^work4you_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("work4you_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'work4you slack manifest' if unsure), and (b) the other bot's "
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Work4You session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Work4You owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_work4you_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_work4you_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_work4you_no_thread_response"):
        ...
        force_process = bool(event.get("_work4you_force_process"))
        ...
        # the shape produced by peer Work4You agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "work4you_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "work4you_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "work4you_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "work4you_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "work4you_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "work4you_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``work4you_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``work4you_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"work4you_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "work4you_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "work4you_confirm_once": "once",
            "work4you_confirm_always": "always",
            "work4you_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "work4you_approve_once": "once",
            "work4you_approve_session": "session",
            "work4you_approve_always": "always",
            "work4you_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "work4you_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/work4you <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/work4you
        ...
        if slash_name in {"work4you", ""}:
            # Legacy /work4you <subcommand> [args] routing + free-form questions.
        ...
        # "Running /cmd…" ack shown by handle_work4you_command).
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_work4you_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    from work4you_cli.config import get_env_value, remove_env_value, save_env_value
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under WORK4YOU_HOME, and print
        ...
        try:
            ...
            target = Path(get_work4you_home()) / "slack-manifest.json"
    ...
    print()
    print_info("   Full guide: https://work4you.ai/docs/user-guide/messaging/slack/")
    print()
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `work4you setup` to install Slack support.",
