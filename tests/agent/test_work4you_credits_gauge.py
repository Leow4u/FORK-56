"""Tests for the Work4You-credits subscription % gauge in build_work4you_credits_snapshot.
...
from agent.account_usage import build_work4you_credits_snapshot, render_account_usage_lines
...
def _acct(**kwargs):
    ...
    kwargs.setdefault("portal_base_url", "https://portal.work4you.ai")
...
def test_gauge_present_with_monthly_credits():
    snap = build_work4you_credits_snapshot(_acct(
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_work4you_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=Work4YouPortalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
        paid_service_access_info=Work4YouPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
...
def test_failopen_none_and_logged_out():
    ...
    assert build_work4you_credits_snapshot(_acct(logged_in=False)) is None
