...
Work4You uses a SQLite database (`~/.work4you/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `work4you_state.py`


## Architecture Overview

```
~/.work4you/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `work4you_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `work4you_state.py` for the exact SQL.
...
Multiple work4you processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.work4you/state.db
...
Default path: `~/.work4you/state.db`

This is derived from `work4you_constants.get_work4you_home()` which resolves to
`~/.work4you/` by default, or the value of `WORK4YOU_HOME` environment variable.
