...
def test_output_path_rejects_work4you_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    work4you_home = tmp_path / "work4you-home"
    work4you_home.mkdir()
    monkeypatch.setattr(file_safety, "_work4you_home_path", lambda: work4you_home)
    monkeypatch.setattr(file_safety, "_work4you_root_path", lambda: work4you_home)

    target = work4you_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    work4you_home = tmp_path / "work4you-home"
    token_dir = work4you_home / "mcp-tokens"
