...
- OpenViking server running and reachable from Work4You

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Work4You can identify older servers that expose the legacy status-only health
...
connection values into Work4You, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.work4you/.env`; for a named profile use
...
OpenViking's server config is separate from Work4You:
...
Work4You-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `work4you` | Work4You peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Work4You lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Work4You sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Work4You built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Work4You action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Work4You
...
file URIs, such as
`viking://user/peers/work4you/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/work4you/memories/preferences/mem_abc123.md`. Files
