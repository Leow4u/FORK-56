...
For occasional contributors and PR authors. Full developer docs: https://work4you.ai/docs/developer-guide/
...
Config: `~/.work4you/config.yaml` (settings), `~/.work4you/.env` (API keys) — both under `$WORK4YOU_HOME` when it is set.
...
`_WORK4YOU_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_work4you_home()` for paths,
...
- Tests auto-redirect `WORK4YOU_HOME` to temp dirs — never touch real `~/.work4you/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/work4you_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_work4you_home()` from `work4you_constants` for all paths (profile-safe)
