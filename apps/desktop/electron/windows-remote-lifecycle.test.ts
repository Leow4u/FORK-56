  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Work4You is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Work4You is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Work4You's\\python.exe" }, 'inspect', [
    'C:\\x y\\work4you.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'work4you_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Work4You''s/)
  assert.match(script, /C:\\x y\\work4you\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { work4youPath: lock.work4youPath, work4youHome: lock.work4youHome }
