...
Requires:
    ...
    manually: ``<work4you-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Work4You owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        work4you_action = data.get("work4you_action", "")
        session_key = data.get("session_key", "")

        if not work4you_action or not session_key:
        ...
        choice = choice_map.get(work4you_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="work4you_approve",
            data={**btn_data_base, "work4you_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="work4you_approve",
                data={**btn_data_base, "work4you_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="work4you_approve",
                    data={**btn_data_base, "work4you_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="work4you_approve",
            data={**btn_data_base, "work4you_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.work4you/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Work4You
