...
Set via `work4you model` (picker) or `work4you setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://work4you.ai/docs/integrations/providers
...
| work4you | OAuth device code | `work4you auth add work4you` (or `WORK4YOU_API_KEY`) |
| openai-codex | OAuth | `work4you auth add openai-codex` |
| qwen-oauth | OAuth | `work4you auth add qwen-oauth` |
| minimax-oauth | OAuth | `work4you auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`work4you auth`).
Fallback chain when the primary fails: `work4you fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`work4you_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   work4you config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
