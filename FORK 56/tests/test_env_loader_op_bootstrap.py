...
1. ``load_work4you_dotenv()`` auto-loads ``~/.work4you/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_work4you_dotenv(work4you_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "work4you_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
