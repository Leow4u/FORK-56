"""disk_cleanup — ephemeral file cleanup for Work4You.
...
Rules:
  ...
  - empty dirs    → always delete (under WORK4YOU_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly WORK4YOU_HOME and /tmp/work4you-*
Never touches: ~/.work4you/logs/ or any system directory.
"""
...
try:
    ...
    def get_work4you_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("WORK4YOU_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".work4you").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$WORK4YOU_HOME/logs/``."""
    return get_work4you_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$WORK4YOU_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under WORK4YOU_HOME or ``/tmp/work4you-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    work4you_home = get_work4you_home()
    try:
        path.resolve().relative_to(work4you_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/work4you-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("work4you-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "work4you", "backups", "profiles", ".worktrees",
...
# Paths under $WORK4YOU_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so WORK4YOU_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        work4you_home = get_work4you_home()
        for parent in ("cron", "cronjobs"):
            base = work4you_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside WORK4YOU_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under WORK4YOU_HOME, but never recurse into known
    # durable state trees.  Some installs place the Work4You checkout, venv,
    # and desktop build under WORK4YOU_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    work4you_home = get_work4you_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in work4you_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    work4you_home = get_work4you_home()
    try:
        rel = path.resolve().relative_to(work4you_home)
        ...
        # Path isn't under WORK4YOU_HOME (e.g. /tmp/work4you-*) — fall through.
        pass
