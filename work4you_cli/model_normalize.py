...
- **Aggregators** (OpenRouter, Work4You, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  Work4You revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, Work4You, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Work4You' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Work4You provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``work4you_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from work4you_cli.models import normalize_copilot_model_id
