...
the actual `setup.sh` that creates the project workspace, configures Work4You
...
> [Work4You/kanban-video-pipeline](https://github.com/Work4You/kanban-video-pipeline).
...
2. **Create profiles** — `work4you profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.work4you/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `work4you kanban create` assigned to the director
...
```bash
work4you profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.work4you/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.work4you/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Work4You
...
Each profile gets a `SOUL.md` at `~/.work4you/profiles/<name>/SOUL.md` that
...
```bash
work4you kanban create "Direct production of <video title>" \
    --assignee director \
...
the Work4You `.env` (`${WORK4YOU_HOME:-$HOME/.work4you}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _work4you_env="${WORK4YOU_HOME:-$HOME/.work4you}/.env"
    if grep -q "^${var}=" "$_work4you_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_work4you_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_work4you_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY work4you ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY work4you OPENROUTER_API_KEY || exit 1
# ...
```
