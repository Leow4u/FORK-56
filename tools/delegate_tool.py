...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``work4you-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``work4you-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Work4You Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. work4you-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (work4you-cli,
    # work4you-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Work4You Portal).
    ...
    #
    # Work4You Portal is dual-wire within a single provider: anthropic/* → Messages,
    # everything else → chat_completions. Same-provider inheritance would pin a
    # child Work4You/Qwen subagent onto the parent's Claude Messages wire (or the
    # reverse). agent_init honors an explicit api_mode above its work4you branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"work4you", "work4you-portal", "work4you"}:
        from work4you_cli.providers import work4you_api_mode

        effective_api_mode = work4you_api_mode(effective_model)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.work4you/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_work4you_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("WORK4YOU_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from work4you_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # WORK4YOU_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Work4You-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("WORK4YOU_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        # SYNCHROWORK4YOU execution so the result returns in this same turn instead
        ...
        if not _async_ok:
            ...
            # with that id in X-Work4You-Session-Id once the batch completes.
            ...
            # WORK4YOU_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `work4you -z`, a cron job, a Kanban "
                    "worker, or a stateless HTTP endpoint). The subagent(s) ran "
                    "SYNCHROWORK4YOULY and the result is included above."
                )
        ...
        try:
            ...
            _source = get_session_env("WORK4YOU_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("WORK4YOU_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and WORK4YOU_SESSION_KEY is not in the CLI
        ...
        if isinstance(_cap_result, dict):
            _cap_result["note"] = (
                ...
                "SYNCHROWORK4YOULY and the result is included above. Raise "
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, work4you, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'work4you auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Work4You config.

    Prefer the shared persistent loader because it follows the active
    WORK4YOU_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``WORK4YOU_IGNORE_USER_CONFIG=1`` (``work4you chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("WORK4YOU_IGNORE_USER_CONFIG") == "1"
