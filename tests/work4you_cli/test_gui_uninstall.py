"""Tests for work4you_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import work4you_cli.gui_uninstall as gu


def _make_agent(work4you_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = work4you_home / "work4you"
    (agent_root / "work4you_cli").mkdir(parents=True)
    (agent_root / "work4you_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(work4you_home: Path) -> None:
    """Create the source-built GUI artifacts a `work4you desktop` run produces."""
    desktop = work4you_home / "work4you" / "apps" / "desktop"
    ...
    (work4you_home / "work4you" / "node_modules").mkdir(parents=True)
    (work4you_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(work4you_home: Path) -> None:
    (work4you_home / "config.yaml").write_text("x: 1\n")
    (work4you_home / ".env").write_text("KEY=secret\n")
    (work4you_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(work4you_home)
    _make_gui_build(work4you_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(work4you_home)
    ...
    assert summary["work4you_home"] == str(work4you_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `work4you desktop` installs is removable."""
    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))

    from work4you_cli import linux_desktop_entry as lde
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    work4you_home = tmp_path / ".work4you"
    _make_agent(work4you_home)
    icon = lde.icon_path(work4you_home / "work4you")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(work4you_home)
    ...
    assert lde.icon_path(work4you_home / "work4you").exists()
    # The agent itself survives a GUI uninstall.
    assert (work4you_home / "work4you" / "work4you_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".work4you")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import work4you_cli.uninstall as uninstall
