"""
Work4You Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.work4you/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Work4You PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing work4you or work4you PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Work4You" comment and following line
                if '# Work4You' in line or '# work4you' in line:
                    skip_next = True
                    continue
                if skip_next and ('work4you' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing work4you
                if 'work4you' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Work4You-owned file, and
...
def remove_wrapper_script():
    """Remove the work4you wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "work4you",
        Path.home() / ".local" / "bin" / "work4you-acp",
        Path.home() / ".local" / "bin" / "work4you",
        Path("/usr/local/bin/work4you"),
        Path("/usr/local/bin/work4you-acp"),
        Path("/usr/local/bin/work4you"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains work4you_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'work4you_cli' in content or 'work4you' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(work4you_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``work4you`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Work4You home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (work4you_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``work4you gateway run`` processes
    ...
    try:
        from work4you_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from work4you_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``WORK4YOU_HOME`` and ``WORK4YOU_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\work4you\git\cmd``, ``%LOCALAPPDATA%\work4you\git\bin``,
#      ``%LOCALAPPDATA%\work4you\git\usr\bin``, ``%LOCALAPPDATA%\work4you\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\work4you\git\`` and Node to
...
#      ``%LOCALAPPDATA%\work4you\gateway-service\`` and sometimes
...
def _work4you_path_markers(work4you_home: Path) -> list[str]:
    """Path-entry substrings that identify Work4You-owned User-PATH entries."""
    root = str(work4you_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare work4you install dir.
    markers = [root + "\\work4you", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if WORK4YOU_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "work4you".  We don't want to catch
    # unrelated entries like "cwork4you-foo" or "ephermeral", so we look for
    # backslash-work4you as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(work4you_home: Path) -> list[str]:
    """Strip Work4You-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _work4you_path_markers(work4you_home)
            kept: list[str] = []
...
def remove_work4you_env_vars_windows() -> list[str]:
    """Delete WORK4YOU_HOME and WORK4YOU_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("WORK4YOU_HOME", "WORK4YOU_GIT_BASH_PATH"):
                try:
...
def _is_default_work4you_home(work4you_home: Path) -> bool:
    """Return True when ``work4you_home`` points at the default (non-profile) root."""
    try:
        from work4you_constants import get_default_work4you_root
        return work4you_home.resolve() == get_default_work4you_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its WORK4YOU_HOME directory.
    ...
    current WORK4YOU_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m work4you_cli.main` so we don't depend on a `work4you`
    #    wrapper that may be half-removed mid-uninstall.
    work4you_invocation = [_sys.executable, "-m", "work4you_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                work4you_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's WORK4YOU_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    summary = gui_install_summary(work4you_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {work4you_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(work4you_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Work4You agent at {work4you_home / 'work4you'}")
        print(f"  • Your config, sessions, and secrets under {work4you_home}")
        print()
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie WORK4YOU_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_work4you_home(work4you_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.work4you data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {work4you_home / 'config.yaml'}")
    print(f"  Secrets: {work4you_home / '.env'}")
    print(f"  Data:    {work4you_home / 'cron/'}, {work4you_home / 'sessions/'}, {work4you_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their WORK4YOU_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, work4you_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Work4You PATH entries from shell configs / Windows User PATH")
    print("  • Work4You wrapper scripts and Work4You-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Work4You config/data: {work4you_home}")
        if _is_default_work4you_home(work4you_home):
        ...
        print(f"  • Keep Work4You config/data: {work4you_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``work4you`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$WORK4YOU_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in work4you_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\work4you\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(work4you_home))))
        ...
        log_info("Removing WORK4YOU_HOME / WORK4YOU_GIT_BASH_PATH User env vars...")
        removed_env = remove_work4you_env_vars_windows()
        if removed_env:
    ...
    log_info("Removing work4you command...")
    ...
    #     (only when they still point into this Work4You home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Work4You-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(work4you_home)
    ...
    #     step-5 rmtree(work4you_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE WORK4YOU_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from work4you_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(work4you_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.work4you/, just remove the work4you subdir
            if work4you_home in project_root.parents or project_root.parent == work4you_home:
    ...
    #     under WORK4YOU_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(work4you_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(work4you_home)
    ...
    # 5. Optionally remove ~/.work4you/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile WORK4YOU_HOME dirs live under
        ...
        try:
            if work4you_home.exists():
                shutil.rmtree(work4you_home)
                log_success(f"Removed {work4you_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {work4you_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {work4you_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {work4you_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://work4you.ai/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://work4you.ai/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m work4you_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import work4you_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``work4you_constants`` + ``work4you_cli.colors``
    (and lazily ``work4you_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m work4you_cli.uninstall")
    parser.add_argument(
        "--mode",
