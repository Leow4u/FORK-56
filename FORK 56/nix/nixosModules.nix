# nix/nixosModules.nix — the NixOS module for work4you
#
...
#
# Container mode: work4you runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $WORK4YOU_HOME/.env
# and read by work4you at startup — no container recreation needed for env changes.
#
# Tool resolution: the work4you wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.work4you;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      work4you = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      work4youHome = "${cfg.stateDir}/.work4you";
      ...
      # WORK4YOU_HOME via addToSystemPackages, so they can save settings through the
      ...
      # work4you user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "work4you-container-entrypoint" ''
        set -eu

        WORK4YOU_UID="''${WORK4YOU_UID:?WORK4YOU_UID must be set}"
        WORK4YOU_GID="''${WORK4YOU_GID:?WORK4YOU_GID must be set}"

        # ── Group: ensure a group with GID=$WORK4YOU_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$WORK4YOU_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="work4you"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$WORK4YOU_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$WORK4YOU_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$WORK4YOU_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$WORK4YOU_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="work4you"
          TARGET_HOME="/home/work4you"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$WORK4YOU_UID" -g "$WORK4YOU_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$WORK4YOU_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$WORK4YOU_UID:$WORK4YOU_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure WORK4YOU_HOME is owned by the target user.
        ...
        if [ -n "''${WORK4YOU_HOME:-}" ] && [ -d "$WORK4YOU_HOME" ]; then
          find "$WORK4YOU_HOME" \! -user "$WORK4YOU_UID" -exec chown "$WORK4YOU_UID:$WORK4YOU_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/work4you-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/work4you-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/work4you ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/work4you
          chmod 0440 /etc/sudoers.d/work4you
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$WORK4YOU_UID" --regid="$WORK4YOU_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $WORK4YOU_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "work4you-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        work4you_bin=${containerDataDir}/current-package/bin/work4you
      '';

      # Default: /var/lib/work4you/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the work4you group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit work4youHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.work4you =
        common.sharedOptions {
          defaultPackage = work4you;
          defaultPackageText = lib.literalExpression "work4you.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/work4you";
              description = "State directory. Contains .work4you/ subdir (WORK4YOU_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the work4you CLI to environment.systemPackages and export
                WORK4YOU_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.work4you symlink to the service
                  stateDir. These users are automatically added to the work4you group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.work4you.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the work4you CLI to system PATH and export WORK4YOU_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.work4you/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.WORK4YOU_HOME = work4youHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.work4you";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.work4you.workingDirectory;
                optionPath = "services.work4you";
              }
              ++ [
                {
                  ...
                  message = "services.work4you: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the work4you user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.work4you: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed work4you
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure work4you is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${work4youHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${work4youHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."work4you-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  mkdir -p ${work4youHome}
                  mkdir -p ${cfg.stateDir}/home
                  mkdir -p ${cfg.workingDirectory}
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${work4youHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${work4youHome} ${cfg.workingDirectory}
                  ...
                  find ${work4youHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${work4youHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${work4youHome}/$_subdir"
                    chmod 2770 "${work4youHome}/$_subdir"
                    find "${work4youHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg work4youHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${work4youHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${work4youHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${work4youHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.work4you";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${work4youHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "work4you: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.work4you -> stateDir/.work4you for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.work4you = {
              ...
              # $WORK4YOU_HOME/.env by the activation script. load_work4you_dotenv()
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.work4you = {
              description = "Work4You Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  WORK4YOU_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  WORK4YOU_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env WORK4YOU_UID="$WORK4YOU_UID" \
                    --env WORK4YOU_GID="$WORK4YOU_GID" \
                    --env WORK4YOU_HOME=${containerDataDir}/.work4you \
                    --env WORK4YOU_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/work4you gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
