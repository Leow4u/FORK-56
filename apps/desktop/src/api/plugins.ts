...
async function activeConnection(): Promise<Work4YouConnection> {
  ...
  return window.work4youDesktop.getConnection(getApiRequestProfile())
}

/** Options for a plugin REST call — mirrors the app's own `work4youDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see Work4YouApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.work4youDesktop?.api) {
    throw new Error('Work4You desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return work4youApi<T>({
