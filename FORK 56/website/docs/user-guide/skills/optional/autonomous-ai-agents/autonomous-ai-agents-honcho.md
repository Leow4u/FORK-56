---
title: "Honcho — Configure and troubleshoot Honcho memory for Work4You"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Work4You"
---
...
Configure and troubleshoot Honcho memory for Work4You.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `work4you skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Work4You

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Work4You profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/work4you#running-honcho-locally-with-work4you

### Verify

```bash
work4you honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Work4You profile's AI peer
...
Honcho models conversations as interactions between **peers**. Work4You creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Work4You instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Work4You run |
| `global` | Single session across all directories |

Manual override: `work4you honcho map my-project-name`
...
Each Work4You profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block work4you.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `work4you.coder` host block in `honcho.json`
...
```bash
work4you honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Work4You when Honcho memory is active.
...
Config file: `$WORK4YOU_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `work4you honcho setup`. Ensure `memory.provider: honcho` is in `~/.work4you/config.yaml`.

### Memory not persisting across sessions
Check `work4you honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `work4you profile create <name> --clone`. For existing profiles: `work4you honcho sync`.
...
| Command | Description |
|---------|-------------|
| `work4you honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `work4you honcho status` | Show resolved config, connection test, peer info for active profile |
| `work4you honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `work4you honcho disable` | Disable Honcho for the active profile |
| `work4you honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `work4you honcho peers` | Show peer identities across all profiles |
| `work4you honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `work4you honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `work4you honcho sessions` | List known directory-to-session-name mappings |
| `work4you honcho map <name>` | Map current working directory to a Honcho session name |
| `work4you honcho identity` | Seed AI peer identity or show both peer representations |
| `work4you honcho sync` | Create host blocks for all Work4You profiles that don't have one yet |
| `work4you honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Work4You + Honcho |
| `work4you memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `work4you memory status` | Show active memory provider and config |
| `work4you memory off` | Disable external memory provider |
