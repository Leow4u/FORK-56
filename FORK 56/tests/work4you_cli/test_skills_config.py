"""Tests for work4you_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("work4you_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("work4you_cli.config.load_config")
    ...
    @patch("work4you_cli.config.load_config")
    @patch.dict("os.environ", {"WORK4YOU_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """WORK4YOU_SESSION_PLATFORM should be used when WORK4YOU_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_work4you_platform_takes_precedence(self, tmp_path, monkeypatch):
        """WORK4YOU_PLATFORM should win over WORK4YOU_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
