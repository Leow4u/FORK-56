import type { InputEvent, Key } from '@work4you/ink'
import * as Ink from '@work4you/ink'
 ...
 * but that accepts `key.meta` on macOS — and work4you-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.WORK4YOU_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which work4you-ink decodes as meta+'d'; without this branch it
