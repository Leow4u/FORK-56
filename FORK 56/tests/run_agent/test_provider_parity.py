...
class TestBuildApiKwargsWork4YouPortal:
    def test_includes_work4you_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == work4you_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Work4You) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_work4you_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _WORK4YOU_MODEL, get_text_auxiliary_client
        work4you_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_work4you_auth", return_value=work4you_auth), \
    ...
    def test_custom_endpoint_when_no_nous(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Work4You keys are available.
