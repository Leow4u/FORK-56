"""
Photon Spectrum (iMessage) platform adapter for Work4You.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Work4You dep
...
#
# Out-of-process senders (cron subprocesses, `work4you send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_work4you_home() honors profile overrides — never hardcode ~/.work4you.
    from work4you_constants import get_work4you_home

    return get_work4you_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (work4you status, test collection, plugin discovery).
...
# artifacts so Work4You sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `work4you photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `work4you photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `work4you setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, work4you status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: work4you photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: work4you photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `work4you update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``work4you photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Work4You does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Work4You defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Work4You-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Work4You checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `work4you photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                ...
                raise PhotonSidecarStartupError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `work4you photon setup`)",
                    code="SIDECAR_DEPS_MISSING",
                    retryable=False,
                )
        # A `work4you update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `work4you send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`work4you send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Work4You gateway (which spawns the sidecar and "
                    "records its address under <work4you-home>/runtime/"
    ...
    headers = {"X-Work4You-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Work4You plugin loader at startup."""
