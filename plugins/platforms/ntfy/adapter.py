"""ntfy platform adapter (Work4You plugin).
...
a Work4You dependency.

This adapter ships as a Work4You platform plugin under
``plugins/platforms/ntfy/``. The Work4You plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "work4you-in"              # subscribe topic (incoming)
          publish_topic: "work4you-out"     # optional — defaults to topic
...
_ECHO_TAG = "work4you"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``work4you cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Work4You dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `work4you gateway status` without
