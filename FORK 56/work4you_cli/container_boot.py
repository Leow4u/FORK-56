...
``$WORK4YOU_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-work4you-setup
(the stage2 hook) has chowned the volume and seeded $WORK4YOU_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$WORK4YOU_HOME``,
    not under ``profiles/``). The dispatcher in ``work4you_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``work4you gateway start`` (no ``-p``) targets. Without it,
    bare ``work4you gateway start`` inside the container would land on
    ...
    ``$WORK4YOU_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        work4you_home: The container's WORK4YOU_HOME (typically /opt/data).
            Profiles live under ``<work4you_home>/profiles/<name>/``;
            the default profile lives at ``<work4you_home>`` itself.
    ...
    # ``work4you gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(work4you_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(work4you_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(work4you_home),
    ))

    profiles_root = work4you_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `work4you profile create` (config.yaml
            # is not — that comes later via `work4you setup`). Use it as the
            ...
            # via ``work4you -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(work4you_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = work4you_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("WORK4YOU_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the work4you args.
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``work4you`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``work4you <subcommand>``, so an
    explicit leading ``work4you`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `work4you <subcommand>`; peel an explicit work4you.
    if args and Path(args[0]).name == "work4you":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``work4you dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted WORK4YOU_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `work4you -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit work4you
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the work4you user) won't hit EACCES. See
...
def _write_reconcile_log(
    work4you_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $WORK4YOU_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted WORK4YOU_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        work4you_home=work4you_home, scandir=scandir,
    )
    for a in actions:
        print(
