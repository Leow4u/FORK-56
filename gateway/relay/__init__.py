"""Relay/connector support package for the Work4You gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``work4you gateway enroll`` writes them to
    ``~/.work4you/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``WORK4YOU_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``work4you gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from work4you_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Work4You:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``work4you gateway enroll`` CLI. Three modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Work4You Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Work4You Portal** (default): ``resolve_work4you_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Work4You Portal (default, unchanged behaviour).
        from work4you_cli.auth import resolve_work4you_access_token

        return resolve_work4you_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Work4You access token. In
    that case the runtime resolves the agent's own Work4You access token (the same
    ``resolve_work4you_access_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.work4you/.env``.
    ...
    sets neither ``WORK4YOU_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``work4you gateway enroll``: has a PINNED
        ``GATEWAY_RELAY_SECRET`` -> skipped (the secret-present guard below).
      - A self-hosted box with a relay URL but no NAS identity:
        ``resolve_work4you_access_token()`` fails -> graceful no-op.
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'work4you'}"
