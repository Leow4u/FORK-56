# Work4You Security Policy

This document describes Work4You's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Leow4u/FORK-56/security/advisories/new)
or **security@work4you.ai**. Do not open public issues for
security vulnerabilities. **Work4You does not operate a bug
bounty program.**
...
- Environment details (`work4you version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Work4You access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Work4You's documentation
...
Work4You supports two OS-level isolation postures. They address
...
Work4You supports this in two ways:

- **Work4You's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Work4You's in-process heuristics
...
Work4You filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Work4You
itself. Bugs in Work4You's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Work4You does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Work4You's own documentation, or reasonable
  operator expectations would predict — including cases where
  Work4You has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  work4you-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Work4You's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Work4You. Bugs in Work4You
...
- Work4You includes supply-chain guards for MCP server
