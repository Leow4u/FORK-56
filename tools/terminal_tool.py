...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Work4You exit
...
# display_work4you_home imported lazily at call site (stale-module safety during work4you update)
...
from tools.tool_backend_helpers import (
    ...
    work4you_tool_gateway_unavailable_message,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of work4you directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "work4you-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("WORK4YOU_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("WORK4YOU_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("WORK4YOU_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (WORK4YOU_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (WORK4YOU_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Work4You password prompt or the sudo -S password-pipe path.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``work4you=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Work4You processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Work4You processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Work4You processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, Work4YouSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.work4you/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``work4you
    ...
    be stale from ``work4you setup``). Environment values for omitted terminal
    ...
    try:
        from work4you_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from work4you_cli.config import _is_ssh_remote_tilde_cwd
    ...
    return {
        ...
        # Startup orphan reaper for work4you-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Work4You processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Work4You Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Work4You tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # restart|stop targeting work4you-gateway) must never run inside the
        ...
        # never restart. This mirrors the `work4you gateway restart` guard in
        # work4you_cli/gateway.py and the cron-path guard in work4you_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_WORK4YOU_GATEWAY") == "1":
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Work4You cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `work4you gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # work4you dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in work4you dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `work4you -z`, a "
                        ...
                        _gw_platform = _gse("WORK4YOU_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("WORK4YOU_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("WORK4YOU_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("WORK4YOU_SESSION_USER_ID", "")
                            _gw_user_name = _gse("WORK4YOU_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("WORK4YOU_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("WORK4YOU_INTERACTIVE"):
                    output += (
