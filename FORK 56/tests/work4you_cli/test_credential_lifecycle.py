...
WORK4YOU_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def work4you_home(monkeypatch, tmp_path):
    """Fresh WORK4YOU_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("WORK4YOU_HOME", str(home))
    from work4you_cli.config import invalidate_env_cache
...
def test_delete_clears_provider_models_cache(work4you_home):
    _write_env(work4you_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(work4you_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = work4you_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(work4you_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(work4you_home, OPENAI_API_KEY=old)
    _write_config(
        work4you_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = work4you_home.joinpath("config.yaml").read_text(encoding="utf-8")
