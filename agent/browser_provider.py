...
``kind: backend``) or ``~/.work4you/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        ...
        Used by :mod:`work4you_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Work4You gating, and the
