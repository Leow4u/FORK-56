...
describe("voiceFieldVisible", () => {
  ...
  it("hides every STT provider sub-field when STT is disabled", () => {
    const config = cfg({
      stt: { enabled: false, provider: "local", local: {} },
    });
  ...
  it("tracks a provider switch", () => {
    expect(
      voiceFieldVisible(
        "tts.openai.voice",
        cfg({ tts: { provider: "openai", openai: {} } }),
      ),
    ).toBe(true);
    expect(
      voiceFieldVisible(
...
describe("VOICE_CONFIG_KEYS", () => {
  it("matches the desktop Voice section key count", () => {
    expect(VOICE_CONFIG_KEYS).toHaveLength(41);
  });
});
