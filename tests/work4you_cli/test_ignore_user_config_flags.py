"""Tests for --ignore-user-config and --ignore-rules flags on `work4you chat`.
...
files. In Work4You the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.work4you/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → Work4YouCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("WORK4YOU_IGNORE_USER_CONFIG", "WORK4YOU_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``WORK4YOU_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<work4you_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With WORK4YOU_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._work4you_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("WORK4YOU_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``Work4YouCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting WORK4YOU_IGNORE_RULES=1 flips Work4YouCLI.ignore_rules True."""
        monkeypatch.setenv("WORK4YOU_IGNORE_RULES", "1")

        # Import Work4YouCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of Work4YouCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.Work4YouCLI)
        # Replicate the exact logic from cli.py Work4YouCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("WORK4YOU_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``work4you_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("WORK4YOU_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("WORK4YOU_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("WORK4YOU_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("WORK4YOU_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "WORK4YOU_IGNORE_USER_CONFIG" not in os.environ
        assert "WORK4YOU_IGNORE_RULES" not in os.environ
