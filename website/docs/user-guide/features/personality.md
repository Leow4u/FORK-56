...
description: "Customize Work4You's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Work4You's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `WORK4YOU_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Work4You is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Work4You now seeds a default `SOUL.md` automatically in:

```text
~/.work4you/SOUL.md
```

More precisely, it uses the current instance's `WORK4YOU_HOME`, so if you run Work4You with a custom home directory, it will use:
...
- Work4You creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Work4You loads `SOUL.md` only from `WORK4YOU_HOME`
- Work4You does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Work4You falls back to a built-in default identity
...
If Work4You loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `WORK4YOU_HOME`, the personality belongs to the Work4You instance itself.

That also makes it easier to teach users:
- "Edit `~/.work4you/SOUL.md` to change Work4You' default personality."
...
- how Work4You should handle uncertainty, disagreement, or ambiguity
...
## What Work4You injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Work4You falls back to a built-in default identity ("You are Work4You, an intelligent AI assistant created by Work4You..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Work4You ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Work4You, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Work4You its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.work4you/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.work4you/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-work4you)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Work4You speaks
- `display.skin` and `/skin` affect how Work4You looks in the terminal
