...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # work4you_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(work4you_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(work4you_home) / "mem0.json"
    existing = {}
...
def _setup_platform(work4you_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "work4you-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "work4you"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.work4you/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.work4you/.env (or unset it) or Work4You will keep "
    ...
    provider.save_config(provider_config, work4you_home)

    if env_writes:
        _write_env(Path(work4you_home) / ".env", env_writes)
...
def _setup_selfhosted(work4you_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "work4you-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "work4you")
...
def _setup_oss(work4you_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(work4you_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "work4you-user")
    ...
    if env_writes:
        _write_env(Path(work4you_home) / ".env", env_writes)
    _save_mem0_json(work4you_home, {"mode": "oss", "user_id": user_id, "agent_id": "work4you", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, work4you_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # work4you_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "work4you-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "work4you"
...
def _setup_oss_interactive(work4you_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], work4you_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], work4you_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'work4you-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "work4you-user")

    agent_id = input("  Agent ID [work4you]: ").strip()
    agent_id = agent_id or "work4you"
    ...
    if env_writes:
        _write_env(Path(work4you_home) / ".env", env_writes)
    _save_mem0_json(work4you_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(work4you_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(work4you_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(work4you_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(work4you_home, config, flags)
        return
