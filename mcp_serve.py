"""
Work4You MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Work4You-specific extra)

Usage:
    work4you mcp serve
    work4you mcp serve --verbose
...
logger = logging.getLogger("work4you.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using WORK4YOU_HOME."""
    try:
        from work4you_constants import get_work4you_home
        return get_work4you_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("WORK4YOU_HOME", Path.home() / ".work4you")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from work4you_constants import get_work4you_home
        directory_file = get_work4you_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("WORK4YOU_HOME", Path.home() / ".work4you")
...
class EventBridge:
    ...
    This is the Work4You equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from work4you_constants import get_work4you_home
            db_file = get_work4you_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("WORK4YOU_HOME", Path.home() / ".work4you")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Work4You MCP server with all tools registered."""
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Work4You MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
