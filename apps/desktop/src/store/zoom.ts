...
export function setZoomPercent(percent: number): void {
  window.work4youDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.work4youDesktop?.zoom) {
  void window.work4youDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.work4youDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
