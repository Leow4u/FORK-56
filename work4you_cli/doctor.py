"""
Doctor command for work4you CLI.
...
from work4you_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
WORK4YOU_HOME = get_work4you_home()
_DHH = display_work4you_home()  # user-facing display path (e.g. ~/.work4you or ~/.work4you/profiles/coder)

# Load environment variables from ~/.work4you/.env so API key checks work
_env_path = get_env_path()
load_work4you_dotenv(work4you_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from work4you_cli.colors import Colors, color
from work4you_cli.models import _WORK4YOU_USER_AGENT
from work4you_cli.vercel_auth import describe_vercel_auth
from work4you_constants import OPENROUTER_MODELS_URL
...
from work4you_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Work4You containers"
...
def _work4you_database_paths(work4you_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Work4You-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from work4you_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, work4you_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((work4you_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(work4you_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``work4you_cli.sqlite_safe_read``).
    ...
    """
    from work4you_cli.sqlite_safe_read import (
        has_live_connection,
...
def _report_database_journal_modes(
    work4you_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from work4you_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = work4you_home if work4you_home is not None else WORK4YOU_HOME
    try:
        databases = _work4you_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Work4You databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.work4you/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("WORK4YOU_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from work4you_cli.auth import get_minimax_oauth_auth_status
...
from work4you_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'work4you sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. WORK4YOU_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("WORK4YOU_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("WORK4YOU_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches work4you_cli.__version__.
    ...
    """
    try:
        from work4you_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != work4you_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'work4you update', or set "
            "work4you_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    for static in ("main-work4you", "dashboard"):
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `work4you doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from work4you_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the WORK4YOU_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("WORK4YOU_MANAGED_DIR", "").strip():
...
def run_doctor(args):
    ...
    os.environ.setdefault("WORK4YOU_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.work4you/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    try:
        import sqlite3
        from work4you_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Work4You already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and work4you_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.work4you/.env (primary location for user config)
    env_path = WORK4YOU_HOME / '.env'
    if env_path.exists():
        ...
        # matches work4you_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    # Check ~/.work4you/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from work4you_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from work4you_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.work4you/.env or run 'work4you setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from work4you_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from work4you_cli.config import atomic_config_write
        ...
        # Detect stale WORK4YOU_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives WORK4YOU_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from work4you_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("WORK4YOU_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"WORK4YOU_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `work4you setup` run)",
                )
                if should_fix:
                    if remove_env_value("WORK4YOU_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale WORK4YOU_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove WORK4YOU_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the WORK4YOU_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale WORK4YOU_MAX_ITERATIONS in .env shadows config.yaml — "
        ...
        try:
            from work4you_cli.config import load_env as _load_env_depr
            from work4you_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        from work4you_cli.auth import (
            get_work4you_auth_status_local,
        ...
        work4you_status = get_work4you_auth_status_local()
        if work4you_status.get("logged_in"):
            check_ok("Work4You Portal auth", "(logged in)")
        else:
            check_warn("Work4You Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Work4You' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed Work4You/Codex/Gemini/MiniMax rows above.
    try:
        from work4you_cli.auth import get_xai_oauth_auth_status
    ...
    for subdir_name in expected_subdirs:
        subdir_path = work4you_home / subdir_name
    ...
    soul_path = work4you_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Work4You a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Work4You Persona\n\n"
                "<!-- Edit this file to customize how Work4You communicates. -->\n\n"
                "You are Work4You, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = work4you_home / "memories"
    ...
    state_db_path = work4you_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from work4you_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from work4you_state import is_malformed_db_error, repair_state_db_schema
        ...
        # with a runaway WAL was previously invisible to every Work4You
        ...
        try:
            from work4you_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'work4you sessions optimize-storage' "
    ...
    wal_path = work4you_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "work4you"
        ...
        _cmd_link = _cmd_link_dir / "work4you"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(work4you not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/work4you → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/work4you points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/work4you → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/work4you — run 'work4you doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/work4you exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/work4you not found",
                    "(work4you command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/work4you → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/work4you symlink — run 'work4you doctor --fix'")
    ...
    try:
        from work4you_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'work4you[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `work4you update` does, so a session's first browser call
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable WORK4YOU_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Work4You one.
                        check_info(
    ...
    _section("Skills Hub")
    hub_dir = WORK4YOU_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        from work4you_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = WORK4YOU_HOME / "config.yaml"
    ...
    try:
        from work4you_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "work4you -p" in content:
                            _m = _re.search(r"work4you -p (\S+)", content)
    ...
    # `work4you doctor --live` (real network calls; bounded + read-only).
    try:
        from work4you_cli.doctor_live import maybe_run_live_checks
