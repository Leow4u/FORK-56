...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", work4you_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Work4You"],
        search_results=[{"memory": "Uses Work4You", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Work4You") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Work4You memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    ...
    assert merged["sm_source"] == "work4you"
    ...
    assert merged2["sm_source"] == "work4you"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "work4you-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", work4you_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "work4you_coder"
...
def test_container_tag_env_var_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    p = SupermemoryMemoryProvider()
    p.initialize("s1", work4you_home=str(tmp_path), platform="cli")
...
def test_ingest_conversation_uses_client_base_url(monkeypatch, base_url, expected_url):
    ...
    client._container_tag = "work4you"
    client._timeout = 1.0
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "work4you"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("work4you_cli.config.save_config", fake_save_config)
