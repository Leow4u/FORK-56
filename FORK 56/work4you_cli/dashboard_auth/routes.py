...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``WORK4YOU_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /work4you`` (Mission Control deploys) — we
    ...
    from work4you_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``work4you_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("work4you_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from work4you_cli.dashboard_auth import native_flow
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    from work4you_cli.dashboard_auth.base import RefreshExpiredError
