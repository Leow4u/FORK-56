"""Tests for work4you backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew work4you_cli.backup's datetime forward instead of sleeping.
    ...
    import work4you_cli.backup as _backup

    shim = getattr(_backup.datetime, "_work4you_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _work4you_test_shim = True
...
def _make_work4you_tree(root: Path) -> None:
    """Create a realistic ~/.work4you directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "work4you_state.db"):
    ...
    # work4you repo (should be EXCLUDED)
    (root / "work4you").mkdir(exist_ok=True)
    (root / "work4you" / "run_agent.py").write_text("# big file\n")
    (root / "work4you" / ".git").mkdir()
    (root / "work4you" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_work4you(self):
        from work4you_cli.backup import _should_exclude
        assert _should_exclude(Path("work4you/run_agent.py"))
        assert _should_exclude(Path("work4you/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        work4you_home.mkdir()
        _make_work4you_tree(work4you_home)
        ...
        import work4you_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = work4you_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, work4you_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside WORK4YOU_HOME."""
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        _make_work4you_tree(work4you_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(work4you_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Work4You backup."""
        from work4you_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Work4You configuration has been restored." in out
        assert "work4you gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        (work4you_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (work4you_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (work4you_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            work4you_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (work4you_home / "gateway.pid").write_text("4242")
        (work4you_home / "processes.json").write_text('{"live": true}')
        ...
        assert (work4you_home / "gateway.pid").read_text() == "4242"
        assert (work4you_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (work4you_home / "cron.pid").exists()
        assert not (work4you_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"work4you": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (work4you_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".work4you"
        src_home.mkdir(parents=True)
        _make_work4you_tree(src_home)

        monkeypatch.setenv("WORK4YOU_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from work4you_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".work4you"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("WORK4YOU_HOME", str(dst_home))
        ...
        # work4you should NOT be present
        assert not (dst_home / "work4you").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from work4you_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from work4you_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".work4you/", "")
            zf.writestr(".work4you/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_work4you_home(self, tmp_path, monkeypatch):
        """Backup handles empty work4you home (no files to back up)."""
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        # Only excluded dirs, no actual files
        (work4you_home / "__pycache__").mkdir()
        (work4you_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        (work4you_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = work4you_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        (work4you_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (work4you_home / "config.yaml").exists()
        assert (work4you_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`work4you import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the WORK4YOU_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (work4you_home / "config.yaml").write_text(original)
        monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (work4you_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(work4you_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside WORK4YOU_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        work4you_home = dst_home / ".work4you"
        work4you_home.mkdir()
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        store = work4you_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = work4you_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        target = work4you_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo work4you import`` onto a user-owned (or Docker/NAS volume-owned)
        WORK4YOU_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "work4you_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``work4you import`` is the
        ...
        work4you_home = tmp_path / ".work4you"
        work4you_home.mkdir()
        target = work4you_home / "helper.sh"
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from work4you_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from work4you_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from work4you_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def work4you_home(self, tmp_path):
        """Create a fake WORK4YOU_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, work4you_home):
        from work4you_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(work4you_home=work4you_home)
        db_copy = work4you_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, work4you_home, monkeypatch, capsys):
        ...
        snap_id = backup_mod.create_quick_snapshot(work4you_home=work4you_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (work4you_home / "state-snapshots" / snap_id / "manifest.json")
    ...
    def test_snapshot_includes_pairing_directories(self, work4you_home):
        ...
        (work4you_home / "platforms" / "pairing").mkdir(parents=True)
        (work4you_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (work4you_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (work4you_home / "pairing").mkdir()
        (work4you_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (work4you_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = work4you_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (work4you update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, work4you_home, capsys):
        ...
        """
        import json
        from work4you_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", work4you_home=work4you_home)
        assert first_id is not None
        first_dir = work4you_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", work4you_home=work4you_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = work4you_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, work4you_home=work4you_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, work4you_home):
        ...
        from work4you_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = work4you_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(work4you_home=work4you_home)
        copy = (
            work4you_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, work4you_home=work4you_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, work4you_home, monkeypatch):
        ...
        import work4you_cli.backup as bk
        from work4you_cli.backup import create_quick_snapshot

        board = work4you_home / "kanban" / "boards" / "work"
        ...
        copy = work4you_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``work4you update``
    ...
    def work4you_home(self, tmp_path):
        ...
        root.mkdir()
        _make_work4you_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, work4you_home):
        """Pre-update backup should include the same user data that
        ``work4you backup`` would, and should exclude the same directories."""
        from work4you_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(work4you_home=work4you_home)
        ...
        # work4you repo excluded
        assert not any(n.startswith("work4you/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, work4you_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(work4you_home=work4you_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (work4you_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, work4you_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside WORK4YOU_HOME."""
        from work4you_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def work4you_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".work4you"
        root.mkdir()
        _make_work4you_tree(root)
        # Point WORK4YOU_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("WORK4YOU_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve WORK4YOU_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(work4you_home, value):
        import yaml
        (work4you_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(work4you_home):
        d = work4you_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(work4you_home):
        d = work4you_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, work4you_home, capsys):
        ...
        self._set_mode(work4you_home, "off")
        from work4you_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(work4you_home)
        assert not self._zips(work4you_home)



    def test_config_full_mode(self, work4you_home, capsys):
        self._set_mode(work4you_home, "full")
        from work4you_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(work4you_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (work4you claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``work4you claw migrate`` runs before mutating ~/.work4you/."""
    ...
    def test_restorable_with_work4you_import(self, work4you_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Work4You
        backup — `work4you import` should accept it."""
        from work4you_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(work4you_home=work4you_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, work4you_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from work4you_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(work4you_home=work4you_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(work4you_home=work4you_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`work4you update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, work4you_home: Path, label="pre-update"):
        from work4you_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, work4you_home=work4you_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from work4you_cli.backup import restore_cron_jobs_if_emptied
        work4you_home = tmp_path / ".work4you"
        jobs_path = work4you_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(work4you_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, work4you_home=work4you_home)
        assert result is not None
...
# location, NOT under WORK4YOU_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, work4you_home: Path) -> None:
        work4you_home.mkdir(parents=True, exist_ok=True)
        (work4you_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (work4you_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (work4you_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        work4you_home = tmp_path / ".work4you"
        self._make_min_tree(work4you_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under WORK4YOU_HOME,
        ...
        # External state did NOT leak into WORK4YOU_HOME.
        assert not (work4you_home / "_external").exists()
