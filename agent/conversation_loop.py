...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in work4you_state.py — kept local
# to avoid importing work4you_state at module load time (its module-level
# DEFAULT_DB_PATH = get_work4you_home() / "state.db" breaks tests that
# monkeypatch get_work4you_home to return a str).
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.work4you/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Work4You tool use: "
    ...
    return (
        ...
        f"context, but Work4You needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Work4You config, set `model.ollama_num_ctx: 65536` "
...
def _work4you_entitlement_message(capability: str) -> str:
    try:
...
def _print_work4you_entitlement_guidance(agent, capability: str) -> bool:
    message = _work4you_entitlement_message(capability)
    if not message:
        return False
...
def _is_work4you_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "work4you":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.work4you.ai")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_work4you_inference_route(provider, base_url):
        return _work4you_entitlement_message(capability)
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `work4you auth reset anthropic`. Until "
...
def _try_refresh_work4you_paid_entitlement_credentials(agent) -> bool:
    """Refresh Work4You runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_work4you_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Work4You' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
def run_conversation(
    ...
    # Adopt any ~/.work4you/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see work4you_state.classify_persistence_error).
    ...
    # all run inside Codex). Default Work4You path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Work4You internals.
        #
        # Work4You invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Work4You tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Work4You Portal rate limit guard ──────────────────────
            # If another session already recorded that Work4You is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "work4you":
                try:
                    from agent.work4you_rate_guard import (
                        work4you_rate_limit_remaining,
                        format_remaining as _fmt_work4you_remaining,
                    )
                    _work4you_remaining = work4you_rate_limit_remaining()
                    if _work4you_remaining is not None and _work4you_remaining > 0:
                        _work4you_msg = (
                            f"Work4You Portal rate limit active — "
                            f"resets in {_fmt_work4you_remaining(_work4you_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_work4you_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_work4you_msg}")
            ...
            try:
                ...
                try:
                    from work4you_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("WORK4YOU_DUMP_REQUESTS"):
                ...
                # Clear Work4You rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Work4You.
                if agent.provider == "work4you":
                    try:
                        from agent.work4you_rate_guard import clear_work4you_rate_limit
                        clear_work4you_rate_limit()
                    except Exception:
                        pass
                from agent import relay_llm
                ...
                if (
                    ...
                    and _is_work4you_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.work4you_paid_entitlement_refresh_attempted
                ):
                    _retry.work4you_paid_entitlement_refresh_attempted = True
                    if _try_refresh_work4you_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Work4You paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "work4you"
                    and status_code == 401
                    and not _retry.work4you_auth_retry_attempted
                ):
                    _retry.work4you_auth_retry_attempted = True
                    if agent._try_refresh_work4you_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Work4You agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Work4You 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_work4you_entitlement_guidance(agent, "Work4You model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: work4you auth add work4you")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.work4you.ai")
                ...
                # and omits the field, and Work4You' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from work4you_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `work4you model`."
                        )
                ...
                # ── Work4You Portal: record rate limit & skip retries ─────
                # When Work4You returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Work4You Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Work4You).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Work4You model for minutes.  We use
                # ``is_genuine_work4you_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_work4you_rate_limit = False
                    try:
                        from agent.work4you_rate_guard import (
                            is_genuine_work4you_rate_limit,
                            record_work4you_rate_limit,
                        )
                        ...
                        _genuine_work4you_rate_limit = is_genuine_work4you_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_work4you_rate_limit:
                            record_work4you_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Work4You 429 looks like upstream capacity "
                    ...
                    if _genuine_work4you_rate_limit:
                        ...
                        # top-of-loop Work4You guard handles fallback or
                ...
                # which Work4You' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Work4You' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Work4You may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See work4you-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "work4you" and _print_work4you_entitlement_guidance(
                            agent,
                            "Work4You model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "work4you"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}   💡 Work4You Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: work4you portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.work4you.ai", force=True)
                                # ``:free`` is OpenRouter slug syntax; Work4You Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Work4You Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Work4You catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.work4you/config.yaml` to extend the per-call "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Work4You mid-turn, resume logic still sees the
                    ...
                    from work4you_state import classify_persistence_error
                ...
                # timeout (WORK4YOU_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from work4you_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("WORK4YOU_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
