...
`work4you skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.work4you.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.work4you.blueprint`` is a mapping containing
    ...
    work4you = meta.get("work4you") if isinstance(meta, dict) else None
    blueprint = work4you.get("blueprint") if isinstance(work4you, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.work4you.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.work4you.blueprint`` block) they can hand
    to ``work4you skills publish`` to share. ``body`` is the plain-language
