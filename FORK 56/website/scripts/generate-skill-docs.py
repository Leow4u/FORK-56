...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Leow4u/FORK-56/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    work4you_meta = (fm.get("metadata") or {}).get("work4you") or {}
    tags = work4you_meta.get("tags") or []
    related = work4you_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `work4you skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Work4You"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Work4You ships with a large built-in skill library copied into `~/.work4you/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Work4You also syncs bundled skills on `work4you update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.work4you/skills/` tree, it is still shipped with Work4You; restore it with `work4you skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with work4you — install via work4you skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with work4you under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
