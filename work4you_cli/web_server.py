"""
Work4You — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m work4you_cli.main web          # Start on http://127.0.0.1:9119
    python -m work4you_cli.main web --port 8080
"""
...
from work4you_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `work4you dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["WORK4YOU_WEB_DIST"]) if "WORK4YOU_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``work4you gateway run`` — but the
    desktop app spawns a ``work4you dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same WORK4YOU_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``work4you_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from work4you_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `work4you update` current before the
    ...
    """
    try:
        from work4you_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `work4you update` kept 500ing every
    ...
    # Import work4you_cli.gateway eagerly *before* the lifespan yield so the
    ...
    if os.getenv("WORK4YOU_DESKTOP") == "1":
        ...
        try:
            from work4you_cli.gateway import _reap_unsupervised_gateway_orphans
...
from work4you_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# WORK4YOU_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("WORK4YOU_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Work4You-Session-Token"
...
#
# This list is defined in ``work4you_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from work4you_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Work4You-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``work4you-0day``
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from work4you_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `work4you egress setup` "
            "and `work4you egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during work4you update. "
...
from work4you_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Work4You provider slug — fine.
    ...
    1. Vendor-name → Work4You-provider mapping: when the provider string is
       not a known Work4You provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from work4you_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from work4you_cli.model_normalize import normalize_model_for_provider
    from work4you_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Work4You
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from work4you_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Work4You process is using it.
    """
    from work4you_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "WORK4YOU_DASHBOARD_FILES_ROOT"
...
# WORK4YOU_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to WORK4YOU_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Work4You credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no WORK4YOU_HOME resolution, so it blocks these trees wherever
...
def _default_work4you_root_is_opt_data() -> bool:
    raw = os.environ.get("WORK4YOU_HOME", "").strip()
    if not raw:
        return False
    try:
        from work4you_constants import get_default_work4you_root

        root = get_default_work4you_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``work4you update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Work4You root is actually /opt/data
    # (the container/hosted layout) or when WORK4YOU_DASHBOARD_FILES_ROOT is set.
    if _default_work4you_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Work4You inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``work4you:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.work4you-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``work4you_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from work4you_cli import web_git as _web_git  # noqa: E402
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from work4you_cli.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from work4you_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Work4You home
# (get_default_work4you_root()) — NOT the profile-scoped WORK4YOU_HOME — so every
...
def _read_or_create_install_id() -> Optional[str]:
    """Read (or mint + persist) the install id under the root Work4You home.
    ...
    root = get_default_work4you_root()
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors work4you_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_work4you_home() at call time
    ...
    try:
        ...
        # WORK4YOU_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of work4you_cli.gateway blocks the
        ...
        # and which providers are registered so ``work4you status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Work4You" or
        ...
        auth_flows: list[str] = []
        try:
            from work4you_cli.dashboard_auth import (
        ...
        # Work4You bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Work4You auth dies terminally (invalid_grant / quarantine)
        ...
        work4you_session_valid = "unknown"
        try:
            from work4you_cli.auth import get_work4you_session_validity
            work4you_session_valid = get_work4you_session_validity()
        except Exception:
            work4you_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_work4you": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "work4you_session_valid": work4you_session_valid,
        }
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_work4you_home())
            )
        ...
        # work4you_session_valid above. Deliberately NOT folded into
        ...
        try:
            ...
            status["memory"] = await asyncio.get_running_loop().run_in_executor(
                ...
                functools.partial(
                    ...
                    profile_dir if profile_dir else get_work4you_home(),
                ),
            )
        except Exception:
        ...
        # disk-exhaustion incidents). One statvfs call on WORK4YOU_HOME's
        ...
        try:
            from work4you_state import SessionDB as _SDB
            from work4you_constants import get_work4you_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Work4You Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the work4you home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_work4you_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_work4you_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Work4You Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
def _get_portal_status_sync():
    ...
    try:
        ...
        feats = get_work4you_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_nous", False):
                    state = "via Work4You Portal"
...
async def run_prompt_size():
    try:
        proc = _spawn_work4you_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_work4you_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_work4you_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.work4you/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_work4you_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "work4you-update": "work4you-update.log",
...
def _spawn_work4you_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``work4you <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``work4you_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "work4you_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _WORK4YOU_GATEWAY=1. Inheriting it makes a spawned `work4you gateway restart`
    ...
    action_env = {**os.environ, "WORK4YOU_NONINTERACTIVE": "1"}
    action_env.pop("_WORK4YOU_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("WORK4YOU_ACTION_ID")
    if action_id:
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["work4you", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``work4you gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``work4you gateway restart`` children race each other on the
    ...
    proc = _spawn_work4you_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``work4you gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``WORK4YOU_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/work4you/update")
async def update_work4you():
    """Kick off ``work4you update`` in the background."""
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("work4you-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "work4you-update",
    ...
    existing = _ACTION_PROCS.get("work4you-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("work4you-update")
        if action_id:
    ...
    try:
        proc = _spawn_work4you_action(
            ["update"],
            "work4you-update",
            env_overrides={"WORK4YOU_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn work4you update")
...
@app.get("/api/work4you/update/check")
async def check_work4you_update(force: bool = False):
    """Report whether a Work4You update is available, without applying it.
    ...
    if install_method == "apt":
        payload["message"] = (
            "Work4You is managed by Termux APT; run `pkg upgrade work4you`."
        )
        return payload
    ...
    try:
        ...
        if force:
            try:
                (get_work4you_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.work4you/config.yaml`` under ``tts.``.
    """
...
from work4you_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from work4you_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from work4you_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from work4you_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Work4You supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_work4you_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/work4you is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (WORK4YOU_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        work4you_home / f"{name}.json",
        work4you_home / name / "config.json",
    ):
    ...
    # Holographic stores under plugins.work4you-memory-store.
    plugins_cfg = cfg.get("plugins") if isinstance(cfg, dict) else {}
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("work4you-memory-store")
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_work4you_home()))
            return
...
async def get_egress_status():
    ...
    from work4you_cli.proxy_cli import format_status_text
...
# in work4you_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from work4you_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `work4you model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Work4You this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Work4You and None otherwise. `model` may be
    ...
    if slug == "work4you":
        try:
            from work4you_cli.models import (
                get_curated_work4you_model_ids,
            ...
            return {"provider": "work4you", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (work4you) failed")
            return {"provider": "work4you", "model": "", "free_tier": None}

    # Non-Work4You: preferred silent default when the provider's curated list
    ...
    try:
        ...
        from work4you_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from work4you_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from work4you_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.work4you/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from work4you_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to Work4You, mirror the CLI's
        # post-model-selection behaviour (work4you_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_work4you_managed_defaults):
        # auto-route any *unconfigured* tools through the Work4You Tool Gateway.
        # This is purely additive — apply_work4you_managed_defaults skips every
        ...
        if provider.strip().lower() == "work4you":
            try:
                from work4you_cli.work4you_subscription import apply_work4you_managed_defaults
                from work4you_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_work4you_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-work4you gating
                # must never block saving the model assignment.
                _log.debug("apply_work4you_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``work4you model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from work4you_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. work4you with $0 balance) keeps paying 402s
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different WORK4YOU_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_work4you_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``work4you model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from work4you_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from work4you_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `work4you model` provider — would be
        ...
        # Vertex — a `work4you model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `work4you model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from work4you_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level WORK4YOU_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from work4you_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Work4You from Telegram DMs, groups, and topics.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Work4You to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Work4You in Matrix rooms and direct messages.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Work4You via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Work4You to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Work4You inside Feishu / Lark.",
    ...
    "weixin": {
        ...
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Work4You through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Work4You to a QQ Bot from the QQ Open Platform.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Work4You.",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Work4You from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Work4You over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Work4You through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Work4You over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://work4you.ai/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Work4You to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
    ...
    "relay": {
        ...
        "description": "Generic relay adapter fronted by the Work4You Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
# work4you_cli/setup_hidden_env.py. Shared with the `work4you setup gateway`
# wizard so the surfaces ask for the same things.
from work4you_cli.setup_hidden_env import (  # noqa: E402
...
def _whatsapp_session_path() -> Path:
    from work4you_constants import get_work4you_dir

    return get_work4you_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.work4you.ai"
_TELEGRAM_ONBOARDING_USER_AGENT = f"Work4YouDashboard/{__version__}"
@dataclass
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Work4You appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the WORK4YOU_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized WORK4YOU_HOME)
...
# Anthropic, device-code for Work4You/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``work4you auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.work4you/.anthropic_oauth.json`` — Work4You-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_work4you_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_work4you_oauth_credentials,
            _get_work4you_oauth_file,
        )
    except ImportError:
        read_work4you_oauth_credentials = None  # type: ignore
        _get_work4you_oauth_file = None  # type: ignore

    work4you_creds = None
    if read_work4you_oauth_credentials:
        try:
            work4you_creds = read_work4you_oauth_credentials()
        except Exception:
            work4you_creds = None
    if work4you_creds and work4you_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "work4you_pkce",
            "source_label": f"Work4You PKCE ({_get_work4you_oauth_file() if _get_work4you_oauth_file else None})",
            "token_preview": _truncate_token(work4you_creds.get("accessToken")),
            "expires_at": work4you_creds.get("expiresAt"),
            "has_refresh_token": bool(work4you_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Work4You even
    when they also have a separate Work4You-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Work4You never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "work4you auth add work4you",
        "docs_url": "https://portal.work4you.ai",
        "status_fn": None,  # dispatched via auth.get_work4you_auth_status
    },
    {
        "id": "openai-codex",
        "name": "ChatGPT or Codex Subscription",
        "flow": "device_code",
        "cli_command": "work4you auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "work4you auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Work4You's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "work4you auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "work4you auth add xai-oauth",
        "docs_url": "https://work4you.ai/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "work4you auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from work4you_cli import auth as hauth
        if provider_id == "work4you":
            ...
            raw = hauth.get_work4you_auth_status_local()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "work4you_portal",
                "source_label": raw.get("portal_base_url") or "Work4You Portal",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Work4You, so the disconnect
    ...
    user sees exactly what executes, and Work4You then stops resolving the token.
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``work4you model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `work4you model` universe (zero-edit for new plugins).
    try:
        from work4you_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"work4you auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("work4you_pkce", "claude_code", ...)
    ...
    sync with the `work4you model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Work4You-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from agent.anthropic_adapter import _get_work4you_oauth_file
                    oauth_file = _get_work4you_oauth_file()
                ...
                try:
                    from work4you_cli.auth import clear_provider_auth
...
#          → persists to ~/.work4you/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Work4You, OpenAI Codex):
#     1. POST /api/providers/oauth/{work4you|openai-codex}/start
...
# Guarded so work4you web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Work4You file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``work4you auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_work4you_oauth_file
    oauth_file = _get_work4you_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "work4you-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Work4You, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "work4you":
        ...
        import httpx
        pconfig = PROVIDER_REGISTRY["work4you"]
        portal_base_url = (
            os.getenv("WORK4YOU_PORTAL_BASE_URL")
            or os.getenv("WORK4YOU_PORTAL_BASE_URL")
            or pconfig.portal_base_url
        ).rstrip("/")
        client_id = pconfig.client_id
        scope = pconfig.scope

        def _do_work4you_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_work4you_device_request
        )
        sid, sess = _new_oauth_session("work4you", "device_code", profile=profile)
        ...
        threading.Thread(
            target=_work4you_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Work4You's device-code
    ...
    if provider_id == "xai-oauth":
        from work4you_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _work4you_poller(session_id: str) -> None:
    """Background poller that drives a Work4You device-code flow to completion."""
    from work4you_cli.auth import (
        _poll_for_token,
        refresh_work4you_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _work4you_device_code_login (validate/refresh JWT)
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_work4you_oauth_from_state(
                auth_state,
                timeout_seconds=15.0,
                force_refresh=False,
            )
            from work4you_cli.auth import persist_work4you_credentials
            persist_work4you_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: work4you login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("work4you device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_work4you_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Work4You. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``work4you auth add minimax-oauth``.
    """
    from work4you_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `work4you auth add xai-oauth`: first credential may become
            ...
            # prior ``work4you auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``work4you model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Work4You "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from work4you_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Work4You, OpenAI Codex, MiniMax, xAI).
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `work4you update` until the first message forced a writable open.
    """
    from work4you_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from work4you_state import SessionDB, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``work4you serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `work4you sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_work4you_home()
...
async def get_logs(
    ...
    from work4you_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_work4you_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from work4you_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (WORK4YOU_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.work4you``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    WORK4YOU_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, WORK4YOU_HOME)."""
    from work4you_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["work4you_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_work4you_home_override(str(home))
    try:
...
from work4you_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from work4you_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/work4you_cli/
        # test_config_read_guard.py). The WORK4YOU_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``work4you gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (work4you_cli.mcp_config), so
# servers managed here show up under `work4you mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from work4you_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from work4you_cli.mcp_security import validate_mcp_server_entry
...
from work4you_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from work4you_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from work4you_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.work4you_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import Work4YouTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_work4you_home_override(flow.work4you_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.work4you_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = Work4YouTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        work4you_home=flow.work4you_home,
                    )
            ...
            reset_work4you_home_override(home_token)
    except Exception as exc:
        msg = str(exc)
...
# spawn the real `work4you gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_work4you_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_work4you_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from work4you_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `work4you auth remove`) can seed again.
        # Mirrors the `work4you auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``work4you auth remove`` uses: each source cleans up its
    ...
    from work4you_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_work4you_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_work4you_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_work4you_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"work4you-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_work4you_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_work4you_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    def _run():
        ...
        try:
            from work4you_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <work4you_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_work4you_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_work4you_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``work4you`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_work4you_action`` tracks one process/log per name, so a shared
...
from work4you_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from work4you_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `work4you skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Work4You)",
    "work4you-index": "Work4You Index",
    "skills-sh": "skills.sh",
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local WORK4YOU_HOME override so the write lands in the target
    ...
    token = set_work4you_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local WORK4YOU_HOME override (same mechanism as
    ...
    """
    from work4you_constants import set_work4you_home_override, reset_work4you_home_override
    from work4you_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the WORK4YOU_HOME override. Returns the
    number of skills newly disabled.
    """
    from work4you_constants import set_work4you_home_override, reset_work4you_home_override
    from work4you_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_work4you_home()`` at call
       time — the context-local override from ``set_work4you_home_override``
    ...
    retargeted to the *current* ``get_work4you_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a WORK4YOU_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_work4you_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_work4you_home()`` at call time
...
from work4you_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from work4you_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Work4You Subscription" image row instead carries the legacy
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Work4You,
...
#
# The endpoint spawns the same ``work4you --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from work4you_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from work4you_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from work4you_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from work4you_cli.dashboard_auth.audit import AuditEvent, audit_log
        from work4you_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``work4you --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``WORK4YOU_TUI_RESUME`` env var —
    matching what ``work4you_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``WORK4YOU_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``WORK4YOU_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``WORK4YOU_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``WORK4YOU_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_work4you_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``work4you -p <name>`` performs. The in-process
    ``WORK4YOU_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from work4you_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Work4You TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides WORK4YOU_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("WORK4YOU_TUI_DISABLE_MOUSE", "1")
    env.setdefault("WORK4YOU_TUI_INLINE", "1")
    ...
    env["WORK4YOU_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["WORK4YOU_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["WORK4YOU_TUI_RESUME"] = resume

    if sidecar_url:
        env["WORK4YOU_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile WORK4YOU_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["WORK4YOU_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``WORK4YOU_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("WORK4YOU_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from work4you_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="work4you-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Work4You Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Work4You CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="work4you-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from work4you_cli.console_engine import Work4YouConsoleEngine

        engine = Work4YouConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("WORK4YOU_TUI_RESUME") or raw_resume
...
# WORK4YOU_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`work4you_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from work4you_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Work4You Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="work4you-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/work4you/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /work4you`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__WORK4YOU_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `work4you serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("WORK4YOU_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (work4you serve): web UI disabled — use "
            "`work4you dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/work4you``)
        ...
        ``__WORK4YOU_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__WORK4YOU_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__WORK4YOU_BASE_PATH__="{prefix}";'
                f"window.__WORK4YOU_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__WORK4YOU_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__WORK4YOU_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__WORK4YOU_BASE_PATH__="{prefix}";'
                f"window.__WORK4YOU_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``WORK4YOU_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Work4You Teal canvas, then
    ...
    # under ``/work4you`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Work4You backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.work4you/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.work4you/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_work4you_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``WORK4YOU_HOME``.
    """
    themes_dir = get_process_work4you_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.work4you/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as work4you_cli.plugins):
    1. User plugins:    ~/.work4you/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.work4you/plugins/  (only if WORK4YOU_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from work4you_cli.plugins import get_bundled_plugins_dir
    ...
    # WORK4YOU_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``WORK4YOU_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the work4you root (``~/.work4you/plugins``). Scan the default
    # root as well (``get_default_work4you_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``WORK4YOU_HOME`` unchanged when it *is* the root), mirroring how
    # ``work4you_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from work4you_constants import get_default_work4you_root

    user_plugin_roots = [get_process_work4you_home() / "plugins"]
    root_plugins = get_default_work4you_root() / "plugins"
    ...
    # ``work4you_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("WORK4YOU_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".work4you" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from work4you_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_work4you_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"work4you auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from work4you_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from work4you_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from work4you_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.work4you/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.work4you/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"work4you_dashboard_plugin_{plugin['name']}"
...
from work4you_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``WORK4YOU_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("WORK4YOU_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``WORK4YOU_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``work4you.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``WORK4YOU_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("WORK4YOU_PARENT_PID")
    start_marker = os.environ.get("WORK4YOU_PARENT_START_MARKER")
    nonce = os.environ.get("WORK4YOU_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("WORK4YOU_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``WORK4YOU_SERVE_HEADLESS``), so
    ...
    try:
        from work4you_cli.work4you_auth_keepalive import start_work4you_auth_keepalive

        start_work4you_auth_keepalive()
    except Exception as exc:
        _log.debug("Work4You auth keepalive did not start: %s", exc)
    ...
    # the work4you-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing WORK4YOU_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Work4You exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import work4you as _work4you_plugin

                if _work4you_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • work4you: {_work4you_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `work4you dashboard register` (Work4You Portal) or "
            ...
            try:
                ...
                from work4you_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`work4you plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # WORK4YOU_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("WORK4YOU_DESKTOP") == "1":
                try:
                    from work4you_cli.dashboard_procs import (
            ...
            # for standalone `work4you serve` (no WORK4YOU_PARENT_PID env).
            ...
            ready_token = "WORK4YOU_BACKEND_READY" if headless else "WORK4YOU_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  Work4You backend listening on {host}:{actual_port}", flush=True)
            else:
