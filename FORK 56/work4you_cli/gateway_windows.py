...
def _preserve_work4you_home_path(path: str | Path) -> str:
    """Render Work4You-owned paths under the configured WORK4YOU_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\work4you`` as a symlink/junction to
    ...
    path lives under WORK4YOU_HOME.
    """
    candidate = Path(path)
    try:
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Work4You profile."""
    from work4you_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "work4you_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("WORK4YOU_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("WORK4YOU_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("WORK4YOU_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["WORK4YOU_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["WORK4YOU_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["WORK4YOU_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("WORK4YOU_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("WORK4YOU_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("WORK4YOU_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during work4you_cli boot.
    from work4you_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\work4you\\gateway-service\\<task_name>.cmd``
    (or ``<WORK4YOU_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Work4You installs stay self-contained).
    """
    _assert_windows()
    from work4you_cli.config import get_work4you_home

    script_dir = Path(get_work4you_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``WORK4YOU_HOME`` whenever it
    ...
    source checkout only if ``WORK4YOU_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    to that terminal like a normal foreground ``work4you gateway run``.
    ...
    lines.append(f'set "WORK4YOU_HOME={work4you_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "WORK4YOU_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports work4you_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_work4you_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_work4you_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_work4you_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "work4you_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_work4you_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('WORK4YOU_HOME')}) = {_quote_vbs_string(work4you_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('WORK4YOU_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_work4you_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_work4you_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    work4you_home = str(Path(get_work4you_home()))
    profile_arg = _profile_arg(work4you_home)

    content = _build_gateway_cmd_script(python_path, working_dir, work4you_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, work4you_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Work4You builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_work4you_home_path(get_python_path())
    )
    project_root = _preserve_work4you_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    work4you_home = str(Path(get_work4you_home()))
    profile_arg = _profile_arg(work4you_home)

    argv = [python_exe, "-m", "work4you_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "WORK4YOU_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_work4you_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_work4you_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (WORK4YOU_HOME, VIRTUAL_ENV,
    ...
    from work4you_cli.config import get_work4you_home
    from work4you_cli.gateway import PROJECT_ROOT
    ...
    try:
        work4you_home = str(Path(get_work4you_home()).resolve())
    except Exception:
        work4you_home = ""
    ...
    if work4you_home:
        env_overlay["WORK4YOU_HOME"] = work4you_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    log_dir = Path(get_work4you_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("WORK4YOU_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("WORK4YOU_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from work4you_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `work4you -p <profile> gateway install` must be safe.
    ...
    from work4you_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"work4you {profile_arg} gateway start" if profile_arg else "work4you gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: work4you gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Work4You gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: work4you gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: work4you gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from work4you_cli.config import get_work4you_home
        print(f"    type {Path(get_work4you_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_work4you_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from work4you_cli.config import get_work4you_home

    work4you_home = Path(get_work4you_home())
    print()
    print("Next steps:")
    print("  work4you gateway status                      # Check status")
    print(f"  type {work4you_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Work4You gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: work4you gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: work4you gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `work4you gateway status`."
        )
