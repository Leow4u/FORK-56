...
Work4You can expose two different X surfaces:
...
:::tip
If you're paying Portal for an xAI model anyway, Live Search calls bill against the same xAI key configured for chat. See [Work4You Portal](/integrations/work4you-portal).
:::
...
|------------|--------|-------|
| **SuperGrok / X Premium+ OAuth** | Browser login at `accounts.x.ai`, refreshed automatically | `work4you auth add xai-oauth` — see [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** (preferred) | Paid xAI API key | Set in `~/.work4you/.env` |
...
Auto-enables when xAI credentials (OAuth token or `XAI_API_KEY`) are present. Disable explicitly via `work4you tools` → Search → x_search if you don't want this.
...
The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.work4you/.env` or run `work4you auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
...
1. **Toolset not enabled.** Run `work4you tools` and confirm `🐦 X (Twitter) Search` is checked.
2. **No xAI credentials.** The check_fn returns False, so the schema stays hidden. Run `work4you auth status` to confirm xai-oauth login state, and check that `XAI_API_KEY` is set (if you're using the API-key path).
...
- [xurl skill](../skills/optional/social-media/social-media-xurl.md) — official X API CLI for authenticated account actions
