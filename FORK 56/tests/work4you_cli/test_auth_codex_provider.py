"""Tests for Codex auth — tokens stored in Work4You auth store (~/.work4you/auth.json)."""
...
def _setup_work4you_auth(work4you_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Work4You auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_work4you_auth(work4you_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (work4you_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((work4you_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``work4you auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``work4you auth add openai-codex``.
    ...
    (work4you_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `work4you auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``work4you auth add openai-codex`` for accounts B and C
    ...
    (work4you_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``work4you auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Work4You auth store, not ~/.codex/."""
    work4you_home = tmp_path / "work4you"
    ...
    _save_codex_tokens({"access_token": "work4you-at", "refresh_token": "work4you-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Work4You store
    assert not (codex_home / "auth.json").exists()

    # Work4You auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "work4you-at"


def test_resolve_returns_work4you_auth_store_source(tmp_path, monkeypatch):
    work4you_home = tmp_path / "work4you"
    _setup_work4you_auth(work4you_home)
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "work4you-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run work4you auth".
    """
    from work4you_cli.auth import (
    ...
    assert "work4you auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from work4you_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from work4you_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch work4you_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("work4you_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
