...
cache problem must never block Work4You startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Work4You home used for cache paths.

    ``home_path`` is whatever ``load_work4you_dotenv()`` already resolved;
    falling back to ``$WORK4YOU_HOME`` / ``~/.work4you`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from work4you_constants import get_work4you_home

        home_path = get_work4you_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<work4you_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if WORK4YOU_HOME is ever made traversable.
            try:
