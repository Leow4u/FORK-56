...
description: Control your smart home with Work4You via Home Assistant integration.
...
Work4You integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Work4You"
5. Copy the token
...
Work4You registers four tools for smart home control:
...
Configure which events the agent sees in `~/.work4you/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Work4You".
...
The adapter reads credentials from `~/.work4you/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Work4You profile
...
running Work4You — `curl -H "Authorization: Bearer <token>" <url>/api/` should
