"""Offline, non-destructive recovery for a damaged Work4You session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``work4you_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``work4you_state._backup_db_file``.
    """
    from work4you_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="work4you-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Work4You process using this profile and retry. "
                "This includes the interactive `work4you` CLI session this "
                ...
                "Work4You process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `work4you` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from work4you_cli.session_lost_and_found import (
