...
- ``work4you_cli.providers``        -- canonical provider identity + overlays
- ``work4you_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Work4You wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Work4You.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Work4You versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``work4you model`` wizard — not a
    ...
    while ``work4you model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Work4You itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Work4You itself discovered (``models_discovered: true``
...
_WORK4YOU_MODEL_WARNING = (
    "Work4You Work4You 3 & 4 models are NOT agentic and are not designed "
    "for use with Work4You. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Work4You Work4You 3 / Work4You 4 chat families.
# The previous substring check (`"work4you" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``work4you-brain:qwen3-14b-ctx16k`` that just
# happen to carry "work4you" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Work4You/Work4You-3-Llama-3.1-70B, work4you-4-405b, openrouter/work4you3:70b
# Negative examples it must NOT match:
#   work4you-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_WORK4YOU_WORK4YOU_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])work4you[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_work4you_work4you_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Work4You Work4You 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_WORK4YOU_WORK4YOU_NON_AGENTIC_RE.search(model_name))


def _check_work4you_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Work4You Work4You 3/4 chat model."""
    if is_work4you_work4you_non_agentic(model_name):
        return _WORK4YOU_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``work4you config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from work4you_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "work4you")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "work4you")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Work4You, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Work4You, Codex, GMI, Ollama, models.dev and
    ...
    ``work4you_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'work4you model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'work4you doctor' — config issues detected:"
        ...
        from work4you_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from work4you_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from work4you_cli.config import load_config
            from work4you_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from work4you_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from work4you_cli.models import should_use_ollama_native_catalog
    ...
    if not validation.get("accepted"):
        override = False
        if user_providers:
            from work4you_cli.config import is_provider_enabled
    ...
    # --- Work4You Portal dual-wire override ---
    ...
    if target_provider in {"work4you", "work4you-portal", "work4you"}:
        from work4you_cli.providers import work4you_api_mode

        api_mode = work4you_api_mode(new_model)
    ...
    # work4you_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from work4you_cli.models import normalize_opencode_base_url
    ...
    work4you_warn = _check_work4you_model_warning(new_model)
    if work4you_warn:
        warnings.append(work4you_warn)
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Work4You provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from work4you_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from work4you_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from work4you_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from work4you_cli.providers import WORK4YOU_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from work4you_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Work4You-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for work4you_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(work4you_id)
        if (
            _alias_target
            and _alias_target != work4you_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = work4you_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(work4you_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != work4you_id:
            continue
        if work4you_id.lower() in seen:
            continue
        if work4you_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(work4you_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from work4you_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(work4you_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(work4you_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        work4you_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(work4you_id)
            seen.add(work4you_id.lower())

    # --- Section 2: Work4You-only providers (WORK4YOU_OVERLAYS) ---
    _mdev_to_work4you = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in WORK4YOU_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        work4you_slug = _mdev_to_work4you.get(pid, pid)
        if work4you_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or work4you_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or work4you_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(work4you_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(work4you_slug)
            seen.add(pid.lower())
            seen.add(work4you_slug.lower())
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_work4you_tier: bool = False,
    ...
    Uses the curated model lists from work4you_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_work4you_tier`` bypasses the short Work4You tier cache for explicit
    ...
    from work4you_cli.models import (
        ...
        clear_provider_models_cache, get_curated_work4you_model_ids,
    )
    ...
    # Compared against work4you_id / mdev_id (section 1), pid / work4you_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from work4you_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by work4you provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "work4you" pulls from the remote model-catalog manifest published at
    # https://work4you.ai/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Work4You release. Falls back to the in-repo
    # _PROVIDER_MODELS["work4you"] snapshot when the manifest is unreachable.
    curated["work4you"] = get_curated_work4you_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from work4you_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from work4you_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Work4You-mapped providers ---
    from work4you_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from work4you_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for work4you_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip work4you_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose work4you_id matches the canonical profile name so
        ...
        # same work4you_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if work4you_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (WORK4YOU_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Work4You may not route yet.
        ...
        if not has_creds:
            try:
                from work4you_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(work4you_id)
                )
        ...
        # /model picker sees the SAME list `work4you model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(work4you_id)
        if not model_ids:
            model_ids = curated.get(work4you_id, [])
            if work4you_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(work4you_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or work4you_id == current_provider
    ...
    # --- 2. Check Work4You-only providers (work4you, openai-codex, copilot, opencode-go) ---
    from work4you_cli.providers import WORK4YOU_OVERLAYS
    from work4you_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Work4You provider ID.
    # WORK4YOU_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Work4You IDs ("copilot").
    _mdev_to_work4you = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in WORK4YOU_OVERLAYS.items():
        ...
        # Resolve Work4You slug — e.g. "github-copilot" → "copilot"
        work4you_slug = _mdev_to_work4you.get(pid, pid)
        if work4you_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or work4you_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(work4you_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or work4you_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(work4you_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(work4you_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", work4you_slug, exc)
        ...
        # configured.
        if not has_creds and work4you_slug == "anthropic":
            try:
                ...
                if (work4you_creds and work4you_creds.get("accessToken")) or \
        ...
        if work4you_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(work4you_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(work4you_slug)
                model_ids = _ids if _ids else (curated.get(work4you_slug, []) or curated.get(pid, []))
            except Exception:
            ...
            if not model_ids:
                model_ids = curated.get(work4you_slug, []) or curated.get(pid, [])
                if work4you_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(work4you_slug, model_ids)
        total = len(model_ids)
        if work4you_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": work4you_slug,
            "name": get_label(work4you_slug),
            "is_current": work4you_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "work4you",
        ...
        seen_slugs.add(work4you_slug.lower())
        _record_builtin_endpoint(work4you_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or WORK4YOU_OVERLAYS (keeps /model in sync
    # with `work4you model`).
    try:
        from work4you_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Work4You writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Work4You's own writer uses (mirrors section-4 grouping).
            ...
            # ``work4you model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``work4you model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from work4you_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Work4You's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, work4you_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``work4you model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from work4you_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`work4you_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from work4you_cli.models import fetch_openrouter_models
