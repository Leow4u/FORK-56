...
load_work4you_dotenv(
    work4you_home=_work4you_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.work4you/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_work4you_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("WORK4YOU_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("WORK4YOU_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
_LONG_HANDLERS = frozenset(
    {
        ...
        # pool checks, pricing fetch, Work4You tier check, optional custom-provider
        ...
        # auto-detect → a SYNCHROWORK4YOU faster-whisper lazy install (uv/pip
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("WORK4YOU_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("work4you_gateway_runtime_session_record", default=None)
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Work4You' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Work4You-managed CLIs (browser-use, uvx, uv) even when the parent
    ...
    try:
        ...
        managed_bin = str(Path(get_work4you_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent Work4YouCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Work4You agent → needs provider credentials.
        ...
        # gateway's launch WORK4YOU_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["WORK4YOU_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            work4you_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"WORK4YOU_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the Work4You venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Work4You-managed CLIs
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `work4you --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("WORK4YOU_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `work4you --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from work4you_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from work4you_cli.active_sessions import release_orphaned_leases
...
# ``profile`` on those calls; we open that profile's db and bind its WORK4YOU_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_work4you_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s WORK4YOU_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_work4you_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_work4you_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from work4you_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``work4you dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("WORK4YOU_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `work4you` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # WORK4YOU_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_work4you_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``work4you`` / ``work4you --tui`` / CLI) is started from a
    ...
    try:
        ...
        from work4you_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from work4you_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``work4you_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _work4you_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_work4you_home_override()
        home = override if isinstance(override, str) and override else _work4you_home
...
def _load_cfg() -> dict:
    ...
    ``work4you_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _save_cfg(cfg: dict):
    ...
    path = _work4you_home / "config.yaml"
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read WORK4YOU_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # WORK4YOU_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["WORK4YOU_GATEWAY_SESSION"] = "1"
    os.environ["WORK4YOU_EXEC_ASK"] = "1"
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Work4You Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Work4You Desktop app and start a new "
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``work4you config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_work4you_home_override()
    return Path(override if isinstance(override, str) and override else _work4you_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Work4You activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="work4you-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("WORK4YOU_MODEL", "")
        or os.environ.get("WORK4YOU_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from work4you_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``WORK4YOU_DESKTOP=1`` and ``WORK4YOU_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``WORK4YOU_DESKTOP_TERMINAL=1`` → "tui"
        (``work4you --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``work4you --tui``.)
    """
    if is_truthy_value(os.environ.get("WORK4YOU_DESKTOP")) and not is_truthy_value(
        os.environ.get("WORK4YOU_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads WORK4YOU_MODEL /
    WORK4YOU_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`work4you --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads WORK4YOU_MODEL /
    # WORK4YOU_INFERENCE_MODEL, which `work4you --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("WORK4YOU_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("WORK4YOU_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from work4you_state import _BARE_BILLING_PROVIDERS
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind WORK4YOU_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_work4you_home_override(profile_home) if profile_home else None
    )
    try:
...
def _load_approval_mode() -> str:
    ...
    canonical ``work4you_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`work4you_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("WORK4YOU_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_WORK4YOU_CORE_TOOLS`` — every other
    ...
    paths run with ``WORK4YOU_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Work4You desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("WORK4YOU_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Work4You): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `work4you --tui` both land here. See
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _WORK4YOU_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off work4you-cli's tool universe — can't
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (WORK4YOU_MODEL /
    # WORK4YOU_INFERENCE_MODEL / WORK4YOU_TUI_PROVIDER / WORK4YOU_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `work4you --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # WORK4YOU_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("WORK4YOU_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from work4you_cli.personality import available_personalities
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Work4You window, and answers
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via work4you_cli.personality (single owner)."""
    from work4you_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as work4you_cli.personality.resolve_personality — (name,
    ...
    """
    from work4you_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to work4you_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("WORK4YOU_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("WORK4YOU_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``Work4YouCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    cfg = _load_cfg()
    from work4you_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("WORK4YOU_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("WORK4YOU_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("WORK4YOU_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("WORK4YOU_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped WORK4YOU_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / WORK4YOU_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The WORK4YOU_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_work4you_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Work4You has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_work4you_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("WORK4YOU_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + work4you_cli/work4you_billing.py) — same as /topup.


def _serialize_billing_error(exc) -> dict:
    """Map a BillingError into the result.error envelope the TUI branches on."""
    from work4you_cli.work4you_billing import (
...
#
# Layout:  $WORK4YOU_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_work4you_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from work4you_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=WORK4YOU_SESSION_KEY`` (see
    ...
    # DB when WORK4YOU_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``WORK4YOU_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn WORK4YOU_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_work4you_home_override(_profile_home_str)
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from work4you_cli.voice import is_continuous_active
            ...
            # Run while any profile-specific WORK4YOU_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile WORK4YOU_HOME
    override, so ``get_work4you_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _work4you_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile WORK4YOU_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from work4you_cli.model_switch import parse_model_switch_args
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("WORK4YOU_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["WORK4YOU_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("WORK4YOU_YOLO_MODE", None)
                    nv = "0"
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through work4you_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from work4you_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under WORK4YOU_HOME (~/.work4you by
    ...
    work4you_home = os.path.realpath(str(get_work4you_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == work4you_home
        or real.startswith(work4you_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    WORK4YOU_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: WORK4YOU_HOME itself and the dirs in
    ...
    work4you_home = os.path.normcase(os.path.realpath(str(get_work4you_home())))
    return real in _non_workspace_dirs() or real == work4you_home
...
def _discover_repos_payload(
    ...
    repos even with zero work4you sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (work4you home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Work4You sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Work4You
...
def _model_picker_context(agent):
    ...
    from work4you_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Work4You" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("WORK4YOU_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("WORK4YOU_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``work4you_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["WORK4YOU_VOICE"] = "0"
                    os.environ["WORK4YOU_VOICE_TTS"] = "0"
                    try:
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via work4you_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["WORK4YOU_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["WORK4YOU_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            # are tolerated.
            try:
                from work4you_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import work4you_cli.voice  # noqa: F401
