"""Work4You skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Work4You itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.work4you/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.work4you/skins/ therefore
...
.. code-block:: python
    ...
    from work4you_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.work4you/skins/
...
Drop a YAML file in ``~/.work4you/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (optional; default splash has none)
    banner_hero: str = ""    # Rich-markup hero art (replaces the default pixel mark)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Work4You — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Work4You gold.
        "colors": {
        ...
        "branding": {
            "agent_name": "Work4You",
            "welcome": "Welcome to Work4You! Type your message or /help for commands.",
            "goodbye": "Goodbye! ◆",
...
def _skins_dir() -> Path:
    ...
    return get_work4you_home() / "skins"
