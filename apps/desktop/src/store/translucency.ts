 ...
 * the desktop shows through — see `@work4you/shared/translucency`, which owns the
 ...
 * web contents to read (see the `[data-work4you-glass]` block in styles.css).
 */
...
} from '@work4you/shared/translucency'
 ...
 * sets `work4youDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.work4youDesktop?.glassSupported === 'boolean'
    ? window.work4youDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.work4youDesktop?.translucencySupported === 'boolean'
    ? window.work4youDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-work4you-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-work4you-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-work4you-glass-on', glassLive)
  root.toggleAttribute('data-work4you-glass', glassOn)
  root.toggleAttribute('data-work4you-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-work4you-glass-scope', scope)
  } else {
    root.removeAttribute('data-work4you-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.work4youDesktop?.setTranslucency?.(state)
