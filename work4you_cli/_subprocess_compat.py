...
Work4You is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_WORK4YOU_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `work4you
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Work4You' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Work4You shells out to git from many non-interactive contexts — MCP catalog
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``work4you update`` on slow-WMI machines
