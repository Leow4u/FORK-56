...
通过回调/webhook 模式，将 Work4You 作为企业自建应用接入企业微信（WeCom）。

:::info WeCom Bot 与 WeCom 回调
Work4You 支持两种企业微信集成模式：
...
3. Work4You 解密消息，将其加入 agent 处理队列
...
（仅在通过 `work4you gateway install` 注册 systemd/launchd 服务后，才使用 `work4you gateway start`。）
