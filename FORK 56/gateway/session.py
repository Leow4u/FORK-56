...
# ``WORK4YOU_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``WORK4YOU_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("WORK4YOU_AUTO_CONTINUE_FRESHNESS")
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `work4you tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `work4you tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `work4you tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_work4you_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Work4You session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_work4you_home()`` for the turn (its docstring lists "sessions"
    ...
    def _open_session_db_for_active_scope(self):
        ...
        time, and that helper follows the context-local WORK4YOU_HOME override
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.work4you/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.work4you/state.db "
                "and are shown by `work4you sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from work4you_state import CompressionSessionClosedError
