"""Run the read-tool eval through the REAL Work4You AIAgent.

For each task: fresh temp WORK4YOU_HOME, fresh fixture workspace, real
...
Usage:
  python3 evals/readtool/runner.py --model anthropic/claude-opus-4.8 \\
      --provider work4you --reps 3 --label baseline
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    work4you_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".work4you"
    ...
    try:
        ...
        shutil.rmtree(work4you_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.work4you/.env; set +a  — then relaunch."
        )
