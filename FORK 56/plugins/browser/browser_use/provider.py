...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Work4You tool
gateway (which Work4You uses to bill Browser Use sessions to a Work4You
...
Auth env vars (one of)::
    ...
    # OR a managed Work4You gateway entry (configured via 'work4you setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Work4You
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Work4You tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Work4You gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Work4You auth stack which can be
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use",
                token_reader=None if refresh_token else peek_work4you_access_token,
            )
            if managed is None:
                return None
            return {
                "api_key": managed.work4you_user_token,
        ...
        # Strict selection: "work4you" (or legacy use_gateway: true) → managed
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Work4You only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"work4you_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the work4you tools picker: the "Browser Use" row now
        # activates the CLI-based backend (tools/browser_use_cli.py). This
        # provider stays registered for the Work4You gateway path and un-migrated
