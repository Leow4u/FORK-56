...
Bug 1 — OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, …). The ``work4you model`` CLI shows only the curated
...
Bug 2 — OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's Work4YouOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
from work4you_cli import models as M
...
def test_openrouter_overlay_does_not_list_openai_api_key():
    overlay = WORK4YOU_OVERLAYS["openrouter"]
