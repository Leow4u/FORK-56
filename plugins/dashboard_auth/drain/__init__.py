...
What it is
----------
A service-to-service auth provider. ``work4you-account-service`` (NAS) provisions a
...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    WORK4YOU_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``WORK4YOU_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``WORK4YOU_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("WORK4YOU_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
    ...
    try:
        from work4you_cli.dashboard_auth.token_auth import register_token_route
