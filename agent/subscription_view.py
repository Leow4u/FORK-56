...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``WORK4YOU_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
_DEV_FIXTURE_PORTAL = "https://portal.work4you.ai/billing"
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``WORK4YOU_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``WORK4YOU_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("WORK4YOU_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown WORK4YOU_DEV_SUBSCRIPTION_FIXTURE: {name}")
