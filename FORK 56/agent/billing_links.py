...
class BillingBlock:
    ...
    ``is_nous`` is the routing bit: Work4You has a first-class in-app billing surface
...
def is_work4you_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Work4You-managed inference gateway."""
    if (provider or "").strip().lower() == "work4you":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.work4you.ai")


def _work4you_billing_url() -> Optional[str]:
    """Best-effort Work4You portal billing URL (text-surface fallback; Work4You prefers the in-app flow)."""
    try:
        from work4you_cli.work4you_account import work4you_portal_billing_url

        return work4you_portal_billing_url(None)
    except Exception:
        return "https://portal.work4you.ai/billing"
...
def build_billing_block(
    *,
    ...
    if is_work4you_inference_route(slug, base_url):
        return BillingBlock(slug or "work4you", "Work4You Portal", model, _work4you_billing_url(), True, message or "")
