...
409), and is driven by:
  ...
  * the ``work4you sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``work4you_cli/``) so the
...
**Work4You admin**. We read that off the access token, which rides on the same
bearer ``resolve_work4you_runtime_credentials()`` returns; we decode the JWT
...
status conflates "may administer Work4You" with "has Skill Sync enabled", and has
...
is toggled via ``work4you sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.work4you/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``work4you sync enable`` — it is
...
# namespace from work4you's local ``content_hash`` (skills_guard.py:846),
...
#
# We reuse resolve_work4you_runtime_credentials() for the bearer (it honors the
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    plugins/dashboard_auth/work4you/__init__.py:463.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the Work4You bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, work4you_admin, claims}``.
    ...
    try:
        ...
        raise SyncInertError(f"no Work4You credentials: {e}") from e
    ...
    work4you_admin = claims.get(WORK4YOU_ADMIN_CLAIM) is True
    return {
        "api_key": api_key,
        "base_url": (creds or {}).get("base_url"),
        "owner": str(owner),
        "work4you_admin": work4you_admin,
        "claims": claims,
    }
...
def dev_gate_open() -> bool:
    ...
    try:
        return bool(resolve_identity().get("work4you_admin"))
...
# sync.base_url (or the WORK4YOU_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.work4you.ai"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: WORK4YOU_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("WORK4YOU_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Work4You Cloud instance can be set
...
# precedence as base_url: the WORK4YOU_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   WORK4YOU_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   WORK4YOU_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   WORK4YOU_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``WORK4YOU_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Work4You Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the Work4You-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("WORK4YOU_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``WORK4YOU_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``work4you sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "WORK4YOU_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``WORK4YOU_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``work4you sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Work4You Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("WORK4YOU_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.work4you/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.work4you/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Work4You Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``WORK4YOU_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.work4you/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.work4you/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``work4you sync device --name`` to set an explicit
    label."""
    ...
    # Work4You Cloud (and any templated deployment) can seed the label
    # declaratively via WORK4YOU_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `work4you sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("WORK4YOU_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.work4you/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the Work4You bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.work4you/skills/.sync_state as JSON.
#
...
# path under ~/.work4you/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.work4you/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Work4You speaks "
            f"{WIRE_VERSION} — update Work4You to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "work4you skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (work4you sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.work4you/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a Work4You admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "work4you skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("work4you_admin"):
            return None  # access gate: inert unless the user is a Work4You admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (WORK4YOU_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``work4you sync status``. Never raises."""
    status: Dict[str, Any] = {
        "work4you_admin": False,
        "logged_in": False,
    ...
    try:
        ...
        status["work4you_admin"] = bool(identity.get("work4you_admin"))
    except SyncInertError:
        pass
    except Exception as e:
...
#
# Org skills live under a DISTINCT local namespace, ~/.work4you/skills/_org/
...
#
# `work4you sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.work4you/skills/_org/<org_id>/``.
