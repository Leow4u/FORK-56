# nix/homeManagerModules.nix — the Home Manager module for work4you
#
# This module is the user-level equivalent of nixosModules.default. Work4You is
...
#
# `services.work4you` is the same option set on both modules. All of the
...
#   changed   stateDir (+ "/.work4you")  -> work4youHome, set directly
#
# To use the module:
  ...
  flake.homeManagerModules.default =
    ...
    in
    {
      options.services.work4you =
        ...
        // {
          work4youHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.work4you";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.work4you"'';
            description = ''
              The value of WORK4YOU_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.work4you` to it.
              This module sets WORK4YOU_HOME directly. Thus an existing
              ~/.work4you continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.work4you-work";
          };
          ...
          installPackage = lib.mkOption {
            ...
            default = true;
            description = ''
              Add the work4you CLI to home.packages, and export WORK4YOU_HOME
              ...
              WORK4YOU_HOME with environment.variables. That variable applies
              to the full system and replaces the WORK4YOU_HOME of each other
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf cfg.installPackage {
            ...
            home.sessionVariables.WORK4YOU_HOME = cfg.work4youHome;
          })
          ...
          # ── Activation: directories, config, secrets, documents ────────
          {
            ...
            home.activation.work4youAgentSetup =
              lib.hm.dag.entryAfter
                ...
                (
                  common.mkStateScript {
                    inherit pkgs cfg;
                    inherit (cfg) work4youHome workingDirectory;
                    run = "$DRY_RUN_CMD ";
          ...
          (lib.mkIf (isLinux && cfg.gateway.enable) {
            systemd.user.services.work4you = mkUnit {
          ...
          (lib.mkIf (isLinux && cfg.backend.mode != "none") {
            systemd.user.services.work4you-backend = mkUnit {
          ...
          (lib.mkIf (isDarwin && cfg.gateway.enable) {
            launchd.agents.work4you = mkAgent {
              argv = common.gatewayArgv cfg;
              logName = "work4you";
            };
          })
          ...
          (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
            launchd.agents.work4you-backend = mkAgent {
              argv = common.backendArgv cfg;
              logName = "work4you-backend";
            };
          })
        ]
      );
