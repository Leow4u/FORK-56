...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Work4You terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Work4You's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `work4you setup --portal` — and you're ready to `work4you chat`. See [Work4You Portal](/integrations/work4you-portal).
:::

:::tip
Work4You also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `work4you --tui` — see the [TUI](tui.md) guide.
:::
...
work4you chat --query-file prompt.txt
work4you chat --query-file - < prompt.txt

# With a specific model
work4you chat --model "anthropic/claude-sonnet-4"

# With a specific provider
work4you chat --provider work4you        # Use Work4You Portal
work4you chat --provider openrouter  # Force OpenRouter
...
work4you chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
work4you --continue             # Resume the most recent CLI session (-c)
work4you --resume <session_id>  # Resume a specific session by ID (-r)
work4you --resume latest        # Resume the most recent session (same as -c)
work4you --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
work4you chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
work4you -w                         # Interactive mode in worktree
work4you -w -z "Fix issue #123"     # Single query in worktree
```

### Plugin management

The `work4you plugins` commands manage native Work4You plugins and portable Agent
...
work4you plugins disable <plugin-name>
work4you plugins update <plugin-name>
work4you plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Work4You currently
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `WORK4YOU_YOLO_MODE` is on (either `work4you --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Work4You to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.work4you/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Work4You supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Work4You replies |
...
```bash
work4you -s work4you-dev,github-auth
work4you chat -s github-pr-workflow -s github-auth
```

Work4You loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.work4you/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.work4you/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Work4You..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Work4You also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Work4You — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Work4You is working, Work4You prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Work4You to the background — just like any terminal process. The shell prints a confirmation:

```
Work4You has been suspended. Run `fg` to bring Work4You back.
```
...
Resume this session with:
  work4you --resume 20260225_143052_a1b2c3
...
```bash
work4you --continue                          # Resume the most recent CLI session
work4you -c                                  # Short form
work4you -c "my project"                     # Resume a named session (latest in lineage)
work4you --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
work4you --resume "refactoring auth"         # Resume by title
work4you --resume latest                     # Resume the most recent session (same as -c)
work4you --resume latest --in ./my-project   # Latest session for ./my-project's workspace
work4you -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `work4you sessions rename <id> <title>` from the command line. Use `work4you sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Work4You's SQLite state database under `~/.work4you/state.db`. The database keeps:
...
Work4You immediately confirms the task and gives you back the prompt:
