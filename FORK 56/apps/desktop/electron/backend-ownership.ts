  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (const entry of entries) {
        ...
        // if it reaches reapOrphans (see main.ts startWork4You + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:work4you(?:\.exe)?|work4you_cli\.main|work4you_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
