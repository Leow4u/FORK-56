...
from work4you_cli import setup as setup_mod
from work4you_cli.setup import setup_model_provider
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("work4you_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("work4you_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "work4you_cli.main._model_flow_named_custom",
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    monkeypatch.setattr("work4you_cli.setup.managed_work4you_tools_enabled", lambda: True)
    monkeypatch.setenv("WORK4YOU_HOME", str(tmp_path))
    ...
    monkeypatch.setattr("work4you_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("work4you_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "work4you_cli.setup.get_work4you_subscription_features",
        lambda config: type("Features", (), {"work4you_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from work4you_cli.setup import setup_terminal_backend
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("work4you_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("work4you_cli.setup.prompt", fake_prompt)
