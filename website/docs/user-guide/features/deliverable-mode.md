...
When Work4You runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.work4you/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.work4you/config.yaml` (switchable per session
via `/personality`).
...
If you use Work4You' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.work4you/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Work4You's deliverable
