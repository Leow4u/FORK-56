 ...
 * `work4you-boot-background` / `work4you-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports WORK4YOU_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.work4you matches get_work4you_home()'s default.
const bootFilePath = () => join(process.env.WORK4YOU_HOME ?? join(homedir(), '.work4you'), 'tui-theme-boot.json')

// Never touch the user's real ~/.work4you from test runs (the TS suite has no
// WORK4YOU_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into WORK4YOU_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.WORK4YOU_TUI_THEME || env.WORK4YOU_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.WORK4YOU_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.WORK4YOU_TUI_BACKGROUND
  ) {
    env.WORK4YOU_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.WORK4YOU_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.WORK4YOU_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into WORK4YOU_TUI_THEME.
