"""Regression tests for the codex_app_server → Work4You UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_work4you_tools_mcp_server_emits_bare_tool_name(self):
        """The work4you-tools MCP server wraps Work4You' own tools for codex;
        ...
        mcp.work4you-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "work4you-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "work4you-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "work4you docs"
        assert calls[0].args[3] == {"query": "work4you docs"}
