 ...
 * server-computed `status` (keys ∧ Work4You entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `work4you tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `work4you
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the Work4You Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. Work4You Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_work4you_auth) {
        // Managed Work4You row selected without Portal entitlement: the config
        ...
        notify({
          ...
          action: { label: copy.work4youAuthSignIn, onClick: () => void signInToWork4YouPortal() }
        })
  ...
  // Drive the existing Work4You Portal OAuth device-code flow (the same session
  ...
  async function signInToWork4YouPortal() {
    try {
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isExpanded && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_work4you_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.work4youIncluded}</p>
                )}
