...
The PID file lives at ``{WORK4YOU_HOME}/gateway.pid``.  WORK4YOU_HOME defaults to
``~/.work4you`` but can be overridden via the environment variable.  This means
separate WORK4YOU_HOME directories naturally get separate PID files — a property
...
from work4you_constants import get_work4you_home, _get_platform_default_work4you_home
...
_GATEWAY_KIND = "work4you-gateway"
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_work4you_home() / "gateway-starts.log"
...
def _get_process_work4you_home() -> Path:
    """Return the process-level WORK4YOU_HOME, skipping context-local overrides.
    ...
    ``get_work4you_home()`` honors ``_WORK4YOU_HOME_OVERRIDE`` contextvar used for
...
def _canonical_work4you_home(path: Path | str) -> Path:
    """Return a stable absolute WORK4YOU_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_work4you_home(left: Path | str, right: Path | str) -> bool:
    """Compare WORK4YOU_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_work4you_home(left))) == os.path.normcase(
        str(_canonical_work4you_home(right))
    )


# Mirrors work4you_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (work4you_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a WORK4YOU_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.work4you/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ``"default"`` for the deployment's root home, and ``None`` when no label
    can be inferred.  Never raises — this feeds diagnostics only.
    """
    try:
        canonical = _canonical_work4you_home(home)
    except Exception:
        return None
    ...
    try:
        ...
        if _same_work4you_home(canonical, get_default_work4you_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_work4you_home(canonical, _get_platform_default_work4you_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    WORK4YOU_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``work4you_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting WORK4YOU_HOME."""
    home = _get_process_work4you_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("WORK4YOU_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "work4you" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Work4You' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Work4You gateway lifecycle subcommand from a command line.
    ...
    (``"C:\\Program Files\\...\\work4you-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Work4You's
    ...
    for token in tokens:
        ...
        if basename in ("work4you-gateway", "work4you-gateway.exe"):
            return "run"
    ...
    has_gateway_entry = (
        ...
        or any(t.rsplit("/", 1)[-1] in ("work4you", "work4you.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `work4you gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Work4You-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Work4You gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a WORK4YOU_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.work4you`` or ``$WORK4YOU_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``WORK4YOU_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a WORK4YOU_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"work4you_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit WORK4YOU_HOME= on the argv. WORK4YOU_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "work4you_home=" in command_lc and f"work4you_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # WORK4YOU_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "work4you_home": str(_canonical_work4you_home(_get_process_work4you_home())),
    }
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `work4you status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``WORK4YOU_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``work4you_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    WORK4YOU_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different WORK4YOU_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from work4you_home.
    profile = _profile_label_for_home(_get_process_work4you_home())
    if profile:
...
# enabled in the user's systemd (e.g. ``work4you.service`` + ``work4you-
# gateway.service``).
#
...
def _get_takeover_marker_path(work4you_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``work4you_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = work4you_home or _get_process_work4you_home()
    return _canonical_work4you_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-WORK4YOU_HOME --replace while
    ...
    our_home = _get_process_work4you_home()
    target_home = record.get("target_work4you_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_work4you_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_work4you_home")
        if replacer_home is not None and not _same_work4you_home(
    ...
    # ``work4you gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's WORK4YOU_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_work4you_home(
            target_home or _get_process_work4you_home()
        )
        ...
        record = {
            ...
            "target_work4you_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_work4you_home": str(
                _canonical_work4you_home(_get_process_work4you_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target WORK4YOU_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("work4you_home")
    if not isinstance(pid_record_home, str) or not _same_work4you_home(
