...
## 2026-07-13: Scope plugin manager state by Work4You home/profile (keyed cache)

Status: Accepted

Context:
Work4You supports multiple profiles via different Work4You home directories.
Homes are switched two ways in a running process: the `WORK4YOU_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_work4you_home_override()` (`work4you_constants.py`), which
...
`get_work4you_home() / "plugins"`, and context-engine plugins (e.g.
`work4you-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_work4you_home_override()` was invisible to a
   naive "did `WORK4YOU_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `work4you_plugins.<slug>` by
   ...
   `work4you_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Work4You home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_work4you_home()`
  (which itself already consults `get_work4you_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `WORK4YOU_HOME` or `set_work4you_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_work4you_home_override()`) rather than only the env-var path, and
