...
Uses a throwaway WORK4YOU_HOME so it never touches ~/.work4you. No real names.
"""
...
def _fresh_home():
    """Point WORK4YOU_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_work4you_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["WORK4YOU_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER WORK4YOU_HOME is set.
...
# a different get_work4you_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"WORK4YOU_HOME (throwaway): {HOME}")
