...
#88251: ``work4you update`` treated a failed desktop pack as non-fatal, printed
...
from work4you_cli.update_cmd import (
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "work4you_constants.with_work4you_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "work4you_constants.display_work4you_home", lambda: str(tmp_path), raising=False
    )
...
def test_summary_omits_success_banner_when_desktop_rebuild_failed(capsys):
    ...
    assert "work4you desktop" in out
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_work4you_home", lambda: tmp_path)
