...
Ledger path resolution (first wins):
  --ledger PATH
  $WORK4YOU_CITATION_LEDGER
  $WORK4YOU_HOME/cache/citations/ledger.json
"""
...
from _work4you_home import get_work4you_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("WORK4YOU_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_work4you_home() / "cache" / "citations" / "ledger.json"
