 ...
 *   1. WORK4YOU_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\work4you\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect WORK4YOU_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.WORK4YOU_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'work4you', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'work4you', 'git', 'usr', 'bin', 'bash.exe'))
  }
