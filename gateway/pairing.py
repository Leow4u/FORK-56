...
PAIRING_DIR = get_work4you_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``work4you_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{WORK4YOU_HOME}/pairing`` while newer code/docs may
    write ``{WORK4YOU_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own WORK4YOU_HOME using the same legacy/consolidated layout rules
    as ``work4you -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current WORK4YOU_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp WORK4YOU_HOME
        ...
        if profile:
            ...
            # ``work4you -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `work4you` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> work4you pairing approve ...` as root, "
                    "re-run with `docker exec -u work4you <container> ...` and "
                    "chown the existing file to the work4you user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``work4you
