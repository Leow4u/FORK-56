...
def cron_store(tmp_path, monkeypatch):
    ...
    (work4you_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("WORK4YOU_HOME", str(work4you_home))
    monkeypatch.setattr(J, "WORK4YOU_DIR", work4you_home)
    monkeypatch.setattr(J, "CRON_DIR", work4you_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", work4you_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", work4you_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_work4you_now", lambda: now):
            due = J.get_due_jobs()
