 ...
 * / firecrawl-gateway / fal-queue-gateway.
 *
 * Tool calls from the agent send the Portal access token as the OpenAI
...
export function hasGatewayScope(scopes: string[]): boolean {
  return (
    scopes.includes(TOOL_INVOKE_SCOPE) || scopes.includes(INFERENCE_INVOKE_SCOPE)
  )
}

/**
 * Paid Portal access always covers OpenAI audio. Free tool-pool coverage is
 * the JWT `tool_access.coverage["openai-audio"]` flag (same map as
 ...
 */
export function isOpenAIAudioCovered(
  payload: JWTPayload,
  paidAccess: boolean | null,
): boolean {
  ...
  if (rec.enabled === true) {
    ...
    if (coverage && typeof coverage === 'object') {
      const audio = (coverage as Record<string, unknown>)['openai-audio']
...
export async function verifyInvokeBearer(
  ...
  if (!isOpenAIAudioCovered(payload, paidAccess)) {
    throw Object.assign(new Error('openai_audio_not_covered'), { status: 403 })
  }
