"""Tests for the Linux XDG desktop entry installed by ``work4you desktop``."""
...
def test_install_writes_entry_with_absolute_exec_and_icon(tmp_path, xdg_home, monkeypatch):
    ...
    work4you_bin = tmp_path / "bin" / "work4you"
    work4you_bin.parent.mkdir()
    work4you_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "work4you_cli.relaunch.resolve_work4you_bin", lambda: str(work4you_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)

    assert entry == xdg_home / "applications" / "work4you.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `work4you` would not resolve.
    assert values["Exec"] == f"{work4you_bin} desktop"
    ...
    assert values["Name"] == "Work4You"
    assert values["Terminal"] == "false"


def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("work4you_cli.relaunch.resolve_work4you_bin", lambda: "/usr/bin/work4you")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("work4you_cli.relaunch.resolve_work4you_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m work4you_cli.main desktop")
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Work4You is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "work4you"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "work4you"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("work4you_cli.relaunch.resolve_work4you_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
